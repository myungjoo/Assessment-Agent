...
> **같은 `@Cron` claim 은 문서 안 6 지점 중복** — **76** (다이어그램 label) · **126** (표 `Scheduler` row) · **143** · **175** (각주 2) · **234 · 235** (`## Contracts` 표 2 row) 다. `## Contracts` 표 자체의 참 / 거짓 판정은 본 각주 소관이 아니다 (파생 영향 (3)) — 판정 근거 전문은 [REQ-COVERAGE-AUDIT § 12.54](../use-cases/REQ-COVERAGE-AUDIT.md).
