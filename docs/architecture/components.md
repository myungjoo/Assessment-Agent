...
> **`--> worker` (73 행) 도 거짓** — backend_api 디렉토리군 → worker 호출은 **0** (`person.service.ts` **101** 행은 주석 1 구) 이고 실 REST 진입은 worker 자신의 controller 와 `src/scheduling/` runner 이며 역방향 import 가 오히려 **22** 지점이다. 반면 label `in-process method call` 은 **111** 행 표기 규약과 [ADR-0003](../decisions/ADR-0003-deployment.md) **34** 행 `Decision §1` 을 **승계 참** 이다 — 결선 실재와 별개 축이며, 대응 `## Contracts` row **5** 개 (**252 ~ 256** 행) 의 판정은 파생 영향 (3) 소관이다. 판정 근거 전문은 [REQ-COVERAGE-AUDIT § 12.55](../use-cases/REQ-COVERAGE-AUDIT.md).
...
> 대응 `## Contracts` row **4** 개 (**259 ~ 262** 행) 는 edge 4 개와 1:1 대응하나 그 판정은 파생 영향 (3) 소관이다 — 판정 근거 전문은 [REQ-COVERAGE-AUDIT § 12.56](../use-cases/REQ-COVERAGE-AUDIT.md).
...
> **88 행 주석의 `ADR-0003 §4` pointer 는 참** (결선 실재와 별개 축) — [ADR-0003-deployment.md](../decisions/ADR-0003-deployment.md) **78** 행 `### Decision §4 — 외부 네트워크 boundary = direct outbound from app process` 로 drift **0** 이다. 대응 `## Contracts` row **2** 개 (**264 · 265** 행) 는 github edge 3 을 1 row 로 **3 : 1 축약** 하나 그 판정은 파생 영향 (3) 소관이다 — 판정 근거 전문은 [REQ-COVERAGE-AUDIT § 12.57](../use-cases/REQ-COVERAGE-AUDIT.md).
