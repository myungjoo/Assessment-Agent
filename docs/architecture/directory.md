...
> **`용도` 컬럼 6 서술 ↔ 실 파일 책임 대조 (T-1434 실측 각주)** — 위 각주가 별도 slice 로 위임한 **서술 내용 축** 의 실측이다. `용도` 컬럼 claim 은 **검증 가능 12** 와 **검증 불가 3** (`domain-cohesion 유지` 같은 형태 무관 범주 / 설계 의도) 으로 갈리며, 전자의 판정은 **참 7 · 부분참 3 · 거짓 2** 다. **거짓 2** — `repositories/` 가 예시한 `UserRepository.findActiveByGroupId(...)` 는 `grep -rn "findActiveByGroupId" src/` **0** 으로 repo 전역 부재 (실 surface = `create` · `findByEmail` · `findById` · `updateRole` · `countAll` · `findAll` **6**) 이고, `entities/` 의 "re-export wrapper" 는 위 각주의 `*.entity.ts` **0** 판정을 그대로 승계한다.
> **부분참 3** — `providers/` 의 "5 LLM provider (custom / Azure OpenAI / Anthropic / Google Gemini / OpenAI)" 는 실 `ls src/llm/providers/*.adapter.ts` **4** 로, custom 과 OpenAI 가 `openai-compatible.adapter.ts` 1 파일에 통합됐다 (해당 파일 1 · 13 · 114 행 주석이 "custom/openai 두 provider 가 본 wire 포맷 공유" 로 근거를 박제 — **5 번째 adapter 파일 신설은 중복**). `adapters/` 의 "3 instance (`com` / `sec` / `ecode`)" 는 라우팅 자체는 shipped (`grep -rn "ecode" src/github/` **31**, 단일 `github-adapter.service.ts` + `github-instance-config.ts` sub-config) 이나 key `com` 은 `grep -rn '"com"' src/github/` **0** 이고 실 예시 key 는 `public` 이며, 표가 인용한 [components.md](components.md) 문구도 실제로는 "GitHub Adapter **3-instance** 묶음 결정" 이다. **참 7** 은 `dto/` 의 class-validator (`src/*/dto/*.ts` **82** 중 **64** 가 import) · `guards/` 의 `RolesGuard` 와 3 권한 (`roles.guard.ts` 의 `SuperAdmin ⊇ Admin ⊇ User`) · `repositories/` 의 "Prisma client wrapping" (`UserRepository` 가 `PrismaService.user` delegate 에 1:1 forwarding) · 외부 pointer `REQ-038` · `REQ-044` · [ADR-0002](../decisions/ADR-0002-db.md) 의 대상 실재 등이다.
> 본 각주도 **사실 기록** 이지 표 재작성이 아니다 — 위 59 ~ 64 행 공통 4 항목 산문의 "module 당 1 개" 전제 역시 실측 `.module.ts` **14** vs `.controller.ts` **19** · `.service.ts` **51** · `.service.spec.ts` **51** 로 배수가 어긋나지만 같은 시점 보존 사유로 **무편집** 이고, 같은 "5 provider" claim 의 두 번째 사본 (아래 `## 9 module 별 디렉토리 mapping` 표 LlmModule row 의 파일명 5 열거) 은 mapping 표 소관이라 본 slice 가 닫지 않았다 (판정 근거는 [REQ-COVERAGE-AUDIT § 12.32](../use-cases/REQ-COVERAGE-AUDIT.md)).
...
> **`표준 sub-dir` · `비고` 두 컬럼 ↔ 실 파일 대조 (T-1435 실측 각주)** — 위 각주가 별도 slice 로 위임한 **컬럼 내용 축** 이다. ② 경로 미실재 2 row (`AssessmentModule` · `SchedulerModule`) 를 대상외로 뺀 **7 row 의 검증 가능 claim 28** 은 **참 14 · 부분참 5 · 거짓 8 · 유보 1** 로 갈린다. `표준 sub-dir` 축 1 회 실측 (`ls -d src/*/dto/ src/*/guards/ src/*/entities/ src/*/repositories/ src/*/adapters/ src/*/providers/`) 은 `dto/` **8** · `providers/` **1** · 나머지 4 종 **0** 이라 — 표가 `GithubModule` · `ConfluenceModule` 에 준 `dto/` 는 **두 module 모두 부재 (거짓)**, `guards/` · `repositories/` · `adapters/` 는 책임만 flat 파일로 실현된 **부분참**, `entities/` 만 책임 자체가 미shipped 인 **거짓** 이다 (뒤 두 판정은 77 ~ 79 · 81 ~ 82 행 각주 승계).
> **`비고` 파일명 축의 새 사실** — 이름까지 지정한 4 파일 중 **실재 0** 이다: `github.adapter.ts` · `confluence.adapter.ts` 의 실체는 flat `{github,confluence}-adapter.service.ts` 이고, `llm.service.ts` 는 부재 (`modelId` 를 다루는 실 파일은 `src/llm/difficulty-mapping.service.ts` · `llm-gateway.interface.ts`). LlmModule row 의 5 provider 파일명 열거도 **suffix 규약** 이 실 `*.adapter.ts` 와 달라 (`ls src/llm/providers/*.provider.ts` → `No such file`) 독자가 없는 경로를 열게 되지만, **개수 축 (5 vs 4) 판정은 위 82 행 각주 (T-1434) 가 이미 박제** 했으므로 여기서 반복하지 않는다. `WebModule` 의 `(controller only)` 도 실 `src/web/` = `web.module.ts` **1** · controller **0** 이라 거짓이다.
> 본 각주도 **사실 기록** 이지 표 재작성이 아니다 — `@Global()` 적용 (`persistence.module.ts` 12 행) · `auth.service.ts` · `RolesGuard` · `dto/` (auth · user · llm) · `providers/` · Persistence 특수 서술 · UserModule 의 "controller endpoint 노출" (`src/user/*.controller.ts` **7**) · Github · Confluence 의 "controller 미노출" 은 **실측 참** 이라 그대로 두고, 대상외 `SchedulerModule` 의 `@nestjs/schedule` claim 은 개명체 `src/scheduling/` 에서 성립 (grep **5**) 이며 `WebModule` 의 serve-static · SPA fallback 서술은 본 slice 실측 예산 밖이라 **유보** 다 (판정 근거는 [REQ-COVERAGE-AUDIT § 12.33](../use-cases/REQ-COVERAGE-AUDIT.md)).
