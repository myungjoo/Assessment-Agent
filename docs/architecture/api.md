...
**합계**: 약 50 endpoint / 13 resource prefix / 8 UC cover (T-0117/T-0118/T-0119 박제로 `/api/assessments` CRUD 정정 + `/api/contributions` 4 + `/api/summaries` 4 추가, prefix 9 → 11; T-0214 박제로 `/api/permission-denied-records` audit 조회 endpoint 1 추가, prefix 11 → 12; T-0237/T-0238 박제로 `/api/users/{id}/instance-access` grant(POST)/revoke(DELETE) WRITE endpoint 2 추가, prefix 12 → 13 (ADR-0027 grant chain); T-0315~T-0318 박제로 `/api/assessment-evaluation/period` endpoint 1 추가 후 T-0320~T-0323 박제로 Admin full-persist 분기 shipped (ADR-0037 ACCEPTED — 같은 endpoint 가 role 로 User self-only ephemeral / Admin full-persist 2 경로 dispatch, §Decision1·2·3·4 모두 shipped; overwrite/재평가만 DEFERRED); `/api/assessments` 의 batch 4 건 [`/run`·bulk `DELETE`·`/reeval`·`/reset`] 은 P5 evaluation pipeline 의존 미구현 deferred). 향후 UC 추가·세분화 시 본 표가 source — endpoint 신설은 본 표 갱신 PR 의 reviewer 점검 대상.
