...
| POST | `/api/assessment-evaluation/period` | [UC-01](../use-cases/UC-01-evaluation-execution.md) | 인증 User 가 **자기 자신**의 임의 기간 평가문을 요청하는 ephemeral manual trigger (REQ-009, REQ-040, REQ-045 — README R-9 User 경로). request body `PeriodBridgeDto` { `personId`(string, `@IsNotEmpty`), `period`(string, `@IsNotEmpty` — day/week/month literal 검증은 service 책임), `scope`(string, `@IsNotEmpty` — commit/document/aggregate literal 검증은 service 책임), `periodStart`(ISO-8601 string, `@IsISO8601` — 비-ISO 400), `mode?`(선택, `@IsIn(["fill","reeval"])` — 단 ephemeral 경로는 persist 미수행이라 본 endpoint 에서 무시) }. **self-only(fail-closed)**: `@CurrentUser("sub")` principal userId 가 `dto.personId` 와 **일치할 때만** 진행 — principal sub 부재(undefined/null)거나 personId 불일치면 `ForbiddenException` 403 (타인 평가문 요청 차단, generateEphemeral 위임 미수행). 처리 흐름: `PersonService.findByIdWithIdentities(personId)`(row 부재 시 404 전파) → resolved person 의 `serviceIdentities` 만 조립 → `PeriodBridgeEphemeralService.generateEphemeral(person, { since: periodStart }, { modelId: undefined })` 위임(**fresh in-memory collect → evaluate**, ADR-0037 §Decision4 fresh-collect source-of). response **200 `EvaluationResult[]`**(가공 0 그대로 forward, 각 원소 `{ unitId, narrative, difficulty, contribution, volume }` — ADR-0032 §3) — **persist 호출 0 / DB write 0**(ephemeral, ADR-0037 §Decision1·§Decision5 schema0). since 는 `periodStart` pass-through(도출 0 — Admin/collection 책임). modelId 는 본 slice 미지정. error: 400 (DTO 위반 — whitelist + forbidNonWhitelisted + transform ValidationPipe) / 401 (cookie 부재·토큰 invalid) / 403 (self-only 불일치) / 404 (Person row 부재 P2025 → NotFoundException). **T-0315(DTO)~T-0318(e2e) 박제 ([ADR-0037](../decisions/ADR-0037-period-collection-evaluate-bridge.md) §Decision1 User self-only ephemeral / §Decision4 fresh collect / §Decision5 dep0·credential0·schema0) — RBAC enforced (User self-only via JwtAuthGuard+RolesGuard, @Roles("User") User+ escalation).** Admin full-persist 경로(§Decision2 double-write / §Decision3 idempotency)는 PROPOSE 상태(사용자 ADR PR 검토 대기)라 미shipped. | User (self-only) |
