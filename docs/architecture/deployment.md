...
> **`## DB / Persistence` 전반부 (도입 2 문단 · `### 배포 토폴로지` · `### Migration 정책`) ↔ 실 `docker-compose.yml` · `prisma/` · `src/persistence/` · `.github/workflows/ci.yml` 대조 (T-1442 실측 각주)** — 19 행의 **채택: PostgreSQL + Prisma** 는 참이다 (compose 14 행 `image: postgres:16-alpine` · `prisma/schema.prisma` 실재). 23 행의 image tag 표기도 실 compose 와 1:1 이나 "동일 host 의 다른 process **또는** 로컬 Docker container" 라는 양자 서술은 **부분참** 이다 — 실 경로는 `docker-compose.yml` 1 ~ 4 행이 선언한 **compose 단일 통합 경로** (`postgres` + `app` 2 service · `container_name: assessment-agent-postgres` · `ports: "5432:5432"`) 로 확정됐고, host process 직접 기동을 지원하는 자산은 repo 에 없다.
> **24 행이 예시로 든 service 이름 `db:5432` 는 실제와 다르다** — `services:` 하위 key 는 **`postgres`** (13 행) · `app` (36 행) 뿐이라, 예시임을 감안해도 독자가 그대로 접속 문자열을 만들면 이름 해석에 실패하는 **오도 risk** 가 남는다 (실 내부 경로는 `postgres:5432`). 반면 `DATABASE_URL` 표준 명칭은 **참** 으로 실재하고 (`.env.example` 12 행 · `.github/workflows/ci.yml` 66 ~ 70 행), 같은 행의 "구체 변수 이름은 T-0015 의 secret 단락이 결정" 은 이미 이행돼 **시점 서술이 낡았다**.
> **25 행의 PrismaService singleton 은 참이나 pool 결정은 미이행이다** — 실체는 `src/persistence/prisma.service.ts` 29 행 `export class PrismaService extends PrismaClient implements OnModuleInit` 로 문서가 밝히지 않은 `src/persistence/` 에 있고, `connection_limit` · `pool_timeout` · `statement_timeout` · `poolSize` 는 `src` · `prisma` · `docker-compose.yml` · `.env.example` 전체에서 **0 hit** 이라 "Pool 크기와 statement timeout 의 구체 값은 P3 Persistence layer task 에서 결정" 은 **시점 낡음** (현 phase `P4-complete / P5-in-progress`) 과 **미결정 잔존** 두 축이 겹친다 — 실 pool 은 `@prisma/adapter-pg` 의 default 로 남아 있다.
> **26 행 worker 조건절은 조건 자체가 미성립** — [ADR-0003](../decisions/ADR-0003-deployment.md) 은 32 행 `### Decision §1 — Monolithic NestJS process (in-process queue OK)` 를 채택해 worker 분리를 결정한 적이 없다 (거짓은 아니나 전건 불성립). 30 ~ 31 행 중 `prisma migrate deploy` 는 **참** (`.github/workflows/ci.yml` 216 행 `run: pnpm prisma migrate deploy` · `deploy/daily-test-step-deps-schema.test.sh` 112 행) 이나 **`prisma migrate dev` 는 `package.json` · CI · `Dockerfile` · `deploy/` 에서 0 hit** 이라 개발 환경 명령은 script 로 등재되지 않은 정책 서술이며, `prisma/migrations/` 누적 **14 개** 가 전부 git tracked 인 것은 참이다.
> **32 행 "CI 통합은 P3 phase 의 task 에서 … 도입. 본 task 는 정책만 박제" 는 낡았다** — `ci.yml` 209 행에 `- name: Prisma migrate deploy` step 이 이미 있어 미래형 서술이 지난 시점을 가리키며, `§ 12.15` append-only 방침상 시점 marker 는 in-place 갱신 대신 본 각주로 사실만 병기했다. 23 행이 인용한 **[CLAUDE.md](../../CLAUDE.md) §1 은 "기술 스택 (확정)"** 이고 `single-operator` 는 CLAUDE.md 전체에서 **0 hit** 이라 그 pointer 는 부정확하다 (CLAUDE.md 는 무편집 — 판정 근거는 [REQ-COVERAGE-AUDIT § 12.40](../use-cases/REQ-COVERAGE-AUDIT.md)).
...
> **`## DB / Persistence` 후반부 (`### Backup / restore 전략` · `### Raw data 저장 금지 (REQ-032)` · `### 후속 진행`) ↔ 실 `prisma/schema.prisma` · `deploy/README.md` · `docs/ops/runbook.md` · `.claude/agents/reviewer.md` 대조 (T-1443 실측 각주)** — 41 행의 `pg_dump` / `pg_restore` backup 은 **참** 이다 (`deploy/README.md` 227 행 · `docs/ops/runbook.md` 94 · 99 행에 절차 실재). 같은 행의 **"README 57 행 (export / backup / restore)" pointer 도 유효** 하다 (README 57 행 = "평가 자료가 저장된 공간은 쉽게 export하여 backup하고 restore하여 reset할 수도 있어야 한다"). 다만 runbook 의 실 복원 명령은 plain SQL dump + `psql` 이라 `pg_restore` 자체는 명령으로 등재돼 있지 않고, README 57 행 요구의 주 충족 경로도 지금은 DB-level dump 가 아니라 **app-level export / import job** (`src/export/` · `src/import/` — `docs/PLAN.md` `## Phase P7` 의 R-57 이 `implemented-on-main`) 이다.
> **42 행의 "자동 backup" 은 여전히 미도입이나 "P7 phase 의 task" 라는 미래형은 낡았다** — 자동화 자산은 `.github/workflows/ci.yml` · `deploy/` · `scripts/` · `src/scheduling/` 어디에도 없고 (`deploy/README.md` 227 행의 수동 1 줄이 전부), 반면 P7 은 이미 부분 진입해 R-72 · R-73 · R-74 · R-50 · R-57 이 `implemented-on-main` 이다 (현 phase 표기 `P4-complete / P5-in-progress`). 두 축은 분리해 읽어야 한다 — **미이행** 은 자동화 쪽, **시점 낡음** 은 phase 배정 쪽이다.
> **47 행의 raw 본문 column 금지는 전수 실측 결과 참이다** — `prisma/schema.prisma` 는 `model` **15** 개 · `String` 출현 **74** 회인데 `body` · `content` · `message` · `diff` · `patch` · `rawText` 계열 column 은 **0** 이다 (grep hit 은 전부 주석이거나 `difficulty` 오탐). `String` 인 `narrative` 2 개 (`Assessment` · `Summary`) 는 schema 주석이 밝히듯 **LLM 이 생성한 정성 평가문** 이라 raw 원본이 아니고, `ExportJob` · `ImportJob` 도 본문 대신 `artifactRef` 참조만 보유한다. 인용된 [ADR-0002](../decisions/ADR-0002-db.md) `Decision §2` pointer 도 유효하다 (`## Decision` 41 행의 근거 **2** 번 항목 = "REQ-032 schema-level 강제").
> **48 행의 reviewer 규약은 미이행이다** — `.claude/agents/*.md` 전체에서 `REQ-032` · `raw data` · `raw 본문` 이 **0 hit** 이고 `.claude/agents/reviewer.md` 의 `8 check 구체 sub-check` 에 schema / `String` column 항목이 없다. 원문은 "…확인 — … REQUEST_CHANGES" 라는 **현재형 서술** 이라 독자가 이를 **현행 자동 게이트로 오인** 할 수 있다 — 실제로는 [ADR-0002](../decisions/ADR-0002-db.md) 가 "grep 가능" 이라 적은 가능성 서술이 아직 체크리스트로 내려오지 않은 상태이며, 체크리스트 보강은 `.claude/` 소관의 별도 task 다 (본 각주는 agent 정의 무편집).
> **49 · 53 행의 "P3 에서 진행" 은 전부 이행돼 낡았다** — column 설계 · 인덱스 · unique constraint 는 `@@index` **14** · `@@unique` **19** 로 실재하고, `prisma` / `@prisma/client` 는 `package.json` 에 `^7.8.0` 으로 install 됐으며, PrismaService NestJS module 도 `src/persistence/prisma.service.ts` · `src/persistence/persistence.module.ts` 로 존재하고 `prisma/migrations/` 는 **14** 개다. 따라서 53 행의 "코드 변경은 0 LOC" 는 본 단락을 채운 `T-0014` 시점의 기록으로만 유효하다.
> `§ 12.15` append-only 방침상 위 시점 서술 (42 · 49 · 53 행) 은 새 phase 배정을 창작하지 않고 원문 보존 + 본 각주 병기로 처리했고, 48 행 규약 역시 `.claude/agents/` 편집이 Out of Scope 라 사실 기록에 머문다 — 전 판정 근거는 [REQ-COVERAGE-AUDIT § 12.41](../use-cases/REQ-COVERAGE-AUDIT.md).
