...
| **AssessmentEvaluationModule** | **P5 평가(evaluation/scoring) backbone** — 이미 수집된 `Activity[]` 를 입력으로 받아 평가 결과를 산출하는 shipped layer (T-0291 신설 / T-0293 controller, [ADR-0032](../decisions/ADR-0032-p5-evaluation-contract.md)). `EvaluationOrchestratorService`(T-0292) 가 상위 compose — `Activity[]` → 매퍼 → 평가-side dedup → `scoreUnit` → `EvaluationResult[]`; `EvaluationScoringService` 가 단위 평가 — prompt 조립 → `LlmGateway.generate`(`LLM_GATEWAY` token 주입) → classify + volume 산출. controller `AssessmentEvaluationController`(POST `/api/assessment-evaluation/evaluate`, Admin RBAC — manual trigger HTTP 진입점, T-0293) 는 검증된 DTO 를 orchestrator 에 thin-delegate (가공 0, service-layer error raw 전파). `LLM_GATEWAY` → `LlmHttpGateway` **useExisting** 바인딩으로 LlmModule 이 export 한 gateway singleton 을 token 의존으로 재사용 (test 에서 mock gateway 주입 용이, 중복 인스턴스 0). 두 service 모두 export — 후속 bridge / 집계 slice 가 inject. **AppModule 등록 완료** (app.module.ts imports). 결과 영속화 / Prisma migration / period·personId→수집 bridge 는 본 module 밖 (§5 schema 게이트 deferred). | LlmModule (단방향 — 평가 → llm, llm 은 평가를 모름), AuthModule (controller guard) | Backend API (평가 manual trigger), Worker (평가 부분) | REQ-038 (평가 조회 backbone), REQ-040 (manual trigger), REQ-049 (LLM 사용) | [ADR-0032](../decisions/ADR-0032-p5-evaluation-contract.md) (P5 평가 계약), ADR-0003 §1 (monolithic) |
