...
| **Backend API** | AssessmentModule + UserModule + AuthModule + PermissionDeniedRecordModule + AssessmentCollectionModule | 1:N. 단일 component 가 5 module 로 분할 — controller 책임이 domain (assessment / user / auth / permission-denied audit / collection manual-trigger) 별로 분산되기 때문. PermissionDeniedRecordModule 은 audit 조회 부분 (`GET /api/permission-denied-records`) 을, AssessmentCollectionModule 은 수집 manual-trigger (`POST /api/assessment-collection/collect`, [ADR-0031](../decisions/ADR-0031-collection-manual-trigger.md), T-0274) 를 담당. 각 module 의 controller 가 자신의 endpoint 를 노출하며, AppModule 이 모두 묶어 단일 HTTP server 로 동작. |
...
총 8 component → 10 module 의 N:N mapping. Backend API component 의 1:5 분할 (assessment / user / auth / permission-denied audit / collection manual-trigger), Worker component 의 1:2 분할 (수집 AssessmentCollectionModule / 평가 AssessmentModule, ADR-0029 §1), PermissionDeniedRecordModule 이 Backend API (audit) + DB Persistence (영속화) 에 걸치고 AssessmentCollectionModule 이 Backend API (manual trigger, ADR-0031) + Worker (수집) 에 걸치는 N:N 이 분기점이며, 나머지 component 는 1:1 mapping.
...
Refs: T-0014, T-0015, T-0016, T-0017, T-0217, T-0251, T-0255, T-0266, T-0269, T-0276, ADR-0001, ADR-0002, ADR-0003, ADR-0022, ADR-0029, ADR-0030, ADR-0031, REQ-005, REQ-006, REQ-007, REQ-008, REQ-015, REQ-026, REQ-031, REQ-032, REQ-033, REQ-038, REQ-039, REQ-040, REQ-044, REQ-049, REQ-051, REQ-052, REQ-053, REQ-054, REQ-055
