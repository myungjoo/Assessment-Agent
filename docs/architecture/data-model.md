...
| **LlmProviderConfig** | 5 provider (custom / Azure OpenAI / Anthropic / Google Gemini / OpenAI) 별 설정 1 row — endpoint URL / API key (encrypted at rest, 별도 ADR) / model 식별자. **다중 row 모델** (각 provider 별 1+ row, custom 은 3 model 슬롯 모두 차지 가능 — REQ-051). **전역 기본 provider 는 본 entity 의 컬럼이 아니라 단일 슬롯 table `LlmDefaultProvider` (고정 id 1 row → `llmProviderConfigId` FK `onDelete: Restrict`) 로 표현** — Admin 의 Web UI 명시 선택이 seed env · `updatedAt` · row 수 등 모든 자동 규칙보다 우선하며, 슬롯 부재 시에만 "row 1 개" fallback 이 도달 가능하다 ([ADR-0062](../decisions/ADR-0062-llm-default-provider-explicit-selection.md) § Decision 2·4 — **PROPOSED**, schema 실체화는 T-1863). | [UC-05](../use-cases/UC-05-llm-config.md) | REQ-049, REQ-051, REQ-052, REQ-053, REQ-054, REQ-055 | LlmModule |
...
> **코드 only model 각주 ([T-1427](../tasks/T-1427-data-model-entity-vs-prisma-model-audit.md) 문서 ↔ 코드 3 축 대조 실측)** — `grep -c '^model ' prisma/schema.prisma` = **15** 이나 위 표의 실체 row 는 **14** (`sed -n '22,36p' … | grep -c '^| \*\*'`) 로, 표에 row 가 없는 **코드 only model 이 1 개** 있다: **`UserInstanceAccess`** (`prisma/schema.prisma` **234** 행 — `(userId, instanceRef)` 식별자 binding 만 보유, `User` 와 N:1 `onDelete: Cascade`, token / 자격증명 컬럼 미정의). 소관은 [modules.md](modules.md) **47 ~ 48** 행 각주의 `UserInstanceAccessModule` (`src/user-instance-access/`, T-0238 — `AppModule` 비등록이고 `permission-denied-record.module.ts` **37** 행 명시 import 로만 DI 그래프 진입) 이라 **정본 표 미기재 module** 에 걸려 있다. 반대 방향 (문서 실체 row ∧ 코드 model 부재) 은 **0** 이다.
> 본 각주는 **사실 기록** 이지 표 row 도 카운트 대상도 아니다 — **18** · **38** 행의 `14 entity` 는 위 표 실체 row 14 만 세며 본 각주의 1 개를 **포함하지 않는다** ([modules.md](modules.md) 48 행이 못박은 계상 경계를 그대로 승계). row 신설은 entity 집합의 선언적 변경이라 ADR 선행 대상이고 § 3 mermaid ER diagram · § 6 REQ → entity coverage 의 동시 갱신을 요구해 **별도 slice 소관** 이다 ([REQ-COVERAGE-AUDIT § 12.25](../use-cases/REQ-COVERAGE-AUDIT.md)).

**합계**: 14 entity (+ 1 conceptual mention) / 4 module (UserModule / AuthModule / AssessmentModule / LlmModule) / 9 UC cover ([T-1418](../tasks/T-1418-data-model-uc09-entity-derivation-judgment.md) 판정으로 [UC-09](../use-cases/UC-09-user-defined-period-evaluation.md) 가 Assessment row 에 병기 — 신규 entity 0 이라 entity / module 수치는 불변, 근거 [REQ-COVERAGE-AUDIT.md](../use-cases/REQ-COVERAGE-AUDIT.md) § 12.16). 본 합계는 [T-0039](../tasks/T-0039-group-part-entity-and-repository.md) (mergeCommit c25a5de) 가 PersonGroupMembership join entity 를 schema-level 박제한 결과 10 → 11 로, 그리고 [ADR-0044](../decisions/ADR-0044-export-import-job-persistence.md) (T-0484, Q-0040 옵션1 승인) 가 ExportJob/ImportJob 영속 entity 를 박제한 결과 11 → 13 으로 shift. ExportJob/ImportJob 의 구체 Prisma schema 코드·migration 은 후속 task (§7 / ADR-0044 §Out of scope). 그 뒤 [T-1426](../tasks/T-1426-data-model-count-and-module-vocab-resync.md) 실측 (`sed -n '22,36p' … | grep -c '^| \*\*'` = 14) 이 § 2 표 실체 row 를 **14** 로 확인해 앞머리 tally 를 13 → 14 로 정정했다 — 위 이력 chain (10 → 11 → 13) 에 등장하지 않는 유일한 row 인 PermissionDeniedRecord 가 tally 미반영으로 누적된 1 어긋남의 해소이며, § 2 표 row 자체는 무편집이다 (근거 [REQ-COVERAGE-AUDIT.md](../use-cases/REQ-COVERAGE-AUDIT.md) § 12.24). 향후 entity 추가는 본 표 갱신 PR 의 reviewer 점검 대상. **(2026-08-03 [T-1427](../tasks/T-1427-data-model-entity-vs-prisma-model-audit.md) 실측 — 위 문장의 `ExportJob/ImportJob 의 구체 Prisma schema 코드·migration 은 후속 task` 는 T-0484 시점의 기록이며, 현재는 `model ExportJob` (`prisma/schema.prisma` **614** 행) · `model ImportJob` (**649** 행) 과 migration `20260618000000_export_import_job` 이 shipped 라 그 전제가 더는 성립하지 않는다. 같은 실측이 코드 only model **1** 개 (`UserInstanceAccess`) 를 확인했으나 채택안이 표 직후 각주라 tally `14` 는 **불변** — 위 각주 참조 · 근거 [REQ-COVERAGE-AUDIT.md](../use-cases/REQ-COVERAGE-AUDIT.md) § 12.25.)**
...
erDiagram
    ...
    LlmProviderConfig ||--o| LlmDefaultProvider : "1:0..1 (전역 기본 provider 슬롯 — ADR-0062 PROPOSED, schema 미실체)"
...
12. **LlmProviderConfig ↔ LlmDefaultProvider (1:0..1)** — 전역 기본 provider 슬롯 ([ADR-0062](../decisions/ADR-0062-llm-default-provider-explicit-selection.md) § Decision 2). 고정 id 1 row 만 사는 단일 슬롯 table 이 `llmProviderConfigId` FK 로 "지금 기본인 config" 를 가리키며, `onDelete: Restrict` 가 기본 지정 중인 config 의 삭제를 P2003 → **409** 로 차단한다 (관계 8 DifficultyMapping 의 in-use 409 패턴 mirror — 먼저 다른 row 를 기본으로 지정한 뒤 삭제). 슬롯이 존재하면 [LlmProviderConfigResolver](../../src/llm/llm-provider-config-resolver.service.ts) 의 default 해석이 row 수와 무관하게 그 config 를 채택하고, 슬롯 부재 시에만 "row 1 개 → 그 row / row 0 · row ≥ 2 → 한국어 fail-fast" fallback 이 도달한다 (ADR-0062 § Decision 4 — [ADR-0048](../decisions/ADR-0048-default-model-id-source.md) § Decision 2 의 부분 supersede).
    - **본 관계는 ADR-0062 가 `PROPOSED` 인 시점의 선반영이다** — `model LlmDefaultProvider` 는 [prisma/schema.prisma](../../prisma/schema.prisma) 에 **아직 존재하지 않으며** T-1863 이 schema + migration 으로 실체화한다. 따라서 § 2 entity 표 row 신설 · 합계 `14 → 15` · § 6 REQ → entity coverage 갱신은 그 표 직후 각주가 못 박은 대로 **별도 slice (T-1868 doc-sync) 소관** 이고, 본 문서의 현재 tally `14` 는 **불변** 이다.
...
- **ExportJob / ImportJob 의 구체 Prisma schema 코드 / migration / artifact 저장소** — [ADR-0044](../decisions/ADR-0044-export-import-job-persistence.md) (T-0484, Q-0040 옵션1 승인) 가 § 2 의 ExportJob/ImportJob entity 책임·필드·invariant·module 경계를 conceptual 박제했으나, `prisma/schema.prisma` 의 `model ExportJob`/`model ImportJob` 코드 + migration SQL + AssessmentModule controller/service 구현 + 누적 45 helper(T-0437~T-0483) 배선 + **artifact 저장소 mechanism**(로컬 파일시스템 vs S3-호환 object storage — 새 외부 dependency 가능성 시 별도 § 5 게이트) + job row retention/cleanup 정책 + merge mode conflict resolution 알고리즘은 모두 본 문서 범위 밖 — 후속 task chain(ADR-0044 §Out of scope / §Follow-ups). 본 문서는 entity / 관계 / invariant 의 source 만 제공. **materialization 전략(`ExportDump` envelope → byte stream)과 artifact 저장 위치(default = 응답 본문 직접 streaming, 영속 저장 0; 외부 object-storage 는 별도 사용자 게이트)는 [ADR-0046](../decisions/ADR-0046-export-dump-materialization-storage.md)(T-0505, Q-0042 게이트1 승인)가 새 외부 dependency 0 으로 박제 — ADR-0044 §Out of scope 이 deferred 한 두 piece 를 닫음.** **(2026-08-03 [T-1427](../tasks/T-1427-data-model-entity-vs-prisma-model-audit.md) 실측 closure — 위 열거 중 `model ExportJob` / `model ImportJob` 코드 (`prisma/schema.prisma` **614** · **649** 행) 와 migration SQL (`prisma/migrations/20260618000000_export_import_job/migration.sql` **20** · **39** 행 `CREATE TABLE`) 은 **이미 shipped** 라 그 두 piece 에 대해서는 `후속 task chain` 전제가 성립하지 않는다. `본 문서 범위 밖` 이라는 범위 선언 자체는 7 행 MVA conceptual-only 경계 그대로 **불변** 이고, 나머지 열거 항목의 shipped 여부는 본 slice 가 측정하지 않았다 (명칭 축 대조만) · 근거 [REQ-COVERAGE-AUDIT.md](../use-cases/REQ-COVERAGE-AUDIT.md) § 12.25.)**
