...
- **lock 획득 (`claude/lock-driver` 브랜치 commit-ref CAS)**: 브랜치 tip commit 의 `lock.json` 을 본다(미존재면 free).
  ...
  - 비어있음(브랜치 미존재 또는 tip 이 tombstone `{"holder":null}`) 또는 stale(since ≥ 60분) → **`scripts/acquire-lock.sh <holder> <session>` 로 lock 획득**
    (ad-hoc git plumbing 금지 — 이 헬퍼가 lock ref tip tree 의 `lock.json` **만** 교체하고 **claims.json 및 그 외 모든 엔트리를 보존**한다.
    직접 `lock.json` 단독 fresh tree 로 commit 해 push 하면 다른 driver 의 활성 claim 을 wipe → double-claim(#588) — select-claim.sh 동형 tree-보존 패턴 재사용으로 차단).
    헬퍼가 `git push … --force-with-lease=refs/heads/claude/lock-driver:<observed-old-sha>` CAS 를 수행(브랜치 미존재 시 zero-sha expect-absent CAS — "브랜치 생성") + CAS race 재시도(기본 3회) + 빈 commit push 가드(브랜치 삭제 방지).
    - exit 0 → 획득(stdout = 새 tip sha). (stale 탈취였다면 journal 1줄: `stole stale lock from <prev>`.)
    - non-zero(CAS reject 소진) → 경쟁 패배. 재 fetch → 여전히 held 면 즉시 종료(no-op).
...
- **claim-pickup 분기 (오직 flags.fineGrainedConcurrency == true 일 때 — ADR-0036 stage3)**:
    ...
    (a2) **maxConcurrentClaims 게이트 (stage 5a — T-0348 / 5b — T-0350)**: (b) select+claim
        **직전**, [1] lock(critical section) 보유 상태에서 (a) reclaim 후의
        ...
        간주**(가장 보수적 — §D8 (a) fail-safe 강등 계약 정합). 5b 현재값 = 2
        (T-0350). **5b direct-only 조건** (ADR-0036 §D8 (e)): 활성 claim ≥ 1
        존재 상태에서의 추가 claim 은 (i) 기존 활성 claim 의 task 와 후보 task 가
        **모두 `commitMode: direct`** (각 task 파일 frontmatter 직독 판정 — script
        변경 없이 LOOP 절차 계약) 이고 (ii) `scripts/validate-claim-candidate.sh`
        의 (a)(b) 재검증(touchesFiles 교집합 0 + dependsOn 머지됨) PASS 일 때만
        허용한다. **후보 또는 기존 claim task 의 commitMode 판독이 불확실하면
        no-op**(모르면-직렬화 fail-safe — §D8 (a) 정합). **pr-mode task 는 활성
        claim 0 일 때만 단독 claim** — 동시 claim 은 충돌해도 코드가 아닌 문서인
        direct 끼리만 (위험 표면 최소).
    ...
    **토글 ON(현 상태 — stage 5b, T-0350: flags.fineGrainedConcurrency=true +
    maxConcurrentClaims=2)이라 본 분기는 런타임 활성**이다. 5a(T-0348, 병렬 0)가
    claim 메커니즘(reclaim → select+claim → lock-free 진행 → release)을 실사용
    fire(T-0349)에서 무사고 검증했고, 5b 에서는 위 (a2) direct-only 조건 하에
    direct task 2개까지만 동시 claim 가능하다(pr-mode 는 여전히 단독).
    §D8 (d) 회로 차단기 강등(같은 유형 2회 누적) 시 flags 가 false 로 자동 복귀
    하고 본 분기는 다시 inert — 그때는 아래 단일-task pickup(currentTask/nextTask/
    planner dispatch) 경로가 동작한다. ADR-0036 §Decision 1/§rollout stage5.
...
- **획득**: 다른 holder 가 점유 & since < 60분 → 즉시 종료(no-op). 비어있음/stale(≥60분) →
  **`scripts/acquire-lock.sh <holder> <session>` 로 획득**(ad-hoc git plumbing 금지). 이 canonical 헬퍼가 lock ref tip tree 를 base 로 `lock.json` **만** 교체하고
  **claims.json 및 그 외 모든 tree 엔트리를 보존**한 commit 을 `--force-with-lease=refs/heads/claude/lock-driver:<observed-old-sha>` CAS 로 push 한다
  (브랜치 미존재 시 `<observed-old-sha>` = `0{40}` zero-sha CAS). 직접 `lock.json` 단독 tree 로 push 하면 다른 driver 의 활성 claim(claims.json)을 wipe → double-claim(#588) — 헬퍼가 이를 차단.
  exit 0 = 획득(stdout = 새 tip sha), non-zero(CAS reject 소진) = 경쟁 패배(재 fetch 후 held 면 종료). CAS race 재시도(기본 3회) + 빈 commit push 가드 내장.
...
- **해제**: 정상·BLOCKED 종료 시 항상 **`scripts/acquire-lock.sh release` 로 tombstone release**(ad-hoc git plumbing 금지). 이 헬퍼가 lock ref tip tree 를 base 로 `lock.json` 을 tombstone(`{"holder":null,"since":""}`)으로 교체하되
  **claims.json 및 그 외 모든 엔트리를 보존**한 채 동일 `--force-with-lease` CAS 로 push 한다(release 가 활성 claim 을 wipe 하지 않음 — 획득과 동형 보존 원칙).
