...
- **lock CAS 는 모든 진입점의 무조건 선행 의무 (ADR-0034 — cloud 포함)**: cron · 로컬 `/loop` · **cloud `/loop`** · headless 어느 진입점이든, executor 호출 / commit / PR 생성을 시작하기 **전에 반드시 위 `claude/lock-driver` CAS 획득을 시도**한다. lock 을 "비권위 mirror 만 동기하고 진행" 하는 경로는 **금지**.
  - **`gh 부재`·`main 직접 push 불가` 는 lock-skip 근거가 아니다**: lock CAS 는 `gh` 가 아니라 raw `git push … --force-with-lease` 로 수행하고, lock 저장소는 main 이 아니라 허용 prefix `claude/lock-driver` 다. 두 제약은 [4] 의 pr-mode stand-down 판정에는 영향을 주지만 **lock 획득 자체를 면제하지 않는다**.
  - **미획득(경쟁 패배 / 타 holder since<60분) → 즉시 no-op 종료(stand-down)**. "lock 못 잡았지만 feature branch + draft PR 로 격리 진행" 패턴은 **금지** — 이것이 2026-06-09 #246/#247 중복-PR 사고의 직접 메커니즘이다(ADR-0034 Context). 외화할 곳이 없으면 그냥 종료하고 lock 보유 driver / 다음 fire 에 맡긴다.
  - **`claude/*` push 권한조차 없는 극단 환경**(403 재발 등): lock CAS 불가 → 위 규칙대로 **무조건 stand-down**. credential/proxy 한계는 BLOCKED 가 아니라 no-op 종료로 흡수.
...
### Cloud 진입점 규율 (ADR-0034 — lock-bypass 중복-PR 사고 차단)

cloud `/loop` / cron@cloud 진입점은 lock 을 **건너뛰지 않는다**. 권위 `claude/lock-driver` CAS 는 `claude/*` 허용 prefix 안이라 cloud proxy 에서 credential 0 으로 수행 가능하다(ADR-0028 의 lock-저장소 이전 목적). 실측: cloud proxy 에서 `git push --dry-run origin HEAD:refs/heads/claude/_probe` → `[new branch]` 허용 + `--force-with-lease=refs/heads/claude/lock-driver:<tip>` dry-run 거부 없음.

- **금지 패턴**: "gh 부재 / main push 불가 → 권위 lock 미변경, mirror 만 동기, feature branch + draft PR 로 격리 작업". 이 경로가 2026-06-09 두 cloud `/loop` 세션(`loop@cloud-turn-cap-5` / `…-10`)이 같은 Q-0029/ADR-0033/T-0297 을 동시 수행해 PR #246/#247 중복을 낳은 직접 원인이다. lock tip 은 그동안 `38c22fab…` 로 미변경 — mutex 가 전혀 작동하지 않았다.
- **허용 동작**: lock CAS 획득에 성공한 1개 driver 만 작업한다. 획득 실패 진입점은 stand-down(no-op). `gh`/MCP 부재로 pr-mode 작업을 수행할 수 없는 환경이면, lock 을 잡았더라도 그 pr-mode task 를 claim 취소(tombstone 으로 lock 해제)하고 no-op 종료한다(§1[4] ADR-0010 정합) — 이때도 lock 미획득 상태로 작업하지 않는다.
