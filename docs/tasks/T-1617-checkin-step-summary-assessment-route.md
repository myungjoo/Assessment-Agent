...
estimatedFiles: 1
created: 2026-08-19
createdAt: 2026-08-19T18:37:10Z
independentStream: perf-checkin-baseline
dependsOn: [T-1616]
touchesFiles: [test/perf/assessment-measure-confirm-realdb.perf-spec.ts]
plannerNote: P5 perf — ADR-0056 §Decision 3 (b) step 요약 실호출을 person 에 이어 두 번째 route(assessment) 로 확산. route 고유 seam 중심, 일반 계약 복제 금지.
---

# T-1617 — assessment 실 DB perf-spec 으로 step 요약 실호출 확산 (두 번째 route)

## Why

[ADR-0056](../decisions/ADR-0056-perf-baseline-checkin-ci.md) `§Decision 3 (b)` 의 "상대 회귀를
로그와 step 요약으로 가시화" 계약은 T-1610~T-1615 6 slice 로 조각이 다 갖춰졌고, T-1616 이
`test/perf/person-read-realdb.perf-spec.ts` 에서 **첫 실호출처**를 열어 CI 로그에 emit
`status=appended` 실측 1 줄을 처음 박았다. 그러나 체크인 baseline 이 확정된 route 는
`baseline-ci-realdb-{person,assessment,contribution,summary,app-root}-read.json` **5 개**인데
step 요약 실호출은 아직 **person 1 개뿐** 이라, 나머지 4 route 는 회귀를 비교하고도 CI 화면에
아무 것도 남기지 않는다.

본 slice 는 그 확산의 **두 번째 route** 만 맡는다 —
`test/perf/assessment-measure-confirm-realdb.perf-spec.ts` (label `ci-realdb-assessment-read`,
T-1600 실측 → T-1601 체크인 완료). 나머지 3 route(contribution · summary · app-root) 와
`.github/workflows/ci.yml` 편입(`§Follow-ups (b)`)은 각각 별도 slice 다.

본 route 의 고유 seam 은 person route 에 없던 **`period` optional 분기**(지정 3 row / 미지정 5 row)
와 **실 JWT guard 401 전량 표본** 이다. 국면은 그 두 축 중심으로만 새로 쓰고, 주입값 형태 위반 ·
`null` deps 전파 · 기본값 결선 같은 **일반 계약 국면은 helper colocated spec 과 person route 가
이미 cover** 하므로 복제하지 않는다(T-1575 중복 국면 삭제 선례).
...
- `test/perf/assessment-measure-confirm-realdb.perf-spec.ts` — 본 task 가 **유일하게 수정하는
  파일**(592 행). 특히 `WEEK_ROWS`/`MONTH_ROWS`/`TOTAL_ROWS`(`56~58 행`) · `ITER`(`61 행`) ·
  `WIRING_ITER`(`64 행`) 상수, `lastStatus`(`72 행`) · `tmpRoot`(`74 행` 선언 · `87~89 행`
  `beforeEach` mkdtemp · `91~94 행` `afterEach` rmSync + `truncateAll`), `dirOf`(`102~104 행`) ·
  `seed`(`107 행`, Person 1 + Assessment 5) · `read(query, authed = true)`(`127 행`, `authed=false`
  면 401 분기) · `rows`(`136 행`), 그리고 본 task 가 국면을 추가할
  `describe("체크인 baseline 실측 clock 관찰(ci-realdb-assessment-read)")`(`318 행` ~ 파일 끝) 안의
  지역 helper `measureRealClock` · `checkWithLogs` · `enabledEnv` · `expectEnabledOutcome` ·
  `metricsLineOf` 와 상수 `REAL_CLOCK_ITER`(`330 행`) · `REAL_CLOCK_ITER_MIN`(`332 행`) ·
  `realClockEnv`(`336~340 행`) · `METRIC_KEYS`(`342~350 행`).
- `test/perf/person-read-realdb.perf-spec.ts` `645~920 행` — T-1616 이 연 **정본 패턴**(nested
  `describe("step 요약 배선 실호출(emitCheckinStepSummaryForSpec)")` + 지역 helper
  `STEP_SUMMARY_TITLE` · `summaryEnv` · `SUMMARY_ITER` · `depsWith` · `summaryPathOf` ·
  `checkForSummary` · `comparedFor`). **구조만 승계**하고 문구 · label · 국면 수는 본 route 에 맞게
  갈아끼운다(국면 전량 1:1 복제 금지).
- `test/perf/checkin-baseline-spec-wiring.ts` `60 행`(`checkCheckinBaselineForSpec`) ·
  ...
  `132 행`(`defaultStepSummarySinkDeps()` — 호출 시점 `process.env` + `fs.appendFileSync` utf-8
  바인딩) · `162 행`(`emitCheckinStepSummaryForSpec(outcome, sectionTitle, deps?)` — `deps` 가
  `undefined` 일 때만 기본값으로 채우고 `null` 은 그대로 넘겨 위임 `TypeError` 로 노출) 과
...
- [ ] `describe("체크인 baseline 실측 clock 관찰(ci-realdb-assessment-read)")` 안에 **step 요약 축
      ...
      요약 축 fixture label 은 실측 label(`ci-realdb-assessment-read`) · 배선 label
      (`realdb-assess-mc`) 과 **분리된 전용 label** 을 쓴다.
...
- [ ] **error path 2+** — (a) append 함수가 던지는 주입 `deps` 로 같은 `compared` 국면을 태우면
      **예외 전파 0** 이고 `status === "failed"` · `reason === "append-threw"` 로만 보고된다
      (exit code 불변 직접 증거), (b) **본 route 고유 guard 축** — `read(query, false)`(Cookie
      미부착 → 전량 401 · `errorRate=1` · `count=0`) 표본으로 만든 outcome 을 emit 에 태워도
      throw 0 이고 동일 판별 union 으로 끝난다(오류 표본 축에서도 관찰-only 유지).
...
- [ ] **negative cases 충분 cover** — 최소 2 종을 각각 별도 `it` 로: (a) emit 호출 전후로
      ...
      (b) **본 route 고유 `period` 분기 축** — `period` 지정(`?personId=..&period=week`, 응답
      `WEEK_ROWS` 행) 과 미지정(응답 `TOTAL_ROWS` 행) 두 표본의 outcome 을 연속 emit 해도 두 호출이
      서로 독립이다: append 가 각 1 회씩 정확히 2 회 호출되고, 두 outcome 객체가 **변형되지 않으며**,
      두 번째 호출 결과가 첫 번째의 상태를 이어받지 않는다(부작용 누적 0 · 인자 변형 0).
      저장소 실경로(`test/perf/baselines/`) 무오염 · 임시 트리 정리는 기존 `afterEach` 가 이미
      보장하므로 **재작성 금지**.
- [ ] 기존 국면 불변 — `WEEK_ROWS` · `MONTH_ROWS` · `TOTAL_ROWS` · `ITER` · `WIRING_ITER` ·
      `REAL_CLOCK_ITER` · `REAL_CLOCK_ITER_MIN` · `realClockEnv` 값, 기존 `it` 들의 본문과 단언,
...
- [ ] diff 는 **1 파일 · 300 LOC 이내**를 지킨다. 신규 `it` 은 **7 개 이내** 로 묶고(person route 의
      9 개를 그대로 복제하지 않는다 — 일반 계약 국면은 이미 cover 됨), cap 이 위태로우면 분기 (3) 의
...
- **나머지 3 route 확산 금지** — contribution · summary · app-root 의
  `*-measure-confirm-realdb.perf-spec.ts` 는 본 task 에서 건드리지 않는다(각각 별도 slice).
  `person-read-realdb.perf-spec.ts` 도 **읽기 전용**(참조만, 0 LOC 변경).
