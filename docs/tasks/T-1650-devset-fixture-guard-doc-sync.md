...
- [x] `docs/ops/realdata-scale-devset.md` 에 **소절 1 개 신설**(`## 재생성(refresh) 명령` 직전, 제목은 `## 기계 판독 사본 · drift guard` 류) — 다음 3 사실을 담는다:
...
- [x] `docs/ops/load-resilience-test-plan.md` `§5` item 5 의 **잔여 ①** 서술 갱신 — "실 dataset seed" 축이 (a) 입력 데이터(133 로그인 fixture) 와 (b) 이중 정본 drift guard 까지 확보됐고 **잔여는 seed 실행 경로**(133 `Person` + 각자 github `ServiceIdentity` 적재 → 실 수집 왕복)임을 T-1648 · T-1649 의 commit sha · PR 번호와 함께 명시. 잔여 ① 자체는 **해소로 표기하지 않는다**(seed 실행 0).
- [x] 같은 item 5 의 "본 item 의 잔여는 위 ① … 1 개뿐" 문장이 위 갱신과 모순되지 않게 정합(잔여 개수 1 개는 유지, 그 내용이 좁혀졌음을 반영).
- [x] `docs/PLAN.md` `141 행` 말미에 fixture + guard 박제 사실 한 문장 추가. `140 행` checkbox 는 `[ ]` 유지(변경 금지).
- [x] **수치 · 임계 무변경 검증** — `§3` 표의 임계 숫자(p95 ≤ 900ms · error rate < 1%), `§3.1` 1~6 회차 실측 기록, ADR-0057 `D4` 외삽 산식 서술은 **문자 그대로 보존**. `git diff` 로 해당 영역 변경 0 확인.
- [x] **코드 · 데이터 무변경 검증** — `git diff --name-only` 결과가 위 3 개 `docs/` 파일뿐이고 `src/` · `test/` · `.github/workflows/` · `package.json` · `test/load/realdata-devset-logins.json` 변경이 0 인지 확인.
- [x] 문서에 적은 commit sha · PR 번호 · symbol 이름이 실제와 일치하는지 대조(`git log --oneline | grep -E "c95b7dec|87cdb828"`, Required Reading 의 helper 파일 export 목록).
...
- 완료: 2026-08-22T14:50Z (cron@aa-local-65bb97ad-2337)
- main commit: `5a52ecc0` (direct, 3 파일 `+32/-3`)
- CI run **32579447795** (`5a52ecc0`) conclusion `success` — 본 turn 안에서 확인(R-114 충족).
- 결과: `docs/ops/realdata-scale-devset.md` 에 `## 기계 판독 사본 · drift guard` 소절 신설(`## 재생성(refresh) 명령` 직전, fixture 경로 · guard 에러 정책 · 편집 규칙 3 사실), `docs/ops/load-resilience-test-plan.md` `§5` item 5 의 잔여 ① 를 "seed 실행 경로" 로 좁혀 갱신(해소 표기 없음, 잔여 개수 1 유지), `docs/PLAN.md` `141 행` 말미에 fixture + guard 박제 한 문장 추가(`140 행` checkbox `[ ]` 유지).
- 무변경 검증: `§3` 표 임계(p95 ≤ 900ms · error rate < 1%) · `§3.1` 1~6 회차 기록 · ADR-0057 `D4` 산식 문자 그대로 보존, diff 가 `docs/` 3 파일뿐(`src/` · `test/` · 워크플로 · `package.json` · fixture JSON 변경 0).
