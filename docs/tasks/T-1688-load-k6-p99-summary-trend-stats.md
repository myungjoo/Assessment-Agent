...
- [ ] `test/load/s1-batch.js` · `s2-read.js` · `s3-concurrent.js` 세 스크립트의 `export const options` 에 각각
...
- [ ] 설계 조항 ② 준수 — 세 스크립트의 `thresholds` 배열 · 임계 숫자(`3000` · `0.01` · `BATCH_P95_MS` · `STUB_BASELINE_P95_MS`) ·
...
- [ ] 설계 조항 ① 준수 — `package.json` 의 `dependencies` / `devDependencies` 에 새 키 **0**(k6 내장 기능만 사용).
- [ ] **happy-path test**: drift-guard spec(`test/smoke/load-workflow-k6-harness-wiring-drift.smoke-spec.ts`) 끝에 새 describe 를 append 해,
      세 스크립트가 각각 `summaryTrendStats` 를 선언하고 그 배열이 기본 6 종 + `p(99)` 를 모두 포함함을 단언한다(스크립트당 1+ it).
- [ ] **error path test**: 존재하지 않는 스크립트 경로에 대해 읽기가 `throw` 하고(0-byte silent fallback 으로 false-PASS 되지 않음),
      새로 추가한 추출 helper 에 non-string 입력을 주면 `TypeError` 임을 단언하는 it 1+.
- [ ] **flow / 분기 cover**: 새 추출 helper 의 분기마다 합성 입력으로 it 1+ — (a) `summaryTrendStats` 배열이 있는 입력 → 원소 목록 반환,
      (b) 배열이 없는 입력 → 미발견 정규형(빈 배열 또는 `null`, throw 아님), (c) 따옴표 종류(`"` / `'`) · 줄바꿈 배치가 달라도 같은 정규형.
- [ ] **negative cases 충분 cover** (아래 5 종 각각 it 1+):
...
- [ ] `pnpm lint && pnpm build && pnpm test` 통과 + `pnpm test:smoke` 로 새 describe 포함 green.
- [ ] `pnpm test:cov` 통과 (line ≥ 80% / function ≥ 80%).
- [ ] 새 `workflow_dispatch` · rerun · 실측 수치 회수 **0** — 본 task 는 배선만 하고 값 회수는 후속 slice 다.
