---
id: T-1578
title: contribution measure→confirm 실 DB perf-spec 에 체크인 baseline 배선 factory 얹기
phase: P5
status: DONE
commitMode: pr
coversReq: [REQ-048]
...
estimatedFiles: 1
created: 2026-08-17
createdAt: 2026-08-17T02:37:00Z
completedAt: 2026-08-17T03:48:48Z
prNumber: 1259
independentStream: perf-baseline-checkin
dependsOn: [T-1577]
touchesFiles:
  - test/perf/contribution-measure-confirm-realdb.perf-spec.ts
plannerNote: "P5 성능 검증 — ADR-0056 §Follow-ups (b) 확산 slice: realdb 세 번째 소비자(3-level FK chain)에 factory 호출 1 회(신규 로직 0, ~55 LOC)"
---

# T-1578 — contribution measure→confirm 실 DB perf-spec 에 체크인 baseline 배선 factory 얹기

## Why

ADR-0056 `§Follow-ups (b)` 의 체크인 baseline 확인 배선은 현재 mock 기반 `*-measure-confirm`
perf-spec 4 개(summary · assessment · contribution · app-root)와 실 DB 계열 **2 개**
(T-1576 `summary-measure-confirm-realdb`, T-1577 `assessment-measure-confirm-realdb`)에만
얹혀 있다. 아직 미배선인 `contribution-measure-confirm-realdb` 는 **`Person → Assessment →
Contribution` 3-level FK chain 을 부모 id(`assessmentId`)로 긁는** 고유 query 형태라, 앞선 두
소비자와 다른 실 DB 표본에서도 배선이 동일하게 동작하는지(토글 off 무동작 · 토글 on 판정 ·
exit code 불변)가 미관측이다. T-1577 Follow-ups 의 "나머지 `*-realdb` 계열 배선 확산" 중
**소비자 1 개**만 여는 연속 slice 다(PLAN P5 성능 검증 / REQ-048 조회 p95 < 3s 계열).
...
- `test/perf/contribution-measure-confirm-realdb.perf-spec.ts` — 본 task 가 유일하게 수정하는
  파일. 기존 `env`(`realdb-contrib-mc`) / `tmpRoot` / `dirOf` / `read` / `qOf` 조립을 그대로
  재사용한다.
- `test/perf/assessment-measure-confirm-realdb.perf-spec.ts` 의 파일 끝 factory 호출부 +
  `stepClock` 정의 + `WIRING_ITER` 상수 — **직전 slice(T-1577)의 실 DB 배선 관용구 선례**. 본
  task 는 같은 관용구를 그대로 따른다.
...
- [ ] `test/perf/contribution-measure-confirm-realdb.perf-spec.ts` 파일 끝(최상위 describe 안)에서
      `registerCheckinBaselineWiringSuite` 를 **1 회** 호출한다 — `envMeta` 는 기존 `env`
      (`realdb-contrib-mc`), `tempDir` 은 기존 `dirOf` 기반 임시 경로, `measure` 는
...
- [ ] 실 DB 반복 비용을 감안해 factory 주입 `measure` 의 `iterations` 는 소규모(2~3)로 고정하고,
      ...
      매칭 0 건 부모 query(예: `?assessmentId=checkin-wiring-probe` → 200 + 빈 배열)로 태워
      기존 국면의 자식 수량 단언(`PRIMARY_CHILDREN` · `OTHER_CHILDREN`)에 간섭하지 않는다.
- [ ] **happy-path** — 토글 off / 토글 on × 존재 / 토글 on × 기본 바인딩 국면(factory happy 3 개)이
      본 spec 에서 전부 실행돼 pass. `pnpm test:perf` 로
      `contribution-measure-confirm-realdb.perf-spec.ts` 실행 시 배선 describe 의 국면 **10 개**가
      추가로 보고된다.
...
- [ ] 기존 국면(happy · 분기 · error · negative 전량)의 제목 · 단언 · 순서 **불변** — diff hunk 는
      import 추가 · `stepClock` 등 측정 조립 helper · 파일 끝 factory 호출 · 주석에 한정.

## Out of Scope

- 다른 `*-realdb` · `*-read` perf-spec 배선(본 task 는 소비자 **1 개**만 — `app-root` · `person`
  realdb 는 후속 slice).
