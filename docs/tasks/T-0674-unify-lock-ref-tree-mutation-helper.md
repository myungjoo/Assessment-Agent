...
plannerNote: "P5 fix-2 slice 1a — lib-lock-tree.sh 헬퍼 신설 + 단위test + acquire/select 2 script 라우팅 + ci 배선. reclaim 라우팅은 T-0675(slice 1b). #588 구조적 차단."
---

# T-0674 — lock-ref tree mutation 공통 헬퍼 신설 + acquire/select 라우팅 (fix-2 slice 1a)

> **분할 이력**: 본 task 는 원래 acquire/select/reclaim 3 script 를 모두 헬퍼로 라우팅하는 단일 task 였으나, 구현+test 가 6 파일/+368 LOC 로 hard cap(5 파일/300 LOC)을 초과해 2 slice 로 분할됐다(planner split, CLAUDE.md §3). 본 slice **1a** = 헬퍼 신설 + 단위 test + `acquire-lock.sh`·`select-claim.sh` 2 script 라우팅 + ci.yml 배선. 나머지 `reclaim-stale-claim.sh` 라우팅 + 잔여 test case 는 **T-0675(slice 1b, dependsOn: [T-0674])**.

## Why

double-claim(#588) 의 근본 원인은 **lock ref tip tree 를 변경하는 경로가 여러 개**인데 그중 하나(ad-hoc lock-acquire)가 `claims.json` 을 보존하지 않고 `lock.json` 단독 fresh tree 로 덮어쓴 것이었다. fix-1(T-0673)이 canonical `acquire-lock.sh` 를 신설해 acquire/release 경로를 막았으나, 같은 "tip tree 를 base 로 한두 blob 만 교체 + `--force-with-lease` CAS push + self-contained git identity + 빈 commit 가드 + CAS race 재시도" 로직이 **`acquire-lock.sh`·`select-claim.sh`·`reclaim-stale-claim.sh` 3 곳에 복붙으로 중복**돼 있다(T-0673 Out of Scope 가 fix-2 로 명시). 복붙 중복은 #588 류 재발 위험의 구조적 원천이다 — 한 script 만 패턴을 어기면 다시 claims.json wipe 가 난다.

본 slice **1a** 는 그 tree-보존 mutation 을 단일 공통 헬퍼 `scripts/lib-lock-tree.sh` 로 추출하고, **`acquire-lock.sh`·`select-claim.sh` 2 script 를 그 헬퍼 호출로 라우팅**한다. 남은 `reclaim-stale-claim.sh` 라우팅은 cap 때문에 **T-0675(slice 1b)** 로 분리된다(헬퍼는 본 slice 에서 신설·단위 test 완결되므로 slice 1b 는 이미 검증된 함수를 호출만 추가). ADR-0036 §Decision 1 의 "보존 불변"(claims.json 동거 tree·CAS 원자성)을 lock-ref 변경 경로에서 단일 구현으로 강제하는 첫 단계다. CLAUDE.md §10 동시 실행 정책 + ADR-0036 §Decision 1/8 정합.
...
- [ ] `scripts/lib-lock-tree.test.sh` 신설 — select-claim.test.sh bare-repo + 2 clone 골격 차용. **헬퍼 자체를 본 slice 에서 완결 검증**(slice 1b 는 이미 검증된 함수 호출만 추가). 다음 R-112 4종 + negative 충분 cover — **헬퍼의 모든 분기를 본 slice 의 test 가 cover**(라우팅 대상이 acquire/select 2 script 뿐이어도 헬퍼 분기 검증은 script 비의존이므로 여기서 끝낸다):
  - [ ] **happy-path**(기능): 기존 claims.json + 다른 파일이 있는 tip 에서 `lock_tree_cas_push` 로 lock.json 만 교체(1 blob 경로) → tip tree 에 **claims.json 및 그 외 파일 byte-동일 보존**, lock.json 만 갱신(#588 회귀 가드 — 보존 깨지면 fail).
  - [ ] **error path**: `old_sha` 부재(빈 문자열) → zero-sha expect-absent lease 로 첫 생성 성공. parent 없는 commit.
  - [ ] **branch cover**: (a) 1 blob 교체(acquire) 경로, (b) 2 blob 교체(claim, claims.json+lock.json) 경로 + sibling 보존, (c) old_sha 존재 vs 부재 분기 각 1+.
  - [ ] **negative 충분 cover**(예외 분기마다 1+): (1) CAS lease mismatch(틀린 old_sha) → push reject(return 20), (2) 빈/누락 commit → push 차단(return 30, 브랜치 삭제 방지 — `git ls-remote` 로 ref 가 삭제 안 됨 확인).
  ...
  - [ ] (참고) temp-file-collision 동시 호출 검증 case 는 cap 때문에 slice 1b(T-0675)로 이연 — 본 slice 는 헬퍼가 호출별 고유 임시 경로를 쓰도록 **구현**은 완결하되 그 동시성 검증 case 만 T-0675 에 둔다.
- [ ] `scripts/acquire-lock.test.sh`·`scripts/select-claim.test.sh` 가 **변경 후에도 전부 pass**(헬퍼 추출이 외부 동작을 바꾸지 않음 증명 — 회귀 가드). 필요 시 sourcing 경로만 조정. (`reclaim-stale-claim.test.sh` 는 slice 1b 에서 라우팅 변경 후 회귀 확인 — 본 slice 는 reclaim 미변경이라 무관.)
...
- **`reclaim-stale-claim.sh` 의 헬퍼 라우팅** — cap 분할로 **T-0675(slice 1b, dependsOn: [T-0674])** 로 분리. 본 slice 는 reclaim 미변경.
- **temp-file-collision 동시 호출 검증 test case** — slice 1b 로 이연(헬퍼 구현은 고유 임시 경로로 본 slice 에서 완결, 그 검증 case 만 T-0675).
...
- **fix-2 slice 1b(T-0675)**: `reclaim-stale-claim.sh` 를 본 slice 신설 헬퍼로 라우팅 + temp-file-collision 동시 호출 test case + reclaim 회귀 가드. dependsOn: [T-0674].
