...
estimatedFiles: 1
created: 2026-08-16
createdAt: 2026-08-16T22:39:09Z
independentStream: perf-baseline-checkin
dependsOn: [T-1575]
touchesFiles:
  - test/perf/summary-measure-confirm-realdb.perf-spec.ts
plannerNote: "P5 성능 검증 — ADR-0056 §Follow-ups (b) 확산 slice: 미배선 realdb 계열 첫 소비자에 factory 호출 1 회(신규 로직 0, ~70 LOC)"
---

# T-1576 — summary measure→confirm 실 DB perf-spec 에 체크인 baseline 배선 factory 얹기

## Why

ADR-0056 `§Follow-ups (b)` 의 체크인 baseline 확인 배선은 현재 mock 기반 `*-measure-confirm`
perf-spec **4 개**(summary · assessment · contribution · app-root)에만 얹혀 있다. 실 DB
round-trip 계열(`*-realdb`)은 한 곳도 배선되지 않아, **실 query 지연이 섞인 표본**에서도 체크인
baseline 확인 배선이 동일하게 동작하는지(토글 off 무동작 · 토글 on 판정 · exit code 불변)가
미관측 seam 으로 남아 있다. T-1575 Follow-ups 의 "`*-realdb` · `*-read` 계열 factory 배선"
항목 중 **첫 소비자 1 개**만 여는 slice 다(PLAN P5 성능 검증 / REQ-048 조회 p95 < 3s 계열).
...
- `test/perf/summary-measure-confirm-realdb.perf-spec.ts` — 본 task 가 유일하게 수정하는 파일
  (기존 `env` / `tmpRoot` / `dirOf` / `read` 조립을 그대로 재사용).
...
- `test/perf/contribution-measure-confirm.perf-spec.ts` 의 factory 호출부(파일 끝)와 `stepClock`
  정의 — 배선 관용구 선례.
...
- [ ] `test/perf/summary-measure-confirm-realdb.perf-spec.ts` 파일 끝(최상위 describe 안)에서
      `registerCheckinBaselineWiringSuite` 를 **1 회** 호출한다 — `envMeta` 는 기존 `env`
      (`realdb-summary-mc`), `tempDir` 은 기존 `dirOf` 기반 임시 경로, `measure` 는
...
- [ ] 전역 토글 저장 · 원복은 factory 의 `beforeEach` / `afterEach` 소관 — 본 spec 에 지역
      ...
      `reseedAuthenticatedActors` 순서는 변경 0.
- [ ] `pnpm lint --max-warnings=0` · `pnpm build` · `pnpm test` (전체 unit suite) 통과.
- [ ] `pnpm test:cov` 통과 (line ≥ 80% / function ≥ 80%).
- [ ] 기존 국면(happy ① · ② · 분기 ⓑ · ⓒ · error (a) · (b) · negative (a)~(e))의 제목 · 단언 ·
      순서 **불변** — diff hunk 는 import 추가 · `stepClock` 등 측정 조립 helper · 파일 끝 factory
      호출 · 주석에 한정.
