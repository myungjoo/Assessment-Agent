...
plannerNote: "P6 contract-guard refactor 이관 slice 3(users-list). T-1201 helper 와 글자-동일한 5종만 import 교체 — users-list 의 richer extractHandlerMethods(5-field hasBody/hasParam/hasQuery)·pathParams 는 shared export 와 의미 상이라 inline 유지. 단일 test 파일 1개, AdminView.tsx·타 spec 전부 파일-disjoint → stage 5b 동시 claim 안전, dependsOn=[T-1201](머지 완료)."
---

# T-1204 — contract-guard 소비 spec 이관 slice 3 (users-list)

## Why

T-1201(PR #1093, `8abaf0d3`)이 30+ contract-guard spec 이 중복 복사하던 invariant 추출기를 `web/src/views/__contract-guard__/contract-extractors.ts` 공용 helper 로 신설했고, T-1202(PR #1094, `cfe79997`)가 첫 소비처 `schedules-list`, T-1203(PR #1095, `8abaf0d3` 후속)이 `persons-list` 를 이관했다. T-1203 Follow-up 이 지목한 **다음 이관 대상은 `AdminView.users-list-contract.test.ts`** 다(REQ-060 unit test 유지보수성).

users-list 는 persons-list 와 동형인 GET-list spec 으로, **일부 추출기가 공용 helper 와 의미가 다른 변형(variant)** 을 들고 있다(richer `extractHandlerMethods` + `pathParams`). 따라서 본 slice 는 T-1201/T-1203 이 정한 원칙("변형이 있는 함수는 이관에서 제외하고 Follow-up 에 남긴다")대로 **글자-동일한 5종만** 공용 helper import 로 교체하고, 변형은 inline 유지한다. **동작(test 개수·통과 결과·coverage)은 완전 무변경** — 순수 중복 제거 refactor. 단일 test 파일 1개만 수정하므로 `AdminView.tsx`·다른 contract-guard spec 전부와 파일-disjoint 하여 stage 5b 동시 claim 에 안전하고, `dependsOn` 은 이미 머지된 T-1201 뿐이다.

## Required Reading

- `web/src/views/__contract-guard__/contract-extractors.ts` — **import 원천**. export 목록과 각 시그니처를 확인한다. 이 spec 이 import 할 **글자-동일한 5종**: `stripComments`, `extractControllerRoute`, `normalizeRoute`, `composeRoute`, `stripQuery`. **주의 — import 하면 안 되는 export**: 공용 `extractHandlerMethods`(반환 타입이 2-field `{method, subPath}` 인 simple 변형)·공용 `HandlerDecorator`(2-field 인터페이스)·`pathSegments`(전 세그먼트 반환)·`extractHandlerParams`. 이들은 users-list 의 inline 변형과 의미가 다르므로 import 금지(아래 대조 참조).
- `web/src/views/AdminView.users-list-contract.test.ts` — **이관 대상**. 삭제할(→ import 교체) **글자-동일 5종**: `stripComments`(L15~21), `extractControllerRoute`(L22~25), `normalizeRoute`(L95), `composeRoute`(L96~99), `stripQuery`(L101). **inline 유지(이관 금지) 변형/per-spec**:
  - `interface HandlerDecorator`(L26~32) — 공용은 `{method, subPath}` 2-field 이나 여기는 `{method, subPath, hasBody, hasParam, hasQuery}` **5-field 변형**. 유지.
  - `extractHandlerMethods`(L35~70) — 공용은 simple 2-field 매핑이나 여기는 멀티라인 시그니처 괄호-균형 매칭으로 `hasBody/hasParam/hasQuery` 까지 채우는 **richer 변형**. 유지(내부에서 참조하는 `stripComments` 는 import 된 것을 사용 — 의미 동일이라 무해).
  - `pathParams`(L100) — `route.split('/').filter((seg) => seg.startsWith(':'))`(`:` 접두 세그먼트만). 공용 `pathSegments`(전 세그먼트)와 **의미 상이**. 유지(공용에 대응 export 없음).
  - `extractUsersFireMethod`(L75~82)·`interface BackendContract`(L83~90)·`interface WebFire`(L91~94)·`diffContract`(L104~116) — per-spec 발사기·타입·대조 로직. 유지.
- `web/src/views/AdminView.persons-list-contract.test.ts`(L1~14 및 import 블록) — **이관 스타일 참조**(read-only). T-1203 이 이관한 import 블록 형태(참조하는 5종만 알파벳 정렬 import + 한국어 주석 3줄)를 mirror 한다. users-list 는 persons-list 와 동형이므로 **import 목록이 동일**(위 5종).

## Acceptance Criteria

**정확히 파일 1개**만 수정한다(`AdminView.users-list-contract.test.ts`). 신규 파일 0, 다른 spec·production 코드 0 LOC.
...
- [ ] **변형/per-spec 은 inline 유지** — `HandlerDecorator`(5-field 변형)·`extractHandlerMethods`(richer 변형)·`pathParams`·`extractUsersFireMethod`·`BackendContract`·`WebFire`·`diffContract` 는 삭제·이관·시그니처 변경 금지. 공용 helper 의 `extractHandlerMethods`/`HandlerDecorator`/`pathSegments`/`extractHandlerParams` 를 import 하지 않는다(의미 상이 → 동작 변경 위험).
...
- [ ] happy-path 무회귀: 이관 후 정상 계약 정합 test(`diffContract(usersFire(0), LIST_CONTRACT)` → `[]`, `buildUsersPath(0)` → `/api/users`)가 그대로 green.
...
- [ ] 분기 무회귀: `composeRoute` subPath 유/무·`normalizeRoute` leading-slash 유/무 등 import 된 helper 분기가 이 spec 의 대조 경로(2-way GET 판별·mutation 대조군)에서 이전과 동일하게 동작. 특히 inline 유지한 richer `extractHandlerMethods` 가 import 된 `stripComments` 를 호출해도 `list`/`detail`/`changeRole`/`signup` 매핑이 이전과 동일.
...
- **다른 contract-guard spec(groups-list·parts-list·llm-provider-list 등 남은 28개) 이관 금지** — 본 slice 는 users-list 단일 파일만. 각 spec 은 후속 slice(파일당 소diff, 파일-disjoint)로 하나씩.
