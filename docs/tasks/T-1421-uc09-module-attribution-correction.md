...
1. **[INDEX.md](../use-cases/INDEX.md) 25 · 39 행 원자 정정** (T-1420 FU5 승계) — 25 행이 `8 NestJS module 명` 이라 적으면서 **9 개** 를 열거하고 그 9 개가 [modules.md](../architecture/modules.md) 실측 **12** 와도 어긋난다. 본 slice 의 병기는 이 모순을 우회만 했다. 두 지점을 한 slice 에서 원자 처리.
2. **UC-09 `§5` sequence participant 12 행 병기 여부 판정** — `§9` 만 정본 병기를 받아 `§5` 와 표현 밀도가 갈린다. mermaid diagram 이라 blast radius 실측 후 판정 (병기 / 무편집 택 1).
3. **[modules.md](../architecture/modules.md) 192 · 44 · 205 행 `11 module` vs 실측 12 정정** (T-1420 FU3) · **[data-model.md](../architecture/data-model.md) 38 행 `13 entity` vs 실 row 14** (FU2) · **[api.md](../architecture/api.md) 223 행 링크 범위** (FU1) 잔존 — 각각 별도 slice.

## 완료 기록 (2026-08-03)

- **AC 2 채택안 = (B) 정본 병기.** (A) 전면 치환은 blast radius **17 행 / 2 파일** + `§5` sequence · [INDEX.md](../use-cases/INDEX.md) 무편집 경계 동시 위반으로 기각, (C) 무편집은 B 가 cap 안에서 무모순 성립하므로 기각. 반영은 UC-09 `§9` **131 · 136 · 137 행** 3 지점 in-place 1:1 (`§12.15` 판별상 셋 다 날짜 stamp 0 = living 서술).
- **수치 불변** — `5 component + 6 module` 불변 (module distinct **6**, 병기분은 부기라 산정 제외). UC-09 `wc -l` **174** 불변 · `AssessmentModule` hit **14** (§5 **12** 포함) 불변 · audit `^| REQ-` **66** · `^## ` **12** 불변 · [modules.md](../architecture/modules.md) **256** · [INDEX.md](../use-cases/INDEX.md) **123** 무편집.
- **AC 7** — 본 task 는 `commitMode: direct` + production code **0 LOC** · 분기 **0** 이라 [CLAUDE.md](../../CLAUDE.md) §3.2 doc-only 면제 조항으로 **R-110 tester 호출 · R-112 4 항목 (happy / error / branch / negative) · `pnpm test:cov` 가 전부 N/A** 다.
- 판정 전문은 [REQ-COVERAGE-AUDIT.md](../use-cases/REQ-COVERAGE-AUDIT.md) `§ 12.19`.
