...
- **S1 규약 정본 승격** (1 순위) — 초안 7 조의 반례 검증 (특히 (e) `L` prefix ↔ 파일 내 일관성 충돌) 후 확정 절 신설. `direct` · 2 파일 · ≈ 65 LOC.
- **S2 `CLAUDE.md` §12 pointer 3 줄 + §3.1 rule 5 명문화** — `§ 12.72` AC 3 Follow-up 을 흡수해 한 slice 로 처리.
- **S3 ADR 군 `~`·`–` 혼용 파일 정규화** — 시급성 낮음 (전면 소급 치환 금지 판정, `§ 12.75` AC 4 (b)).
- **census 모집단 확장** — `docs/tasks/*` · `docs/progress/*` 는 본 census 밖 (`§ 12.75` 한계 3).
- **grep 함정 박제** — 범위 census 에서 bracket `[~–]` 는 multibyte 분해로 조용히 0 을 내므로 alternation `(~|–)` 를 쓴다.

## 결과 요약 (완료)

- census 5 문서군 × 4 형식 = ① `~` **278** · ② `–` **75** · ③ `L` **16** · ④ 단위어 **73** (④ 는 ① ~ ③ 부분집합) → 구분자 최빈 `~` 채택.
- 규약 초안 **7 조** (구분자 · 경계 포함 · 빈 줄 배제 · 단일 행 · `L` prefix · 불연속 병기 · 시점 기록 예외) · 각 근거는 `§ 12.68` ~ `§ 12.74` 승계 (새 판정 0).
- 박제 위치 = 후보 ① audit 절 (`direct`) 선택 · 이행 분해안 4 slice (1 순위 S1) · 전면 소급 치환 **금지** 판정.
- [REQ-COVERAGE-AUDIT.md](../use-cases/REQ-COVERAGE-AUDIT.md) `§ 12.75` 신설 (70 행 · 6776 → 6846) · 편집 대상 문서 **0** (census 읽기 전용).
