...
- **Status: DONE** (2026-06-16T04:39Z, cron@aalocal-s1-4510926 fire, fineGrainedConcurrency claim 경로).
- UC-03 §6.3 에 "shipped 구현 참조 (P7 R-50)" addendum 1 문단 추가 — `POST /api/schedules/backfill/:personId` (T-0421 PR #340, Admin+) manual 진입점 + `BackfillRunnerService.runBackfill` (T-0419) + `AssessmentBackfillChecker` (T-0420) 중복 차단 proxy 박제, `Person.backfilledAt` 전용 영속 표식·`NewPersonEvent` 자동 emit 미shipped 명시. §11 References 에 api.md §5 backfill 링크 1줄. §3/§5 diagram/§8/§9/§10/mermaid 불변.
- +3/-0, UC-03 1 파일, src/test 0 LOC (R-110/R-112 면제 — direct doc-only). pre-check git grep 0건 (gap 미해소 확인 후 편집).
