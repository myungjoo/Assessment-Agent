...
- [x] **AC 1 — 실측 선행 (날조 금지)**: 편집 전에 다음을 직접 측정해 `§ 12.30` 에 **명령과 출력을 함께** 인용한다. 기대값과 **다르면 그 축의 편집을 중단** 하고 불성립 사실을 `§ 12.30` 에 기록한다.
...
- [x] **AC 2 — 지점 판정표**: AC 1 (iii) 의 **트리 전용 3** + **실재 전용 7** + AC 1 (v) 의 **트리 미기재 root 항목** 각각에 대해 `in-place 수정` / `원문 보존 + 각주 부기` / `무편집` 중 하나를 판정한 표를 만든다. 각 row 는 **항목 · 축 (src 하위 / root) · 트리 서술 또는 부재 1 구 · 실재 여부 · 판정 · 근거 1 구** 6 컬럼.
...
- **AC 3 채택안 = (B)** — ASCII 블록 (21 ~ 50 행) **원문 무편집** + 블록 직후 3 축 대조 각주 blockquote **2 행** append ([directory.md](../architecture/directory.md) `wc -l` 184 → **187**, +3). (A) · (C) 는 19 · 52 행의 T-0021 시점 blueprint 선언과 자기모순 (`§ 12.15` 위반) 이라 기각, (D) 는 독자 오도 risk 최대라 기각.
- **실측 (AC 1)** — 트리 축 디렉토리 **11** + 파일 2, 실 `src/` 디렉토리 **15** / `*.module.ts` **14**, 3 축 차집합 **양쪽 8 · 트리 전용 3 (`assessment` · `scheduler` · `config`) · 실재 전용 7**, 양변 검산 `11 = 8 + 3` · `15 = 8 + 7`. root 축 tracked **26** = 트리 기재 9 + 미기재 **17**. baseline 6 값 전부 일치 (중단 지점 0).
- **audit `### 12.30` 신설** — `## 11. References` 앞 순수 append **120 행** (cap 120 충족), `^## ` **12** · `^| REQ-` **66** 불변. 변경 파일 **3** · 추가 **124 LOC** · 삭제 **0** · `git status --porcelain src/ test/ prisma/ web/` 빈 출력.
...
- **[REQ-COVERAGE-AUDIT § 12.30](../use-cases/REQ-COVERAGE-AUDIT.md) 파생 영향 8 항** 이 정본 목록 — 특히 신규 잔여 ⑦ [directory.md](../architecture/directory.md) `## 각 module 디렉토리의 표준 sub-structure` (54 ~ 79 행) sub-dir 열거 ↔ 실 `src/*/` 하위 실측 대조, 그리고 ⑤ 트리 항목 수 CI drift-guard spec 이 본 slice 의 재-stale 을 막는 유일한 축이다. 전부 후속 slice 소관 (본 slice 편집 금지).
