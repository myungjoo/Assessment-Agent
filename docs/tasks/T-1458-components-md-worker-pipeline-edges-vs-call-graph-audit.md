...
- [x] **AC 1 — 실측 선행 (날조 금지)**: 편집 전에 다음을 직접 측정해 `§ 12.56` 에 **명령과 출력을 함께** 인용한다. 기대값과 **다르면 그 축의 판정을 실측대로 뒤집는다** (Why 의 ① ~ ⑦ 은 가설일 뿐이다).
...
- [x] **AC 4 — 채택안 반영**: AC 3 채택안대로만 편집한다.
...
- **AC 2 판정** — ① `worker --> github_adapter` (80) **참** · ② `--> confluence_adapter` (81) **참** · ③ `--> llm_gateway` (82) **참** · ④ `--> db_persistence` (83) **부분참** (evaluation 계열만 `PrismaService` 직접 주입 · collection 계열 Prisma 의존 **0** → `ContributionService` 경유 간접) · ⑤ label `in-process method call` **참 (승계)** · ⑥ label `Prisma typed query` 근거 승계 **부분참**. planner 가설 ① ~ ③ 은 실측으로 확인, ④ 는 "우회 존재" 가 아니라 **collection 계열 hit 0** 이라는 다른 사유로 부분참이 됐다.
- **AC 3 채택 = (B)** (각주군 말미 append + stale 숫자 치환 2 지점). (C) 는 (viii) ⓐ **36** 지점으로 cap 초과 기각, (D) 는 `§ 12.15` append-only 위반 기각, (A) 는 탐색성 축에서 기각.
- **AC 4 반영** — components.md **267 → 273** (blockquote 5 행 + 빈 줄 1 행), in-place 정정 2 지점 (**161** 행 `194 → 205`, **197** 행 `239 ~ 243 → 245 ~ 249`). heading 재-drift **6 회 연속** 재현.
- **AC 5 ~ 6** — audit `§ 12.56` 순수 append (**5422 → 5498**, +76), `grep -c '^### 12\.'` **55 → 56**, `## ` **12 불변** · `| REQ-` **66 불변**, 허용 구간 밖 hunk **0** · 순수 삭제 **0**.

## Follow-ups

- 신규 발견 (`§ 12.56` 파생 영향 (23) 에 박제) — **`worker --> backend_api` 미표기 결선**: collection 계열이 `src/user/contribution.service.ts` `ContributionService` 를 경유해 영속하는 경로에 대응하는 edge 가 다이어그램에 없다 (역방향 **73** 행은 `§ 12.55` 가 거짓 확정). 신규 edge 추가는 다이어그램 구조 변경이라 별도 판단이 필요하다.
- 나머지는 `§ 12.56` 파생 영향 (1) ~ (22) 목록 참조 — 다음 대조 1 순위는 `%% External egress` **9** 개 (89 ~ 97 행).
