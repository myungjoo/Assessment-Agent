...
- [x] **AC 3 — `R5` 존치 조건 연동 판정 (2 ~ 3 구)**: 파일별로 `L` prefix 를 **존치 / 정정** 중 무엇으로 판정하는지 근거와 함께 적는다.
...
- [x] **AC 4 — 행별 면제 판별 후 in-place 정규화 (6 후보 행)**: 후보 **6 행** (`ADR-0005` 105 · 166 / `ADR-0006` 165 · 173 · 174 / `ADR-0030` 24 · 93 중 실측 확정분) 각각에 대해 **정정 / 면제** 를 판별하고 대조표에 남긴다.
...
- [x] **AC 5 — 결정 내용 무편집 검산 (1 구)**: `git diff` 상 3 ADR 의 변경이 **구분자 · prefix 치환뿐** 임을 확인해 적는다 — 추가 행 수 = 삭제 행 수, 각 hunk 가 후보 6 행 안에 있음, `## Decision` · `## Consequences` · `## Alternatives` 의 **문장 실질 변경 0**, ADR 의 `Status` 행 무편집.
- [x] **AC 6 — audit 절 신설**: [REQ-COVERAGE-AUDIT.md](../use-cases/REQ-COVERAGE-AUDIT.md) 에 **`### 12.79`** 를 `## 11. References` **직전** 에 신설한다 (삽입 직전 `grep -n '^## 11\. References'` 로 좌표 재실측). 구성: 위치 · 계보 (`§ 12.78` 파생 1 순위 S3 batch 2) → AC 1 선정 · 한정 소급 판정 → AC 2 census 재실측 + `§ 12.78` 집계 정정 → AC 3 `R5` 연동 판정 → **행별 대조표** (파일 · 행 · 원 표기 · 판정 · 근거 조문) → AC 5 무편집 검산 → 진척 · 한계 → 파생 영향 (목록만 — batch 3 대상 후보 포함). **절 ≤ 60 행** — 초과 예상 시 census 를 집계값 1 구로 압축하고 사유를 박제한다.
...
- **`§ 12.76 R5` 예시 갱신 후보** — 예시의 존치 파일 목록 중 `ADR-0005` 가 본 slice 로 stale (조문 본문 편집이라 `§ 12.15` append-only 와 충돌 · 별도 판단 필요).
- **S3 batch 3** — 잔여 혼용 ADR 6 파일 · en dash 단독 3 파일 중 3, 그리고 `L` 잔존 **28 ADR** + 비-ADR (`directory.md` · `integrator.md` · `reviewer.md` · `race-patterns.md` · `p3-*.md`).
- **ADR 총수 실측 차이** — 본 slice 실측 `ls docs/decisions/ADR-*.md` = **53 파일** 로 `§ 12.78` 기재 (54) 와 1 차이. 집계 base 재확인 후보 (낮음).

## 결과 요약 (2026-08-05 완료)

- **AC 1 ~ AC 3 판정** — [REQ-COVERAGE-AUDIT.md](../use-cases/REQ-COVERAGE-AUDIT.md) `§ 12.79` 신설 (**55 행** · ≤ 60 충족, `## 11. References` 직전 삽입 · 6992 → **7047 행**). 선정 근거 · 한정 소급 · mode (rule 5 **두 번째 선례**) 판정 박제.
- **AC 2 정정 2 건** — ① `§ 12.78` 의 `R5` 대상 집계 (`L[0-9]+-[0-9]+` ASCII hyphen 한정) 가 en dash `L56–63` 과 단일 행 `L59` 류를 놓쳐 실제 대상은 **2 파일이 아니라 31 파일**. ② 같은 undercount 가 본 task 후보 목록 (6 행) 에 전파돼 실측 후보는 **29 행** — 대상을 3 파일 안 29 행 전량으로 확장 (파일 경계 불변).
- **AC 3 연동 판정** — `~` hit 성격 실측 결과 `ADR-0006` 4 · `ADR-0030` 1 은 전량 수량 · 식별자 범위라 **좌표 `~` 는 3 파일 모두 0** (task 전제 정정). 그럼에도 본 slice 의 `R1` 정정이 좌표 `~` 를 유입시켜 존치 근거가 소멸하므로 (`§ 12.76` 한계 3) **3 파일 모두 `L` prefix 정정**.
- **AC 4 · AC 5 실집행** — 치환 **29 행** (`ADR-0005` 16 · `ADR-0006` 10 · `ADR-0030` 3) · 면제 (`R6` · `R7`) **0** · 범위 밖 판정 **8 좌표**. `git diff --numstat` = `16/16` · `10/10` · `3/3` (추가 = 삭제), word-diff 변경 토큰 **45 쌍 전량 구분자 · `L` prefix · 단위어 `행`** (부수 변경은 `ADR-0006`:24 조사 `가` → `이` 1 건), Decision · Consequences · Alternatives 실질 변경 **0** · `Status` 행 hunk **0**.
- **AC 7 검산** — 자기 준수 `git diff -U0 <5 파일> | grep '^+' | grep -cE '[0-9]+ *– *[0-9]+|L[0-9]+'` = **11** (0 아님 — `§ 12.79` 대조표 10 + 본 요약 1 로 전량 **원 표기 인용** 이며, 본 slice 가 신규 작성한 좌표 표기는 전량 `~` · `N 행`). `git diff --stat` = **정확히 5 파일 · +104/-38** 로 ≤ 300 LOC. `git status --short` 에 `README.md` · `CLAUDE.md` · `docs/requirements.md` · `docs/architecture/` · 다른 `docs/decisions/*` · `.claude/agents/` · `docs/LOOP.md` · `src/` **없음**. markdown 무손상 — fence **178 (짝수)** · 신설 표 컬럼 **6 일치** · heading `12.78` → `12.79` 연속 · 3 ADR `## ` heading 수 불변 (6 · 5 · 5) · 링크 수 불변 (43 · 28 · 10) · 행 수 불변 (181 · 182 · 96) · `L` · en dash 잔존 **0**. doc-only 라 `pnpm test` 불요 ([CLAUDE.md](../../CLAUDE.md) §3.2 direct doc-only 면제).
