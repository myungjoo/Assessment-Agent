...
- [x] **AC 2 — 계수 함정 ① 검산 (glob 두 개 다 불변).** slice 23 파일명에 `read` 가 **없어**
  ...
  금지**.
- [x] **AC 3 — 계수 함정 ② (재분류 0) 반영.** slice 23 의 대상 `GET /api/persons` 는 **slice 1 이 이미
  ...
  에 1 구절로 덧붙인다.
- [x] **AC 4 — 규모 축 1 → 2 route 갱신 (본 task 고유).** 부하계획 `§ 5` item 5 의 규모 민감도 단락
...
- [x] **AC 5 — PLAN `142 행` 갱신.** ① perf-spec 개수 `56` → **57**, read 경로는 **51 그대로 두되**
...
- [x] **AC 6 — 부하계획 `§ 5` item 5 본문 갱신.** "slice 22 까지 도달" 서술(`135 행`)에 slice 23
  ...
  **15 endpoint (조회 31 route)** 는 **불변** 임을 명시한다. 셈법 서술(`486 행` · `497 행` 부근)에는
  ...
  완화하지 않는다.
- [x] **AC 7 — 인벤토리 머리말만 갱신, 표·분류는 전부 불변.** `542 행` 의
  ...
  **"(slice 23 시점 확인분, T-1536 작성 → T-1546 갱신)"** 으로 고치고 머리말의 편집 전 실측 개수
  ...
  않는다** (AC 3).
- [x] **AC 8 — 오독 차단 단락 보존 + 완료 선언 0 (계수 함정 ②).** 오독 차단 단락(`623~630 행`) 의
...
- [x] **AC 9 — REQ-048 재판정 갱신 + 표 구조 보존.** `docs/requirements.md` REQ-048 행(`67 행`)의
  ...
  **status 토큰 `IN_PROGRESS` 불변**, "시각화(web) 렌더 측정 축 부재" · "실 scale 부하 미검증" ·
  ...
  파이프 개수 **8 불변** 을 확인한다.
- [x] **AC 10 — REQ-047 오독 차단.** 세 문서 어디에도 slice 23 의 **200 row 표본** 이 REQ-047 실
...
- **perf slice 24 착수 축 선정** — (A) 부류 mock spec 30 개 retire 판단 · baseline 파일 확정 +
  임계 fix · 다른 endpoint 규모 민감도 · web 렌더 측정 중 무엇을 먼저 잡을지는 planner 몫이다
  (본 task 는 결론을 문서에 적지 않았다 — Out of Scope).
- **인벤토리 (B) 0 · (C) 0 상태의 다음 갱신 트리거 정의** — 재분류가 0 인 slice 가 이어지면
  `잔여 read route 인벤토리` 머리말의 "slice N 시점 확인분" 만 갱신되는 doc-sync 가 반복된다.
  머리말 갱신 조건을 인벤토리 절 자체에 1 구절로 박제할지 별도 판단 필요.
...
- 편집 전 실측 계수 (AC 1): perf-spec **57** · `*read*` **51** · `*realdb*` **23** ·
  `*read*realdb*` **21**, `it(` 실측 **8 test**, main SHA `68d319e8` (PR #1233).
- 반영: 도메인 **15 불변** · 조회 route **31 불변** · 인벤토리 (A) **30** / (B) **0** / (C) **0**
  전부 불변 (**재분류 0**, AC 3) · mock 잔존 **30 불변** — 이번엔 파일명에 `read` 가 없어
  **피감수도 감수도 늘지 않아** `51 − 21 = 30` 이 식도 결과도 그대로다 (slice 3 에 이은 두 번째
  사례, AC 2 — T-1544 의 "둘 다 늘어 같다" 문형 미사용).
- 규모 축 **1 → 2 route** 갱신 (AC 4) — `:id/persons` + `GET /api/persons`, 목록 route 자체의
  총 row 수 20 vs 200 + 필터 선택도 (active 120 / inactive 80). 같은 자리에 **해소 아님**
  (나머지 endpoint 미측정 · 상대 비교용 소규모 · 대소 관계 wall-clock 비결정성으로 미단언) 명시.
- 완료 선언 **0 유지** (AC 8 · AC 11) — PLAN `140 행` checkbox `[ ]` · REQ-048 `IN_PROGRESS` ·
  부하계획 item 5 "본 item 은 미완" · 잔여 4 축 보존. (A) 표 · (B) · (C) · 보수 분류 단락 ·
  자체 검산 미수정 (AC 7). REQ-047 행 미수정 + 200 row 가 실 scale 부하가 아님을 3 문서에 명시
  (AC 10). REQ-048 행은 **1 행 · 파이프 8 개 유지** (AC 9).
- AC 1~12 전부 ok. 코드 변경 0 의 doc-only 라 architect · tester 미호출 (§3.2 R-110 direct 면제).
