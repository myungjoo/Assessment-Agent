...
- slice iii-b1 (T-0263): `CollectionPersistenceService` 영속화 경계 분리(`persistActivities(activities, assessmentId)`) — collect→persist 결합을 끊어 author 필터 hook 가능하게.
- slice iii-b2: `collectForPerson(person, since?): Promise<Contribution[]>` 진입 — `buildCollectionSpec`(T-0261) → 수집(`Activity[]`) → `filterActivitiesByAuthor` 적용 → `persistActivities` 영속화. `assessmentId` 주입 경계 포함.
...
- DONE 2026-06-06 (loop@AKIHA-s68 turn 6). PR-225 squash-merge `5013934`, reviewer APPROVE round 1/7 (0 BLOCKER/0 MAJOR/0 MINOR, 1 INFO[instanceKey↔service 설계 endorsed]), 4-게이트 PASS, CI green (race 미발생).
- 산출: `domain/author-filter.ts`(`filterActivitiesByAuthor` — instance 별 독립 매칭, source-type-aware) + colocated spec (10 case, 신규 파일 cov 100%). +253 LOC/2 파일 (cap 이내).
