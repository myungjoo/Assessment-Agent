...
- [x] **AC 6 — `L` 축 잔여 갱신.** `grep -lE 'L[0-9]+' docs/decisions/ADR-*.md | wc -l` 과 비-ADR 대상 `grep -lE 'L[0-9]+' docs/architecture/*.md .claude/agents/*.md | wc -l` 을 정정 후 실행해 **`L` 잔존 파일 수** 를 갱신 기록하고 `§ 12.85` AC 6 의 값 (ADR **13** · 비-ADR **14**) 과 대조. 감소분이 본 batch 처리 파일 수와 일치하는지 확인하고, 불일치 시 원인 (AC 3 면제 잔존 · 부분 이월) 을 명시. 잔존 ADR 목록을 좌표 수와 함께 열거해 batch 10 편성 근거를 남긴다.
