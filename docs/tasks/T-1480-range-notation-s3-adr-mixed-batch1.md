...
- [x] **AC 3 — 행별 면제 판별 후 in-place 정규화 (11 후보 행)**: 후보 11 행 (`ADR-0001` 4 · `ADR-0002` 2 · `ADR-0003` 5) 각각에 대해 **정정 / 면제** 를 판별하고 대조표에 남긴다.
...
- [x] **AC 4 — 결정 내용 무편집 검산**: `git diff` 상 3 ADR 의 변경이 **구분자 치환뿐** 임을 확인해 1 구 적는다 — 추가 행 수 = 삭제 행 수, 각 hunk 가 후보 11 행 안에 있음, `## Decision` · `## Consequences` · `## Alternatives` 의 **문장 실질 변경 0**, ADR 의 `Status` 행 무편집.
- [x] **AC 5 — audit 절 신설**: [REQ-COVERAGE-AUDIT.md](../use-cases/REQ-COVERAGE-AUDIT.md) 에 **`### 12.78`** 을 `## 11. References` **직전** 에 신설한다 (삽입 직전 `grep -n '^## 11\. References'` 로 좌표 재실측). 구성: 위치 · 계보 (`§ 12.77` 파생 1 순위 S3 · 첫 batch) → AC 1 한정 소급 판정 → census 표 (파일 · en dash · `~` · 혼용 여부) → **행별 대조표** (파일 · 행 · 정정 / 면제 · 근거 조문) → 진척 · 한계 → 파생 영향 (목록만 — batch 2 대상 파일 후보 포함). **절 ≤ 60 행** — 초과 예상 시 census 표를 집계값 1 구로 압축하고 그 사유를 박제한다.
- [x] **AC 6 — 자기 준수 + 무손상 · 범위 검산**: 다음을 모두 확인해 절 또는 완료 요약에 적는다.
...
- **Status: DONE** — AC 1 ~ AC 6 전량 충족. 산출은 [REQ-COVERAGE-AUDIT.md](../use-cases/REQ-COVERAGE-AUDIT.md) `§ 12.78` 신설 (**56 행**) + 혼용 ADR 3 파일 in-place 정규화.
- **AC 1** — 한정 소급 정합 **허용** 판정 (① `§ 12.75` 분해안의 사전 승인 + census 의 파일 · 행 단위 특정 ② 91 행 일괄 치환 금지선 유지 · 3 파일 밖 무편집 ③ `CLAUDE.md` §3.1 rule 5 첫 적용 · 결정 실질 무편집).
- **AC 2** — 혼용 ADR **10 파일** 재실측, `§ 12.77` 집계와 **일치 (불일치 0)**. `R5` (`L` prefix) 대상 2 파일 (`ADR-0005` · `ADR-0030`) 은 batch 2 로 이월.
- **AC 3** — 11 후보 행 판별: **정정 10 행 (치환 12 건)** · **면제 1 행** (`ADR-0003`:21, `R6` 인용 원문) · `R7` 해당 0 · `R4` 적용 0. 행 수 불변 (146 · 127 · 173), en dash 잔존 **3 = 면제 · 규약 범위 밖 판정분과 동수**.
- **AC 4** — `git diff --numstat` 추가 = 삭제 (`4/4` · `2/2` · `4/4`), Decision · Consequences · Alternatives · `Status` 행 hunk **0**.
- **AC 6** — 자기 준수 grep **15** (내역: 대조표 11 + 판별 · 내역 문단 2 + ADR 편집 행 잔존 2 — 전부 `R6` 인용 원문이거나 규약 범위 밖, 신규 좌표 표기는 전량 `~`), audit **6936 → 6992 행**, `git diff --stat` **5 파일 · +82/-17**, 범위 밖 파일 변경 0, markdown 무손상 (fence 짝수 · 표 컬럼 6 · heading 연속 · `## ` 6 · 7 · 6 불변 · 링크 14 · 23 · 12 불변). doc-only 라 `pnpm test` 불요.
