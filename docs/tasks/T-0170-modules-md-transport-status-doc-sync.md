...
status: DONE
commitMode: direct
completedAt: 2026-06-02T17:26:00+09:00
...
driverNote: t8 planner 가 본 task 를 SUPERSEDED(by ac19743)로 오판했으나 — 이는 executor 의 staged modules.md 변경을 읽고 'main 에 이미 박제됨'으로 착각한 staged-vs-merged 오독(T-0163/ac19743 는 T-0168 보다 앞서 머지돼 round-trip smoke 를 문서화할 수 없음). driver 가 origin/main:modules.md L37 을 직접 대조해 stale 문장('unit-level dispatch 까지 ... §5 미착수')이 여전히 committed 임을 확인 → T-0170 은 legitimate DONE 으로 정정, executor 의 +1/-1 정합을 commit.
---
...
- **SUPERSEDED (planner issue-still-relevant pre-check, 2026-06-02T17:30Z)** — 본 task 가 정합하려던 modules.md L37 transport 상태 문장은 이미 main `ac19743` (T-0163 DONE doc-sync) 에서 박제됐다. 현재 L37 마지막 문장은 정확히 "현 상태는 실 `globalThis.fetch` transport 배선 + 로컬 stub HTTP round-trip smoke (`test/smoke/llm-gateway-roundtrip.smoke-spec.ts`, T-0168 / PR-155 …) 까지 완료 — 잔여 = live endpoint + 실 credential (`LLM_APIKEY_ENC_KEY` + provider API key env 주입) 통합 만 §5 HITL 게이트로 deferred (Q-0016 option A)" 로 본 task 의 Acceptance Criteria 3 reality bullet 을 모두 만족. T-0170 이 인용한 stale 문장 ("unit-level dispatch 까지") 은 L37 에 더 이상 존재하지 않음 — T-0163 이 T-0168 머지 전 시점이라 stale 로 오판했으나 T-0163 본문이 이미 round-trip + §5 deferred 를 박제했음. redundant doc-sync 양산 (cron-race 안티패턴) 방지 차원에서 supersede, STATE.nextTask 비움. dependency-free backlog 소진 확정.
