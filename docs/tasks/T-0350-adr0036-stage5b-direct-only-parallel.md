...
## Result (2026-06-11T07:29Z, loop@cloud-0en1pb t6)

- ADR-0036 stage 5b 진입 박제: LOOP.md §1[2] (a2) 에 direct-only 동시 claim 조건 (두 task 모두 `commitMode: direct` + validate-claim-candidate (a)(b) PASS, 판독 불확실 시 no-op, pr-mode 는 활성 claim 0 일 때만 단독) + concurrency.md §7 게이트 충족 근거 (T-0349 실사용 fire 무사고 + incidents 4종 0) 와 5b→5c 게이트 + CLAUDE.md §10 현황 5b 동기.
- `docs/STATE.json` (`flags.maxConcurrentClaims` 1→2, `adr0036Rollout.stage5` 5b-active) 은 §9 single-writer 에 따라 driver 가 직접 편집. ADR-0036 본문 변경 0, src/ 변경 0, 새 dependency 0.
