...
- **S3 (시급성 낮음)** — `~`·`–` 혼용 파일 정규화. 규모는 `§ 12.76` 파생 (2) 실측 승계 (ADR 군 **10 파일** · 5 문서군 **17 파일**) 라 cap (5 파일) 초과 → ADR 군 안에서 3 파일씩 재분할 필요.
- **rule 5 회색지대 예시 보강** — "비-결정 / 결정" 경계가 문언 판정이라 회색지대 (예: ADR Consequences 의 표현 정리) 판정 선례가 **2 건 이상** 쌓이면 §3.1 rule 5 에 예시 1 행을 덧댄다. 현 선례는 `§ 12.73` · `§ 12.74` 집행분뿐이라 이번엔 미실행.
- **`R2` · `R3` 병합 후보 (낮음)** — `§ 12.76` 파생 (3) 그대로 이월. 조문 재설계라 pointer slice 에서는 착수 불가.
- **anchor 좌표계 이행 FU14 (낮음 유지)** — 이행 시 신설 소절의 요약 3 점 (`R1` · `R4` · `R5`) 도 재검토 대상. 관계 언급까지만.

## 완료 요약 (2026-08-05)

- **AC 1** — `CLAUDE.md` §3.1 판정 규칙에 **rule 5** 를 rule 4 직후 (**142 행**) append. 3 요소 충족 — ① 규범 (`docs/decisions/*` · `docs/architecture/*` 의 **비-결정 수정** = `direct`) ② 경계 (결정 내용 자체 = `pr`) ③ 근거 좌표 (`§ 12.72` AC 3). 기존 rule 1 ~ 4 는 **무편집** (diff 에 삭제 hunk 0) 이고, "새 판정이 아니라 `§ 12.73` ~ `§ 12.76` 4 slice 가 `direct` 로 집행한 **선례의 명문화**" 를 조문 안에 1 구 박제.
- **AC 2** — §12 에 소절 **`### 범위 좌표 표기 (행 범위)`** 를 `### 혼합이 자연스러운 경우` 직후 · `### 과거와의 호환` 직전 (**452 ~ 456 행**) 신설. bullet **3** (상한 5 미만) — 정본 pointer (`§ 12.76` 의 `R1` ~ `R7` · 인용 형태 `§ 12.76 R3` · 상대 경로 링크 1) / 실무 요약 3 점 (`R1` · `R4` · `R5`) / 적용 범위 · 발효. **SSOT 검산** — 조문 규범 문장 · 예시 · 근거 복제 **0**, 요약 3 점은 조문 번호 참조를 동반한 축약이며 어긋나면 `§ 12.76` 우선임을 첫 bullet 에 명시.
- **AC 3** — 신규 추가분 대상 `git diff -U0 <3 파일> | grep '^+' | grep -cE '[0-9]+ *– *[0-9]+|L[0-9]+-[0-9]+'` = **0**. `R1` · `R4` · `R5` 전부 충족이라 산출물 내 정정 **0 건**. 예외 (`R6` 병기 화법으로 인용된 타 문서 원 표기) 는 위반이 아니나 본 slice 는 그런 인용을 새로 들이지 않아 예외 적용도 **0**.
- **AC 4** — `### 12.77` 을 `## 11. References` 직전 (재실측 좌표 **6888 행**) 에 신설. 구성 = 위치 · 계보 → 편집 2 지점 대조표 (5 컬럼 · 2 행) → SSOT 판정 → AC 3 결과 → 진척 · 한계 · 파생 영향. 절 길이 **34 행 ≤ 55** (압축 불요).
- **AC 5** — `wc -l` audit **6901 → 6936 (+35)** · `CLAUDE.md` **465 → 472 (+7)** · `git diff --stat` **3 파일 · +54/-1 LOC** (cap 300 내 · 삭제 1 은 본 task 파일의 Follow-ups placeholder 교체) · `git status --short` 에 `README.md` · `docs/requirements.md` · `docs/architecture/` · `docs/decisions/` · `.claude/agents/` · `docs/LOOP.md` · `src/` **없음** · audit fence **178 (짝수)** · 신설 표 컬럼 5 로 4 행 일치 · heading `12.76` → `12.77` 순번 연속 · `CLAUDE.md` `### ` **20 → 21 (+1)** · `## ` **15 불변**. doc-only 라 `pnpm test` 불요 ([CLAUDE.md](../../CLAUDE.md) §3.2 direct doc-only 면제).
