...
- [ ] **(DEFERRED) overwrite / 이미 영속화된 평가문 재평가** (replace existing — ADR-0033 reeval/reset-and-recreate 경로) — Q-0032 결정으로 v1 은 first-write-wins read-through(ADR-0037 §Decision3), overwrite 는 별도 후속 ADR/task 로 plan 만 — **오너 승인 2026-07-07 (Q-0051): DEFERRED 해제. overwrite/재평가 재개 승인, 후속 ADR(ADR-0033 경로) 선행. 권장 착수 1순위(옵션 5).**
