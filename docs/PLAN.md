...
- [ ] **(DEFERRED) overwrite / 이미 영속화된 평가문 재평가** (replace existing — ADR-0033 reeval/reset-and-recreate 경로) — Q-0032 결정으로 v1 은 first-write-wins read-through(ADR-0037 §Decision3), overwrite 는 별도 후속 ADR/task 로 plan 만 — **오너 승인 2026-07-07 (Q-0051): DEFERRED 해제. overwrite/재평가 재개 승인, 후속 ADR(ADR-0033 경로) 선행. 권장 착수 1순위(옵션 5).** — mechanism ADR 박제 완료: [ADR-0053](decisions/ADR-0053-overwrite-reeval-mechanism.md) ACCEPTED (T-0804, PR #718 merge 667ad68c) — 명시 mode 시에만 reset-and-recreate(ADR-0033 §D3) 재사용, 무플래그 default 는 first-write-wins 보존(ADR-0037 §D3 조건분기 supersede), 새 schema migration 0. 구현 chain 은 ADR-0053 §Follow-ups(dependency-free 순수 조각→orchestrator→route→e2e).
