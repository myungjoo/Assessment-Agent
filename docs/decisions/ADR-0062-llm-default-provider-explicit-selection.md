...
- **실패 매핑** — `400` (DTO 위반: `llmProviderConfigId` 누락 / non-string / 빈 문자열 / `forbidNonWhitelisted` extra 키), `404` (존재하지 않는 `llmProviderConfigId` — 슬롯 upsert 가 FK 위반 `P2003` 을 던지므로 `NotFoundException` 으로 변환한다). **여기서의 `P2003` 은 `delete` 의 `P2003` 과 방향이 반대** 라는 점을 T-1864 / T-1865 가 주의해야 한다: `delete` 의 것은 "이 config 를 누가 쓰고 있다" (→ `409`) 이고, 슬롯 upsert 의 것은 "가리키려는 config 가 없다" (→ `404`) 다. 같은 코드가 호출 지점에 따라 다른 status 로 매핑되므로 변환은 service 의 각 메서드 안에서 하고 공용 helper 로 합치지 않는다. 이미 기본인 config 를 다시 지정하는 재호출은 `200` (upsert 라 idempotent — `PUT` 시멘틱 정합).
