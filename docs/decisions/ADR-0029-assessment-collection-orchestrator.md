...
- **REQ-031**([docs/requirements.md](../requirements.md) 58 행) — 재수집 중복 방지 + 최근 1주 재수집 OK. dedup + incremental since 결정이 backbone.
...
- `AssessmentCollectionModule` 은 `GithubModule` / `ConfluenceModule` / `UserModule` 을 import 한다(import 방향 = collection → adapter(GithubModule/ConfluenceModule) + collection → user, leaf adapter 와 user domain 이 collection 을 모르는 단방향 유지 — [modules.md acyclic 검증](../architecture/modules.md) 보존). **실 구현 정합(T-0254, reviewer MINOR-1 closeout)**: `Contribution` 영속화는 `PersistenceModule` 직접 import 가 아니라 `UserModule` 이 export 하는 `ContributionService.create` 를 경유하므로, 실 DI edge 는 `collection → user`(PrismaService 는 `UserModule` 의 repository 가 `@Global` 로 주입받음)다. 본 ADR 초안의 "collection → persistence" 표기를 실 edge 인 "collection → user" 로 정정한다. `PermissionDeniedEmitter` port 는 기존 adapter 가 이미 보유하므로 collection module 은 emit 을 트리거만 하고 직접 영속화하지 않는다.
