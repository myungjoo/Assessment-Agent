...
### Decision §2 — double-write 경계: collection-side(Contribution FK persist) ↔ evaluation-side(EvaluationResultPersistService) — Admin full 경로 한정, User ephemeral 은 write 0 (PROPOSE, human review)

> **본 §2 는 [Q-0031](../STATE.json) 이 "architect 가 ADR 에서 PROPOSE → 사용자가 ADR PR 에서 검토" 로 지정한 결정이다.** 아래는 architect 의 PROPOSE 이며 사용자 ADR PR 검토 대상이다.

**PROPOSE: Admin full 경로의 두 write — collection-side(`collectForPerson` 내부 `persistActivities` 의 Contribution FK persist)와 evaluation-side(`EvaluationResultPersistService` 의 Assessment/Contribution write) — 의 중복을 회피하기 위해, bridge 는 collection-side persist 를 우회하고 평가 영속화 단일 경로로 일원화한다. 즉 bridge 의 수집 단계는 persist-free `collectActivities`(in-memory `Activity[]`) 를 source 로 쓰고(§Decision 4), 영속화는 evaluation-side(ADR-0033 reset-and-recreate) 가 단독 책임진다. User ephemeral 경로는 어느 write 도 타지 않는다(§Decision 1).**

경계 PROPOSE(human review 대상):
...
- **PROPOSE(채택안 A — 평가 영속화 일원화)**: bridge 는 **collection-side persist 를 타지 않는다**. 수집은 `collectActivities`(persist-free orchestrator, in-memory `Activity[]`) 까지만 수행하고, `filterActivitiesByAuthor`(author 귀속 필터, 순수 함수)를 in-memory 로 적용한 뒤 그 `Activity[]` 를 `evaluateActivities` 에 넘긴다. **영속화는 evaluation-side 한 곳**(`EvaluationResultPersistService`, ADR-0033 reset-and-recreate)에서만 일어난다. 이로써 double-write 가 single-write 로 축약돼 중복/충돌 표면이 사라진다. collection-side `persistActivities` 는 기존 manual collection trigger(ADR-0031 endpoint)의 책임으로 남고 bridge 는 그것을 재사용하지 않는다(코드 변경 0 — 단지 호출하지 않을 뿐).
...
### Decision §3 — 동시 호출 idempotency: 같은 (personId, period, scope, periodStart) concurrent bridge 호출의 직렬화 (PROPOSE, human review)

> **본 §3 도 [Q-0031](../STATE.json) 이 PROPOSE → ADR PR 검토 로 지정한 결정이다.**

**PROPOSE: 같은 `(personId, period, scope, periodStart)` 좌표에 대한 동시 bridge 호출(Admin full 경로)은 evaluation-side 의 reset-and-recreate `$transaction` + `Assessment.@@unique` 위에서 직렬화한다. 동시성으로 인한 중복 row 는 `@@unique` 위반(P2002)이 차단하며, bridge 는 이를 `ConflictException(409)` 으로 전파한다(ADR-0033 정합). User ephemeral 경로는 write 0 이라 idempotency 무관(부수효과 없는 순수 산출).**

idempotency PROPOSE(human review 대상):

- **직렬화 backbone = `$transaction` + `@@unique`**: ADR-0033 §3 의 reset-and-recreate 는 단일 `prisma.$transaction(delete-if-exists → create)` 로 묶인다. 두 동시 호출이 같은 좌표로 들어오면 — DB transaction 격리 + `@@unique([personId, period, scope, periodStart])` 가 한쪽만 성공시키고 다른 쪽은 P2002 로 reject 한다. bridge 는 이 P2002 를 ADR-0033 의 기존 매핑(`ConflictException(409)`)으로 전파한다 — 새 동시성 제어(application lock / advisory lock) **도입 0**, 기존 DB-level 제약 재사용.
- **fill / reeval 모드와의 관계**: ADR-0033 의 `PersistMode = "fill" | "reeval"` 을 bridge 가 그대로 재사용한다(새 enum 0). `fill` 동시 호출 = 한쪽 create 성공 + 다른 쪽 no-op/P2002(idempotent — row 수 불변). `reeval` 동시 호출 = delete→create 가 transaction 으로 직렬화돼 마지막 1 row 만 남는다(중복 0). 어느 모드도 동시성으로 중복 row 를 만들지 않는다.
- **수집 단계의 동시성**: bridge 의 수집(`collectActivities`)은 persist-free in-memory 라(§Decision 2) 동시 수집이 DB 상태를 경쟁시키지 않는다 — idempotency 책임은 evaluation-side persist 한 곳으로 국소화된다(경쟁 표면 최소화).
- **검증 경계**: 동시 호출 idempotency 는 impl slice 의 e2e(실 PostgreSQL, ADR-0004)에서 "같은 좌표 동시 2 호출 → 최종 row 1 개 + 한쪽 409 또는 양쪽 동일 결과" 를 negative/concurrency test 로 검증한다(R-112 negative cases 충분 cover — 동시성 경계도 cover).
...
- **double-write 가 single-write 로 축약(§Decision 2 PROPOSE)** — bridge 가 collection-side persist 를 우회하고 evaluation-side 일원화하므로 중복/충돌 표면이 사라진다(human review 후 확정).
...
- **idempotency 가 기존 DB 제약 재사용으로 보장(§Decision 3 PROPOSE)** — 새 동시성 제어 도입 0, `$transaction` + `@@unique`(ADR-0033) 재사용으로 동시 호출 중복을 차단.
...
- **§Decision 2/3 는 PROPOSE 상태 — human review 의존** — double-write 일원화 방향(§2)과 idempotency 직렬화 방식(§3)은 architect PROPOSE 이며 사용자 ADR PR 검토로 확정된다(Q-0031). 검토에서 방향이 바뀌면 impl chain 의 Acceptance 가 조정돼야 한다(의도된 ADR-first 흐름 — 코드 전 경계 확정).
...
(ADR-0037 ACCEPTED 후 planner 가 dependency chain 으로 분해 — 각 ≤300 LOC / ≤5 파일 + R-112 4 종(+ negative cases 충분 cover). dependency 순서.)
...
- [ ] **slice 2 — period→collection→evaluate orchestration bridge service** (`commitMode: pr`) — `collectActivities`(persist-free) → `filterActivitiesByAuthor` → `evaluateActivities` compose + Admin persist(`EvaluationResultPersistService`) / User ephemeral(persist 0) 구조적 분기 + mocked-LLM/mocked-collection unit(R-112 4 종 + negative: User 호출 시 persist 미호출 / 빈 수집 / collection throw 흡수 / 모드 분기).
- [ ] **slice 3 — controller endpoint** (`commitMode: pr`) — 예: `POST /api/assessment-evaluation/period` — DTO 수신 + orchestration 위임 + Admin/User 응답(Admin 영속 식별자 / User ephemeral 결과) + colocated controller unit(orchestrator mock).
...
- [ ] **slice 5 — e2e** (`commitMode: pr`, ADR-0004 실 PostgreSQL) — Admin full persist round-trip(평가 결과 영속 검증) + **User ephemeral DB-write-0 검증**(호출 후 row 증가 0) + 동시 호출 idempotency(같은 좌표 동시 2 호출 → 최종 row 1 + 409 또는 동일 결과).
