...
status: ACCEPTED
date: 2026-09-07
relatedTask: [T-1948, T-1949, T-1950, T-1951, T-1952, T-1953, T-1954, T-1955]
...
**ACCEPTED**. `§ Follow-ups` (a)~(c) 의 구현 chain 이 **전량 머지** 돼 본 ADR 의 결정이 main 에 안착했다 — (a) 판별 helper 신설 + Confluence mapper 배선 (T-1949, PR #1530 → merge `c93fb3cf`) · GitHub mapper 배선 (T-1950, PR #1531 → merge `695c3595`), (b) detection 신호 helper 신설 (T-1951, PR #1532 → merge `e70a762e`) · signals container 7 번째 필드 배선 (T-1952, PR #1533 → merge `a98ad3df`), (c) 상향 adjuster helper 신설 (T-1953, PR #1534 → merge `aee16428`) · adjustments pipeline step (9) 배선 (T-1954, PR #1535 → merge `c204dd88`) — 각 chain 이 cap 준수를 위해 2 slice 로 갈려 총 **6 slice** 다. `§ Decision 1~5` 의 결정 축 (신호 산출 경계 · 판별 규칙 · 부여 축 · LLM 관계 · 영속 불변) 은 전량 안착했다.

**잔여 항목은 (c) 의 상향 건수 관측 로그 1 건뿐** 이다 — `evaluation-adjustments-pipeline.ts` · `evaluation-algorithm-research-adjust.ts` 전수에 `Logger` · `console` 참조가 0 이라 과잉 발동을 수치로 인지할 경로가 아직 없다. 이는 `§ Consequences` 의 오탐 완화 (iv) 보조 수단이고 결정 축이 아니라 ACCEPTED 를 막지 않으며, 후속 slice 로 이월한다 ([T-1955](../tasks/T-1955-adr0064-algorithm-research-doc-sync-req019-rejudge.md) `§ Follow-ups`). `§ Follow-ups` 의 "(확장 지점, task 아님)" 은 task 가 아니라 후속 ADR 을 선행 조건으로 하는 재검토 항목이라 미착수 그대로 남는다.
