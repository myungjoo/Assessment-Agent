...
status: ACCEPTED
date: 2026-09-07
relatedTask: [T-1948, T-1949, T-1950, T-1951, T-1952, T-1953, T-1954, T-1955, T-1956, T-1957]
...
**ACCEPTED**. `§ Follow-ups` (a)~(c) 의 구현 chain 이 **전량 머지** 돼 본 ADR 의 결정이 main 에 안착했다 — (a) 판별 helper 신설 + Confluence mapper 배선 (T-1949, PR #1530 → merge `c93fb3cf`) · GitHub mapper 배선 (T-1950, PR #1531 → merge `695c3595`), (b) detection 신호 helper 신설 (T-1951, PR #1532 → merge `e70a762e`) · signals container 7 번째 필드 배선 (T-1952, PR #1533 → merge `a98ad3df`), (c) 상향 adjuster helper 신설 (T-1953, PR #1534 → merge `aee16428`) · adjustments pipeline step (9) 배선 (T-1954, PR #1535 → merge `c204dd88`) · 상향 건수 관측 로그 (T-1956, PR #1536 → merge `e0a0dff0`) — 각 chain 이 cap 준수를 위해 slice 로 갈렸고 (c) 만 3 slice 라 총 **7 slice** 다. `§ Decision 1~5` 의 결정 축 (신호 산출 경계 · 판별 규칙 · 부여 축 · LLM 관계 · 영속 불변) 은 전량 안착했다.

**(a)~(d) 전 항목이 완료** 돼 chain 이 닫혔다 — 마지막 잔여였던 (c) 의 상향 건수 관측 로그를 [T-1956](../tasks/T-1956-algorithm-research-uplift-count-log.md) (PR #1536 → merge `e0a0dff0`) 가 배선했다. 실측 좌표는 [evaluation-adjustments-pipeline.ts](../../src/assessment-evaluation/domain/evaluation-adjustments-pipeline.ts) `98 행` `Logger` import · `130 행` `EvaluationAdjustmentsPipeline` logger 인스턴스 · `374~387 행` 상향 건수 산출 + 0 건 억제 조건부 `logger.log` 다 — 즉 종전의 "두 파일에 `Logger` · `console` 참조 0" 서술은 **반증됐다**. 이로써 `§ Consequences` 의 오탐 완화 (iv) 가 충족돼 과잉 발동을 수치로 인지할 경로가 열렸고, 로그는 건수 + 고정 한국어 문구만 남겨 unitId · author · narrative · 원본 title 유출이 0 이다 (REQ-032 정합). `§ Follow-ups` 의 "(확장 지점, task 아님)" 은 task 가 아니라 본 ADR 을 augment 하는 후속 ADR 을 선행 조건으로 하는 재검토 항목이라 본 chain 의 완료 판정 대상이 아니다.

작성 시점 (제안 단계) 의 성격은 기록으로 보존한다 — 본 ADR slice 자체는 **정책만 박제** 했고 코드를 1 LOC 도 만들지 않았다 (그 slice 의 diff 는 본 문서 1 개뿐이고 `src/` · `web/` · `test/` · `prisma/` · `package.json` · `.github/workflows/` 변경이 **0** 이었다). 파생 신호 산출 · detection helper · uplift adjuster · pipeline 배선을 전부 `§ Follow-ups` 로 이월한 것이 그 선택이다 ([ADR-0063](ADR-0063-commit-content-fingerprint-dedup.md) · [ADR-0062](ADR-0062-llm-default-provider-explicit-selection.md) 의 doc-only ADR 선례 동형). 본 문서의 ACCEPTED 승격 · [requirements.md](../requirements.md) `38 행` REQ-019 재판정 · [PLAN.md](../PLAN.md) `103 행` 품질 분류 bullet 갱신은 T-1955 가 수행했다 ((d)).
