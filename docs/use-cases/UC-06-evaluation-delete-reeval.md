...
### 6.5 shipped 구현 참조 (P7 REQ-041)

> 본 subsection 은 P2 설계기의 conceptual 서술 (§5 sequence diagram / §9 mapping) 을 **변경하지 않고**, 이후 P7 stream 에서 실제 shipped 된 REQ-041 trigger 1 (최근 N 일 manual delete) 구현 사실만 박제하는 addendum 이다. §5 의 conceptual endpoint (`DELETE /api/assessments` 등) 는 P2 설계 의도대로 그대로 두고, 구체 endpoint 계약은 [api.md](../architecture/api.md) 가 권위다.

본 UC §3 trigger 1 (최근 N 일 manual delete) 의 실 진입점은 `POST /api/schedules/recent-deletion/:personId` REST endpoint ([T-0428](../tasks/T-0428-uc-06-recent-deletion-manual-endpoint.md), PR #346) 로 shipped 됐다 — Admin+ RBAC, `RecentDeletionDto` (`instants` ISO `string[]` + 선택 `days`) body, 202 + `RecentDeletionRunResult` `{personId, deletedCount, recollected}` 응답. 내부적으로 `RecentDeletionRunnerService.runRecentDeletion` ([T-0427](../tasks/T-0427-uc-06-recent-deletion-runner.md), PR #344) 이 삭제 → 같은 기간 재수집 (`CollectionTriggerService` 위임) 을 orchestrate 한다 — §5 의 "삭제 + 비어있는 구간 자동 재수집" invariant 의 실 구현. 단 실 deleter (`RECENT_DELETION_DELETER`) provider 바인딩은 schema/repository 게이트 동반 별도 sub-slice 로 **아직 미shipped** 라 현재 `deletedCount:0` 이 기본이다 (삭제 자체는 no-op, 재수집 위임만 동작). 정확한 계약은 [api.md §5](../architecture/api.md) 표 ([T-0430](../tasks/T-0430-uc-06-recent-deletion-api-doc.md)) 참조.
...
- [docs/architecture/api.md §5](../architecture/api.md) — §6.5 addendum 의 source. REQ-041 trigger 1 의 shipped 계약 (`POST /api/schedules/recent-deletion/:personId` recent-deletion 행) 권위. UC §5 conceptual endpoint 대비 실 구현 매핑.
