...
| --- | --- | --- | --- | --- | --- | --- |
| UC-01 | 평가 실행 (자동 cron + manual trigger) | Scheduler / Admin | Scheduler, Worker, GitHub Adapter, Confluence Adapter, LLM Gateway, DB Persistence | SchedulerModule, AssessmentModule (실 shipped 는 [modules.md](../architecture/modules.md) 39 행 정본 기준 미shipped placeholder — 본 UC 의 수집 축 = AssessmentCollectionModule · 평가 축 = AssessmentEvaluationModule, 같은 문서 40 · 41 행. 병기는 부기라 [UC-01](UC-01-evaluation-execution.md) §9 의 `6 module` 산정 불변), GithubModule, ConfluenceModule, LlmModule, PersistenceModule | REQ-005, REQ-006, REQ-007, REQ-014, REQ-015, REQ-039, REQ-040, REQ-049, REQ-051, REQ-052, REQ-053, REQ-054, REQ-055 | DONE |
| UC-02 | 평가 결과 조회 / sort / filter / 시계열 | User / Admin | Web UI, Backend API, DB Persistence | WebModule, AssessmentModule (조회·sort·filter·시계열은 [modules.md](../architecture/modules.md) 39 행 정본이 본 module 의 잔여 책임으로 박제하나 아직 **미shipped placeholder** — 실 코드/AppModule 등록 0. 병기는 부기라 [UC-02](UC-02-evaluation-query.md) §9 의 `4 module` 산정 불변), AuthModule, PersistenceModule | REQ-038, REQ-042, REQ-046, REQ-048 | DONE |
...
| UC-06 | 평가 결과 manual delete + 재수집 | Admin | Web UI, Backend API, DB Persistence | WebModule, AssessmentModule ([modules.md](../architecture/modules.md) 39 행 정본 기준 미shipped placeholder — REQ-041 최근 N일 delete→재수집의 실 shipped 축은 SchedulerModule ④ + AssessmentCollectionModule, 같은 문서 42 행. 병기는 부기라 [UC-06](UC-06-evaluation-delete-reeval.md) §9 의 `4 module` 산정 불변), AuthModule, PersistenceModule | REQ-037, REQ-041, REQ-045 | DONE |
| UC-07 | Export / Import / Backup / Restore | Admin | Web UI, Backend API, DB Persistence | WebModule, AssessmentModule ([modules.md](../architecture/modules.md) 39 행 정본 기준 미shipped placeholder — export/import 실 shipped 코드의 module 귀속은 정본 12 표에 아직 미기재라 후속 slice 소관; **T-1428 문서↔코드 대조** — 실 shipped 축은 `ExportModule` (`src/export/export.module.ts`) · `ImportModule` (`src/import/import.module.ts`) 이고 [modules.md](../architecture/modules.md) **47** 행 T-1425 각주가 이 둘을 "정본 표 미기재 실 shipped module" 로 이미 기록했다 (같은 문서 48 행 경계상 두 명은 정본 12 어휘 밖 — 25 행 계약을 지켜 컬럼 값으로 승격하지 않는 **사실 기록** 이며, 정본 표 row 신설은 ADR 게이트 소관). 병기는 부기라 [UC-07](UC-07-export-import.md) §9 의 `4 module` 산정 불변), AuthModule, PersistenceModule | REQ-030, REQ-032, REQ-045 | DONE |
| UC-08 | 권한 부족 인식·통지 (GitHub / Confluence) | System (GitHub Adapter / Confluence Adapter emit → Web UI 표시) | GitHub Adapter, Confluence Adapter, Backend API, Web UI | GithubModule, ConfluenceModule, AssessmentModule ([modules.md](../architecture/modules.md) 39 행 정본 기준 미shipped placeholder — 권한 부족 record 영속화·audit 조회의 실 shipped 는 PermissionDeniedRecordModule, 같은 문서 37 행. 병기는 부기라 [UC-08](UC-08-permission-denied.md) §9 의 `4 module` 산정 불변), WebModule | REQ-008, REQ-016 | DONE |
...
Scheduler (`@nestjs/schedule` in-process cron) 의 cron 시각 도달 또는 Admin 의 manual trigger 가 발화. AssessmentModule (Worker 흡수 — [modules.md](../architecture/modules.md) "Components ↔ Modules mapping") 의 평가 파이프라인 service 가 3 GitHub instance + Confluence + LLM gateway 를 거쳐 commit / 문서 별 기여도·난이도·양·LLM 평가문을 생성, DB Persistence (raw 미저장, REQ-032 schema-level 강제) 에 저장. 후속 task 가 평가 파이프라인의 단계별 sequence 와 실패 / 재시도 / 부분 성공 경로를 분해. **T-1428 문서↔코드 대조 부기** (위 산문은 T-0019 시점 기록이라 원문 무편집) — 위 `AssessmentModule` 은 [modules.md](../architecture/modules.md) **39** 행 정본 기준 **미shipped placeholder** (`src/assessment/` 부재) 이고, 본 UC 의 실 shipped 축은 수집 = `AssessmentCollectionModule` (`src/assessment-collection/`, 같은 문서 **40** 행) · 평가 = `AssessmentEvaluationModule` (`src/assessment-evaluation/`, **41** 행) 이다. 병기는 부기라 §2 표 31 행 · [UC-01](UC-01-evaluation-execution.md) §9 의 `6 module` 산정 불변. → [UC-01-evaluation-execution.md](UC-01-evaluation-execution.md)
...
GitHub Adapter / Confluence Adapter 가 외부 시스템의 4xx 응답을 감지하면 PermissionDeniedEvent 를 emit. AssessmentModule 이 event 를 받아 DB 에 권한 부족 기록을 남기고, Web UI 가 사용자 (REQ-008 — GitHub) 및 관리자 (REQ-016 — Confluence) 모두 인식할 수 있도록 표시. 시스템 자체가 actor 인 use case — 사람이 직접 trigger 하지 않으나 사람이 인식·대응할 수 있어야 함. **T-1428 문서↔코드 대조 부기** (위 산문은 T-0019 시점 기록이라 원문 무편집) — 위 `AssessmentModule` 은 [modules.md](../architecture/modules.md) **39** 행 정본 기준 **미shipped placeholder** (`src/assessment/` 부재) 이고, 권한 부족 record 영속화·audit 조회의 실 shipped 축은 `PermissionDeniedRecordModule` (`src/permission-denied/`, 같은 문서 **37** 행) 이다. 병기는 부기라 §2 표 38 행 · [UC-08](UC-08-permission-denied.md) §9 의 `4 module` 산정 불변. → [UC-08-permission-denied.md](UC-08-permission-denied.md)
