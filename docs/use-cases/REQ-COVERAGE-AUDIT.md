...
### 12.50 components.md `## Component table` **Scheduler** row ↔ 실 `src/scheduling/**` 인벤토리 · `ADR-0003 §3` 재측정 · REQ 대조 — 원문 보존 + 각주 1 블록 (T-1452), 표 7 row 완결

> **본 절의 위치 · 계보** — `§ 12.49` (T-1451) 이 `Confluence Adapter` row 를 닫으며 파생 영향 (1) 로 **"다음 slice 1 순위 = `Scheduler` row 단독 (표 완결)"** 을 지목했고, 본 절이 그 지목을 집행해 **`## Component table` 전 7 row 대조를 완결** 한다. **계보** — `T-1430` ~ `T-1435` (directory.md) → `T-1436` ~ `T-1444` (deployment.md) → `T-1445` (`## 개요`) → `T-1446` (row 1) → `T-1447` (row 2 ~ 3) → `T-1448` (row 4) → `T-1449` (row 5) → `T-1450` (row 6) → `T-1451` (row 7 = `Confluence Adapter`) → **`T-1452` (본 절 — 표 축 일곱 번째 slice, 마지막 row `Scheduler`)**. pointer 축이 `ADR-0003 §3` 이라 `§ 12.47` ~ `§ 12.49` 가 3 회 승계한 **`§4` 판정을 재승계할 수 없어 본 절이 `§3` 좌표를 직접 재측정** 했다. 판정 enum 은 선행 절과 같은 `참 / 부분참 / 거짓` 3 값이고, 측정은 전부 read-only `find` · `grep` · `sed` · `wc` · `git` 이며 **cron 실발화 · live spec 실행 0** 이다. token · 실 credential 인용 **0** (CLAUDE.md §9).

#### 실측 (AC 1 — 편집 전 측정, 명령과 출력)

- (i) 좌표 — `grep -n '^#\{1,3\} ' docs/architecture/components.md` → `1:# Component view` / `5:## 개요` / `22:## Deployment 컨텍스트` / `28:## Component diagram` / `115:## Component table` / `168:## GitHub Adapter — 3 instance 묶음 vs 분리 결정` / `200:## Contracts` / `226:## References`. **task 가 예고한 115 · 126 · 168 좌표는 전부 유효** 했고 `sed -n '126p'` 가 `| **Scheduler** | @nestjs/schedule 기반 in-process cron + manual trigger 단일 진입점. cron 표현식은 DB 에 저장되어 process restart 후에도 복원. Admin UI 의 cron 갱신은 SchedulerRegistry 의 dynamic 등록. manual trigger 는 Backend API endpoint 가 동일 service 메서드 호출 — duplication 0. | 입력: 시간 trigger (cron) 또는 Backend API endpoint 의 manual trigger. 출력: Worker 또는 Backend API service 메서드의 in-process invocation (@Cron decorator handler). | REQ-039 (Admin cron 주기 지정), REQ-040 (Admin manual trigger) | ADR-0003 §3 (@nestjs/schedule in-process) / P7 Scheduling & ops task |` 를 반환했다.
- (i-b) **claim 이분** — 검증 가능 **11** 개: ① in-process cron 근거 ② 단일 진입점 ③ DB 저장 + restart 복원 ④ SchedulerRegistry dynamic 등록 ⑤ manual trigger duplication 0 ⑥ 입력 2 종 ⑦ 출력 대상 (Worker / Backend API service 메서드) ⑧ `@Cron` decorator handler ⑨ `ADR-0003 §3` 좌표 ⑩ `P7 Scheduling & ops task` phase pointer ⑪ REQ-039 · REQ-040. **검증 불가** — "in-process" 의 런타임 동일 process 실증 (실행 금지), "Admin UI 의 cron 갱신" 에서 UI 주체 서술 (`web/` 측 배선은 본 row 판정 대상 밖), grep 근사의 전수성.
- (ii) 인벤토리 · 단일 진입점 — `find src -ipath '*schedul*' -name '*.ts' -not -name '*.spec.ts' | sort` → **14** 개 (`assessment-backfill-checker.service.ts` · `backfill-plan.ts` · `backfill-runner.service.ts` · `backfill.controller.ts` · `cron-schedule.controller.ts` · `cron-schedule.service.ts` · `deletion-window-select.ts` · `dto/recent-deletion.dto.ts` · `dto/upsert-cron-schedule.dto.ts` · `recent-deletion-plan.ts` · `recent-deletion-runner.service.ts` · `recent-deletion-window.ts` · `recent-deletion.controller.ts` · `scheduling.module.ts`) — **task 예상 14 와 일치**. `grep -rn 'export class' src/scheduling/*.ts | grep -v spec` → **8** 개 (`assessment-backfill-checker.service.ts:29 AssessmentBackfillChecker` · `backfill-runner.service.ts:59 BackfillRunnerService` · `backfill.controller.ts:49 BackfillController` · `cron-schedule.controller.ts:78 CronScheduleController` · `cron-schedule.service.ts:44 CronScheduleService` · `recent-deletion-runner.service.ts:71 RecentDeletionRunnerService` · `recent-deletion.controller.ts:73 RecentDeletionController` · `scheduling.module.ts:99 SchedulingModule`) = **controller 3 · service / runner 4 · module 1** — task 예상과 일치.
- (iii) in-process · dynamic 등록 — `grep -rn '@nestjs/schedule' src --include='*.ts' | grep -v spec | head -5` → `src/app.module.ts:14 import { ScheduleModule } from "@nestjs/schedule";` · `src/scheduling/cron-schedule.service.ts:1` 주석 · (나머지 3 hit 은 `src/assessment-evaluation/` 주석). `grep -rn '@Cron(' src --include='*.ts' | grep -v spec` → **0 hit**. `grep -n 'SchedulerRegistry\|addCronJob\|CronJob\|CronTime' src/scheduling/cron-schedule.service.ts | head -8` → **1** 행 `CronScheduleService — SchedulerRegistry(@nestjs/schedule) 위의 얇은 wrapper` · **4** 행 `정적 빌드타임 @Cron 데코레이터가 아니라 SchedulerRegistry 동적 등록이라 재배포 없이` · **6** 행 `ScheduleModule.forRoot() 로 SchedulerRegistry 가 전역 주입된다(빈 registry 초기 계약)` · **17** 행 `import { SchedulerRegistry } from "@nestjs/schedule";` · **18** 행 `import { CronJob, CronTime } from "cron";`. `sed -n '45,58p' src/app.module.ts` → **49 ~ 51** 행 `ScheduleModule.forRoot() (T-0412 추가) — ADR-0042 §Decision 2. … 현 단계는 동적 registry 활성화만 — 정적 @Cron job 정의 0 (후속 ③ scheduler service 책임).`
- (iv) DB 저장 + restart 복원 — `grep -n 'model ' prisma/schema.prisma | grep -i 'cron\|sched'` → **0 hit**. `grep -rn 'onModuleInit\|OnModuleInit\|restore\|rehydrate' src/scheduling/*.ts | grep -v spec` → **0 hit**. `grep -rn 'cronExpression\|cron_expression' src --include='*.ts' | grep -v spec | head -5` → **4 hit 전부 in-memory 경로** (`cron-schedule.controller.ts:98` 주석 · `:110 dto.cronExpression` · `cron-schedule.service.ts:52 cronExpression: string` · `:60 isValidCronExpression(cronExpression)`) — 영속화 write 지점 **0**.
- (v) contract · manual trigger — `grep -n '@Controller\|@Get\|@Put\|@Post\|@Delete' src/scheduling/cron-schedule.controller.ts | head -10` → **70** `@Controller("api/schedules")` · **90** `@Get()` · **103** `@Put()` · **120** `@Delete(":name")` · **140** `@Post("trigger")`. `grep -n '  async \|^  [a-z][A-Za-z]*(' src/scheduling/cron-schedule.service.ts | head -8` → **45** `constructor(private readonly schedulerRegistry: SchedulerRegistry)` · **50** `registerOrReplace(` · **80** `remove(name: string): void` · **88** `list(): string[]` · **94** `exists(name: string): boolean`. `sed -n '128,145p'` (controller) → **129 ~ 131** 행 `주입된 tickHandler 를 service 를 거치지 않고 직접 1회 호출한다 — cron tick callback 과 동일한 실행 추상을 공유해 중복 구동 로직을 방지한다` + **144 ~ 145** 행 `async trigger(): Promise<void> { await this.tickHandler(); }`, token 은 **68** 행 `export const CRON_TICK_HANDLER = "CRON_TICK_HANDLER";`. 출력 축 재확인 — `sed -n '30,50p' src/scheduling/scheduling.module.ts` → **32 ~ 45** 행 `defaultCronTickHandlerProvider` 가 `cron tick 발화 — 실 평가 pipeline 미결선(stub, Out of Scope)` 만 로깅하는 no-op 이고, `grep -rn 'CollectionTrigger\|collect(' src/scheduling/*.ts | grep -v spec | head -5` → `backfill-runner.service.ts:4 · :18 · :61` · `recent-deletion-runner.service.ts:7 · :23` 로 **결선은 backfill · 재수집 runner 경로뿐** 이다 (`§ 12.45` 의 "`Scheduler` 자동 trigger 미결선" 판정과 동일 결론).
- (vi) pointer · REQ — `grep -n '^### Decision §3' docs/decisions/ADR-0003-deployment.md` → **62** 행 `### Decision §3 — Scheduler 위치 = @nestjs/schedule (in-process)` · **138** 행 `### Decision §3 — Scheduler 위치` (후자는 Alternatives 표 절 heading, Decision 정본은 62 행). `sed -n '62,68p'` → **64** 행 `시간 trigger 는 @nestjs/schedule 모듈 (NestJS 내장, node-cron 기반) 을 Backend NestJS process 내부에서 사용` · **68** 행 `Admin 이 UI 에서 cron 주기를 변경 → DB 의 schedule 설정 row 갱신 → SchedulerModule 이 dynamic 하게 CronJob 재등록`. **본 축은 `§ 12.47` ~ `§ 12.49` 의 `§4` 승계가 적용 불가** 라 위 1 명령이 본 절의 직접 측정이다. `grep -n 'P7\|Scheduling' docs/PLAN.md | head -5` → **122** 행 (P6 Admin 패널 bullet) · **131** 행 `## Phase P7 — Scheduling & operations` · **133** 행 오너 승인 (`P7 진입 + @nestjs/schedule 새 외부 dependency 승인`) · **171** 행 phase 흐름. `grep -n 'REQ-039\|REQ-040' docs/requirements.md | cut -c1-160` → **58** `| REQ-039 | 72 | Admin cron 주기 지정 | FR | P7 | unit + e2e | DONE (GET·PUT·DELETE /api/schedules 런타임 cron registry) |` · **59** `| REQ-040 | 73 | Admin manual trigger | FR | P7 | e2e | DONE (POST /api/schedules/trigger 즉시 1회 발화) |`.
- (vii) baseline — `wc -l` components.md **236** · audit **4995** · requirements.md **97** · deployment.md **232** · directory.md **203** · modules.md **259** · PLAN.md **175** · `prisma/schema.prisma` **666** 전부 일치. `grep -c '^## '` components.md **7** · audit **12**, audit `grep -c '^| REQ-'` **66** · `grep -c '^### 12\.'` **49** 전부 일치 (`§ 12.49` 가 정정한 components.md `^## ` = 7 기준이 유효했다).

#### 지점 판정표 (AC 2)

판정 기준 3 축 — ① **문서 성격**: **1 ~ 4 행** blockquote 가 "본 문서는 P1 T-A3 의 산출물" 을 선언하는 시점 산출물이나, 표는 `Web UI` row 처럼 shipped 현황으로 여러 차례 갱신된 흔적이 있어 blueprint · 현황이 혼재한다. ② `§ 12.15` **정합**: `sed -n '126p' … | grep -o 'T-[0-9]\{4\}\|shipped\|20[0-9][0-9]'` → **0 hit** 이라 원문을 시점 기록으로 특정할 수 없어 in-place 치환의 정당성이 약하다. ③ **선례**: T-1430 ~ T-1451 의 "원문 보존 + 실측 각주" 가 지배적이고, [T-1429](../tasks/T-1429-api-md-module-vocab-and-uc-range-resync.md) 의 in-place 1:1 치환 · [T-1436](../tasks/T-1436-directory-md-web-frontend-section-vs-src-audit.md) 의 혼합은 좌표 · 어휘가 기계적으로 확정되는 경우에 한정됐다.

| claim (1 구) | 실측 결과 | 판정 | 처리 | 근거 1 구 |
| --- | --- | --- | --- | --- |
| "`@nestjs/schedule` 기반 in-process cron" | `app.module.ts:14` `ScheduleModule` root import + service 17 ~ 18 행 `SchedulerRegistry` / `cron` import | 참 | 무편집 | 외부 cron (system cron · CronJob 리소스) 호출 지점이 0 이라 in-process 근거가 코드로 닫힌다 |
| "manual trigger 단일 진입점" | `src/scheduling` `export class` 8 개 = controller 3 · service / runner 4 · module 1 | 부분참 | 원문 보존 + 각주 부기 | cron 진입점 본체는 `CronScheduleService` + `CronScheduleController` 1 쌍이나 backfill · 재수집 controller 2 개가 별도 REST 진입점으로 공존한다 |
| "cron 표현식은 DB 에 저장되어 process restart 후에도 복원" | schema cron / sched `model` 0 hit · `onModuleInit` / `restore` 0 hit · `cronExpression` 4 hit 전부 in-memory | 거짓 (미구현) | 원문 보존 + 각주 부기 | ADR-0003 §3 68 행의 **설계 의도** 를 blueprint 로 적은 문장이고, 미구현을 문서에 맞추려 model 을 새로 만드는 것은 Out of Scope 다 |
| "Admin UI 의 cron 갱신은 SchedulerRegistry 의 dynamic 등록" | service 4 · 17 ~ 18 · 50 행 `registerOrReplace` + controller 103 행 `@Put()` | 참 | 무편집 | 동적 등록 경로가 REST `PUT /api/schedules` → `SchedulerRegistry` 로 1:1 이어진다 |
| "manual trigger 는 … 동일 service 메서드 호출 — duplication 0" | controller 140 ~ 145 행이 `CronScheduleService` 를 거치지 않고 `CRON_TICK_HANDLER` (68 행) 를 직접 호출 | 부분참 | 원문 보존 + 각주 부기 | duplication 0 이라는 결론은 참이나 공유 단위가 service 메서드가 아니라 tick handler 추상이라 경로 표기가 어긋난다 (129 ~ 131 행 주석) |
| "입력: 시간 trigger (cron) 또는 Backend API endpoint 의 manual trigger" | controller 70 행 route prefix + 90 · 103 · 120 · 140 행 4 endpoint (`@Post("trigger")` 202) | 참 | 무편집 | 두 입력 축 모두 실 표면으로 존재한다 |
| "출력: Worker 또는 Backend API service 메서드의 in-process invocation" | `scheduling.module.ts` 32 ~ 45 행 기본 handler 가 logging no-op stub | 거짓 (미결선) | 원문 보존 + 각주 부기 | `§ 12.45` 의 미결선 판정을 승계하지 않고 재확인했으며, 결선은 코드 소관 `pr` task 다 (파생 영향 (18)) |
| "(`@Cron` decorator handler)" | `@Cron(` src 전체 0 hit + `app.module.ts` 51 행 "정적 @Cron job 정의 0" | 거짓 | 원문 보존 + 각주 부기 | 실 등록은 데코레이터가 아니라 registry 동적 등록이라 handler 표기가 구현 방식과 반대다 |
| `ADR-0003 §3 (@nestjs/schedule in-process)` | 62 행 Decision heading 일치 (138 행은 Alternatives 동명 heading), 64 행 결정 문구가 괄호 병기와 1:1 | 참 | 무편집 | **`상위 slice 판정 승계` 가 아니다** — `§ 12.49` 가 `§4` 승계 불가를 못박아 본 절이 `§3` 을 직접 실측했고 그 실측이 근거다 |
| `P7 Scheduling & ops task` | PLAN 131 행 `## Phase P7 — Scheduling & operations` + 133 행 오너 승인 | 참 (축약 표기) | 무편집 | `ops` 는 `operations` 축약이라 phase 좌표 지시는 어긋나지 않는다 |
| REQ ID 2 개 (REQ-039 · REQ-040) | requirements.md 58 ~ 59 행 실재, 병기 문구 `Admin cron 주기 지정` · `Admin manual trigger` 가 실 제목과 1:1 | 참 (2 ID 묶음) | 무편집 | 두 ID 의 판정 · 처리 · 근거가 동일해 묶음 조건을 충족하며, row 를 `REQ-` 로 시작하지 않게 적어 `^\| REQ-` 66 불변을 지켰다 |

#### 처리 방식 판정 (AC 3)

판정 기준 4 축 — ① `§ 12.15` 정합 (126 행 시점 marker 0 hit) · ② 오도 risk (독자가 표만 읽고 cron 영속화 · restart 복원 · 정적 `@Cron` 존재 · 자동 trigger 결선을 shipped 로 오인할 비용) · ③ cap (≤ 300 LOC · 변경 파일 3 고정) · ④ 각주 누적 구조 (본 slice 로 **7 블록째이자 마지막 블록** — `§ 12.49` AC 3.5 가 `표 완결까지 현 규약 유지` 로 이미 결착).

| 후보 | 판정 | 근거 |
| --- | --- | --- |
| (A) row 셀 in-place 동기 | 기각 | 치환 대상이 복원 서술 · `@Cron` handler · 단일 진입점 · manual trigger 경로 · 출력 대상으로 **5 지점** 이라 셀 재작성에 가까워지고, marker 0 hit 상 원문이 시점 기록인지 blueprint 인지 가리지 못한 채 P1 산출물을 덮어쓴다 |
| **(B) 원문 무편집 + T-1451 각주 직후 blockquote 1 개 신설** | **채택** | T-1437 ~ T-1451 화법을 그대로 승계해 표 축 7 slice 의 기록 형식이 균질하고, 거짓 3 · 부분참 2 를 각주 첫 구의 row 한정 + **표 완결** 선언과 함께 외화해 오도 risk 를 낮추면서 cap 안에 든다 |
| (C) 혼합 (거짓 지점만 in-place) | 기각 | 거짓이 3 지점 (복원 · 출력 · `@Cron`) 이나 그중 둘은 **미구현 사실** 이라 in-place 로 고치면 blueprint 서술 자체를 삭제하게 되고, 같은 셀 안의 부분참 2 건과 판정 화법이 갈린다 |
| (D) 전 지점 무편집 + audit 기록만 | 기각 | 표를 읽는 독자가 audit 절까지 오지 않으면 cron 영속화 · 자동 trigger 결선을 shipped 계약으로 오인한다 — `§ 12.46` ~ `§ 12.49` 가 각주를 둔 이유와 동일 |

#### 반영 결과 (AC 4) + 무편집 경계

- [components.md](../architecture/components.md) 에 **각주 blockquote 1 개 (6 행) + 앞 빈 줄 1 행 = +7 행** 을, T-1451 블록 마지막 행 (구 166 행) 과 `## GitHub Adapter …` heading 사이에 삽입했다. **in-place 치환 0**. `wc -l` **236 → 243** (허용 +7 이내).
- 각주 첫 구에 **"본 각주는 `Scheduler` row 한정"** + **"이로써 `## Component table` 7 row 대조가 완결"** 을 함께 뒀다. 문구 · 파일 이름 · 메서드 이름 · 수치 · 행 번호 · § 번호 · REQ ID · task ID 는 전부 위 실측 출력과 1:1 이며 **창작 0**.
- 무편집 확인 — 1 ~ 4 행 blockquote · 119 ~ 125 행 7 row · **126 행 원문** · 128 ~ 166 행 기존 각주 6 블록 · 구 168 행 이후 전 구간 (`## Contracts` · `## References` · mermaid) 전부 무변경 (아래 hunk 1 개로 실증).
- 새 pointer 추가 **0** — 실측 중 관찰된 `ADR-0042` (동적 cron service / controller slice) 는 본문 pointer 셀에 넣지 않고 본 절 파생 영향에만 적었다. secret · token 인용 **0**.

#### `## Component table` 7 row 대조 완결 선언

row → 절 매핑 — `Web UI` **`§ 12.44`** (T-1446) · `Backend API` + `Worker` **`§ 12.45`** (T-1447) · `DB Persistence` **`§ 12.46`** (T-1448) · `LLM Gateway` **`§ 12.47`** (T-1449) · `GitHub Adapter` **`§ 12.48`** (T-1450) · `Confluence Adapter` **`§ 12.49`** (T-1451) · `Scheduler` **`§ 12.50`** (본 절, T-1452). 이로써 표 **7 row** 대조가 완결됐다 (각주 blockquote **7** 블록 누적, in-place 치환 누적 **0**).

#### 파생 영향 (목록만 — 본 slice 편집 금지)

(1) **표 뒤 각주 7 블록 일괄 anchor 이행 재판정** (`§ 12.49` AC 3.5 · 한계 4 가 "`Scheduler` row 종료 직후" 로 이월 — **표가 완결됐으므로 다음 slice 1 순위 후보**) / (2) **`Scheduler` row pointer 셀 보강** — 실측이 드러낸 `ADR-0042` (§Decision 2 동적 cron slice) 가 pointer 셀에 미등재, 새 pointer 추가는 본 slice 금지라 별도 slice 소관 / (3) `## Deployment 컨텍스트` (22 ~ 26 행) "8 component 동일 process" claim ↔ 표 7 row 카운트 어긋남 (**7 회째 이월**) / (4) `## Component diagram` mermaid node ↔ 실 module 대조 / (5) `## GitHub Adapter — 3 instance 묶음 vs 분리 결정` sub-section (175 ~ 206 행) 본문 ↔ 코드 대조 (`§ 12.48` FU4 미소진) / (6) `## Contracts` 표 (207 ~ 232 행) ↔ 실 계약 표면 대조 / (7) `Confluence Adapter` row pointer 셀 보강 (`§ 12.49` FU2) / (8) `@nestjs/config` 미도입 전수 sweep (`§ 12.39` FU3, ADR 게이트) / (9) reviewer 규약 미이행 (`§ 12.41` FU2) / (10) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (`§ 12.41` FU3) / (11) README 행 번호 pointer drift sweep / (12) REQ 번호 체계 잔재 sweep (`§ 12.38` FU3) / (13) `CLAUDE.md` §1 pointer 부정확 (T-1442 FU3) / (14) UC-09 `§ 5` sequence participant 병기 (**36 회째 이월**) / (15) modules.md 카운트 claim 대조 (`§ 12.34` FU1) / (16) 행 번호 → anchor 좌표계 이행 (**30 회째 이월**) / (17) `§ 12.44` 한계 — "mutation 러너 26 개" 정의 미확정 / (18) **`Scheduler` cron → 평가 pipeline 미결선** (`§ 12.45` FU15 — **본 절 실측이 `scheduling.module.ts` 32 ~ 45 행 no-op stub 으로 재확인**, 코드 소관이라 `pr` task 로만 처리 가능) / (19) `ADR-0003` 의 "단일 DB 인스턴스" 좌표 부재 (`§ 12.46` FU16) / (20) LLM provider 배포 config ADR 3 종 pointer 미등재 (`§ 12.47` FU5) / (21) GitHub adapter ADR 3 종 pointer 미등재 (`§ 12.48` FU3).

#### 불변 검산 (AC 6)

- `wc -l` — components.md **236 → 243** (+7, ≤ 243) · audit **4995 → 5073** (+78, ≤ +115) · requirements.md **97 불변** · deployment.md **232 불변** · directory.md **203 불변** · modules.md **259 불변** · PLAN.md **175 불변** · `prisma/schema.prisma` **666 불변**.
- `grep -c` — components.md `^## ` **7 불변** · audit `^## ` **12 불변** · audit `^| REQ-` **66 불변** · audit `^### 12\.` **49 → 50**.
- `git diff -U0 -- docs/architecture/components.md | grep '^@@'` → `@@ -167,0 +168,7 @@` **hunk 1 개** — AC 4 허용 구간 (T-1451 블록 직후 ~ `## GitHub Adapter` heading 직전) 밖 hunk **0**.
- `git diff --numstat` → `7	0	docs/architecture/components.md` — **순수 삭제 0** (in-place 치환 자체가 0 이라 삭제 행이 없다).
- `git status --porcelain src/ test/ web/ prisma/ deploy/ docker-compose.yml Dockerfile .github/ package.json README.md .claude/ docs/decisions/ docs/ops/ docs/PLAN.md docs/requirements.md` → **빈 출력** (코드 · schema · frontend · 배포자산 · CI · 의존성 · ADR · PLAN · requirements 무변경). `git status --porcelain` 전체는 **2 파일** (components.md · 본 audit — task 파일 status 갱신은 driver 소관) 로 cap 이내.
...
1. **"단일 진입점" 부분참 판정은 `src/scheduling` 디렉토리 기준이다** — 다른 module 이 자체 cron 유사 진입점을 두지 않는다는 전수 증명은 §7 예산 밖이며, `@Cron(` 0 hit 이 그 위험을 크게 줄일 뿐이다.
2. **"DB 저장 + restart 복원" 거짓 판정은 schema · scheduling 디렉토리 grep 근사다** — `cronExpression` 계열 식별자를 쓰지 않는 우회 영속화 (예: 범용 설정 테이블) 가 없다는 증명까지는 하지 못했다.
...
### 12.54 components.md `## Component diagram` mermaid **`%% Scheduler triggers` edge 2 개** ↔ 실 `src/scheduling/**` 호출 그래프 · `@Cron handler` label claim 대조 — edge 1 부분참 · edge 1 거짓 확정 (T-1456)

> **본 절의 위치 · 계보** — `§ 12.53` (T-1455) 이 다이어그램의 **node 축** 을 닫으면서 파생 영향 **(1)** 로 **"mermaid edge 가 주장하는 결선 ↔ 실 호출 그래프 대조 — 다음 대조 1 순위"** 를 지목했고, 그 안에서 `scheduler -- "in-process trigger<br/>(@Cron handler)" --> worker` (**76** 행) 가 `§ 12.50` 의 **미결선 (거짓)** 판정과 상충할 수 있다고 특정했다. edge 는 총 **23** 개라 한 slice 로 닫으면 cap 을 넘기므로 mermaid 자신의 `%%` 주석 그룹 단위로 split 했고, 본 절은 그 **1/6 — `%% Scheduler triggers` 그룹 (edge 2 개)** 을 집행한다. **node 축 (`§ 12.53`) · 표 row 본문 (`§ 12.44` ~ `§ 12.50`) · 나머지 edge 그룹 5 개 · `## Contracts` 표 는 재판정하지 않는다.**

#### 실측 (AC 1 — 편집 전 측정, 명령과 출력)

- (i) **좌표 재확인** — `grep -n '^#\{1,3\} ' docs/architecture/components.md` → `1:# Component view` / `5:## 개요` / `22:## Deployment 컨텍스트` / `28:## Component diagram` / `115:## Component table` / `189:## GitHub Adapter …` / `221:## Contracts` / `247:## References` — **task 예고 좌표 전부 유효 (drift 0)**. `grep -n '^\s*%%\|^```' …` → `30:```mermaid` · `32 %% External actors / systems` · `48 %% In-process components` · `61 %% External DB` · `64 %% User-facing flow` · `68 %% Backend orchestration` · **`75 %% Scheduler triggers`** · `79 %% Worker pipeline` · `85 %% DB persistence boundary` · `88 %% External egress` · `99 %% Styling` · `106:``` ` — mermaid 블록 **30 ~ 106**, edge 그룹 주석 6 개 확정.
- (ii) **edge 전수 열거 + 그룹 분해 (split 근거 실증)** — `grep -cE '^\s+[a-z_]+ -- ' …` → **23**. (i) 의 주석 좌표로 분해하면 **산출식 = user-facing 2 (65 ~ 66) + orchestration 5 (69 ~ 73) + scheduler 2 (76 ~ 77) + worker pipeline 4 (80 ~ 83) + db boundary 1 (86) + external egress 9 (89 ~ 97) = 23** 이다. **본 slice 의 판정 대상은 `%% Scheduler triggers` 그룹 2 개뿐** 이고 나머지 **21** 개는 파생 영향 (1) 의 후속 slice 소관이다.
- (iii) **대상 edge 원문 인용** — `sed -n '75,77p' …` → `    %% Scheduler triggers` / `    scheduler -- "in-process trigger<br/>(@Cron handler)" --> worker` / `    scheduler -- "in-process trigger" --> backend_api`. 3 컬럼 분해 — **76**: 출발 `scheduler` · 도착 `worker` · label `in-process trigger<br/>(@Cron handler)`, **77**: 출발 `scheduler` · 도착 `backend_api` · label `in-process trigger`.
- (iv) **실 호출 그래프 측정 (본 slice 의 축)** — ⓐ `grep -rn '@Cron(' src --include=*.ts | grep -v spec` → **0 hit** (정적 decorator handler 부재). ⓑ `grep -rn 'CRON_TICK_HANDLER' src --include=*.ts | grep -v spec` → **8 hit**, 실 provider 는 `src/scheduling/scheduling.module.ts` **36 ~ 45** 행 `defaultCronTickHandlerProvider` = `logger.log("cron tick 발화 — 실 평가 pipeline 미결선(stub, Out of Scope)")` **no-op stub** (주입 지점은 `cron-schedule.controller.ts` **68** 행 token · **82** 행 `@Inject`). `cron-schedule.service.ts` **20 · 22** 행 — `// CronTickHandler — cron 발화 시 호출되는 callback` / `export type CronTickHandler = () => void | Promise<void>` 라 **주입형 callback 타입** 이다. ⓒ `grep -rn 'assessment-evaluation\|assessment-collection' src/scheduling --include=*.ts | grep -v spec` → **4 hit** — `assessment-backfill-checker.service.ts` **24** (`SinceDerivationService`) · `backfill-runner.service.ts` **18** · `recent-deletion-runner.service.ts` **23** (`CollectionTriggerService`) · `scheduling.module.ts` **13** (`AssessmentCollectionModule`). 실 호출 지점은 `backfill-runner.service.ts` **108** 행 `await this.collectionTriggerService.triggerCollection({…})` 이고 그 진입은 `backfill.controller.ts` **50** 행 `constructor(private readonly runner: BackfillRunnerService)` 의 **REST** 경로다. ⓓ `grep -rn 'CronScheduleService' src … | grep -v 'cron-schedule.service.ts'` → 실 주입은 `cron-schedule.controller.ts` **80** 행 `private readonly service: CronScheduleService` **1 지점** 이며 나머지는 `scheduling.module.ts` provider/export 등록 (**26 · 71 · 94**) 과 주석뿐 — **`src/scheduling/` 밖에서 주입하는 module 은 0** 이다.
- (v) **방향 축 실측 (Why ③)** — `grep -n 'private readonly\|@Controller\|@Put\|@Post' src/scheduling/cron-schedule.controller.ts | head` → **70** `@Controller("api/schedules")` · **80** `private readonly service: CronScheduleService` · **82** `@Inject(CRON_TICK_HANDLER) … tickHandler` · **103** `@Put()` · **140** `@Post("trigger")`. 즉 관측되는 유일한 결선은 **controller → service** 이고, 그 controller 는 `src/scheduling/` 소속이라 `§ 12.53` (iv) 의 node ↔ module 매핑상 **`scheduler` node 내부** 다. 77 행 edge 가 주장하는 `scheduler → backend_api` 와 **같은 방향이 아니며** (외부에서 본 HTTP 진입은 도리어 `→ scheduler` 로 반대), backend_api 로 나가는 호출 자체가 없다.
- (vi) **ADR 승계 축** — `grep -n 'scheduler\|Scheduler' docs/decisions/ADR-0003-deployment.md | head` → **62** `### Decision §3 — Scheduler 위치 = @nestjs/schedule (in-process)` · **70** `in-process scheduler 가 monolithic 의 자연스러운 귀결. 별도 process / 별도 큐 broker 도입 없이 trigger → handler 가 한 hop` (**42** 행도 `in-process scheduler 가 평가 pipeline 을 같은 process 의 service 메서드로 직접 호출`). edge label 의 `in-process trigger` 어구는 이 결정을 **문자 그대로 승계** 한다 — 단 이는 **trigger 의 위치 · 방식** 축이지 **결선의 실재** 축이 아니므로 (iv) 와 별개로 판정한다.
- (vii) **중복 claim 좌표 확인 (판정은 이월)** — `grep -n '@Cron' docs/architecture/components.md` → **76** (다이어그램 edge label) · **126** (표 `Scheduler` row 의 `(@Cron decorator handler)`) · **143** (`§ 12.45` 계열 각주) · **175** (`§ 12.50` 각주 `@Cron decorator handler 는 거짓`) · **234** (`| Scheduler | Worker | … | NestJS @Cron decorator handler |`) · **235** (`| Scheduler | Backend API | … | NestJS @Cron handler 가 controller/service 호출 |`) — **6 지점** 이다 (task 예고 3 지점은 **반증** — 표 row · Backend API 각주 · `## Contracts` **2** row 를 세지 않았다). `## Contracts` 표의 234 · 235 두 row 가 **본 절이 판정한 edge 2 개와 1:1 대응** 한다는 사실만 기록하고, **그 표의 참 / 거짓 판정은 하지 않는다 — 파생 영향 (3) 소관** 이다.
- (viii) **삽입 파급 실측 (AC 3 입력)** — `§ 12.53` (viii) 이 확정한 **28** 지점 + T-1455 신규 각주 (**184 ~ 187** 행) 의 자기 참조 **12** 지점 (184 행 `49 ~ 59` · `119 ~ 126` · `111` 3 · 185 행 `119` 1 · 187 행 `128` + `133 · 139 · 147 · 154 · 160 · 167 · 173` 8) = **40** 지점이 기준이다. ⓐ **mermaid 블록 직후 (107 행) 삽입** → 107 이후를 가리키는 좌표가 전부 밀려 **33 지점** stale (잔존 유효 7 = 174 행 `1 ~ 4` 1 · 180 행 `24` 2 · 182 행 `3` · `20` · `24` 3 · 184 행 `49 ~ 59` 1). ⓑ **각주군 말미 (187 행 뒤) 삽입** → 188 이후를 가리키는 좌표만 밀리므로 **1 지점** (**161** 행 `**189** 행 `## GitHub Adapter …``). `grep -noE '\*\*[0-9]{3}( ~ [0-9]{3})?\*\* 행' …` 로 180 이상 좌표 11 건을 재확인했고 그중 components.md 자기 참조는 161 행 1 건뿐이었다 (나머지는 소스 파일 좌표). **수치 2 개 = ⓐ 33 · ⓑ 1**.
- (ix) **baseline** — `wc -l` components.md **257** · audit **5283** · ADR-0003 **173** · requirements.md **97** · deployment.md **232** · directory.md **203** · modules.md **259** · PLAN.md **175**, `grep -c '^## '` components.md **7** · audit **12**, audit `grep -c '^| REQ-'` **66** · `grep -c '^### 12\.'` **53**, components.md `grep -c '^> '` **57** — task 예고 (ix) 와 **전부 일치 (drift 0)**. PLAN.md 미완 bullet 은 `grep -n '^- \[ \]'` → **106 · 108 · 109 · 140 · 151** (+ 164 검토 bullet) 로 전부 오너 · credential · dependency 게이트다.

#### 판정표 (AC 2 — `%% Scheduler triggers` 2 edge 한정)

| 축 | 실측 근거 (행 번호 포함) | 판정 | 근거 1 구 |
| --- | --- | --- | --- |
| ① `scheduler --> worker` (**76** 행) 결선의 실재 | (iv) ⓑ stub provider `scheduling.module.ts` **36 ~ 45** · ⓒ `backfill-runner.service.ts` **18 · 108** → `CollectionTriggerService.triggerCollection` | **부분참** | `scheduler` → `worker` in-process 호출은 실재하나 그 진입이 `backfill.controller.ts` **50** 행 **REST runner** 경로이고, **cron 발화 경로는 no-op stub 이라 미결선** 이다 (Why ② 는 부분 반증 — 결선 자체는 성립). |
| ② 그 edge label 의 `@Cron handler` 어구 정확성 | (iv) ⓐ `@Cron(` **0 hit**, (vii) **175** 행 각주 동일 판정 | **거짓** | 정적 `@Cron` decorator handler 가 `src` 전체에 하나도 없으므로 label 이 지목한 trigger 실체가 코드에 존재하지 않는다. |
| ③ `scheduler --> backend_api` (**77** 행) 결선의 실재 · 방향 | (iv) ⓓ 주입 **1 지점** `cron-schedule.controller.ts` **80** · (v) **70 · 103 · 140** 행 | **거짓** | 관측되는 유일한 결선은 `scheduler` node **내부** 의 controller → service 이고 방향도 HTTP → scheduler 라 edge 표기와 반대이며, scheduler 에서 backend_api 로 나가는 호출은 **0** 이다. |
| ④ label `in-process trigger` 의 ADR-0003 §3 승계 | (vi) ADR-0003 **62** 행 Decision §3 heading · **70** 행 `trigger → handler 가 한 hop` · **42** 행 | **참 (trigger 위치 축 한정)** | ADR 이 박제한 in-process scheduler 결정을 label 이 그대로 승계한다 — 다만 이는 trigger 의 **위치 · 방식** 축이고 결선 실재 축 (① · ③) 과 무관하다. |
| ⑤ 같은 `@Cron` claim 의 문서 내 중복 표기 | (vii) `grep -n '@Cron'` → **76 · 126 · 143 · 175 · 234 · 235** 6 지점 | **참 (중복 사실만)** | 동일 claim 이 다이어그램 label · 표 row · 각주 2 · `## Contracts` 2 row 로 **6 회** 되풀이되며, 그중 `## Contracts` 2 row 의 참 / 거짓은 파생 영향 (3) 이 판정한다. |

- 축 ① · ② · ③ 중 **② · ③ 이 `거짓` · ① 이 `부분참`** 이므로 AC 3 의 "(A) 자동 기각" 조건이 발동한다.

#### 처리 방식 판정 (AC 3 — 채택 1 · 기각 3)

| 후보 | 판정 | 근거 1 구 |
| --- | --- | --- |
| (A) 현행 유지 + 무편집 (audit 기록만) | **기각 (자동)** | AC 2 축 ② · ③ 이 `거짓` 이라 AC 3 의 자동 기각 조건이 발동하며, 기준 ④ 탐색성 상 다이어그램만 본 독자가 `@Cron handler` 결선을 실재로 오독한다. |
| **(B) 각주군 말미 append (187 행 뒤) + stale 좌표 ≤ 2 지점 정정** | **채택** | 기준 ① append-only 정합 (본문 삭제 0) · 기준 ② 파급 최소 ((viii) ⓑ **1 지점**) · 기준 ③ cap (변경 2 파일 ≤ 3 · diff ≪ 300 LOC) · 기준 ④ T-1453 안내 blockquote (**128 ~ 131** 행) 매핑 재사용 — 4 축 전부 우세. |
| (C) `## Component diagram` 절 안 각주 삽입 (mermaid 블록 직후 107 행) | **기각** | 기준 ② — (viii) ⓐ 가 stale 좌표 **33 지점** 을 만들어 AC 4 의 `정정 3 지점 이상 시 (B) 로 철회` 조건이 즉시 발동한다 (`§ 12.53` 의 22 지점보다 11 늘었다). |
| (D) mermaid edge · label in-place 수정 (edge 삭제 · 방향 반전 · label 문구 교체) | **기각** | 기준 ① — 기존 본문을 고쳐 쓰는 것은 `§ 12.15` append-only 방침과 정면 충돌하며, AC 3 이 다이어그램 구조 변경을 명시 금지한다. |

- **mermaid edge 를 지우거나 방향을 뒤집거나 label 을 고쳐 쓰는 선택지는 채택하지 않는다** — 다이어그램 구조 변경은 본 doc-audit stream 의 scope 밖이며, 축 ② · ③ 이 `거짓` 으로 판정돼도 처리는 **각주 병기** 로만 한다. **코드를 고쳐 결선을 만드는 처리도 하지 않는다** — 파생 영향 (17) 의 `pr` task 소관이다.

#### 반영 결과 (AC 4 — 채택안 (B) 대로만 편집)

- **신규 blockquote 1 블록 (4 행 + 앞 빈 줄 1 행 = +5 행)** 을 마지막 각주 블록 (**187** 행) 뒤 · `## GitHub Adapter …` heading 직전에 삽입했다 (허용 ≤ 5 행). 내용은 ① edge **23** 개 그룹 산출식 + 본 slice scope 2 개 · ② `scheduler --> worker` 부분참 (REST runner 실재 / cron 경로 stub) · ③ `@Cron handler` 거짓 + `scheduler --> backend_api` 거짓 (방향 반대) + `in-process trigger` ADR 승계 참 · ④ `@Cron` claim **6 지점** 중복 4 구다.
- **in-place 좌표 정정 1 지점** — **161** 행의 `**189** 행 `## GitHub Adapter …`` → `**194** 행` (숫자 1 개 치환, 문장 재작성 0). 허용 ≤ 2 지점 안이다.
- **재-drift 재현 확인** — 편집 후 `grep -n '^## ' docs/architecture/components.md` → `5 · 22 · 28 · 115 · 194 · 226 · 252`. `§ 12.51` `175` → `180`, `§ 12.52` `180` → `184`, `§ 12.53` `184` → `189` 에 이어 본 slice 가 `189` → `194` 로 **4 회째 재현** 됐다 (행 번호 좌표는 각주가 붙을 때마다 썩는다 — 파생 영향 (14) 의 직접 증거 1 건 추가).
- **무편집 경계** — mermaid 블록 (**30 ~ 106** 행, edge 23 개 전부 포함) · `다이어그램 표기` bullet (**108 ~ 113** 행) · 표 본체 (**117 ~ 126** 행) · 1 ~ 4 행 blockquote · `## 개요` 각주 (**16 ~ 20** 행) · 안내 blockquote (**128 ~ 131** 행) · 각주 9 블록의 판정 문장 · `## Contracts` · `## References` 전 구간 무편집이다. `docs/decisions/ADR-0003-deployment.md` · [modules.md](../architecture/modules.md) · `docs/PLAN.md` · `docs/requirements.md` 는 인용까지만이라 diff **0**, `src/` 는 read-only `grep` 뿐이라 diff **0** 이다.

#### 파생 영향 (목록만 — 본 slice 편집 금지)

(1) **나머지 edge 그룹 대조 5 건** — `%% User-facing flow` **2** (65 ~ 66) · `%% Backend orchestration` **5** (69 ~ 73) · `%% Worker pipeline` **4** (80 ~ 83) · `%% DB persistence boundary` **1** (86) · `%% External egress` **9** (89 ~ 97). **다음 대조 1 순위는 `%% Backend orchestration` 5 개** — `backend_api -- "in-process method call" --> worker` (**73** 행) 가 본 절 축 ① 의 REST runner 경로와 직접 맞물려 미결선 축을 반대편에서 검증한다. / (2) `## GitHub Adapter — 3 instance 묶음 vs 분리 결정` sub-section 본문 ↔ 코드 대조 (`§ 12.48` FU4 미소진) / (3) **`## Contracts` 표 ↔ 실 계약 표면 대조** (본 절 (vii) 이 `Scheduler | Worker` **234** · `Scheduler | Backend API` **235** 두 row 의 중복 claim 좌표를 재료로 남겼다) / (4) row pointer 셀 보강 2 건 (`Scheduler` = `ADR-0042` 미등재 `§ 12.50` FU2 · `Confluence Adapter` `§ 12.49` FU2) / (5) LLM · GitHub adapter ADR pointer 미등재 (`§ 12.47` FU5 · `§ 12.48` FU3) / (6) `@nestjs/config` 미도입 전수 sweep (`§ 12.39` FU3, ADR 게이트) / (7) reviewer 규약 미이행 (`§ 12.41` FU2) / (8) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (`§ 12.41` FU3) / (9) README 행 번호 pointer drift 전수 sweep / (10) REQ 번호 체계 잔재 sweep (`§ 12.38` FU3) / (11) `CLAUDE.md` §1 pointer 부정확 (T-1442 FU3) / (12) UC-09 `§ 5` sequence participant 병기 (**41 회째 이월**) / (13) modules.md 카운트 claim 대조 (`§ 12.34` FU1, ADR 게이트) / (14) **행 번호 → anchor 좌표계 이행** (**35 회째 이월** — 본 절 AC 4 의 `189` → `194` 재-drift **4 회째** 재현과 (viii) ⓐ **33** vs ⓑ **1** 이 재료를 더 보탰다) / (15) 각주 heading 참조 anchor 이행 축소 scope (`§ 12.51` FU19 split 제안 미소진) / (16) `§ 12.44` 한계 "mutation 러너 26 개" 정의 미확정 / (17) **`Scheduler` cron → 평가 pipeline 미결선** (`§ 12.50` FU18 — **코드 소관, `pr` task 로만**. 본 절 축 ② · ③ 이 `거짓` 으로 판정돼 그 근거가 **1 건 더** 쌓였다) / (18) `ADR-0003` "단일 DB 인스턴스" 좌표 부재 (`§ 12.46` FU16) / (19) `Web UI` node 의 process subgraph 소속 표기 (`§ 12.53` FU19 미소진) / (20) **AC 3 기각 후보의 split 제안 — 없음** (기각 3 건이 전부 방침 · 파급 사유라 cap 사유가 아니다).

#### 불변 검산 (AC 6)

- `wc -l` — components.md **257 → 262** (+5, 허용 ≤ 263) · audit **5283 → 5352** (+69, 본 절 자체 **68** 행 ≤ 100) · **ADR-0003 173 불변** · requirements.md **97 불변** · deployment.md **232 불변** · directory.md **203 불변** · modules.md **259 불변** · PLAN.md **175 불변**.
- `grep -c` — components.md `^## ` **7 불변** · `^> ` **57 → 61** (신규 각주 4 행) · audit `^## ` **12 불변** · audit `^| REQ-` **66 불변** · audit `^### 12\.` **53 → 54**.
- `git diff -U0 -- docs/architecture/components.md | grep '^@@'` → `@@ -161 +161 @@` · `@@ -188,0 +189,5 @@` — **hunk 2 개** 이며 둘 다 AC 4 허용 구간 (stale 좌표 1 지점 · 각주군 말미 삽입) 안이다. 허용 구간 밖 hunk **0**.
- `git diff --numstat -- docs/architecture/components.md` → `6	1	docs/architecture/components.md` — 삭제 **1** 행은 `**189** → **194**` 숫자 치환의 짝이며 **순수 삭제 0** 이다.
- `git status --porcelain src/ test/ web/ prisma/ deploy/ docker-compose.yml Dockerfile .github/ package.json README.md .claude/ docs/decisions/ docs/ops/ docs/PLAN.md docs/requirements.md` → **빈 출력**. `git status --porcelain` 전체는 **2 파일** (components.md · 본 audit) 로 3 파일 이내다 — task 파일의 `Status` 갱신은 driver 의 bookkeeping commit 소관이다 (STATE single-writer §9).
...
1. **scheduler edge 2 개만 닫았다** — 나머지 **21** edge 의 결선 · label 참 / 거짓은 재판정 대상 밖이라, 본 절이 인용한 `backend_api → worker` (**73** 행) 는 축 ① 의 방증으로만 쓰였고 그 결선 자체의 판정은 파생 영향 (1) 소관으로 남는다.
2. **호출 그래프 측정은 grep 근사다** — import · 생성자 주입 · 단일 호출 지점 (`backfill-runner.service.ts` **108** 행) 까지만 확인했고 런타임 call graph · 조건 분기별 도달 가능성은 실행해 보지 않았다 (`pnpm test` 미실행 — Out of Scope).
3. **`## Contracts` **234 · 235** 두 row 의 참 / 거짓은 미판정이다** — 본 절은 (vii) 에서 **중복 사실과 좌표** 만 박제했고, 그 표가 edge 와 같은 오류를 되풀이하는지는 파생 영향 (3) 이 판정한다.
4. **삽입 파급 수치 ⓐ 33 · ⓑ 1 은 `§ 12.53` (viii) 과 같은 패턴 · 구간 (128 ~ 187 행) 기준이다** — 그 밖 구간 (1 ~ 127 행) 의 서술형 좌표까지 세면 ⓐ 는 더 커질 수 있으나 두 후보의 **대소 관계** 는 뒤집히지 않는다.

### 12.55 components.md `## Component diagram` mermaid **`%% Backend orchestration` edge 5 개** (69 ~ 73 행) ↔ 실 `src/` 호출 그래프 · `backend_api` node 외연 대조 — edge 1 참 · edge 4 거짓 확정 (T-1457)

> **본 절의 위치 · 계보** — `§ 12.54` (T-1456) 가 edge 축의 첫 그룹 (`%% Scheduler triggers` **2** 개) 을 닫으면서 파생 영향 **(1)** 에서 **다음 대조 1 순위를 `%% Backend orchestration` 5 개 (69 ~ 73 행) 로 명시 지목** 했고, 특히 `backend_api --> worker` (**73** 행) 를 "REST runner 진입이 어느 node 소관인지 반대편에서 재는" 미결 축으로 남겼다. 본 절은 그 지목을 집행한다 — edge 그룹 split **2/6**. **node 축 (`§ 12.53`) · 표 row 본문 (`§ 12.44` ~ `§ 12.50`) · scheduler edge 2 개 (`§ 12.54`) · 나머지 edge 그룹 4 개 · `## Contracts` 표 는 재판정하지 않는다.**

#### 실측 (AC 1 — 편집 전 측정, 명령과 출력)

- (i) **좌표 재확인** — `grep -n '^#\{1,3\} ' docs/architecture/components.md` → `1:# Component view` / `5:## 개요` / `22:## Deployment 컨텍스트` / `28:## Component diagram` / `115:## Component table` / `194:## GitHub Adapter …` / `226:## Contracts` / `252:## References` — **task 예고 좌표 전부 유효 (drift 0)**. `grep -n '^\s*%%\|^```' …` → 블록 `30:```mermaid` ~ `106:``` `, edge 그룹 주석 **64 · 68 · 75 · 79 · 85 · 88** — **`68 %% Backend orchestration`** 확정.
- (ii) **edge 그룹 재확인** — `grep -cE '^\s+[a-z_]+ -- ' docs/architecture/components.md` → **23**. 내역은 user-facing **2** (65 ~ 66) + orchestration **5** (69 ~ 73) + scheduler **2** (76 ~ 77) + worker pipeline **4** (80 ~ 83) + db boundary **1** (86) + external egress **9** (89 ~ 97) 로 `§ 12.54` 산출식이 **여전히 성립** 한다. **본 절 대상은 orchestration 5 개뿐** 이다.
- (iii) **대상 edge 원문** — `sed -n '68,73p' …` 출력의 5 edge 를 `from → to | label` 로 분해하면 `backend_api → db_persistence | "Prisma typed query"` (69) · `backend_api → llm_gateway | "in-process method call"` (70) · `backend_api → github_adapter | 동 label` (71) · `backend_api → confluence_adapter | 동 label` (72) · `backend_api → worker | 동 label` (73) 이다 — **출발 node 는 5 개 모두 `backend_api`**, label 은 **1 종 (Prisma) + 4 종 동일 (in-process method call)**.
- (iv) **`backend_api` node 외연 정의 (선결 측정)** — `grep -rn '@Controller(' src --include=*.ts | grep -v spec` → decorator **20** 개 / 파일 **20** 개. 디렉토리별로 `user` **7** · `scheduling` **3** · `llm` **2** · `src` (root `app.controller.ts`) · `auth` · `export` · `import` · `permission-denied` · `user-instance-access` · `assessment-collection` · `assessment-evaluation` 각 **1** 이다. components.md **186** 행 (`§ 12.53`) 의 node ↔ 디렉토리 매핑에 따라 **자기 node 를 이미 가진 디렉토리를 제외** 한다 — `src/scheduling/` **3** = `scheduler` node · `src/assessment-collection|evaluation/` **2** = `worker` node · `src/llm/` **2** = `llm_gateway` node. 따라서 **`backend_api` = `src/` (root) · `src/auth/` · `src/user/` · `src/user-instance-access/` · `src/permission-denied/` · `src/export/` · `src/import/` 의 controller 13 개와 그 service 계층** 으로 정의하며, **이 정의가 아래 5 축 판정의 기준** 이다.
- (v) **결선 실측** — ⓐ `grep -rl 'PrismaService' src --include=*.ts | grep -v spec` → **41** 파일 (`user` 11 · `assessment-evaluation` 7 · `import` 6 · `export` 5 · `llm` 3 · `user-instance-access` 2 · `persistence` 2 · `permission-denied` 2 · `github` · `confluence` · `assessment-collection` 각 1) 이고 그중 backend_api 소속이 **26**, `import … PrismaService … from` 는 **21** 지점 전부 `"../persistence/prisma.service"` 단일 경로다 (우회 **0**). ⓑ `grep -rn 'LLM_GATEWAY\|LlmHttpGateway' src … | grep -v '^src/llm/'` → `app.module.ts` (wiring) · `assessment-evaluation/` **4** 파일 · adapter 2 파일뿐 — **backend_api 디렉토리군 0**. ⓒ `grep -rn 'GithubAdapter\|GithubInstanceClient' … | grep -v '^src/github/'` → `assessment-collection/` **5** 파일 + `confluence/` 2 + `app.module.ts` — **backend_api 0**. ⓓ `grep -rn 'ConfluenceAdapter\|ConfluenceSpaceTraversalService' … | grep -v '^src/confluence/'` → `assessment-collection/` **3** 파일뿐 — **backend_api 0**. ⓔ `grep -rn 'CollectionTriggerService' src …` → `assessment-collection/` **3** 파일 (controller **49** 행 생성자 주입) · `scheduling/` **3** 파일 (`backfill-runner.service.ts` **18** 행 import · **108** 행 호출, `recent-deletion-runner.service.ts`) · `user/person.service.ts` **1** — 그러나 마지막 것은 **101** 행 주석 1 구이고 코드 참조가 아니다. 보강으로 `grep -rn 'from "\.\./(user|auth|export|import|permission-denied|user-instance-access)/' src/assessment-*` → **22** 지점인 반면 그 역방향 (`backend_api` → `assessment-*` import) 은 **0** 이다.
- (vi) **label · ADR 승계 축** — `sed -n '111p' …` → `화살표 label 의 "in-process method call" 은 NestJS DI container 안의 service 메서드 호출`. `grep -n 'process' docs/decisions/ADR-0003-deployment.md` → **32** 행 `### Decision §1 — Monolithic NestJS process`, **34** 행 `HTTP API / scheduler / 평가 파이프라인 / LLM gateway / GitHub & Confluence adapter 가 동일 process 안에서 동작한다`. 즉 label 어구는 표기 규약과 ADR 결정을 **승계** 하며, 이는 **결선 실재 여부와 별개 축** 이다.
- (vii) **중복 claim · 좌표 stale** — `grep -n '^| Backend API |' …` → **234 · 235 · 236 · 237 · 238** 로 edge 69 ~ 73 과 **순서까지 1:1** (DB Persistence · LLM Gateway · GitHub Adapter · Confluence Adapter · Worker). 같은 결선이 다이어그램 + 표 **2 곳** 에 중복 박제돼 있다. 이어 `grep -n '^| Scheduler |' …` → **239 · 240** 인데 components.md **192** 행 각주는 `@Cron` 중복 지점으로 **234 · 235** 를 인용하고 있어 **정확히 +5 만큼 stale** 이다 (T-1456 이 자기 각주 **5** 행을 삽입하며 자기 인용 좌표를 밀어버린 자기 참조 drift). **`## Contracts` 표의 참 / 거짓 판정은 하지 않는다** — 파생 영향 (3) 소관이다.
- (viii) **삽입 파급** — components.md 자기 좌표 토큰만 (외부 파일 · ADR · PLAN 좌표 제외) 세면 ⓐ **mermaid 블록 직후 (106 행 뒤) 삽입 시 밀리는 지점 21** (161 · 180×2 · 182 · 184×3 · 185 · 187×7 · 190 · 192×5), ⓑ **각주군 말미 (192 행 뒤) 삽입 시 3** (161 의 `194`, 192 의 `234 · 235`) 로 **ⓑ 가 7 배 유리** 하다. `§ 12.54` 의 ⓐ **33** · ⓑ **1** 과 절대값이 다른 것은 계수 규칙 차이 (본 절은 외부 파일 좌표를 제외) 이며 **대소 관계는 동일** 하다.
- (ix) **baseline** — `wc -l` components.md **262** · audit **5352** · ADR-0003 **173** · requirements.md **97** · deployment.md **232** · directory.md **203** · modules.md **259** · PLAN.md **175**. `grep -c '^## '` components.md **7** · audit **12**, audit `grep -c '^| REQ-'` **66** · `grep -c '^### 12\.'` **54**, components.md `grep -c '^> '` **61** — task 예고와 **전부 일치**.

#### 판정 (AC 2 — orchestration edge 5 + label 축)

| 축 | 실측 근거 | 판정 | 근거 1 구 |
| --- | --- | --- | --- |
| ① `backend_api --> db_persistence` "Prisma typed query" (**69** 행) | (v) ⓐ — backend_api 소속 **26** 파일, import 경로 **21/21** 이 `../persistence/prisma.service` | **참** | 결선이 실재하고 우회 경로가 **0** 이라 방향 · label 모두 실측과 일치한다. |
| ② `backend_api --> llm_gateway` (**70** 행) | (v) ⓑ — backend_api 디렉토리군 hit **0**, 소비자는 `assessment-evaluation/` 4 파일 | **거짓** | gateway 실 소비자는 `worker` node 뿐이라 같은 결선이 **80 ~ 83** 행 `worker --> llm_gateway` 로 이미 표현돼 있다. |
| ③ `backend_api --> github_adapter` (**71** 행) | (v) ⓒ — backend_api hit **0**, 소비자는 `assessment-collection/` 5 파일 | **거짓** | adapter 주입처가 전부 worker 계열이라 backend_api 발 결선이 실재하지 않는다. |
| ④ `backend_api --> confluence_adapter` (**72** 행) | (v) ⓓ — backend_api hit **0**, 소비자는 `assessment-collection/` 3 파일 | **거짓** | ③ 과 동형이며 소비자 집합이 더 좁다 (`assessment-collection/` 단독). |
| ⑤ `backend_api --> worker` (**73** 행) | (v) ⓔ — backend_api → worker 호출 **0** (`person.service.ts` **101** 행은 주석), 역방향 import **22** | **거짓** | `§ 12.54` 가 확정한 REST runner 경유 결선은 `backfill-runner.service.ts` **18 · 108** 행 = **`src/scheduling/` 소속이라 (iv) 외연상 `scheduler` node 실적** 이고, 그 밖 REST 진입은 worker 자신의 controller (`assessment-collection.controller.ts` **49** 행) 소관이다. 실제 의존 방향은 **worker → backend_api** 로 edge 표기와 반대다. |
| ⑥ label `in-process method call` 의 규약 · ADR 승계 (**70 ~ 73** 행) | (vi) — **111** 행 정의 + ADR-0003 **32 · 34** 행 `Decision §1` | **참** | 어구 자체는 표기 규약과 monolithic 결정을 정확히 승계한다 — **결선 실재 여부와 별개 축** 이므로 ② ~ ⑤ 의 거짓이 본 축을 뒤집지 않는다. |

**위 판정은 (iv) 의 `backend_api` 외연 정의 (자기 node 를 가진 디렉토리 제외, controller 13 개 기준) 위에서만 유효** 하다 — [modules.md](../architecture/modules.md) **200** 행의 **1:N** 매핑처럼 `AssessmentCollectionModule` · `AssessmentEvaluationModule` 의 **controller 책임까지 Backend API 로 귀속** 시키는 계층형 외연을 취하면 ⑤ 는 **부분참** 이 된다 (한계 2 참조).

#### 처리 방식 판정 (AC 3)

| 후보 | ① append-only (`§ 12.15`) | ② 좌표 drift ((viii)) | ③ cap (3 파일 / ≤ 300 LOC) | ④ 탐색성 | 결과 |
| --- | --- | --- | --- | --- | --- |
| (A) 현행 유지 + 무편집 | 정합 | 0 | 통과 | 최악 | **기각** — 축 ② ~ ⑤ 가 **거짓** 이라 오도가 다이어그램에 그대로 남는다 (AC 3 의 자동 기각 조건). |
| (B) 각주군 말미 append + stale 좌표 ≤ 2 지점 정정 | 정합 (판정 문장 신설, 기존 문장 무수정) | **3** | 통과 (+6/-1) | 좋음 — 선행 각주 2 블록과 같은 자리 | **채택** |
| (C) `## Component diagram` 절 안 삽입 | 정합 | **21** (ⓑ 의 7 배) | 좌표 전수 정정이 cap 압박 | 최상 | **기각** — 자기 참조 drift 21 지점을 새로 만들고 그 정정이 (B) 대비 이득을 상쇄한다. |
| (D) mermaid edge · label in-place 수정 | **위반** (기존 서술 파괴 · 판정 이력 소실) | — | — | — | **기각** — ① 축에서 먼저 탈락한다. |

**mermaid edge 를 지우거나 방향을 뒤집거나 label 을 고쳐 쓰는 선택지는 채택하지 않는다** — 다이어그램 구조 변경은 본 doc-audit stream 의 scope 밖이라 축 ② ~ ⑤ 가 **거짓** 이어도 처리는 **각주 병기** 로만 한다. **코드를 고쳐 결선을 만드는 처리도 금지** — 파생 영향 (17) 의 `pr` task 소관이다.

#### 반영 결과 (AC 4)

- components.md 각주군 말미 (**192** 행 뒤, `## GitHub Adapter …` heading 직전) 에 blockquote **4 행 + 앞 빈 줄 1 행 = +5 행** 을 신설해 (iv) 외연 정의 · 축 ① ~ ⑥ 판정을 병기했다.
- in-place 정정은 **1 지점 (숫자 2 개)** — **192** 행 `@Cron` 중복 좌표 `234 · 235` → `244 · 245` (편집 후 `grep -n '^| Scheduler |'` 실측값). (vii) 이 stale 로 확정한 지점 외 문장 재작성은 **0**.
- **재-drift 5 회째 재현 확인** — `§ 12.51` `175`→`180`, `§ 12.52` `180`→`184`, `§ 12.53` `184`→`189`, `§ 12.54` `189`→`194` 에 이어 본 절 삽입으로 `## GitHub Adapter …` heading 이 **194 → 199** 로 다시 밀렸고 (`grep -n '^## '` 재측정: `5 · 22 · 28 · 115 · 199 · 231 · 257`), 그 결과 **161** 행이 인용한 `**194**` 가 **새로 stale** 이 됐다. AC 4 의 in-place ≤ 2 지점 제약 안이라 본 절에서 고치지 않고 파생 영향 (14) 의 근거로 보탠다 — **행 번호 좌표계 자체가 5 연속 slice 에서 자기 무효화** 를 재현했다.
- mermaid 블록 · `다이어그램 표기` bullet · 표 본체 · 1 ~ 4 행 blockquote · `## 개요` 각주 · 안내 blockquote · 기존 각주 10 블록의 판정 문장 · 신규 각주 뒤 전 구간 **무편집**.

#### 불변 검산 (AC 6)

- `wc -l` — components.md **262 → 267** (+5, 상한 269 이내) · audit **5352 → 5422** (+70, 본 절 자체는 **70** 행으로 상한 100 이내) · **ADR-0003 173 불변** · requirements.md **97 불변** · deployment.md **232 불변** · directory.md **203 불변** · modules.md **259 불변** · PLAN.md **175 불변**.
- `git diff -U0 -- docs/architecture/components.md | grep '^@@'` → **`@@ -192 +192,6 @@` 단일 hunk** — AC 4 허용 구간 (각주군 말미) 밖 hunk **0**.
- `git diff --numstat` → `6 1 docs/architecture/components.md` — 삭제 **1** 행은 **192** 행 stale 숫자 치환의 짝 (같은 행 재출력) 이라 **순수 삭제 0**.
...
- `grep -c '^## '` audit **12 불변** · `grep -c '^| REQ-'` **66 불변** · `grep -c '^### 12\.'` **54 → 55** · components.md `grep -c '^## '` **7 불변** · `grep -c '^> '` **61 → 65**.

#### R-110 / R-112 면제 (AC 8)

본 task 는 `commitMode: direct` doc-only 로 production code **0 LOC** · 분기 **0** 이라 [CLAUDE.md](../../CLAUDE.md) §3.2 의 direct-mode 면제 조항에 따라 `tester` 호출 · happy / error / flow / negative 4 항목 · `pnpm test:cov` 가 모두 **N/A** 다 (측정은 전부 read-only `grep` · `sed` · `wc` · `git`).

#### 파생 영향 (목록만 — 본 slice 편집 금지)

(1) **나머지 edge 그룹 대조 4 건** — `%% User-facing flow` **2** · `%% Worker pipeline` **4** · `%% DB persistence boundary` **1** · `%% External egress` **9** ((ii) 재확인). **다음 대조 1 순위는 `%% Worker pipeline` 4 개 (80 ~ 83 행)** — 본 절이 ② ~ ⑤ 의 실 소비자를 전부 `worker` 로 귀속시켰으므로 그 반대편 결선이 실제로 성립하는지가 곧바로 이어지는 축이다. / (2) `## GitHub Adapter — 3 instance 묶음 vs 분리 결정` 본문 ↔ 코드 대조 (`§ 12.48` FU4). / (3) **`## Contracts` 표 ↔ 실 계약 표면 대조** — 본 절 (vii) 이 `Backend API` from row **5** 개의 1:1 좌표 (편집 후 **239 ~ 243**) 를 재료로 남겼다. / (4) row pointer 셀 보강 2 건 (`Scheduler` = `ADR-0042` 미등재 `§ 12.50` FU2 · `Confluence Adapter` `§ 12.49` FU2). / (5) LLM · GitHub adapter ADR pointer 미등재 (`§ 12.47` FU5 · `§ 12.48` FU3). / (6) `@nestjs/config` 미도입 전수 sweep (`§ 12.39` FU3, ADR 게이트). / (7) reviewer 규약 미이행 (`§ 12.41` FU2). / (8) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (`§ 12.41` FU3). / (9) README 행 번호 pointer drift 전수 sweep. / (10) REQ 번호 체계 잔재 sweep (`§ 12.38` FU3). / (11) `CLAUDE.md` §1 pointer 부정확 (T-1442 FU3). / (12) UC-09 `§ 5` sequence participant 병기 (**43 회째 이월**). / (13) modules.md 카운트 claim 대조 (`§ 12.34` FU1, ADR 게이트). / (14) **행 번호 → anchor 좌표계 이행** (**37 회째 이월** — 본 절 AC 4 의 재-drift **5 회째** 재현과 **161** 행 신규 stale 이 근거). / (15) 각주 heading 참조 anchor 이행 축소 scope (`§ 12.51` FU19). / (16) `§ 12.44` 한계 "mutation 러너 26 개" 정의 미확정. / (17) **`Scheduler` cron → 평가 pipeline 미결선** (`§ 12.50` FU18 · `§ 12.54` 재확인 — **코드 소관, `pr` task 로만**). / (18) `ADR-0003` "단일 DB 인스턴스" 좌표 부재 (`§ 12.46` FU16). / (19) `Web UI` node 의 process subgraph 소속 표기 (`§ 12.53` FU19). / (20) **`Backend API` node 외연 정의의 문서 미박제** — 본 절 (iv) 가 판정 기준으로 세운 정의는 audit 과 신규 각주에만 있고 components.md 본문 (표 · bullet) 에는 없다. / (21) **modules.md 200 행 1:N 매핑 ↔ (iv) 디렉토리 외연의 상충 해소** — 두 외연이 ⑤ 의 판정을 `거짓` / `부분참` 으로 가르므로 어느 쪽을 정본으로 삼을지 별도 slice 가 결정해야 한다.

#### 한계

1. **orchestration edge 5 개만 닫았다** — 나머지 **18** edge 의 결선 · label 판정은 파생 영향 (1) 소관이며, 본 절이 방증으로 인용한 `worker --> llm_gateway` 등 (**80 ~ 83** 행) 도 그쪽에서 정식 판정한다.
2. **⑤ 의 판정은 외연 정의에 의존한다** — (iv) 의 디렉토리 기준에서는 **거짓** 이지만 modules.md **200** 행의 계층형 1:N 매핑 (module 의 controller 책임 = Backend API, service layer = Worker) 을 취하면 worker 자신의 manual-trigger controller 가 backend_api 실적이 되어 **부분참** 이 된다. 본 절은 `§ 12.53` 이 확정한 디렉토리 매핑을 승계했을 뿐 두 외연의 우열을 판정하지 않았다 — 파생 영향 (21).
...
### 12.62 components.md `## Contracts` 표 **`Backend API` 발신 5 row** (273 ~ 277 행) ↔ 실 `src/` 호출 그래프 · in-process seam 대조 — row 1 참 · 부분참 3 · 거짓 1 확정 · orchestration 그룹 5/5 마감 (T-1464)

`§ 12.61` 이 `## Contracts` 축을 열어 census (data row **17** ↔ mermaid edge **23** · 미등재 edge **0** · fan-out 축약 **6**) 를 확정하고 user-facing **2** row 를 판정하며 파생 영향 **(1)** 에서 지목한 **`Backend API` 발신 5 row (273 ~ 277 행)** 를 그대로 승계하는 slice 다. 동시에 `§ 12.55` 가 자기 각주 (components.md **197** 행) 에서 "대응 `## Contracts` row **5** 개 (**273 ~ 277** 행) 의 판정은 파생 영향 소관" 이라고 **명시 이월** 한 것을 회수한다. **`backend_api` node 외연 (13) 은 본 절 신설이 아니라 `§ 12.55` 승계 + 유효성 재검증** 이다 (`§ 12.61` 이 브라우저 outbound seam 을 `§ 12.59` 에서 승계한 것과 동형). **edge 축 · node 축 · `## Component table` row 본문 · `## Contracts` 잔여 10 row 는 재판정하지 않는다.**

**AC 1 실측 (명령 + 출력).**

- (i) 좌표 — `grep -n '^#\{1,3\} ' docs/architecture/components.md` → `1 # Component view` · `5 ## 개요` · `22 ## Deployment 컨텍스트` · `28 ## Component diagram` · `115 ## Component table` · `233 ## GitHub Adapter …` · `265 ## Contracts` · `299 ## References` (편집 전). `grep -n '^| '` → `## Component table` 표 **117 ~ 126** · `## Contracts` 표 **269 ~ 287** 로 좌표 분리. `grep -n '^> ' … | tail -12` → 마지막 blockquote 행 = **297** (T-1463 각주 말미, 삽입점 후보 ⓐ). 본 task 의 265 · 299 · 297 예고는 전부 **정합 (stale 0)**.
- (ii) census 승계 — `awk 'NR>=265' … | grep -c '^| '` → **19**, header · 구분 **2** 를 빼 **data row 17** 재확인. `§ 12.61` 의 그룹별 edge : row (user-facing **2:2** · orchestration **5:5** · scheduler **2:2** · worker **4:4** · db boundary **1:1** · external egress **9:3**) 를 **1 구 인용 승계** 하며 **재산출하지 않는다**. 본 절 종료 시 **표 17 중 7 판정 완료 · 잔여 10** (`§ 12.61` 의 2/17 에 **+5**).
- (iii) 대상 row 원문 — `sed -n '269,277p' docs/architecture/components.md`. header **269** = `| from | to | sync/async | message format | 비고 |`, 구분 **270**. 5 컬럼 분해:

| 행 | from | to | sync/async | message format | 비고 |
| --- | --- | --- | --- | --- | --- |
| 273 | Backend API | DB Persistence | sync | Prisma typed query (in-process) | ADR-0002. NestJS DI container 의 PrismaService singleton 경유. |
| 274 | Backend API | LLM Gateway | sync | TypeScript method call (in-process) | ADR-0003 §1 monolithic — 동일 process 내 method call. provider 추상화 layer. |
| 275 | Backend API | GitHub Adapter | sync | TypeScript method call (in-process) | 동일 process. 본 contract 가 외부 GitHub 호출의 단일 진입점. |
| 276 | Backend API | Confluence Adapter | sync | TypeScript method call (in-process) | 동일 process. |
| 277 | Backend API | Worker | sync (또는 fire-and-forget) | TypeScript method call (in-process) | manual trigger flow. fire-and-forget 시 Worker 내부에서 background task. 구체는 P5. |

- (iv) `backend_api` 외연 재검증 — `grep -rln '@Controller(' src --include=*.ts | grep -v spec | wc -l` → **20** (파일 기준; `grep -rn` 선언 행 기준은 **22**). 디렉토리 분포 `… | cut -d/ -f2 | sort | uniq -c` → `app.controller.ts` **1** · `assessment-collection` **1** · `assessment-evaluation` **1** · `auth` **1** · `export` **1** · `import` **1** · `llm` **2** · `permission-denied` **1** · `scheduling` **3** · `user` **7** · `user-instance-access` **1**. 자기 node 를 갖는 **7** (scheduling 3 = `scheduler` · collection|evaluation 2 = `worker` · llm 2 = `llm_gateway`) 을 빼 **13** — `§ 12.55` (components.md **194** 행) 의 정의와 **동일**, T-1457 이후 controller 증감 **0** 이라 승계 정의가 판정에 미치는 영향도 **0** 이다.
- (v) row 273 — ⓐ AC 가 지목한 `src/persistence/prisma.module.ts` 는 **부재** 이고 실 파일명이 `persistence.module.ts` 라 대상을 치환해 측정했다: `grep -n '@Global\|@Injectable\|Scope\.' src/persistence/persistence.module.ts src/persistence/prisma.service.ts` → `persistence.module.ts:12:@Global()` · `prisma.service.ts:28:@Injectable()` (scope 인자 **0**). `grep -rn 'Scope\.REQUEST\|Scope\.TRANSIENT' src … | wc -l` → **0**. ⓑ `grep -n 'providers\|exports' src/persistence/persistence.module.ts` → **14** `providers: [PrismaService]` · **15** `exports: [PrismaService]`. ⓒ `grep -rln 'PrismaService' src --include=*.ts | grep -v spec` → 전체 **41** 파일, backend_api 외연 안 **26** (`§ 12.55` 의 26 과 동일 — 언급 기준) · import 기준 **17**; import 출처는 `…/persistence/prisma.service` **21** 파일 + `./prisma.service` (persistence 내부) **2** = **23** 로 **우회 0**. ⓓ `grep -rn 'new PrismaClient' src … | wc -l` → **0**.
- (vi) row 274 · 275 · 276 — ⓐ `§ 12.55` 의 **backend_api 디렉토리군 hit 0** (LLM · GitHub · Confluence) 을 components.md **196** 행 1 구로 인용 승계. ⓑ `grep -rln 'LLM_GATEWAY\|LlmGateway' src … | cut -d/ -f2 | sort | uniq -c` → `app.module.ts` **1** (DI 배선) · `assessment-evaluation` **3** · `llm` **5**. ⓒ `GithubInstanceClient|GithubCollection` → `app.module.ts` **1** · `assessment-collection` **6** · `confluence` **1** · `github` **2**. ⓓ `ConfluenceAdapterService|ConfluenceSpaceTraversal` 주입 → `assessment-collection` **3** · `confluence` **4** (외연 안 `Confluence` 문자열 **7** hit 은 `permission-denied` emitter 이름 **2** · `user` DTO/주석 **3** · `user-instance-access` **2** 로 전부 호출이 아니다). ⓔ 단일성 — `grep -rn 'api.github.com\|Octokit\|@octokit' src … | cut -d: -f1 | sort -u` → `src/github/github-adapter.service.ts` · `github-live-test-gating.ts` · `github-request.builder.ts` **3** 개뿐. 본 축은 **row 셀 문구 축** 이며 edge 재판정이 아니다.
- (vii) row 277 — ⓐ `grep -rn 'AssessmentEvaluation\|AssessmentCollection' src … | wc -l` → **40** 지점 / **9** 파일 (`app.module.ts` **1** · `assessment-collection` **3** · `assessment-evaluation` **3** · `llm` **1** · `scheduling` **1**) 이고 backend_api 외연 디렉토리 한정 재측정은 **0** hit — `§ 12.55` 의 "호출 0 · 역방향 import 22 지점" (components.md **197** 행) 이 방향 축 그대로 재확인된다 (지점 수는 grep 범위 차로 40). ⓑ `grep -rn 'void this\.\|setImmediate\|\.catch((' src/scheduling src/assessment-evaluation …` → **0**; `fire-and-forget` 문자열 실 파일은 `src/permission-denied/persisting-permission-denied-emitter.ts` · `…-confluence-…ts` (**49** · **58** 행 `void this.recordService`) 와 `src/scheduling/cron-schedule.controller.ts` 주석뿐. ⓒ `ls src/assessment-evaluation/*.service.ts | wc -l` → **12**; `grep -n 'P5' docs/PLAN.md` → **94** `## Phase P5 — Evaluation pipeline`; `grep -n '"phase"' docs/STATE.json` → **3** `"phase": "P4-complete / P5-in-progress"`.
- (viii) 축 ⑥ · ⑦ — ⓐ **289** 행 정의문 원문: `"sync" 는 호출자 thread 가 응답까지 await 한다는 의미 (in-process 또는 외부 HTTPS 둘 다 포함). "async" 는 외부 HTTPS 경계를 넘는 호출`. ⓑ `grep -n 'Decision' docs/decisions/ADR-0003-deployment.md` → **32** `### Decision §1 — Monolithic NestJS process (in-process queue OK)` · **34** 본문이 "HTTP API / scheduler / 평가 파이프라인 / LLM gateway / GitHub & Confluence adapter 가 동일 process" 명시. `grep -c 'Prisma' docs/decisions/ADR-0002-db.md` → **20**, `grep -c 'DI\|singleton\|Injectable'` → **0**. `sed -n '117p'` → `| component | 책임 | 입력/출력 contract | 관련 REQ | 관련 ADR / 문서 |` ↔ `## Contracts` header (**269** 행) 는 **5** 컬럼이며 `관련 REQ` 컬럼 **없음**; 17 row 중 비고 셀에 REQ 를 인라인 병기한 row 는 **5** (**271 · 272 · 283 · 285 · 286**).
- (ix) 좌표 stale · 삽입 파급 — `grep -n '\*\*[0-9]\{2,3\}\*\* 행' …` + heading 실측 대조로 **stale 0 지점** (T-1461 **6** → T-1462 **0** → T-1463 **0** 에 이어 **4 회째 추세**: 편집이 문서 말미 append 로만 이뤄지는 한 자기 좌표는 밀리지 않는다). 좌표 토큰 총 **217** 개 중 계수 규칙 (**`§ 12.55` → `§ 12.61` 계수 규칙 승계** — components.md 자기 좌표 토큰만 세고 외부 파일 좌표는 제외, 범위 · 나열 토큰 `A ~ B` · `A · B` 는 1 지점) 적용 결과: 삽입점 ⓐ **T-1463 각주 뒤 (297 행 뒤)** → 밀리는 자기 좌표 **0 지점**, 삽입점 ⓑ **각주군 말미 (231 행 뒤)** → **11 지점** (161 `233` · 197 `273 ~ 277` · 203 `280 ~ 283` · 210 `285 · 286` · 217 `287` · 224 `271 · 272` · 231 `284` · 291 `271 · 272` · 291 `273 ~ 287` · 292 `267` · 297 `289`).
...
**진척.** 본 절로 `## Contracts` 표 **17 row 중 7 판정 완료 · 잔여 10** (**278 ~ 287** 행) 이며 **orchestration 그룹 5/5 마감** 이다.

**AC 2 판정표.** (판정은 (iv) 의 `backend_api` node 외연 **13** 위에서만 유효하며, **그 정의는 본 절 신설이 아니라 `§ 12.55` 승계 + 재검증** 이다.)

| 축 | 실측 근거 | 판정 | 근거 1 구 |
| --- | --- | --- | --- |
| ① row **273** 의 `NestJS DI container 의 PrismaService singleton 경유` | (v) ⓐ ~ ⓓ — `prisma.service.ts` **28** 행 `@Injectable()` · `persistence.module.ts` **12** 행 `@Global()` · **14 ~ 15** 행 providers/exports · `Scope.*` **0** · `new PrismaClient` **0** · 외연 안 사용 **26** 파일 · 우회 **0** | **참** | 결선은 `§ 12.55` (components.md **195** 행) 승계 참이고 DI scope 도 default singleton 이 코드로 확증된다 |
| ② row **274** 의 `Backend API → LLM Gateway` 결선 실재 | (vi) ⓐⓑ — 소비 디렉토리 `assessment-evaluation` **3** · `llm` **5** · `app.module.ts` **1**, backend_api 외연 hit **0** | **거짓** | `§ 12.55` edge 70 거짓 판정의 **인용 승계** 이며 edge 재판정이 아니라 row 셀 문구 축이다 |
| ③ row **275** 의 결선 실재 + `본 contract 가 외부 GitHub 호출의 단일 진입점` | (vi) ⓐⓒⓔ — 소비 `assessment-collection` **6** · `github` **2** · `confluence` **1**, 외연 hit **0** / octokit 계열 hit 파일 `src/github/` **3** | **부분참** | (가) **발신 주체는 거짓** (실 소비자는 `worker`), (나) **단일성 자체는 참** 이나 그 진입점의 주체는 본 contract 가 아니라 `github_adapter` 다 |
| ④ row **276** 의 결선 실재 + `동일 process` | (vi) ⓐⓓ — 주입 `assessment-collection` **3** · `confluence` **4**, 외연 안 `Confluence` **7** hit 은 전부 주석 · DTO · emitter 이름 | **부분참** | 결선은 거짓 (`§ 12.55` edge 72 승계) 이나 `동일 process` 는 [ADR-0003](../decisions/ADR-0003-deployment.md) **32 · 34** 행 승계 참이다 |
| ⑤ row **277** 의 `sync (또는 fire-and-forget)` + `manual trigger flow` + `구체는 P5` | (vii) ⓐⓑⓒ — 외연 → worker **0** hit · 역방향 **40** 지점 / **9** 파일 · fire-and-forget 실 구현 `permission-denied` **2** · phase `P4-complete / P5-in-progress` · eval service **12** | **부분참** | 결선 · fire-and-forget · trigger 경로 3 요소는 거짓이나 `구체는 P5` 미래 시제는 그 결선이 **아직 미이행** 이라 **참** — row 272 의 `구체는 P3 Auth task` 와 결론이 갈린다 |
| ⑥ 5 row 의 `sync/async` 값 (4 `sync` + 1 이중) ↔ **289** 행 정의문의 in-process 판별력 | (viii) ⓐ — 정의의 `sync` 가 in-process · 외부 HTTPS 를 모두 포함 | **부분참** | 정의는 존재하나 5 row 를 같은 값으로 수렴시켜 판별력 **0** 이고 `fire-and-forget` 는 분류 항목 자체가 없다 (`§ 12.61` 축 ⑦ 경계 미규정을 승계하되 본 축은 **in-process 쪽** 면) |
| ⑦ 비고 셀 ADR pointer 정합 + REQ pointer **0** | (viii) ⓑ — ADR-0003 **32 · 34** drift **0** · ADR-0002 `Prisma` **20** / `DI\|singleton\|Injectable` **0** · header **269** 5 컬럼 vs **117** 6 컬럼 | **부분참** | ADR 좌표는 전부 실재하나 273 행 DI 문구의 정본은 ADR 이 아니라 코드이고, REQ pointer **0** 은 `## Contracts` 에 REQ 컬럼이 **없어** 결손이 아니다 (다만 17 중 **5** row 만 비고 인라인 병기 — 관행 불균일) |

다중 표기 수치는 **row 5 : 실 결선 1 : 실 발신 node 2** (`§ 12.58` `5 : 1 : 4` · `§ 12.59` · `§ 12.61` `2 : 1 : 1` · `§ 12.60` `1 : 1 : 29` 와 같은 형식) — row 5 개 중 실재 결선은 **273** 뿐이고, 나머지 4 의 실 발신 node 는 `worker` (274 · 275 · 276) 이거나 **부재** (277) 다.

**AC 3 처리 방식 판정 (채택 1 · 기각 3).**

| 후보 | ① append-only (`§ 12.15`) | ② 좌표 drift 파급 | ③ cap (≤ 300 LOC · 3 파일) | ④ 탐색성 | 판정 |
| --- | --- | --- | --- | --- | --- |
| (A) 현행 유지 + 무편집 | 정합 | **0** | 정합 | 판정이 표 곁에 남지 않아 다음 slice 가 재측정한다 | **기각** — 축 ② 가 `거짓` · ③ ~ ⑦ 이 `부분참` 이라 자동 기각 조건에 걸린다 |
| (B) 각주군 말미 (231 행 뒤) append | 정합 | **11 지점** | 정정 지점이 상한 **8** 초과 | 판정 대상 표와 **74 행** 떨어진다 | **기각** — 축 ② · ③ 에서 상한 초과 (split 제안은 파생 영향 (29) 에 기록) |
| (C) `## Contracts` 절 안 · T-1463 각주 직후 (297 행 뒤) | 정합 (순수 append) | **0 지점** | 정합 (+8 행) | 판정 대상 표 바로 아래 · 직전 slice 각주와 연속 | **채택** |
| (D) 표 셀 · 서문 · 289 행 정의문 in-place 수정 | **위반** | — | — | — | **기각** — 축 ① 에서 먼저 기각 (`§ 12.15` append-only) |

**각주 위치 관행 확정 (`§ 12.61` 한계 6 소진).** `## Contracts` 축 각주는 **`## Contracts` 절 안 · 직전 slice 각주 뒤에 이어 붙인다**. 근거는 축 ② (파급 **0** vs **11**) 와 축 ④ (판정 대상 표와의 거리) 실측이며, **다음 slice 는 이 관행을 재고하지 않는다**.

**AC 4 반영 결과.** (C) 대로 components.md **297** 행 뒤에 **빈 줄 1 행 + blockquote 7 행** 을 삽입했다 (기존 각주 블록 간 구분이 전부 빈 줄 1 행이라 **분리** 를 택했다 — 연속 이어붙임은 T-1463 각주와 한 블록으로 렌더링돼 slice 경계가 사라진다). `wc -l` **309 → 317** (+8, 상한 내), in-place 정정 **0 지점** ((ix) 가 stale **0** 으로 확정). 편집 후 `grep -n '^## '` 재측정 → `## References` **299 → 307** 만 이동하고 나머지 **6** heading 불변이라 **자기 좌표 재-drift 는 0** — `§ 12.51` → `§ 12.60` 의 재-drift 사슬은 `§ 12.61` 에 이어 본 절에서도 **재현되지 않았다**. 무편집 경계 준수: 표 본체 (269 ~ 287) · 서문 (267) · 289 행 정의문 · T-1463 각주 (291 ~ 297) · mermaid (30 ~ 106) · 표기 bullet (108 ~ 113) · `## Component table` 표 (117 ~ 126) · 1 ~ 4 행 blockquote · 각주 16 블록 · `## References` 절 전부 **무편집**, `src/` · `docs/decisions/` 도 무편집이다.

**파생 영향 (목록만 — 본 slice 편집 금지).** (1) **다음 slice 1 순위 = `## Contracts` 잔여 10 row 중 `Scheduler` 발신 2 row (278 · 279 행)** — 대응 edge 2 개 (**76 · 77** 행) 를 edge 축 첫 slice 가 이미 닫아 판정 입력이 그대로 재사용되므로 한계 비용 최저 (잔여 그룹 분해: Scheduler **2** · Worker **4** (280 ~ 283) · DB→PostgreSQL **1** (284) · 외부 egress **3** (285 ~ 287)) / (2) fan-out 축약 **6** 의 표 등재 여부 (`§ 12.61` FU2 미소진 — 표 편집이라 본 stream 밖) / (3) `## GitHub Adapter — 3 instance 묶음 vs 분리 결정` 본문 ↔ 코드 대조 (`§ 12.48` FU4) / (4) row pointer 셀 보강 2 건 (`§ 12.50` FU2 · `§ 12.49` FU2) / (5) LLM · GitHub adapter ADR pointer 미등재 (`§ 12.47` FU5 · `§ 12.48` FU3) / (6) `@nestjs/config` 미도입 전수 sweep (`§ 12.39` FU3, ADR 게이트) / (7) reviewer 규약 미이행 (`§ 12.41` FU2) / (8) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (`§ 12.41` FU3) / (9) README 행 번호 pointer drift 전수 sweep / (10) REQ 번호 체계 잔재 sweep (`§ 12.38` FU3) / (11) `CLAUDE.md` §1 pointer 부정확 (T-1442 FU3) / (12) UC-09 `§ 5` sequence participant 병기 (**53 회째 이월**) / (13) modules.md 카운트 claim 대조 (`§ 12.34` FU1, ADR 게이트) / (14) **행 번호 → anchor 좌표계 이행** (**47 회째 이월** — 본 절도 삽입점 선택만으로 파급을 **11 → 0** 으로 줄여 좌표계 교체의 긴급도를 낮춘다) / (15) 각주 heading 참조 anchor 이행 축소 scope (`§ 12.51` FU19) / (16) `§ 12.44` 한계 "mutation 러너 26 개" 정의 미확정 / (17) `Scheduler` cron → 평가 pipeline 미결선 (`§ 12.50` FU18 — 코드 소관, `pr` task) / (18) `ADR-0003` "단일 DB 인스턴스" 좌표 부재 (`§ 12.46` FU16 · `§ 12.60` FU17) / (19) `Web UI` node 의 process subgraph 소속 표기 (`§ 12.53` FU19 · `§ 12.61` FU19) / (20) **node · row 외연 정의의 문서 미박제** (`§ 12.55` FU20 · `§ 12.60` FU19 · `§ 12.61` FU20 — 본 절이 `backend_api` 외연 **13** 을 **한 번 더 승계** 해 정의가 audit 절에만 살아 있는 구조가 4 회째 반복됐다는 점이 박제 필요의 근거를 더한다) / (21) modules.md **200** 행 1:N 매핑 ↔ 디렉토리 외연 상충 (`§ 12.56` FU21) / (22) 가변 instance 수 ↔ 고정 표기 정합 (`§ 12.57` FU22 · `§ 12.58` FU22) / (23) **`worker --> backend_api` 미표기 결선** (`§ 12.56` FU23 — 본 절 축 ⑤ 가 역방향 참조 **40** 지점 / **9** 파일 로 재검증했다) / (24) dev / prod 2 모드의 다이어그램 · 표 미분리 (`§ 12.59` FU24 · `§ 12.61` FU24) / (25) `prisma migrate deploy` 채널 미표기 + `PostgreSQL 16+` version claim 정정 (`§ 12.60` FU25) / (26) 인증 규약의 문서 단일 정본 부재 (`§ 12.61` FU26) / (27) **`sync/async` 컬럼 정의 (289 행) 의 경계 미규정** (`§ 12.61` FU27 — 본 절 축 ⑥ 이 **in-process 판별력 0** 과 `fire-and-forget` 분류 부재를 더했다) / (28) **`## Contracts` 표의 REQ pointer 컬럼 부재** — 축 ⑦ 이 `## Component table` 과의 규약 차이를 확정했고 17 중 **5** row 만 비고 인라인 병기라 관행이 불균일하나, 컬럼 신설 · 셀 보강은 표 편집이라 본 stream 밖 / (29) **(B) 후보 split 제안** — (B) 는 좌표 정정 **11 지점** 이 상한 **8** 을 넘어 cap 사유로 기각됐으므로, 각주군 말미 이전이 필요해지면 "좌표 정정 전용 slice" 로 분리한다.

**AC 8 — R-110 / R-112 면제 근거.** 본 task 는 `commitMode: direct` doc-only 로 production code **0 LOC** · 분기 **0** 이라 [CLAUDE.md](../../CLAUDE.md) §3.2 direct-mode 면제 조항에 따라 `tester` 호출 · happy / error / flow / negative 4 항목 · `pnpm test:cov` 가 전부 **N/A** 다.

**불변 검산.** `wc -l` — components.md **309 → 317** (+8) · audit **5898 → 5968** (+70, 100 이내) · ADR-0003 **173 불변** · ADR-0002 **127 불변** · requirements.md **97 불변** · deployment.md **232 불변** · directory.md **203 불변** · modules.md **259 불변** · PLAN.md **175 불변**. `grep -c '^## '` components.md **7 불변** · audit **12 불변**, audit `grep -c '^| REQ-'` **66 불변** · `grep -c '^### 12\.'` **61 → 62**, components.md `grep -c '^> '` **101 → 108**. `git diff -U0 -- docs/architecture/components.md | grep '^@@'` → **`@@ -298,0 +299,8 @@` 1 개** 로 AC 4 허용 구간 (297 행 뒤) 밖 hunk **0**. `git diff --numstat` → components.md **8 / 0** 이라 **순수 삭제 0**. `git status --porcelain src/ web/ test/ prisma/ deploy/ docker-compose.yml Dockerfile .github/ package.json README.md .claude/ docs/decisions/ docs/ops/ docs/PLAN.md docs/requirements.md docs/architecture/modules.md` → **빈 출력** (특히 **`src/persistence/` · `src/assessment-*/` 무편집**), `git status --porcelain` 전체 **2 파일** (components.md · REQ-COVERAGE-AUDIT.md — 상한 **3** 이내이며 task 파일 frontmatter `status` 갱신은 driver bookkeeping commit 소관이라 본 commit 에 포함되지 않는다).

**한계.**

1. **측정은 grep 정적 근사다** — 호출 그래프를 실행해 확인하지 않았고 `pnpm` 계열 명령은 전부 Out of Scope 였다. DI 주입은 문자열 hit 으로 근사했으므로 token 기반 간접 주입 (`useExisting` · custom provider) 은 과소 계수될 수 있다.
2. **`singleton` 판정은 NestJS default scope 규약에 의존한다** — `@Injectable()` 에 scope 인자가 없으면 singleton 이라는 framework 계약을 전제했고 런타임 instance 동일성을 실측하지 않았다. pool size · transaction 경계 · shutdown hook 적정성은 열지 않았다 (`pr` task 소관).
3. **역방향 참조 지점 수가 `§ 12.55` 의 22 와 다르다 (40)** — 방향 축 결론 (backend_api → worker **0**) 은 동일하나 grep 패턴 · 범위가 달라 지점 수는 직접 비교할 수 없다.
4. **`구체는 P5` 판정은 결선 유무 기준이다** — P5 pipeline 자체의 완성도 · 잔여 작업 산정은 하지 않았고 (`docs/PLAN.md` · `docs/STATE.json` 무편집), phase 토큰만 읽었다.
5. **잔여 10 row 는 census 개수 재확인까지다** — 셀 본문 판정은 후속 slice 소관이며 285 · 287 row 의 다중 `to` 표기도 그대로 남는다.
6. **각주 위치 관행은 확정했으나 좌표계 자체는 그대로다** — 파생 영향 (14) 의 anchor 이행이 이뤄지기 전까지 삽입점 선택이 계속 파급 계산을 요구한다.

### 12.63 components.md `## Contracts` 표 **`Scheduler` 발신 2 row** (278 · 279 행) ↔ 실 `src/scheduling/` 호출 그래프 대조 — 축 8 중 거짓 3 · 부분참 5 확정 · scheduler 그룹 2/2 마감 (T-1465)

> **본 절의 위치 · 계보** — `§ 12.62` (T-1464) 가 `## Contracts` 표의 orchestration **5** row 를 닫으면서 파생 영향 **(1)** 로 **잔여 10 row 중 `Scheduler` 발신 2 row (278 · 279 행)** 를 다음 slice 1 순위로 지목했고 (근거: 대응 mermaid edge **2** 개 (**76 · 77** 행) 를 edge 축 첫 slice `§ 12.54` 가 이미 닫아 판정 입력이 그대로 재사용 — 한계 비용 최저), 본 절은 그 지목을 그대로 승계한다. 동시에 `§ 12.54` 가 자기 (vii) · 축 ⑤ 에서 **"`## Contracts` 2 row 의 참 / 거짓은 파생 영향 (3) 이 판정한다"** 고 **명시 이월** 한 것을 회수한다 (`§ 12.62` 가 `§ 12.55` 의 이월을 회수한 것과 **동형**). **`scheduler` node 외연 (= `src/scheduling/` 단일 디렉토리) 은 본 절 신설이 아니라 `§ 12.53` · `§ 12.54` 승계 + 유효성 재검증** 이다. **edge 축 · node 축 · `## Component table` row 본문 · `## Contracts` 잔여 15 row 는 재판정하지 않는다.**

**AC 1 실측 (명령 + 출력).**

- (i) 좌표 — `grep -n '^#\{1,3\} ' docs/architecture/components.md` → `1 # Component view` · `5 ## 개요` · `22 ## Deployment 컨텍스트` · `28 ## Component diagram` · `115 ## Component table` · `233 ## GitHub Adapter …` · `265 ## Contracts` · `307 ## References` (편집 전) — task 예고 **265 · 307 정합 (stale 0)**. `grep -n '^| '` → `## Component table` 표 **117 ~ 126** · `## Contracts` 표 **269 ~ 287** 로 좌표 분리. `grep -n '^| Scheduler |' …` → **278 · 279** 로 task 예고와 **일치** (예고된 `§ 12.54` 당시 좌표 234 · 235 로부터의 이동은 이미 반영된 상태였다). `grep -n '^> ' … | tail -10` → 마지막 blockquote 행 **305** (T-1464 각주 말미 = 삽입점).
- (ii) census 승계 — `awk 'NR>=265' … | grep -c '^| '` → **19**, header · 구분 **2** 를 빼 **data row 17** 재확인. `§ 12.61` 의 그룹별 edge : row (user-facing **2:2** · orchestration **5:5** · **scheduler 2:2** · worker **4:4** · db boundary **1:1** · external egress **9:3**) 를 **1 구 인용 승계** 하며 **재산출하지 않는다**. 본 절 종료 시 **표 17 중 9 판정 완료 · 잔여 8** (`§ 12.62` 의 7/17 에 **+2**).
- (iii) 대상 row 원문 — `sed -n '269,270p;278,279p' docs/architecture/components.md`. header **269** = `| from | to | sync/async | message format | 비고 |`, 구분 **270**. 5 컬럼 분해:

| 행 | from | to | sync/async | message format | 비고 |
| --- | --- | --- | --- | --- | --- |
| 278 | Scheduler | Worker | sync (handler 실행 자체) | NestJS `@Cron` decorator handler | ADR-0003 §3 in-process scheduler. cron 시각 도달 시 handler 직접 호출. |
| 279 | Scheduler | Backend API | sync | NestJS `@Cron` handler 가 controller/service 호출 | manual trigger 와 동일 service 메서드 호출 — duplication 0 (ADR-0003 §3). |

- (iv) `scheduler` 외연 재검증 — components.md **186** 행 (`scheduler` = `src/scheduling/`) · **189** 행 (`§ 12.54` 각주 머리 "본 각주는 scheduler edge 2 개 한정") 1 구 인용 승계. `ls src/scheduling/*.ts | wc -l` → **24** (spec 포함), spec 제외 **12**; `find src/scheduling -name '*.ts' | grep -v spec | wc -l` → **14** (top-level 12 + `dto/` 2) 로 `§ 12.50` 의 **14** 와 동일. `grep -rln '@Controller(' src/scheduling --include=*.ts | grep -v spec` → `backfill.controller.ts` · `cron-schedule.controller.ts` · `recent-deletion.controller.ts` **3** — `§ 12.62` (iv) 의 `scheduling 3` 과 동일. **T-1456 이후 증감 0 이라 승계 정의가 판정에 미치는 영향도 0** 이다 (재정의가 아니라 재검증).
- (v) row 278 — ⓐ `grep -rn '@Cron(' src --include=*.ts | grep -v spec | wc -l` → **0** (`§ 12.54` 의 0 hit 재확인 — 축 ② 뒤집힘 없음). ⓑ `grep -n 'SchedulerRegistry\|CronJob\|CronTime' src/scheduling/*.ts | grep -v spec | head -8` → `cron-schedule.service.ts` **17** `import { SchedulerRegistry } from "@nestjs/schedule"` · **18** `import { CronJob, CronTime } from "cron"` (+ **1 · 4 · 6 · 27 · 30 · 35** 행 주석) — dynamic 등록 경로 실재. ⓒ `grep -n 'CRON_TICK_HANDLER\|provide:\|useValue\|useFactory' src/scheduling/scheduling.module.ts | head -10` → **22** import · **36** `provide: CRON_TICK_HANDLER` · **37** `useFactory` · **81** `provide: ALREADY_BACKFILLED_CHECKER`; **35 ~ 45** 행 본문은 `logger.log("cron tick 발화 — 실 평가 pipeline 미결선(stub, Out of Scope)")` 만 실행하는 **no-op stub** 이라 `§ 12.52` · `§ 12.54` 판정 유지. ⓓ `grep -rn 'triggerCollection\|CollectionTriggerService' src … | cut -d: -f1 | sort -u | head -6` → `assessment-collection` **3** (`assessment-collection.controller.ts` · `.module.ts` · `collection-trigger.service.ts`) · `scheduling` **3** (`backfill-runner.service.ts` · `backfill.controller.ts` · `recent-deletion-runner.service.ts`) — 실 결선의 **진입은 REST** (`backfill.controller.ts`) 이지 cron 이 아니다.
- (vi) row 279 — ⓐ `grep -rn 'CronScheduleService' src … | cut -d: -f1 | sort -u` → `app.module.ts` · `scheduling/cron-schedule.controller.ts` · `scheduling/cron-schedule.service.ts` · `scheduling/dto/upsert-cron-schedule.dto.ts` · `scheduling/scheduling.module.ts` — `src/scheduling/` **내부 4** + `app.module.ts` **1** (DI 배선). ⓑ `grep -rln 'scheduling/' src … | grep -v '^src/scheduling' | head -6` → `src/app.module.ts` **1 개뿐**. ⓒ `grep -n 'CRON_TICK_HANDLER\|@Post\|@Put\|registerOrReplace' src/scheduling/cron-schedule.controller.ts` → **68** `export const CRON_TICK_HANDLER` · **82** `@Inject(CRON_TICK_HANDLER) private readonly tickHandler` · **103** `@Put()` · **108** `this.service.registerOrReplace(` (주입 handler 를 callback 인자로 전달) · **140** `@Post("trigger")` — manual trigger 와 cron tick 이 **같은 handler 추상 1 개** 를 공유하나 trigger 는 `CronScheduleService` 를 **거치지 않는다**.
- (vii) 축 ⑥ · ⑦ — ⓐ **289** 행 정의문 원문: `"sync" 는 호출자 thread 가 응답까지 await 한다는 의미 (in-process 또는 외부 HTTPS 둘 다 포함). "async" 는 외부 HTTPS 경계를 넘는 호출`. ⓑ `grep -n 'Decision' docs/decisions/ADR-0003-deployment.md | head -6` → **20 ~ 24** motivation 목록 · **28** `## Decision`; `sed -n '62p;70p'` → **62** `### Decision §3 — Scheduler 위치 = @nestjs/schedule (in-process)` · **70** `3. Decision §1 (monolithic) 과의 정합 — … trigger → handler 가 한 hop`. ⓒ REQ pointer **0** 은 `§ 12.62` 축 ⑦ (`## Contracts` header **269** 행 5 컬럼 · `관련 REQ` 컬럼 부재) 을 **1 구 인용 승계** 하며 재판정하지 않는다.
- (viii) 축 ⑧ — `grep -n '@Cron' docs/architecture/components.md` → **76** (다이어그램 label) · **126** (`## Component table` `Scheduler` row) · **143** · **175** (각주 2) · **191 · 192** (`§ 12.54` 각주 자신의 판정 문장) · **278 · 279** (`## Contracts` 2 row). **192** 행이 박제한 6 지점은 현재 **76 · 126 · 143 · 175 · 278 · 279** 로 전부 유효해 **stale 0 지점** 이다 — task 예고 (**234 · 235** 가 밀려 stale) 는 **반증** 이며, `git log -S'278 · 279' -- docs/architecture/components.md` → `b54c059e (T-1461)` 로 그 in-place 정정이 이미 수행됐음이 확인된다 (`§ 12.62` (ix) 가 기록한 "T-1461 6 지점 정정" 과 정합). audit `§ 12.54` (vii) 원문의 **234 · 235** 는 append-only 라 고치지 않는다.
- (ix) 좌표 stale · 삽입 파급 — `grep -n '\*\*[0-9]\{2,3\}\*\* 행' …` → **80** 행 / 토큰 **171** 개. 그중 **≥ 290** 인 토큰은 **6** 개 (151 `348` · 163 `370` · 167 `293` · 170 `326 · 353` · 205 `370` · 207 `370`) 인데 전부 `schema.prisma` · `github-adapter.service.ts` · `confluence-adapter.service.ts` 등 **외부 파일 좌표** 라 계수 규칙상 제외 — components.md **자기 좌표 stale 0 지점** (T-1461 **6** → T-1462 **0** → T-1463 **0** → T-1464 **0** 에 이어 **5 회째 추세**: 편집이 문서 말미 append 로만 이뤄지는 한 자기 좌표는 밀리지 않는다). 계수 규칙 (**`§ 12.55` → `§ 12.62` 승계** — components.md 자기 좌표 토큰만 세고 외부 파일 좌표는 제외, 범위 · 나열 토큰 `A ~ B` · `A · B` 는 1 지점) 적용: 삽입점 **T-1464 각주 뒤 (305 행 뒤)** → 밀리는 자기 좌표 **0 지점** (≥ 306 을 가리키는 자기 좌표 토큰이 **0** 개).
...
**진척.** 본 절로 `## Contracts` 표 **17 row 중 9 판정 완료 · 잔여 8** (**280 ~ 287** 행) 이며 **scheduler 그룹 2/2 마감** 이다.

**AC 2 판정표.** (판정은 (iv) 의 `scheduler` node 외연 = `src/scheduling/` 위에서만 유효하며, **그 정의는 본 절 신설이 아니라 `§ 12.53` · `§ 12.54` 승계 + 재검증** 이다.)

| 축 | 실측 근거 | 판정 | 근거 1 구 |
| --- | --- | --- | --- |
| ① row **278** 의 `Scheduler → Worker` 결선 실재 | (v) ⓑⓓ — `backfill-runner.service.ts` **108** 행 `triggerCollection` in-process 호출 · 진입은 `backfill.controller.ts` REST · cron 경로는 stub | **부분참** | `§ 12.54` 축 ① (edge 76 부분참) 의 **인용 승계** 이며 edge 재판정이 아니라 row 셀 문구 축이다 |
| ② row **278** 의 `NestJS @Cron decorator handler` message format | (v) ⓐⓑ — `@Cron(` **0 hit** · 실 등록은 `cron-schedule.service.ts` **17 ~ 18 · 50** 행 dynamic | **거짓** | edge label (`@Cron handler`) 보다 `decorator` 한 단어가 더해져 **정적 decorator 방식** 을 명시하므로 오기의 강도가 더 크다 |
| ③ row **278** 비고의 `cron 시각 도달 시 handler 직접 호출` | (v) ⓒ · (vi) ⓒ — controller **82 → 108** 행 handler 전달 (중간 layer 0) ↔ `scheduling.module.ts` **36 ~ 45** 행 no-op stub | **부분참** | 기계적 hop 은 참 (ADR-0003 **70** 행 `한 hop` 과 정합) 이나 handler 가 stub 이라 **Worker contract 로서는 미이행** 이다 |
| ④ row **279** 의 결선 실재 · 방향 | (vi) ⓐⓑ — `CronScheduleService` 주입이 `src/scheduling/` 내부 **4** + `app.module.ts` **1**, 밖에서의 import 도 `app.module.ts` **1** | **거짓** | 관측되는 유일한 결선은 controller → service 로 **node 내부** 이고 방향도 HTTP → scheduler 라 표기와 반대다 (`§ 12.54` 축 ③ 승계) |
| ⑤ row **279** 비고의 `manual trigger 와 동일 service 메서드 호출 — duplication 0` | (vi) ⓒ · (v) ⓓ — **140** 행 `@Post("trigger")` 와 **108** 행 cron callback 이 **68** 행 `CRON_TICK_HANDLER` 단일 추상 공유 | **부분참** | (가) `duplication 0` 은 **사실 참**, (나) `동일 service 메서드 호출` 경로 표기는 **거짓** (`§ 12.50` 승계), (다) 축 ④ 상 발신 자체가 부재라 비고 전체는 **미이행 설계 의도** 다 |
| ⑥ 2 row 의 `sync/async` 값 — **278** 행만 `(handler 실행 자체)` 괄호 한정 ↔ **289** 행 정의문 | (vii) ⓐ — 정의의 `sync` 가 in-process · 외부 HTTPS 를 모두 포함 | **부분참** | `§ 12.62` 축 ⑥ 의 **in-process 판별력 0** 을 인용 승계하되 본 축은 **괄호 한정 비대칭** 면이다 — 정의문에 "호출자 thread 점유 범위" 분류 항목이 없어 그 한정이 무엇을 제외하는지 규정되지 않는다 |
| ⑦ 비고 셀 `ADR-0003 §3` pointer 정합 + REQ pointer **0** (승계) | (vii) ⓑⓒ — ADR-0003 **62** 행 heading · **70** 행 본문 실재 · header **269** 행 5 컬럼 | **부분참** | pointer 대상은 전부 실재하나 278 행이 그 pointer 로 뒷받침한다고 건 `@Cron decorator` 의 정본은 ADR 이 아니라 코드이고 hit 이 **0** 이다; REQ **0** 은 컬럼 부재라 결손 아님 (`§ 12.62` 축 ⑦ 승계 · 재판정 아님) |
| ⑧ `§ 12.54` (vii) 의 `@Cron` **6 지점** 좌표 stale | (viii) — 현 hit **76 · 126 · 143 · 175 · 278 · 279** (+ 각주 자신 191 · 192) · `git log -S` → `b54c059e (T-1461)` | **거짓 (예고 반증 · stale 0 지점)** | components.md **192** 행의 6 좌표는 전부 유효하며 예고된 234 · 235 stale 은 T-1461 이 이미 정정했다 (audit `§ 12.54` 원문은 append-only 라 그대로 둔다) |

다중 표기 수치는 **row 2 : 실 결선 1 : 실 발신 node 1** (`§ 12.58` `5 : 1 : 4` · `§ 12.59` · `§ 12.61` `2 : 1 : 1` · `§ 12.60` `1 : 1 : 29` · `§ 12.62` `5 : 1 : 2` 와 같은 형식) — 278 에 대응하는 in-process 결선 **1** 개만 실재하고 그 발화 진입도 REST 이며, 279 의 실 발신 node 는 **부재** (역방향 backend_api → scheduler 만 관측) 다.

**AC 3 처리 방식 판정 (채택 1 · 기각 2).**

| 후보 | ① append-only (`§ 12.15`) | ② 좌표 drift 파급 | ③ cap (≤ 300 LOC · 3 파일) | ④ 탐색성 | 판정 |
| --- | --- | --- | --- | --- | --- |
| (A) 현행 유지 + 무편집 | 정합 | **0** | 정합 | 판정이 표 곁에 남지 않아 다음 slice 가 재측정한다 | **기각** — 축 ② · ④ · ⑧ 이 `거짓` 이라 자동 기각 조건에 걸리며, 전 축이 참이어도 탐색성만으로 재고 대상이었다 |
| (B) `## Contracts` 절 안 · T-1464 각주 직후 (305 행 뒤) blockquote ≤ 7 행 + stale 좌표 in-place 정정 ≤ 8 지점 | 정합 (순수 append) | **0 지점** | 정합 (+8 행 · 3 파일) | 판정 대상 표 바로 아래 · 직전 slice 각주와 연속 | **채택** (stale **0** 이라 in-place 정정 실행분은 **0 지점**) |
| (C) 표 셀 · 서문 · **289** 행 정의문 in-place 수정 | **위반** | — | — | — | **기각** — 축 ① 에서 먼저 기각 (`§ 12.15` append-only) |

**각주 위치 관행 승계.** `§ 12.62` 가 축 ② (파급 **0** vs **11**) · 축 ④ (표와의 거리) 실측으로 **"`## Contracts` 축 각주는 `## Contracts` 절 안 · 직전 slice 각주 뒤에 이어 붙인다"** 를 확정하고 **다음 slice 는 재고하지 않는다** 고 못 박았으므로, 본 절은 각주군 말미 (231 행 뒤) 이전 후보를 **평가하지 않았다**.

**AC 4 반영 결과.** (B) 대로 components.md **305** 행 뒤에 **빈 줄 1 행 + blockquote 7 행** 을 삽입했다 (기존 각주 블록 간 구분이 전부 빈 줄 1 행이라는 `§ 12.62` 실측 관행 승계 — 실측도 동일했다). `wc -l` **317 → 325** (+8, 상한 내), in-place 정정 **0 지점** ((viii) · (ix) 가 stale **0** 으로 확정). 편집 후 `grep -n '^## '` 재측정 → `## References` **307 → 315** 만 이동하고 나머지 **6** heading 불변이라 **자기 좌표 재-drift 는 0** — `§ 12.51` → `§ 12.60` 의 재-drift 사슬은 `§ 12.61` · `§ 12.62` 에 이어 본 절에서도 **재현되지 않았다**. 무편집 경계 준수: 표 본체 (269 ~ 287) · 서문 (267) · **289** 행 정의문 · T-1463 각주 (291 ~ 297) · T-1464 각주 (299 ~ 305) · mermaid (30 ~ 106) · 표기 bullet (108 ~ 113) · `## Component table` 표 (117 ~ 126) · 1 ~ 4 행 blockquote · 각주 16 블록 · `## References` 절 전부 **무편집** 이며, `src/scheduling/` 을 포함한 `src/` 와 `docs/decisions/` 도 무편집이다.

**AC 8 — R-110 / R-112 면제 근거.** 본 task 는 `commitMode: direct` doc-only 로 production code **0 LOC** · 분기 **0** 이라 [CLAUDE.md](../../CLAUDE.md) §3.2 direct-mode 면제 조항에 따라 `tester` 호출 · happy / error / flow / negative 4 항목 · `pnpm test:cov` 가 전부 **N/A** 다.

**불변 검산.** `wc -l` — components.md **317 → 325** (+8) · audit **5968 → 6035** (+67, 100 이내) · ADR-0003 **173 불변** · requirements.md **97 불변** · deployment.md **232 불변** · directory.md **203 불변** · modules.md **259 불변** · PLAN.md **175 불변**. `grep -c '^## '` components.md **7 불변** · audit **12 불변**, audit `grep -c '^| REQ-'` **66 불변** · `grep -c '^### 12\.'` **62 → 63**, components.md `grep -c '^> '` **108 → 115**. `git diff -U0 -- docs/architecture/components.md | grep '^@@'` → **`@@ -306,0 +307,8 @@` 1 개** 로 AC 4 허용 구간 (305 행 뒤) 밖 hunk **0**. `git diff --numstat` → components.md **8 / 0** 이라 **순수 삭제 0**. `git status --porcelain src/ web/ test/ prisma/ deploy/ docker-compose.yml Dockerfile .github/ package.json README.md .claude/ docs/decisions/ docs/ops/ docs/PLAN.md docs/requirements.md docs/architecture/modules.md` → **빈 출력** (특히 **`src/scheduling/` 무편집** — 미결선을 코드로 수리하지 않았다), `git status --porcelain` 전체 **2 파일** (components.md · REQ-COVERAGE-AUDIT.md — 상한 **3** 이내이며 task 파일 frontmatter `status` 갱신은 driver bookkeeping commit 소관이라 본 commit 에 포함되지 않는다).

**파생 영향 (목록만 — 본 slice 편집 금지).** (1) **다음 slice 1 순위 = `## Contracts` 잔여 8 row 중 `Worker` 발신 4 row (280 ~ 283 행)** — 대응 edge 4 개 (**80 ~ 83** 행) 를 `§ 12.56` 이 이미 닫아 판정 입력이 그대로 재사용되므로 한계 비용 최저 (잔여 그룹 분해: Worker **4** (280 ~ 283) · DB → PostgreSQL **1** (284) · 외부 egress **3** (285 ~ 287)) / (2) fan-out 축약 **6** 의 표 등재 여부 (`§ 12.61` FU2 미소진 — 표 편집이라 본 stream 밖) / (3) `## GitHub Adapter — 3 instance 묶음 vs 분리 결정` 본문 ↔ 코드 대조 (`§ 12.48` FU4) / (4) row pointer 셀 보강 2 건 (`§ 12.50` FU2 · `§ 12.49` FU2) / (5) LLM · GitHub adapter ADR pointer 미등재 (`§ 12.47` FU5 · `§ 12.48` FU3) / (6) `@nestjs/config` 미도입 전수 sweep (`§ 12.39` FU3, ADR 게이트) / (7) reviewer 규약 미이행 (`§ 12.41` FU2) / (8) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (`§ 12.41` FU3) / (9) README 행 번호 pointer drift 전수 sweep / (10) REQ 번호 체계 잔재 sweep (`§ 12.38` FU3) / (11) `CLAUDE.md` §1 pointer 부정확 (T-1442 FU3) / (12) UC-09 `§ 5` sequence participant 병기 (**54 회째 이월**) / (13) modules.md 카운트 claim 대조 (`§ 12.34` FU1, ADR 게이트) / (14) **행 번호 → anchor 좌표계 이행** (**48 회째 이월** — 본 절 축 ⑧ 이 같은 claim 6 지점 중 2 지점이 이미 한 차례 밀려 T-1461 의 in-place 정정을 요구했음을 실측했다) / (15) 각주 heading 참조 anchor 이행 축소 scope (`§ 12.51` FU19) / (16) `§ 12.44` 한계 "mutation 러너 26 개" 정의 미확정 / (17) **`Scheduler` cron → 평가 pipeline 미결선** (`§ 12.50` FU18 — 본 절 축 ① · ③ 이 `CRON_TICK_HANDLER` stub no-op 으로 재확인했다; 코드 소관 `pr` task) / (18) `ADR-0003` "단일 DB 인스턴스" 좌표 부재 (`§ 12.46` FU16 · `§ 12.60` FU17) / (19) `Web UI` node 의 process subgraph 소속 표기 (`§ 12.53` FU19 · `§ 12.61` FU19) / (20) **node · row 외연 정의의 문서 미박제** (`§ 12.55` FU20 · `§ 12.61` FU20 · `§ 12.62` FU20 — 본 절이 `scheduler` 외연을 **한 번 더 승계** 해 정의가 audit 절에만 살아 있는 구조가 5 회째 반복됐다) / (21) modules.md **200** 행 1:N 매핑 ↔ 디렉토리 외연 상충 (`§ 12.56` FU21) / (22) 가변 instance 수 ↔ 고정 표기 정합 (`§ 12.57` FU22 · `§ 12.58` FU22) / (23) `worker --> backend_api` 미표기 결선 (`§ 12.56` FU23) / (24) dev / prod 2 모드의 다이어그램 · 표 미분리 (`§ 12.59` FU24 · `§ 12.61` FU24) / (25) `prisma migrate deploy` 채널 미표기 + `PostgreSQL 16+` version claim 정정 (`§ 12.60` FU25) / (26) 인증 규약의 문서 단일 정본 부재 (`§ 12.61` FU26) / (27) **`sync/async` 컬럼 정의 (289 행) 의 경계 미규정** (`§ 12.61` FU27 · `§ 12.62` FU27 — 본 절 축 ⑥ 이 **괄호 한정 비대칭** 을 증상으로 더했다) / (28) `## Contracts` 표의 REQ pointer 컬럼 부재 (`§ 12.62` FU28 승계) / (29) **`@Cron` claim 중복 6 지점의 단일 정본화** — 축 ⑧ 이 좌표는 현재 stale **0** 임을 확정했으나 같은 claim (그것도 **거짓** 인 claim) 이 다이어그램 label · 표 row · 각주 2 · `## Contracts` 2 row 로 흩어진 구조 자체는 남았고, 그 정본화는 표 · 다이어그램 편집이라 본 stream 밖이다.

**한계.**

1. **측정은 grep 정적 근사다** — 호출 그래프를 실행해 확인하지 않았고 `pnpm` 계열 명령은 전부 Out of Scope 였다. DI 주입은 문자열 hit 으로 근사했으므로 token 기반 간접 주입은 과소 계수될 수 있다.
2. **`duplication 0` 의 "사실 참" 은 정적 공유 여부까지다** — manual trigger 와 cron tick 이 같은 token handler 를 공유한다는 배선만 확인했고, 두 경로의 런타임 동시 발화 · 중복 실행 방지 · timezone 처리는 열지 않았다 (`pr` task 소관 — Out of Scope).
3. **축 ① 은 `§ 12.54` 판정의 인용 승계다** — edge 76 자체를 재판정하지 않았고 재검증도 `CollectionTriggerService` 소비 파일 목록 수준이라, backfill 경로 안쪽의 호출 깊이는 재추적하지 않았다.
4. **축 ⑧ 은 components.md 안에서만 stale 을 셌다** — 같은 `@Cron` claim 이 다른 문서 (modules.md · deployment.md · ADR) 에도 있는지는 세지 않았고, audit `§ 12.54` 원문의 234 · 235 는 append-only 방침상 그대로 남는다.
5. **잔여 8 row 는 census 개수 재확인까지다** — 셀 본문 판정은 후속 slice 소관이며 285 · 287 row 의 다중 `to` 표기도 그대로 남는다.
6. **각주 위치 관행은 승계했을 뿐 재검증하지 않았다** — `§ 12.62` 의 파급 실측 (0 vs 11) 을 그대로 인용했고 본 절에서 후보 (B) 대안의 파급을 재산출하지는 않았다.

### 12.64 components.md `## Contracts` 표 **`Worker` 발신 4 row** (280 ~ 283 행) ↔ 실 `src/assessment-*` 호출 그래프 대조 — 축 8 중 거짓 2 · 부분참 5 · 참 1 확정 · worker 그룹 4/4 마감 (T-1466)

> **본 절의 위치 · 계보** — `§ 12.63` (T-1465) 이 `## Contracts` 표의 scheduler **2** row 를 닫으면서 파생 영향 **(1)** 로 **잔여 8 row 중 `Worker` 발신 4 row (280 ~ 283 행)** 를 다음 slice 1 순위로 지목했고 (근거: 대응 mermaid edge **4** 개 (**80 ~ 83** 행) 를 `§ 12.56` 이 이미 닫아 판정 입력이 그대로 재사용 — 한계 비용 최저), 본 절은 그 지목을 그대로 승계한다. 동시에 `§ 12.56` 이 자기 각주 말미 (components.md **203** 행) 에서 **"대응 `## Contracts` row 4 개 (280 ~ 283 행) 는 edge 4 개와 1:1 대응하나 그 판정은 파생 영향 (3) 소관"** 이라고 **명시 이월** 한 것을 회수한다 (`§ 12.62` 가 `§ 12.55` 의 · `§ 12.63` 이 `§ 12.54` 의 이월을 회수한 것과 동형). **`worker` node 외연 (= `src/assessment-collection/` + `src/assessment-evaluation/` 2 디렉토리) 은 본 절 신설이 아니라 `§ 12.55` · `§ 12.56` 승계 + 유효성 재검증** 이다. **edge 축 · node 축 · `## Component table` row 본문 · `## Contracts` 잔여 13 row 는 재판정하지 않는다.**

**AC 1 실측 (명령 + 출력).**

- (i) 좌표 — `grep -n '^#\{1,3\} ' docs/architecture/components.md` → `1 # Component view` · `5 ## 개요` · `22 ## Deployment 컨텍스트` · `28 ## Component diagram` · `115 ## Component table` · `233 ## GitHub Adapter …` · `265 ## Contracts` · `315 ## References` 로 task 기재값과 **일치 (stale 0)**. `grep -n '^| '` 는 `## Component table` **117 ~ 126** · `## Contracts` **269 ~ 287** 두 구간으로 갈리고, `grep -n '^| Worker |'` → **280 · 281 · 282 · 283** **4** 행. `grep -n '^> ' … | tail -10` 마지막 = **313** (T-1465 각주 끝 = 삽입점).
- (ii) census 승계 — `awk 'NR>=265' … | grep -c '^| '` → **19**, header · 구분 **2** 를 빼 **data row 17** 재확인. `§ 12.61` census 표 (user-facing 2:2 · orchestration 5:5 · scheduler 2:2 · **worker 4:4** · db 1:1 · egress 9:3) 는 **1 구 인용 승계 (재산출 없음)**.
- (iii) 대상 row 원문 — `sed -n '269,270p;280,283p' docs/architecture/components.md`. header **269** = `| from | to | sync/async | message format | 비고 |` · 구분 **270**. data row 4 개의 5 컬럼 분해는 아래 표.

| 행 | from | to | sync/async | message format | 비고 |
| --- | --- | --- | --- | --- | --- |
| 280 | Worker | GitHub Adapter | sync | TypeScript method call (in-process) | 평가 파이프라인의 commit/Issue fetch path. |
| 281 | Worker | Confluence Adapter | sync | TypeScript method call (in-process) | 평가 파이프라인의 page fetch path. |
| 282 | Worker | LLM Gateway | **sync (await)** | TypeScript method call (in-process) | 평가문 생성 / 난이도 분류 호출. provider 별 latency 가 다양하므로 concurrency limit 동반 (ADR-0003 §1). |
| 283 | Worker | DB Persistence | sync | Prisma typed query (in-process) | 평가 결과 저장. raw 저장 금지 (REQ-032) — schema 가 raw column 미정의. |

- (iv) `worker` 외연 재검증 — components.md **199** 행 각주 1 구 (`worker` = `src/assessment-collection/` 수집 + `src/assessment-evaluation/` 평가, modules.md **201** 행 1:2 매핑과 일치) 를 그대로 승계. `find … -name '*.ts' | grep -v spec | wc -l` → **97**, `grep -rln '@Controller(' …` → **2** 파일 (`assessment-collection.controller.ts` · `assessment-evaluation.controller.ts`). 외연 자체는 불변이라 **판정에 미치는 영향 0**.
- (v) row 280 · 281 — ⓐ `grep -rn 'GithubAdapterService\|GithubRequestBuilder' … | grep -v spec` → **0 hit**. 실 주입은 wrapper 로, `grep -rn 'GithubInstanceClient' …` 의 code line 이 `github-collection.service.ts` **29** (import) · **71** (constructor) · `github-org-repo-enumerate.service.ts` **20** (import) · **28** (constructor) = **4** 로 `§ 12.56` 수치 유지. ⓑ `ConfluenceSpaceTraversalService` code line = `confluence-collection.service.ts` **37** (import) · **54** (constructor) = **2** 로 유지. ⓒ 어휘 — `github-collection.service.ts` **37** 행 `export type GithubActivityEndpoint = "commits" | "pulls" | "issues"` + **45 ~ 47** 행 suffix 표 + **90** 행 `requestAllPagesForInstance`, 그리고 `github-org-repo-enumerate.service.ts` **43** 행의 org repo enumerate 호출로 실 fetch 종류는 **4** (commits · pulls · issues · repo enumerate). confluence 는 `confluence-collection.service.ts` **12** (raw `pages: unknown[]`) · **41** (`page-dedup` import) · **71** (`for (const { spaceKey, pages } of results)`) 로 page 단위가 실재하나 호출 단위는 `traverseInstance` (SPACE).
- (vi) row 282 — ⓐ `grep -rn 'LlmGateway\|llmGateway' src/assessment-evaluation … | wc -l` → **12** (`§ 12.56` 의 **8** 에서 증가 — 실측을 따르되 결선 참 판정은 불변), `LLM_GATEWAY` token 은 **18**, 실 호출은 `evaluation-scoring.service.ts` **99** 행 `await this.gateway.generate(...)`. ⓑ **`concurrency limit` 실측** — `grep -rn 'p-limit\|pLimit\|Semaphore\|concurrency\|CONCURRENCY\|maxConcurrent\|chunk\|batchSize' src --include=*.ts | grep -v spec | head -10` 의 hit 은 **전부 `src/export/` 의 chunked streaming 주석** 이고 limit primitive 는 **0**, `grep -n 'p-limit\|bottleneck\|async-sema' package.json` 은 **exit 1 (0 hit)**. 반대로 직렬 강제가 명시적이다 — `summary-batch-run.ts` **30 · 62 · 124** 행 / `summary-batch-pipeline.ts` **37** 행 / `dto/run-unevaluated-fill-batch.ts` **27 · 110** 행이 `Promise.all` 병렬을 **금지** 하고 `evaluation-orchestrator.service.ts` **163 ~ 164** 행이 for-of 순차 await 다 → **무제한 fan-out 0 · 실효 동시성 1**. ⓒ `evaluation-prompt.ts` **5 ~ 8** 행 — `classifyNarrative` 가 LLM `narrative` 를 `difficulty` + `contribution` 으로 **결정적으로 환원** 하는 순수 함수 (의존성 0 · 실 `generate` 호출 0) 이고 `evaluation-scoring.service.ts` **64** 행이 `gateway.generate` 를 **정확히 1 회** 로 규정 → 난이도는 **규칙 기반 후처리**.
- (vii) row 283 — ⓐ `grep -rn 'PrismaService' src/assessment-collection src/assessment-evaluation … | grep -v spec` 의 code line 은 `evaluation-result-persist.service.ts` **29 · 90** · `summary-persist.service.ts` **31 · 82** = **4** 이고 나머지 hit 은 전부 주석이며, `src/assessment-collection/` 은 **0** (`assessment-collection.module.ts` **34 ~ 35** 행이 "collection service 자체는 Prisma 의존 0" 을 명시) → `§ 12.56` 부분참 유지. ⓑ 축 ⑤ 는 components.md **149** 행 각주 (schema **15** model 전수 · raw 본문 컬럼 **0**) **1 구 인용 승계** 이며 `prisma/schema.prisma` **재측정 0**.
- (viii) 축 ⑥ · ⑦ · ⑧ — ⓐ **289** 행 정의문 원문: `"sync" 는 호출자 thread 가 응답까지 await 한다는 의미 (in-process 또는 외부 HTTPS 둘 다 포함)`. ⓑ `grep -n 'Decision' docs/decisions/ADR-0003-deployment.md | head -6` → **20 · 21 · 22 · 23 · 24 · 28** 이고 `sed -n '34p'` → `**결정 (1 줄)**: Backend 는 **단일 NestJS process** 로 시작한다…` 로 `ADR-0003 §1` pointer **실재**. `grep -n 'REQ-032' docs/requirements.md` → **51** 행 `| REQ-032 | 59 | 🔥 Raw data 저장 금지 …` **실재**. pointer 보유는 4 중 **2** (**282 · 283**), **280 · 281 은 0**. ⓒ `grep -c 'TypeScript method call (in-process)'` → **7**, `grep -n 'Prisma typed query (in-process)'` → **122 · 273 · 283**.
- (ix) 좌표 stale · 삽입 파급 — `grep -n '\*\*[0-9]\{2,3\}\*\* 행' … | wc -l` → **85** 지점. 본 slice 가 인용 · 의존한 좌표 (**79 ~ 83 · 108 ~ 113 · 121 · 122 · 149 · 199 ~ 203 · 265 · 269 ~ 287 · 289 · 291 ~ 313 · 315**) 는 (i) 실측과 **전량 일치 → stale 0 지점** 으로 T-1462 ~ T-1465 에 이어 **6 회 연속 0** 이다. 삽입 파급도 **0 지점** — 자기 좌표 토큰의 최대값이 **293** 이라 삽입점 **313** 뒤로 밀리는 자기 참조가 없고, **314** 이상 값 (**326 · 348 · 353 · 370 · 435 · 455 · 491 · 666**) 은 전부 외부 파일 좌표라 `§ 12.55` → `§ 12.63` 계수 규칙 (자기 좌표만 · 범위 · 나열 토큰은 1 지점) 대로 **제외** 한다 (규칙 그대로 승계).
- (x) baseline — `wc -l` components.md **325** · audit **6035** · ADR-0003 **173** · requirements.md **97** · deployment.md **232** · directory.md **203** · modules.md **259** · PLAN.md **175**, `grep -c '^## '` components.md **7** · audit **12**, audit `grep -c '^| REQ-'` **66** · `grep -c '^### 12\.'` **63**, components.md `grep -c '^> '` **115** — 전부 task 기재값과 일치.

**진척.** 본 절로 `## Contracts` 표 **17 row 중 13 판정 완료 · 잔여 4** (**284 ~ 287** 행) 이며 (`§ 12.63` 의 **9/17** 에 **+4**), **worker 그룹 4/4 마감** 이다. 그룹 진척: user-facing 2/2 · orchestration 5/5 · scheduler 2/2 · worker 4/4 · db 0/1 · egress 0/3.

**AC 2 판정표.** (판정은 (iv) 의 `worker` node 외연 = `src/assessment-collection/` + `src/assessment-evaluation/` 위에서만 유효하며, 그 외연은 **본 절 신설이 아니라 `§ 12.55` · `§ 12.56` 승계** 다.)

| 축 | 실측 근거 | 판정 | 근거 1 구 |
| --- | --- | --- | --- |
| ① row **280** 결선 + 비고 `commit/Issue fetch path` | (v) ⓐⓒ — 주입 **4** (29 · 71 · 20 · 28 행) · 실 fetch 종류 **4** (37 · 45 ~ 47 · 43 행) | **부분참** | 결선은 `§ 12.56` edge 80 참의 **인용 승계** (edge 재판정 아님) 이나 비고가 `pulls` 와 org repo enumerate 를 누락해 **4 종 중 2 종** 만 적는다 |
| ② row **281** 결선 + 비고 `page fetch path` | (v) ⓑⓒ — 주입 **2** (37 · 54 행) · page 순회 **71** 행 · dedup **41** 행 | **참** | edge 81 참 승계가 유효하고 누락 호출 종류 **0** 이다 — 다만 실 호출 단위는 SPACE `traverseInstance` 이고 page 는 그 산출 항목이다 |
| ③ row **282** 결선 + 비고 `concurrency limit 동반` · `난이도 분류 호출` | (vi) ⓐⓑⓒ — LLM hit **12** · limit primitive **0** · package.json **0** · 직렬 강제 **6** 지점 · `classifyNarrative` 순수 함수 | **거짓** | 결선 축만 승계 참이고 비고 2 claim 은 모두 거짓 — limit 은 라이브러리 · 상수 · semaphore 어느 것도 **0 hit** 이며 코드는 반대로 `Promise.all` 을 금지해 **실효 동시성 1**, 난이도는 LLM 호출이 아니라 narrative 후처리다 |
| ④ row **283** 결선 · `Prisma typed query` | (vii) ⓐ — evaluation **4** code line (29 · 90 · 31 · 82) · collection **0** (34 ~ 35 행) | **부분참** | `§ 12.56` edge 83 부분참을 그대로 승계 — `Worker` 외연 2 디렉토리 중 **절반만** DB 발신을 갖고 수집 절반은 `ContributionService` 경유 간접 경로다 |
| ⑤ row **283** 비고 `raw 저장 금지 (REQ-032)` 의 자리 | (vii) ⓑ — components.md **149** 행 각주 승계 · `## Component table` **122** 행 동일 claim | **부분참** | 내용은 참 (schema 15 model 전수 raw 컬럼 0) 이나 sync/async · message format 과 달리 **계약 표기가 아니라 schema 정책** 이고 **122** 행과 **중복 표기 2 지점** 이다 |
| ⑥ 4 row 의 `sync/async` — **282** 만 `(await)` 괄호 한정 ↔ **289** 행 정의문 | (viii) ⓐ — 정의문이 `sync` 를 "응답까지 await" 로 규정 | **부분참** | 값 자체는 in-process await 라 참이나 괄호 한정이 정의문의 **동어반복** 이라 제외하는 것이 0 이고, 나머지 3 row 가 맨 `sync` 인 **비대칭** 은 `§ 12.62` · `§ 12.63` 에 이은 **3 회째 증상** 이다 |
| ⑦ 비고 셀 pointer 분포 + 대상 실재 + REQ 컬럼 부재 (승계) | (viii) ⓑ — ADR-0003 **34** 행 · requirements.md **51** 행 실재 · 보유 **2/4** | **부분참** | pointer 대상 실재는 **2/2** 참이나 4 중 **2** 만 보유하는 불균일이고 (280 · 281 은 0), `ADR-0003 §1` 은 단일 process 를 정할 뿐 `concurrency limit` 을 뒷받침하지 않는다 — REQ pointer 컬럼 부재는 `§ 12.62` · `§ 12.63` 축 ⑦ **승계 (재판정 아님)** |
| ⑧ message format 셀 문자열 공유로 인한 판별력 | (viii) ⓒ — `TypeScript method call (in-process)` **7** row · `Prisma typed query (in-process)` **273 · 283** (+ **122** 행) | **거짓** | 같은 문자열이 **274 ~ 277 · 280 ~ 282** 7 row 에 공유돼 셀만으로 발신 · 수신 쌍이 갈리지 않는다 — `§ 12.62` 축 ⑥ 의 `sync` 판별력 0 과 **같은 병리의 다른 컬럼** 이다 |

다중 표기 수치는 **row 4 : 실 결선 4 (283 은 절반) : 실 발신 node 1** (`§ 12.58` `5 : 1 : 4` · `§ 12.59` · `§ 12.61` `2 : 1 : 1` · `§ 12.60` `1 : 1 : 29` · `§ 12.62` `5 : 1 : 2` · `§ 12.63` `2 : 1 : 1` 과 같은 형식) 이다. 특기 사항은 그 **1** node 안에서 발신 주체가 완전히 갈린다는 점이다 — collection 은 LLM hit **0**, evaluation 은 GitHub · Confluence wrapper hit **0** 으로 **교차 0** 이라 4 row 중 어느 디렉토리도 단독으로 `Worker` 발신 전량을 갖지 않는다.

**AC 3 처리 방식 판정 (채택 1 · 기각 2).**

| 후보 | ① append-only (`§ 12.15`) | ② 좌표 drift 파급 | ③ cap (≤ 300 LOC · 3 파일) | ④ 탐색성 | 판정 |
| --- | --- | --- | --- | --- | --- |
| (A) 현행 유지 + 무편집 | 정합 | **0** | 정합 | 판정이 표 곁에 남지 않아 다음 slice 가 audit 전문을 다시 열어야 한다 | **기각** — 축 ③ · ⑧ 이 **거짓** 이라 AC 3 규정상 자동 기각이고, 축 ④ 탐색성도 열위다 |
| (B) `## Contracts` 절 안 · T-1465 각주 직후 (**313** 행 뒤) blockquote **8** 행 + stale 좌표 in-place 정정 | 정합 (표 · 정의문 무편집) | **0 지점** ((ix) 실측 — 자기 좌표 최대 **293** < 삽입점) | +9 행 / 2 파일 | 판정 대상 표 (**280 ~ 283** 행) 와 **30 행** 이내 | **채택** — 4 축 전부 우위이며 정정 지점도 **0** 이라 in-place 치환이 불필요했다 |
| (C) 표 셀 · 서문 · **289** 행 정의문 in-place 수정 | **위반** | — | — | — | **기각** — 축 ① 에서 먼저 탈락 (`§ 12.15` append-only) 이며 AC 4 · Out of Scope 가 표 셀 · 정의문 편집을 명시 금지한다 |

**각주 위치 관행 승계.** `§ 12.62` 가 축 ② (파급 **0** vs **11**) · 축 ④ (표와의 거리) 실측으로 "판정 대상 표와 같은 절 안, 직전 각주 뒤" 관행을 확정하고 `§ 12.63` 이 승계한 것을 본 절도 **그대로 승계** 했다 — 각주군 말미 (**231** 행 뒤) 이전 후보는 AC 3 에서 평가하지 않았다.

**AC 4 반영 결과.** (B) 대로 components.md **313** 행 뒤에 **빈 줄 1 행 + blockquote 8 행** 을 삽입했다 (기존 각주 블록 간 구분이 빈 줄 1 행이라는 실측 관행 유지). in-place stale 정정은 **0 지점** ((ix) 가 stale 0 을 확정). 무편집 경계 — 표 본체 (**269 ~ 287**) · 서문 (**267**) · **289** 행 정의문 · T-1463 (**291 ~ 297**) · T-1464 (**299 ~ 305**) · T-1465 (**307 ~ 313**) 각주 본문 · mermaid (**30 ~ 106**) · 표기 bullet (**108 ~ 113**) · `## Component table` 표 본체 (**117 ~ 126**) · 각주 16 블록 (특히 **149**) · `## References` 절 전부 무편집. 재-drift 사슬 (`§ 12.51` **175** → `§ 12.60` **233**) 은 **본 절에서 재현되지 않았다** — 삽입으로 밀린 heading 은 `## References` (**315 → 324**) **1** 개뿐이고 그 좌표를 참조하는 각주 · 표 셀이 **0** 이라 후속 정정이 발생하지 않았다.

**AC 8 — R-110 / R-112 면제 근거.** 본 task 는 `commitMode: direct` doc-only 로 production code **0 LOC** · 분기 **0** 이라 [CLAUDE.md](../../CLAUDE.md) §3.2 direct-mode 면제 조항에 따라 `tester` 호출 · happy / error / flow / negative 4 항목 · `pnpm test:cov` 가 전부 **N/A** 다. 측정은 전부 read-only `grep` · `sed` · `awk` · `find` · `wc` · `git` 이며 `pnpm` 계열 실행 **0** 이다.

**불변 검산.** `wc -l` — components.md **325 → 334** (+9, 상한 334 충족) · audit **6035 → 6104** (+69, 100 이내) · ADR-0003 **173 불변** · requirements.md **97 불변** · deployment.md **232 불변** · directory.md **203 불변** · modules.md **259 불변** · PLAN.md **175 불변**. `grep -c '^## '` components.md **7 불변** · audit **12 불변**, audit `grep -c '^| REQ-'` **66 불변** · `grep -c '^### 12\.'` **63 → 64**, components.md `grep -c '^> '` **115 → 123**. `git diff -U0 -- docs/architecture/components.md | grep '^@@'` → `@@ -314,0 +315,9 @@` **hunk 1 개** 로 AC 4 허용 구간 (T-1465 각주 뒤 · `## References` 직전) 밖 hunk **0**. `git diff --numstat` → components.md **9 / 0** 으로 **순수 삭제 0**. `git status --porcelain src/ web/ test/ prisma/ deploy/ docker-compose.yml Dockerfile .github/ package.json README.md .claude/ docs/decisions/ docs/ops/ docs/PLAN.md docs/requirements.md docs/architecture/modules.md` → **빈 출력** (특히 **`src/assessment-collection/` · `src/assessment-evaluation/` 무편집** — 축 ③ 이 `concurrency limit` 을 거짓으로 확정했어도 limit 을 구현하지 않았다). `git status --porcelain` 전체는 **3 파일 이내**.

**파생 영향 (목록만 — 본 slice 편집 금지).** (1) **다음 slice 1 순위 = `## Contracts` 잔여 4 row = `DB Persistence → PostgreSQL` **1** row (**284** 행, 대응 edge **86** 행 `§ 12.60`) + 외부 egress **3** row (**285 ~ 287** 행, 대응 edge **89 ~ 97** 행 `§ 12.59` · `§ 12.60`)** — **한 slice 로 묶기를 권한다**: 두 그룹 모두 판정 입력 (edge 판정) 이 이미 닫혀 재사용 비용이 0 이고, 남은 row 가 **4** 개뿐이라 분할 시 slice 당 고정비 (좌표 실측 · census 재확인 · 각주 삽입) 가 판정 본문보다 커진다. 단 egress 3 row 는 fan-out 축약 **6** (edge 9 : row 3) 이 몰려 있어 축 수가 늘어나므로 축 상한 초과가 보이면 `284` 단독 + egress 3 으로 2 slice 분할한다 / (2) fan-out 축약 **6** 의 표 등재 여부 (`§ 12.61` FU2 미소진 — 표 편집이라 본 stream 밖) / (3) `## GitHub Adapter — 3 instance 묶음 vs 분리 결정` 본문 ↔ 코드 대조 (`§ 12.48` FU4) / (4) row pointer 셀 보강 2 건 (`§ 12.50` FU2 · `§ 12.49` FU2) / (5) LLM · GitHub adapter ADR pointer 미등재 (`§ 12.47` FU5 · `§ 12.48` FU3) / (6) `@nestjs/config` 미도입 전수 sweep (`§ 12.39` FU3, ADR 게이트) / (7) reviewer 규약 미이행 (`§ 12.41` FU2) / (8) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (`§ 12.41` FU3) / (9) README 행 번호 pointer drift 전수 sweep / (10) REQ 번호 체계 잔재 sweep (`§ 12.38` FU3) / (11) `CLAUDE.md` §1 pointer 부정확 (T-1442 FU3) / (12) UC-09 `§ 5` sequence participant 병기 (**55 회째 이월**) / (13) modules.md 카운트 claim 대조 (`§ 12.34` FU1, ADR 게이트) / (14) **행 번호 → anchor 좌표계 이행** (**49 회째 이월**) / (15) 각주 heading 참조 anchor 이행 축소 scope (`§ 12.51` FU19) / (16) `§ 12.44` 한계 "mutation 러너 26 개" 정의 미확정 / (17) `Scheduler` cron → 평가 pipeline 미결선 (`§ 12.50` FU18 · `§ 12.63` FU17 — 코드 소관 `pr` task) / (18) `ADR-0003` "단일 DB 인스턴스" 좌표 부재 (`§ 12.46` FU16 · `§ 12.60` FU17) / (19) `Web UI` node 의 process subgraph 소속 표기 (`§ 12.53` FU19 · `§ 12.61` FU19) / (20) node · row 외연 정의의 문서 미박제 (`§ 12.55` FU20 · `§ 12.62` FU20 · `§ 12.63` FU20 — 본 절이 `worker` 외연을 **한 번 더 승계** 했고, 축 ④ 처럼 "외연 2 디렉토리 중 절반만 참" 판정이 반복되는 만큼 외연 정본의 부재 비용이 누적된다) / (21) modules.md **200** 행 1:N 매핑 ↔ 디렉토리 외연 상충 (`§ 12.56` FU21) / (22) 가변 instance 수 ↔ 고정 표기 정합 (`§ 12.57` FU22 · `§ 12.58` FU22) / (23) `worker --> backend_api` 미표기 결선 (`§ 12.56` FU23 — 본 절 축 ④ 의 "수집 절반이 `ContributionService` 경유" 가 근거를 보탰다) / (24) dev / prod 2 모드의 다이어그램 · 표 미분리 (`§ 12.59` FU24 · `§ 12.61` FU24) / (25) `prisma migrate deploy` 채널 미표기 + `PostgreSQL 16+` version claim 정정 (`§ 12.60` FU25) / (26) 인증 규약의 문서 단일 정본 부재 (`§ 12.61` FU26) / (27) **`sync/async` 컬럼 정의 (289 행) 의 경계 미규정** (`§ 12.61` FU27 · `§ 12.62` FU27 · `§ 12.63` FU27 — 본 절 축 ⑥ 의 `(await)` 한정이 **3 회째 증상** 이며, 정의문이 이미 await 를 규정해 한정이 동어반복이 된다는 점을 더한다) / (28) `## Contracts` 표의 REQ pointer 컬럼 부재 (`§ 12.62` FU28 승계 — 본 절 축 ⑦ 의 **pointer 분포 불균일 (4 중 2 만 보유)** 을 더한다) / (29) `@Cron` claim 중복 6 지점의 단일 정본화 (`§ 12.63` FU29 승계) / (30) **`concurrency limit` claim 의 코드 미이행** — 축 ③ 이 거짓으로 확정했으나 수리 방향이 두 갈래다: (가) 표 비고를 실제 (직렬 순회) 대로 고치는 문서 수정 또는 (나) limit 을 실제로 도입하는 코드 변경. (나) 는 `p-limit` 류 **새 dependency** 또는 자체 semaphore **설계 결정** 이라 [CLAUDE.md](../../CLAUDE.md) §5 ADR 게이트 · `pr` task 소관이며 본 doc stream 밖이다 / (31) **message format 컬럼의 판별력 부재** — 축 ⑧ 이 7 row 공유를 확정했으나 컬럼 값 재설계는 표 편집이라 본 stream 밖이다.

**한계.**

1. **측정은 grep 정적 근사다** — 호출 그래프를 실행해 확인하지 않았고 `pnpm` 계열 실행 **0** 이라 런타임 동시성 · 실 endpoint 호출 여부는 검증 범위 밖이다. 특히 축 ③ 의 "실효 동시성 1" 은 코드가 병렬을 **금지한다는 주석 · 구조** 근거이지 실행 관측이 아니다.
2. **축 ① ~ ④ 의 결선 축은 `§ 12.56` 판정의 인용 승계다** — edge 80 ~ 83 자체를 재판정하지 않았고, 승계가 유효한지만 hit 수 재측정으로 확인했다. 축 ③ 의 hit **12** (구 **8**) 처럼 수치가 움직여도 참 / 부분참 결론은 불변이었다.
3. **축 ⑤ 는 schema 를 재측정하지 않았다** — components.md **149** 행 각주의 15 model 전수 판정을 그대로 인용했고 본 절이 가른 것은 그 claim 이 계약 셀에 놓일 자리인지뿐이다.
4. **축 ⑨ 의 stale 계수는 본 slice 가 인용한 좌표 집합 안에서만 0 이다** — **85** 지점 전량을 1:1 대조하지는 않았고, 미인용 구간의 stale 여부는 후속 slice 소관이다.
5. **잔여 4 row 는 census 개수 재확인까지다** — **284 ~ 287** 행의 셀 본문 판정은 후속 slice 소관이며 **285 ~ 287** 의 다중 `to` 표기와 fan-out 축약 **6** 도 그대로 남는다.
6. **`GitHub Adapter` · `Confluence Adapter` 수신 node 의 외연은 재정의하지 않았다** — 주입 대상이 wrapper (`GithubInstanceClient` · `ConfluenceSpaceTraversalService`) 라는 `§ 12.56` 관측을 승계했을 뿐, 그 wrapper 가 adapter node 에 속하는지 자체는 `§ 12.53` node 축 소관이라 열지 않았다.

### 12.65 components.md `## Contracts` 표 **`DB Persistence → PostgreSQL` 1 row** (284 행) ↔ 실 `src/persistence` wire 표면 대조 — 축 9 중 거짓 1 · 부분참 6 · 참 2 확정 · db 그룹 1/1 마감 (T-1467)

> **본 절의 위치 · 계보** — `§ 12.64` (T-1466) 가 `## Contracts` 표의 worker **4** row 를 닫으면서 파생 영향 **(1)** 로 **잔여 4 row (`DB Persistence → PostgreSQL` 1 + 외부 egress 3)** 를 다음 slice 1 순위로 지목했고, 본 절은 그 지목을 승계하되 같은 FU (1) 이 단 단서 — "egress 3 row 는 fan-out 축약 **6** (edge 9 : row 3) 이 몰려 있어 축 수가 늘어나므로 축 상한 초과가 보이면 `284` 단독 + egress 3 으로 2 slice 분할한다" — 를 planner 가 **분할 쪽으로 판단** 해 **284 행 1 row 만** 다룬다 (분할 판단의 사후 실증은 AC 3 참조). 동시에 `§ 12.60` 이 자기 각주 말미 (components.md **231** 행) 에서 **"대응 `## Contracts` row 는 284 행 1 개로 edge 1 과 1:1 이고 … 판정은 파생 영향 (1) 소관"** 이라고 **명시 이월** 한 것을 회수한다 (`§ 12.62` 가 `§ 12.55` 의 · `§ 12.63` 이 `§ 12.54` 의 · `§ 12.64` 가 `§ 12.56` 의 이월을 회수한 것과 동형이며 **4 회 연속**). **DB connection seam 정의 (= 실 wire 를 여는 곳이 `prisma.service.ts` 단일 파일로 수렴) 는 본 절 신설이 아니라 `§ 12.60` 승계 + 유효성 재검증** 이다. **edge 축 · node 축 · `## Component table` row 본문 · `## Contracts` 잔여 3 row 는 재판정하지 않는다.**

**AC 1 실측 (명령 + 출력).**

- (i) 좌표 — `grep -n '^#\{1,3\} ' docs/architecture/components.md` → `1 # Component view` · `5 ## 개요` · `22 ## Deployment 컨텍스트` · `28 ## Component diagram` · `115 ## Component table` · `233 ## GitHub Adapter …` · `265 ## Contracts` · `324 ## References` 로 task 기재값과 **일치 (stale 0)**. `grep -n '^| '` 는 `## Component table` **117 ~ 126** · `## Contracts` **269 ~ 287** 두 구간으로 갈리고, `grep -n '^| DB Persistence |'` → **284** **1** 행 단독. `grep -n '^> ' … | tail -6` 마지막 = **322** (T-1466 각주 끝 = 삽입점) 이고 **323** 은 빈 줄이다.
- (ii) census 승계 — `awk 'NR>=265' … | grep -c '^| '` → **19**, header · 구분 **2** 를 빼 **data row 17** 재확인. `§ 12.61` census 표 (user-facing 2:2 · orchestration 5:5 · scheduler 2:2 · worker 4:4 · **db 1:1** · egress 9:3) 는 **1 구 인용 승계 (재산출 없음)**. 본 절 판정 대상은 **db 1 row** 다.
- (iii) 대상 row 원문 — `sed -n '269,270p;284p' docs/architecture/components.md`. header **269** = `| from | to | sync/async | message format | 비고 |` · 구분 **270**. data row 1 개의 5 컬럼 분해는 아래 표.

| 행 | from | to | sync/async | message format | 비고 |
| --- | --- | --- | --- | --- | --- |
| 284 | DB Persistence | PostgreSQL | sync | Prisma client (TCP 5432, libpq protocol) | ADR-0002. connection pool singleton. |

- (iv) DB connection seam 재검증 — components.md **226** 행 각주 1 구 (`new PrismaClient` **0 hit** 이고 `PrismaService` 를 쓰는 **41** 파일 중 실 wire 를 여는 곳은 `prisma.service.ts` **단일 파일** 로 수렴) 를 그대로 승계. `grep -rn 'new PrismaClient' src --include=*.ts | grep -v spec | wc -l` → **0**, `grep -rln 'PrismaService' src --include=*.ts | grep -v spec | wc -l` → **41** 로 **증감 0** → 정의는 불변이고 **판정에 미치는 영향 0** 이다 (**재정의가 아니라 재검증**).
- (v) 축 ① · ② · ⑤ — ⓐ `sed -n '15,45p' src/persistence/prisma.service.ts` → **18** 행 `import { PrismaPg } from "@prisma/adapter-pg";` · **24 ~ 25** 행 `export function buildPrismaAdapter(): PrismaPg { return new PrismaPg({ connectionString: process.env.DATABASE_URL ?? "" });` · **34** 행 `super({ adapter: buildPrismaAdapter() });` · **40 ~ 41** 행 `async onModuleInit() { await this.$connect();` 으로 `§ 12.60` 의 **25 · 34 · 41** 좌표가 **전량 일치**. ⓑ `grep -n '@Global()\|providers\|exports' src/persistence/persistence.module.ts` → **12** `@Global()` · **14** `providers: [PrismaService]` · **15** `exports: [PrismaService]` 로 singleton 근거 유지. ⓒ `grep -rn 'connection_limit\|pool_timeout\|max:\|PoolConfig' src prisma --include=*.ts --include=*.prisma | grep -v spec` → hit **1** 인데 그 1 은 `since-derivation.service.ts` **40** 행 `assessments.reduce((max: Assessment, current: Assessment) =>` 라 pool 과 **무관한 false-positive** 이고, `connection_limit` · `pool_timeout` · `PoolConfig` 자체는 **0 hit** 이다 → **pool 실재 · 구성은 기본값**.
- (vi) 축 ④ (본 절 신설 축) — `grep -n '"pg"\|pg-native\|libpq\|@prisma/adapter-pg\|@prisma/client' package.json` → **33** `"@prisma/adapter-pg": "^7.8.0"` · **34** `"@prisma/client": "^7.8.0"` · **41** `"pg": "^8.21.0"` 이고 `pg-native` · `libpq` 는 **0 hit**. `grep -rn 'pg-native\|libpq' src prisma package.json | wc -l` → **0**. 즉 **libpq 바인딩이 저장소 어디에도 없다** — 실 경로는 `@prisma/adapter-pg` → `pg` (node-postgres, wire protocol 을 순수 JS 로 구현) 다. 정확한 대체 표기 후보는 `PostgreSQL frontend/backend wire protocol` 이며 **제안까지만** 이고 표는 고치지 않았다.
- (vii) 축 ③ · ⑨ — ⓐ `grep -rn '5432' src prisma --include=*.ts --include=*.prisma | grep -v spec | wc -l` → **0**, `grep -n '5432' docker-compose.yml` → **22** 행 `- "5432:5432"` → `TCP 5432` 는 **배포 · 로컬 환경 층의 사실** 이라는 `§ 12.60` 판정을 그대로 승계한다. ⓑ `grep -rn 'prisma migrate deploy' .github/workflows/ci.yml` → **172 · 306 · 342 · 364** 는 주석이고 실 발화는 **216** 행 `run: pnpm prisma migrate deploy` **1** 지점인데, 표 **17** row 어디에도 migration 채널 row 는 **0** 이다 (`§ 12.60` FU25 의 **row 축 재확인** 이며 edge 재판정 아님).
- (viii) 축 ⑥ · ⑦ · ⑧ — ⓐ `grep -n 'ADR-0002' docs/architecture/components.md` → **12** 지점 (**16 · 26 · 62 · 112 · 122 · 147 · 149 · 150 · 273 · 284 · 305 · 327**) 이고 그중 `## Contracts` 표 안은 **273 · 284** **2** 지점. `grep -n '^## Decision' docs/decisions/ADR-0002-db.md` → **41**, `sed -n '43p'` → `**Persistence DB 는 PostgreSQL, ORM 은 Prisma** 를 채택한다.` 이고 frontmatter **4** 행 `status: ACCEPTED` → pointer 대상 **실재**. 단 `connection pool` 어구는 **33** 행 Context 의 REQ-048 근거 문장에만 있고 결정문에는 **0** 이다. ⓑ **289** 행 정의문 원문: `"sync" 는 호출자 thread 가 응답까지 await 한다는 의미 (in-process 또는 외부 HTTPS 둘 다 포함). "async" 는 외부 HTTPS 경계를 넘는 호출이라 …`. ⓒ `grep -c 'Prisma typed query (in-process)'` → **4** (**122 · 273 · 283 · 322** — 마지막은 T-1466 각주 인용), `grep -c 'Prisma client (TCP 5432'` → **2** (**231** 각주 · **284** 행) 로 표 안에서는 **1** row 단독이다.
- (ix) 좌표 stale · 삽입 파급 — `grep -n '\*\*[0-9]\{2,3\}\*\* 행' … | wc -l` → **91** 지점 (`§ 12.64` 의 **85** 에서 T-1466 각주 추가분만큼 증가). 본 slice 가 인용 · 의존한 좌표 (**86 · 108 ~ 113 · 122 · 148 · 226 ~ 231 · 265 · 269 ~ 287 · 289 · 291 ~ 322 · 324**) 는 (i) 실측과 **전량 일치 → stale 0 지점** 으로 T-1462 ~ T-1466 에 이어 **7 회 연속 0** 이다. 삽입 파급도 **0 지점** — 자기 좌표 토큰의 최대값이 **293** 이라 삽입점 **322** 뒤로 밀리는 자기 참조가 없고, **323** 이상 값 (**326 · 348 · 353 · 370 · 435 · 455 · 491 · 666**) 은 전부 외부 파일 좌표라 `§ 12.55` → `§ 12.64` 계수 규칙 (자기 좌표만 · 범위 · 나열 토큰은 1 지점) 대로 **제외** 한다 (규칙 그대로 승계).
- (x) baseline — `wc -l` components.md **334** · audit **6104** · ADR-0002 **127** · requirements.md **97** · deployment.md **232** · directory.md **203** · modules.md **259** · PLAN.md **175**, `grep -c '^## '` components.md **7** · audit **12**, audit `grep -c '^| REQ-'` **66** · `grep -c '^### 12\.'` **64**, components.md `grep -c '^> '` **123** — 전부 task 기재값과 일치.

**진척.** 본 절로 `## Contracts` 표 **17 row 중 14 판정 완료 · 잔여 3** (**285 ~ 287** 행 egress) 이며 (`§ 12.64` 의 **13/17** 에 **+1**), **db 그룹 1/1 마감** 이다. 그룹 진척: user-facing 2/2 · orchestration 5/5 · scheduler 2/2 · worker 4/4 · **db 1/1** · egress 0/3.

**AC 2 판정표.** (판정은 (iv) 의 **DB connection seam** = 실 wire 를 여는 곳이 `prisma.service.ts` 단일 파일로 수렴한다는 정의 위에서만 유효하며, 그 정의는 **본 절 신설이 아니라 `§ 12.60` 승계** 다.)

| 축 | 실측 근거 | 판정 | 근거 1 구 |
| --- | --- | --- | --- |
| ① row **284** 결선 | (v) ⓐ — `prisma.service.ts` **25 · 34 · 41** 행 | **참** | process 경계를 넘는 유일한 non-HTTP wire 라 `§ 12.60` edge 86 참 판정의 **인용 승계** 가 유효하다 (edge 재판정 아님) |
| ② message format 의 `Prisma client` | (v) ⓐ + (vi) — **18** 행 adapter import · package.json **33 · 41** 행 | **부분참** | 상위 API 이름은 맞으나 실 socket 주체는 `PrismaClient` 가 아니라 `@prisma/adapter-pg` 의 `PrismaPg` → `pg` Pool 이다 |
| ③ `TCP 5432` 의 층위 | (vii) ⓐ — `src` · `prisma` **0 hit** · docker-compose.yml **22** 행 | **부분참** | 포트는 실재하되 근거가 배포 자산뿐이라 **앱 계약이 아니라 배포 · 로컬 환경 층의 사실** 이다 (`§ 12.60` 배포 층 판정 승계) |
| ④ `libpq protocol` claim | (vi) — `pg-native` · `libpq` `src` · `prisma` · package.json 전수 **0 hit** | **거짓** | libpq 바인딩이 실재하지 않고 실 경로는 wire protocol 을 순수 JS 로 구현한 `pg` 라, 정확한 표기는 `PostgreSQL frontend/backend wire protocol` 이다 |
| ⑤ `connection pool singleton` claim | (v) ⓑⓒ — `@Global()` **12 · 14 ~ 15** 행 · pool 옵션 **0 hit** | **부분참** | singleton **1** 은 참이나 `connection_limit` · `pool_timeout` · `PoolConfig` 정본이 **0** 이라 **pool 은 실재하되 구성은 driver 기본값** 이다 |
| ⑥ `ADR-0002` pointer 실재 · 정합 | (viii) ⓐ — ADR-0002 **4 · 41 · 43** 행 실재 · `connection pool` 은 **33** 행 Context 뿐 | **부분참** | 대상 실재는 참이나 row 의 2 claim 중 **1** (DB · ORM 선택) 만 지지하고 `§` 세분이 **0** 이라 pointer 정밀도가 낮다 (`§ 12.62` pointer 축 인용 승계) |
| ⑦ `sync` 값 ↔ **289** 행 정의문 | (viii) ⓑ — 정의문이 `async` 를 "외부 HTTPS 경계" 로 · `sync` 를 "in-process 또는 외부 HTTPS" 로 열거 | **부분참** | 값 자체는 `$connect` · query await 라 참이나 **284** 는 HTTPS 가 아닌 외부 process 경계라 어느 열거 항에도 들지 않아 정의문이 이 row 를 분류할 근거를 갖지 못한다 — `§ 12.61` ~ `§ 12.64` 에 이은 **4 회째 증상이자 성격이 다른 첫 사례** (앞 3 회는 in-process 판별력 · 괄호 한정) |
| ⑧ message format 셀 판별력 — `§ 12.64` 축 ⑧ 의 반례 여부 | (viii) ⓒ — `Prisma client (TCP 5432` **표 안 1** row · `Prisma typed query (in-process)` **273 · 283** (+ **122**) | **부분참** | 같은 컬럼이 in-process ↔ wire **층위는 가른다** 는 점에서 판별력 0 의 **제한적 반례** 이나, 판별력이 생긴 이유는 wire row 가 표에서 유일해서이지 컬럼 설계가 발신 · 수신 쌍을 식별해서가 아니다 |
| ⑨ migration 채널 미표기 | (vii) ⓑ — `.github/workflows/ci.yml` **216** 행 실 발화 · 표 row **0** | **참** | 앱 process 밖 두 번째 결선이 다이어그램 (`§ 12.60`) 에 이어 **표에도 미표기** 임이 확인됐다 (row 축 재확인이며 edge 재판정 아님) |

다중 표기 수치는 **row 1 : 실 결선 1 : 실 소비 지점 29** (`§ 12.60` `1 : 1 : 29` · `§ 12.61` · `§ 12.63` `2 : 1 : 1` · `§ 12.62` `5 : 1 : 2` · `§ 12.64` `4 : 4 : 1` 과 같은 형식) 로 **edge 축과 완전히 동일** 하다 — 표 17 row 중 row : 결선이 **1:1 로 정확히 맞아떨어지는 유일한 row** 이며, 은닉은 결선 개수가 아니라 **소비 지점 29 의 축약** 과 (vii) ⓑ 의 migration 채널 누락 쪽에 있다.

**AC 3 처리 방식 판정 (채택 1 · 기각 2).**

| 후보 | ① append-only (`§ 12.15`) | ② 좌표 drift 파급 | ③ cap (≤ 300 LOC · 3 파일) | ④ 탐색성 | 판정 |
| --- | --- | --- | --- | --- | --- |
| (A) 현행 유지 + 무편집 | 정합 | **0** | 정합 | 판정이 표 곁에 남지 않아 다음 slice 가 audit 전문을 다시 열어야 한다 | **기각** — 축 ④ 가 **거짓** 이라 AC 3 규정상 자동 기각이고, 축 ④ (탐색성) 도 열위다 |
| (B) `## Contracts` 절 안 · T-1466 각주 직후 (**322** 행 뒤) blockquote **7** 행 + stale 좌표 in-place 정정 | 정합 (표 · 정의문 무편집) | **0 지점** ((ix) 실측 — 자기 좌표 최대 **293** < 삽입점 **322**) | +8 행 / 3 파일 | 판정 대상 row (**284** 행) 와 **40** 행 이내 | **채택** — 4 축 전부 우위이며 정정 지점도 **0** 이라 in-place 치환이 불필요했다 |
| (C) 표 셀 · 서문 · **289** 행 정의문 in-place 수정 | **위반** | — | — | — | **기각** — 축 ① 에서 먼저 탈락 (`§ 12.15` append-only) 이며 AC 4 · Out of Scope 가 표 셀 · 정의문 편집을 명시 금지한다 |

**각주 위치 관행 승계.** `§ 12.62` 가 축 ② (파급 **0** vs **11**) · 축 ④ (표와의 거리) 실측으로 "판정 대상 표와 같은 절 안, 직전 각주 뒤" 관행을 확정하고 `§ 12.63` · `§ 12.64` 가 승계한 것을 본 절도 **그대로 승계** 했다 — 각주군 말미 (**231** 행 뒤) 이전 후보는 AC 3 에서 평가하지 않았다.

**분할 판단의 사후 실증.** `sed -n '285,287p'` 로 egress 3 row 를 인용해 row 당 판정 필요 claim 을 계수하면 — **285** (GitHub Adapter → `github.com / sec / ecode`): 결선 · protocol 2 종 (`HTTPS REST v3 / GraphQL v4`) · auth (`PAT`, `Authorization: token ...`) · pointer (`ADR-0003 §4`) · 4xx 이벤트 (`PermissionDeniedEvent`, REQ-008) · `async (외부 HTTPS)` 시제 = **6**, **286** (Confluence): 동형 **6** (REQ-016), **287** (LLM Gateway): 위 6 에 `provider 5 종` 수 claim + `P4 LLM gateway task` 미래 시제 defer 가 더해져 **7** → 합 **19** claim 이고, 여기에 fan-out 축약 **6** (edge 9 : row 3) 판정 축 **1** 이 별도로 붙는다. claim 종류별로 묶어 축을 최대한 압축해도 egress 만으로 **7 ~ 8 축** 이며 본 절의 db **9 축** 과 합치면 **16 ~ 17 축** 이라, `§ 12.61` 이후 전 slice 가 지킨 **축 ~9 · 절 ≤ 100 행** 규약을 넘긴다 → **planner 의 284 단독 분할 판단은 옳았다**. 다만 **정직하게 적을 한계** 도 있다 — 본 절이 실제로 소비한 고정비 (좌표 실측 · census 재확인 · 각주 삽입) 는 판정 본문과 비슷한 규모라, `§ 12.64` FU (1) 이 통합을 권한 근거 ("slice 당 고정비가 판정 본문보다 커진다") 자체는 **여전히 유효** 하다. 즉 분할은 축 상한 · 행 상한 때문에 옳았을 뿐 고정비 측면에서는 손해였다.

**AC 4 반영 결과.** (B) 대로 components.md **322** 행 뒤에 **빈 줄 1 행 + blockquote 7 행** 을 삽입했다 (기존 각주 블록 간 구분이 빈 줄 1 행이라는 `§ 12.62` ~ `§ 12.64` 실측 관행 유지). in-place stale 정정은 **0 지점** ((ix) 가 stale 0 을 확정). 무편집 경계 — 표 본체 (**269 ~ 287**) · 서문 (**267**) · **289** 행 정의문 · T-1463 (**291 ~ 297**) · T-1464 (**299 ~ 305**) · T-1465 (**307 ~ 313**) · T-1466 (**315 ~ 322**) 각주 본문 · mermaid (**30 ~ 106**) · 표기 bullet (**108 ~ 113**) · `## Component table` 표 본체 (**117 ~ 126**) · 각주 16 블록 (특히 **148 · 226 ~ 231**) · `## References` 절 전부 무편집. 재-drift 사슬 (`§ 12.51` **175** → `§ 12.60` **233**) 은 **본 절에서도 재현되지 않았다** — 삽입으로 밀린 heading 은 `## References` (**324 → 332**) **1** 개뿐이고 그 좌표를 참조하는 각주 · 표 셀이 **0** 이라 후속 정정이 발생하지 않았다.

**AC 8 — R-110 / R-112 면제 근거.** 본 task 는 `commitMode: direct` doc-only 로 production code **0 LOC** · 분기 **0** 이라 [CLAUDE.md](../../CLAUDE.md) §3.2 direct-mode 면제 조항에 따라 `tester` 호출 · happy / error / flow / negative 4 항목 · `pnpm test:cov` 가 전부 **N/A** 다. 측정은 전부 read-only `grep` · `sed` · `awk` · `wc` · `git` 이며 `pnpm` · `prisma` CLI 실행 **0** 이다.

**불변 검산.** `wc -l` — components.md **334 → 342** (+8, 상한 343 충족) · audit **6104 → 6173** (+69, 100 이내) · **ADR-0002 127 불변** · requirements.md **97 불변** · deployment.md **232 불변** · directory.md **203 불변** · modules.md **259 불변** · PLAN.md **175 불변**. `grep -c '^## '` components.md **7 불변** · audit **12 불변**, audit `grep -c '^| REQ-'` **66 불변** · `grep -c '^### 12\.'` **64 → 65**, components.md `grep -c '^> '` **123 → 130**. `git diff -U0 -- docs/architecture/components.md | grep '^@@'` → `@@ -323,0 +324,8 @@` **hunk 1 개** 로 AC 4 허용 구간 (T-1466 각주 뒤 · `## References` 직전) 밖 hunk **0**. `git diff --numstat` → components.md **8 / 0** 으로 **순수 삭제 0**. `git status --porcelain src/ web/ test/ prisma/ deploy/ docker-compose.yml Dockerfile .github/ package.json README.md .claude/ docs/decisions/ docs/ops/ docs/PLAN.md docs/requirements.md docs/architecture/modules.md` → **빈 출력** — 특히 **`src/persistence/` · `package.json` 무편집** 이다 (축 ④ 가 `libpq protocol` 을 거짓으로 확정했어도 `pg-native` · `libpq` 를 도입하지 않았고, 축 ⑤ 가 pool 구성 부재를 확인했어도 `connection_limit` 를 추가하지 않았다). `git status --porcelain` 전체는 **3 파일 이내**.

**파생 영향 (목록만 — 본 slice 편집 금지).** (1) **다음 slice 1 순위 = `## Contracts` 잔여 3 row = 외부 egress (`GitHub Adapter` **285** · `Confluence Adapter` **286** · `LLM Gateway` **287** 행, 대응 edge **89 ~ 97** 행 `§ 12.59` · `§ 12.60`)** — 본 절 AC 3 이 계수한 claim 수 (**6 + 6 + 7 = 19** + fan-out 축약 판정 축 1) 를 근거로 **3 row 1 slice 통합을 권한다**: claim 종류가 3 row 에 걸쳐 동형 (결선 · protocol · auth · pointer · 4xx 이벤트 · 시제) 이라 축을 **claim 종류별로 묶으면 7 ~ 8 축** 이 되어 축 상한 안에 들고, 2+1 재분할은 같은 축을 두 번 세우는 중복 비용이 크다. 단 fan-out 축약 **6** (edge 9 : row 3) 이 이 그룹에 전량 몰려 있어 축 하나가 3 row 를 동시에 다루므로, 절이 **100 행** 을 넘길 조짐이 보이면 실측 인용을 요약형으로 압축한다. 본 slice 종료로 `## Contracts` 표 축은 **egress 그룹만 남는다** / (2) fan-out 축약 **6** 의 표 등재 여부 (`§ 12.61` FU2 미소진 — 표 편집이라 본 stream 밖) / (3) `## GitHub Adapter — 3 instance 묶음 vs 분리 결정` 본문 ↔ 코드 대조 (`§ 12.48` FU4) / (4) row pointer 셀 보강 2 건 (`§ 12.50` FU2 · `§ 12.49` FU2) / (5) LLM · GitHub adapter ADR pointer 미등재 (`§ 12.47` FU5 · `§ 12.48` FU3) / (6) `@nestjs/config` 미도입 전수 sweep (`§ 12.39` FU3, ADR 게이트) / (7) reviewer 규약 미이행 (`§ 12.41` FU2) / (8) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (`§ 12.41` FU3) / (9) README 행 번호 pointer drift 전수 sweep / (10) REQ 번호 체계 잔재 sweep (`§ 12.38` FU3) / (11) `CLAUDE.md` §1 pointer 부정확 (T-1442 FU3) / (12) UC-09 `§ 5` sequence participant 병기 (**56 회째 이월**) / (13) modules.md 카운트 claim 대조 (`§ 12.34` FU1, ADR 게이트) / (14) **행 번호 → anchor 좌표계 이행** (**50 회째 이월**) / (15) 각주 heading 참조 anchor 이행 축소 scope (`§ 12.51` FU19) / (16) `§ 12.44` 한계 "mutation 러너 26 개" 정의 미확정 / (17) `Scheduler` cron → 평가 pipeline 미결선 (`§ 12.50` FU18 · `§ 12.64` FU17 — 코드 소관 `pr` task) / (18) `ADR-0003` "단일 DB 인스턴스" 좌표 부재 (`§ 12.46` FU16 · `§ 12.60` FU17) / (19) `Web UI` node 의 process subgraph 소속 표기 (`§ 12.53` FU19 · `§ 12.61` FU19) / (20) node · row 외연 정의의 문서 미박제 (`§ 12.55` FU20 · `§ 12.64` FU20 — 본 절이 **DB connection seam** 을 한 번 더 승계했고, 승계만으로 판정이 성립한다는 사실 자체가 정본 부재 비용을 계속 이연시킨다) / (21) modules.md **200** 행 1:N 매핑 ↔ 디렉토리 외연 상충 (`§ 12.56` FU21) / (22) 가변 instance 수 ↔ 고정 표기 정합 (`§ 12.57` FU22 · `§ 12.58` FU22) / (23) `worker --> backend_api` 미표기 결선 (`§ 12.56` FU23 · `§ 12.64` FU23) / (24) dev / prod 2 모드의 다이어그램 · 표 미분리 (`§ 12.59` FU24 · `§ 12.61` FU24) / (25) **`prisma migrate deploy` 채널 미표기 + `PostgreSQL 16+` version claim 정정** (`§ 12.60` FU25 — 본 절 축 ⑨ 가 **row 축에서도 미표기** 임을 확인해 근거를 보탰다) / (26) 인증 규약의 문서 단일 정본 부재 (`§ 12.61` FU26) / (27) **`sync/async` 컬럼 정의 (289 행) 의 경계 미규정** (`§ 12.61` ~ `§ 12.64` FU27 — 본 절 축 ⑦ 의 **비-HTTPS 외부 경계 분류 불가** 가 **4 회째 증상이자 성격이 다른 첫 사례** 다) / (28) `## Contracts` 표의 REQ pointer 컬럼 부재 (`§ 12.62` FU28 · `§ 12.64` FU28 승계) / (29) `@Cron` claim 중복 6 지점의 단일 정본화 (`§ 12.63` FU29 승계) / (30) **`concurrency limit` claim (282 행) 의 코드 미이행** — `§ 12.64` 축 ③ 이 거짓으로 확정한 잔여 이슈이며, 후속 slice 로 다룰 경우 **doc 축 안에서만** — (가) 각주 병기까지만 허용하고 표 비고 셀 in-place 수정은 `§ 12.15` append-only 관행상 별도 판단이며, (나) `p-limit` 류 도입 · 자체 semaphore 구현은 새 dependency · 설계 결정이라 [CLAUDE.md](../../CLAUDE.md) §5 BLOCKED 게이트 · `pr` task 소관으로 본 doc stream 이 절대 수행하지 않는다 / (31) message format 컬럼의 판별력 (`§ 12.64` FU31 — 본 절 축 ⑧ 이 **제한적 반례가 존재한다** 는 관측을 더한다: 컬럼은 쌍을 식별하지 못하나 in-process ↔ wire 층위는 가른다) / (32) **`libpq protocol` 표기 정정** — 축 ④ 가 **거짓** 으로 확정했으므로 정정이 필요하나 표 셀 수정이라 본 stream 밖이며 (2) 와 같은 **표 편집 batch** 소관이다. 대체 표기 후보 (`PostgreSQL frontend/backend wire protocol`) 는 본 절이 제안까지만 남긴다.

**한계.**

1. **측정은 grep 정적 근사다** — 실제 DB 연결을 열어 protocol 을 관측하지 않았고 `pnpm` · `prisma` CLI 실행 **0** 이라, 축 ④ 의 "순수 JS wire 구현" 은 `pg-native` · `libpq` 부재라는 **패키지 사실** 에서 도출한 결론이지 런타임 관측이 아니다.
2. **축 ① · ③ · ⑤ 의 근거는 `§ 12.60` 판정의 인용 승계다** — edge 86 자체를 재판정하지 않았고 (iv) 로 승계 유효성만 재검증했다 (`new PrismaClient` **0** · `PrismaService` **41** 파일 증감 **0**).
3. **pool 크기 · TLS (`sslmode`) · migration 전략의 적정성은 판정하지 않았다** — 본 절이 가른 것은 **표기가 실 구현과 어긋나는지** 뿐이며 DB 운영 설계는 ADR 게이트 · `pr` task 소관이다.
4. **`DATABASE_URL` 의 값은 확인하지 않았고 문서에도 옮기지 않았다** — 환경변수 **이름** 과 옵션명까지만 기록했다 ([CLAUDE.md](../../CLAUDE.md) §9).
5. **(ix) 의 stale 계수는 본 slice 가 인용한 좌표 집합 안에서만 0 이다** — **91** 지점 전량을 1:1 대조하지는 않았고 미인용 구간의 stale 여부는 후속 slice 소관이다.
6. **잔여 3 row 는 census 개수 재확인 + AC 3 의 claim 계수까지다** — **285 ~ 287** 행의 셀 본문 판정과 다중 `to` 표기 · fan-out 축약 **6** 은 그대로 후속 slice 에 남는다.

### 12.66 components.md `## Contracts` 표 **외부 egress 3 row** (285 ~ 287 행) ↔ 실 `src` outbound · auth 헤더 · 4xx 처리 대조 — 축 8 중 거짓 1 · 부분참 4 · 참 3 확정 · egress 그룹 3/3 마감 = **`## Contracts` 표 축 17/17 종료** (T-1468)

> **본 절의 위치 · 계보** — `§ 12.65` (T-1467) 가 db **1** row 를 닫으며 파생 영향 **(1)** 로 **잔여 3 row = 외부 egress** 를 다음 slice 1 순위로 지목하고 동시에 **"3 row 1 slice 통합을 권한다"** 는 판단 근거 (claim 종류가 3 row 에 걸쳐 동형이라 축을 종류별로 묶으면 **7 ~ 8 축**) 까지 남겼으며, 본 절은 그 권고를 **그대로 채택** 했다 (T-1467 은 반대로 분할을 채택했다 — 두 결정 모두 축 수 실측에 따른 것이지 관행이 아니다). 본 절은 동시에 `§ 12.57` (adapter edge 4) · `§ 12.58` (llm edge 5) 이 닫은 egress edge **9** 의 **row 축 회수** 이며, protocol · auth 축은 **신설이 아니라 승계 + 유효성 재검증** 이다 (`§ 12.62` ~ `§ 12.65` 에 이어 **5 회 연속** 이월 회수). 편집 방침은 `§ 12.15` append-only 를 그대로 따른다.

**AC 1 실측 (요약형).** `§ 12.65` 파생 영향 **(1)** 의 압축 단서 ("절이 **100 행** 을 넘길 조짐이 보이면 실측 인용을 요약형으로 압축한다") 를 적용해 **명령 + 수치** 만 적고 출력 전문은 생략한다.

- (i) **좌표** — `grep -n '^#\{1,3\} ' docs/architecture/components.md` → `1` · `5` · `22` · `28` · `115` · `233` · **`265 ## Contracts`** · **`332 ## References`** 로 task 기재값과 **일치**. `grep -n '^| '` → `## Component table` **117 ~ 126** (**10** 행) · `## Contracts` **269 ~ 287** (**19** 행). 판정 대상 data row 는 `| GitHub Adapter |` **285** · `| Confluence Adapter |` **286** · `| LLM Gateway |` **287**. `grep -n '^> ' … | tail -8` → T-1467 각주 블록 **324 ~ 330** (삽입점 = **330** 행 뒤) 이고 승계 대상 각주는 T-1459 **205 ~ 211** · T-1460 **212 ~ 218** 이다. mermaid 는 `grep -n 'External egress'` → 주석 **88** · edge **89 ~ 97** (**9** 개) 로, task 파일 Why 가 적은 "주석 **87** · edge **88 ~ 97**" 이 **+1 stale** 이나 각주 원문 (**205** · **212** 행) 의 `89 ~ 92` · `93 ~ 97` 은 정확하다 — 즉 stale 은 **외부 파일 (task) 쪽 1 지점** 이고 components.md 자기 좌표는 **0** 이다.
- (ii) **census 승계 재확인** — `§ 12.61` census 표 (user-facing 2:2 · orchestration 5:5 · scheduler 2:2 · worker 4:4 · db 1:1 · **egress 9:3**) 를 **1 구 인용 승계 (재산출 없음)** 하되, 판정 대상인 **egress 9:3** 만 직접 재계수했다 — `sed -n '89,97p' … | wc -l` → **9**, `awk 'NR>=285 && NR<=287 && /^\| /' … | wc -l` → **3**. 본 절 종료로 표 **17 중 17 판정 완료 · 잔여 0** (`§ 12.65` 의 **14/17** 에 **+3**).
- (iii) **대상 row 원문 · 5 컬럼 분해** — `sed -n '269,270p;285,287p' docs/architecture/components.md`. header **269** = `| from | to | sync/async | message format | 비고 |` · 구분 **270**.

| 행 | from | to | sync/async | message format | 비고 |
| --- | --- | --- | --- | --- | --- |
| 285 | GitHub Adapter | github.com / sec / ecode | async (외부 HTTPS) | HTTPS REST v3 / GraphQL v4, PAT auth (`Authorization: token ...`) | ADR-0003 §4 direct egress. 4xx 응답 catch → PermissionDeniedEvent (REQ-008). |
| 286 | Confluence Adapter | confluence.sec.samsung.net 외 | async (외부 HTTPS) | HTTPS REST, PAT auth | ADR-0003 §4. 4xx 응답 catch → PermissionDeniedEvent (REQ-016). |
| 287 | LLM Gateway | 외부 LLM provider 5 종 | async (외부 HTTPS) | HTTPS REST, API key auth (provider 별 header 다름) | ADR-0003 §4. provider 별 endpoint 차이 / model 식별자 / timeout / retry 정책은 P4 LLM gateway task. |

- (iv) **outbound 지점** — `grep -rn 'globalThis\.fetch\|fetchFn(' src --include=*.ts | grep -v spec` → 실 호출 **3 지점** (`github-adapter.service.ts` **370** · `confluence-adapter.service.ts` **455** · `llm-http-gateway.service.ts` **191** 행, 전부 `await this.fetchFn(...)`). `grep -rn 'axios\|Octokit' src … | grep -v spec` → **0 hit** 이라 특정 라이브러리를 전제하지 않고도 `§ 12.57` · `§ 12.58` 의 **주입 `fetchFn` 단일 seam** 정의가 유효함이 재확인된다. 계열별 파일 : 호출 지점 : row 는 github · confluence · llm 이 각각 **1 : 1 : 1** 이고, 대응 edge 만 **3 · 1 · 5** 로 흩어진다.
- (v) **protocol · auth 어휘** (`src` non-spec 전수 `grep -c`) — `GraphQL` **1** (`src/auth/current-user.decorator.ts` **64** 행 주석, GitHub 무관) · `graphql` **0** · `v3+json` **0** · `Authorization` **21** · `Bearer` **46** · `api-key` **15** · `x-api-key` **7** · `anthropic-version` **6**. 핵심 좌표 — `github-request.builder.ts` **29** 행 `GITHUB_PUBLIC_API_BASE` 리터럴 · **103** 행 Enterprise base `/api/v3` · **133** 행 `Authorization: Bearer` · **134** 행 `Accept: application/vnd.github+json`; `confluence-request.builder.ts` **108 · 111** 행 + **127 ~ 144** 행 `buildAuthorizationHeader` 의 Cloud `Basic <base64>` / Server `Bearer` **2 분기**; provider 헤더 **4 종** — `anthropic.adapter.ts` **94 ~ 95** (`x-api-key` + `anthropic-version`) · `azure-openai.adapter.ts` **79** (`api-key`) · `google-gemini.adapter.ts` **93** (`x-goog-api-key`) · `openai-compatible.adapter.ts` **86** (`Authorization: Bearer`). **secret · token · API key 실값과 환경변수 값은 인용하지 않았고 헤더 이름 · 상수 식별자까지만 적었다** ([CLAUDE.md](../../CLAUDE.md) §9).
- (vi) **4xx 이벤트** — `grep -rn 'PermissionDeniedEvent' src --include=*.ts | grep -v spec` → **32** hit · **7** 파일 (심볼 실재이며 hit 0 이 아니다). 실 emit 조건은 `github-adapter.service.ts` **416** 행 · `confluence-adapter.service.ts` **497** 행 의 `if (status === 401 || status === 403)` **2 지점뿐** 이고 `llm-http-gateway.service.ts` 는 매칭 **0** (**199 ~ 200** 행 generic `Error` throw). [requirements.md](../requirements.md) **27** 행 `REQ-008` · **35** 행 `REQ-016` 의 status 값은 둘 다 **`DONE (implemented-on-main)`** 이다.
- (vii) **좌표 stale · 삽입 파급 계수** — 계수 규칙은 `§ 12.55` ~ `§ 12.65` 를 **그대로 승계** (자기 좌표만 · 범위 · 나열 토큰은 1 지점 · 외부 파일 좌표 제외). 본 절이 인용 · 의존한 components.md 자기 좌표 (**88 ~ 97 · 108 ~ 113 · 205 ~ 218 · 265 · 269 ~ 287 · 289 · 324 ~ 330**) 는 (i) 실측과 **전량 일치 → stale 0 지점** 으로 T-1462 ~ T-1467 에 이어 **9 회 연속** 이다. 삽입점 (**330** 행 뒤) 이 자기 좌표 최대값 (**330**) 보다 뒤라 **삽입 파급 0 지점** 이고 in-place 정정도 **0** 이다 (계수 규칙상 제외되는 task 파일 좌표의 **+1 stale** 은 (i) 에 사실로만 남긴다).

**진척.** 본 절로 `## Contracts` 표 **17 row 중 17 판정 완료 · 잔여 0** 이며 **egress 그룹 3/3 마감** 이다. 그룹 진척: user-facing 2/2 · orchestration 5/5 · scheduler 2/2 · worker 4/4 · db 1/1 · **egress 3/3**. edge 축 **23/23** 은 `§ 12.60` 이 이미 마감했으므로 본 절로 **components.md 의 diagram 축과 `## Contracts` 표 축이 모두 종료** 된다.

**AC 2 판정표.** 축은 `§ 12.65` FU (1) 권고대로 **claim 종류별로 3 row 를 묶어** 세웠고, 한 축 안에서 row 간 판정이 갈리면 그 갈림을 명시한다 (`§ 12.62` 가 row 277 을 row 272 와 가른 선례). 판정 대상 밖 (edge 축 · node 축 · `## Component table` row · **271 ~ 284** row) 은 재판정하지 않았다.

| 축 | 실측 근거 | 판정 | 근거 1 ~ 2 구 |
| --- | --- | --- | --- |
| ① 결선 실재 + fan-out 축약 표기 | (iv) 호출 **3** 지점 · (ii) edge **9** : row **3** | **부분참** | 결선 **3** 은 전부 실재하나 (`§ 12.57` · `§ 12.58` 승계의 유효성 재검증), 축약 **6** 은 **표에 전혀 미표기가 아니라 `to` 셀 나열로 부분 표기** 된다 — **285** 의 `github.com / sec / ecode` 가 3 을 · **287** 의 `5 종` 이 5 를 셀 안에 남기고 **286** 만 `외` 로 개수가 불명이다. 은닉을 예상한 가설은 실측으로 **뒤집혔다** |
| ② protocol claim | (v) `graphql` **0** · `v3+json` **0** · builder **29 · 103** 행 | **부분참 (row 갈림)** | **285** 의 `GraphQL v4` 는 **거짓** (`src/github` graphql **0 hit** — `§ 12.57` 거짓 확정의 row 축 승계이며 `src` 전수 **1 hit** 도 `current-user.decorator.ts` **64** 행 주석으로 무관), `v3` 는 Enterprise base (**103** 행 `/api/v3`) 만 참이고 public base (**29** 행) 에는 없어 **3 instance 중 2 한정 참**. **286 · 287** 의 `HTTPS REST` 는 **참** |
| ③ auth claim | (v) builder **133** 행 · **127 ~ 144** 행 2 분기 · provider 헤더 **4** 종 | **부분참 (row 3 이 전부 갈림)** | **285** 의 `Authorization: token ...` 는 **거짓** — 실 헤더는 **133** 행 `Bearer` 다 (PAT 라는 사실 자체는 참). **286** 의 `PAT auth` 는 **부분참** — 실제는 Cloud `Basic` / Server `Bearer` **2 scheme 분기** 라 단일 표기가 한쪽을 은닉한다. **287** 의 `API key auth (provider 별 header 다름)` 는 **참** — 헤더 **4** 종을 정확히 서술해, 같은 사실을 `API key` 단일 표기로 은닉했다고 `§ 12.58` 이 부분참 판정한 **edge label 보다 row 셀이 정밀한 역전 사례** 다 |
| ④ `async (외부 HTTPS)` 값 ↔ **289** 행 정의문 | (iii) + **289** 행 정의문 (`async` = "외부 HTTPS 경계를 넘는 호출") | **참** | 본 3 row 는 정의문의 `async` 열거 항에 **정확히 부합하는 유일한 그룹** 이라, `§ 12.61` ~ `§ 12.65` 가 5 회 보고한 정의 결손이 **처음으로 작동하는 사례** 다 — 따라서 결손은 `async` 정의가 아니라 **sync 열거 항 쪽에 국한** 된다는 결론이 서고 FU27 의 성격이 좁혀진다 |
| ⑤ `ADR-0003 §4` pointer | [ADR-0003-deployment.md](../decisions/ADR-0003-deployment.md) **78** 행 heading · **80** 행 결정문 | **참** | `§` 세분이 실재하고 (**78** 행 `### Decision §4 — 외부 네트워크 boundary = direct outbound from app process`) **80** 행이 "모든 외부 endpoint 에 직접 outbound … 별도 egress proxy / NAT gateway / SOCKS bastion 은 사용하지 않는다" 로 direct egress 를 규정해 3 row 공통 pointer 가 정합이다 — `§` 세분이 **0** 이던 `§ 12.65` 축 ⑥ (ADR-0002) 와 대조된다 |
| ⑥ 4xx catch → `PermissionDeniedEvent` | (vi) 심볼 **32** hit · emit 조건 **2** 지점 · llm **0** | **부분참** | 심볼과 영속 경로는 실재하되 실 조건이 `status === 401 || status === 403` **뿐** 이라 (**416** · **497** 행) `4xx` 라는 폭 넓은 표기가 **404 · 429 를 과대 포함** 한다. **287** 이 이벤트를 걸지 않은 **비대칭은 참** — gateway 는 **199 ~ 200** 행 generic `Error` throw 만 하고 emit **0** 이다. REQ pointer 2 개는 requirements.md **27 · 35** 행 status `DONE` 으로 정합 |
| ⑦ 수 claim · 미래 시제 | (v) provider 헤더 **4** 종 · gateway **129 ~ 134** 행 enum **5** 검사 · [STATE.json](../STATE.json) `"phase": "P4-complete / P5-in-progress"` | **참 (수) · 거짓 (시제)** | `외부 LLM provider 5 종` 은 **참** (**129 ~ 134** 행이 `azure_openai` · `custom` · `openai` · `anthropic` · `google_gemini` **5** 를 검사) 이나 request 조립 adapter 는 **4** 라 `§ 12.58` 의 **5 : 4** 비대칭이 row 축에서도 유지된다. `구체는 P4 LLM gateway task` 는 **거짓** — P4 는 이미 complete 이고 endpoint · model 식별자는 이행됐다 (`timeout` · `retry` 는 gateway 전수 **0 hit** 로 미이행이나 그 잔여를 **P4 로 귀속시킬 수 없다**). `§ 12.61` 이 row 272 의 `P3 Auth task` 를 같은 논리로 거짓 확정한 것과 동형 |
| ⑧ message format 컬럼 판별력 | (iii) 3 row 셀이 전부 상이하나 **286 · 287** 의 protocol 어구만은 동일 | **부분참** | 본 그룹 안에서는 **3/3 이 서로 갈려** `§ 12.64` 축 ⑧ (판별력 **0**) 의 반례가 한 겹 더 쌓이나, 갈리게 하는 것은 protocol 이 아니라 **auth 어구** 다 — `HTTPS REST` 만 떼어내면 **286 · 287** 은 다시 구별 불가라 컬럼의 판별력은 **protocol 층에서 0 · auth 층에서 유효** 로 갈린다 (`§ 12.65` 축 ⑧ 의 "층위는 가르나 쌍은 못 가른다" 와 같은 결) |

**다중 표기 수치.** **row 3 : 실 결선 9 : 실 outbound 호출 지점 3** (`§ 12.60` · `§ 12.65` `1 : 1 : 29` · `§ 12.62` `5 : 1 : 2` · `§ 12.63` `2 : 1 : 1` · `§ 12.64` `4 : 4 : 1` 과 같은 형식). 표 **17** row 를 통틀어 **row < 결선** 인 유일한 그룹이며, `§ 12.61` census 가 서문 전수 claim 을 부분참으로 판정한 근거인 fan-out 축약 **6** (= 9 − 3) 이 전량 여기 몰려 있음이 row 시점에서 확인됐다. 세부는 **285** 가 3 : 1 · **286** 이 1 : 1 · **287** 이 5 : 1 이라 축약은 **285 · 287** 두 row 에만 존재한다.

**AC 3 — 3 row 통합 판단의 사후 실증.** 실측 — 실제 축 수 **8** (`§ 12.65` FU (1) 이 예측한 **7 ~ 8** 의 상단으로 **예측 적중**) · 본 절 행 수 **55** (상한 **100** 이내 — 압축 단서 적용 결과) · 고정비 (계보 + AC 1 (i) (ii) (vii) + AC 4 + 검산 = **9** 행) 대 판정 본문 (AC 1 (iii) ~ (vi) + AC 2 표 + 다중 표기 = **20** 행) = **9 : 20** 으로 고정비 비중 **31%** 다. `§ 12.65` 는 "분할은 축 상한 때문에 옳았을 뿐 고정비 측면에서는 손해였다" 는 한계를 남겼는데, 본 절은 그 고정비를 **1 회만** 지불하고 3 row 를 닫았다 — 2+1 로 재분할했다면 좌표 실측 · census 재확인 · 각주 삽입 · 검산이 **2 배** 로 들고 protocol · auth 축이 두 절에 중복 등장했을 것이므로 **통합은 이득이었다**. 손해도 정직하게 적는다: 축 ② · ③ 이 row 3 개를 한 칸에 담느라 **"근거 1 ~ 2 구" 제한을 넘겨** 셀이 `§ 12.65` 의 row 1 짜리 셀보다 약 **3 배** 로 길어졌고, 그만큼 개별 row 의 판정 가독성은 분할 대비 떨어진다. 즉 **축 수가 상한 안에 들면 통합이 유리하고 그 대가는 셀 밀도** 라는 것이 두 slice 를 통한 결론이다.

**AC 4 반영 결과.** components.md **330** 행 (T-1467 각주 블록 말미) 뒤에 **빈 줄 1 행 + blockquote 8 행** 을 삽입했다 (`§ 12.62` 가 실측으로 확정하고 `§ 12.63` ~ `§ 12.65` 가 승계한 "판정 대상 표와 같은 절 안, 직전 각주 뒤" 관행 + 각주 블록 간 구분 빈 줄 1 행). 무편집 경계 — 표 본체 (**269 ~ 287**) · 서문 (**267**) · **289** 행 정의문 · T-1463 ~ T-1467 각주 본문 (**291 ~ 330**) · mermaid (**28 ~ 113**) · `## Component table` 전량. in-place stale 정정은 **0 지점** ((vii) 이 자기 좌표 stale 0 을 확정).

**AC 7 검산.** `wc -l` — components.md **342 → 351** (+9) · audit **6173 → 6228** (+55 로 **100 행 이내**). `grep -c '^| '` components.md **29 불변** (`## Contracts` data row **17** 유지 · `## Component table` data row **8** 유지) · `grep -c '^## '` **7 불변** · `grep -c '^> '` **130 → 138**. `git diff --stat` = **3 파일** (components.md · 본 파일 · task 파일) · **≤ 300 LOC**. **R-110 / R-112 면제 근거** — `commitMode: direct` doc-only 라 production code **0 LOC** · 분기 **0** 이므로 [CLAUDE.md](../../CLAUDE.md) §3.2 direct doc-only 면제에 따라 `tester` 호출 · happy / error / flow / negative 4 항목 · `pnpm test:cov` 가 전부 **N/A** 이며, 측정은 전부 read-only `grep` · `sed` · `awk` · `wc` 로 `pnpm` 실행 **0** 이다.

**AC 6 파생 영향 (목록만 — 본 slice 편집 금지).** 본 doc stream 이 편집 가능한 것 (표 편집 batch · ADR 게이트 · `pr` task 소관 제외) 만 우선순위로 정렬한다. (1) **다음 slice 1 순위 = components.md `## GitHub Adapter — 3 instance 묶음 vs 분리 결정` 절 (233 ~ 264 행) 본문 ↔ 코드 대조** (`§ 12.65` FU (3) = `§ 12.48` FU4) — 근거 둘: **첫째**, 본 절로 diagram 축 (23/23) 과 `## Contracts` 표 축 (17/17) 이 모두 닫혀 그 절이 **components.md 에 남은 유일한 미판정 산문 축** 이고, **둘째**, 본 절 (iv) (v) 가 실측한 GitHub outbound 지점 (**370** 행) · host 분기 (**29 · 103** 행) · instance 고정 표기가 그 절의 핵심 claim 과 **그대로 겹쳐 한계 비용이 최소** 다 / (2) README 행 번호 pointer drift 전수 sweep (`§ 12.65` FU9) / (3) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (FU8) / (4) UC-09 `§ 5` sequence participant 병기 (FU12 — **57 회째 이월**) / (5) `CLAUDE.md` §1 pointer 부정확 (FU11) / (6) 행 번호 → anchor 좌표계 이행 (FU14 — **51 회째 이월**) + 각주 heading 참조 anchor 축소 scope (FU15) / (7) node · row 외연 정의의 문서 미박제 (FU20 — 본 절이 **주입 `fetchFn` seam** 정의를 또 승계해 정본 부재 비용을 한 번 더 이연했다) / (8) 인증 규약의 문서 단일 정본 부재 (FU26 — 본 절 축 ③ 이 auth 표기 3 종 불일치를 확정해 **근거가 크게 보강** 됐다) / (9) `sync/async` 컬럼 정의 (**289** 행) 경계 미규정 (FU27 — 본 절 축 ④ 가 결손을 **sync 열거 항 쪽으로 국한** 시켜 scope 가 좁아졌다) / (10) message format 컬럼 판별력 (FU31 — 본 절 축 ⑧ 이 **층별로 갈린다** 는 관측을 더했다). **본 stream 밖 (승계만)** — FU (2) · (25) · (30) · (32) 표 편집 batch 이며, 본 절이 그 batch 에 **후보 3 건** 을 보탠다: **285** 의 `GraphQL v4` 삭제 · `Authorization: token ...` → `Bearer` 정정 · **287** 의 `P4 LLM gateway task` 시제 정정 (전부 표 셀 in-place 라 `§ 12.15` append-only 상 본 stream 이 수행하지 않고 제안까지만 남긴다).

**한계.**

1. **측정은 grep 정적 근사다** — 실제 HTTPS 요청을 발신해 헤더 · status 를 관측하지 않았고 `pnpm` 실행 **0** 이라, 축 ② · ③ 의 판정은 **소스 리터럴 사실** 에서 도출한 결론이지 런타임 관측이 아니다.
2. **축 ① · ② 의 일부 근거는 `§ 12.57` · `§ 12.58` 판정의 인용 승계다** — edge **89 ~ 97** 자체를 재판정하지 않았고 (iv) 로 승계 유효성 (주입 `fetchFn` **3** 지점 · `axios` · `Octokit` **0**) 만 재검증했다.
3. **`timeout` · `retry` 미이행의 타당성은 판정하지 않았다** — 본 절이 가른 것은 **표기가 실 구현과 어긋나는지** 뿐이며 재시도 정책 설계는 ADR 게이트 · `pr` task 소관이다 (축 ⑦ 은 귀속 phase 만 거짓으로 확정했다).
4. **secret 값 · 환경변수 값은 확인하지 않았고 문서에도 옮기지 않았다** — 헤더 **이름** 과 상수 **식별자** 까지만 기록했다 ([CLAUDE.md](../../CLAUDE.md) §9).
5. **(vii) 의 stale 계수는 본 절이 인용한 좌표 집합 안에서만 0 이다** — components.md 전량을 1:1 대조하지는 않았고, 계수 규칙상 제외된 task 파일의 **+1 stale** 은 정정하지 않고 사실로만 남겼다.
6. **`## Contracts` 표 축은 닫혔으나 표 자체의 결함은 남는다** — 본 절이 확정한 거짓 · 부분참 (`GraphQL v4` · `token` 헤더 · `P4` 시제 · fan-out 축약 미명시) 의 실제 정정은 전부 **표 편집 batch** 소관이며, 본 절은 각주 병기까지만 수행했다.

### 12.67 components.md `## GitHub Adapter — 3 instance 묶음 vs 분리 결정` 절 (233 ~ 263 행) 산문 ↔ 실 `src/github` 구조 · config · env · pointer 대조 — claim 26 중 사실 주장 13 을 축 8 이 100% 커버 · 거짓 2 · 부분참 6 · 산문 축 1/1 마감 = **components.md 전체 판정 종료** (T-1469)

> **본 절의 위치 · 계보** — `§ 12.66` (T-1468) 이 외부 egress **3** row 를 닫아 `## Contracts` 표 축이 **17/17** 로 종료되고 edge 축은 `§ 12.60` 이 **23/23** 으로 이미 마감한 상태에서, `§ 12.66` 파생 영향 **(1)** 이 본 절을 다음 slice 1 순위로 지목했다 — 근거 둘 (남은 유일한 미판정 산문 축 · `§ 12.66` (iv) (v) 실측과 claim 이 그대로 겹쳐 한계 비용 최소) 을 본 절이 **그대로 채택** 했다. 동시에 `§ 12.48` FU4 → `§ 12.65` FU (3) → `§ 12.66` FU (1) 로 **3 회 이월된 축의 회수** 이며, protocol 축 (④) 은 신설이 아니라 `§ 12.57` · `§ 12.66` 축 ② 의 **승계 + 유효성 재검증** (**7 회 연속** 이월 회수) 이다. 본 절은 **components.md 의 첫 산문 축 판정** 이라 표 축과 달리 claim 경계가 자명하지 않아 추출 규칙 정의가 산출물에 포함된다 (AC 3). 편집 방침은 `§ 12.15` append-only 를 그대로 따른다.

**AC 1 실측 (요약형).** `§ 12.66` 이 **55** 행으로 성공시킨 압축 방식을 승계해 **명령 + 수치** 만 적고 출력 전문은 생략한다.

- (i) **좌표** — `grep -n '^#\{1,3\} ' docs/architecture/components.md` → `1` · `5` · `22` · `28` · `115` · **`233 ## GitHub Adapter …`** · **`265 ## Contracts`** · **`341 ## References`** 로 task 기재값과 **전량 일치**. `wc -l` → **351**. `grep -n '^> ' … | tail -12` → T-1468 각주 블록 **332 ~ 339** 이고, 본 slice 삽입점은 그 블록이 아니라 **판정 대상과 같은 절 안** (`§ 12.62` 확정 관행) 이라 **263 행 뒤** 다. audit 은 **6228** 행 · `### 12.66` **6160** · `## 11. References` **6215** 로 역시 일치.
- (ii) **claim 추출** — `sed -n '233,264p'`. 추출 단위는 **문장 종결 1 = claim 1**, bullet 은 내부 문장 수만큼, 코드블록 fence 전체는 **1 claim** (instance row 3 을 개별 계수하면 필드명 claim 이 3 배로 부풀어 밀도가 왜곡된다). 결과 **총 26 claim** (**235** 1 · **237** 1 · **239** 1 · **240** 1 · **241 ~ 247** 1 · **251 ~ 255** 10 · **257** 1 · **259 ~ 261** 7 · **263** 3). 축 ① ~ ⑧ 의 커버 합집합은 **13 claim = 50.0%** 이고, 미커버 **13** 은 반사실 추론 **4** ("3 sub-component 분리 시 …") · 원칙 / 예시 **3** (DRY · open-closed · `private-corp` 가정) · 결정 시점 의도 **6** 이라 **코드 대조로 참 · 거짓을 물을 수 없다** — 즉 **사실 주장 13 기준 커버율은 100%** 다 (`§ 12.63` 축 ⑤ 의 사실 / 의도 2 분할 승계).
- (iii) **구조** — `ls src/github/` → **14** 파일 중 비-spec **7** (`github-adapter.service.ts` · `github-instance-client.service.ts` · `github-instance-config.ts` · `github-live-test-gating.ts` · `github-request.builder.ts` · `github-token-decrypt.ts` · `github.module.ts`). `grep -rn '^export (class|interface|type|const)' src/github/*.ts | grep -v spec` → **31** 심볼이며 `GithubAdapter` 는 **237** 행, `GithubInstanceClient` 는 **47** 행. `github.module.ts` **55 ~ 64** 행 `providers` = **5 entry** (`GithubAdapter` · `GithubInstanceClient` · `LlmApiKeyCipher` · `PersistingPermissionDeniedEmitter` + `PERMISSION_DENIED_EMITTER` token) · `exports` **2**.
- (iv) **instance key · config** — `grep -rn "'com'|'sec'|'ecode'" src/github/*.ts | grep -v spec` → `com` **0 hit**, 실 union 은 `github-live-test-gating.ts` **37** 행 `"public" | "sec" | "ecode"` (+ **62 · 67** 행 spec 상수). 런타임 config 는 하드코딩이 아니라 env 주도 — `github-instance-config.ts` **21** 행 `GITHUB_INSTANCES_ENV` + **25 ~ 30** 행 suffix **4** 종 (`_HOST` · `_ORG` · `_TOKEN_ENC` · `_REPOS`), `GithubInstanceConfig` (**33** 행) 필드는 `key: string` (**35**) · `host` (**38**) · `orgs: string[]` (**44**) · `repos: string[]` (**51**) · `tokenEnc` (**54**) 로 예시의 `baseUrl` / `token` / `org` 와 **3/3 불일치**.
- (v) **protocol · 차이 축** — `grep -rn 'graphql|GraphQL|/api/v3|api.github.com' src/github/*.ts | grep -v spec` → `graphql` · `GraphQL` **0 hit**, `https://api.github.com` 은 `github-request.builder.ts` **29** 행 상수, `/api/v3` 는 **103** 행 Enterprise 분기뿐 (**95 ~ 103** 행이 host 를 두 base 형태로 가르는 도출 규칙). instance 차이 축은 base URL / PAT / org 외에 **API base 도출 규칙** · **토큰 JIT 복호** (`github-token-decrypt.ts` **48** 행 `decryptGithubInstanceToken` · **70** 행 `decryptGithubInstanceConfigToken`) · **`_REPOS` allowlist** 로 **3 개 더** 계수된다.
- (vi) **env · 메서드 · 심볼** — `grep -rn 'NODE_EXTRA_CA_CERTS|HTTPS_PROXY' src/ deploy/ | wc -l` → **3** 이며 **`src/` 성분은 0** (`deploy/env.prod.example` **42 · 43** 행 = 둘 다 주석 · `deploy/README.md` **228** 행). `grep -rn 'fetchCommits|async ' src/github/github-adapter.service.ts` → `fetchCommits` **0 hit**, 실 메서드는 `request` (**263**) · `requestAllPages` (**287**) · `private fetchAndMap` (**360**) 이고 key-first 시그니처는 `GithubInstanceClient.requestForInstance(key, path, query)` (**64 ~ 68** 행) 에 있다. `grep -rn 'GithubComAdapter|GithubSecAdapter|GithubEcodeAdapter' src/ | wc -l` → **0**. `ls docs/decisions/ | grep ADR-0004` → **`ADR-0004-smoke-e2e-db-mode.md` 1 건 (번호 선점)**. `docs/tasks/T-0016-t-a3-component-view.md` 실재하며 그 **227** 행 Follow-ups 가 "(b) 채택 시 ADR-0004 신설 별도 task 제안" 을 실제로 담는다. **secret · 환경변수 실값은 인용하지 않았고 이름 · 상수 식별자까지만 적었다** ([CLAUDE.md](../../CLAUDE.md) §9).
- (vii) **좌표 stale · 삽입 파급 계수** — 계수 규칙은 `§ 12.55` ~ `§ 12.66` 을 **그대로 승계** (자기 좌표만 · 범위 · 나열 토큰은 1 지점 · 외부 파일 좌표 제외). 본 절이 인용 · 의존한 components.md 자기 좌표 (**233 · 235 ~ 263 · 265 · 341 · 351 · 332 ~ 339**) 는 (i) 실측과 전량 일치 → **stale 0 지점** 으로 T-1462 ~ T-1468 에 이어 **10 회 연속** 이며 in-place 정정도 **0** 이다. 삽입점 (**263** 행 뒤) 이 자기 좌표 최대값보다 앞이라 **삽입 파급은 18 지점** — 기존 blockquote 중 **265 ~ 351** 구간 자기 좌표를 인용한 행 수이며 (행당 1 지점 · 범위 · 나열 1 지점), 전 좌표가 **+6 균일 이동** 이라 상대 관계는 불변이다 (`§ 12.60` ~ `§ 12.66` 은 삽입점이 자기 좌표 뒤라 파급 0 이었다 — **본 절이 파급 > 0 인 첫 사례**).

**AC 2 판정표 (사실 주장 / 설계 의도 분리).**

| 축 | 실측 근거 | 판정 | 근거 (1 ~ 2 구) |
| --- | --- | --- | --- |
| ① 단일 component `GithubAdapter` 1 service 구조 (**237 · 239** 행) | `GithubAdapter` **237** 행 실재 · 비-spec **7** 파일 / **31** 심볼 · instance 라우팅은 `GithubInstanceClient` **47 · 64** 행 | **부분참** | "3 instance 를 3 adapter 로 쪼개지 않는다" 는 결정은 유지되나, 실 분해는 instance 축이 아닌 **관심사 축** 으로 이뤄져 "1 service 가 instance key 를 인자로 받는다" 는 표기가 어긋난다 |
| ② instance key `'com'` / `'sec'` / `'ecode'` (**239 · 241 ~ 247** 행) | `'com'` 비-spec **0 hit** · 실 union `"public" \| "sec" \| "ecode"` (gating **37** 행) · `key: string` (**35** 행) + `GITHUB_INSTANCES` env (**21** 행) | **부분참** | `sec` · `ecode` 2/3 는 참이나 `com` 은 실제 `public` 이고, 집합이 하드코딩 union 이 아니라 **config 주도 가변** 이라 "3" 이 고정 수가 아니다 (`예:` 어법이 부분참을 지탱) |
| ③ config schema 예시 + `실제 구현은 P4 task` (**240 ~ 247** 행) | 필드명 **3/3 불일치** (`host` **38** · `tokenEnc` **54** · `orgs` **44**) · YAML 계층 부재 (env suffix **25 ~ 30**) · [STATE.json](../STATE.json) `phase` = `P4-complete / P5-in-progress` | **거짓** | schema 형태 · 필드명 · 주입 방식이 모두 다르고, 미래 시제는 `§ 12.61` · `§ 12.66` 축 ⑦ 에 이은 **3 회째 잔존** 이다 |
| ④ `REST v3 / GraphQL v4` (**251** 행) | `graphql` · `GraphQL` `src/github` 비-spec **0 hit** · `/api/v3` **103** 행 Enterprise 한정 · public 은 **29** 행 `https://api.github.com` | **부분참 (승계 유효)** | `GraphQL v4` 는 거짓이 유지되고 `v3` 는 3 instance 중 **2** 한정이라 `§ 12.57` · `§ 12.66` 축 ② 판정이 산문 축에서도 그대로 성립한다 |
| ⑤ `instance 별 차이는 base URL / PAT / org 만` (**252** 행) | base 도출 규칙 분기 **95 ~ 103** · 토큰 JIT 복호 **48 · 70** · `_REPOS` **30** 행 = 차이 축 **+3** | **부분참** | `만` 이라는 한정어는 과소이나, 추가된 3 축도 전부 **config 로 흡수** 되어 "분리 불요" 라는 결정 취지 자체는 반증되지 않는다 |
| ⑥ ADR-0003 §4 TLS / proxy 응집 (**254** 행) | [ADR-0003](../decisions/ADR-0003-deployment.md) **78** 행 `### Decision §4` 실재 + 근거 **85** (`HTTPS_PROXY`) · **87** (`NODE_EXTRA_CA_CERTS`) · `src/` **0 hit** · `deploy/` **3 hit** (주석) | **부분참** | pointer 는 `§` 세분까지 실재하고 인용 내용도 정합이나, "단일 HTTP client 에 자연 적용" 은 **Node 런타임 전역 env 라 코드로 반증 · 입증 불가** 이고 실 박제는 배포 예시 주석뿐이다 |
| ⑦ `githubAdapter.fetchCommits('com', ...)` + `1 provider` (**255** 행) | `fetchCommits` **0 hit** · 실 메서드 `request` **263** / `requestAllPages` **287** (첫 인자 = `GithubRequestInput`) · providers **5 entry** (**55 ~ 64** 행) | **거짓** | 메서드명 · 첫 인자 · provider 수가 모두 다르다. 단 key-first 패턴은 `requestForInstance` (**64** 행) 로 실재해 "controller 가 dispatch 로직을 갖지 않는다" 는 **의도는 참** 이다 |
| ⑧ Alternatives 심볼 + `ADR-0004` 예고 + `T-0016` 출처 (**235 · 257 · 261 · 263** 행) | 대안 심볼 3 종 `src/` **0 hit** · `ADR-0004-smoke-e2e-db-mode.md` 번호 선점 · `T-0016` task 파일 + **227** 행 Follow-ups 실재 | **부분참** | 미채택 박제와 출처 · Follow-ups claim 은 참이나, **261 · 263** 행이 2 회 예고한 `ADR-0004` 는 이미 다른 주제가 점유해 **pointer 2 지점이 거짓** 이다 (`§ 12.62` pointer 축 판정 승계) |

**다중 표기 수치.** 표 축의 `row N : 실 결선 N : 실 호출 지점 N` 형식은 **산문 축에 부적용** 이다 — 산문 claim 에는 결선 · 호출 지점처럼 1:1 대응하는 실체가 없고 한 문장이 0 ~ 3 개의 코드 좌표에 걸친다. 대체 형식으로 **`claim N : 사실 주장 N : 실측 대조 지점 N : 가설 정정 N`** 을 정의하고 본 절 값은 **26 : 13 : 26 : 4** 다 (실측 대조 지점 = 인용한 코드 · 문서 좌표의 distinct 수, 범위 · 나열 1 지점). 가설 정정 **4** 는 planner 가설 대비 실측이 뒤집은 축 — ① (instance 라우팅이 별 클래스) · ② (`com` 부재 + config 주도) · ⑤ (`_REPOS` 라는 4 번째 차이 축) · ⑦ (패턴이 단순 부재가 아니라 **다른 클래스에 실재**) 이며, T-1440 이후 planner 기대가 실측에 정정된 **31 회째** 다.

**AC 3 — 산문 축 vs 표 축의 방법론 차이.** (a) **추출 단위** — 위 (ii) 규칙 (문장 1 = claim 1 · bullet 은 내부 문장 수 · 코드블록 fence 전체 = 1). (b) **커버율** — 전체 **26 중 13 = 50.0%**, 사실 주장 기준 **13/13 = 100%**. (c) **고정비 · 밀도** — `§ 12.61` ~ `§ 12.66` 은 row **17** 을 **6** slice 로 처리해 slice 당 **2.8** row 였고 셀 단위로 claim 경계가 자명해 추출 고정비가 **0** 이었다. 산문 축은 추출 규칙 정의 자체가 산출물이라 고정비를 **1 회** 지불하나 1 slice 에 **26 claim / 8 축** 을 처리해 claim 밀도가 표 축 대비 **약 9.3 배** (26 : 2.8) 다 — 즉 **산문은 고정비 高 · 한계비용 低** 이며, 표 축을 6 분할한 것과 달리 산문 축은 **1 slice 통합이 유리** 했다. 손해도 적는다: 미커버 **50%** 가 반증 불가 claim 이라 "판정률" 이 표 축 (row 전량 판정) 보다 구조적으로 낮게 보이며, 이 수치는 **부실이 아니라 산문이라는 매체의 성질** 이므로 향후 산문 축 slice 는 **사실 주장 기준 커버율을 정본 지표로** 쓴다.

**AC 4 반영 결과.** components.md **263** 행 (절 말미 · `## Contracts` heading 앞) 뒤에 **빈 줄 1 행 + blockquote 5 행** 을 삽입했다 (상한 8 행 이내 · `§ 12.62` 가 확정하고 `§ 12.63` ~ `§ 12.66` 이 승계한 "판정 대상과 같은 절 안" 관행). 절 본문 (**235 ~ 263**) · config 코드블록 · `## Contracts` 표 · **289** 행 정의문 · 기존 각주 · mermaid · `## Component table` 은 **무편집** 이고 in-place 정정 **0** 이다.

**AC 7 검산.** `wc -l` — components.md **351 → 357** (+6) · audit **6228 → 6276** (+48 로 본 절 **48 행 = 상한 100 이내**). `grep -c '^| '` components.md **29 불변** · `grep -c '^## '` **7 불변** · ` ``` ` fence **2** (짝수) 로 markdown 문법 무손상. `git diff --stat` **3 파일** (components.md · REQ-COVERAGE-AUDIT.md · 본 task 파일) 이고 doc-only 라 `pnpm` 실행 **0** ([CLAUDE.md](../../CLAUDE.md) §3.2 direct doc-only 면제).

**진척.** components.md 판정 축 = **diagram edge 23/23** (`§ 12.54` ~ `§ 12.60`) + **`## Contracts` 표 17/17** (`§ 12.61` ~ `§ 12.66`) + **산문 1/1** (본 절) 로 **문서 전체 판정 완료 · 잔여 축 0** 이다. `## 개요` · `## Deployment 컨텍스트` · `## Component table` · `## References` 는 diagram · 표 축 slice 들이 대조 기준으로 이미 소비했고 독립 claim 축을 남기지 않았다. 즉 **components.md 는 본 절로 audit 대상에서 종료** 되며, 이후 이 문서에 대한 본 stream 의 작업은 **신규 판정이 아니라 표 · 산문 편집 batch (in-place 정정) 소관** 이다.

**AC 6 파생 영향 (목록만 — 본 slice 편집 금지).** **components.md 는 전량 판정 완료임을 명시적으로 선언한다.** 본 doc stream 이 편집 가능한 것 (표 · 산문 편집 batch · ADR 게이트 · `pr` task 소관 제외) 만 우선순위로 정렬한다. (1) **다음 slice 1 순위 = README 행 번호 pointer drift 전수 sweep** (`§ 12.65` FU9 · `§ 12.66` FU (2)) — 근거 둘: **첫째**, components.md 종료로 architecture 문서 축이 비었고 남은 후보 중 **판정 대상이 자기 완결적** (pointer ↔ 대상 행 1:1) 이라 산문 축의 반증 불가 문제가 없으며, **둘째**, `§ 12.61` ~ `§ 12.67` 이 **7 회 연속** pointer 축 (ADR `§` 세분 · ADR-0004 번호 충돌) 에서 결함을 찾아 **적중률이 실측으로 뒷받침** 된다 / (2) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (FU8 — 본 절 축 ⑥ 이 `deploy/` 3 hit 을 실측해 **입력이 이미 확보** 됐다) / (3) UC-09 `§ 5` sequence participant 병기 (FU12 — **58 회째 이월**) / (4) `CLAUDE.md` §1 pointer 부정확 (FU11) / (5) 행 번호 → anchor 좌표계 이행 (FU14 — **52 회째 이월** · 본 절이 삽입 파급 **18 지점** 을 처음 계수해 **근거가 크게 보강** 됐다) + 각주 heading 참조 anchor 축소 (FU15) / (6) node · row 외연 정의 문서 미박제 (FU20) / (7) 인증 규약 단일 정본 부재 (FU26 — 본 절 축 ⑤ · ⑥ 이 토큰 복호 · TLS env 축을 추가) / (8) `sync/async` 컬럼 정의 (**289** 행) 경계 미규정 (FU27) / (9) message format 컬럼 판별력 (FU31). **본 stream 밖 (승계만)** — 표 편집 batch 에 더해 본 절이 **산문 편집 batch** 를 신설 후보로 남기며 그 항목은 **4** 건이다: **240** 의 `P4 task` 시제 정정 · **241 ~ 247** config 예시를 env 주도 형태로 교체 · **255** 의 `fetchCommits('com', ...)` → `requestForInstance(key, ...)` 정정 · **261 · 263** 의 `ADR-0004` 번호 재지정 (마지막 건은 [CLAUDE.md](../../CLAUDE.md) §5 ADR 게이트라 owner 판단 선행).

**한계.**

1. **측정은 grep 정적 근사다** — 실제 GitHub 호출을 발신하지 않았고 `pnpm` 실행 **0** 이라, 축 ② · ⑤ · ⑦ 은 **소스 리터럴 사실** 에서 도출한 결론이지 런타임 관측이 아니다.
2. **claim 26 중 13 (50.0%) 은 판정하지 않았다** — 반사실 추론 · 원칙 주장 · 결정 시점 의도라 코드로 반증할 수 없어 **미판정으로 남긴 것이며 참으로 간주한 것이 아니다**.
3. **축 ④ 는 `§ 12.57` · `§ 12.66` 판정의 인용 승계다** — edge · row 자체를 재판정하지 않았고 (v) 로 승계 유효성 (`graphql` **0** · `/api/v3` Enterprise 한정) 만 재검증했다.
4. **축 ⑥ 의 "자연 적용" 은 거짓으로 확정하지 못했다** — 두 env 는 Node 전역이라 코드 미참조가 정상이며, 실제 적용 여부는 **런타임 · 배포 환경 관측** 이 있어야 갈린다 (본 절은 `src/` **0 hit** 과 `deploy/` 주석 **3 hit** 이라는 사실까지만 남겼다).
5. **(vii) 의 stale 계수는 본 절이 인용한 좌표 집합 안에서만 0 이다** — components.md 전량을 1:1 대조하지 않았고, 삽입 파급 **18 지점** 도 기존 각주를 편집하지 않았으므로 **정정이 아니라 계수** 다.
6. **판정이 확정한 거짓 · 부분참의 실제 정정은 본 절 소관이 아니다** — 산문 in-place 수정 · ADR-0004 번호 재배정 · 코드 표기 정합은 각각 산문 편집 batch · ADR 게이트 · `pr` task 소관이며, 본 절은 각주 병기와 후보 **4** 건 제안까지만 수행했다.

### 12.68 README 행 번호 pointer drift 전수 census (**126** 지점 / **11** 파일) + 정본 2 파일 (`CLAUDE.md` · `docs/requirements.md`) 산문 pointer **26** 지점 실판정 — 참 24 · 부분참 2 · 거짓 0 · 표기 변종 **10** 종 확정 · 단일 정규식 전수 포착 불가 (T-1470)

> **본 절의 위치 · 계보** — `§ 12.67` (T-1469) 이 components.md 산문 축 1/1 을 닫아 **문서 전체 판정이 종료** 된 뒤, 그 파생 영향 **(1)** (= `§ 12.65` FU9 · `§ 12.66` FU (2)) 이 지목한 **README 행 번호 pointer 축** 을 본 절이 그대로 채택한다 — 근거 둘 (architecture 문서 축 소진 · pointer 는 대상 행 1:1 이라 산문 축의 반증 불가 문제가 구조적으로 없음) 도 승계. 단 "전수" 는 1 slice 에 담기지 않아 **(a) 전수 census (계수)** + **(b) 정본 2 파일 산문 pointer 실판정** 으로 잘랐다 (`CLAUDE.md` = agent 행동 규칙 정본 · `docs/requirements.md` = REQ ID 정본 이라 pointer drift 의 파급이 가장 크고, 둘 다 README 를 유일 상위 출처로 인용해 판정 기준이 단일). 편집 방침은 `§ 12.15` append-only 를 그대로 따르며 pointer in-place 정정은 **0** 이다.

**AC 1 실측 (요약형).** `§ 12.66` (55 행) · `§ 12.67` (48 행) 이 성공시킨 압축 방식을 승계해 **명령 + 수치** 만 적는다.

- (i) **전수 census** — `grep -rn --include='*.md' -oE '(README\.md\)?|README)[^0-9a-zA-Z가-힣]{0,4}[0-9]{1,3}[ ]?(~|–|-|—|,)?[ ]?[0-9]{0,3}[ ]?(행|번째 줄|줄)'` (`docs/progress/` · `docs/tasks/` · 본 파일 제외) → **raw token 63 / 11 파일**. 파일별: `docs/requirements.md` **14** · `CLAUDE.md` **13** · `ADR-0002` **10** · `ADR-0001` **6** · `deployment.md` **6** · `ADR-0003` **5** · `ADR-0042` **4** · `.claude/agents/reviewer.md` **2** · `ADR-0004` · `ADR-0005` · `directory.md` 각 **1**. 같은 조건에서 README 를 **언급만** 하는 md 는 **67** 파일이라, pointer 보유는 그중 **11 (16.4%)** 뿐이다. planner 사전 census (**57 지점 / 18 파일**) 는 **양방향으로 어긋났다** — 파일 수는 단순 언급 파일 (`UC-04` · `LOOP.md` 실측 pointer **0**) 을 과대 계상, 지점 수는 정본 2 파일을 과소 계상 (`CLAUDE.md` 기대 7~8 → 실측 **13**, `requirements.md` 기대 12 → 산문 **13** + 표 컬럼 **66**). 중복 · 연속 토큰 보정 후 **distinct 지점 = 산문 60** (정본 2 파일 **26** + 그 밖 9 파일 **34**) **+ 표 컬럼 66 = 총 126**. `wc -l` → README **151** (기대 일치) · `CLAUDE.md` **465** · `docs/requirements.md` **97** · 본 파일 **6276**.
- (ii) **판정 대상 확정** — 정본 2 파일의 **산문 pointer 26 지점** (`CLAUDE.md` **13** = 114 · 147 · 149 · 151 · 156 · 160 · 171 · 176 · 184 · 186 · 221 · 251 · 355 행 / `requirements.md` **13** = 20 행 3 좌표 · 23 · 28 · 30 · 33 · 34 · 36 · 37 · 39 · 67 · 75 행). 번호 붙인 목록은 아래 **AC 2 판정표의 `#` 컬럼으로 실체화** 했다 (`출처 파일:행` · `주장 좌표` · `인용 어구 요약` 3 요소를 표가 그대로 담아 중복 나열을 피한다 — 절 ≤ 100 행 보호). 표 컬럼형 **66** 지점은 (iv) (v) 의 구조 검사까지만 하고 어구 대조는 이월한다 (66 행 판정표는 상한 초과).
- (iii) **대상 행 대조** — `sed -n '<N>p' README.md` batch. 범위 표기는 시작 · 끝 **두 좌표를 각각** 확인했고, **대조 좌표 총 34** (단일 20 × 1 + 범위 7 × 2). 끝 좌표가 본문이 아닌 경우 **2** 건: `136~139` 의 **139 = 빈 줄** (인용 어구 `pnpm install` 은 **140** 행) · `19~22` 의 **19 · 22 = 둘 다 빈 줄**. `117–128` 의 끝 **128** 은 fence 가 아니라 **마지막 check 본문** 이며 fence 는 **118** (열림) · **129** (닫힘) 로 범위 밖이다.
- (iv) **교차 검증** — 산문 pointer 기준, **2+ 파일이 같은 README 좌표를 인용한 지점 10** (9 · 34 · 37 · 92 · 108 · 110~114 구간 · 112 · 113 · 117~128 구간 · 19~22). 그중 **같은 행을 다른 의미로** 인용한 것은 **2** — **128** 행 (`CLAUDE.md` 221 · 355 는 8 check 블록의 **범위 끝** ↔ `.claude/agents/reviewer.md` 87 · 113 은 **8 번째 check 본문** "PR 에 Comment 를 남긴다") 과 **19~22** (`requirements.md` 39 는 옛 REQ-020 번호 잔재의 **인용** ↔ `ADR-0003` 22 는 자기 주장으로서의 권한 부족 통지). **의미 상충으로 동시 참이 불가능한 지점은 0** — 128 은 마지막 check 이면서 동시에 범위 끝이라 두 인용이 모두 성립한다. 같은 파일 안 2 의미 인용도 **1** 건 (**116** 행 = `CLAUDE.md` 184 · 186 의 이중 합의 ↔ 251 의 round 7) 이며 역시 동시 참이다. 편집 대상 밖 `reviewer.md` **2** 지점은 판정만 하고 (**둘 다 참**) 목록에 남긴다.
- (v) **표기 형식 계수** — 정본 2 파일 **92** 지점이 **10 변종** 으로 갈린다: `README NNN행` **3** · `[README.md](README.md) NNN행` **6** · `README NNN–NNN행` (en dash) **3** · 링크 + en dash 범위 **1** · `README NNN 행` (공백) **9** · `` `README.md` NNN~NNN 행 `` **1** · README 앵커 없는 연속 토큰 `NNN~NNN 행` **2** · `README NNN~NNN 행` **1** · 표 컬럼 bare 숫자 (`| REQ-004 | 9 |`) **63** · 표 컬럼 hyphen 범위 (`45-46`) **3**. (i) 의 census 정규식이 포착한 것은 **24/92 = 26.1%** 뿐이고 미포착 **68** 은 전부 **README 앵커가 같은 토큰에 없는** 형태다 (연속 토큰 2 · 표 컬럼 66). → **단일 정규식으로 전수 포착 불가** 가 실측으로 확정된다 (축 ⑧). 표 컬럼 축의 구조 검사는 통과했다 — 66 값 전부 **1 ≤ N ≤ 151** 범위 안이고 REQ-001 → REQ-066 순서로 **엄격 증가** 하며 (최대 133), spot-check 6 건 (REQ-001 = 1 · REQ-002 = 6 · REQ-005 = 16 · REQ-023 = 45-46 · REQ-048 = 92 · REQ-065 = 117-128) 이 모두 대응한다.
- (vi) **좌표 stale · 삽입 파급 계수** — 계수 규칙은 `§ 12.55` ~ `§ 12.67` 을 **그대로 승계** (자기 좌표만 · 범위 · 나열 토큰은 1 지점 · 외부 파일 좌표 제외). 본 절이 인용 · 의존한 좌표 중 README **151** · 본 파일 **6276** · `§ 12.67` **6215** · `## 11. References` **6263** · `requirements.md` **97** 은 전량 일치하나, task 정의서 Required Reading 이 기재한 `CLAUDE.md` pointer 행 **127 · 188** 이 실측 (**127** 에 pointer 없음 · **188** 이 아니라 **186**) 과 어긋나 **stale 2 지점** 이다 — T-1462 ~ T-1469 의 **10 회 연속 stale 0 기록이 본 절에서 중단** 된다 (task 파일이 "AC 1 에서 재실측" 단서를 달았으나 인용 좌표인 이상 계수 대상이다). in-place 정정은 **0** (해당 좌표는 task 파일 Follow-ups 에만 기록 — `CLAUDE.md` 는 편집 금지). **삽입 파급 0 지점** — `requirements.md` 각주는 **파일 말미 append** 라 기존 행 번호가 불변이고, 본 절은 `## 11. References` **직전** 삽입이라 밀리는 구간의 자기 좌표 인용이 **0** 이다 (`§ 12.67` 은 파급 18 이었다).

**AC 2 판정표 (산문 pointer 26 지점 전수 — 참도 생략하지 않는다).**

| # | 출처 `파일:행` | 주장 좌표 | 인용 어구 요약 | 실 README 행 요약 | 판정 | 근거 (1 구) |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `CLAUDE.md:114` | 109 | 1 task = 1 commit | 109 "한번에 … (하나의 commit에는 하나의 주제)" | **참** | 주제 1 = commit 1 어구가 그대로 실재 |
| 2 | `CLAUDE.md:147` | 110–114 | §3.2 heading "Test·CI 절대 규칙 명문화" | 110 검토 + test / 114 종료 전 CI | **참** | 시작 · 끝 두 좌표 모두 test · CI 지시 |
| 3 | `CLAUDE.md:149` | 110–114 | "다음 지시를 절대 규칙으로" | 동상 | **참** | 5 행 : R-110 ~ R-114 5 규칙이 1:1 |
| 4 | `CLAUDE.md:151` | 110 | R-110 코드 검토 + test case + test 수행 | 110 동어 | **참** | 3 요소 어순까지 일치 |
| 5 | `CLAUDE.md:156` | 111 | R-111 CI 자동 실행 · fail → CI error | 111 동어 | **참** | agent · 개발자 인지 절까지 일치 |
| 6 | `CLAUDE.md:160` | 112 | R-112 기능 / 예외 / flow + negative | 112 동어 | **참** | negative test 문장이 실재 |
| 7 | `CLAUDE.md:171` | 113 | R-113 smoke + e2e 도 CI | 113 동어 | **참** | 3 종 열거가 일치 |
| 8 | `CLAUDE.md:176` | 114 | R-114 commit 후 test · 종료 전 CI | 114 동어 | **참** | 두 시점 요구가 모두 실재 |
| 9 | `CLAUDE.md:184` | 116 | §3.3 heading 이중 합의 | 116 Reviewer + Committer 합의 | **참** | 두 agent 명칭이 그대로 |
| 10 | `CLAUDE.md:186` | 116 | 두 agent 합의해야 merge | 동상 | **참** | 9 와 동일 좌표 · 동일 의미 |
| 11 | `CLAUDE.md:221` | 117–128 | reviewer verdict 8 check | 117 도입문 / 128 마지막 check | **참** | 118 · 129 fence 는 범위 밖 · check 8 개 (121 ~ 128) 를 온전히 포함 |
| 12 | `CLAUDE.md:251` | 116 | Review round 7 초과 | 116 후반 "round 7 까지" | **참** | 1 행에 2 규칙이 겹쳐 9 · 10 과 동시 참 |
| 13 | `CLAUDE.md:355` | 117–128 | reviewer 위임 정당화 | 동상 | **참** | 11 과 동일 범위 · 동일 의미 |
| 14 | `requirements.md:20` | 4~9 | `# Assessment-Agent` 소개 | 4 heading / 9 LLM 코멘트 | **참** | 시작 heading · 끝 본문 모두 소개 절 |
| 15 | `requirements.md:20` | 66~74 | 시각화 · UI 열거 | 66 heading / 74 최근 구간 삭제 | **참** | 열거한 7 항목이 68 ~ 74 에 실재 |
| 16 | `requirements.md:20` | 136~139 | "`pnpm install` 부터의 실행 순서" | 136 heading / **139 = 빈 줄** · `- pnpm install` 은 **140** | **부분참** | 시작은 참이나 끝 좌표가 빈 줄이고 **인용 어구가 범위 밖 1 행** |
| 17 | `requirements.md:23` | 9 | "어떠한 주요 활동이 있었는지" | 9 동어 | **참** | 문자열 완전 일치 |
| 18 | `requirements.md:28` | 21 | "2 월 결과물이 3 월 timestamp…" + "commit ID 는 다르지만 중복된 내용물" | 21 에 두 어구 모두 실재 | **참** | 동일 좌표 2 인용이 모두 대응 |
| 19 | `requirements.md:30` | 25 | "더 높은 점수" (뒷절) | 25 "더 높은 점수와 더 높은 평가 코멘트" | **참** | 뒷절 위치까지 정확 |
| 20 | `requirements.md:33` | 30 | "다른 개발자들의 활동에 도움이 된 경우" | 30 동어 | **참** | 본절 인용이 문자열 일치 |
| 21 | `requirements.md:34` | 31 | "지정된 주소" · 작성 / 업데이트 두 활동 | 31 동어 | **참** | 두 인용 모두 같은 행에 실재 |
| 22 | `requirements.md:36` | 34 | 허용한 셋 (crawling / page List / Hierarchy) | 34 동어 | **참** | 3 항 열거가 일치 |
| 23 | `requirements.md:37` | 37 | "단순 보고성 자료를 질적으로 낮게 평가" (앞절) | 37 동어 | **참** | 앞절 · 뒷절 (copy-paste) 구분까지 정확 |
| 24 | `requirements.md:39` | 19~22 | 권한 부족 통지 (옛 REQ-020 번호 잔재의 인용) | **19 빈 줄** · 20 권한 부족 · 21 중복 제거 (무관) · **22 빈 줄** | **부분참** | 주제가 맞는 행은 **20 단독** 이고 시작 · 끝 두 좌표가 모두 빈 줄 |
| 25 | `requirements.md:67` | 92 | "로딩 및 시각화에 3초 이내" | 92 동어 | **참** | 괄호 안 문자열까지 일치 |
| 26 | `requirements.md:75` | 108 | "well-maintained" 요구 | 108 동어 | **참** | 중복 import 금지 절과 같은 행 |

**다중 표기 수치.** 표 축의 `row N : 실 결선 N : 실 호출 지점 N` 은 **pointer 축에 부적용** 이다 — pointer 는 대상이 README 행 1:1 이라 "결선 · 호출 지점" 에 대응하는 실체가 없다. 대체 형식으로 **`pointer N : 대조 좌표 N : 참 N : 부분참 N : 거짓 N`** 을 정의하고 본 절 값은 **26 : 34 : 24 : 2 : 0** 이다 (대조 좌표 = 단일 20 × 1 + 범위 7 × 2). **거짓 0** 은 정본 2 파일의 pointer 정확도가 높다는 실측 결과이며, 어긋남 **2** 는 모두 **범위 표기의 끝 좌표** 에서 나왔다 (단일 좌표 20 지점은 어긋남 0). 가설 정정은 **5** 축 — 축 ⑤ (117–128 을 부분참 후보로 봤으나 참) · requirements 지점 수 (12 → 산문 13 + 표 66) · census 총계 (57/18 → 63 token/11 파일) · CLAUDE 지점 수 (7~8 → 13) · task Required Reading 좌표 2 stale 이고, T-1440 이후 planner 기대가 실측에 정정된 **32 회째** 다.

**AC 3 — pointer 축 vs 산문 · 표 축의 방법론 차이.** (a) **고정비** — pointer 26 지점을 명령 **2 종** (census `grep -rn -oE` batch 1 + 대상 행 `sed -n` batch 1) 으로 전량 처리해 **지점당 0.08 회**, 인용은 pointer 문맥 1 구 + README 실 행 1 구 = **2 구 / 지점**, 판정표 **1 행 / 지점** 이다. `§ 12.67` 산문 축은 실측 명령 **12** 종 / claim 26 = **0.46 회 / claim** 이었으므로 pointer 축 고정비는 **약 1/6**. (b) **판정 가능 비율** — 산문 축의 미판정 **50.0%** (반사실 · 원칙 · 의도 13/26) 대비 본 축은 산문 pointer **26/26 = 100%** 판정이고 미판정 **0** 이다. pointer 는 "주장 좌표 ↔ 실 행" 이 자기 완결적이라 반증 불가 claim 이 **구조적으로 발생하지 않는다** — `§ 12.67` 파생 영향 (1) 의 지목 근거가 실측으로 확인됐다. 단 census 전체 (**126**) 기준 실판정은 **26 = 20.6%** 이고 나머지는 이월이라, pointer 축의 병목은 **판정 가능성이 아니라 절 길이 상한** 이다. (c) **다중 표기** — 위 대체 형식 (`pointer : 대조 좌표 : 참 : 부분참 : 거짓`) 을 pointer 축 정본 지표로 삼는다.

**AC 4 반영 결과.** `docs/requirements.md` **97** 행 뒤 (파일 말미) 에 **빈 줄 1 행 + blockquote 5 행** 을 추가만 했다 (`§ 12.15` append-only). REQ 본문 · REQ ID · 기존 pointer 표기 · 표 컬럼 값은 **무편집** 이고 어긋난 pointer (#16 · #24) 의 in-place 정정은 **0** 이다. `CLAUDE.md` · `README.md` · `reviewer.md` 는 **판정 대상일 뿐 무편집**.

**AC 7 검산.** `wc -l` — `docs/requirements.md` **97 → 103** (+6) · 본 파일 **6276 → 6340** (+64 로 본 절 **64 행 = 상한 100 이내**). `grep -c '^## '` requirements.md **4 불변** · ` ``` ` fence 본 파일 **164 → 164** (짝수) 로 markdown 문법 무손상. `git status --short` 에 **`CLAUDE.md` · `README.md` 부재** 를 확인했고 `git diff --stat` 은 **3 파일** (`docs/requirements.md` · 본 파일 · 본 task 파일) · **≤ 300 LOC** 다. doc-only 라 `pnpm` 실행 **0** ([CLAUDE.md](../../CLAUDE.md) §3.2 direct doc-only 면제).

**진척.** pointer 축 = **정본 2 파일 산문 26/26 완료** (`CLAUDE.md` 13/13 · `docs/requirements.md` 13/13) · 전체 census **126** 지점 중 **26 판정 (20.6%)** · 잔여 **100** (그 밖 9 파일 산문 **34** + `requirements.md` 표 컬럼 **66**). 문서 축으로는 components.md 종료 (`§ 12.67`) 에 이어 **pointer 축의 첫 batch 마감** 이다.

**AC 6 파생 영향 (목록만 — 본 slice 착수 금지).** 잔여 pointer 를 묶는 3 안을 실측으로 비교했다 — (안 A) **파일 밀도 순** (ADR-0002 10 → ADR-0001 6 → deployment 6 → ADR-0003 5 → ADR-0042 4) · (안 B) **문서 종류별** (ADR 군 6 파일 **27** token = 잔여 산문 34 의 **79.4%** / architecture 2 파일 **7** / agent 1 파일 **2**) · (안 C) **README 좌표 구간별** (저장 55 ~ 64 · agent 108 ~ 128). **(1) 다음 slice 1 순위 = 안 B 의 ADR 군 일괄** — 근거 둘: **첫째** 한 batch 로 잔여 산문의 **79.4%** 를 덮으면서 판정표는 **27 행** 이라 절 ≤ 100 행 안에 들어오고, **둘째** ADR 군은 인용 형식이 `[README.md](…) NN 행` 로 균질해 대조 고정비가 최저이며 본 절이 이미 `ADR-0003` **19–22** 에서 부분참 실적을 확보해 적중 근거가 실측으로 있다 / (2) `requirements.md` **표 컬럼 66** 지점 전수 어구 대조 (본 절 이월 — 66 행 판정표라 **33 × 2 분할** 권장) / (3) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (`§ 12.67` FU (2)) / (4) UC-09 `§ 5` sequence participant 병기 (FU12 — **59 회째 이월**) / (5) `CLAUDE.md` §1 pointer 부정확 (FU11 — 본 절이 §3.2 · §3.3 pointer 13/13 참을 확인해 **대상이 §1 로 좁혀졌다**) / (6) 행 번호 → **anchor 좌표계 이행** (FU14 — **53 회째 이월** · 본 절의 표기 **10 변종** + 정규식 포착률 **26.1%** 실측이 근거를 크게 보강) / (7) node · row 외연 정의 (FU20) / (8) 인증 규약 단일 정본 부재 (FU26) / (9) `sync/async` 컬럼 경계 (FU27) / (10) message format 컬럼 판별력 (FU31). **본 stream 밖 (승계만)** — **pointer 정정 batch** 를 신설 후보로 남기며 현재 항목은 **2** 건 (#16 `136~139` → `136~140` · #24 `19~22` → `20`) 이고, 표 · 산문 편집 batch 는 `§ 12.67` 목록 그대로다.

**한계.**

1. **표 컬럼형 66 지점은 어구 대조를 하지 않았다** — 범위 (1 ~ 151) · 엄격 증가 · 표기 형식 3 종 구조 검사와 spot-check **6** 건까지만이며, 나머지 60 은 **미판정으로 남긴 것이지 참으로 간주한 것이 아니다**.
2. **census 정규식은 92 중 24 (26.1%) 만 포착했다** — 정본 2 파일은 수동 전수 보정했으나 **그 밖 9 파일의 distinct 34 는 같은 미포착 편향** (연속 · 나열 토큰) 이 남은 추정치다.
3. **"어구가 대응한다" 의 임계는 명시 규칙이 아니다** — 본 절은 인용 문자열이 대상 행에 실재하는지를 기준으로 삼았고, 의미 등가의 정성 판단은 하지 않았다.
4. **in-place 정정은 0 이다** (`§ 12.15` append-only) — #16 · #24 의 어긋난 숫자는 그대로 두고 pointer 정정 batch 후보로만 이월했다.
5. **stale 2 지점을 정정하지 않았다** — 해당 좌표는 task 정의서 기재값이고 대상 파일 `CLAUDE.md` 는 편집 금지라, Follow-ups 기록까지만 수행했다.

### 12.69 ADR 군 6 파일 README 행 번호 pointer **41** 지점 실판정 — 참 30 · 부분참 11 · 거짓 0 · census 27 → **41** 정정 (정규식 포착률 65.9%) · 어긋남 11 건이 **전부 범위 표기** (단일 23 지점 0) (T-1471)

> **본 절의 위치 · 계보** — `§ 12.68` (T-1470) 이 pointer 축 전수 census 와 정본 2 파일 산문 **26/26** 판정을 마치고 파생 영향 **(1)** 로 지목한 **안 B = ADR 군 일괄** 을 본 절이 그대로 집행한다. 판정 대상은 `ADR-0001` · `0002` · `0003` · `0004` · `0005` · `0042` 6 파일이며 **전부 무편집** 이다 — [CLAUDE.md](../../CLAUDE.md) §3.1 표가 `docs/decisions/*` **추가** 를 `pr` 로만 규정하고 **기존 ADR 본문 append 의 mode 를 규정하지 않아**, 본 doc stream 의 `direct` 연속성을 지키려 판정 결과를 본 절에만 박제했다 (`§ 12.68` 이 `CLAUDE.md` 를 무편집 판정 대상으로만 다룬 선례 승계). pointer in-place 정정 **0** (`§ 12.15` append-only).

**판정 규칙 (본 절 신설 · 명시).** **참** = 단일 좌표가 대응하거나 범위의 **시작 · 끝 두 좌표 모두** 대응. **부분참** = 두 좌표 중 한쪽만 대응하거나, 두 끝이 모두 무관하되 범위 안에 대응 행이 존재. **거짓** = 좌표 · 범위 어디에도 주장 어구가 없음. **포함 관계 독립 규칙** (AC 1 (iv)) — 범위 pointer 와 그 범위 안의 단일 pointer 가 공존하면 **단일의 판정은 범위 판정과 독립** 이다 (범위 안이라는 사실만으로 단일을 참으로 간주하지 않는다). 해당 지점 **17** (`ADR-0002` 8 · `ADR-0001` 6 · `ADR-0005` 2 · `ADR-0003` 1) 이며, `ADR-0002` 의 `55–64` 는 **부분참** 인데 그 안의 단일 6 지점 (56 · 57 · 58 · 59 · 60 · 63) 은 **전부 참** 이라 독립성이 실측으로 드러났다.

**AC 1 실측 (요약형).** `§ 12.66` ~ `§ 12.68` 이 성공시킨 압축 방식 승계 — 명령 + 수치만.

- (i) **census 재실측** — `grep -rnoE '(README\.md\)?|README)[^0-9a-zA-Z가-힣]{0,4}[0-9]{1,3}[ ]?(~|–|-|—|,)?[ ]?[0-9]{0,3}[ ]?(행|번째 줄|줄)' docs/decisions/` → **보정 전 raw 27 token / 6 파일** 로 planner 기대 (**27 / 6 파일**) 와 정확히 일치하고 파일별 (`ADR-0002` 10 · `ADR-0001` 6 · `ADR-0003` 5 · `ADR-0042` 4 · `ADR-0004` · `ADR-0005` 각 1) 도 일치한다. 그러나 `§ 12.68` 이 확정한 포착률 한계대로 **수동 보정에서 14 지점이 추가** 됐다 — `ADR-0001` **4** (`88–93행` · `106–116행` × 2 회 = README 앵커와 숫자 사이에 `§성능 특성 ` 등 한글이 끼어 미포착) · `ADR-0003` **6** (168 행 References 의 **5 좌표 나열** + 74 행의 `L72` **접두 L 표기** = `§ 12.68` 10 변종에 없던 **11 번째 변종**) · `ADR-0005` **2** · `ADR-0042` **2** (한 행 안 2 ~ 3 좌표 나열). → **보정 후 41 지점 / 6 파일**, 파일별 `ADR-0003` **11** · `ADR-0001` **10** · `ADR-0002` **10** · `ADR-0042` **6** · `ADR-0005` **3** · `ADR-0004` **1**. **planner 기대 27 은 raw 에서만 맞고 실판정 단위에서는 반증** 됐고 밀도 1 위도 `ADR-0002` (기대) 가 아니라 `ADR-0003` 이다 (T-1440 이후 **33 회째** 정정). 본 축 포착률 = **27/41 = 65.9%** 로 정본 2 파일 **26.1%** 보다 크게 높아 "ADR 군은 인용 형식이 균질" 하다는 안 B 근거가 확인된다. census 총계는 **126 → 140** (산문 60 → **74**). `wc -l` → README **151** · `ADR-0001` **146** · `ADR-0002` **127** · `ADR-0003` **173** · `ADR-0004` **139** · `ADR-0005` **181** · `ADR-0042` **101** · 본 파일 **6340** (전량 Required Reading 기재값과 일치).
- (ii) **판정 대상 확정** — 41 지점을 **아래 AC 2 판정표의 `#` 컬럼으로 실체화** 했다 (`출처 파일:행` · `주장 좌표` · `인용 어구 요약` 3 요소를 표가 그대로 담아 중복 나열 회피 — 절 ≤ 100 행 보호). 판정 단위는 **`파일:행` × 주장 좌표** 이며 `§ 12.68` 이 `requirements.md:20` 을 3 좌표 3 행으로 실체화한 선례를 승계한다. 나열 토큰을 1 지점으로 세는 (vi) 의 stale 계수 규칙을 적용하면 같은 집합이 **33** 으로 계수되므로 두 수치를 병기한다.
- (iii) **대상 행 대조** — `sed -n '<N>p' README.md` + `cat -n README.md` 1 회 batch. 단일 **23** × 1 + 범위 **18** × 2 = **대조 좌표 59** (planner 기대 **35 반증** — 범위 지점이 기대 8 → 실측 18 로 2.25 배). 끝 · 시작 좌표가 본문이 아닌 경우 **3** 건 (`88–93` 의 **93 = 빈 줄** · `55–64` 의 **55 = 빈 줄** · `19–22` 의 **19 · 22 = 둘 다 빈 줄**), 두 끝이 모두 무관 본문인 경우 **2** 건 (`33–41` · `71–74`).
- (iv) **포함 관계 규칙** — 위 **판정 규칙** 단락에 명시하고 그대로 적용했다 (**17** 지점, 독립 판정).
- (v) **교차 검증 승계** — `19~22` 의 이중 의미를 **`ADR-0003:22` 자기 주장 (권한 부족 통지)** 측 의미로 재판정한 결과도 **부분참** 이다 (대응 행은 **20 단독**, 19 · 22 는 빈 줄, 21 은 중복 제거라 무관). `§ 12.68` #24 (`requirements.md:39` 경유) 와 **판정이 일치** 하므로 두 판정은 **상충하지 않고 동시 성립** 한다 — `§ 12.68` (iv) 의 "의미 상충 동시 참 불가 0" 이 본 축에서도 유지된다. 2+ 파일이 공유하는 README 좌표는 **10** (`50` · `56` · `57` · `72` · `73` · `92` · `108` · `116` · `88–92` · `117–128`; `19~22` 를 포함하면 11).
- (vi) **stale · 삽입 파급 계수** — 계수 규칙은 `§ 12.55` ~ `§ 12.68` 을 **그대로 승계** (자기 좌표만 · 범위 · 나열 토큰은 1 지점 · 외부 파일 좌표 제외). 본 절이 인용 · 의존한 좌표 — README **151** · 6 ADR **146 / 127 / 173 / 139 / 181 / 101** · 본 파일 **6340** · `## 11. References` **6327** · `§ 12.15` **1002** · `docs/PLAN.md` **175** — 는 **전량 일치해 stale 0 지점** 이다 (`§ 12.68` 의 stale 2 로 끊겼던 연속 0 기록이 본 절에서 **복귀**). **삽입 파급 0 지점** — 본 절은 `## 11. References` **직전** 삽입이라 밀리는 구간에 본 절의 자기 좌표 인용이 없다. `deploy/README.md` **227 행** (`deployment.md:62` · `:63`) 은 **외부 파일 좌표라 census · 판정 모두에서 제외**.

**AC 2 판정표 (41 지점 전수 — 참도 생략하지 않는다).**

| # | 출처 `파일:행` | 주장 좌표 | 인용 어구 요약 | 실 README 행 요약 | 판정 | 근거 (1 구) |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `ADR-0001:20` | 88–93 | §성능 특성 — 100–200 명 / repo 50–100 / 1 h | 88 heading / **93 = 빈 줄** (본문 끝은 92) | **부분참** | 시작은 참이나 끝 좌표가 절 밖 빈 줄 |
| 2 | `ADR-0001:21` | 106–116 | §구현 과정에 대한 제약 — 라이브러리 / 테스트 / CI / review | 106 heading / 116 이중 합의 | **참** | 열거 4 요소가 108 · 112 · 113 · 115 에 실재 |
| 3 | `ADR-0001:33` | 112 | "기능 + 예외 + flow 대부분 커버 unit test" | 112 동어 | **참** | 인용 문자열이 그대로 실재 |
| 4 | `ADR-0001:35` | 113 | smoke / e2e 도 CI 에서 수행 | 113 동어 | **참** | 3 종 열거가 일치 |
| 5 | `ADR-0001:51` | 108 | "이미 import 한 library … 다시 더 import 하지 않도록" | 108 동어 | **참** | 인용 문자열 실재 |
| 6 | `ADR-0001:59` | 113 | supertest 로 smoke / e2e 충족 | 113 동어 | **참** | 4 와 동일 좌표 · 동일 의미 |
| 7 | `ADR-0001:75` | 108 | pnpm strict 가 108 행을 자동 검증 | 108 동어 | **참** | 중복 import 금지 요구가 실재 |
| 8 | `ADR-0001:102` | 108 | npm 은 108 행 enforce 메커니즘 없음 | 108 동어 | **참** | 5 · 7 과 동일 좌표 |
| 9 | `ADR-0001:138` | 88–93 | References — I/O 다발 워크로드 특성 | 1 과 동상 | **부분참** | 1 과 동일 좌표 · 동일 사유 |
| 10 | `ADR-0001:139` | 106–116 | References — 라이브러리 / 테스트 / CI 요구 | 2 와 동상 | **참** | 2 와 동일 좌표 |
| 11 | `ADR-0002:27` | 56 | REQ-029 non-volatile 저장 | 56 동어 | **참** | "Non-volatile하게 … 저장" 실재 |
| 12 | `ADR-0002:28` | 58 | REQ-031 재수집 부분 중복 방지 + 최근 1 주 | 58 동어 | **참** | 두 요소가 같은 행에 실재 |
| 13 | `ADR-0002:29` | 59 | REQ-032 raw data 저장 금지 | 59 동어 | **참** | 금지 문장이 그대로 |
| 14 | `ADR-0002:30` | 60 | REQ-033 건별 기여도 · 난이도 · 양 | 60 동어 | **참** | 3 항 열거가 일치 |
| 15 | `ADR-0002:31` | 63 | REQ-036 상대 비교 + LLM 정성 + Metric | 63 동어 | **참** | 3 요소 모두 실재 |
| 16 | `ADR-0002:32` | 91 | REQ-047 100~200 명 × 50~100 repo / 1 h | 91 동어 | **참** | 수치까지 일치 |
| 17 | `ADR-0002:33` | 92 | REQ-048 조회 3 초 이내 | 92 동어 | **참** | 괄호 안 문자열까지 일치 |
| 18 | `ADR-0002:51` | 57 | export / restore backup | 57 동어 | **참** | export · backup · restore 3 어가 실재 |
| 19 | `ADR-0002:119` | 55–64 | References — 저장 정책 / non-volatile / raw 금지 | **55 = 빈 줄** (heading 은 54) / 64 마지막 bullet | **부분참** | 끝은 참이나 시작 좌표가 빈 줄 |
| 20 | `ADR-0002:120` | 88–92 | References — 성능 NFR (1 h + 3 초) | 88 heading / 92 3 초 조회 | **참** | 두 좌표 모두 성능 절 |
| 21 | `ADR-0003:20` | 7–17 | REQ-005/006/007 3 GitHub instance | **7 = 소개 본문 (무관)** / 17 = github.sec 행 | **부분참** | 끝만 대응 · REQ-007 대응 **18 이 범위 밖** (정확 범위 15–18) |
| 22 | `ADR-0003:21` | 33–41 | REQ-016 Confluence / 지정 SPACE | **33 = 권한 주의 (무관)** / **41 = Update 횟수 (무관)** | **부분참** | 두 끝 모두 무관 · 대응은 범위 안 34 뿐이고 정본 31 은 범위 밖 |
| 23 | `ADR-0003:22` | 19–22 | REQ-020 권한 부족 통지 | **19 · 22 = 둘 다 빈 줄** / 20 권한 부족 · 21 중복 (무관) | **부분참** | 대응은 20 단독 · `§ 12.68` #24 와 동일 판정 |
| 24 | `ADR-0003:23` | 71–74 | REQ-039/040 cron 주기 / manual trigger | **71 = 지표 변화 (무관)** / **74 = 최근 구간 삭제 (무관)** | **부분참** | 대응은 범위 안 72 · 73 이고 두 끝은 모두 무관 |
| 25 | `ADR-0003:24` | 88–92 | REQ-047 NFR | 88 heading / 92 3 초 | **참** | 20 과 동일 좌표 · 동일 의미 |
| 26 | `ADR-0003:74` | L72 | default cron 시각 KST 02:00 예시 | 72 "(예: 매일 KST 새벽 2시)" | **참** | 예시 문자열까지 일치 (`L` 접두 = 11 번째 변종) |
| 27 | `ADR-0003:168` | 7–17 | References 나열 — GitHub | 21 과 동상 | **부분참** | 21 과 동일 좌표 · 동일 사유 |
| 28 | `ADR-0003:168` | 19–22 | References 나열 — 권한 | 23 과 동상 | **부분참** | 23 과 동일 |
| 29 | `ADR-0003:168` | 33–41 | References 나열 — Confluence | 22 와 동상 | **부분참** | 22 와 동일 |
| 30 | `ADR-0003:168` | 71–74 | References 나열 — schedule | 24 와 동상 | **부분참** | 24 와 동일 |
| 31 | `ADR-0003:168` | 88–92 | References 나열 — NFR | 25 와 동상 | **참** | 25 와 동일 |
| 32 | `ADR-0004:24` | 56 | REQ-029 non-volatile 저장 | 56 동어 | **참** | 11 과 동일 좌표 (2 파일 공유) |
| 33 | `ADR-0005:166` | 110–128 | R-110~R-114 test/CI 규칙 | 110 검토 + test / 128 마지막 check | **참** | 시작 · 끝 모두 본문이며 R-110 ~ 8 check 를 온전히 포함 |
| 34 | `ADR-0005:166` | 116 | reviewer / committer 이중 합의 | 116 동어 | **참** | 두 agent 명칭이 그대로 |
| 35 | `ADR-0005:166` | 117–128 | 8 check | 117 도입문 / 128 마지막 check | **참** | `§ 12.68` #11 · #13 과 동일 좌표 · 동일 의미 |
| 36 | `ADR-0042:16` | 72 | REQ-039 cron 주기 런타임 지정 | 72 동어 | **참** | 예시 KST 02:00 까지 일치 |
| 37 | `ADR-0042:17` | 73 | REQ-040 manual trigger | 73 동어 | **참** | "Manual하게 Trigger" 가 실재 |
| 38 | `ADR-0042:18` | 50 | REQ-027 신규 인원 1 년치 backfill | 50 동어 | **참** | 1 주 / 1 년 대비까지 일치 |
| 39 | `ADR-0042:95` | 50 | References 나열 (R-50) | 38 과 동상 | **참** | 38 과 동일 좌표 |
| 40 | `ADR-0042:95` | 72 | References 나열 (R-72) | 36 과 동상 | **참** | 36 과 동일 |
| 41 | `ADR-0042:95` | 73 | References 나열 (R-73) | 37 과 동상 | **참** | 37 과 동일 |

**다중 표기 수치.** `§ 12.68` 이 정의한 pointer 축 정본 지표 **`pointer N : 대조 좌표 N : 참 N : 부분참 N : 거짓 N`** 로 본 절은 **41 : 59 : 30 : 11 : 0** 이다 (`§ 12.68` = **26 : 34 : 24 : 2 : 0**). **거짓 0** 이 두 batch 연속으로 유지돼, README pointer 는 "완전히 틀린 좌표" 가 아니라 **범위 끝 · 시작이 절 경계를 넘거나 못 미치는 형태로만 어긋난다** 는 것이 67 지점 표본에서 확인된다.

**AC 3 — 정본 2 파일 축과의 비교.** (a) **범위 비중 ↔ 부분참 상관** — ADR 군 범위 비중 **18/41 = 43.9%** (정본 **7/26 = 26.9%**), 부분참 산출률 **11/41 = 26.8%** (정본 **2/26 = 7.7%**) 로 축 ③ 가설 **확인** 이다 (비중 1.63 배 ↔ 부분참율 3.5 배). 더 강한 사실 — 어긋남 **11 건이 전부 범위 표기** 에서 나왔고 **단일 23 지점의 어긋남은 0** 으로, `§ 12.68` (단일 20 지점 어긋남 0) 과 **패턴이 완전히 같다**. 범위만 놓고 본 부분참율은 **11/18 = 61.1%** vs 정본 **2/7 = 28.6%**. (b) **문서 종류별 차이** — ADR (결정 시점 고정) 참율 **73.2%** 가 운영 정본 (지속 갱신) **92.3%** 보다 낮지만, 원인은 갱신 지연이 아니라 **표기 습관** 이다: ADR 은 REQ 를 절 단위 **범위** 로 인용하고 정본 2 파일은 **단일 행** 인용을 선호하며, 단일 좌표만 비교하면 양측 모두 어긋남 **0** 으로 동률이다. 즉 정확도의 실제 설명 변수는 **문서 종류가 아니라 표기 형식** 이다. (c) **고정비** — 명령 **7 종** / 41 지점 = **0.17 회 / 지점** 으로 `§ 12.68` **0.08** 의 2.1 배이며, 증가분은 전량 **수동 보정** (미포착 14 지점 탐색에 명령 3 종 추가) 이다. 인용 **2 구 / 지점** · 판정표 **1 행 / 지점** 은 동일하다.

**AC 4 반영 결과.** 본 절 **신설만** 수행했다. `docs/decisions/` 6 ADR · `README.md` · `CLAUDE.md` · `docs/requirements.md` 는 **판정 대상일 뿐 전부 무편집** 이고, 어긋난 pointer 11 건의 in-place 정정은 **0** 이다 (`§ 12.15` append-only).

**AC 6 검산.** `wc -l docs/use-cases/REQ-COVERAGE-AUDIT.md` **6340 → 6421** (+81 = 본 절 **81 행**, 상한 100 이내). ` ``` ` fence **164 → 164** (짝수) 로 markdown 문법 무손상. `git status --short` 에 **`docs/decisions/` · `README.md` · `CLAUDE.md` · `docs/requirements.md` 부재** 를 확인했고 `git diff --stat` 은 **2 파일** (본 파일 · 본 task 파일) · **≤ 300 LOC** 다. doc-only 라 `pnpm` 실행 **0** ([CLAUDE.md](../../CLAUDE.md) §3.2 direct doc-only 면제).

**진척.** pointer 축 = 정본 2 파일 **26** + ADR 군 **41** = **67 판정** / 정정 census **140** = **47.9%**, 산문 기준 **74 중 67 = 90.5%** 로 **산문 축이 사실상 마감** 됐다. 잔여 **73** = 그 밖 3 파일 산문 **7** + `requirements.md` 표 컬럼 **66**.

**한계.**

1. **"어긋남" 의 임계는 여전히 명시 규칙이 아니다** — 본 절이 참 / 부분참 / 거짓 3 분류 규칙을 처음 명문화했으나, `33–41` (#22) 처럼 두 끝이 모두 무관하고 범위 안 1 행만 대응하는 경우를 부분참으로 둘지 거짓으로 둘지는 **경계 판단** 이 남는다.
2. **수동 보정은 전수 보장이 아니다** — 포착률 65.9% 를 `grep -n README` 파일별 재조회로 메웠으나, README 를 **명시하지 않고 숫자만 쓴** 잠재 pointer 는 여전히 미탐지일 수 있다.
3. **in-place 정정 0** — 11 건의 어긋난 범위 좌표를 그대로 뒀고 **pointer 정정 batch** 후보로만 이월했다 (현재 누적 **13** 건 = `§ 12.68` 2 + 본 절 11).
4. **ADR 내용 자체의 사실 검증은 하지 않았다** — 본 절은 **pointer ↔ README 행** 대조 축만이며 ADR 결정 · 근거가 코드와 맞는지는 별개 축이다.
5. **census 총계 140 도 추정치다** — 표 컬럼 **66** 은 `§ 12.68` 이 구조 검사만 했고 본 절도 재계수하지 않았다.

**AC 5 파생 영향 (목록만 — 본 slice 착수 금지).** 잔여 후보를 실측 계수로 비교했다 — (A) `requirements.md` **표 컬럼 66** · (B) `deployment.md` + `directory.md` 산문 raw **7 token → distinct 5** (`deploy/README.md` 227 은 외부 좌표라 제외) · (C) `.claude/agents/reviewer.md` **2**. **(1) 다음 slice 1 순위 = (A) 표 컬럼 전반 33** — 근거 둘: 잔여 **73** 의 **90.4%** 를 차지해 pointer 축 마감의 유일한 병목이고, B · C 는 합쳐 **7** 지점뿐이라 단독 slice 로는 과소여서 **(A) 후반 33 batch 에 합류시키면 잔여가 2 slice 로 소진** 된다 / (2) `deployment.md` + `directory.md` + `reviewer.md` 산문 **7** (위 합류안) / (3) `deploy/README.md` ↔ deployment.md ↔ runbook 3 자 정합 (`§ 12.67` FU (2)) / (4) UC-09 `§ 5` sequence participant 병기 (FU12 — **60 회째 이월**) / (5) `CLAUDE.md` §1 pointer 부정확 (FU11) / (6) 행 번호 → **anchor 좌표계 이행** (FU14 — **54 회째 이월** · 본 절의 **11 번째 표기 변종** (`L72`) 발견과 범위 표기 61.1% 부분참율이 근거를 재차 보강) / (7) node · row 외연 정의 (FU20) / (8) 인증 규약 단일 정본 부재 (FU26) / (9) `sync/async` 컬럼 경계 (FU27) / (10) message format 컬럼 판별력 (FU31). **본 stream 밖 (승계만)** — **pointer 정정 batch** 후보 항목이 **13** 건으로 늘었고 (`§ 12.68` 2 + 본 절 11), **ADR 각주 append 의 commit mode 판정** (§3.1 미규정 구간) 을 신규 후보로 추가한다.

### 12.70 `docs/requirements.md` 표 `README 행` 컬럼 **전반 33** 지점 (REQ-001 ~ REQ-033) 실판정 — 참 33 · 부분참 0 · 거짓 0 (참율 **100%**) · 집단 stale 0 · 단조 역전 0 · 중복 좌표 0 (T-1472)

> **본 절의 위치 · 계보** — `§ 12.69` (T-1471) 이 ADR 군 41 지점을 판정하며 파생 영향 **(1)** 1 순위로 지목한 **안 A = `requirements.md` 표 컬럼 66 의 33 × 2 분할** 중 **전반 33** 을 본 절이 집행한다. 본 축은 앞 두 batch 와 성질이 다르다 — 산문 pointer 가 "README 를 인용하는 서술" 인 반면 표 컬럼은 [requirements.md](../requirements.md) **3 행** ("README 의 모든 지시사항을 추적 가능한 REQ-NNN ID 로 박제") · **7 행** ("단일 source of truth") 이 규정한 **REQ 정의 그 자체의 좌표** 다. `requirements.md` · `README.md` 는 **전부 무편집 판정 대상** 이고 in-place 정정도 **0** 이다 (`§ 12.15` append-only + `§ 12.68` · `§ 12.69` 선례 승계).

**판정 규칙.** `§ 12.69` 를 **그대로 승계** — **참** = 단일 좌표가 대응하거나 범위의 **시작 · 끝 두 좌표 모두** 대응, **부분참** = 두 좌표 중 한쪽만 대응하거나 두 끝이 모두 무관하되 범위 안에 대응 행 존재, **거짓** = 좌표 · 범위 어디에도 주장 어구가 없음. 본 축의 대응 판정 기준은 **`요약` 컬럼 어구 ↔ 실 README 행 내용** 이다.

**AC 1 실측 (요약형).** `§ 12.66` ~ `§ 12.69` 압축 방식 승계 — 명령 + 수치만.

- (i) **census 재실측** — `grep -nE '^\| REQ-' docs/requirements.md | wc -l` → **66 row** (첫 `20:| REQ-001` · 끝 `85:| REQ-066`), `awk -F'|' 'NR>=20 && NR<=52 && /^\| REQ-/ {print NR"\t"$2"\t"$3"\t"$4}' docs/requirements.md` → **33 row / 파일 20 ~ 52 행** 에서 `REQ` · `README 행` · `요약` 3 컬럼이 그대로 추출된다. planner 기대 (**33 지점 / 20 ~ 52 행 / 표 전체 66**) 와 **전 축 일치** — T-1440 이후 33 회 이어지던 census 정정이 본 절에서 처음 끊겼다 (표는 기계 추출 가능한 고정 격자라 산문 census 의 정규식 포착률 문제가 원리적으로 없다). 다만 task 파일 Why 가 인용한 `requirements.md` **5 행** ("모든 지시사항을 REQ-NNN 으로 박제") 은 실측 **3 행** 이고 5 행은 `## 운영 룰` heading 이다 (지점 1 stale — 판정 대상 밖의 부수 발견). `wc -l` → README **151** · `requirements.md` **103** · 본 파일 **6421**.
- (ii) **주장 좌표 형태 분류** — 단일 **32** + 범위 **1** (REQ-023 = `45-46`) + 나열 **0** → **대조 좌표 34** 로 planner 기대 (32 + 1 → 34) 와 일치. 빈 값 · 비숫자 값 **0** (33/33 이 정수 또는 `N-M` 형식). 범위 비중 **1/33 = 3.0%** 는 3 batch 중 최저다 (ADR 군 43.9% · 정본 산문 26.9%).
- (iii) **대상 행 대조** — `sed -n '1,60p' README.md | cat -n` 1 회 batch 로 확보. 34 좌표 전량이 README **1 ~ 60** 안에 있고 아래 판정표대로 **참 33 · 부분참 0 · 거짓 0**. 유일 범위 `45-46` 은 45 (서비스별 상이 ID) · 46 (그 예시) 두 좌표가 모두 `요약` 어구에 대응해 **참** 이다.
- (iv) **집단 stale · 단조성 · 중복 · 커버리지** — (a) 어긋남이 **0** 이라 offset 분포는 **offset 0 × 34** 이며 집단 밀림 · 개별 오류가 **둘 다 부재** 다 (축 ③ 의 "T-0013 일괄 생성 이후 미검증 → 집단 stale" 가설은 **반증**). (b) REQ 번호 ↔ 주장 좌표는 `1 · 6 · 7 · 9 · 16 … 58 · 59 · 60` 으로 **엄격 단조 증가**, 역전 **0**. (c) 2+ REQ 가 같은 좌표를 주장하는 **중복 0** (33 값 전부 distinct). (d) README 1 ~ 60 중 **미커버 26 행** = 빈 줄 **17** + heading **8** (4 · 11 · 15 · 23 · 29 · 36 · 44 · 54) + 비지시 서술 **1** (13 행 "AA의 평가 대상은 크게 코드 작성과 문서 작성으로 나누어 볼 수 있다" = 분류 서술) → **지시 문장인데 REQ 가 없는 행 = 0**, 전반 구간의 누락 후보는 없다.
- (v) **stale · 삽입 파급 · 축 분리** — 계수 규칙은 `§ 12.55` ~ `§ 12.69` 승계 (자기 좌표만 · 범위 · 나열 토큰은 1 지점 · 외부 파일 좌표 제외). 인용 · 의존 좌표 — README **151** · `requirements.md` **103** · 본 파일 **6421** · `## 11. References` **6408** · `§ 12.15` **1002** · `§ 12.69` **6327** · `PLAN.md` **175** — 는 **전량 일치, stale 0 지점** (`§ 12.69` 에 이어 **2 연속 0**). **삽입 파급 0 지점** — `## 11. References` 직전 삽입이라 밀리는 구간에 본 절 자기 좌표 인용이 없다. 축 ⑦ **분리 검증** — `§ 12.68` 이 판정한 `requirements.md` 산문 13 지점은 전부 **`상태` 컬럼** 출처라 pointer **인스턴스 중복 0** 이나, **좌표값 단위로는 7 건이 겹친다** (`:23`→9 · `:28`→21 · `:30`→25 · `:33`→30 · `:34`→31 · `:36`→34 · `:37`→37 이 같은 row 의 `README 행` 컬럼과 동값). 양측 판정이 **7/7 모두 참** 으로 일치해 상충 0 — "집합 분리" 기대는 **인스턴스 기준으로만** 성립한다 (정밀화).

**AC 2 판정표 (33 지점 전수 — 참도 생략하지 않는다).**

| # | REQ | 파일 행 | 주장 좌표 | `요약` 컬럼 어구 | 실 README 행 요약 | 판정 | 근거 (1 구) |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | REQ-001 | 20 | 1 | 본 문서는 Use Case 문서의 기본 | 1 "Use Case 문서의 기본이 되는 Description 역할" | **참** | 어구가 그대로 실재 |
| 2 | REQ-002 | 21 | 6 | Web Interface 제공 Agent System | 6 동어 | **참** | 문자열 완전 일치 |
| 3 | REQ-003 | 22 | 7 | 기여 양 · 질 평가 / 저장 / 표시 | 7 "양과 질을 평가하고 … 저장하고 보여주는" | **참** | 3 요소 열거가 일치 |
| 4 | REQ-004 | 23 | 9 | 수치 지표 + LLM 코멘트 (지정 기간) | 9 "다양한 지표 … 사용자가 지정한 기간 … LLM" | **참** | 3 요소 모두 실재 |
| 5 | REQ-005 | 24 | 16 | github.com 평가 | 16 Github.com 지정 Org / Repo commit | **참** | 도메인 · 대상이 일치 |
| 6 | REQ-006 | 25 | 17 | github.sec.samsung.net | 17 동형 | **참** | 5 와 동일 구조 |
| 7 | REQ-007 | 26 | 18 | github.ecodesamsung.com | 18 동형 | **참** | 5 와 동일 구조 |
| 8 | REQ-008 | 27 | 20 | 접근 권한(read) 부족 인식 · 통지 | 20 "접근 권한 (read) … 인식하여 대응" | **참** | 코드 절 주의사항 좌표가 정확 |
| 9 | REQ-009 | 28 | 21 | Fork/Rebase/Meld 중복 + 시간적 중복 | 21 세 어휘 + 2 월 / 3 월 예시 | **참** | 두 요소가 같은 행에 실재 |
| 10 | REQ-010 | 29 | 24 | 코드 기여 양 · 질 평가 | 24 동어 | **참** | bullet 문이 일치 |
| 11 | REQ-011 | 30 | 25 | 중요 · 어려운 기여 → 높은 점수 | 25 "어렵고 남들이 못할 일 … 더 높은 점수" | **참** | 인용구가 실재 |
| 12 | REQ-012 | 31 | 26 | commit/PR 숫자 abusing 방지 | 26 동어 | **참** | 평가 Metric 어휘까지 일치 |
| 13 | REQ-013 | 32 | 27 | 저성과자 식별 | 27 "기여가 현격히 떨어지는 저성과자" | **참** | 어구가 실재 |
| 14 | REQ-014 | 33 | 30 | Issue 평가 (본인 follow-up 제외) | 30 동어 | **참** | 단서절까지 일치 |
| 15 | REQ-015 | 34 | 31 | Confluence 지정 SPACE 평가 | 31 "지정된 주소의 Confluence … SPACE" | **참** | 작성 / 업데이트 활동까지 일치 |
| 16 | REQ-016 | 35 | 33 | Confluence 권한 부족 인식 · 통지 | 33 문서 절 주의사항 (20 과 동문) | **참** | 동문 2 곳 중 **문서 절** 좌표를 정확히 지목 |
| 17 | REQ-017 | 36 | 34 | crawling vs hierarchy 탐색 정책 | 34 동어 | **참** | 허용 3 항 열거가 일치 |
| 18 | REQ-018 | 37 | 37 | 단순 보고 · copy-paste = zero-contribution | 37 동어 | **참** | 두 어구 모두 실재 |
| 19 | REQ-019 | 38 | 38 | 새 알고리즘 · 외부 연구 소개 = 높은 contribution | 38 동어 | **참** | 열거 3 항이 일치 |
| 20 | REQ-020 | 39 | 39 | 조직 기여 큰 인원 → 높은 점수 | 39 동어 | **참** | 어구가 실재 |
| 21 | REQ-021 | 40 | 40 | 문서 abusing 방지 | 40 "(의미 없는 기여의 단순 반복)" | **참** | 괄호 부연까지 일치 |
| 22 | REQ-022 | 41 | 41 | update 횟수 중립화 | 41 "advantage/disadvantage 둘 다 있어서는 안된다" | **참** | 문자열이 일치 |
| 23 | REQ-023 | 42 | 45-46 | 서비스별 ID 매핑 (1 인물 ↔ N ID) | 45 "각 서비스 별로 … 다른 ID" / 46 myungjoo 예시 | **참** | 시작 · 끝 두 좌표 모두 대응 (범위 폭 2) |
| 24 | REQ-024 | 43 | 47 | primary key 역할 ID 지정 | 47 동어 | **참** | 따옴표 표기까지 일치 |
| 25 | REQ-025 | 44 | 48 | 일부 서비스 ID NULL 허용 | 48 동어 | **참** | confluence 전원 보유 단서 포함 |
| 26 | REQ-026 | 45 | 49 | 인원 CRUD + Deactivate/Activate | 49 동어 | **참** | 휴직 예시까지 일치 |
| 27 | REQ-027 | 46 | 50 | 신규 인원 1 년치 평가 1 회 | 50 동어 | **참** | 1 주 대비 수치가 일치 |
| 28 | REQ-028 | 47 | 51 | group 정책 (다중 임의 + 단일 파트) | 51 동어 | **참** | 두 제약 모두 실재 |
| 29 | REQ-029 | 48 | 56 | 평가 자료 non-volatile 저장 | 56 동어 | **참** | `§ 12.69` #11 · #32 와 동일 좌표 · 동일 판정 |
| 30 | REQ-030 | 49 | 57 | Export/backup + Restore | 57 동어 | **참** | `§ 12.69` #18 과 동일 좌표 |
| 31 | REQ-031 | 50 | 58 | 재수집 중복 방지 + 최근 1 주 | 58 동어 | **참** | `§ 12.69` #12 와 동일 |
| 32 | REQ-032 | 51 | 59 | raw data 저장 금지 | 59 동어 | **참** | `§ 12.69` #13 과 동일 |
| 33 | REQ-033 | 52 | 60 | 건별 기여도 · 난이도 · 양 | 60 동어 | **참** | `§ 12.69` #14 와 동일 |

**다중 표기 수치.** `§ 12.68` 정의 지표 **`pointer N : 대조 좌표 N : 참 N : 부분참 N : 거짓 N`** 로 본 절은 **33 : 34 : 33 : 0 : 0** 이다 (`§ 12.68` = **26 : 34 : 24 : 2 : 0**, `§ 12.69` = **41 : 59 : 30 : 11 : 0**). **거짓 0 이 3 batch 연속** 으로 유지돼, README pointer 는 100 지점 표본에서 "완전히 틀린 좌표" 를 한 건도 내지 않았다. 위 표 #29 ~ #33 의 5 좌표 (56 ~ 60) 는 ADR 군 판정과 **동일 좌표 · 동일 참 판정** 이라 두 축의 **교차 검증 일치 5/5**, 상충 0 이다.

**AC 3 — 산문 축과의 비교.** (a) **표 컬럼이 산문보다 정확한가 = 예** — 참율 **33/33 = 100%** > 정본 2 파일 산문 **24/26 = 92.3%** > ADR 군 산문 **30/41 = 73.2%**. 표 컬럼은 "단일 source of truth" 라는 자기 규정에 걸맞게 **drift 0** 이었고, 축 ③ 이 우려한 집단 stale 은 발생하지 않았다. (b) **`§ 12.69` AC 3 (b) 명제 ("설명 변수는 문서 종류가 아니라 표기 형식") 확인** — 범위 비중 (43.9% → 26.9% → **3.0%**) 과 부분참율 (26.8% → 7.7% → **0%**) 이 3 점에서 **단조 동행** 하고, 3 batch 통틀어 **단일 좌표 78 지점의 어긋남은 0** 이다. 다만 정밀화 1 건 — 본 축의 유일 범위 `45-46` 도 **참** 인데, 폭 2 행의 인접 범위라 절 경계를 넘을 여지가 없다. 즉 부분참을 낳는 것은 범위 표기 자체가 아니라 **절 단위 광역 범위** (ADR 군 범위 평균 폭 약 9 행) 다. (c) **고정비** — 명령 **6 종** / 33 지점 = **0.18 회 / 지점** 으로 `§ 12.69` **0.17** 과 사실상 동률이고 `§ 12.68` **0.08** 의 2.3 배다. 다만 증가분의 성격이 다르다 — `§ 12.69` 는 census 수동 보정이었으나 본 절은 census 가 1 회 `awk` 로 끝났고 증가분이 전량 **축 (iv) 4 종 부가 검사** (offset · 단조 · 중복 · 커버리지) 다. 인용 **2 구 / 지점** · 판정표 **1 행 / 지점** 은 3 batch 동일하다.

**AC 4 반영 결과.** 본 절 **신설만** 수행했다. `docs/requirements.md` · `README.md` · `CLAUDE.md` · `docs/decisions/` 는 **판정 대상일 뿐 전부 무편집** 이고 in-place 정정은 **0** 이다 (`§ 12.15` append-only).

**AC 6 검산.** `wc -l docs/use-cases/REQ-COVERAGE-AUDIT.md` **6421 → 6493** (+72 = 본 절 **72 행**, 상한 100 이내). ` ``` ` fence **164 → 164** (짝수) · 신설 표 컬럼 **8** 개로 markdown 문법 무손상. `git status --short` 에 **`docs/requirements.md` · `README.md` · `CLAUDE.md` · `docs/decisions/` 부재** 를 확인했고 `git diff --stat` 은 **2 파일** (본 파일 · 본 task 파일) · **≤ 300 LOC** 다. doc-only 라 `pnpm` 실행 **0** ([CLAUDE.md](../../CLAUDE.md) §3.2 direct doc-only 면제).

**진척.** pointer 축 = 정본 2 파일 **26** + ADR 군 **41** + 표 컬럼 전반 **33** = **100 판정** / 정정 census **140** = **71.4%**. 표 컬럼은 **66 중 33 = 50%**, 산문은 **74 중 67 = 90.5%** (변동 없음). 잔여 **40** = 후반 33 + 산문 7.

**한계.**

1. **후반 33 (REQ-034 ~ REQ-066) 은 미판정** — 참율 100% 를 표 전체로 일반화할 수 없다. 후반이 겨냥하는 README 61 ~ 151 구간은 절 밀도 · 범위 표기 성향이 다를 수 있다.
2. **대응 임계는 여전히 명시 규칙이 아니다** — `요약` 컬럼은 축약형이라 "대응" 판정에 정성이 남는다 (`§ 12.69` 한계 1 승계). 본 절은 어긋남 0 이라 경계 사례가 실제로 나타나지 않았을 뿐이다.
3. **`상태` 컬럼 산문은 열지 않았다** — 좌표값 7 건 겹침은 `§ 12.68` 판정 기록과의 대조로만 확인했고 본문 재독은 하지 않았다.
4. **커버리지 (iv)(d) 는 행 단위 계수** 라, 한 행 안에 지시가 2+ 개인 경우의 누락은 잡지 못한다.
5. **in-place 정정 0 · 이월 누적 불변** — 본 절은 어긋남이 0 이라 **pointer 정정 batch** 후보가 **13 건** 그대로다.

**AC 5 파생 영향 (목록만 — 본 slice 착수 금지).** **(1) 다음 slice 1 순위 = pointer 축 최종 batch — 후반 33 (REQ-034 ~ REQ-066) + 산문 잔여 7 (`deployment.md` · `directory.md` · `reviewer.md`) 합류 = 40 지점** 이며 `§ 12.69` FU (1) 예고안이 본 절 실측 후에도 **유효** 하다. 검산 — 본 절 비용은 판정표 **1 행 / 지점** + 표 헤더 2 + 표 밖 구조 **37 행** 이므로 40 지점이면 `40 + 2 + 37 = 79 행` + 산문 3 파일 census 명령 1 ~ 2 행 ≈ **81 행** 으로 상한 100 안에 들어온다 (분할 불요). 단 후반 33 에서 범위 · 나열 표기가 늘어 판정표 근거가 길어지면 근거 컬럼을 "n 과 동일 사유" 로 압축한다 / (2) `deploy/README.md` ↔ `deployment.md` ↔ runbook 3 자 정합 (`§ 12.67` FU (2)) / (3) UC-09 `§ 5` sequence participant 병기 (FU12 — **61 회째 이월**) / (4) `CLAUDE.md` §1 pointer 부정확 (FU11) / (5) 행 번호 → **anchor 좌표계 이행** (FU14 — **55 회째 이월** · 본 절이 확인한 "단일 좌표 78 지점 어긋남 0" 은 anchor 이행의 시급성을 **낮추는** 반대 증거이므로 근거 재평가 필요) / (6) node · row 외연 정의 (FU20) / (7) 인증 규약 단일 정본 부재 (FU26) / (8) `sync/async` 컬럼 경계 (FU27) / (9) message format 컬럼 판별력 (FU31) / (10) `requirements.md` **3 행 ↔ task 파일 5 행** 표기 불일치 (본 절 (i) 부수 발견 — task 파일은 append-only 대상 밖이라 정정 batch 후보로만 승계). **본 stream 밖 (승계만)** — **pointer 정정 batch** 후보 **13 건** (증가 0) · **ADR 각주 append 의 commit mode 판정** (§3.1 미규정 구간).

### 12.71 pointer 축 **최종 batch 40** 지점 — 표 컬럼 후반 33 (REQ-034 ~ REQ-066) + 산문 잔여 7 (`deployment.md` · `directory.md` · `reviewer.md`) 실판정 — 참 39 · 부분참 1 · 거짓 0 (참율 **97.5%**) · **pointer 축 140 전량 마감** (참 126 · 부분참 14 · 거짓 0 · 참율 **90.0%**) (T-1473)

> **본 절의 위치 · 계보** — `§ 12.70` (T-1472) 이 파생 영향 **(1)** 1 순위로 지목하고 행당 비용으로 검산 (`40 + 2 + 37 + 산문 census ≈ 81 행`) 한 **합류안** 을 본 절이 그대로 집행한다. 두 성질의 축을 한 절에서 다루므로 판정표를 **blok A (표 컬럼 33)** · **blok B (산문 7)** 로 나누고 다중 표기도 분리 + 합계로 보고한다. `requirements.md` · `README.md` · `deployment.md` · `directory.md` · `.claude/agents/reviewer.md` 는 **전부 무편집 판정 대상** 이고 in-place 정정 · 각주 append 는 **0** 이다 (`§ 12.15` append-only + `§ 12.68` ~ `§ 12.70` 선례 승계).

**판정 규칙.** `§ 12.70` 을 **그대로 승계** — **참** = 단일 좌표가 대응하거나 범위의 **시작 · 끝 두 좌표 모두** 대응, **부분참** = 두 좌표 중 한쪽만 대응하거나 두 끝이 모두 무관하되 범위 안에 대응 행 존재, **거짓** = 좌표 · 범위 어디에도 주장 어구가 없음. 대응 기준은 blok A 가 **`요약` 컬럼 어구 ↔ 실 README 행**, blok B 가 **인용 문장 ↔ 실 README 행** 이다.

**AC 1 실측 (요약형).** `§ 12.66` ~ `§ 12.70` 압축 방식 승계 — 명령 + 수치만.

- (i) **표 축 census 재실측** — `awk -F'|' 'NR>=53 && NR<=85 && /^\| REQ-/ {print NR"\t"$2"\t"$3"\t"$4}' docs/requirements.md` → **33 row / 파일 53 ~ 85 행 / REQ-034 ~ REQ-066** 으로 planner 기대와 **전 축 일치** (표는 고정 격자라 `§ 12.70` 에 이어 census 정정 2 연속 0). **`상태` 컬럼 ($5) 을 출력한 명령은 없다** — 본 slice 의 전 명령이 `$2` · `$3` · `$4` 3 컬럼만 추출했고 20 ~ 52 행 (전반 33) 도 열지 않았다 (context 규율 자기 보고).
- (ii) **산문 축 census 재실측** — `grep -n 'README' docs/architecture/deployment.md docs/architecture/directory.md .claude/agents/reviewer.md` → raw hit **13 행** (deployment 4 · directory 3 · reviewer 6) → README 좌표 토큰 **11** (좌표 없는 언급 3 건 · 외부 파일 좌표 `deploy/README.md` 227 **2 건** 제외) → distinct **(파일, 좌표) 지점 7** (deployment 2 · directory 3 · reviewer 2) 으로 `§ 12.69` AC 5 실측 (**5 + 2 = 7**) 과 일치한다. 병합 근거 — deployment **62 행** 각주의 `57` 재인용 2 건은 48 행과 동일 (파일, 좌표) 이고, reviewer **3 · 7 행** (`117-128`) · **87 · 113 행** (`128`) 도 각각 동일 쌍이다. `wc -l` → README **151** · `requirements.md` **103** · `deployment.md` **232** · `directory.md` **203** · `reviewer.md` **181** · 본 파일 **6493**.
- (iii) **주장 좌표 형태 분류** — blok A = 단일 **31** + 범위 **2** (REQ-038 `68-71` 폭 4 · REQ-065 `117-128` 폭 12) + 나열 0 → 대조 좌표 **35**, 범위 비중 **6.1%** (전반 3.0% 의 2 배) · 평균 폭 **8**. blok B = 단일 **4** + 범위 **3** (`7-22` 폭 16 · `96-103` 폭 8 · `117-128` 폭 12) + 나열 0 → 대조 좌표 **10**, 범위 비중 **42.9%** · 평균 폭 **12**. 합계 단일 **35** + 범위 **5** → **대조 좌표 45**. 빈 값 · 비숫자 값 **0**.
- (iv) **대상 행 대조** — `sed -n '61,151p' README.md | cat -n` 1 회 batch + 61 미만 좌표 (`7-22` · `57`) 및 장문 행 (74 · 84) 만 `awk 'NR==...'` 로 개별 확보. 판정 결과는 아래 두 표대로 **blok A 참 33 · 부분참 0 · 거짓 0**, **blok B 참 6 · 부분참 1 · 거짓 0**.
- (v) **단조성 · 중복 · 커버리지 · 축 간 겹침** — (a) REQ-034 ~ REQ-066 주장 좌표는 `61 · 62 · 63 · 64 · 68 … 117 · 133` 으로 **엄격 단조 증가**, 역전 **0**. **전반 ↔ 후반 경계도 연속** (REQ-033 = 60 → REQ-034 = 61 인접). (b) 중복 **0** (범위 내부 행을 다른 REQ 가 주장하는 사례도 없음). (c) README **61 ~ 151 = 91 행** 중 표 33 pointer 가 커버한 **47 행**, 미커버 **44 행** = 빈 줄 **23** + heading **7** (66 · 76 · 81 · 88 · 94 · 106 · 136) + 비지시 서술 · 구문 **5** (90 성능 미정의 메타 · 98 "후보 LLM은 다음과 같다" 도입 · 129 fence · 138 빌드 환경 전제 · 150 CI 서술) + **지시형이나 REQ 없음 9** (**140 ~ 148**). 후자 9 행은 `# 로컬 빌드 / 테스트` 절의 `pnpm` 명령 안내로 **오너 요구사항이 아니라 개발자 실행 절차** 라 REQ 대상 밖으로 본다 (계수 · 나열까지만, 신설 0). (d) **축 간 겹침** — blok B **7 지점 전부** 가 표 축 좌표와 동값 또는 포함 관계다 (`57` ×2 ↔ 전반 REQ-030 · `72` ↔ REQ-039 · `117-128` ↔ REQ-065 동값, `128` ⊂ REQ-065 범위, `96-103` ⊃ REQ-049 ~ REQ-055, `7-22` ⊃ REQ-005 ~ REQ-007). 판정 **상충 0** — 겹치는 좌표는 양측 모두 참이고, 유일 부분참 (`7-22`) 도 표 축 대응 좌표 (16 · 18) 는 참이라 어긋남이 **산문 측 범위 표기에서만** 발생했다 (`§ 12.70` (v) "인스턴스 분리 · 좌표값 겹침" 패턴 재현 · 강화).
- (vi) **stale · 삽입 파급 계수** — 계수 규칙 `§ 12.55` ~ `§ 12.70` 승계. 인용 · 의존 좌표 — README **151** · `requirements.md` **103** · `deployment.md` **232** · `directory.md` **203** · `reviewer.md` **181** · 본 파일 **6493** · `## 11. References` **6480** · `§ 12.15` **1002** · `§ 12.70` **6408** · `PLAN.md` **175** — 는 **전량 일치, stale 0 지점** (`§ 12.69` · `§ 12.70` 에 이어 **3 연속 0**). **삽입 파급 0 지점** — `## 11. References` 직전 삽입이라 밀리는 구간에 본 절 자기 좌표 인용이 없다.

**AC 2 판정표 blok A — 표 컬럼 후반 33 지점 (전수, 참도 생략하지 않는다).**

| # | REQ | 파일 행 | 주장 좌표 | `요약` 컬럼 어구 | 실 README 행 요약 | 판정 | 근거 (1 구) |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | REQ-034 | 53 | 61 | 일별 활동 요약 (당일 제외) | 61 "종료된 날짜의 활동 … 실행 당일은 자정까지 하지 말자" | **참** | 단서절까지 일치 |
| 2 | REQ-035 | 54 | 62 | 주간/월간 요약 | 62 "주간 (다음주 시작시) / 월간 (다음달 시작시)" | **참** | 괄호 시점까지 일치 |
| 3 | REQ-036 | 55 | 63 | 상대 비교 + LLM 정성 + Metric | 63 세 요소 동어 | **참** | 3 요소 모두 대응 |
| 4 | REQ-037 | 56 | 64 | 미평가 일괄 + Reset & Reeval | 64 동어 | **참** | 두 요소 실재 |
| 5 | REQ-038 | 57 | 68-71 | UI 조회 / sort / filter / 시계열 | 68 "조회하도록 한다" / 71 "시간 흐름에 따라 … 변화" | **참** | 시작 · 끝 두 좌표 대응 (폭 4) |
| 6 | REQ-039 | 58 | 72 | Admin cron 주기 지정 | 72 "(예: 매일 KST 새벽 2시)" | **참** | 예시까지 일치 |
| 7 | REQ-040 | 59 | 73 | Admin manual trigger | 73 동어 | **참** | 문자열 일치 |
| 8 | REQ-041 | 60 | 74 | 최근 N 일 manual delete → 재수집 | 74 "최근 1일 / 7일 / 30일 … 다시 하게 된다" | **참** | 재수집 절까지 일치 |
| 9 | REQ-042 | 61 | 78 | 진행 중 시각화 보호 + 경고 배너 | 78 동어 | **참** | 두 요소 실재 |
| 10 | REQ-043 | 62 | 83 | 전 기능 ID/Password 보호 | 83 동어 | **참** | 문자열 일치 |
| 11 | REQ-044 | 63 | 84 | 첫 로그인 SuperAdmin / 3 등급 / 승급 | 84 "SuperAdmin (첫 로긴) · Admin · User … Admin→User 는 첫 로긴 Admin만" | **참** | 4 요소 모두 실재 |
| 12 | REQ-045 | 64 | 85 | Admin 권한 6 종 | 85 동어 (재작성 · Reset · Import/Export · 인원 · Group) | **참** | 열거가 일치 |
| 13 | REQ-046 | 65 | 86 | User read-only | 86 동어 | **참** | 문자열 일치 |
| 14 | REQ-047 | 66 | 91 | 100~200 명 / 50~100 repo / ~1000 confluence | 91 동어 | **참** | 수치가 일치 |
| 15 | REQ-048 | 67 | 92 | 조회 · 시각화 3 초 이내 | 92 "(로딩 및 시각화에 3초 이내)" | **참** | 괄호 수치 일치 |
| 16 | REQ-049 | 68 | 96 | Admin 이 LLM 모델 지정 | 96 동어 | **참** | 문자열 일치 |
| 17 | REQ-050 | 69 | 97 | 3 난이도 모델 + 난이도 결정 | 97 동어 | **참** | 결정 시점 단서까지 일치 |
| 18 | REQ-051 | 70 | 99 | custom (OpenAI 호환 · 내부 · proxy · 3 슬롯) | 99 동어 | **참** | 4 요소 실재 |
| 19 | REQ-052 | 71 | 100 | Azure OpenAI provider | 100 `azure-openai` | **참** | 후보 열거 행이 정확 |
| 20 | REQ-053 | 72 | 101 | Anthropic provider | 101 `anthropic` | **참** | 19 와 동일 사유 |
| 21 | REQ-054 | 73 | 102 | Google Gemini provider | 102 `google gemini` | **참** | 19 와 동일 사유 |
| 22 | REQ-055 | 74 | 103 | OpenAI provider | 103 `open AI` | **참** | 19 와 동일 사유 |
| 23 | REQ-056 | 75 | 108 | well-known library / 중복 금지 | 108 동어 | **참** | version mismatch 절까지 실재 |
| 24 | REQ-057 | 76 | 109 | 한 commit = 한 주제 | 109 "하나의 coherent한 기능 … 하나의 commit에는 하나의" | **참** | 괄호 단서가 대응 |
| 25 | REQ-058 | 77 | 110 | commit/PR 후 검토 + test 작성 + 수행 | 110 동어 | **참** | 3 요소 실재 |
| 26 | REQ-059 | 78 | 111 | test CI 자동 실행 / fail → CI error | 111 동어 | **참** | 인지 주체까지 일치 |
| 27 | REQ-060 | 79 | 112 | unit test (기능 + 예외 + flow) | 112 동어 | **참** | negative 절까지 실재 |
| 28 | REQ-061 | 80 | 113 | smoke + e2e 도 CI 에서 | 113 동어 | **참** | 문자열 일치 |
| 29 | REQ-062 | 81 | 114 | 활동 후 test + 종료 전 CI | 114 동어 | **참** | 두 시점 모두 실재 |
| 30 | REQ-063 | 82 | 115 | PR 만들면 다른 agent review | 115 동어 | **참** | 문자열 일치 |
| 31 | REQ-064 | 83 | 116 | Reviewer + Committer 합의 / 7 round | 116 동어 | **참** | round 상한까지 일치 |
| 32 | REQ-065 | 84 | 117-128 | Reviewer 8 check | 117 "다음의 항목을 검사하여야" / 128 8 번째 check (PR comment) | **참** | 시작 · 끝 대응 (폭 12, fence blok 경계와 정렬) |
| 33 | REQ-066 | 85 | 133 | 코드 = PR / 진행상황 doc = direct | 133 동어 | **참** | 두 분기 모두 실재 |

**AC 2 판정표 blok B — 산문 잔여 7 지점.**

| # | 파일 | 파일 행 | 주장 좌표 | 인용 어구 | 실 README 행 요약 | 판정 | 근거 (1 구) |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 34 | `deployment.md` | 48 (62 각주 동일) | 57 | "README 57 행 (export / backup / restore)" | 57 "export하여 backup하고 restore하여 reset" | **참** | 3 어휘가 그대로 실재 |
| 35 | `deployment.md` | 149 | L72 | "README.md L72 예시 (KST 02:00)" | 72 "(예: 매일 KST 새벽 2시)" | **참** | 예시값까지 일치 |
| 36 | `directory.md` | 147 | 57 | "README 57 행 (export / restore)" | 57 동어 | **참** | 34 와 동일 좌표 · 사유 |
| 37 | `directory.md` | 197 | L7-22 | "REQ-005~007 GitHub 3 instance" | 7 "각 개발자들의 기여 활동 … 시스템이다" (소개문) / 22 **빈 줄** / 실 대응은 16 ~ 18 | **부분참** | 두 끝 모두 무관하나 대응 행이 범위 안 (폭 **16** — 절 통째 지시) |
| 38 | `directory.md` | 197 | L96-103 | "REQ-049 / REQ-051~055 LLM 5 provider" | 96 "LLM은 Admin이 지정" / 103 `open AI` | **참** | 시작 · 끝 대응 (폭 8, 열거 blok 경계와 정렬) |
| 39 | `reviewer.md` | 3 · 7 | 117-128 | "checklist from README lines 117-128" | 117 검사 항목 도입문 / 128 PR comment check | **참** | blok A #32 와 동일 좌표 · 동일 판정 |
| 40 | `reviewer.md` | 87 · 113 | 128 | "README 128행 … PR에 Comment를 남긴다" | 128 동어 | **참** | 단일 좌표 정확 대응 |

**다중 표기 수치.** `pointer N : 대조 좌표 N : 참 N : 부분참 N : 거짓 N` — **blok A = 33 : 35 : 33 : 0 : 0**, **blok B = 7 : 10 : 6 : 1 : 0**, **합계 = 40 : 45 : 39 : 1 : 0** (참율 **97.5%**).

**AC 3 총결산 · 명제 최종 판정.** (a) **pointer 축 마감** — `§ 12.68` (26 : 34 : 24 : 2 : 0) + `§ 12.69` (41 : 59 : 30 : 11 : 0) + `§ 12.70` (33 : 34 : 33 : 0 : 0) + 본 절 (40 : 45 : 39 : 1 : 0) = **140 : 172 : 126 : 14 : 0**, 참율 **90.0%** 이며 **거짓 0 이 4 batch · 140 지점 연속** 이다. 축별로는 표 컬럼 **66/66 = 100%**, 산문 **60/74 = 81.1%**. 어긋남 **14 건은 전량 범위 표기** 이고 단일 좌표는 **113 지점 (`§ 12.70` 의 78 + 본 절 35) 어긋남 0** 이다. (b) **`§ 12.70` (b) 명제는 정밀화된 형태로 확인** — 같은 `requirements.md` 표 안의 전반 (범위 1, 폭 2) ↔ 후반 (범위 2, 폭 4 · 12) 비교는 "문서 종류" 변수를 통제한 **자연 실험** 인데, 폭이 6 배로 늘어도 부분참은 **양쪽 다 0** 이었다. 결정 변수는 폭 자체가 아니라 **범위 끝 좌표가 의미 단위 경계에 정렬됐는가** 다 — 폭 12 의 `117-128` (fence blok 경계) · 폭 8 의 `96-103` (열거 blok 경계) 은 참, 폭 16 의 `7-22` (절 통째) 만 부분참이다. 즉 "절 단위 광역 범위" 라는 진술은 **폭 지표가 아니라 경계 정렬 지표로 읽어야** 옳다. (c) **고정비** — 측정 명령 **7 종** / 40 지점 = **0.18 회 / 지점** 으로 `§ 12.70` **0.18** · `§ 12.69` **0.17** 과 동률이고 `§ 12.68` **0.08** 의 2.3 배다. 2 축 혼합 batch 의 비용 특성 — census 명령이 축마다 1 개씩 필요해 고정비 분모가 커지지만 대조 batch (`sed` 1 회) 는 공유되므로, **지점당 비용은 단일 축 batch 와 같고 절 구조 (blok 2 개 · 표 2 개) 만 +4 행** 늘었다.

**AC 4 반영 결과 · 분할 판단.** 본 절 **신설만** 수행했다. AC 1 직후 예상 절 길이는 `§ 12.70` 검산식대로 판정표 40 행 + 표 헤더 4 + 표 밖 구조 39 ≈ **83 행** (실측 **85 행**) 으로 상한 100 안이라 **분할하지 않았다** (근거 컬럼 "n 과 동일 사유" 압축 · 실측 인용 요약형을 먼저 적용). blok B 이월은 불요했다.

**AC 6 검산.** `wc -l docs/use-cases/REQ-COVERAGE-AUDIT.md` **6493 → 6578** (+85 = 본 절 **85 행**, 상한 100 이내). ` ``` ` fence **164 → 164** (짝수) · 신설 두 표 컬럼 **8** 개로 일치해 markdown 문법 무손상. `git status --short` 에 **`docs/requirements.md` · `README.md` · `docs/architecture/` · `.claude/agents/` · `CLAUDE.md` · `docs/decisions/` 부재** 를 확인했고 `git diff --stat` 은 **2 파일** (본 파일 · 본 task 파일) · **≤ 300 LOC** 다. doc-only 라 `pnpm` 실행 **0** ([CLAUDE.md](../../CLAUDE.md) §3.2 direct doc-only 면제).

**진척.** pointer 축 = 정본 2 파일 **26** + ADR 군 **41** + 표 컬럼 전반 **33** + 본 절 **40** = **140 판정** / 정정 census **140** = **100% — 축 마감**. 표 컬럼 66/66 · 산문 74/74 모두 전수 판정이 끝났고, 잔여 pointer 지점은 **0** 이다.

**한계.**

1. **census 140 은 정규식 포착분의 정정치** — `§ 12.69` 가 확인한 포착률 (65.9%) 문제는 정정으로 흡수했으나, `README` 문자열 없이 행 번호만 쓰는 pointer 는 원리적으로 포착되지 않아 축 마감이 "전수" 를 보장하지는 않는다.
2. **대응 임계는 여전히 명시 규칙이 아니다** (`§ 12.69` · `§ 12.70` 한계 승계) — blok A 는 `요약` 축약형, blok B 는 인용문이라 판정 기준의 결이 다르다.
3. **(b) 결론의 표본은 범위 32 지점** 이며, "경계 정렬" 변수는 사후 관찰이라 다음 축에서 선험적으로 재검증돼야 한다.
4. **커버리지 (v)(c) 는 행 단위 계수** 라 한 행 안 2+ 지시의 누락은 잡지 못한다. `140 ~ 148` 9 행의 REQ 대상 밖 판단도 정성이다.
5. **in-place 정정 0** — 본 절 부분참 1 건이 더해져 **pointer 정정 batch** 후보는 **14 건** 이 됐다.

**AC 5 파생 영향 (목록만 — 본 slice 착수 금지).** **(1) 다음 slice 1 순위 = (A) pointer 정정 batch 의 방침 확정 + 대상 14 건 목록 확정** — pointer 축이 마감돼 "판정" 재고가 0 이 된 반면 정정 재고는 14 건으로 확정됐고, 착수 전 **`§ 12.15` append-only ↔ in-place 정정의 관계** 와 **ADR 각주 append 의 commit mode 판정** (§3.1 미규정 구간) 을 먼저 확정해야 하므로 방침 slice 를 앞세운다 (B 는 외부 파일 3 자 대조라 규모 미상 · C 는 아래대로 시급성 낮음). **(C) anchor 좌표계 이행 (FU14) 시급성 재평가 결론 = 낮음 유지** — 어긋남 14 건이 **전부 범위 표기** 이고 단일 좌표 113 지점은 무오류라, 좌표계 전면 이행보다 **범위 표기 규약** (절 통째 범위 금지 · 끝 좌표를 의미 단위 경계에 정렬 · 대표 단일 좌표 병기) 이 비용 대비 효과가 크다. / (2) `deploy/README.md` ↔ `deployment.md` ↔ runbook 3 자 정합 (`§ 12.67` FU (2)) / (3) UC-09 `§ 5` sequence participant 병기 (FU12 — **62 회째 이월**) / (4) `CLAUDE.md` §1 pointer 부정확 (FU11) / (5) 범위 표기 규약 신설 (본 절 (b) 파생 — FU14 를 대체하는 저비용 안) / (6) node · row 외연 정의 (FU20) / (7) 인증 규약 단일 정본 부재 (FU26) / (8) `sync/async` 컬럼 경계 (FU27) / (9) message format 컬럼 판별력 (FU31) / (10) `requirements.md` **3 행 ↔ task 파일 5 행** 표기 불일치 (`§ 12.70` (i) 부수 발견 승계). **본 stream 밖 (승계만)** — **pointer 정정 batch** 후보 **14 건** (+1) · **ADR 각주 append 의 commit mode 판정**.

### 12.72 pointer 정정 batch 착수 게이트 — 부분참 **14** 건 목록 실측 + `§ 12.15` append-only ↔ in-place 관계 확정 + commit mode **direct** 판정 (`§3.1` 미규정 구간) + **2 slice** 분해안 (T-1474)

> **본 절의 위치 · 계보** — `§ 12.71` 이 pointer 축 **140 지점 전량** 을 마감하며 (**140 : 172 : 126 : 14 : 0**, 참율 90.0%) 판정 재고를 0 으로 만들었고, 그 **AC 5 파생 (1)** 이 다음 slice 1 순위로 지목한 **(A) pointer 정정 batch** 의 착수 게이트를 여는 절이다. 계보 — `§ 12.68` (정본 2 파일 26) → `§ 12.69` (ADR 군 41) → `§ 12.70` (표 전반 33) → `§ 12.71` (최종 40 · 축 마감) → **본 절 (정정 방침 확정)**. 앞 4 절이 **측정** 이었다면 본 절은 **규칙 결정** 이라 산출물이 판정표가 아니라 목록 · 3 분류 규칙 · commit mode 표 · 분해안 **4 종** 이다. **무편집 사유** — 14 건이 걸린 `docs/decisions/*` · `docs/requirements.md` · `docs/architecture/directory.md` 는 본 절에서 **한 글자도 고치지 않았다** (`§ 12.15` append-only + `§ 12.68` ~ `§ 12.71` 무편집 선례 승계). 정정 실집행은 AC 4 분해안이 지목한 후속 slice 소관이다.
...
```
$ grep -n '부분참' docs/use-cases/REQ-COVERAGE-AUDIT.md | awk -F: '$1>=6263 && $1<=6564'
$ wc -l docs/requirements.md docs/decisions/ADR-0001*.md docs/decisions/ADR-0002*.md docs/decisions/ADR-0003-*.md docs/architecture/directory.md
```

**필터 기준 (명시).** 위 구간의 grep raw hit 은 **37** 개이나 `부분참` 어휘는 절 heading · 판정 규칙 문장 · 다중 표기 수치 · 한계 · 파생 산문에도 나온다. 따라서 **행 첫 문자가 `|` 인 판정표 행만** 계수했다 (raw 37 → 표 행 **14**). 구간별 분류 계수는 `§ 12.68` (6263 ~ 6326) **2** · `§ 12.69` (6327 ~ 6407) **11** · `§ 12.70` (6408 ~ 6479) **0** · `§ 12.71` (6480 ~ 6564) **1** = **14** 로 planner 기대 (2 + 11 + 0 + 1) 와 **일치** — 실측 채택, 차이 0.

**대상 파일 행 수 실측** — `requirements.md` **103** · `ADR-0001-stack.md` **146** · `ADR-0002-db.md` **127** · `ADR-0003-deployment.md` **173** · `directory.md` **203**. pointer 보유 행 (20 · 39 / 20 · 138 / 119 / 20 · 21 · 22 · 23 · 168 / 197) 은 **전부 각 파일 행 수 안** 이라 좌표 승계에 결번 0.

| # | 승계 | 대상 파일 : 행 | 주장 좌표 (README) | 실 대응 좌표 | 분류 |
| --- | --- | --- | --- | --- | --- |
| 1 | `§12.68` #16 | `docs/requirements.md`:20 | 136~139 | 136 ~ **140** (끝 139 = 빈 줄) | (I) |
| 2 | `§12.68` #24 | `docs/requirements.md`:39 | 19~22 | **20** 단독 (19 · 22 빈 줄) | (I) |
| 3 | `§12.69` #1 | `ADR-0001-stack.md`:20 | 88–93 | 88 ~ **92** (93 = 빈 줄) | (I) |
| 4 | `§12.69` #9 | `ADR-0001-stack.md`:138 | 88–93 | #3 과 동상 | (I) |
| 5 | `§12.69` #19 | `ADR-0002-db.md`:119 | 55–64 | **54** ~ 64 (55 = 빈 줄) | (I) |
| 6 | `§12.69` #21 | `ADR-0003-deployment.md`:20 | 7–17 | **15–18** | (I) |
| 7 | `§12.69` #22 | `ADR-0003-deployment.md`:21 | 33–41 | **31** + **34** (불연속) | (II) |
| 8 | `§12.69` #23 | `ADR-0003-deployment.md`:22 | 19–22 | **20** 단독 | (I) |
| 9 | `§12.69` #24 | `ADR-0003-deployment.md`:23 | 71–74 | **72–73** | (I) |
| 10 | `§12.69` #27 | `ADR-0003-deployment.md`:168 | 7–17 | #6 과 동상 | (I) |
| 11 | `§12.69` #28 | `ADR-0003-deployment.md`:168 | 19–22 | #8 과 동상 | (I) |
| 12 | `§12.69` #29 | `ADR-0003-deployment.md`:168 | 33–41 | #7 과 동상 (불연속) | (II) |
| 13 | `§12.69` #30 | `ADR-0003-deployment.md`:168 | 71–74 | #9 와 동상 | (I) |
| 14 | `§12.71` #37 | `docs/architecture/directory.md`:197 | L7-22 | **16 ~ 18** | (I) |

**파일 분포** — ADR 군 **11** (`ADR-0003` 8 · `ADR-0001` 2 · `ADR-0002` 1) · `requirements.md` **2** · `directory.md` **1** = **5 파일**. planner 기대 ② (ADR 군 집중 + `directory.md` 197 이 1 건) **확인**. `CLAUDE.md` · `.claude/agents/*` · `README.md` · 그 밖 `docs/architecture/*` 는 **대상 0 건**. **편집 행 수는 14 건보다 적은 11 행** — `ADR-0003:168` 한 행이 4 건 (#10 ~ #13) 을 함께 담는다.

#### AC 2 — `§ 12.15` append-only ↔ in-place 정정 관계 확정

**규칙 인용 (`§ 12.15` 1022 행 · 3 구).** ① "옛 요약의 수치·판정 문구는 §12.3 306 행 append-only 규약대로 무편집 보존하고, 현행 상태는 pointer 문장 append 로만 가리킨다." ② "대상은 날짜·판정 시점이 문장 안에 박혀 있는 **시점 기록**". ③ "판별 기준은 '그 문장이 어느 시점의 판정을 기록하는가, 아니면 현재 상태를 서술하는가' 하나다."

**적용 범위 결론 (2 구).** append-only 는 **문서 종류가 아니라 문장 성격** 에 걸린다 — 같은 `§ 12.15` 가 `§ 12.13` (918 행) 의 `api.md` 211 · 223 행 · `data-model.md` 168 행 **in-place 치환** 을 "날짜 stamp 없는 living document 의 현행 상태 서술" 이라 구분해 정당화했기 때문이다. 따라서 방침은 (a) audit 문서 자신의 절 추가뿐 아니라 (b) 판정 대상 문서 편집까지 규율하되, **금지가 아니라 판별 기준** 으로 규율한다 — "대상 문서라서 무조건 append" 도 "audit 밖이라서 자유 편집" 도 둘 다 오독이다.

**3 분류 판정 규칙 (본 절 신설 · 순차 적용).**

- **규칙 A → (II)** — 문장에 날짜 · 판정 시점 stamp 가 박혀 있거나 좌표 자체가 **당시 README 판의 증거** 로 기능하면 원 표기 보존 + 각주 append.
- **규칙 B → (I)** — 현행 상태 서술이고 실 대응 좌표가 판정표에서 **유일하게 결정** 되며 **단일 범위로 무손실 표현 가능** 하면 in-place 정정.
- **규칙 C → (II)** — 현행 서술이나 대응이 **불연속** (정본이 원 범위 밖) 이라 단일 범위로 표현 불가 → 원 범위 보존 + 정확 좌표 병기.
- **규칙 D → (III)** — 정정이 의미를 훼손하거나 (의도된 절 통째 지시) 대응 좌표가 유일 결정되지 않으면 무처리.

**배정 결과 — (I) 12 · (II) 2 · (III) 0.** 규칙 A 적용 **0** 인 사유 1 구 — 14 건의 어긋남 원인은 README drift 가 아니라 **작성 시점부터의 범위 끝 부정확** 이며 (`§ 12.71` AC 3 (a) 가 "어긋남 14 건 전량 범위 표기 · 단일 좌표 113 지점 어긋남 0" 으로 README 좌표 안정성을 실측), 원 표기에 "그 시점 판" 증거 가치가 없어 보존 사유가 성립하지 않는다. 규칙 C 적용 **2** 는 #7 · #12 (`33–41` — 대응 34 와 정본 31 이 불연속이라 `31–34` 로 넓히면 무관 2 행을 삼키고 `34` 로 좁히면 정본이 소실). 규칙 D 적용 **0** — #14 (`L7-22`, 폭 16 절 통째 지시) 가 유일 후보였으나 대응 **16 ~ 18** 이 판정표에서 유일 결정돼 규칙 B 가 먼저 성립한다.

#### AC 3 — commit mode 판정 (`CLAUDE.md` §3.1 미규정 구간)

**경계 해석 1 구.** §3.1 `pr` 컬럼은 "**새** `docs/architecture/*` 또는 `docs/decisions/*` **추가**" 로 한정되고 rule 4 는 "새 ADR 자체는 `pr`, 단 ADR 의 status 갱신 한 줄 수정은 `direct`" 라, **기존 ADR 본문의 비-결정 1 줄 수정** 을 이미 `direct` 쪽으로 기울여 뒀다 — pointer 좌표는 결정 내용이 아니라 **참조 장치** 라 status 갱신보다도 결정에서 멀다. **"동작 변경을 일으키는가" 적용 1 구** — 5 파일 모두 코드 · CI · 의존성 · 빌드 산출물과 무관해 `pnpm lint && pnpm build && pnpm test` 결과가 불변이므로 `pr` catch-all ("그 외 동작 변경을 일으키는 모든 파일") **미해당**.

| 파일군 | 본 batch 건수 | §3.1 근거 | 동작 변경 | commit mode |
| --- | --- | --- | --- | --- |
| `docs/decisions/ADR-*.md` (기존 본문 1 줄) | 11 | pr 컬럼 = "새 … 추가" 한정 + rule 4 status 선례 | 없음 | `direct` |
| `docs/requirements.md` | 2 | 양 컬럼 미열거 · pr catch-all 미해당 | 없음 | `direct` |
| `docs/architecture/*.md` (기존 본문) | 1 | pr 컬럼 = "새 … 추가" 한정 | 없음 | `direct` |
| `docs/use-cases/REQ-COVERAGE-AUDIT.md` (audit 절) | slice 당 1 | `§ 12.68` ~ `§ 12.72` 전량 direct 선례 | 없음 | `direct` |
| `CLAUDE.md` · `.claude/agents/*` | 0 | direct 컬럼 명시 | 없음 | `direct` (해당 0) |

**결론** — 정정 batch **전량이 `direct`** 이므로 §3.1 rule 3 (mode 혼합 시 split) · §2.5 (e) 위반 위험 **0** 이고 PR · reviewer 경로 불요. **Follow-ups 후보 1 구** — §3.1 표에 "기존 `docs/decisions/*` · `docs/architecture/*` 본문의 **비-결정 수정** (pointer 좌표 · typo) = `direct`" 를 rule 5 로 명문화할지 (본 절이 판정 선례를 세웠으므로 시급성 낮음 — `CLAUDE.md` 는 본 절에서 무편집).

#### AC 4 — 정정 batch slice 분해안

**파일 검산** — 대상 5 파일 + audit 절 1 + task 파일 1 = **7 파일** > cap 5 이므로 **최소 2 slice**. 처리 방식 (I) · (II) 혼합은 같은 slice 에 허용된다 (§3.1 rule 3 이 금지하는 것은 **commit mode** 혼합이고 본 batch 는 전량 `direct`).

| slice | 대상 파일 (건수) | 편집 행 | 파일 수 검산 | 예상 LOC |
| --- | --- | --- | --- | --- |
| **S1 (1 순위)** | `ADR-0003` (8) · `ADR-0001` (2) · `ADR-0002` (1) | **8 행** (20 · 21 · 22 · 23 · 168 / 20 · 138 / 119) | 3 + audit 1 + task 1 = **5** | ≈ **75** (정정 ±8 + 절 ~55 + task ~5) |
| **S2** | `requirements.md` (2) · `directory.md` (1) | **3 행** (20 · 39 / 197) | 2 + audit 1 + task 1 = **4** | ≈ **55** |

**1 순위 = S1** — ADR 군 **11/14 = 78.6%** 를 한 slice 로 소화하고 (II) 병기 2 건도 전부 여기 있어, 규칙 C 의 실집행 화법을 먼저 세우면 S2 가 기계적으로 따를 수 있다. 예상 규모 수치 — **파일 5 · 편집 행 8 · diff ≈ 75 LOC** (cap 300 LOC · 5 파일 **충족**, 파일 수는 정확히 상한이라 대상 추가 금지).

#### 진척 · 한계 · 파생 영향

**진척.** pointer 축 **판정** 재고 **0** (`§ 12.71` 마감) · **정정** 재고 **14 건 중 착수 0** · 방침 확정 **완료** (목록 · 3 분류 규칙 · commit mode · 분해안 **4/4**).

**한계.** 1. 실 대응 좌표는 `§ 12.68` · `§ 12.69` · `§ 12.71` 판정표 **승계** 이고 본 절은 대상 파일 · README 를 재대조하지 않았다 (파일 행 수 `wc -l` 만 실측) — 정정 slice 는 편집 직전 좌표를 **1 회 재확인** 해야 한다. 2. 규칙 C 의 병기 화법 (각주 vs 인라인 괄호) 은 미확정이며 S1 이 첫 사례로 정한다. 3. 예상 LOC 는 audit 절 길이 가정에 의존해 ±20 오차. 4. (III) 0 은 본 14 건 한정 판정이라 다음 축 (범위 표기 규약) 에서 재검토 대상이다.

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S1 실집행** = 다음 slice 1 순위. (2) S2 실집행. (3) §3.1 rule 5 명문화 (AC 3 Follow-up 후보). (4) 범위 표기 규약 신설 (`§ 12.71` 파생 5 — 본 절 규칙 B · C 가 씨앗). (5) anchor 좌표계 이행 (FU14 — 시급성 "낮음 유지").

### 12.73 pointer 정정 batch **S1 실집행** — ADR 군 11 건을 **8 행 in-place** 정정 + 규칙 C 병기 화법 **인라인 괄호** 확정 (T-1475)

> **본 절의 위치 · 계보** — `§ 12.72` 가 연 정정 batch 착수 게이트의 **AC 4 분해안 1 순위 S1** 을 **실집행** 한 절이다. 계보 — `§ 12.68` ~ `§ 12.71` (판정 · 축 마감) → `§ 12.72` (방침 4 종 확정) → **본 절 (첫 실 정정)**. 87 slice 동안 이어진 **측정** 이 처음 **정정** 으로 전환된 지점이라 산출물이 판정표가 아니라 **재대조표 · 화법 판정 · 편집 대조표** 다. **편집 범위** — `ADR-0003` 5 행 · `ADR-0001` 2 행 · `ADR-0002` 1 행 = **8 행** (추가 8 · 삭제 8, 행 수 증감 0). `README.md` 는 **재대조 입력일 뿐 무편집** 이고 S2 대상 (`requirements.md` · `directory.md`) 은 **미착수** 다.

#### AC 1 — 편집 직전 좌표 재대조 (`§ 12.72` 한계 1 이행)

**사용 명령 (그대로).**

```
$ for r in 7,22 31,41 54,64 71,74 88,93; do sed -n "${r}p" README.md | cat -n; done
$ sed -n '15,25p;165,173p' docs/decisions/ADR-0003-deployment.md   # ADR-0001 은 18,22p;136,140p / ADR-0002 는 117,121p
```

**재대조 결과 (6 쌍 · `주장 → 승계 → 실측 → 판정`).** ① `7–17` (REQ-005~007) → 15–18 → 15 = `## 코드 평가 대상` · 16~18 = 3 GitHub instance · 19 빈 줄 → **일치**.
② `19–22` (REQ-020) → 20 단독 → 19 빈 줄 · 20 = 권한 부족 통지 · 21 = fork 중복 (별 주제) · 22 빈 줄 → **일치**.
③ `33–41` (REQ-016) → 31 + 34 불연속 → 31 = Confluence SPACE 지정 (정본) · 33 = 일반 권한 · 34 = SPACE crawling · 36~41 = `### 평가 목표` → **일치**.
④ `71–74` (REQ-039/040) → 72–73 → 71 = 지표 변화 · 72 = 주기 지정 · 73 = manual trigger · 74 = 최근 구간 삭제 → **일치**.
⑤ `88–93` (성능 특성) → 88–92 → 88 = `# 성능 특성` · 90~92 = NFR 3 줄 · 93 빈 줄 → **일치**.
⑥ `55–64` (저장 정책) → 54–64 → 54 = `# 평가 자료의 저장` · 55 빈 줄 · 56~64 = 정책 9 bullet → **일치**.

**결론 1 구** — 6 쌍 **전량 일치 · 뒤집힘 0 · 정정 불요 반전 0** 이라 승계값을 채택했다 (승계 신뢰가 아니라 **재대조 결과 채택** 이며 두 값이 같았을 뿐이다). **압축 근거 1 구** — 절 ≤ 60 행 상한 때문에 AC 4 단서대로 재대조표를 **표 → 6 행 요약** 으로 압축했고 (표 형식만 버림 · 실측 내용 손실 0), 명령 3 줄도 1 줄 + 주석으로 합쳤다.

#### AC 2 — 규칙 C 병기 화법 확정 (**인라인 괄호 병기** · 본 slice 최초 판정)

대상 **2 건** — #7 (`ADR-0003`:21) · #12 (`ADR-0003`:168 의 `33–41` 성분). 판정은 **인라인 괄호 병기** (각주 불채택), 근거 3 구.

- **(a) 형식 제약** — 두 대상 모두 표 셀이 아닌 **산문 bullet 한 행** 이고, 각주는 정의 블록을 문서 하단에 **추가** 해야 해 `§ 12.72` 가 전제한 "행 수 증감 0 의 in-place 정정" 과 충돌한다 (GFM 각주는 렌더러 의존이라 plain markdown 열람 시 좌표가 분리돼 읽힌다).
- **(b) `§ 12.72` 규칙 C 문언** — "원 범위 보존 + 정확 좌표 **병기**" 는 두 좌표가 **같은 자리에 나란히** 있을 것을 요구한다. 각주는 원 범위만 본문에 남기고 정확 좌표를 밀어내 보존 · 병기의 대칭을 깬다.
- **(c) 파일 내 표기 일관성** — `ADR-0003` 의 pointer 는 전량 `(README <범위> 행)` 인라인 괄호이고 각주 사용례 **0** 이라, 인라인 병기가 기존 표기 체계에 증분 없이 얹힌다.

**확정 화법 예시 1 (S2 가 그대로 따를 문자열).**

```
(README 33–41 행 표기 · 실 대응 31 · 34 행 불연속)
```

산문 bullet 이 아닌 **슬래시 나열 행** (`ADR-0003`:168 형태) 에서는 괄호 중첩을 피해 `33–41 표기 · 실 대응 31 · 34 불연속 (REQ-016 Confluence)` 로 축약한다.

#### AC 3 — 8 행 in-place 정정 대조표

| 파일 : 행 | before | after | 분류 |
| --- | --- | --- | --- |
| `ADR-0003`:20 | `(README 7–17 행)` | `(README 15–18 행)` | (I) |
| `ADR-0003`:21 | `(README 33–41 행)` | `(README 33–41 행 표기 · 실 대응 31 · 34 행 불연속)` | (II) 규칙 C |
| `ADR-0003`:22 | `(README 19–22 행)` | `(README 20 행)` | (I) |
| `ADR-0003`:23 | `(README 71–74 행)` | `(README 72–73 행)` | (I) |
| `ADR-0003`:168 | `7–17 / 19–22 / 33–41 / 71–74 / 88–92` | `15–18 / 20 / 33–41 표기 · 실 대응 31 · 34 불연속 / 72–73 / 88–92` | (I)×3 + (II)×1 · `88–92` 는 참이라 무편집 |
| `ADR-0001`:20 | `(88–93행)` | `(88–92행)` | (I) |
| `ADR-0001`:138 | `(88–93행)` | `(88–92행)` | (I) |
| `ADR-0002`:119 | `55–64 행` | `54–64 행` | (I) |

**검산.** `git diff --numstat` = `2 2` (ADR-0001) · `1 1` (ADR-0002) · `5 5` (ADR-0003) = **추가 8 · 삭제 8** 로 기대치 (5 / 2 / 1) 와 **일치 · 초과 0**. 행 수 불변 (146 / 127 / 173) · README 링크 개수 불변 (2 / 9 / 3) 으로 링크 문법 무손상을 확인했다.

#### 진척 · 한계 · 파생 영향

**진척.** pointer **정정** 축 = 14 건 중 **처리 11 · 잔여 3** (78.6% 소화) · 편집 행 **8/11 파일행** 완료 · 규칙 C 화법 **확정** (`§ 12.72` 한계 2 해소). 판정 축은 `§ 12.71` 마감 그대로 재고 **0**.

**한계.** 1. 재대조는 **8 행이 참조하는 6 쌍** 한정이고 세 ADR 의 나머지 pointer 는 `§ 12.69` 판정 승계 그대로다. 2. 잔여 3 건 (`requirements.md` 20 · 39 · `directory.md` 197) 은 파일 수 cap 때문에 **미착수**. 3. 확정 화법의 S2 적용성은 잔여 3 건이 전부 (I) 라 **실검증 기회가 없다** — 규칙 C 재등장은 다음 축 소관. 4. 정정 좌표는 현 README (**151 행**) 기준이라 README 편집 시 재 drift 한다 (anchor 이행 FU14 미착수).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S2 실집행** (`requirements.md` 2 · `directory.md` 1 = 3 건) = 다음 slice **1 순위**. (2) `§3.1` rule 5 명문화 (`§ 12.72` AC 3 Follow-up). (3) 범위 표기 규약 신설 (`§ 12.72` 파생 4 — 본 절 화법이 두 번째 씨앗). (4) anchor 좌표계 이행 (FU14 — 시급성 "낮음 유지").

### 12.74 pointer 정정 batch **S2 실집행** — `requirements.md` 2 행 · `directory.md` 1 행 in-place 정정 + `19~22` 인용-성격 **자체 서술** 판정 + **정정 축 14/14 마감** (T-1476)

> **본 절의 위치 · 계보** — `§ 12.72` AC 4 분해안의 **2 순위 S2** 를 실집행해 `§ 12.73` (S1 · ADR 군 11 건 · 8 행) 이 남긴 **잔여 3 건** 을 소화하고 **pointer 정정 축 14 건 전량을 마감** 한 절이다. 계보 — `§ 12.68` ~ `§ 12.71` (판정 140 지점 · 축 마감) → `§ 12.72` (규칙 4 종) → `§ 12.73` (S1 8 행) → **본 절 (S2 3 행 · 축 마감)**. 서술 형식은 `§ 12.73` 승계 (재대조 → 판정 → 편집 대조표) 이나 잔여 3 건이 전부 (I) 규칙 B 라 **규칙 C 인라인 괄호 화법 사용 0** 이다. **편집 범위** — `requirements.md` 2 행 (20 · 39) · `directory.md` 1 행 (197) = **3 행** (추가 3 · 삭제 3 · 행 수 증감 0). `README.md` 는 재대조 입력일 뿐 **무편집** 이고 S1 이 고친 3 ADR 은 **재편집 0**.
...
```
$ for n in 15 16 18 19 20 21 22 96 103 136 137 138 139 140 141; do printf '%s: ' "$n"; sed -n "${n}p" README.md; done
$ sed -n '197p' docs/architecture/directory.md      # requirements.md 는 20p · 39p
```

**재대조 결과 (3 쌍 · `주장 → 승계 → 실측 → 판정`).** ① `136~139` (`requirements.md`:20 · 로컬 빌드/테스트) → 136 ~ 140 → 136 = `# 로컬 빌드 / 테스트` · 137 · 139 빈 줄 · 138 = 전제 산문 · **140 = 대상 행이 인용한 `pnpm install` bullet** · 141 = `pnpm lint` → **일치**.
② `19~22` (`requirements.md`:39 · 권한 부족 통지) → **20** 단독 → 19 · 22 빈 줄 · 20 = 권한 부족 통지 · 21 = fork 중복 제거 (별 주제) → **일치**.
③ `L7-22` (`directory.md`:197 · GitHub 3 instance) → 16 ~ 18 → 15 = `## 코드 평가 대상` heading · 16 ~ 18 = 3 instance bullet · 19 빈 줄 → **일치**.

**결론 1 구** — 3 쌍 **전량 일치 · 뒤집힘 0 · 정정 불요 반전 0** 이라 승계 신뢰가 아니라 재대조 결과를 채택했다 (두 값이 같았을 뿐이다). 같은 행의 `L96-103` 성분도 96 = LLM 지정 · 103 = `- open AI` 로 `§ 12.71` #38 참 판정이 재확인돼 **무편집**.

#### AC 2 — `requirements.md`:39 의 인용-성격 판정 (**자체 서술** → 규칙 B 유지)

**원문 대조 명령.** `grep -rn '19~22\|19–22' docs/tasks/T-0015-adr-0003-deployment-rest.md` · `grep -rn '권한 부족 통지 (README 19~22 행)' docs/ README.md`.

**결과 · 판정 1 구.** T-0015 의 REQ-020 언급은 38 행 (`권한 부족 통지` — 좌표 없음) 과 49 행 (`19–22 행 (관리자 권한 / 인증)` — en-dash · 다른 어구) 둘뿐이라 대상 문자열 `"권한 부족 통지 (README 19~22 행)"` 와 char-identical 일치 **0** 이고, repo 내 동일 문자열 다른 hit 은 `docs/tasks/T-1375-*.md` 71 행 (본 행을 쓴 판정 task 자신의 Follow-up) 뿐이다 — 즉 두 출처의 어구를 합성한 **audit 저자 자체 서술 (요약 gloss)** 이라 `§ 12.72` 규칙 A (인용 · 시점 증거) **미해당**, 배정대로 **규칙 B (I) in-place 정정** 을 집행했다 (분기 (b)). 보강 1 구 — `§ 12.73` 이 그 gloss 가 가리키던 `ADR-0003`:22 를 이미 `(README 20 행)` 으로 정정해, 미정정 시 서술이 원 출처와 **역행** 한다.

#### AC 3 — 3 행 in-place 정정 대조표

| 파일 : 행 | before | after | 분류 |
| --- | --- | --- | --- |
| `requirements.md`:20 | `136~139 행` | `136~140 행` | (I) |
| `requirements.md`:39 | `(README 19~22 행)` | `(README 20 행)` | (I) · AC 2 분기 (b) |
| `directory.md`:197 | `L7-22` | `L16-18` | (I) · `L96-103` 무편집 |

**검산.** `git diff --numstat` = `2 2` (`requirements.md`) · `1 1` (`directory.md`) = **추가 3 · 삭제 3** 으로 기대치와 **일치 · 초과 0**. 행 수 불변 (103 / 203) · `|` 개수 불변 (68 / 32, 대상 표 row 는 각 8 개) · `directory.md`:197 링크 1 개 불변으로 표 · 링크 문법 무손상을 확인했다. **동일 문자열 무편집 3 hit** — `requirements.md`:37 의 `136~139` (README 아닌 **소스 코드** 행 좌표) 와 100 행 blockquote 의 `136~139` · `19~22` (부분참 판정을 기록한 **시점 서술** · 규칙 A) 는 행 좌표 지정 치환으로 보존했다.

#### 정정 축 최종 집계 — **14/14 마감 · 잔여 0**

| slice | 건수 | 편집 파일 · 행 | 처리 내역 |
| --- | --- | --- | --- |
| S1 (`§ 12.73`) | 11 | 3 ADR · 8 행 | (I) 9 · (II) 규칙 C 2 |
| **S2 (본 절)** | **3** | 2 파일 · **3 행** | (I) 3 · 무편집 0 |
| **합계** | **14** | **5 파일 · 11 행** | (I) 12 · (II) 2 · (III) 0 · **잔여 0** |

`§ 12.72` AC 2 배정 ((I) 12 · (II) 2 · (III) 0) 과 실집행 결과가 **완전 일치** — 재분류 0 건이다. 이로써 pointer 축은 **판정 140 지점 (`§ 12.71` 마감) · 정정 14 건 (본 절 마감) 모두 재고 0** 이 된다.

#### 진척 · 한계 · 파생 영향

**진척.** pointer **정정** 축 **14/14 (100%)** · 편집 행 **11/11** · 축 마감 선언. 판정 축은 `§ 12.71` 마감 그대로 재고 **0** 이라 pointer 축 전체가 종결됐다.

**한계.** 1. 재대조는 **3 행이 참조하는 3 쌍** 한정이고 두 파일의 나머지 pointer 는 `§ 12.68` · `§ 12.71` 판정 승계 그대로다. 2. 규칙 C 화법은 S2 대상이 전부 (I) 라 **본 절에서도 실검증 기회가 없었다** (`§ 12.73` 한계 3 존속). 3. 정정 좌표는 현 README (**151 행**) 기준이라 README 편집 시 재 drift 한다 (anchor 이행 FU14 미착수). 4. `requirements.md`:100 의 부분참 서술은 시점 기록이라 무편집 보존했고, 그 결과 정정된 본문 (20 · 39 행) 과 **의도적으로 불일치** 한 상태로 남는다.

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **범위 표기 규약 신설** (`§ 12.72` 파생 4 · `§ 12.73` 파생 3 — 정정 축이 마감돼 다음 축 **1 순위**). (2) `§3.1` rule 5 명문화 (`§ 12.72` AC 3 Follow-up). (3) anchor 좌표계 이행 (FU14 — 시급성 "낮음 유지"). (4) `docs/tasks/T-0015-*.md` 등 task 문서의 옛 REQ 번호 잔재 정리 (본 batch 범위 밖 · 시급성 낮음).

### 12.75 범위 표기 규약 축 **착수** — 표기 형식 census (5 문서군 × 4 형식) + 규약 초안 **7 조** + 박제 위치 · commit mode 판정 + 이행 분해안 (T-1477)

> **본 절의 위치 · 계보** — `§ 12.74` 가 pointer **정정** 축을 14/14 로 마감하며 지목한 **파생 영향 (1) = 범위 표기 규약 신설** 을 착수한 절이다. 계보 — `§ 12.68` ~ `§ 12.71` (판정 140 지점) → `§ 12.72` (규칙 A ~ D · 씨앗 1) → `§ 12.73` (규칙 C 인라인 괄호 화법 · 씨앗 2) → `§ 12.74` (축 마감) → **본 절 (새 축의 게이트)**. `§ 12.72` 와 같은 **게이트 slice** 라 실집행 (소급 정정) 은 **0** 이고 산출은 census · 조문 · 위치 판정 · 분해안 **4 종** 뿐이다. **편집 대상 문서 0** — 산출은 본 절 신설과 task 파일이며 census 대상 파일군은 **읽기 전용** (`grep -c` 집계값만 사용 · 본문 무열람).

#### AC 1 — 표기 형식 census (문서군 단위 합산 · 파일별 나열 금지)

**사용 명령 (그대로).**

```
$ sum() { grep -chE "$1" $2 | awk '{s+=$1} END {print s+0}'; }   # 문서군 = 파일 목록 문자열
$ G1="README.md CLAUDE.md"; G2="docs/requirements.md"; G3="$(ls docs/architecture/*.md)"; G4="$(ls docs/decisions/ADR-*.md)"; G5="$(ls .claude/agents/*.md)"
$ for p in '[0-9]+ *~ *[0-9]+' '[0-9]+ *– *[0-9]+' 'L[0-9]+-[0-9]+' '([0-9]+ *(~|–) *[0-9]+|L[0-9]+-[0-9]+) *(행|line)'; do for g in "$G1" "$G2" "$G3" "$G4" "$G5"; do sum "$p" "$g"; done; done
```

| 문서군 (파일 수) | ① 물결 `~` | ② en-dash `–` | ③ `L` prefix | ④ 단위어 동반 (`행`·`line`) |
| --- | --- | --- | --- | --- |
| `README.md` + `CLAUDE.md` (2) | 5 | 6 | 0 | 4 |
| `docs/requirements.md` (1) | 25 | 0 | 0 | 25 |
| `docs/architecture/*.md` (15) | 131 | 24 | 9 | 31 |
| `docs/decisions/ADR-*.md` (53) | 111 | 42 | 3 | 11 |
| `.claude/agents/*.md` (8) | 6 | 3 | 4 | 2 |
| **합계** | **278** | **75** | **16** | **73** |

**오탐 배제 1 구.** 맨 hyphen `[0-9]-[0-9]` 는 날짜 (`2026-08-05`) · 식별자 (`T-1477`) · 버전과 구분 불가라 census 에서 **제외** 했고 ③ 은 `L` prefix 가 붙은 것만 셌다. **실측 함정 1 구** — ④ 를 bracket `[~–]` 로 쓰면 multibyte en-dash 가 바이트 단위로 분해돼 조용히 0 을 내므로 alternation `(~|–)` 로 교체했다 (교체 전 ADR 군 ④ = 0 → 교체 후 11).

**성격 1 구.** 위 집계는 **표기 형식의 분포** 일 뿐 **좌표 정오 판정이 아니다** — 판정 축은 `§ 12.71`, 정정 축은 `§ 12.74` 로 각각 마감돼 재고 0 이며 본 절은 그것을 다시 다투지 않는다. `grep -c` 는 **행 단위 계수** 이고 ④ 는 ① ~ ③ 의 부분집합이라 총합 산정 시 더하지 않는다.

#### AC 2 — 규약 초안 조문 7 개 (각 근거 1 구 · 기존 절 승계 · 새 판정 0)

- **(a) 구분자 단일화 = 물결 `~`** — 근거: AC 1 집계 ① **278** 이 ② 75 + ③ 16 의 합보다 3 배 이상인 최빈 형식.
- **(b) 경계는 양끝 포함** (`a~b` 는 a 행과 b 행을 **둘 다 포함**, b 는 실 대응 마지막 행) — 근거: `§ 12.68` #16 (`136~139` 의 139 가 빈 줄이라 실 대응은 `136~140`) 등 끝 경계 착오가 어긋남 14 건의 주 유형.
- **(c) 빈 줄 · heading 을 범위에 넣지 않는다** (실 대응 행만) — 근거: `§ 12.73` AC 1 ①⑤⑥ 이 heading 15 · 빈 줄 55 · 93 을 배제한 뒤에야 승계 좌표와 일치했다.
- **(d) 단일 행은 `20 행`** (`20~20` 금지) — 근거: `§ 12.72` 목록표 #2 · #8 과 `§ 12.74` 대조표 (`19~22` → `20 행`) 가 이미 단독 표기를 채택했다.
- **(e) `L` prefix 는 신규 표기에서 사용하지 않는다** (문서 · 코드 구분 없이 `N 행`), 단 파일 안이 이미 `L` 표기로 일관되면 그 파일 안에서는 존치 — 근거: census ③ 16 hit 로 소수이나 `§ 12.74` 가 `directory.md`:197 을 `L16-18` 로 정정해 **파일 내 일관성 우선** (`§ 12.73` AC 2 (c)) 을 선례로 남겼다.
- **(f) 불연속 대응은 인라인 괄호 병기** — `(README 33–41 행 표기 · 실 대응 31 · 34 행 불연속)`, 슬래시 나열 행은 축약형 — 근거: `§ 12.72` 규칙 C 를 `§ 12.73` AC 2 가 인라인 괄호로 확정한 문자열 그대로 승계.
- **(g) 시점 기록에는 소급 적용하지 않는다** (날짜 · 판정 시점 stamp 가 박혔거나 당시 판의 증거로 기능하는 문장) — 근거: `§ 12.72` 규칙 A 와 `§ 12.15` append-only, 실례는 `§ 12.74` 한계 4 (`requirements.md`:100 blockquote 를 무편집 보존).

#### AC 3 — 박제 위치 · commit mode 판정 (후보 3 → **① 선택**)

| 후보 | 위치 | `CLAUDE.md` §3.1 근거 | commit mode | 판정 |
| --- | --- | --- | --- | --- |
| **①** | audit 문서 자체 절 (본 절이 곧 정본) | `§ 12.68` ~ `§ 12.74` 전량 direct 선례 (`§ 12.72` AC 3) | `direct` | **선택** |
| ② | `docs/architecture/` 신설 문서 | pr 컬럼 "**새** `docs/architecture/*` **추가**" 명시 | `pr` | 미선택 |
| ③ | `CLAUDE.md` §12 하위 조항 | direct 컬럼에 `CLAUDE.md` 운영규칙 명시 | `direct` | 미선택 |

**선택 근거 2 구.** ① 은 조문의 근거가 전량 `§ 12.68` ~ `§ 12.74` 의 집계 · 판정이라 근거와 조문 사이 참조가 **절 번호 하나** 로 끝나 본 규약이 막으려는 좌표 drift 자체를 최소화한다. 또한 `direct` 라 본 게이트 slice 와 mode 가 같아 §3.1 rule 3 split 이 불요하다. **미선택 사유 각 1 구** — ② 는 `pr` 이라 mode 혼합 금지 (§3.1 rule 3) 로 반드시 별 slice 이고 조문이 아직 초안이라 문서 신설이 이르다. ③ 은 `CLAUDE.md` 가 불변 규칙 문서라 초안 조문 투입이 rule 5 명문화 (파생 2) 와 얽히고 본 slice 의 Out of Scope 가 편집을 금한다.

#### AC 4 — 이행 분해안

**(a) 소급 대상 규모 추정.** 조문 (a) 기준 비-`~` 표기는 ② 75 + ③ 16 = **91 행** (④ 는 부분집합이라 미합산) 이고, 여기서 (e) 파일 내 일관성 존치분과 (g) 시점 기록 제외분이 빠지므로 실 대상은 그보다 **작다**. 정확 건수 확정은 후속 slice 몫이며 본 절은 상한만 제시한다.

**(b) 전면 소급 치환 금지 (근거 2 구).** ① `§ 12.15` append-only 는 시점 기록의 원 표기를 증거로 보존하라 요구하고 `§ 12.72` 규칙 A 가 그 판별 기준이라, 91 행 일괄 치환은 규칙 A 해당분을 가리지 않고 훼손한다 (`§ 12.74` 한계 4 가 그 보존이 **의도적 불일치** 를 낳는다는 실례). ② 91 행은 문서군 4 개 · 파일 수십에 흩어져 cap (5 파일 · 300 LOC) 을 구조적으로 초과하고, slice 로 쪼개도 행마다 규칙 A 판별 비용이 형식 통일 이득을 넘는다. **판정 = 신규 작성분부터 적용 + 소급은 이미 어긋남이 확인된 좌표에 한정** (그 재고는 `§ 12.74` 로 **0**).

| slice | 대상 문서군 | 예상 편집 행 | 파일 수 검산 (≤ 5) | 예상 LOC (≤ 300) | mode |
| --- | --- | --- | --- | --- | --- |
| **S1 (1 순위)** | audit 절만 (조문 7 조 반례 검증 후 **정본 승격**) | 0 (신설 절) | audit 1 + task 1 = **2** | ≈ **65** | `direct` |
| S2 | `CLAUDE.md` §12 하위 pointer 3 줄 + §3.1 rule 5 명문화 | 3 ~ 6 | CLAUDE 1 + audit 1 + task 1 = **3** | ≈ **50** | `direct` |
| S3 (시급성 낮음) | `docs/decisions/ADR-*.md` 중 `~`·`–` **혼용 파일** 한정 정규화 | ≤ 10 | ADR 3 + audit 1 + task 1 = **5** | ≈ **70** | `direct` |
| S4 (조건부) | 후보 ② 채택 시에만 `docs/architecture/` 규약 문서 신설 | 신설 | 신규 1 + task 1 = **2** | ≈ **80** | `pr` (혼합 금지 · 독립 slice) |

**(d) 1 순위 = S1, 사유 1 구.** 조문이 초안인 동안에는 후속 어떤 slice 도 인용할 **정본** 이 없어 축이 진행 불가이며, 이는 `§ 12.72` (방침 확정) → `§ 12.73` (실집행) 의 검증된 순서를 그대로 승계한 것이다.

#### 진척 · 한계 · 파생 영향

**진척.** 범위 표기 규약 축 **착수** — 게이트 산출 **4/4** (census 5 문서군 × 4 형식 · 조문 7 · 위치 · mode 판정 · 분해안 4 slice). 실집행 · 편집 대상 문서 **0** (게이트 정의대로). pointer 축은 판정 · 정정 모두 재고 **0** 유지.

**한계.** 1. census 는 `grep -c` **행 단위** 라 한 행에 범위가 2 개면 1 로 세어 실 hit 은 집계값 **이상** 이다. 2. 조문 7 개는 **초안** 이고 반례 검증 (특히 (e) `L` prefix 폐지 ↔ 파일 내 일관성 충돌) 은 S1 몫이다. 3. 모집단은 pointer 축이 다룬 5 문서군 한정이라 `docs/tasks/*` · `docs/progress/*` 는 census 밖이다. 4. FU14 anchor 좌표계로 이행하면 형식 논의가 anchor 표기에 흡수돼 (a) · (d) · (e) 는 재검토 대상이다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S1 규약 정본 승격** = 다음 slice **1 순위**. (2) S2 = `CLAUDE.md` §12 pointer + §3.1 rule 5 명문화 (`§ 12.72` AC 3 Follow-up 흡수). (3) S3 ADR 혼용 파일 정규화 (시급성 낮음). (4) anchor 좌표계 이행 FU14 (낮음 유지).

### 12.76 범위 표기 규약 **정본 승격** — 초안 7 조 중 **3 조 반례 실측** ((e) 반례 1 건) 후 조문 `R1` ~ `R7` 확정 + 적용 범위 · 발효 선언 (T-1478)

> **본 절의 위치 · 계보** — `§ 12.75` 분해안 (AC 4) 이 **1 순위** 로 지목한 **S1 = 규약 정본 승격** 을 집행한 절이다. 계보 — `§ 12.72` (규칙 A ~ D) → `§ 12.73` · `§ 12.74` (정정 실집행 · 축 14/14 마감) → `§ 12.75` (census · 초안 7 조 · 위치 판정 · 분해안) → **본 절 (정본)**. 실집행 (소급 정정) 은 여전히 **0** 이고 **편집 대상 문서도 0** — 산출은 본 절 신설과 task 파일뿐이며 census 대상 파일군은 **읽기 전용** (`grep -l` · `grep -c` 집계값과 파일명만 사용 · 본문 무열람).

#### AC 1 — 초안 7 조 반례 실측 (3 조 · (e) 필수)

**사용 명령 (그대로).**

```
$ G1="README.md CLAUDE.md"; G2="docs/requirements.md"; G3="$(ls docs/architecture/*.md)"; G4="$(ls docs/decisions/ADR-*.md)"; G5="$(ls .claude/agents/*.md)"; ALL="$G1 $G2 $G3 $G4 $G5"
$ sum() { grep -chE "$1" $2 | awk '{s+=$1} END {print s+0}'; }
$ sum '[0-9]+ *~ *[0-9]+' "$ALL"; sum '[0-9]+ *– *[0-9]+' "$ALL"; sum 'L[0-9]+-[0-9]+' "$ALL"    # (a) 최빈 형식 재확인
$ grep -ohE '([0-9]+) *(~|–) *\1([^0-9]|$)' $ALL | wc -l                                         # (d) 같은 수 반복 잔존
$ for f in $(grep -lE 'L[0-9]+-[0-9]+' $ALL); do echo "$f L=$(grep -cE 'L[0-9]+-[0-9]+' $f) tilde=$(grep -cE '[0-9]+ *~ *[0-9]+' $f)"; done   # (e)
```

| 초안 조문 | 반례 유무 | 집계값 | 판정 |
| --- | --- | --- | --- |
| (a) 구분자 `~` 단일화 | 없음 | `~` **278** : `–` **75** : `L` **16** (`§ 12.75` AC 1 과 재실측 일치) | 초안 그대로 승격 → `R1` |
| (d) `20~20` 금지 | 없음 | 같은 수 반복 표기 **0 건** (역참조 정규식 · 양성 대조로 `20~20` 매칭 확인) | 초안 그대로 승격 → `R4` (사후 금지로만 기능) |
| (e) `L` prefix 폐지 ↔ 파일 내 일관성 존치 | **있음** | `L` 사용 **8 파일** 중 **5 파일 (L 11 hit) 이 이미 `~` 혼용** · 순수 `L` 파일 **3 (L 5 hit)** | **조건 추가** 후 승격 → `R5` |
| (b) 경계 포함 · (c) 빈 줄 배제 · (f) 병기 화법 · (g) 시점 기록 | grep 반증 불가 | — | `§ 12.68` ~ `§ 12.74` 실판정 승계 (새 판정 **0**) |

**(e) 파일별 실측 (16 hit · 소수라 나열 허용).** 혼용 5 = `directory.md` (L1/`~`5) · `p3-to-p4-transition.md` (L1/5) · `race-patterns.md` (L4/3) · `ADR-0030` (L2/1) · `integrator.md` (L3/2). 순수 3 = `p3-implementation-plan.md` (L3/0) · `ADR-0005` (L1/0) · `reviewer.md` (L1/0). 그 외 문서군은 `L` hit **0** 이라 합산만 (`README`+`CLAUDE` 0 · `requirements.md` 0).

**반례의 의미 1 구.** 초안 (e) 의 존치 단서 "파일 안이 이미 `L` 표기로 일관되면" 은 **8 파일 중 3 에만 성립** 하고 나머지 5 는 이미 혼용이라 존치 근거가 없다 — 무조건 존치는 혼용 5 파일을 영구 고착시키므로 조건을 **"해당 파일의 `~` 범위 표기가 0 건일 때만"** 으로 좁힌다.

#### AC 2 — 정본 조문 `R1` ~ `R7` (규범 · 예시 · 근거 3 요소) + 개정 대조

- **R1 (구분자)** — 범위 구분자는 물결 `~` **하나만** 쓴다. 예: `136~140 행` (금지: `136–140 행`). 근거: `§ 12.75` AC 1 ① 278 최빈 + 본 절 AC 1 재실측 일치. **개정 대조: (a) → R1 유지 (반례 0).**
- **R2 (경계)** — `a~b` 는 a 행과 b 행을 **둘 다 포함** 하며 b 는 실 대응 **마지막 행** 이다. 예: 실 대응이 136 부터 140 까지면 `136~140 행`. 근거: `§ 12.68` #16 (끝 경계 착오가 어긋남의 주 유형). **개정 대조: (b) → R2 유지 (grep 반증 불가 · 실판정 승계).**
- **R3 (실 대응)** — 범위 양끝에 빈 줄 · heading 을 **넣지 않는다** (실 대응 행만 센다). 예: heading 15 · 빈 줄 55 를 배제한 `16~54 행`. 근거: `§ 12.73` AC 1 ①⑤⑥. **개정 대조: (c) → R3 유지.**
- **R4 (단일 행)** — 단일 행은 `20 행` 으로 적고 `20~20` 은 **쓰지 않는다**. 예: `20 행` (금지: `20~20`). 근거: `§ 12.72` 목록표 #2 · #8 · `§ 12.74` 대조표 (`19~22` → `20 행`). **개정 대조: (d) → R4 유지 (반례 0 — 잔존 0 이라 사후 금지로만 기능).**
- **R5 (`L` prefix)** — 신규 표기에 `L` prefix 를 **쓰지 않는다** (문서 · 코드 구분 없이 `N 행`). 존치는 **그 파일의 `~` 범위 표기가 0 건일 때만** 허용한다. 예: `197 행` (금지: `L16-18` — 단 `p3-implementation-plan.md` · `ADR-0005` · `reviewer.md` 안에서는 기존 `L` 존치). 근거: 본 절 AC 1 (e) 실측 (8 파일 중 혼용 5). **개정 대조: (e) → R5 조건 추가** — 사유: 존치 단서가 3 파일에만 성립해 무조건 존치 시 혼용 5 파일이 영구 고착된다.
- **R6 (불연속 병기)** — 원문 표기와 실 대응이 어긋나면 **인라인 괄호** 로 병기한다. 예: `(README 33–41 행 표기 · 실 대응 31 · 34 행 불연속)`. 근거: `§ 12.72` 규칙 C 를 `§ 12.73` AC 2 가 확정한 문자열 그대로. **개정 대조: (f) → R6 유지** (예시 안의 `–` 는 **인용된 원문 표기** 라 R1 적용 대상이 아니다).
- **R7 (시점 기록 면제)** — 날짜 · 판정 시점 stamp 가 박혔거나 당시 판의 증거로 기능하는 문장의 범위 표기는 **고치지 않는다**. 예: `§ 12.74` 한계 4 의 `requirements.md`:100 blockquote 무편집 보존. 근거: `§ 12.72` 규칙 A · `§ 12.15` append-only. **개정 대조: (g) → R7 유지.**

**조문 수 1 구.** **7 유지** (병합 · 신설 · 삭제 **0**) — R2 · R3 을 "경계 규칙" 하나로 병합하는 안은 재설계라 실행하지 않고 파생 영향에 **후보로만** 남긴다.

#### AC 3 — 정본 신분 · 적용 범위 · 발효

**① 정본 좌표.** 본 절 `§ 12.76` 이 범위 표기 규약의 **정본** 이며 후속 slice 는 `§ 12.76 R3` 형태로 인용한다. `§ 12.75` AC 2 의 (a) ~ (g) 는 **초안 (superseded by `§ 12.76`)** 이나 `§ 12.15` append-only 에 따라 `§ 12.75` 본문은 **편집하지 않았다** — supersede 선언은 본 절 안에서만 한다.

**② 적용 범위.** 모집단은 `§ 12.75` AC 1 의 **5 문서군** (`README.md` + `CLAUDE.md` / `docs/requirements.md` / `docs/architecture/*.md` / `docs/decisions/ADR-*.md` / `.claude/agents/*.md`). `docs/tasks/*` · `docs/progress/*` 는 **범위 밖** (`§ 12.75` 한계 3 — census 모집단 미포함), **코드 주석 · 코드 내 문자열도 범위 밖** (본 규약은 문서의 좌표 표기 규약이다).

**③ 발효.** **신규 작성분부터 적용** 하고 **전면 소급 치환은 금지** 한다 (`§ 12.75` AC 4 (b) 판정 승계 — 91 행 일괄 치환은 규칙 A 해당분을 훼손하고 cap 5 파일 · 300 LOC 을 구조적으로 초과). 소급은 **이미 어긋남이 확인된 좌표에 한정** 하며 그 재고는 `§ 12.74` 로 **0** 이므로 발효 시점의 소급 실집행 대상도 **0** 이다.

#### 진척 · 한계 · 파생 영향

**진척.** 범위 표기 규약 축 **S1 마감** — 초안 7 조 중 grep 반증 가능한 **3 조 ((a) · (d) · (e)) 실측**, 반례 **1 건 ((e))** 확인 후 조문 `R1` ~ `R7` **정본 승격** (유지 **6** · 조건 추가 **1** · 신설 · 삭제 **0**). 편집 대상 문서 **0** · 소급 실집행 **0** 유지. 남은 slice = S2 · S3 · S4 (조건부).

**한계.** 1. 반례 실측은 **3 조** 뿐이고 (b) · (c) · (f) · (g) 는 grep 으로 반증 불가라 실판정 승계에 의존한다. 2. (e) 실측도 `grep -c` **행 단위** 라 한 행에 2 hit 이면 1 로 세어 파일별 값은 하한이다. 3. R5 의 존치 조건은 **현 시점 스냅숏** 이라 순수 3 파일에 `~` 가 새로 들어오면 존치 근거가 소멸한다 (그때는 R1 이 우선). 4. FU14 anchor 좌표계로 이행하면 형식 논의가 anchor 표기에 흡수돼 R1 · R4 · R5 는 재검토 대상이다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S2 = `CLAUDE.md` §12 pointer 3 줄 + §3.1 rule 5 명문화** — 인용할 정본이 생겼으므로 다음 **1 순위**. (2) S3 = ADR 혼용 파일 정규화 (`~`·`–` 동시 사용 **ADR 10 파일** · 5 문서군 전체로는 **17 파일** 실측 — 시급성 낮음). (3) R2 · R3 병합 재설계 후보 (낮음 — 본 slice 는 승격이라 미실행). (4) anchor 좌표계 이행 FU14 (낮음 유지).

### 12.77 범위 표기 규약 S2 — `CLAUDE.md` §12 정본 pointer 소절 신설 + §3.1 rule 5 명문화 (T-1479)

> **본 절의 위치 · 계보** — `§ 12.76` (정본) 의 파생 영향 (1) 이 **1 순위** 로 지목한 **S2** 를 집행한 절이다. 계보 — `§ 12.72` (규칙 A ~ D · rule 5 Follow-up 후보 제기) → `§ 12.73` · `§ 12.74` (정정 실집행 · 축 마감) → `§ 12.75` (census · 초안 7 조 · 분해안 S1 ~ S4) → `§ 12.76` (정본 `R1` ~ `R7`) → **본 절 (규약의 운영규칙 반영)**. rule 5 는 `§ 12.72` AC 3 결론이 남긴 Follow-up 후보를 `§ 12.75` 분해안이 S2 로 **흡수** 한 잔여 항목이다. 소급 정정은 여전히 **0** — 편집은 `CLAUDE.md` **2 지점** 뿐이고 census 대상 파일군은 **무열람**.

#### AC 1 · AC 2 — 편집 2 지점 대조표

| # | 파일 · 지점 | 삽입 좌표 (편집 후 실측) | 삽입 요지 | 근거 절 |
| --- | --- | --- | --- | --- |
| 1 | `CLAUDE.md` §3.1 판정 규칙 목록 | rule 4 직후 → **142 행** (1 행) | **rule 5** — ① 규범: 기존 `docs/decisions/*` · `docs/architecture/*` 본문의 **비-결정 수정** (행 좌표 pointer 정정 · typo · 표기 정규화) = `direct` ② 경계: **결정 내용 자체** (ADR 의 Decision · Consequences 의 실질, architecture 의 구조 판단) = 종전대로 `pr` ③ 근거 좌표: `§ 12.72` AC 3 | `§ 12.72` AC 3 결론 |
| 2 | `CLAUDE.md` §12 | `### 혼합이 자연스러운 경우` 직후 · `### 과거와의 호환` 직전 → **452 ~ 456 행** (heading 452 · bullet 454 ~ 456) | 소절 **`### 범위 좌표 표기 (행 범위)`** 신설 · bullet **3** — ① 정본 pointer (`§ 12.76` 의 `R1` ~ `R7` · 인용 형태 `§ 12.76 R3` · 상대 경로 링크 1) ② 실무 요약 3 점 (`R1` · `R4` · `R5`) ③ 적용 범위 · 발효 | `§ 12.76` AC 2 · AC 3 |

**rule 1 ~ 4 무편집 검산.** `git diff` 상 §3.1 변경은 **추가 1 행뿐** 이고 기존 rule 1 ~ 4 의 문구 · 번호 · 순서는 삭제 hunk 에 나타나지 않았다 (append only). rule 5 조문 안에 "새 판정이 아니라 `§ 12.73` ~ `§ 12.76` 4 slice 가 실제로 `direct` 로 집행한 **선례의 명문화**" 를 1 구 박제했다.

**SSOT 판정 1 구.** `CLAUDE.md` 신설 소절은 **pointer 이지 정본이 아니다** — `R1` ~ `R7` 의 규범 문장 · 예시 · 근거 복제는 **0** 이고, 실무 요약 3 점은 모두 조문 번호 (`R1` · `R4` · `R5`) 참조를 동반한 축약이라 조문과 어긋나면 `§ 12.76` 이 우선한다 (그 우선 관계는 소절 첫 bullet 이 명시). 조문 전문을 두 곳에 두면 본 규약이 막으려는 drift 를 규약 자체가 재생산하므로 bullet 은 허용 상한 5 가 아니라 **3** 으로 눌렀다.

#### AC 3 — 자기 준수 검산 (규약을 본 slice 산출물에 적용)

**사용 명령 (그대로).**

```
$ git diff -U0 CLAUDE.md docs/use-cases/REQ-COVERAGE-AUDIT.md docs/tasks/T-1479-*.md | grep '^+' | grep -cE '[0-9]+ *– *[0-9]+|L[0-9]+-[0-9]+'
```

**결과.** 신규 추가분 전체에서 위반 hit **0** — `R1` (구분자 `~`) · `R4` (단일 행은 `N 행`) · `R5` (`L` prefix 미사용) 3 조 모두 충족이라 산출물 내 정정은 **0 건**. 본 절 · rule 5 · 신설 소절 · task 파일의 범위 표기는 `136 ~ 143 행` · `452 ~ 456 행` 형태로 통일했고 단일 행은 `142 행` 처럼 `N 행` 으로만 적었다.

**예외 1 구.** `R6` 병기 화법으로 **인용된 원문 표기** (타 문서가 쓴 `–` 구분자 범위를 그대로 인용하는 경우) 는 R1 적용 대상이 아니라 위반이 아니다 — 다만 본 slice 는 그런 원문 인용을 새로 들이지 않아 예외 적용 건수도 **0** 이고, 그래서 위 집계값 0 은 예외 공제 없이 그대로 읽힌다.

#### 진척 · 한계 · 파생 영향

**진척.** 범위 표기 규약 축 **S2 마감** — 정본 (`§ 12.76`) 이 운영규칙 (`CLAUDE.md`) 에 **pointer 로** 반영됐고 (`### ` heading **20 → 21** · `## ` 불변 15), `§ 12.72` 이래 4 절이 이월해 온 **rule 5 Follow-up 이 소멸** 했다. 소급 정정 · census 파일군 편집은 여전히 **0**. 남은 slice = S3 (ADR 혼용 정규화) · S4 (조건부 · `pr`).

**한계.** 1. 신설 소절의 요약 3 점은 `R1` · `R4` · `R5` 만 담아 `R2` · `R3` · `R6` · `R7` 은 pointer 를 따라가야 보인다 (SSOT 보존의 의도된 대가). 2. 본 slice 는 규약을 **선언** 만 하고 5 문서군의 기존 표기는 그대로다 (`§ 12.76` AC 3 ③ 발효 규칙 — 소급 재고 0 유지). 3. rule 5 의 "비-결정 / 결정" 경계는 **문언 판정** 이라 회색지대 (예: Consequences 의 표현 정리) 는 case-by-case 로 남고 선례도 `§ 12.73` · `§ 12.74` 집행분뿐이다. 4. FU14 anchor 좌표계로 이행하면 신설 소절의 요약 3 점도 재검토 대상이다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) S3 = ADR 혼용 파일 정규화 (동시 사용 ADR **10 파일** · 5 문서군 **17 파일** · cap 초과라 3 파일씩 재분할 — 시급성 낮음). (2) rule 5 회색지대 판정 선례가 2 건 이상 쌓이면 조문에 예시 1 행 보강. (3) `R2` · `R3` 병합 재설계 후보 (낮음). (4) anchor 좌표계 이행 FU14 (낮음 유지).

### 12.78 범위 표기 규약 S3 batch 1 — 혼용 ADR 3 파일 (`ADR-0001` · `ADR-0002` · `ADR-0003`) `R1` 정규화 실집행 — 11 후보 행 중 정정 **10 행** (치환 **12 건**) · 면제 **1 행** (T-1480)

> **본 절의 위치 · 계보** — `§ 12.77` (S2 마감) 의 파생 영향 (1) 이 남은 **1 순위** 로 지목한 **S3** 의 **첫 batch** 다. 계보 — `§ 12.72` ~ `§ 12.74` (pointer 정정 축) → `§ 12.75` (census · 초안 7 조 · 분해안 S1 ~ S4) → `§ 12.76` (정본 `R1` ~ `R7`) → `§ 12.77` (`CLAUDE.md` §12 pointer · §3.1 rule 5) → **본 절 (규약의 첫 문서 실집행)**. S3 전량은 혼용 ADR **10 파일** 이라 cap 초과로 3 파일씩 재분할 (`§ 12.77` 파생 1) 됐고 본 절이 그 batch 1 이다. 편집은 census 로 좌표가 특정된 **3 파일 · 11 후보 행** 밖으로 나가지 않았다.

#### AC 1 — 한정 소급 정합 판정 (`§ 12.76` AC 3 ③)

**① 허용 근거.** `§ 12.76` AC 3 ③ 은 소급을 "**이미 어긋남이 확인된 좌표에 한정**" 하는데, 본 slice 는 `§ 12.75` AC 4 분해안 표가 **S3 로 사전 승인** 한 범위이고 AC 2 census 가 대상을 **파일 · 행 단위 (3 파일 · 11 행)** 로 특정했으므로 "확인된 좌표" 요건을 문언 그대로 충족한다. `§ 12.76` 이 말한 "발효 시점 소급 실집행 대상 **0**" 은 **pointer 축의 정정 재고** 에 관한 진술 (`§ 12.74` 승계) 이라 표기 축의 census 확정분과 충돌하지 않는다.

**② 금지선.** `§ 12.75` AC 4 (b) 의 **91 행 일괄 치환 금지** 는 그대로 유효하며 본 slice 는 3 파일 밖 파일을 **열지도 고치지도 않았다** (`git status --short` 검산 = 5 파일 · 그중 편집 대상 문서는 ADR 3 + audit 1). 행마다 `R6` (인용 원문) · `R7` (시점 기록) 면제 판별을 **선행** 했고 면제분은 무편집으로 남겼다.

**③ mode 근거.** `CLAUDE.md` §3.1 **rule 5** (T-1479 신설 — 기존 `docs/decisions/*` 본문의 **비-결정 수정** = `direct`) 의 **첫 적용 선례** 다. 편집은 구분자 1 문자 치환뿐이라 ADR 의 `Status` 행 · `## Decision` · `## Consequences` · `## Alternatives` 의 실질은 **무편집** 이다 (AC 4 검산).

#### AC 2 — 혼용 census 재실측 + 본 batch 3 파일 선정

**census 집계 1 구 (표 압축 · 사유 동반).** `docs/decisions/ADR-*.md` **54 파일** 을 `grep -cE '[0-9]+ *– *[0-9]+'` 와 `grep -cE '[0-9]+ *~ *[0-9]+'` 로 동시 집계한 결과 **혼용 (둘 다 ≥ 1) = 10 파일** — `ADR-0001` (en 4 / `~` 1) · `ADR-0002` (2/3) · `ADR-0003` (5/4) · `ADR-0006` (2/4) · `ADR-0008` (4/1) · `ADR-0013` (2/1) · `ADR-0014` (1/3) · `ADR-0018` (3/1) · `ADR-0033` (3/1) · `ADR-0035` (4/3) 로 `§ 12.77` 의 **10 파일** 과 **일치 (불일치 0)**. en dash 단독 4 파일 (`ADR-0005` 1 · `ADR-0016` 2 · `ADR-0017` 6 · `ADR-0021` 3) 은 혼용이 아니라 batch 3 이후 몫이다. 표 대신 1 구로 압축한 사유 — AC 5 의 **절 ≤ 60 행** 제약에서 판정 증거는 행별 대조표가 우선하고 census 는 집계값만으로 재현 가능하다.

**`R5` 대상 1 구.** `grep -lE 'L[0-9]+-[0-9]+' docs/decisions/ADR-*.md` = **2 파일** (`ADR-0005` 1 hit · `ADR-0030` 2 hit) 이며 **본 batch 의 편집 대상에 넣지 않았다** — `ADR-0030` 은 `~` 도 1 건 있어 `R1` · `R5` 이중 대상이라 **batch 2 의 우선 후보** 로 이월한다.

**선정 근거 1 구.** 본 batch 3 파일은 ① 셋 다 혼용 확정이고 ② `§ 12.73` 정정 축이 이미 in-place 편집한 동일 파일군이라 diff 검산 비용이 최소이며 ③ constraint 근거 ADR 3 종 (stack · db · deployment) 이라 타 문서 인용 빈도가 가장 높아 표기 drift 차단 효과가 크다.

#### AC 3 · AC 4 — 행별 대조표 (11 후보 행) + 결정 내용 무편집 검산

| # | 파일 | 행 | 원 표기 | 판정 | 근거 조문 |
| --- | --- | --- | --- | --- | --- |
| 1 | `ADR-0001` | 20 | `(88–92행)` + 수량 `100–200명` · `50–100개` | **정정** (좌표만 `88~92행`) · 수량 2 건 **규약 범위 밖** | `R1` / AC 3 ② |
| 2 | `ADR-0001` | 21 | `(106–116행)` | **정정** → `106~116행` | `R1` |
| 3 | `ADR-0001` | 138 | `(88–92행)` | **정정** → `88~92행` | `R1` |
| 4 | `ADR-0001` | 139 | `(106–116행)` | **정정** → `106~116행` | `R1` |
| 5 | `ADR-0002` | 119 | `54–64 행` | **정정** → `54~64 행` | `R1` |
| 6 | `ADR-0002` | 120 | `88–92 행` | **정정** → `88~92 행` | `R1` |
| 7 | `ADR-0003` | 20 | `(README 15–18 행)` | **정정** → `15~18 행` | `R1` |
| 8 | `ADR-0003` | 21 | `(README 33–41 행 표기 · 실 대응 31 · 34 행 불연속)` | **면제 (무편집)** — 병기 화법의 **인용된 원문 표기** | `R6` |
| 9 | `ADR-0003` | 23 | `(README 72–73 행)` | **정정** → `72~73 행` | `R1` |
| 10 | `ADR-0003` | 24 | `(README 88–92 행)` | **정정** → `88~92 행` | `R1` |
| 11 | `ADR-0003` | 168 | `15–18` / `33–41 표기 · 실 대응 …` / `72–73` / `88–92` | **부분 정정 3 건** (`15~18` · `72~73` · `88~92`) · `33–41` 면제 | `R1` + `R6` |

**`R7` · `R4` 판별 1 구.** 11 행 어디에도 날짜 · 판정 시점 stamp 가 없고 당시 판의 증거로 기능하는 문장도 아니라 **`R7` 해당 0** 이며, 정정 결과 단일 행이 된 경우도 없어 **`R4` 적용 0** 이다. 수량 범위 (`100–200명` · `50–100개`) 는 `§ 12.76` AC 3 ② 가 규약을 **문서의 좌표 표기 규약** 으로 한정했으므로 `R1` 대상이 아니라고 판정해 무편집으로 뒀다 (동일 내용을 `ADR-0003`:24 는 이미 `100~200 명` 으로 적어 문서 간 불일치가 남으므로 파생 영향에 후보로 이월).

**정정 후 재실측 1 구.** 행 수 **불변** (`wc -l` 전후 `ADR-0001` 146 · `ADR-0002` 127 · `ADR-0003` 173), en dash hit = `ADR-0001` **1** (수량 행) · `ADR-0002` **0** · `ADR-0003` **2** (21 · 168 행) 로 **잔존 3 = 면제 · 범위 밖 판정분과 동수** (신규 잔존 0).

**AC 4 무편집 검산 1 구.** `git diff --numstat` = `4/4` · `2/2` · `4/4` 로 **추가 행 수 = 삭제 행 수 (10/10)** 이고 모든 hunk 가 위 후보 11 행 안에 있으며, `## Decision` · `## Consequences` · `## Alternatives` 의 문장 실질 변경과 `Status` 행 편집은 **0** (해당 구역에 hunk 자체가 없음).

#### AC 6 — 자기 준수 · 무손상 · 범위 검산

**자기 준수.** `git diff -U0 <5 파일> | grep '^+' | grep -cE '[0-9]+ *– *[0-9]+|L[0-9]+-[0-9]+'` = **15**. 0 이 아닌 내역 1 구 — 대조표 11 행의 **원 표기 인용** + `R7` · `R4` 판별 문단 1 + 본 문단 1 + ADR 편집 행에 남은 면제 · 범위 밖 2 (`ADR-0001`:20 수량 범위 · `ADR-0003`:168 의 `33–41`) 로 **전부 `R6` 인용 원문이거나 규약 범위 밖** 이며, 본 slice 가 **새로 작성한 좌표 표기** (절 번호 인용 · 행 좌표) 는 전량 `~` 단일 구분자다.

**무손상 · 범위.** `wc -l docs/use-cases/REQ-COVERAGE-AUDIT.md` = **6936 → 6992** (신설 **56 행**), `git diff --stat` = **정확히 5 파일 · +82/-17** (≤ 300), `git status --short` 에 `README.md` · `CLAUDE.md` · `docs/requirements.md` · `docs/architecture/` · 다른 `docs/decisions/*` · `.claude/agents/` · `docs/LOOP.md` · `src/` **없음**. markdown 무손상 — audit 파일 ` ``` ` fence **짝수 (신설 0)** · 신설 표 컬럼 **6 일치** · heading 순번 `12.77` → `12.78` 연속 · 3 ADR 의 `## ` heading 수 불변 (6 · 7 · 6) · 링크 수 불변 (14 · 23 · 12). doc-only 라 `pnpm test` 불요 (`CLAUDE.md` §3.2 direct doc-only 면제).

#### 진척 · 한계 · 파생 영향

**진척.** 범위 표기 규약 축이 **선언 → 실집행** 으로 넘어갔다 — S3 batch 1 마감 (혼용 ADR **3/10 파일** 정규화 · 치환 **12 건** · 면제 **1 행**), `CLAUDE.md` §3.1 **rule 5 의 첫 적용 선례** 확보, 한정 소급 원칙 (`§ 12.76` AC 3 ③) 위반 **0**. 남은 slice = S3 batch 2 · 3 (ADR 잔여 7 + 비-ADR 혼용 7) · S4 (조건부 · `pr`).

**한계.** 1. census 는 `grep -c` **행 단위** 라 한 행에 범위가 2 개면 1 로 세어 파일별 값은 **하한** 이다 (`ADR-0003`:168 이 실례 — 1 hit 이지만 좌표 4 개). 2. 수량 범위의 `R1` 적용 여부는 본 절이 **범위 밖** 으로 판정했으나 조문에 명문이 없어 회색지대로 남는다. 3. 정정 대상은 en dash 구분자뿐이고 `R2` (경계) · `R3` (실 대응) 정합은 `§ 12.73` 판정 승계로만 담보된다 (본 slice 는 좌표 값을 재검증하지 않았다). 4. FU14 anchor 좌표계로 이행하면 본 정규화 자체가 anchor 표기에 흡수된다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S3 batch 2 = 혼용 ADR 3 파일** (`ADR-0005`·`ADR-0030` 계열 `R5` 이중 대상 우선 → `ADR-0006` · `ADR-0008` · `ADR-0013` 중 3) — 다음 1 순위. (2) S3 batch 3 이후 = 잔여 ADR + 비-ADR 혼용 파일 (`§ 12.77` 실측 17 - ADR 10). (3) **수량 범위 표기 통일 판정** — 조문 예시 1 행 보강 후보 (낮음). (4) rule 5 회색지대 예시 보강 — 본 절이 선례 **1 건째** 라 2 건째 축적 시. (5) `R2` · `R3` 병합 후보 · anchor FU14 (낮음 유지).

### 12.79 범위 표기 규약 S3 batch 2 — `L` prefix 이중 대상 ADR 3 파일 `R1` · `R5` 정규화 (T-1481)

> **본 절의 위치 · 계보** — `§ 12.78` (S3 batch 1) 파생 영향 (1) 이 **1 순위** 로 지목한 **S3 batch 2** 를 집행한 절이다. 계보 — `§ 12.75` (census · 초안 7 조) → `§ 12.76` (정본 `R1` ~ `R7`) → `§ 12.77` (S2 · `CLAUDE.md` 반영) → `§ 12.78` (S3 batch 1 · 혼용 ADR 3) → **본 절 (`R5` 이중 대상 ADR 3)**. batch 1 이 `R1` 단독이었던 것과 달리 본 slice 는 `R1` 과 `R5` 가 **연동** 되는 3 파일 (`ADR-0005` · `ADR-0006` · `ADR-0030`) 을 한 slice 로 묶었다.

#### AC 1 — 대상 선정 · 한정 소급 · mode 판정

**① 선정 근거.** `§ 12.78` 파생 (1) 이 "`R5` 이중 대상 우선" 을 지목했고 본 batch 3 파일은 모두 `L` prefix 좌표 보유 파일이다 (AC 2 실측 — `ADR-0005` 15 행 · `ADR-0006` 10 행 · `ADR-0030` 3 행).

**② 한정 소급 준수.** AC 2 census 가 대상을 **파일 · 행 단위로 특정** 했으므로 `§ 12.76` AC 3 ③ 의 "이미 어긋남이 확인된 좌표에 한정" 요건을 충족한다. **전면 일괄 치환은 여전히 금지** 이며 3 파일 밖은 열지도 고치지도 않았다 (AC 7 `git status --short` 검산).

**③ mode 근거.** `CLAUDE.md` §3.1 **rule 5** (기존 `docs/decisions/*` 본문의 **비-결정 수정** = `direct`) 의 **두 번째 적용 선례** 다 (첫 선례 `§ 12.78` ③). 편집은 구분자 · prefix 치환뿐이고 ADR 의 `Status` 행 · Decision · Consequences · Alternatives 실질은 **무편집** (AC 5 검산).

#### AC 2 — `R5` census 재실측 + `§ 12.78` 집계 정정 (2 건)

**census 집계 1 구 (표 압축 · 사유 동반).** `grep -lE 'L[0-9]+' docs/decisions/ADR-*.md` = **31 파일** (실측 시점 ADR 총 **53 파일**) 이 `L` 표기 보유이고, 본 batch 3 파일의 행 단위 실측은 `ADR-0005` **15** · `ADR-0006` **10** · `ADR-0030` **3** (합 **28 행**), `~` hit (`grep -cE '[0-9]+ *~ *[0-9]+'`) 는 **0 · 4 · 1** 이다. 표 대신 집계 1 구로 압축한 사유 — 절 ≤ 60 행 제약에서 판정 증거는 아래 대조표가 우선하고 census 는 집계값만으로 재현 가능하다 (`§ 12.78` AC 2 선례 승계).

**정정 (1) — 대상 파일 수.** `§ 12.78` 의 `R5` 대상 집계는 `L[0-9]+-[0-9]+` (**ASCII hyphen · 범위 한정**) 패턴이라 en dash 를 쓴 `L56–63` (`ADR-0006`) 과 단일 행 `L59` 류를 통째로 놓쳤다 — 실제 `R5` 대상은 **2 파일이 아니라 31 파일** 이다. `§ 12.15` append-only 이므로 `§ 12.78` 본문은 **편집하지 않고** 본 절에서만 정정 선언한다.

**정정 (2) — 후보 행 수.** 같은 undercount 가 본 slice 의 후보 목록 (**6 행**) 에도 전파돼 있었다 — 실측 후보는 3 파일 합 **29 행** (`L` 보유 28 행 + en dash 전용 `ADR-0005`:166). `R5` 연동 판정 (AC 3) 이 **파일 단위** 로 성립하므로 후보를 6 행으로 묶으면 같은 파일 안에 정정분과 미정정분이 공존해 AC 4 잔존 검산 자체가 성립하지 않는다. 따라서 대상을 **3 파일 안의 29 행 전량** 으로 확장했다 — **파일 경계는 불변** 이라 한정 소급 위반이 아니다.

#### AC 3 — `R5` 존치 조건 연동 판정

**① `~` hit 의 성격 판별.** `R5` 존치 조건 ("그 파일의 `~` 범위 표기가 **0 건** 일 때만") 의 입력을 행 단위로 실측하니 `ADR-0006` 의 4 hit 은 전량 수치 · 수량 · 식별자 범위 (`0.0~1.0` · `0~100` · `ADR-0001~0003` · `100~200명` · `50~100 repo`) 이고 `ADR-0030` 의 1 hit 도 식별자 (`REQ-005~008`) 다. `§ 12.76` AC 3 ② 가 규약을 **문서의 좌표 표기 규약** 으로 한정했으므로 **좌표 표기 `~` 기준으로는 3 파일 모두 0 건** — 즉 셋 다 문언상 존치 대상이었다. 본 slice task 가 전제한 "`ADR-0006` · `ADR-0030` 은 `~` ≥ 1 이라 존치 불충족" 은 raw grep 기준 판정이므로 본 절이 정정한다 (결론은 ② 로 같아진다).

**② 그럼에도 3 파일 모두 정정.** 본 slice 가 같은 파일의 `R1` 후보를 정정하는 순간 (`ADR-0005`:166 en dash · `ADR-0006`:173 · 174 `L56–63` · `ADR-0030`:24 · 93 `L15-18`) **좌표 `~` 가 새로 유입** 돼 존치 근거가 소멸한다 — `§ 12.76` **한계 3** 이 예고한 "순수 파일에 `~` 가 새로 들어오면 그때는 `R1` 이 우선" 상황 그대로다. 존치를 지키려면 `R1` 정정을 보류해야 하는데 이는 정본 조문의 1 순위를 뒤집는 선택이라 채택하지 않는다. 따라서 `R1` 정정과 `L` prefix 정정을 **같은 slice 안에서 동시 집행** 한다.

**③ 조문 예시 대조.** `§ 12.76 R5` 예시의 "`p3-implementation-plan.md` · `ADR-0005` · `reviewer.md` 안에서는 기존 `L` 존치" 중 **`ADR-0005` 항목은 본 slice 로 무효화** 됐다 (해당 파일 `L` 잔존 **0**). 예시는 승격 시점 **스냅숏** 이므로 조문 **본문은 편집하지 않고** (`§ 12.15` append-only · Out of Scope) 파생 영향 (3) 에 갱신 후보로 이월한다.

#### AC 4 · AC 5 — 대조표 (29 후보 행 · 유형별 압축) + 무편집 검산

| # | 파일 | 행 | 원 표기 | 판정 | 근거 조문 |
| --- | --- | --- | --- | --- | --- |
| 1 | `ADR-0005` | 105 | `integrator.md L57-69` | **정정** → `57~69 행` | `R1` + `R5` |
| 2 | `ADR-0005` | 136 ~ 149 (14 행) | 도구 대조표 link label 의 `L12` · `L19` · `L20` · `L24` · `L46` · `L48` · `L54` · `L66` ~ `L69` · `L75` · `L85` · `L86` · `L97` · `L106` · `L140` · `L141` · `L143` | **정정** → `12 행` 형태 | `R5` + `R4` |
| 3 | `ADR-0005` | 166 | `110–128행` · `117–128행` (동행 `R-110~R-114` 는 식별자) | **정정 2 건** → `110~128행` · `117~128행` · 식별자 **범위 밖** | `R1` / AC 3 ② |
| 4 | `ADR-0006` | 23 ~ 28 · 101 · 165 (8 행) | `L56` · `L59` · `L60` · `L61` · `L62` · `L63` · `L82` (link label 15 + 본문 1) | **정정** → `56 행` 형태 | `R5` + `R4` |
| 5 | `ADR-0006` | 173 · 174 | `L56–63` | **정정** → `56~63 행` | `R1` + `R5` |
| 6 | `ADR-0006` | 111 · 156 · 165 · 167 | `0.0~1.0` · `0~100` · `ADR-0001~0003` · `100~200명` · `50~100 repo` | **범위 밖 (무편집)** | AC 3 ② |
| 7 | `ADR-0030` | 24 · 93 | `L15-18` | **정정** → `15~18 행` | `R1` + `R5` |
| 8 | `ADR-0030` | 33 · 6 | `README L20` / `REQ-005~008` | **정정** → `20 행` / 식별자 **범위 밖** | `R5` + `R4` / AC 3 ② |

**`R6` · `R7` · `R4` 판별 1 구.** 29 행 어디에도 타 문서 표기의 **인용 원문 병기** (`R6`) 나 날짜 · 판정 시점 stamp (`R7`) 가 없어 **면제 0** 이고, `L` 제거로 단일 행이 된 24 행에는 `R4` 를 적용해 `20~20` 형태를 만들지 않았다. 수량 · 식별자 범위 **8 좌표** 는 `§ 12.78` `R7` · `R4` 판별 1 구의 선례를 승계해 **규약 범위 밖 · 무편집** 으로 뒀다.

**정정 후 재실측 1 구.** 행 수 **불변** (`ADR-0005` 181 · `ADR-0006` 182 · `ADR-0030` 96), `L` prefix 잔존 **0 · 0 · 0** · en dash 잔존 **0 · 0 · 0** 으로 **면제 판정분 (0) 과 동수** (신규 잔존 0), 링크 수 불변 (43 · 28 · 10) · `## ` heading 수 불변 (6 · 5 · 5).

**AC 5 무편집 검산 1 구.** `git diff --numstat` = `16/16` · `10/10` · `3/3` 로 **추가 행 수 = 삭제 행 수 (29/29)** 이고, `git diff --word-diff` 의 변경 토큰 **45 쌍 전량이 구분자 · `L` prefix · 단위어 `행`** 뿐 (유일한 부수 변경은 `ADR-0006`:24 의 조사 `가` → `이` — `L59 가` 가 `59 행` 뒤에서 비문이 되므로 필수) 이며, 모든 hunk 가 위 대조표 29 행 안에 있어 `## Decision` · `## Consequences` · `## Alternatives` 의 **문장 실질 변경 0** · `Status` 행 hunk **0** 이다.

#### 진척 · 한계 · 파생 영향

**진척.** S3 batch 2 마감 — `R1` · `R5` **이중 대상 ADR 3 파일** 정규화 (치환 **29 행** · 면제 **0** · 범위 밖 판정 **8 좌표**), `R5` 존치 조건의 입력을 **좌표 표기 `~`** 로 확정 (AC 3 ①), `§ 12.78` 집계 **2 건 정정**, `CLAUDE.md` §3.1 rule 5 **두 번째 선례** 확보. 남은 slice = S3 batch 3 이후 · S4 (조건부 · `pr`).

**한계.** 1. `R5` 대상은 실측 **31 ADR 파일** 이라 본 batch 후에도 **28 파일** 이 남는다 (batch 3 이후 몫 · 시급성 낮음). 2. `L` 좌표가 가리키는 **값 자체의 정확성은 재검증하지 않았다** (`§ 12.74` 판정 승계 — 본 slice 는 표기 형식만 본다). 3. census 는 여전히 `grep -c` **행 단위** 라 한 행에 2 hit 이면 1 로 세어 파일별 값은 **하한** 이다 (`ADR-0006`:23 이 실례 — 1 hit 이지만 좌표 2 개). 4. 수량 · 식별자 범위의 `R1` 적용 여부는 `§ 12.78` 한계 2 와 같이 조문 명문이 없는 **회색지대** 로 남는다. 5. FU14 anchor 좌표계로 이행하면 본 정규화가 anchor 표기에 흡수된다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S3 batch 3 = 잔여 혼용 ADR** (`ADR-0008` · `ADR-0013` · `ADR-0014` · `ADR-0018` · `ADR-0033` · `ADR-0035`) 또는 en dash 단독 (`ADR-0016` · `ADR-0017` · `ADR-0021`) 중 3 파일 — 다음 **1 순위**. (2) `L` 잔존 **28 ADR** + 비-ADR (`directory.md` · `integrator.md` · `reviewer.md` · `race-patterns.md` · `p3-*.md`) 의 `R5` 정규화. (3) **`§ 12.76 R5` 예시 갱신 후보** — `ADR-0005` 항목이 stale (본 절 AC 3 ③ · 조문 본문 편집이라 별도 판단 필요). (4) 수량 · 식별자 범위 표기 통일 판정 (낮음 유지). (5) `R2` · `R3` 병합 · anchor FU14 (낮음 유지).

### 12.80 범위 표기 규약 S3 batch 3 — 잔여 혼용 ADR 3 파일 (`ADR-0008` · `ADR-0013` · `ADR-0014`) `R1` · `R5` 정규화 (T-1482)

**위치 · 계보.** `§ 12.79` (S3 batch 2) **파생 영향 (1)** 이 지목한 **1 순위 = S3 batch 3 (잔여 혼용 ADR 중 3 파일)** 의 실집행 절이다. 규범 정본은 `§ 12.76` 조문 `R1` ~ `R7` 이고 대상 확장 논거는 `§ 12.79` AC 2 정정 (2) · AC 3 ② 를 그대로 승계한다.

#### AC 1 — 대상 선정 · 한정 소급 · mode 판정

**① 선정 근거.** `§ 12.79` 파생 (1) 의 잔여 혼용 ADR 6 후보 (`ADR-0008` · `ADR-0013` · `ADR-0014` · `ADR-0018` · `ADR-0033` · `ADR-0035`) 중 **ID 순 앞 3 파일** 을 취했다. 셋 다 `L` prefix 보유 (실측 15 · 13 · 7 행) 라 batch 1 (`R1` 단독) 이 아니라 batch 2 와 동형의 **`R1` + `R5` 이중 slice** 다.

**② 한정 소급 준수.** AC 2 census 가 대상을 **파일 · 행 단위로 특정** 하므로 `§ 12.76` AC 3 ③ 의 "이미 어긋남이 확인된 좌표에 한정" 을 충족한다. **전면 일괄 치환은 여전히 금지** 이며 3 파일 밖은 열지도 고치지도 않았다 (AC 7 `git status --short` 검산).

**③ mode 근거.** `CLAUDE.md` §3.1 **rule 5** (기존 `docs/decisions/*` 본문의 **비-결정 수정** = `direct`) 의 **세 번째 적용 선례** 다 (선례 `§ 12.78` ③ · `§ 12.79` ③). 편집은 구분자 · `L` prefix · 단위어 · 조사뿐이고 ADR 의 frontmatter `status` 행 · Decision · Consequences · Alternatives 실질은 **무편집** (AC 5 검산).

#### AC 2 — 후보 census 재실측 (집계 1 구)

`grep -nE '[0-9]+ *– *[0-9]+|L[0-9]+'` 행 단위 실측은 `ADR-0008` **15** · `ADR-0013` **13** · `ADR-0014` **7** (합 **35 행**) 로 Required Reading 기준값과 **차이 0** 이고, `grep -oE` 토큰 단위로는 **16 · 15 · 13 (합 44 좌표)** 다. `~` raw hit 은 **1 · 1 · 3** 이나 좌표 표기 기준으로는 **0 · 0 · 0** (판별은 AC 3 ①). **행 수 ≠ 좌표 수** — `§ 12.79` 한계 3 승계로 `grep -c` 는 한 행 2 hit 을 1 로 세어 행 단위 값은 **하한** 이다 (`ADR-0008`:149 · `ADR-0014`:14 · 16 · 120 이 실례).

#### AC 3 — `R5` 존치 조건 연동 판정

**① `~` hit 의 성격 판별.** raw hit 5 건은 전량 수량 · 식별자 범위다 — `ADR-0008`:24 `~5~6 task` (수량) · `ADR-0013`:75 `Decision §1~3` (절 식별자) · `ADR-0014`:14 · 118 `R-99~103` · 43 `0008~0013` (식별자). `§ 12.76` AC 3 ② 가 규약을 **문서의 좌표 표기 규약** 으로 한정하므로 **좌표 `~` 는 3 파일 모두 0 건** — 문언상 셋 다 존치 대상이었다 (`§ 12.79` AC 3 ① 판정 승계).

**② 그럼에도 3 파일 모두 정정.** 본 slice 가 같은 파일의 `R1` 후보 (`ADR-0008`:179 ~ 182 · `ADR-0013`:20 · 112 · `ADR-0014`:14) 를 정정하는 순간 **좌표 `~` 가 새로 유입** 돼 존치 근거가 소멸한다 — `§ 12.76` **한계 3** 이 예고한 "순수 파일에 `~` 가 새로 들어오면 그때는 `R1` 이 우선" 상황이다. 존치를 지키려면 `R1` 정정을 보류해야 하는데 이는 정본 1 순위를 뒤집는 선택이라 채택하지 않는다 (batch 2 선례 그대로). **반대 결론 파일 0**.

**③ 조문 예시 대조.** `§ 12.76 R5` 예시의 존치 파일 목록 (`p3-implementation-plan.md` · `ADR-0005` · `reviewer.md`) 에 본 slice 3 파일은 **미포함** 이라 **추가 stale 0** 이다 (`ADR-0005` 항목의 기존 stale 은 `§ 12.79` AC 3 ③ 이 이미 파생 영향으로 이월 — 조문 **본문은 편집하지 않는다**).

#### AC 4 · AC 5 — 대조표 (35 후보 행 · 유형별 압축) + 무편집 검산

| # | 파일 | 행 | 원 표기 | 판정 | 근거 조문 |
| --- | --- | --- | --- | --- | --- |
| 1 | `ADR-0008` | 17 · 25 · 30 ~ 33 · 41 · 156 · 183 (9 행 · 9 좌표) | link label 의 `L23` · `L32` · `L83` ~ `L86` · `L19` | **정정** → `23 행` 형태 | `R5` + `R4` |
| 2 | `ADR-0008` | 173 | Alternatives 표 본문의 `README L83` | **정정** → `83 행` | `R5` + `R4` |
| 3 | `ADR-0008` | 179 ~ 182 (4 행) | `L80–86` · `L15–25` · `L26–37` · `L39–53` | **정정** → `80~86 행` 형태 | `R1` + `R5` |
| 4 | `ADR-0008` | 149 | CI lint 회차 기록 (T-0087 round 1 lesson) 의 `L60` + `L230` | **면제 (무편집)** — 당시 판의 증거 · prettier 보정 전 좌표 | `R7` |
| 5 | `ADR-0013` | 14 · 28 ~ 30 · 40 · 50 · 56 · 78 · 101 · 103 · 110 (11 행 · 13 좌표) | `L84` · `L31` · `L33` · `L34` | **정정** → `84 행` 형태 | `R5` + `R4` |
| 6 | `ADR-0013` | 20 · 112 | `L31–34` | **정정** → `31~34 행` | `R1` + `R5` |
| 7 | `ADR-0014` | 14 · 16 · 43 · 114 · 118 ~ 120 (7 행 · 12 좌표) | `L85/L88` · `L31` · `L150` · `L172` | **정정** → `85/88 행` · `150 행` 형태 | `R5` + `R4` |
| 8 | `ADR-0014` | 14 | 코드 파일 좌표 `L33–40` (`llm-provider-config.repository.ts`) | **정정** → `33~40 행` — `R5` 가 문서 · 코드 구분 없음 | `R1` + `R5` |
| 9 | 3 파일 | `ADR-0008`:24 · `ADR-0013`:75 · `ADR-0014`:14 · 43 · 118 | `~5~6 task` · `§1~3` · `R-99~103` · `0008~0013` | **범위 밖 (무편집)** | AC 3 ① |

**`R6` · `R7` · `R4` 판별 1 구.** 인용 원문 병기 (`R6`) 는 **0 건**, 시점 기록 면제 (`R7`) 는 **1 행 (2 좌표)** = 위 #4 뿐이며, `L` 제거로 단일 행이 된 35 좌표에는 `R4` 를 적용해 `23~23` 형태를 만들지 않았다. 조사 보정은 **5 건** (`ADR-0013`:14 · 50 · 78 `가`→`이`, :40 `는`→`은`, `ADR-0014`:16 `는`→`은` — `L34 가` 가 `34 행` 뒤에서 비문이 되므로 필수).

**정정 후 재실측 1 구.** 행 수 **불변** (197 · 124 · 136), 링크 수 불변 (34 · 34 · 49), `## ` heading 수 불변 (5 · 5 · 5), `L` prefix 잔존 **1 행 (2 좌표) · 0 · 0** · en dash 잔존 **0 · 0 · 0** 으로 **면제 판정분 (#4) 과 정확히 동수** (신규 잔존 0).

**AC 5 무편집 검산 1 구.** `git diff --numstat` = `14/14` · `13/13` · `7/7` 로 **추가 행 수 = 삭제 행 수 (34/34)** 이고, 모든 hunk 가 위 대조표 행 안에 있어 `## Decision` · `## Consequences` · `## Alternatives` 의 **문장 실질 변경 0** · frontmatter `status` 행 hunk **0** 이다. `git diff --word-diff` 의 변경 토큰도 **구분자 · `L` prefix · 단위어 `행` · 조사 5 건** 뿐 (치환 좌표 **42**).

#### 진척 · 한계 · 파생 영향

**진척.** S3 batch 3 마감 — 잔여 혼용 ADR 3 파일 정규화 (후보 **35 행 / 44 좌표** 중 정정 **34 행 / 42 좌표** · 면제 **1 행 / 2 좌표** · 범위 밖 **5 좌표**), `R7` 면제가 `§ 12.76` 정본 승격 이후 **처음 실제 발동** (`§ 12.78` `R7` 해당 0 · `§ 12.79` 면제 0), `CLAUDE.md` §3.1 rule 5 **세 번째 선례** 확보. 남은 slice = S3 batch 4 이후 · S4 (조건부 · `pr`).

**한계.** 1. `R5` 잔여는 `grep -lE 'L[0-9]+' docs/decisions/ADR-*.md` 실측 **26 파일** 로 줄었다 (`§ 12.79` 한계 1 의 28 → 본 batch 로 `ADR-0013` · `ADR-0014` 2 파일 제거; `ADR-0008` 은 `R7` 면제 좌표를 남겨 **잔류**). 2. `L` 좌표가 가리키는 **값 자체의 정확성은 재검증하지 않았다** (`§ 12.74` 판정 승계 — 표기 형식만). 3. census 는 여전히 행 단위라 파일별 값은 **하한** 이다 (AC 2 실례 4 건). 4. 수량 · 식별자 범위의 `R1` 적용 여부는 `§ 12.78` 한계 2 · `§ 12.79` 한계 4 와 같이 조문 명문이 없는 **회색지대** 로 남는다. 5. `L85/L88` 류 **복수 좌표 병기 형식** (`85/88 행`) 은 조문에 명문이 없어 최소 변경 원칙으로 `/` 를 보존했다 — 표기 통일은 후속 판단. 6. FU14 anchor 좌표계 이행 시 본 정규화는 anchor 표기에 흡수된다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S3 batch 4 = 잔여 혼용 ADR** (`ADR-0018` · `ADR-0033` · `ADR-0035`) 또는 en dash 단독 (`ADR-0016` · `ADR-0017` · `ADR-0021`) 중 3 파일 — 다음 **1 순위**. (2) `L` 잔존 **26 ADR** + 비-ADR (`directory.md` · `integrator.md` · `reviewer.md` · `race-patterns.md` · `p3-*.md`) 의 `R5` 정규화. (3) `§ 12.76 R5` 예시 갱신 후보 (`§ 12.79` 파생 (3) 이월 · 본 slice 는 추가 stale 0). (4) 복수 좌표 병기 (`85/88 행`) · 수량 · 식별자 범위 표기 통일 판정 (낮음 유지). (5) `R2` · `R3` 병합 · anchor FU14 (낮음 유지).

**AC 7 자기 준수 · 범위 검산 1 구.** 신규 추가분 대상 자기 준수 grep = **11** (ADR 3 파일 몫 **0 · 0 · 0** — 본 절 대조표 · 판별 문단 **10** + task 파일 Follow-ups **1** 로 전량 **원 표기 인용** 이라 `R6` 예외). `wc -l` **7047 → 7101** (신설 **54 행** · 절 ≤ 60 충족), `git diff --stat` **5 파일 · 91 삽입 / 35 삭제** (≤ 300 LOC), `git status --short` 에 `README.md` · `CLAUDE.md` · `docs/requirements.md` · `docs/architecture/` · 다른 `docs/decisions/*` · `.claude/agents/` · `docs/LOOP.md` · `src/` **전무**, audit 파일 fence **178 (짝수)** · 신설 표 **6 컬럼 균일** · heading 순번 `12.79` → `12.80` 연속 · 링크 수 **360 불변**. doc-only 라 `pnpm test` 불요 (`CLAUDE.md` §3.2 direct doc-only 면제).

### 12.81 범위 표기 규약 S3 batch 4 — 잔여 혼용 ADR 3 파일 (`ADR-0018` · `ADR-0033` · `ADR-0035`) `R1` · `R5` 정규화 (T-1483)

**위치 · 계보.** `§ 12.80` (S3 batch 3) **파생 영향 (1)** 이 지목한 **1 순위 = S3 batch 4** 의 실집행 절이다. 두 후보군 중 **잔여 혼용 ADR 3 파일** 을 택했으며, 규범 정본은 `§ 12.76` 조문 `R1` ~ `R7` 이고 판정 선례는 `§ 12.79` AC 3 ① · `§ 12.80` AC 2 · AC 3 · 한계 5 를 승계한다.

#### AC 1 — 대상 선정 · 한정 소급 · mode · cap 판정

**① 선정 근거.** `§ 12.80` 파생 (1) 의 두 후보군 (잔여 혼용 3 · en dash 단독 3) 중 **혼용 3** 을 택했다 — 셋 다 `L` prefix 와 en dash 를 동시 보유해 batch 2 · 3 과 **동형의 `R1` + `R5` 이중 slice** 이고, 판정 선례 (`§ 12.79` · `§ 12.80`) 의 재사용도가 en dash 단독 군보다 높다. en dash 단독 3 은 batch 5 로 이월한다.

**② 한정 소급 준수.** AC 2 census 가 대상을 **파일 · 행 단위로 특정** 하므로 `§ 12.76` AC 3 ③ 의 "이미 어긋남이 확인된 좌표에 한정" 을 충족한다. **전면 일괄 치환은 여전히 금지** 이며 3 파일 밖은 열지도 고치지도 않았다 (AC 7 `git status --short` 검산).

**③ mode 근거.** `CLAUDE.md` §3.1 **rule 5** (기존 `docs/decisions/*` 본문의 **비-결정 수정** = `direct`) 의 **네 번째 적용 선례** 다 (선례 `§ 12.78` ③ · `§ 12.79` ③ · `§ 12.80` ③). 편집은 구분자 · `L` prefix · 단위어 · 조사뿐이고 ADR 의 frontmatter `status` 행 · Decision · Consequences · Alternatives 실질은 **무편집** (AC 5 검산).

**④ cap 가드 (이월 0).** 실측 정정 확정 좌표 **54** 로 임계 **70** 이하이고 ADR 3 파일 실 diff 는 **38/38 (76 LOC)** 라 본 절 · task 파일을 더해도 **250 LOC 미만** 이다 — 따라서 `ADR-0035` 를 batch 5 로 **이월하지 않고** 3 파일 전량을 본 slice 에서 마감했다.

#### AC 2 — 후보 census 재실측 (집계 1 구)

`grep -nE '[0-9]+ *– *[0-9]+|L[0-9]+'` 행 단위 실측은 `ADR-0018` **5** · `ADR-0033` **8** · `ADR-0035` **25** (합 **38 행**), `grep -oE` 토큰 단위는 **6 · 12 · 36** (합 **54 좌표**) 로 Required Reading 기준값과 **차이 0** 이다. raw `~` 는 **6 · 9 · 9** (합 **24**) 이고 그 중 **좌표 표기는 1 · 3 · 5** (합 **9**), 나머지 **15** 는 수량 · 식별자 범위다 (판별은 AC 3 ①). **행 수 ≠ 좌표 수** — `§ 12.80` AC 2 승계로 `grep -c` 는 한 행 다중 hit 을 1 로 세어 행 단위 값은 **하한** 이며, 본 batch 는 `ADR-0035` 에서 격차가 특히 크다 (25 행 / 36 좌표 — :37 · :179 가 각각 3 · 5 좌표).

#### AC 3 — `R5` 존치 조건 연동 + 회색지대 3 종 판정

**① 존치 판정 — 3 파일 모두 정정 (반대 결론 0).** batch 2 · 3 과 달리 본 3 파일은 **정정 전부터 좌표 `~` 를 보유** 한다 (`ADR-0018`:186 의 `L83~84` 1 · `ADR-0033`:140 의 개구간 3 · `ADR-0035` 의 `L61~L63` 류 5). 따라서 `§ 12.76 R5` 의 존치 조건 ("그 파일의 `~` 범위 표기가 0 건일 때만") 이 **처음부터 불충족** 이라 `§ 12.79` AC 3 ② · `§ 12.80` AC 3 ② 의 "`R1` 정정이 `~` 를 유입시켜 존치 근거가 소멸한다" 논거를 **동원할 필요조차 없다** (동원해도 결론 동일).

**② 회색지대 3 종 판정.** (가) **개구간** (`ADR-0033`:140 `L274~`) — 끝 좌표가 없어 `R1` (구분자) · `R4` (단일 행) 어느 조문에도 해당하지 않으므로 **`L` 만 제거 + 단위어 부착** 한 `274 행~` 을 택했다 (개구간 구조 보존 = 최소 변경 · `§ 12.80` 한계 5 의 복수 좌표 병기 처리와 동형). (나) **양끝 prefix 병기** (`L61~L63`) — 구분자가 이미 `~` 라 `R1` 은 충족이고 `R5` 로 양끝 `L` 을 제거해 `61~63 행` 으로 맞췄다 (7 건). (다) **코드 파일 대상 좌표** (`prisma/schema.prisma` · `src/user/assessment.service.ts` 등) — `R5` 본문의 "문서 · 코드 구분 없이" 문언으로 **정정 대상** 이며, `§ 12.76` AC 3 ② 의 "코드 주석 · 코드 내 문자열은 범위 밖" 은 **표기가 실린 위치** 기준이라 ADR 본문에 실린 본 건과 충돌하지 않는다 (대상 코드 파일 자체는 열지도 고치지도 않았다).

**③ 조문 예시 대조.** `§ 12.76 R5` 예시의 존치 파일 목록 (`p3-implementation-plan.md` · `ADR-0005` · `reviewer.md`) 에 본 slice 3 파일은 **미포함** 이라 **추가 stale 0** 이다 (`ADR-0005` 항목의 기존 stale 은 `§ 12.79` AC 3 ③ 이 이미 파생 영향으로 이월 — 조문 **본문은 편집하지 않는다**).

#### AC 4 · AC 5 — 대조표 (38 후보 행 · 유형별 압축) + 무편집 검산

| # | 파일 | 행 | 원 표기 | 판정 | 근거 조문 |
| --- | --- | --- | --- | --- | --- |
| 1 | `ADR-0018` | 31 · 32 · 33 (3 행 · 4 좌표) | REQ 매핑 본문의 `L31–33` · `L19–22, L33` · `L34` | **정정** → `31~33 행` · `19~22 행, 33 행` · `34 행` | `R1` + `R5` + `R4` |
| 2 | `ADR-0018` | 184 · 186 (2 행 · 2 좌표) | References link label 의 `L9–22` · `L83~84` | **정정** → `9~22 행` · `83~84 행` (URL 무편집) | `R1` + `R5` / `R5` |
| 3 | `ADR-0033` | 17 · 21 · 62 · 74 (4 행 · 5 좌표) | 코드 · schema 좌표 `L38–39` · `L28–29` · `L274–349` · `L17–20` · `L322` | **정정** → `38~39 행` · `322 행` 형태 | `R1` + `R5` + `R4` (회색 다) |
| 4 | `ADR-0033` | 31 · 32 · 145 (3 행 · 4 좌표) | README link label · References 의 `L56` · `L59` | **정정** → `56 행` · `59 행` | `R5` + `R4` |
| 5 | `ADR-0033` | 140 (1 행 · 3 좌표) | 개구간 `Assessment(L274~)` · `(L309~)` · `(L335~)` | **정정** → `274 행~` 형태 (개구간 보존) | `R5` (회색 가) |
| 6 | `ADR-0035` | 23 · 37 · 76 · 116 · 179 (5 행 · 11 좌표) | 양끝 병기 `L61~L63` · `L61~L62` 와 `L61–63` | **정정** → `61~63 행` · `61~62 행` | `R1` + `R5` (회색 나) |
| 7 | `ADR-0035` | 27 · 28 · 38 · 39 · 47 · 57 · 72 · 82 ~ 84 · 87 · 98 · 129 · 136 · 140 · 155 · 173 · 177 · 179 · 180 · 195 (20 행 · 25 좌표) | 단일 좌표 `L40` · `L45` · `L59` · `L61` ~ `L64` · `L72` · `L74` · `L97` 과 범위 `L341–355` | **정정** → `40 행` · `341~355 행` 형태 | `R5` + `R4` (+ `R1` 3 건) |
| 8 | 3 파일 | (편집 없음) | 수량 · 식별자 범위 raw `~` **15** (`T-0173~T-0182` · 축 `(1)~(5)` · `100` ~ `250` · `§Decision 1~4` · `0~3` · `28~31 일` 등) | **범위 밖 (무편집)** | AC 3 ① |

**`R6` · `R7` · `R4` 판별 1 구.** 인용 원문 병기 (`R6`) **0 건** · 시점 기록 면제 (`R7`) **0 건** — 후보 38 행 중 날짜 · round stamp 가 박히거나 당시 판의 증거로 기능하는 문장은 없고, 시점성이 의심된 `ADR-0035`:195 (P7 backlog `- [ ]` 항목) 도 **미래 작업을 가리키는 살아있는 pointer** 라 `R7` 비해당으로 명시 판별해 정정했다. `L` 제거로 단일 행이 된 좌표에는 `R4` 를 적용해 `40~40` 형태를 만들지 않았고, 조사 보정은 **1 건** (`ADR-0035`:23 `가`→`이` — `L63 가` 가 `63 행` 뒤에서 비문이 되므로 필수).

**정정 후 재실측 1 구.** 행 수 **불변** (208 · 157 · 196), 링크 수 불변 (76 · 29 · 38), `## ` heading 수 불변 (5 · 6 · 6), `L` prefix 잔존 **0 · 0 · 0** · en dash 잔존 **0 · 0 · 0** 으로 면제 · 범위 밖 판정분 (`L` · en dash 해당 **0 건**) 과 **정확히 동수** 다 — `§ 12.80` (면제 1 행 잔류) 과 달리 본 batch 는 3 파일 전량 잔존 0.

**AC 5 무편집 검산 1 구.** `git diff --numstat` = `5/5` · `8/8` · `25/25` 로 **추가 행 수 = 삭제 행 수 (38/38)** 이고, 모든 hunk 가 위 대조표 행 안에 있어 `## Decision` · `## Consequences` · `## Alternatives` 의 **문장 실질 변경 0** · frontmatter `status` · `date` 행 hunk **0** (최소 hunk 좌표 17) 이다. `git diff --word-diff` 의 변경 토큰도 **구분자 · `L` prefix · 단위어 `행` · 조사 1 건** 뿐 (치환 좌표 **54**).

#### 진척 · 한계 · 파생 영향

**진척.** S3 batch 4 마감 — 잔여 혼용 ADR 3 파일 정규화 (후보 **38 행 / 54 좌표 전량 정정** · 면제 **0** · 범위 밖 raw `~` **15**), **회색지대 3 종** (개구간 · 양끝 prefix 병기 · 코드 파일 좌표) 의 첫 실판정 확보, `CLAUDE.md` §3.1 rule 5 **네 번째 선례**. 혼용 ADR 재고 **0**. 남은 slice = S3 batch 5 (en dash 단독) 이후 · S4 (조건부 · `pr`).

**한계.** 1. `R5` 잔여는 `grep -lE 'L[0-9]+' docs/decisions/ADR-*.md` 실측 **23 파일** 로 줄었다 (`§ 12.80` 한계 1 의 26 → 본 batch 3 파일이 잔존 0 으로 전량 제거); en dash 잔여 ADR 은 **5 파일**. 2. `L` 좌표가 가리키는 **값 자체의 정확성은 재검증하지 않았다** (`§ 12.74` 판정 승계 — 표기 형식만). 3. census 는 여전히 행 단위라 파일별 행 값은 **하한** 이다. 4. 회색지대 3 종은 조문 명문 없이 **판정 선례로만** 존재한다 — `R8` 조문화는 선례 2 ~ 3 slice 후 판단 (`§ 12.80` 한계 4 · 5 승계). 5. 개구간 `274 행~` 형태는 본 절이 처음 도입한 표기라 후속 slice 가 다른 형태를 택하면 불일치 risk 가 있다. 6. FU14 anchor 좌표계 이행 시 본 정규화는 anchor 표기에 흡수된다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S3 batch 5 = en dash 단독 ADR** (`ADR-0016` · `ADR-0017` · `ADR-0021`) — 다음 **1 순위** (혼용 재고 0 이라 후보군이 en dash 단독 · `L` 단독 둘로 좁혀졌다). (2) `L` 잔존 **23 ADR** + 비-ADR (`directory.md` · `integrator.md` · `reviewer.md` · `race-patterns.md` · `p3-*.md`) 의 `R5` 정규화. (3) `§ 12.76 R5` 예시 갱신 후보 (`§ 12.79` 파생 (3) 이월 · 본 slice 는 추가 stale 0). (4) 회색지대 3 종 (`R8`) 조문화 판정 — 선례 3 건 확보로 우선순위 **중간** 으로 상향. (5) `R2` · `R3` 병합 · anchor FU14 (낮음 유지).

**AC 7 자기 준수 · 범위 검산 1 구.** 신규 추가분 대상 자기 준수 grep = **10** (ADR 3 파일 · task 파일 몫 **0 · 0 · 0 · 0** — 전량 본 절 대조표 · 판별 문단의 **원 표기 인용** 이라 `R6` 예외). `wc -l` **7101 → 7156** (신설 **55 행** · 본문 **54 행** 으로 절 ≤ 60 충족), `git diff --stat` **5 파일 · 95 삽입 / 39 삭제** (≤ 300 LOC), `git status --short` 에 `README.md` · `CLAUDE.md` · `docs/requirements.md` · `docs/architecture/` · 다른 `docs/decisions/*` · `.claude/agents/` · `docs/LOOP.md` · `src/` · `prisma/` **전무**, audit 파일 fence **178 (짝수)** · 신설 표 **6 컬럼 균일** · heading 순번 `12.80` → `12.81` 연속 · 링크 수 **360 불변**. doc-only 라 `pnpm test` 불요 (`CLAUDE.md` §3.2 direct doc-only 면제).

### 12.82 범위 표기 규약 S3 batch 5 — en dash 단독 ADR 3 파일 (`ADR-0016` · `ADR-0017` · `ADR-0021`) `R1` · `R5` 정규화 (T-1484)

**위치 · 계보.** `§ 12.81` (S3 batch 4) **파생 영향 (1)** 이 **1 순위** 로 지목한 **S3 batch 5 = en dash 단독 ADR 3 파일** 의 실집행 절이다. 규범 정본은 `§ 12.76` 조문 `R1` ~ `R7` 이고, 판정 선례는 `§ 12.79` AC 3 ② · `§ 12.80` AC 2 · 한계 5 · `§ 12.81` AC 3 ② 를 승계한다.

#### AC 1 — 대상 선정 · 한정 소급 · mode · cap 판정

**① 선정 근거.** `§ 12.81` 파생 (1) 의 1 순위 지목 (혼용 재고 0 이후 남은 두 후보군 중 en dash 단독 군) 을 그대로 집행했다. ADR **3** + 본 audit + task 파일 = **정확히 5 파일** 이라 후보를 하나라도 더 얹으면 `CLAUDE.md` §3 의 5 파일 cap 위반이므로, 잔여 en dash ADR (`ADR-0001` · `ADR-0003`) 은 batch 6 으로 이월했다.

**② 한정 소급 준수.** AC 2 census 가 대상을 **파일 · 행 단위로 특정** 하므로 `§ 12.76` AC 3 ③ 의 "이미 어긋남이 확인된 좌표에 한정" 을 충족한다. **전면 일괄 치환은 여전히 금지** 이며 3 파일 밖은 열지도 고치지도 않았다 (AC 7 `git status --short` 검산).

**③ mode 근거.** `CLAUDE.md` §3.1 **rule 5** (기존 `docs/decisions/*` 본문의 **비-결정 수정** = `direct`) 의 **다섯 번째 적용 선례** 다 (선례 `§ 12.78` ③ · `§ 12.79` ③ · `§ 12.80` ③ · `§ 12.81` ③). 편집은 구분자 · `L` prefix · 단위어뿐이고 ADR 의 `## Decision` · `## Consequences` · `## Alternatives` 실질과 frontmatter `status` · `date` 는 **무편집** (AC 5 검산).

**④ cap 가드 (이월 0).** 실측 정정 확정 좌표 **15** 로 임계 **70** 이하이고 ADR 3 파일 실 diff 는 **14/14 (28 LOC)** 라 본 절 · task 파일을 더해도 **250 LOC 미만** 이다 — 따라서 `ADR-0017` 을 batch 6 으로 **이월하지 않고** 3 파일 전량을 본 slice 에서 마감했다.

#### AC 2 — 후보 census 재실측 (집계 1 구)

`grep -nE '[0-9]+ *– *[0-9]+|L[0-9]+'` 행 단위 실측은 `ADR-0016` **4** · `ADR-0017` **7** · `ADR-0021` **3** (합 **14 행**), `grep -oE` 토큰 단위는 **4 · 7 · 4** (합 **15 좌표**) 로 Required Reading 기준값과 **차이 0** 이다. raw `~` 는 **2 · 1 · 2** (합 **5**) 이고 그중 **좌표 표기는 0 · 0 · 0** (합 **0**) — 5 건 전량이 식별자 범위 (`ADR-0001 ~ ADR-0015` · `~ ADR-0016` · `~ ADR-0020`) 또는 열거 번호 범위 (축 `(1)~(5)` · `Decision §(i)~(v)`) 라 규약 범위 밖이며, 이 **좌표 `~` = 0** 이 AC 3 ① 존치 판정의 직접 입력이다. **행 수 ≠ 좌표 수** — `§ 12.80` AC 2 승계로 `grep -c` 는 한 행 다중 hit 을 1 로 세어 행 단위 값은 **하한** 이고, 본 batch 의 격차는 `ADR-0021`:37 **1 곳** (1 행 / 2 좌표) 뿐이다.

#### AC 3 — `R5` 존치 조건 실판정 + 회색지대 승계 2 종 + 신규 변형 1 종

**① 존치 조건 실동원 — 3 파일 모두 정정 (반대 결론 0).** batch 4 (`§ 12.81` AC 3 ①) 와 달리 본 3 파일은 정정 전 좌표 `~` 가 **0 건** 이라 `§ 12.76 R5` 존치 조건 ("그 파일의 `~` 범위 표기가 0 건일 때만") 이 **형식상 성립** 한다. 그럼에도 정정으로 결론내는 근거는 두 축이다 — (가) 본 slice 의 `R1` 정정 (en dash → `~`) 이 같은 파일에 좌표 `~` 를 **유입** 시키므로 존치 근거가 **자기 소멸** 한다 (`§ 12.79` AC 3 ② · `§ 12.80` AC 3 ② 논거를 본 batch 가 처음으로 **실제 동원**), (나) `R5` 존치 예시 목록 3 파일 (`p3-implementation-plan.md` · `ADR-0005` · `reviewer.md`) 에 본 3 파일은 **미포함** 이다. 두 축이 같은 결론이라 무편집 존치로 남긴 파일은 **0** 이다.

**② 회색지대 승계 2 종.** (다) **코드 파일 대상 좌표** (`ADR-0017`:16 의 `github-adapter.service.ts L24–25`, :37 ~ :39 의 `llm-apikey-cipher.service.ts L48–73` · `auth.module.ts L60–73` · `llm-live-test-gating.ts L54–93`) — `R5` 본문의 "문서 · 코드 구분 없이" 로 **정정 대상** 이고, `§ 12.76` AC 3 ② 의 "코드 주석 · 코드 내 문자열은 범위 밖" 은 **표기가 실린 위치** 기준이라 ADR 본문에 실린 본 건과 충돌하지 않는다 (대상 코드 파일은 열지도 고치지도 않았다 · `§ 12.81` 회색 (다) 승계). **복수 좌표 병기** (`ADR-0021`:37 의 `L19–22, L33`) 는 `§ 12.80` 한계 5 의 최소 변경 처리를 승계해 **각 좌표를 개별 정정** (`19~22 행, 33 행`) 하고 병기 구조는 보존했다.

**③ 신규 변형 1 종 — 규약 범위 밖 문서 대상 좌표.** `ADR-0016`:18 · :155 · `ADR-0017`:130 의 `[docs/PLAN.md L81](../PLAN.md)` 는 `docs/PLAN.md` 가 `§ 12.76` AC 3 ② 의 **5 문서군 밖** 이지만, 회색 (다) 와 **동형** 으로 판정 기준은 **표기가 실린 위치** (ADR 본문 = 범위 안) 이므로 **정정 대상** 이다 (`81 행`). 링크 label 안이라 **URL 은 무편집** 이며 `grep -c '](' ` 불변으로 검산했다. **④ 조문 예시 대조** — `R5` 예시의 존치 파일 목록에 본 3 파일이 미포함이라 **추가 stale 0** 이고, 기존 stale (`ADR-0005`) 은 `§ 12.79` AC 3 ③ 이 이미 이월했다 (조문 **본문은 편집하지 않는다**).

#### AC 4 · AC 5 — 대조표 (14 후보 행 · 유형별 압축) + 무편집 검산

| # | 파일 | 행 | 원 표기 | 판정 | 근거 조문 |
| --- | --- | --- | --- | --- | --- |
| 1 | `ADR-0016` · `ADR-0017` | 18 · 155 · 130 (3 행 · 3 좌표) | 링크 label 의 `[docs/PLAN.md L81]` | **정정** → `[docs/PLAN.md 81 행]` (URL 무편집) | `R5` + `R4` (신규 변형 ③) |
| 2 | 3 파일 | 33 · 34 · 30 · 31 · 35 · 36 (6 행 · 6 좌표) | REQ 매핑 본문의 `README L7–18` · `L19–22` · `L31–33` | **정정** → `7~18 행` · `19~22 행` · `31~33 행` | `R1` + `R5` |
| 3 | `ADR-0021` | 37 (1 행 · 2 좌표) | 복수 좌표 병기 `L19–22, L33` | **정정** → `19~22 행, 33 행` (병기 구조 보존) | `R1` + `R5` + `R4` (회색 다·병기) |
| 4 | `ADR-0017` | 16 (1 행 · 1 좌표) | 코드 파일 좌표 `github-adapter.service.ts L24–25` | **정정** → `24~25 행` | `R1` + `R5` (회색 다) |
| 5 | `ADR-0017` | 37 · 38 · 39 (3 행 · 3 좌표) | 코드 파일 좌표 `L48–73` · `L60–73` · `L54–93` | **정정** → `48~73 행` 형태 | `R1` + `R5` (회색 다) |
| 6 | 3 파일 | (편집 없음) | 식별자 · 열거 범위 raw `~` **5** (`ADR-0001 ~ ADR-0015` · 축 `(1)~(5)` · `§(i)~(v)` 등) | **범위 밖 (무편집)** | AC 2 · AC 3 ① |

**`R6` · `R7` · `R4` 판별 1 구.** 인용 원문 병기 (`R6`) **0 건** · 시점 기록 면제 (`R7`) **0 건** — 시점성이 의심되어 명시 판별 의무가 걸린 References 목록 행 (`ADR-0016`:155 · `ADR-0017`:130) 도 날짜 · round stamp 없이 `docs/PLAN.md` Phase P4 bullet 을 가리키는 **살아있는 pointer** 라 `R7` 비해당으로 정정했다. 단일 행 좌표 (`L81` · `L33`) 에는 `R4` 를 적용해 `81~81` 형태를 만들지 않았고, `L` 제거로 비문이 된 조사가 없어 **조사 보정 0 건** 이다.

**정정 후 재실측 1 구.** 행 수 **불변** (172 · 139 · 187), 링크 수 불변 (56 · 55 · 56), `## ` heading 수 불변 (5 · 5 · 5), `L` prefix 잔존 **0 · 0 · 0** · en dash 잔존 **0 · 0 · 0** 으로 면제 · 범위 밖 판정분 (`L` · en dash 해당 **0 건**) 과 **정확히 동수** 다 (`§ 12.81` 과 동일하게 전량 잔존 0).

**AC 5 무편집 검산 1 구.** `git diff --numstat` = `4/4` · `7/7` · `3/3` 으로 **추가 행 수 = 삭제 행 수 (14/14)** 이고, hunk 좌표 (16 · 18 · 30 ~ 39 · 130 · 155) 가 전부 `## Context` (14 행~) 와 `## References` (152 · 119 · 170 행~) 구간의 대조표 행 안이라 `## Decision` · `## Consequences` · `## Alternatives` hunk **0** · frontmatter (1 ~ 8 행) `status` · `date` hunk **0** 이다. `git diff --word-diff` 의 변경 토큰도 **구분자 · `L` prefix · 단위어 `행`** 뿐 (치환 좌표 **15** · 조사 보정 0).

#### 진척 · 한계 · 파생 영향

**진척.** S3 batch 5 마감 — en dash 단독 ADR 3 파일 정규화 (후보 **14 행 / 15 좌표 전량 정정** · 면제 **0** · 범위 밖 raw `~` **5**), **`R5` 존치 조건의 첫 실동원 판정** (형식상 성립 → `R1` 유입에 의한 자기 소멸 논거로 정정 확정), **신규 변형 1 종** (규약 범위 밖 문서 대상 좌표) 판정 확보, `CLAUDE.md` §3.1 rule 5 **다섯 번째 선례**. 남은 slice = S3 batch 6 (`ADR-0003` 등) 이후 · S4 (조건부 · `pr`).

**한계.** 1. `R5` 잔여는 `grep -lE 'L[0-9]+' docs/decisions/ADR-*.md` 실측 **20 파일** 로 줄었고 (`§ 12.81` 한계 1 의 23 → 본 batch 3 파일 전량 제거), en dash 잔여 ADR 은 **2 파일** (`ADR-0001` · `ADR-0003` — 5 → 2). 2. 좌표가 가리키는 **값 자체의 정확성은 재검증하지 않았다** (`§ 12.74` 판정 승계 — 표기 형식만). 3. census 는 여전히 행 단위라 파일별 행 값은 **하한** 이다. 4. 신규 변형 (범위 밖 문서 대상) 은 **선례 1 건** 뿐이라 조문화 판단은 `R8` 묶음으로 이월한다. 5. 회색지대는 이제 4 종 (개구간 · 양끝 prefix 병기 · 코드 파일 좌표 · 범위 밖 문서 좌표) 으로 늘었고 여전히 조문 명문 없이 **판정 선례로만** 존재한다. 6. FU14 anchor 좌표계 이행 시 본 정규화는 anchor 표기에 흡수된다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S3 batch 6 = en dash 잔여 `ADR-0003`** (좌표 3 행) + `L` 잔존 ADR 2 ~ 3 파일 묶음 — 다음 **1 순위** (`ADR-0001` 의 `100–200명` · `50–100개` 는 수량 범위라 범위 밖 판정 후보). (2) `L` 잔존 **20 ADR** + 비-ADR (`directory.md` · `integrator.md` · `reviewer.md` · `race-patterns.md` · `p3-*.md`) 의 `R5` 정규화. (3) `§ 12.76 R5` 예시 갱신 후보 (`§ 12.79` 파생 (3) 이월 · 본 slice 추가 stale 0). (4) 회색지대 **4 종** (`R8`) 조문화 판정 — 선례 4 건 확보로 우선순위 **중간** 유지. (5) `R2` · `R3` 병합 · anchor FU14 (낮음 유지).

**AC 7 자기 준수 · 범위 검산 1 구.** 신규 추가분 대상 자기 준수 grep = **9** (ADR 3 파일 · task 파일 몫 **0 · 0 · 0 · 0** — 전량 본 절 대조표 · 판별 문단의 **원 표기 인용** 이라 `R6` 예외). `wc -l` **7156 → 7209** (신설 **53 행** · 절 본문 **52 행** 으로 ≤ 60 충족), `git diff --stat` **5 파일 · 79 삽입 / 22 삭제** (≤ 300 LOC), `git status --short` 에 `README.md` · `CLAUDE.md` · `docs/requirements.md` · `docs/PLAN.md` · `docs/architecture/` · 다른 `docs/decisions/*` · `.claude/agents/` · `docs/LOOP.md` · `src/` · `prisma/` **전무**, audit 파일 fence **178 (짝수)** · 신설 표 **6 컬럼 균일** · heading 순번 `12.81` → `12.82` 연속 · 대상 ADR 링크 수 **56 · 55 · 56 불변**. doc-only 라 `pnpm test` 불요 (`CLAUDE.md` §3.2 direct doc-only 면제).

### 12.83 범위 표기 규약 S3 batch 6 — en dash 잔여 2 + `L` 순수 1 ADR (`ADR-0001` · `ADR-0003` · `ADR-0011`) `R1` · `R5` · `R6` 판정 — 후보 7 행 중 정정 **4 행** · `R6` 면제 **2 행** · 범위 밖 **1 행** (T-1485)

**위치 · 계보.** `§ 12.82` (S3 batch 5) **파생 영향 (1)** 이 **1 순위** 로 지목한 **S3 batch 6 = en dash 잔여 + `L` 잔존 ADR 묶음** 의 실집행 절이다. 규범 정본은 `§ 12.76` 조문 `R1` ~ `R7` 이고 판정 선례는 `§ 12.79` AC 3 ②③ · `§ 12.80` AC 2 · 한계 5 · `§ 12.81` AC 3 ① · `§ 12.82` AC 3 ①③ 을 승계한다. 본 batch 는 "잔여를 다 고치는" slice 가 아니라 **면제 · 범위 밖을 실판정해 en dash 축을 닫는** slice 다.

#### AC 1 — 대상 선정 · 한정 소급 · mode · cap 판정

**① 선정 근거.** `§ 12.82` 파생 (1) 이 1 순위로 지목한 en dash 잔여 2 파일 (`ADR-0001` · `ADR-0003`) 에 `L` 순수 파일 1 (`ADR-0011` — `~` **0** · en dash **0**) 을 얹어 3 파일로 편성했다. `ADR-0011` 을 택한 이유는 `R5` 존치 조건을 **자기 소멸 논거 없이** 시험할 수 있는 첫 파일이기 때문이다. ADR **3** + 본 audit + task 파일 = **정확히 5 파일** 이라 후보를 더 얹으면 `CLAUDE.md` §3 의 5 파일 cap 위반이다.

**② 한정 소급 준수.** AC 2 census 가 대상을 **파일 · 행 단위로 특정** 하므로 `§ 12.76` AC 3 ③ 의 "이미 어긋남이 확인된 좌표에 한정" 을 충족한다. **전면 일괄 치환은 여전히 금지** 이며 3 파일 밖은 열지도 고치지도 않았다 (AC 7 `git status --short` 검산).

**③ mode 근거.** `CLAUDE.md` §3.1 **rule 5** (기존 `docs/decisions/*` 본문의 **비-결정 수정** = `direct`) 의 **여섯 번째 적용 선례** 다 (선례 `§ 12.78` ③ · `§ 12.79` ③ · `§ 12.80` ③ · `§ 12.81` ③ · `§ 12.82` ③). 편집은 `L` prefix 제거 + 단위어 부착뿐이고 ADR 의 결정 실질과 frontmatter `status` · `date` 는 **무편집** (AC 5 검산).

**④ cap 가드 (이월 0).** 확정 정정 좌표는 **4** 로 임계 **70** 을 크게 밑돌고 ADR 3 파일 실 diff 는 **4/4 (8 LOC)** 라 이월 없이 3 파일 전량을 본 slice 에서 마감했다 — 본 batch 의 산출 부피는 정정이 아니라 **판정 문단** 쪽에 있다.

#### AC 2 — 후보 census 재실측 (집계 1 구)

`grep -nE '[0-9]+ *– *[0-9]+|L[0-9]+'` 행 단위 실측은 `ADR-0001` **1** · `ADR-0003` **3** · `ADR-0011` **3** (합 **7 행**), `grep -oE` 토큰 단위는 **2 · 3 · 3** (합 **8 좌표**) 로 Required Reading 기준값 (`1` · `3` · `3` 행) 과 **차이 0** 이다. raw `~` 는 **6 · 13 · 0** (합 **19**) 이고 그중 **좌표 표기는 4 · 6 · 0** (합 **10**), 나머지 **9** 는 수량 (`100~200` · `50~100` 각 3 쌍) · 식별자 (`T-0002~T-0005` · `REQ-005~007`) · 근사 기호 1 이다. **행 수 ≠ 좌표 수** — `§ 12.80` AC 2 승계로 `grep -c` 는 한 행 다중 hit 을 1 로 세어 행 단위 값은 **하한** 이며, 본 batch 의 격차는 `ADR-0001`:20 **1 곳** (1 행 / 2 좌표) 뿐이다.

#### AC 3 — 판정 3 종 독립 결론

**① `ADR-0001` — 수량 범위라 규약 범위 밖 (무편집).** `:20` 원문은 "평가 대상자 100–200명, GitHub repo 50–100개, Confluence page 1000여건" 으로, 두 en dash 좌표 뒤에 단위어 **`명` · `개`** 가 붙은 **수량 범위** 이지 행 좌표가 아니다. `§ 12.76` AC 3 ② 는 규약을 "**문서의 좌표 표기 규약**" 으로 한정하므로 본 2 좌표는 **범위 밖** 이고 (`§ 12.79` AC 3 ① 의 `100~200명` · `50~100 repo` 무편집 선례와 동형), 같은 행의 행 좌표 `88~92행` 은 이미 `R1` 충족이라 `ADR-0001` 은 **파일 전체 무편집** 이다.

**② `ADR-0003` — en dash 2 행은 `R6` 면제 · `L72` 1 행은 정정.** `:21` 의 `(README 33–41 행 표기 · 실 대응 31 · 34 행 불연속)` 은 `§ 12.76 R6` 의 **예시 문자열과 문자 그대로 동일** 하다 — `§ 12.73` AC 2 가 확정한 병기 화법이 그대로 실린 자리이고 en dash 는 **인용된 원문 (README) 표기** 라 `R6` 단서 ("예시 안의 `–` 는 인용된 원문 표기라 R1 적용 대상이 아니다") 로 **면제 (무편집)**. `:168` References 행의 `33–41 표기 · 실 대응 31 · 34 불연속` 은 단위어 `행` 만 생략된 **압축형 동형** 이라 같은 근거로 **면제 (무편집)** — 두 행 모두 정정하면 병기가 가리키는 "원문이 이렇게 적혀 있다" 는 사실 자체가 훼손된다. 반면 `:74` 의 `L72` 는 인용이 아니라 **살아있는 단일 행 pointer** 라 `R5` (`L` 미사용) + `R4` (`72~72` 금지) 로 **정정** 했다.

**③ `ADR-0011` — 존치 조건 성립에도 정정 (자기 소멸 논거 미동원).** 본 파일은 `~` **0** · en dash **0** 이라 `§ 12.76 R5` 존치 조건 ("그 파일의 `~` 범위 표기가 0 건일 때만") 이 성립하고, 정정 대상 3 좌표가 전부 **단일 행** (`L86` → `86 행`) 이라 `R1` 정정에 의한 `~` 유입도 없어 `§ 12.82` AC 3 ① 의 **자기 소멸 논거를 동원할 수 없는 첫 파일** 이다. 그럼에도 정정으로 결론낸 근거는 두 축이다 — (가) `R5` 존치 예시 목록 3 파일 (`p3-implementation-plan.md` · `ADR-0005` · `reviewer.md`) 은 **예시적 (열거적이 아님)** 이다: 조문이 목록을 `예:` 괄호 안에 두었고 `§ 12.76` 한계 3 이 이를 "현 시점 **스냅숏**" 으로 명시했으며 `§ 12.79` AC 3 ③ 이 실제로 `ADR-0005` 항목을 stale 로 만들었다. 게다가 조문 문언은 존치를 "**허용한다**" 이지 강제하지 않으므로 `R5` 본문 1 순위 규범 (`L` prefix 미사용) 이 우선한다. (나) 대상이 5 문서군 **밖** 문서 (`docs/PLAN.md`) 를 가리키는 좌표라는 점은 `§ 12.82` AC 3 ③ 의 **두 번째 선례** 로, 판정 기준은 **표기가 실린 위치** (ADR 본문 = 범위 안) 라 정정 대상이다 — 같은 `[docs/PLAN.md L81]` 표기를 `§ 12.82` 가 이미 정정했으므로 존치는 규약 내 불일치를 낳는다.

#### AC 4 · AC 5 — 대조표 (7 후보 행 · 유형별 압축) + 무편집 검산

| # | 파일 | 행 | 원 표기 | 판정 | 근거 조문 |
| --- | --- | --- | --- | --- | --- |
| 1 | `ADR-0001` | 20 (1 행 · 2 좌표) | 수량 범위 `100–200명` · `50–100개` | **범위 밖 (무편집)** | `§ 12.76` AC 3 ② (AC 3 ①) |
| 2 | `ADR-0003` | 21 (1 행 · 1 좌표) | `(README 33–41 행 표기 · 실 대응 31 · 34 행 불연속)` | **면제 (무편집)** — 조문 예시와 문자 동일 | `R6` |
| 3 | `ADR-0003` | 168 (1 행 · 1 좌표) | References 압축형 `33–41 표기 · 실 대응 31 · 34 불연속` | **면제 (무편집)** — 동형 병기 | `R6` |
| 4 | `ADR-0003` | 74 (1 행 · 1 좌표) | Decision 본문의 `[README.md](../../README.md) L72 의 예시` | **정정** → `72 행 의 예시` | `R5` + `R4` |
| 5 | `ADR-0011` | 14 · 28 · 108 (3 행 · 3 좌표) | 링크 label 의 `[docs/PLAN.md L86]` | **정정** → `[docs/PLAN.md 86 행]` (URL 무편집) | `R5` + `R4` (범위 밖 문서 좌표 선례 2) |
| 6 | 3 파일 | (편집 없음) | raw `~` **19** 중 수량 · 식별자 · 근사 **9** (`100~200` · `50~100` · `T-0002~T-0005` · `REQ-005~007`) | **범위 밖 (무편집)** | AC 2 · `§ 12.79` AC 3 ① |

**`R6` · `R7` · `R4` 판별 1 구.** 인용 원문 병기 (`R6`) **2 행** — 본 축 최초의 `R6` 실발동이다 (`§ 12.78` ~ `§ 12.82` 는 전부 0). 시점 기록 면제 (`R7`) **0 건** — `:74` 는 날짜 · round stamp 없이 README 의 cron 예시를 가리키는 살아있는 pointer 라 `R7` 비해당이다. 단일 행 좌표 (`L72` · `L86`) 4 건에는 `R4` 를 적용해 `72~72` 형태를 만들지 않았고, `L72 의` → `72 행 의` 는 후행 조사가 그대로 자연스러워 (`ADR-0033`:62 `17~20 행 의` 선례 동형) **조사 보정 0 건** 이다.

**정정 후 재실측 1 구.** 행 수 **불변** (146 · 173 · 123), 링크 수 불변 (19 · 17 · 42), `## ` heading 수 불변 (6 · 6 · 5), `L` prefix 잔존 **0 · 0 · 0**, en dash 잔존 **2 · 2 · 0** (합 4) 으로 면제 (`ADR-0003` 2) + 범위 밖 (`ADR-0001` 2) 판정분 **4 와 정확히 동수** — 미판정 잔존 0.

**AC 5 무편집 검산 1 구.** `git diff --numstat` = `0/0` · `1/1` · `3/3` 으로 **추가 행 수 = 삭제 행 수 (4/4)** 이고 (`ADR-0001` 은 무편집이라 `0/0`), `git diff --word-diff` 변경 토큰도 **`L` prefix 제거 + 단위어 `행`** 뿐이다. hunk 좌표는 `ADR-0011`:14 · 28 (`## Context` 12 행~) · :108 (`## References` 106 행~) 3 건이 비-결정 구간이고, `ADR-0003`:74 **1 건** 은 본 축 최초로 **`## Decision` (28 ~ 91 행) 구간 안** 에 떨어졌다 — `CLAUDE.md` §3.1 rule 5 의 경계는 **구간** 이 아니라 **결정 실질** 이므로 (문언: "결정 내용 자체 … 의 실질") 표기 정규화 1 토큰인 본 hunk 는 여전히 비-결정 수정이다 (문장 · 수치 · 링크 URL 무변경). `## Consequences` · `## Alternatives` hunk **0** · frontmatter (`status` · `date`) hunk **0**.

#### AC 6 — en dash 축 마감 선언 + 잔여 갱신

정정 후 `grep -lE '[0-9] *– *[0-9]' docs/decisions/ADR-*.md` = **2 파일** (`ADR-0001` · `ADR-0003` — `§ 12.82` 한계 1 의 값 **2** 와 동일 · 감소 0), `grep -lE 'L[0-9]+' docs/decisions/ADR-*.md` = **18 파일** (`§ 12.82` 한계 1 의 **20** → `ADR-0003` · `ADR-0011` 2 파일 제거). **en dash 축 마감을 선언한다** — 잔존 4 좌표 전량이 `R6` 면제 **2** + 수량 범위 **2** 로 실판정 완료돼 **정정 대상 재고 0** 이고, 파일 수 2 가 남는 것은 미처리가 아니라 **판정 결과** 다. 따라서 batch 7 이후로 넘기는 en dash 잔여는 **없다** (넘기는 것은 `L` 축 18 파일뿐).

#### 진척 · 한계 · 파생 영향

**진척.** S3 batch 6 마감 — 후보 **7 행 / 8 좌표** 중 정정 **4 행 / 4 좌표** · `R6` 면제 **2 행** · 범위 밖 **1 행 (2 좌표)**, **en dash 축 마감 선언** (정정 재고 0), `R6` 면제의 **첫 실발동**, `R5` 존치 조건을 **자기 소멸 논거 없이** 뒤집은 첫 판정 (예시 목록 = 예시적 · 존치는 재량), 수량 범위 · 범위 밖 문서 좌표 선례 각 1 추가, `CLAUDE.md` §3.1 rule 5 **여섯 번째 선례** (+ Decision 구간 hunk 의 rule 5 경계 판정). 남은 slice = S3 batch 7 (`L` 축) 이후 · S4 (조건부 · `pr`).

**한계.** 1. en dash 축 마감은 **본 절 기록에 의존** 한다 — `grep -lE` 회귀 감시는 앞으로도 2 파일을 계속 반환하므로 자동 검출로는 마감 여부를 구분할 수 없다 (판정 근거는 본 절 AC 3 ①② · AC 6). 2. 좌표가 가리키는 **값 자체의 정확성은 재검증하지 않았다** (`§ 12.74` 판정 승계 — 표기 형식만). 3. census 는 여전히 행 단위라 파일별 값은 **하한** 이다 (`ADR-0001`:20 이 실례). 4. 회색지대는 **5 종** (개구간 · 양끝 prefix 병기 · 코드 파일 좌표 · 범위 밖 문서 좌표 · **수량 vs 좌표 범위 판별**) 으로 늘었고 여전히 조문 명문 없이 판정 선례로만 존재한다 (`R8` 이월). 5. AC 3 ③ 이 `R5` 존치 조건을 사실상 **사문화** 시켰다 — 조문 자체의 개정 (조건 삭제 또는 목록 갱신) 판단은 이월. 6. FU14 anchor 좌표계 이행 시 본 정규화는 anchor 표기에 흡수된다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S3 batch 7 = `L` 잔존 18 ADR 중 대형 1 ~ 2 파일** (`ADR-0024` 21 행 / 26 좌표 · `ADR-0039` 15 행 / 22 좌표 · `ADR-0022` 12 행 / 14 좌표) — 다음 **1 순위** (en dash 후보는 소진). (2) 비-ADR `L` 정규화 (`directory.md` · `integrator.md` · `reviewer.md` · `race-patterns.md` · `p3-*.md`). (3) **`§ 12.76 R5` 개정 판단** — 예시 갱신 (`§ 12.79` 파생 (3) 이월) + 존치 조건 사문화 대응으로 우선순위 **상향**. (4) `R8` 조문화 (회색 **5 종** + 수량 범위 판별) — 선례 6 건 확보로 **중간** 유지. (5) `R2` · `R3` 병합 · anchor FU14 (낮음 유지).

**AC 7 자기 준수 · 범위 검산 1 구.** 신규 추가분 대상 자기 준수 grep = **11** (ADR 3 파일 몫 **0 · 0 · 0** · 본 절 **9** + task 파일 **2**) 로 전량 본 절 대조표 · 판정 문단과 task 완료 요약의 **원 표기 인용** 이라 `R6` 예외다. `wc -l` **7209 → 7266** (신설 **57 행** · 절 본문 **56 행** 으로 ≤ 60 충족), `git diff --stat` **4 파일 · 74 삽입 / 13 삭제** (≤ 300 LOC · ≤ 5 파일 — `ADR-0001` 은 범위 밖 판정으로 무편집이라 touchesFiles 5 중 실 변경 4), `git status --short` 에 `README.md` · `CLAUDE.md` · `docs/requirements.md` · `docs/PLAN.md` · `docs/architecture/` · 다른 `docs/decisions/*` · `.claude/agents/` · `docs/LOOP.md` · `src/` · `prisma/` **전무**, audit 파일 fence **178 (짝수)** · 신설 표 **6 컬럼 균일** · heading 순번 `12.82` → `12.83` 연속 · 대상 ADR 링크 수 **19 · 17 · 42 불변**. doc-only 라 `pnpm test` 불요 (`CLAUDE.md` §3.2 direct doc-only 면제).

### 12.84 범위 표기 규약 S3 batch 7 — `L` 축 대형 ADR 2 파일 (`ADR-0024` · `ADR-0039`) `R5` · `R4` 정규화 — 후보 **36 행 / 48 좌표 전량 정정** (단위어 부착 **43 회**) + 병기 변형 4 종 첫 판정 (T-1486)

**위치 · 계보.** `§ 12.83` (S3 batch 6) **파생 영향 (1)** 이 **1 순위** 로 지목한 **S3 batch 7 = `L` 잔존 18 ADR 중 대형 파일** 의 실집행 절이다. `§ 12.83` AC 6 이 en dash 축 마감을 선언했으므로 본 batch 부터 후보는 **`L` 축 단일 축** 이다. 규범 정본은 `§ 12.76` 조문 `R1` ~ `R7`, 판정 선례는 `§ 12.79` AC 3 ①②③ · `§ 12.80` AC 2 · `§ 12.81` AC 3 ①② · `§ 12.82` AC 3 ③ · `§ 12.83` AC 3 ③ · AC 5 를 승계한다.

#### AC 1 — 대상 선정 · 한정 소급 · mode · cap 판정

**① 선정 근거.** `L` 잔존 18 ADR 중 hit 수 최대 2 파일 (`ADR-0024` 21 행 / 26 좌표 · `ADR-0039` 15 행 / 22 좌표) 을 편성했다 — 대형 파일을 뒤로 미룰수록 소규모 묶음 slice 가 cap 에 눌려 축 마감이 지연되기 때문이다. 3 순위 `ADR-0022` (12 행 / 14 좌표) 를 얹지 않은 이유는 ADR **2** + 본 audit + task 파일 = **4 파일** 인 현 편성에 1 파일을 더하면 5 파일 cap 자체는 충족하나 정정 좌표가 **62** 로 늘어 아래 ④ cap 가드 임계 **70** 에 근접하고, 본 batch 가 처음 여는 병기 변형 판정 문단이 절 ≤ 60 행 제약과 충돌하기 때문이다.

**② 한정 소급 준수.** AC 2 census 가 대상을 **파일 · 행 단위로 특정** 하므로 `§ 12.76` AC 3 ③ 의 "이미 어긋남이 확인된 좌표에 한정" 을 충족한다. **전면 일괄 치환은 여전히 금지** 이며 본 2 ADR 밖은 열지도 고치지도 않았다 (AC 7 `git status --short` 검산). 치환도 파일 전체 정규식이 아니라 census 가 특정한 **36 행 좌표에만** 적용했다.

**③ mode 근거.** `CLAUDE.md` §3.1 **rule 5** (기존 `docs/decisions/*` 본문의 **비-결정 수정** = `direct`) 의 **일곱 번째 적용 선례** 다 (선례 `§ 12.78` ③ · `§ 12.79` ③ · `§ 12.80` ③ · `§ 12.81` ③ · `§ 12.82` ③ · `§ 12.83` ③). 편집은 `L` prefix 제거 + 단위어 `행` 부착뿐이고 ADR 의 결정 실질 · 링크 URL · frontmatter `status` · `date` 는 **무편집** (AC 5 검산).

**④ cap 가드 (이월 0).** 확정 정정 좌표는 **48** 로 임계 **70** 을 밑돌아 `ADR-0039` 를 batch 8 로 **이월하지 않고** 2 파일 전량을 본 slice 에서 마감했다 (`§ 12.81` AC 1 ④ 선례 승계). 실 diff 도 `21/21` + `15/15` = **72 LOC** 로 300 LOC cap 대비 여유가 크다.

#### AC 2 — 후보 census 재실측 (집계 1 구)

`grep -nE 'L[0-9]+'` 행 단위 실측은 `ADR-0024` **21** · `ADR-0039` **15** (합 **36 행**), `grep -oE` 좌표 단위는 **26 · 22** (합 **48 좌표**) 로 Required Reading 의 planner 기준값 (**21 행 / 26 좌표** · **15 행 / 22 좌표**) 과 **차이 0** 이다. raw `grep -oE '[0-9] *~ *[0-9]'` 는 **20 · 1**, en dash 는 **0 · 0**. **행 수 ≠ 좌표 수** — `§ 12.80` AC 2 승계로 행 단위 값은 한 행 다중 hit 을 1 로 세는 **하한** 이며, 본 batch 의 격차는 `ADR-0024` **5** (`:29` +1 · `:35` +1 · `:196` +2 · `:197` +1) · `ADR-0039` **7** (`:20` +2 · `:120` +2 · `:121` +1 · `:123` +1 · `:125` +1) 로 **9 행에 집중** 한다.

#### AC 3 — 판정 3 종 독립 결론

**① `R5` 존치 조건 — `ADR-0024` 는 불성립 · `ADR-0039` 는 성립 (task 전제 정정).** `ADR-0024` 의 raw `~` **20** 은 전량이 `L133~144` 류 **좌표 표기** 라 `§ 12.76 R5` 존치 조건 ("그 파일의 `~` 범위 표기가 0 건일 때만") 이 **애초에 불성립** — `§ 12.79` AC 3 ② 의 단순 승계로 정정이며 자기 소멸 (`§ 12.82` AC 3 ①) · 예시 목록 (`§ 12.83` AC 3 ③) 논거를 동원할 필요가 없다. 반면 `ADR-0039` 의 raw `~` **1** 은 `:62` 의 `월 28~31 일` = **수량 범위** 라 좌표 `~` 는 **0**, 즉 존치 조건은 **성립** 한다 — 본 slice task 가 전제한 "2 파일 모두 불성립" 은 raw grep 기준 판정이므로 본 절이 정정한다 (`§ 12.79` AC 3 ① 이 같은 유형의 전제를 정정한 선례 승계). 게다가 `ADR-0039` 의 22 좌표는 **전량 단일 행** 이라 `R4` 정정으로 `~` 가 유입되지도 않아 자기 소멸 논거도 불가하다. 그럼에도 정정으로 결론낸 근거는 `§ 12.83` AC 3 ③ **동형** — 존치 예시 목록은 **예시적** (`§ 12.76` 한계 3 "스냅숏") 이고 조문 문언이 존치를 "**허용한다**" 로 재량화했으므로 `R5` 본문 1 순위 규범 (`L` 미사용) 이 우선하며, 같은 `[README L61]` 형태를 `§ 12.83` 이 이미 정정한 이상 존치는 규약 내 불일치를 낳는다. 결론은 두 파일 모두 **전량 정정** 으로 같다.

**② 병기 변형 4 종 첫 판정 — 단위어 `행` 은 "묶음당 1 회".** (가) **콤마 병기** `ADR-0024`:29 `L19~22, L33` → `19~22, 33 행`, (나) **슬래시 병기** `ADR-0039`:20 `[ADR-0006 L54/L122]` → `[ADR-0006 54/122 행]` · `ADR-0024`:197 `L37~53/L133~144` → `37~53/133~144 행`, (다) **`+` 병기** `ADR-0039`:123 `§Decision3 + L41 + L86` → `§Decision3 + 41 + 86 행`, (라) **링크 label 안** `[README L61](../../README.md)` → `[README 61 행](../../README.md)` (URL 무편집) 과 **heading 안** `ADR-0039`:16 `### ... (ADR-0035 §Decision3 / L41)` → `(... / 41 행)`. 부착 횟수는 **하나의 구 안에서 `,` · `/` · `+` 로 이어진 연속 좌표 묶음에 끝 1 회**, 문장 · 링크 label 이 갈리면 **좌표마다 1 회** 로 확정했다 — 근거는 `R4` (단일 행에 `~` 를 만들지 않듯 단위어도 최소 부착) 와 `§ 12.76 R6` 예시 `실 대응 31 · 34 행 불연속` 이 나열 끝에 `행` 을 **1 회만** 붙인 화법이다. `R1` 은 병기 구분자를 규율하지 않으므로 원문의 `,` · `/` · `+` 는 **보존** 했다 (구분자를 `~` 로 바꾸면 불연속 좌표가 연속 범위로 오독된다). 그 결과 48 좌표에 대한 단위어 부착은 **43 회** (묶음 5 건이 2 좌표씩 흡수).

**③ 코드 파일 대상 좌표 — 정정 (회색 다 승계).** `ADR-0024` 26 좌표 중 **21** 이 `src/permission-denied/*.ts` · `prisma/schema.prisma` 를 가리키고 5 문서군 밖 문서 (`README.md`) 는 5 다. `§ 12.76` AC 3 ② 는 **코드 주석 · 코드 내 문자열** 을 범위 밖으로 두었을 뿐이고, `§ 12.81` AC 3 ② (회색 다) · `§ 12.82` AC 3 ③ 승계로 판정 기준은 **표기가 실린 위치** (= ADR 본문, 범위 안) 이므로 전량 정정 대상이다. 이미 `~` 를 쓰는 **20 좌표** (`L133~144` 류) 는 `R1` 을 **이미 준수** 해 `R5` 만 적용했고, 단일 행 **28 좌표** (`ADR-0024` 6 + `ADR-0039` 22) 는 `R4` 를 함께 적용해 `435~435` 형태를 만들지 않았다 — 따라서 본 batch 의 **`R1` 신규 정정은 0** 이다.

#### AC 4 · AC 5 — 대조표 (36 후보 행 · 유형별 압축) + 무편집 검산

| # | 파일 | 행 | 원 표기 | 판정 | 근거 조문 |
| --- | --- | --- | --- | --- | --- |
| 1 | `ADR-0024` | 18 · 33 · 34 · 36 · 37 · 64 · 66 · 72 · 73 · 100 · 101 · 111 · 180 · 198 (14 행 · 14 좌표) | 단독 범위 `L133~144` · `L131~141` 류 | **정정** → `133~144 행` | `R5` (`R1` 이미 준수) |
| 2 | `ADR-0024` | 28 · 54 · 129 (3 행 · 3 좌표) | 단일 행 `L33` · `L435` | **정정** → `33 행` · `435 행` | `R5` + `R4` |
| 3 | `ADR-0024` | 35 · 196 (2 행 · 5 좌표) | 한 행에 범위 + 단일 혼재 (`L432~444` … `(L435)`) | **정정** → 구가 갈리므로 좌표마다 `행` | `R5` + `R4` |
| 4 | `ADR-0024` | 29 · 197 (2 행 · 4 좌표) | 콤마 병기 `L19~22, L33` · 슬래시 병기 `L37~53/L133~144` | **정정** → `19~22, 33 행` · `37~53/133~144 행` (묶음당 1 회) | `R5` + `R4` + `R6` 화법 |
| 5 | `ADR-0039` | 14 · 30 · 33 · 98 · 99 · 119 · 124 (7 행 · 7 좌표) | 본문 · 링크 label 의 단일 행 (`[README L61]` · `[ADR-0029 L107]`) | **정정** → `61 행` · `[ADR-0029 107 행]` (URL 무편집) | `R5` + `R4` |
| 6 | `ADR-0039` | 16 · 36 (2 행 · 2 좌표) | **heading 안** 좌표 (`… / L41)` · `(PLAN.md L109, 2026-06-11)`) | **정정** → `41 행` · `109 행` (`R7` 비해당) | `R5` + `R4` (`R7` 판별) |
| 7 | `ADR-0039` | 18 · 20 · 123 · 125 (4 행 · 8 좌표) | 병기 `L54/L122` · `§Decision3 + L41 + L86` | **정정** → `54/122 행` · `41 + 86 행` | `R5` + `R4` + `R6` 화법 |
| 8 | `ADR-0039` | 120 · 121 (2 행 · 5 좌표) | 문장 분리 나열 (`L61 — … L72 — … L9 —`) | **정정** → 좌표마다 `행` | `R5` + `R4` |

**`R6` · `R7` · `R4` 판별 1 구.** 인용 원문 병기 (`R6`) **0 건** — 본 batch 에 `§ 12.83` 류의 원문 표기 인용은 없고 `R6` 는 화법 근거로만 원용했다. 시점 기록 면제 (`R7`) **0 건** — 유일한 후보인 `ADR-0039`:36 heading `(PLAN.md L109, 2026-06-11)` 의 날짜는 **사용자 결정 일자** stamp 이지 좌표 측정 시점 stamp 가 아니고, 뒤따르는 blockquote 가 인용문일 뿐 좌표 자체는 현행 `PLAN.md` 를 가리키는 **살아있는 pointer** 라 비해당이다 (`§ 12.83` AC 4 의 `R7` 비해당 판정과 동형). `R4` 적용 단일 행 좌표 **28**. `L` 제거로 인한 **조사 보정 0 건** — 좌표 뒤가 전부 `)` · `—` · 공백 + 코드 span (`133~144 행 \`list(actor, query?)\``) · 조사 없는 문장 경계라 `행` 삽입만으로 자연스럽다.

**AC 5 무편집 검산 1 구.** `git diff --numstat` = **`21/21` · `15/15`** 로 **추가 행 수 = 삭제 행 수** 이고 변경 토큰은 `L` prefix 제거 + 단위어 `행` 뿐이다 (문장 · 수치 · 링크 URL 무변경). hunk 구간 분포는 `ADR-0024` **Context 8 · Decision 9 · Alternatives 1 · References 3**, `ADR-0039` **Context 7 · Alternatives 2 · References 6** 으로 `## Decision` · `## Alternatives` 구간 hunk **11 건** 이 발생했으나 `§ 12.83` AC 5 판정 승계 — rule 5 의 경계는 **구간이 아니라 결정 실질** 이므로 표기 정규화뿐인 본 hunk 는 전량 비-결정 수정이다. `## Consequences` hunk **0** · frontmatter (`status` · `date`) hunk **0**. 정정 후 재실측: 행 수 **203 · 128 불변**, 링크 수 **104 · 22 불변**, `## ` heading 수 **5 · 6 불변**, `L` prefix 잔존 **0 · 0** 으로 면제 · 이월 판정분 **0 과 동수** (미판정 잔존 0).

#### AC 6 — `L` 축 잔여 갱신

정정 후 `grep -lE 'L[0-9]+' docs/decisions/ADR-*.md` = **16 파일** (ADR 총 **53**) 로 `§ 12.83` AC 6 의 **18** 에서 **2 감소** — 본 batch 처리 파일 수 **2** 와 정확히 일치하며 부분 이월 · 면제 잔존은 **0** 이다. 비-ADR 대상 `grep -lE 'L[0-9]+' docs/architecture/*.md .claude/agents/*.md` = **14 파일** 로 본 batch 미대상이라 **불변**. 따라서 `L` 축 잔여는 ADR **16** + 비-ADR **14** = **30 파일** 이다.

#### 진척 · 한계 · 파생 영향

**진척.** S3 batch 7 마감 — 후보 **36 행 / 48 좌표 전량 정정** (단위어 부착 **43 회** · 이월 **0**), **병기 변형 4 종** (콤마 · 슬래시 · `+` · 링크 label / heading 안) 의 첫 판정과 "묶음당 단위어 1 회" 확정, `R5` 존치 조건의 **파일별 갈림** (`ADR-0024` 불성립 · `ADR-0039` 성립) 을 raw grep 전제 정정과 함께 실판정, 코드 파일 좌표 선례 **3 번째**, `CLAUDE.md` §3.1 rule 5 **일곱 번째 선례**. `L` 잔존 ADR **18 → 16**. 남은 slice = S3 batch 8 이후 · S4 (조건부 · `pr`).

**한계.** 1. 좌표가 가리키는 **값 자체의 정확성은 재검증하지 않았다** (`§ 12.74` 판정 승계 — 표기 형식만). 2. census 는 행 단위 grep 을 병용하므로 파일별 행 값은 **하한** 이다 (본 batch 의 격차 12 이 실례). 3. 회색지대는 **5 종** (개구간 · 양끝 prefix 병기 · 코드 파일 좌표 · 범위 밖 문서 좌표 · 수량 vs 좌표 판별) + **병기 변형 4 종** 으로 늘었고 여전히 조문 명문 없이 선례로만 존재한다 (`R8` 이월). 4. `R5` 존치 조건은 본 절에서 성립 파일 (`ADR-0039`) 조차 정정으로 결론나 **사문화가 재확인** 됐다 — 조문 개정 판단은 여전히 이월. 5. 단위어 부착 규칙 (묶음당 1 회) 은 본 절 선례일 뿐 조문이 아니라 다음 batch 가 좌표마다 부착해도 grep 으로 검출되지 않는다. 6. FU14 anchor 좌표계 이행 시 본 정규화는 anchor 표기에 흡수된다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S3 batch 8 = `L` 잔존 ADR 16 중 `ADR-0022` (12 행 / 14 좌표) + `ADR-0019` · `ADR-0049` · `ADR-0037`** — 다음 **1 순위**. (2) 비-ADR `L` 정규화 **14 파일** (`directory.md` · `integrator.md` · `reviewer.md` · `race-patterns.md` · `p3-*.md`) — `R5` 존치 예시 목록과 정면 충돌이라 (3) 선행 여부 판단 필요. (3) **`§ 12.76 R5` 개정 판단** (존치 조건 삭제 + 예시 갱신) — 사문화 재확인으로 우선순위 **상향 유지**. (4) `R8` 조문화 (회색 5 종 + **병기 변형 4 종** + 단위어 부착 규칙) — 선례 축적으로 **상향** 후보. (5) `R2` · `R3` 병합 · anchor FU14 (낮음 유지).

**AC 7 자기 준수 · 범위 검산 1 구.** 신규 추가분 대상 자기 준수 grep (`grep -nE '[0-9]+ *– *[0-9]+|L[0-9]+'`) = **17** (본 절 **12** + task 파일 **5**) 로 전량 대조표 · 판정 문단 · 완료 요약의 **원 표기 인용** 이라 `R6` 예외다. `wc -l docs/use-cases/REQ-COVERAGE-AUDIT.md` **7266 → 7323** (신설 **57 행** · 절 본문 **56 행** 으로 ≤ 60 충족), `git diff --stat` **4 파일 · 106 삽입 / 45 삭제** (≤ 300 LOC · ≤ 5 파일), `git status --short` 에 `README.md` · `CLAUDE.md` · `docs/requirements.md` · `docs/PLAN.md` · `docs/architecture/` · 다른 `docs/decisions/*` · `.claude/agents/` · `docs/LOOP.md` · `src/` · `prisma/` **전무**, audit 파일 fence **178 (짝수)** · 신설 표 **6 컬럼 균일** · heading 순번 `12.83` → `12.84` **연속** · 대상 ADR 링크 수 **104 · 22 불변**. doc-only 라 `pnpm test` 불요 (`CLAUDE.md` §3.2 direct doc-only 면제).

### 12.85 범위 표기 규약 S3 batch 8 — `L` 축 ADR 3 파일 (`ADR-0022` · `ADR-0019` · `ADR-0049`) `R5` · `R4` 정규화 — 후보 **28 행 / 39 좌표 전량 정정** (단위어 부착 **37 회**) + 병기 변형 2 종 · "이하 원문" 구간 `R6` 판정 (T-1487)

**위치 · 계보.** `§ 12.84` (S3 batch 7) **파생 영향 (1)** 이 **1 순위** 로 지목한 **S3 batch 8 = `L` 잔존 16 ADR 중 `ADR-0022` · `ADR-0019` · `ADR-0049`** 의 실집행 절이다. `§ 12.83` AC 6 의 en dash 축 마감 선언에 따라 후보는 여전히 **`L` 축 단일 축** 이다. 규범 정본은 `§ 12.76` 조문 `R1` ~ `R7`, 판정 선례는 `§ 12.79` AC 3 ①② · `§ 12.80` AC 2 · 대조표 #4 · `§ 12.81` AC 3 ①② · `§ 12.82` AC 3 ③ · `§ 12.83` AC 3 ②③ · AC 5 · `§ 12.84` AC 3 ①②③ 을 승계한다.

#### AC 1 — 대상 선정 · 한정 소급 · mode · cap 판정

**① 선정 근거.** `§ 12.84` 파생 (1) 이 열거한 4 후보 중 hit 수 상위 3 (`ADR-0022` 12 행 / 14 좌표 · `ADR-0019` 9 행 / 16 좌표 · `ADR-0049` 7 행 / 9 좌표) 을 편성했다. 함께 열거된 `ADR-0037` (7 행 / 9 좌표) 을 얹지 않은 이유는 ADR **3** + 본 audit + task 파일 = **정확히 5 파일** 이라 1 파일을 더하면 `CLAUDE.md` §3 의 5 파일 cap 을 그 자체로 위반하기 때문이다 — 좌표 수 (39 → 48) 는 아래 ④ 임계에 여유가 있었으므로 제약은 파일 수 쪽이다.

**② 한정 소급 준수.** AC 2 census 가 대상을 **파일 · 행 단위로 특정** 하므로 `§ 12.76` AC 3 ③ 의 "이미 어긋남이 확인된 좌표에 한정" 을 충족한다. **전면 일괄 치환은 여전히 금지** 이며 본 3 ADR 밖은 열지도 고치지도 않았다 (AC 7 `git status --short` 검산). 치환도 파일 전체 정규식이 아니라 census 가 특정한 **28 행 좌표에만** 행 번호를 명시해 적용했다.

**③ mode 근거.** `CLAUDE.md` §3.1 **rule 5** (기존 `docs/decisions/*` 본문의 **비-결정 수정** = `direct`) 의 **여덟 번째 적용 선례** 다 (선례 `§ 12.78` ③ · `§ 12.79` ③ · `§ 12.80` ③ · `§ 12.81` ③ · `§ 12.82` ③ · `§ 12.83` ③ · `§ 12.84` ③). 편집은 `L` prefix 제거 + 단위어 `행` 부착뿐이고 ADR 의 결정 실질 · 링크 URL · frontmatter `status` · `date` 는 **무편집** (AC 5 검산).

**④ cap 가드 (이월 0).** 확정 정정 좌표는 **39** 로 임계 **70** 을 크게 밑돌아 `ADR-0049` 를 batch 9 로 **이월하지 않고** 3 파일 전량을 본 slice 에서 마감했다 (`§ 12.81` AC 1 ④ · `§ 12.84` AC 1 ④ 선례 승계). 실 diff 도 `12/12` + `9/9` + `7/7` = **56 LOC** 로 300 LOC cap 대비 여유가 크다.

#### AC 2 — 후보 census 재실측 (집계 1 구)

`grep -nE 'L[0-9]+'` 행 단위 실측은 `ADR-0022` **12** · `ADR-0019` **9** · `ADR-0049` **7** (합 **28 행**), `grep -oE 'L[0-9]+'` 좌표 단위는 **14 · 16 · 9** (합 **39 좌표**) 로 Required Reading 의 planner 기준값 (**12 / 14** · **9 / 16** · **7 / 9**) 과 **차이 0** 이다. raw `grep -oE '[0-9] *~ *[0-9]'` 는 **11 · 14 · 2**, en dash 는 **0 · 0 · 0**. **행 수 ≠ 좌표 수** — `§ 12.80` AC 2 승계로 행 단위 grep 은 한 행 다중 hit 을 1 로 세는 **하한** 이며, 본 batch 총 격차 **11** 중 `ADR-0019` 가 **7** (16−9) 로 지금까지 batch 중 최대다. 그 7 은 `:18` · `:19` · `:85` · `:93` · `:116` · `:148` · `:149` **7 행이 각각 2 좌표** 를 담아 발생하며 (한 행에 3 좌표 이상은 0), 나머지 격차는 `ADR-0022` **2** (`:28` · `:107`) · `ADR-0049` **2** (`:36` · `:48`) 다.

#### AC 3 — 판정 3 종 독립 결론

**① `R5` 존치 조건 — 3 파일 전부 불성립.** raw `~` 를 좌표 범위 vs 수량 범위로 분해하면 (`§ 12.84` AC 3 ① 의 `ADR-0039` 판별 방식 승계) `ADR-0022` **11 전량이 좌표 범위** (`L198~225` 류), `ADR-0019` **14 전량 좌표 범위** (`L213~250` 류), `ADR-0049` **2 전량 좌표 범위** (`L36~39` · `L36~41`) 이고 수량 · 식별자 범위는 **3 파일 모두 0** 이다. 따라서 `§ 12.76 R5` 존치 조건 ("그 파일의 `~` 범위 표기가 0 건일 때만") 은 세 파일 모두 **애초에 불성립** — `§ 12.79` AC 3 ② 의 단순 승계로 정정이며, 성립 파일이 **0** 이라 `§ 12.83` AC 3 ③ 의 예시 목록 · 재량 논거나 `§ 12.82` AC 3 ① 의 자기 소멸 논거를 **동원할 필요가 없다** (`§ 12.84` 의 `ADR-0039` 유형은 본 batch 에 재발하지 않았다).

**② 병기 구분자 2 변형 첫 판정 — 규칙 무수정 적용.** (가) **가운뎃점 병기** `ADR-0019`:93 `` `fetchAndMap` L271~275·L290~294 `` → `` `fetchAndMap` 271~275·290~294 행 `` — `·` 를 `,` 로 **바꾸지 않는다**: `R1` 은 **범위 구분자** (`~`) 만 규율하고 병기 구분자는 규율하지 않으며 (`§ 12.84` AC 3 ② 확정), `·` 는 `§ 12.76 R6` 예시 `실 대응 31 · 34 행 불연속` 이 쓴 화법 그대로라 오히려 조문 예시와 정합한다. (나) **prefix 생략 병기** `ADR-0049`:130 `L31 / 40 / 41` → `31 / 40 / 41 행` — `L` 이 첫 좌표에만 붙은 생략형이라 정정 후 3 좌표가 **동형으로 수렴** 하고, 원 구분자 ` / ` 는 `§ 12.84` (나) 슬래시 병기 승계로 **보존** 한다 (구분자를 `~` 로 바꾸면 불연속 좌표가 연속 범위로 오독된다). 두 변형 모두 낱말 개재 없이 인접하므로 단위어는 **묶음 끝 1 회** — `§ 12.84` AC 3 ② 의 규칙이 수정 없이 적용된다. 다만 좌표 사이에 **함수명이 개재** 하면 (`ADR-0019`:18 `` `requestAllPages` L213~250 / `fetchAndMap` L257~301 ``) 묶음이 깨져 **좌표마다 부착** 했다 — `§ 12.84` 의 "문장 · 링크 label 이 갈리면 좌표마다" 를 **낱말 개재** 로 확장한 첫 사례다.

**③ "이하 원문" 구간 — `R6` · `R7` 동시 비해당 (7 좌표 전량 정정).** 판정 기준은 세 축이다. (가) **blockquote 마크업은 보조 기준일 뿐** — `>` 가 `:30` 까지만 붙어 후속 6 행은 형식상 통상 본문이나, `:30` 자신은 마크업 안이라 형식만으로는 갈리지 않는다. (나) **저자 test 가 1 차 기준** — `R6` 면제 (`§ 12.83` AC 3 ②) 는 표기의 **저자가 인용원** 일 때 (en dash 가 README 자신의 표기일 때) 성립하는데, 본 7 좌표는 전부 **본 ADR 저자가 쓴 README / requirements 를 향한 pointer** 이고 인용원 문서에는 `L41` 형태가 **존재하지 않아** `§ 12.74` 의 char-identical 대조 대상 자체가 없다 → `R6` 불성립. `§ 12.72` 규칙 A 의 인용 재분류도 **인용 표시가 붙은 텍스트** 를 재분류하는 규칙이지, "이하 원문" 선언이 뒤따르는 본문 전체를 인용으로 만들지 않는다 — 보존 대상은 banner `:26` 이 명시한 **미채택 설계 사고 흔적 (내용)** 이지 좌표 표기 형식이 아니다. (다) **`R7` 도 비해당** — `§ 12.80` 대조표 #4 (`ADR-0008`:149) 의 면제는 좌표가 **prettier 보정 전 당시 파일 상태** 를 가리키는 증거였기 때문인데, 본 7 좌표는 날짜 stamp 없이 **현행 README / requirements** 를 가리키는 살아있는 pointer 이고 banner 의 `2026-06-22` 는 supersede 일자 stamp 이지 좌표 측정 시점 stamp 가 아니다 (`§ 12.83` AC 4 · `§ 12.84` `R7` 비해당 판정 동형). 결론: 대조표의 **면제 기록 0** · `L` 잔존 반영 **0**.

#### AC 4 · AC 5 · AC 6 — 대조표 (28 후보 행 · 유형별 압축) + 무편집 검산 + 잔여 갱신

| # | 파일 | 행 | 원 표기 | 판정 | 근거 조문 |
| --- | --- | --- | --- | --- | --- |
| 1 | `ADR-0022` | 18 · 19 · 20 · 29 · 31 · 71 · 110 · 168 · 169 · 170 (10 행 · 10 좌표) | 단독 범위 `L198~225` · `L39~41` (8) + 링크 뒤 단일 행 `L33` · `L56` (2) | **정정** → `198~225 행` · `33 행` | `R5` (`R1` 이미 준수) + 단일 행 2 에 `R4` |
| 2 | `ADR-0022` | 28 (1 행 · 2 좌표) | 콤마 병기 `L19~22, L33` | **정정** → `19~22, 33 행` (묶음당 1 회) | `R5` + `R4` + `R6` 화법 |
| 3 | `ADR-0022` | 107 (1 행 · 2 좌표) | `+` 병기이나 낱말 개재 `L211~217 port + L229~232 주석` | **정정** → 좌표마다 `행` | `R5` (AC 3 ②) |
| 4 | `ADR-0019` | 34 · 84 (2 행 · 2 좌표) | 단독 범위 `L19~22` · `L213~250` | **정정** → `19~22 행` | `R5` |
| 5 | `ADR-0019` | 18 · 19 · 85 · 148 · 149 (5 행 · 10 좌표) | 함수명 개재 슬래시 병기 `` `requestAllPages` L213~250 / `fetchAndMap` L257~301 `` | **정정** → 좌표마다 `행` | `R5` + `R6` 화법 |
| 6 | `ADR-0019` | 93 (1 행 · 2 좌표) | **가운뎃점 병기** `L271~275·L290~294` | **정정** → `271~275·290~294 행` (`·` 보존) | `R5` + `R6` 화법 |
| 7 | `ADR-0019` | 116 (1 행 · 2 좌표) | 링크 label 갈린 단일 행 `L210` · `L262` | **정정** → `210 행` · `262 행` | `R5` + `R4` |
| 8 | `ADR-0049` | 30 · 34 (2 행 · 2 좌표) | blockquote "이하 원문" 선언 행 `L41 / R-41` · **heading 안** `README L41` | **정정** (`R6` · `R7` 동시 비해당) | `R5` + `R4` (AC 3 ③) |
| 9 | `ADR-0049` | 36 · 48 · 87 · 129 (4 행 · 6 좌표) | 본문 · References pointer `L41 은` · `(L31, …)` · `(README L36~39)` | **정정** → 구마다 `행` | `R5` (+ 단일 행 4 에 `R4`) |
| 10 | `ADR-0049` | 130 (1 행 · 1 좌표) | **prefix 생략 병기** `L31 / 40 / 41` | **정정** → `31 / 40 / 41 행` (구분자 · 생략형 보존) | `R5` + `R4` + `R6` 화법 |

**`R6` · `R7` · `R4` 판별 1 구.** 인용 원문 병기 (`R6`) **0 건** — AC 3 ③ 이 "이하 원문" 구간까지 비면제로 판정했고 `R6` 는 병기 화법 근거로만 **3 회** 원용했다. 시점 기록 면제 (`R7`) **0 건** (AC 3 ③ (다)). 이미 `~` 를 쓰는 좌표는 **27** (11 + 14 + 2) 으로 `R1` 을 **이미 준수** 해 `R5` 만 적용했고, 단일 행 좌표 **12** (`ADR-0022` 3 · `ADR-0019` 2 · `ADR-0049` 7) 에는 `R4` 를 병용해 `33~33` 형태를 만들지 않았다 — 따라서 본 batch 의 **`R1` 신규 정정은 0** 이다. 39 좌표에 대한 단위어 부착은 **37 회** (묶음 2 건이 각 2 좌표를 흡수 — 대조표 #2 · #6). `L` 제거로 인한 **조사 보정 0 건** — `L39~41 의` → `39~41 행 의` · `L41 은` → `41 행 은` 이 그대로 자연스러워 `§ 12.83` · `§ 12.84` 의 조사 보정 0 선례와 동형이다.

**AC 5 무편집 검산 1 구.** `git diff --numstat` = **`12/12` · `9/9` · `7/7`** 로 **추가 행 수 = 삭제 행 수** 이고 변경 토큰은 `L` prefix 제거 + 단위어 `행` 뿐이다 (문장 · 수치 · 링크 URL 무변경). hunk 구간 분포는 `ADR-0022` **Context 6 · Decision 3 · References 3**, `ADR-0019` **Context 3 · Decision 3 · Consequences 1 · References 2**, `ADR-0049` **헤더 blockquote 1 · Context 3 · Alternatives 1 · References 2** 로 `## Decision` **6** · `## Consequences` **1** · `## Alternatives` **1** = 8 건이 발생했으나 `§ 12.83` AC 5 · `§ 12.84` AC 5 판정 승계 — rule 5 의 경계는 **구간이 아니라 결정 실질** 이므로 표기 정규화뿐인 본 hunk 는 전량 비-결정 수정이다. frontmatter (`status` · `date`) hunk **0**. 정정 후 재실측: 행 수 **180 · 160 · 138 불변**, 링크 수 **72 · 70 · 27 불변**, `## ` heading 수 **5 · 5 · 7 불변**, `L` prefix 잔존 **0 · 0 · 0** 으로 면제 · 이월 판정분 **0 과 동수** (미판정 잔존 0).

**AC 6 `L` 축 잔여 갱신 1 구.** 정정 후 `grep -lE 'L[0-9]+' docs/decisions/ADR-*.md` = **13 파일** (ADR 총 **53**) 로 `§ 12.84` AC 6 의 **16** 에서 **3 감소** — 본 batch 처리 파일 수 **3** 과 정확히 일치하며 면제 잔존 · 부분 이월은 **0** 이다. 비-ADR 대상 `grep -lE 'L[0-9]+' docs/architecture/*.md .claude/agents/*.md` = **14 파일** 로 본 batch 미대상이라 **불변** (`§ 12.84` AC 6 값과 동일). 따라서 `L` 축 잔여는 ADR **13** + 비-ADR **14** = **27 파일** 이다.

#### 진척 · 한계 · 파생 영향

**진척.** S3 batch 8 마감 — 후보 **28 행 / 39 좌표 전량 정정** (단위어 부착 **37 회** · 면제 **0** · 이월 **0**), **병기 변형 2 종** (가운뎃점 · prefix 생략) 첫 판정으로 변형 누적 **6 종**, `R5` 존치 조건의 **3 파일 연속 불성립** 실측, "이하 원문" 보존 선언 구간의 `R6` · `R7` **동시 비해당** 첫 판정과 **저자 test** 의 1 차 기준 확립, 행↔좌표 격차 **최대 7** (`ADR-0019`) 실증, `CLAUDE.md` §3.1 rule 5 **여덟 번째 선례**. `L` 잔존 ADR **16 → 13**. 남은 slice = S3 batch 9 이후 · S4 (조건부 · `pr`).

**한계.** 1. 좌표가 가리키는 **값 자체의 정확성은 재검증하지 않았다** (`§ 12.74` 판정 승계 — 표기 형식만). 2. census 는 행 단위 grep 을 병용하므로 파일별 행 값은 **하한** 이다 (본 batch 총 격차 **11** · `ADR-0019` 단독 **7** 이 실례). 3. 회색지대는 **5 종** (개구간 · 양끝 prefix 병기 · 코드 파일 좌표 · 범위 밖 문서 좌표 · 수량 vs 좌표 판별) + **병기 변형 6 종** (`,` · `/` · `+` · label / heading 내 · `·` · prefix 생략) 으로 늘었고 여전히 조문 명문 없이 선례로만 존재한다 (`R8` 이월). 4. 단위어 부착 규칙 (묶음당 1 회 · 낱말 개재 시 좌표마다) 은 `§ 12.84` 한계 5 승계로 **조문이 아니라 선례** 라 다음 batch 가 달리 붙여도 grep 으로 검출되지 않는다. 5. AC 3 ③ 의 **저자 test** 도 본 절 선례일 뿐이고, "인용원에 `L` 표기가 없다" 는 판단은 규약 census 결과에 의존할 뿐 인용원 문서를 열어 char-identical 대조를 수행하지는 않았다. 6. FU14 anchor 좌표계 이행 시 본 정규화는 anchor 표기에 흡수된다 (**관계 언급만 · 착수 금지**).

**파생 영향 (목록만 — 본 slice 착수 금지).** (1) **S3 batch 9 = `L` 잔존 13 ADR 중 `ADR-0037` (7 행 / 9 좌표) + `ADR-0038` · `ADR-0044` · `ADR-0023`** — 다음 **1 순위** (본 slice 가 파일 수 cap 으로 이월한 `ADR-0037` 포함). (2) 비-ADR `L` 정규화 **14 파일** (`directory.md` · `integrator.md` · `reviewer.md` · `race-patterns.md` · `p3-*.md`) — `R5` 존치 예시 목록과 정면 충돌이라 (3) 선행 여부 판단 필요. (3) **`§ 12.76 R5` 개정 판단** (존치 조건 삭제 + 예시 갱신) — 본 절의 3 파일 연속 불성립으로 조건의 실효 범위가 더 좁아져 **상향 유지**. (4) `R8` 조문화 (회색 5 종 + 병기 변형 **6 종** + 단위어 부착 규칙 + **저자 test**) — 선례 축적으로 **상향**. (5) `R2` · `R3` 병합 · anchor FU14 (낮음 유지).

**AC 7 자기 준수 · 범위 검산 1 구.** 신규 추가분 대상 자기 준수 grep (`grep -nE '[0-9]+ *– *[0-9]+|L[0-9]+'`) = **19** (본 절 **14** + task 파일 **5**) 로 전량 대조표 · 판정 문단 · 완료 요약의 **원 표기 인용** 이라 `R6` 예외다. `wc -l docs/use-cases/REQ-COVERAGE-AUDIT.md` **7323 → 7380** (신설 **57 행** · 절 본문 **56 행** 으로 ≤ 60 충족), `git diff --stat` **5 파일 · 94 삽입 / 37 삭제** (≤ 300 LOC · ≤ 5 파일), `git status --short` 에 `README.md` · `CLAUDE.md` · `docs/requirements.md` · `docs/PLAN.md` · `docs/architecture/` · 다른 `docs/decisions/*` · `.claude/agents/` · `docs/LOOP.md` · `src/` · `prisma/` **전무**, audit 파일 fence **178 (짝수)** · 신설 표 **6 컬럼 균일** · heading 순번 `12.84` → `12.85` **연속** · 대상 ADR 링크 수 **72 · 70 · 27 불변**. doc-only 라 `pnpm test` 불요 (`CLAUDE.md` §3.2 direct doc-only 면제).
