...
- **각주 — 임계 fix 시점**: 위 표의 "baseline 후 fix" 표기는 아래 baseline 실측 기록으로
  **실측 5 회분**(run `32459501970` · `32503914467` · `32524618230` · `32533779832` ·
  `32540981922`)이 확보된 상태다. 앞 2 회는 표본 10 명, 뒤 3 회는 표본 133 명이라 **실 scale
  축(표본 133)의 같은 조건 반복 표본이 3 개**가 됐고 그 batch p95 기술통계는 평균 **734.32ms** ·
  표본표준편차 **25.02ms**(변동계수 약 **3.41%**) 다(`§3.1` 5 회차). 분산이 작다는 근거는
  생겼지만 **표의 임계 숫자는 본 회차에서도 바꾸지 않는다** — 숫자 확정은 마진 설계를 포함한
  별도 slice 의 일이고, 본 회차는 그 착수가 가능하다는 판정까지만 한다(`§5` item 5 잔여 ②).

### 3.1 baseline 실측 기록 (S1, 5 회분)
...
5. **baseline 확정 + 임계 fix** — **S1 baseline 실측 5 회 완료**(T-1637 run **32459501970** ·
   T-1639 run **32503914467** · T-1641 run **32524618230** · T-1642 run **32533779832** ·
   T-1643 run **32540981922**, 앞 넷은 2026-08-21 · 다섯째는 2026-08-22,
   ...
   **760.91ms → 730.81ms → 711.23ms**(표본 133) 이고 `http_req_failed` 은 다섯 회 모두
   **0.00%**(0/26 · 0/26 · 0/272 · 0/272 · 0/272) 로
   임계를 모두 통과했으며 수치·환경 메타는 위 `§3.1` 에 회차별로 박제했다.
   **§3 표의 "baseline 후 fix" 임계 숫자는 본 회차에서도 무변경** — 실 scale 축 표본이 3 개가 돼
   분산 추정은 가능해졌지만, 숫자 확정은 마진 설계를 포함한 별도 slice 의 일이다(§3 각주).
   ...
   **표본 확보 측면은 해소** — 실 scale 축(표본 133)의 같은 조건 표본이 T-1643 run
   `32540981922` 로 **3 개**가 됐고(3·4·5 회차 760.91ms → 730.81ms → 711.23ms), 그 batch p95
   기술통계는 평균 **734.32ms** · 범위 **49.68ms** · 표본표준편차 **25.02ms** · 변동계수
   **3.41%** 다(계산 근거는 위 `§3.1` 5 회차). **판정: (a) 임계 확정을 별도 slice 로 착수한다.**
   근거는 세 가지다 — ㉠ 산포가 평균의 3~7%(표준편차 3.41% · 범위 6.77%) 로 작아 마진을 얹은
   임계 숫자를 정하기에 충분하고, ㉡ 세 값 모두 1h 예산의 약 **0.02%** 라 임계 후보와 실측 사이
   여유가 3 자릿수 배수이며, ㉢ 남은 지배적 불확실성은 분산이 아니라 **작업부하 대표성**(LLM
   stub · 수집 왕복 0 · 단일 iteration)이라 **같은 조건의 반복을 더 쌓아도 줄지 않는다**. 다만
   세 값이 단조 감소라 추세 성분을 배제할 수 없으므로(위 `§3.1` 5 회차 해석), 착수할 fix slice 는
   평균 기반의 빡빡한 숫자 대신 **max 또는 평균+k·표준편차 형태의 마진 임계**로 잡고 그 임계가
   **stub 조건 baseline** 임을 표에 함께 명시해야 한다. 본 slice 는 판정까지이고 `§3` 표 숫자는
   손대지 않았다. ③ 환경 메타 회수 경로
