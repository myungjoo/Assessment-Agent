...
| S1 평가 배치 부하 | `http_req_duration{route:batch}` p95 (관찰용) | ≤ 900ms (stub 조건 baseline, 표본 133) | 회귀 관찰 — REQ-047 판정 임계 아님 |
...
- **S1 관찰용 p95 임계 도출식(본 회차 확정)**: 표본 133 조건 batch p95 3 회분(760.91 ·
  730.81 · 711.23ms)의 평균 **734.32ms** + 3 × 표본표준편차 **25.02ms** = **809.38ms** 를
  100ms 단위로 올림해 **900ms** 로 굳혔다 — 실측 max **760.91ms** 보다 139.09ms 크다
  (기술통계 계산 근거는 `§3.1` 5 회차).
...
- **각주 — 임계 fix 시점**: 위 표의 "baseline 후 fix" 표기 중 **S1 축은 본 회차(T-1644)로
  ...
  **900ms**)을 택한 이유는 3 표본이 760.91 → 730.81 → 711.23 으로 **단조 감소**라 추세 성분을
  ...
  (`§5` item 5).
- **S1 관찰용 p95 게이트 재확정 규칙 (사전 박제, T-1668)**: 위 각주가 fix 한 `900ms` 를
  **언제 · 얼마나** 바꿀지를 다음 표본을 보기 **전에** 굳힌다 — 표본을 본 뒤 규칙을 정하면 그
  표본에 맞춘 사후 정당화(over-fitting)가 되기 때문이다. 판정자는 표본 수치만 대입하면 결론이
  하나로 나온다.
  - **① 재확정 트리거** — 다음 둘 중 **하나라도** 참이면 재확정에 착수한다. **(a)** 실 run 의 k6
    `THRESHOLDS` 출력에서 `p(95)<900` 이 `✗` 로 나온 경우(즉시). **(b)** 실 scale 표본(표본 133
...
5. **baseline 확정 + 임계 fix** — **S1 baseline 실측 6 회 완료**(T-1637 run **32459501970** ·
   ...
   **§3 표의 S1 축 임계 숫자는 T-1644 로 확정**됐다 — 관찰용 `http_req_duration{route:batch}`
   p95 **≤ 900ms(stub 조건 baseline, 표본 133)** + error rate **< 1%**(도출식·근거는 §3 각주).
   ...
   **임계 재확정 규칙 pointer (T-1668)**: 본 item 의 S1 관찰용 p95 `900ms` 를 언제 · 얼마나
