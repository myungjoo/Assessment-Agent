...
- **S2 · S3 의 `baseline 후 fix` 표기는 무변경**: 두 축은 baseline 실측이 아직 **0 회** 라
  ...
  **위 "실측 아직 **0 회**" 는 T-1644 시점 사실이며 현행이 아니다**(무효화 표기, T-1704 — 원문은
...
#### S2 · S3 `baseline 후 fix` 표기 해제 판단 (사전 박제, T-1704)

위 `§3` 임계 표에 남은 `baseline 후 fix` 태그를 **언제 · 무엇을 확인하면 떼는지**를 표본을 더
보기 **전에** 굳힌다 — `§3.1` 의 S2 1~6 회차 · S3 1~5 회차는 매 회차 "표본이 N 회뿐이라 재확정
근거가 되지 못한다" 고만 적어 왔고 그 N 의 문턱은 판정자마다 달라질 수 있으며, 표본을 더 본 뒤에
요건을 정하면 그 표본에 맞춘 사후 정당화(over-fitting)가 되기 때문이다. 바로 위
`#### S2 단계 분해 확대 판단 (사전 박제, T-1703)` 과 아래 `#### K6_SEED_PERSONS 상한 상향 판단
(T-1686)` 이 밟은 **"집행 전 판단 소절"** 서식(사실 확정 → 요건 열거 → 조건 2 개 → 분기 결론
3 값 → 재개 트리거)을 그대로 승계하며, 본 소절은 **새 측정 0 · 새 dispatch 0 · 코드 변경 0** 이다.

- **① 사실 확정 (기박제 원문 인용 · pointer 만 — 새 측정 · 새 통계 산출 0)**
  - **(i) 태그가 남은 칸은 정확히 2 개**다 — 위 표 S2 행의 `p50 latency / throughput |
    baseline 후 fix (관찰용)`(근거 열 "관찰") 과 S3 행의 `error rate (동시성 단계별) |
    < 1% (baseline 후 fix)`(근거 열 "graceful degradation"). 나머지 6 행에는 태그가 없다.
  - **(ii) 두 축의 회수된 실측**은 `### 3.1` 헤더 표기대로 **S2 6 회분** · **S3 5 회분**이다.
    그중 `#### S2 1 회차` 는 S1 leg 게이트 crossed 로 S2 step 이 `skipped` 라 **회수된 수치가
    0 개**이므로 수치가 있는 S2 표본은 `#### S2 2 회차` ~ `#### S2 6 회차` **5 개**이고, 그 5 회는
    S3 1~5 회차와 **같은 run**(32780975839 · 32798553930 · 32833365988 · 32843613484 ·
    32879776505)이다.
  - **(iii) S1 축은 이미 태그를 뗐다** — error rate 칸은 위 `S1 error rate < 1% 확정 근거` bullet
    대로 **baseline 5 회 run 의 `http_req_failed` 전량 `0.00%`**(0/26 · 0/26 · 0/272 · 0/272 ·
    0/272)로, 관찰용 p95 칸은 위 `각주 — 임계 fix 시점` 대로 **실 scale(표본 133) 3 표본**
    (760.91 · 730.81 · 711.23ms)의 평균 734.32 + 3 × 25.02 = 809.38 → 100ms 올림 **900ms** 로
    확정해 뗐다(현행 **1200ms** 는 그 뒤 두 차례 재산정의 결과 — T-1695 / T-1696 / T-1697).
- **② 해제 요건 3 항 (S1 선례에서만 승계 — 새 요건 발명 0)**
  - **(ㄱ) 표본 수** — S1 축이 해제한 시점의 표본 수를 그대로 쓴다: **관찰용 수치 칸은 동일 조건
    표본 3 개**(위 ①-(iii) 의 실 scale 3 표본), **error rate 칸은 동일 조건 표본 5 개**(위
    ①-(iii) 의 baseline 5 회 run). 그보다 더 요구하지도, 그 미만으로 낮추지도 않는다.
  - **(ㄴ) 조건 동일성** — 표본 **전량**의 **환경 메타 7 항목**(커널 · 아키텍처 · vCPU · 메모리 ·
    DB image · 부하 대상 image · 표본 인원 `K6_S1_PERSONS` / `K6_SEED_PERSONS`) · **부하 프로파일**
    (VU · 지속 또는 `stages`) · **dataset 규모**가 전부 같아야 한다. 하나라도 갈리면 그 표본은
    같은 축으로 섞지 않는다.
  - **(ㄷ) 산정식** — 수치 칸은 T-1668 규칙 ② 의 **`평균 + 3 × 표본표준편차` → 100ms 단위 올림**,
    error rate 칸은 S1 선례 그대로 **전 표본이 임계를 여유 있게 만족**. `max` 기반 · p99 기반 ·
    임의 배수 같은 **새 식은 발명하지 않는다**. T-1668 규칙 ③ 의 **outlier 제거 금지 · 축 혼합
    금지 · 하향 금지**도 그대로 승계한다.
- **③ 판정 규칙 (T-1686 · T-1703 서식 승계 — 조건 2 개 → 분기 결론 3 값)**
  - **ⓐ 그 칸이 (ㄱ) · (ㄴ) 요건을 현재 충족하는가** — 수치가 회수된 동일 조건 표본이 (ㄱ) 의 수를
    채우고 그 전량이 (ㄴ) 를 만족하면 **true**, 하나라도 미달이면 **false** 다.
  - **ⓑ 그 칸이 판정용인가 관찰용인가** — 위 표 "근거" 열이 REQ pass/fail 판정을 지면 **판정용**,
    "관찰" · "회귀 관찰" 이면 **관찰용**이다. 관찰용 칸은 태그를 떼도 **REQ 판정 임계가 한 자도
    움직이지 않는다**(S1 관찰용 p95 각주의 성격 구분과 같은 층위).
  - **분기 결론 3 값** — **ⓐ false → `해제 유예`**(ⓑ 무관 — 근거가 모자라면 떼지 않는다) ·
    **ⓐ true + ⓑ 판정용 → `해제 채택`**(판정 임계를 지는 칸을 잠정 상태로 남기지 않는다 — S1
    error rate 칸 선례가 정확히 이 경우다) · **ⓐ true + ⓑ 관찰용 → `해제 불요`**(떼도 판정면
    이동이 **0** 이라 얻는 것이 없고, ② -(ㄷ) 가 승계한 **하향 금지** 탓에 한 번 굳힌 관찰용
    숫자는 되돌릴 수 없어 조기 확정의 비용만 남는다). S1 관찰용 p95 칸이 관찰용인데도 해제된 것은
    그 숫자를 [`s1-batch.js`](../../test/load/s1-batch.js) 의 `STUB_BASELINE_P95_MS` 회귀 게이트로
    **실제 배선할 수요가 서 있었기 때문**(T-1645)이며, 그 수요는 아래 재개 트리거가 다룬다.
- **④ 결론 — 칸마다 1 값** (③ 을 위 ①-(i) 의 두 칸에 기계 대입한 결과)
  - **S2 `p50 latency / throughput`(관찰용) → `해제 불요`.** ⓐ 는 **true** 다 — 수치가 회수된
    동일 조건 표본이 **5 개**(S2 2~6 회차)로 (ㄱ) 의 관찰용 칸 요건 **3 개**를 채우고,
    `#### S2 6 회차` 가 "**7 항목이 S2 1~5 회차와 전부 동일**" 로 (ㄴ) 를 이미 확인했다(프로파일
    `vus: 5` · `duration: "20s"` 도 무변경 — 바로 위 T-1703 소절 ①). 그러나 ⓑ 가 **관찰용**이라
    결론은 `해제 불요` 다. **재개 트리거** — S2 p50 또는 throughput 을
    [`s2-read.js`](../../test/load/s2-read.js) 의 회귀 관찰 게이트 상수(S1 `STUB_BASELINE_P95_MS`
    동형)로 배선하거나 그 칸을 판정용으로 올리는 별도 판정이 서는 경우.
  - **S3 `error rate (동시성 단계별)`(판정용) → `해제 채택`.** ⓐ 는 **true** 다 — S3 표본
    **5 개**가 (ㄱ) 의 error rate 칸 요건 **5 개**를 정확히 채우고 그 전량이 `http_req_failed`
    **`✓ 'rate<0.01' rate=0.00%`**(0/22752 · 0/14867 · 0/24323 · 0/25205 · 0/23510)로 (ㄷ) 의
    "전 표본이 임계를 여유 있게 만족" 을 만족하며, 다섯 표본은 위 ①-(ii) 대로 S2 2~6 회차와
    **같은 run** 이라 (ㄴ) 의 환경 메타 7 항목 동일이 같은 인용으로 서고 ramping `stages` 와 공유
    dataset(133 건)도 5 회차 무변경이다(T-1682 행 수 로그 · T-1691 단계별 `Trend` 배선은 `seed` ·
    `teardown` tag 와 관찰 전용 metric 이라 판정 tag 오염 **0** — `S3 2 회차` · `S3 3 회차` 인용).
    ⓑ 는 **판정용**(근거 열 "graceful degradation")이므로 결론은 `해제 채택` 이다. **집행은
    T-1668 규칙 ④ split 승계** — 표의 숫자 `< 1%` 와
    [`s3-concurrent.js`](../../test/load/s3-concurrent.js) 의 임계 `rate<0.01` 이 **이미 같은
    값**이라 코드 변경이 **0** 이므로 문서 `direct` **단독 slice** 가 `(baseline 후 fix)` 표기만
    떼며, 판단과 집행을 **한 commit 에 합치지 않는다**.
  - **T-1701 이월 ③(`§3` 임계 표 재조정) 과의 관계 — 종결(집행 1 건만 승계).** 이월 ③ 은 위 두
    칸의 조정 여부를 묻는 항목이고 본 소절이 칸마다 결론 **1 값**을 박았으므로 **판단 축으로서는
    종결**이며, 남는 것은 `해제 채택` 이 나온 S3 칸의 **표기 집행 1 건**뿐이다.
- **⑤ 판정면 불변 · 코드 0 LOC** — 본 소절은 판단만 적는다. 위 `§3` 임계 표 8 행 ·
  `각주 — 임계 fix 시점`(T-1644) · `S1 관찰용 p95 게이트 재확정 규칙`(T-1668) 소절 · 기박제 회차
  본문은 **문자 단위 무변경**이고, `test/` · `src/` · `.github/workflows/` · `package.json` 의
  diff 는 **0 파일**이다.

#### 실 수집 왕복(§5 잔여 ①) 해소 경로 판단 (사전 박제, T-1706)

아래 `§5` item 5 에 마지막으로 남은 **잔여 ①(실 수집 왕복 0)** 을 두고 **무엇을 하면 이 잔여가
닫히는가 · 애초에 닫는 것이 본 문서 소관인가** 를 배선 **전에** 굳힌다 — 이 축은 8 · 9 회차부터
매 회차 "미해소 유지" 로만 승계돼 왔을 뿐 해소 경로가 어디에도 박제돼 있지 않아, 배선 slice 를
먼저 열면 자격증명 취급([CLAUDE.md](../../CLAUDE.md) `§5` BLOCKED 트리거) 여부를 코드 착수 뒤에
되묻게 되기 때문이다. 바로 위 `#### S2 단계 분해 확대 판단 (사전 박제, T-1703)` ·
`#### S2 · S3 baseline 후 fix 표기 해제 판단 (사전 박제, T-1704)` 이 밟은 **"집행 전 판단 소절"**
서식(사실 확정 → 후보 열거 → 조건 2 개 → 기계 대입 → 분기 결론 3 값 → 재개 트리거)을 그대로
승계하며, 본 소절은 **새 측정 0 · 새 dispatch 0 · 코드 변경 0** 이다.

- **① 사실 확정 (기박제 수치 인용만 — 새 측정 · 새 추정 0)**
  - **(ㄱ) seed 성공 run `32665014391`** — 아래 `§3.1` `#### 8 회차` 대로 `133 로그인 실 dataset
    seed 적재` step 이 `devset seed 완료 — person 133 건 / serviceIdentity 133 건 적재` 를 찍었다.
    즉 **`Person` 133 · github `ServiceIdentity` 133 은 실제로 적재된 상태**다.
  - **(ㄴ) 표본 로그 run `32677333740`** — 아래 `#### 9 회차` 대로 k6 console 이
    `[s1-batch] devset 표본 취득 133명 / 요청 133명` 을 남겨 표본 수 **133** 이 간접 추론이 아니라
    **직접 회수**로 확정됐고, seed 건수도 8 회차와 동일해 재현성이 **연속 2 회** 붙었다.
  - **(ㄷ) 그럼에도 왕복은 0** — 두 회차 모두 `http_reqs` 가 **7** 이고 `iteration_duration` 은
    8 회차 758.79ms · 9 회차 **825.88ms** 다. 50~100 repo · ~1000 page 를 실제로 도는 iteration
    이라면 나올 수 없는 크기이며, 이는 두 회차 소절이 이미 "실 수집 왕복은 여전히 0" 으로 적어 둔
    판정 그대로다.
  - **(ㄹ) 왕복이 0 인 이유는 두 가지로 이미 박제돼 있다** — LLM 은 `LOAD_TEST_STUB=1` **stub**
    ([ADR-0057](../decisions/ADR-0057-s1-batch-load-io-isolation.md) `D1`) 이고, 부하 job 에
    GitHub/Confluence **자격증명이 0** 이다(같은 ADR `D5` 가 seed 하는 것은 `LlmProviderConfig`
    단일 row 이며 어느 값도 실 provider 에 인증되지 않는다).
- **② 후보 3 종** — 잔여 ① 을 닫을 수단을 아래 **3 종**으로 굳히고 각각 (i) 비용 · (ii) 판정면
  영향 · (iii) 충돌하는 기박제 결정을 적는다.
  - **㉠ 실 자격증명 주입** — GitHub/Confluence token 을 부하 job 에 투입해 실 수집 왕복을 실제로
    발화시킨다. (i) repo secret 신설 + workflow env 배선 + 외부 API rate limit · 데이터 노출 검토가
    붙는다. (ii) batch iteration 안에서 실 수집 왕복이 돌면 `http_req_duration{route:batch}` 의
    표본이 **stub 조건이 아니게 되어** 1h 예산 외삽(ADR-0057 `D4`)의 측정 입력 자체가 바뀐다 —
    **판정면 이동 있음**. (iii) ADR-0057 `D1` 의 stub 격리 결정과 정면 충돌하고,
    [CLAUDE.md](../../CLAUDE.md) `§5` 의 **외부 자격증명 필요** · **Security/auth 관련 변경** 두
    BLOCKED 트리거를 함께 건드린다.
  - **㉡ 왕복 재현 대체안** — record-replay fixture 또는 수집 어댑터 stub 으로 왕복의 *shape*
    (호출 수 · 순서)만 재현하고 자격증명은 **0** 으로 둔다. (i) fixture 정본 + drift guard + 어댑터
    분기까지 별도 `pr` slice 몇 칸이 든다. (ii) 재현되는 것은 shape 뿐이고 **실 GitHub/Confluence
    latency 는 재현하지 못하므로** 그 위에서 얻은 batch p95 는 실 수집 조건의 판정 입력이 되지
    못한다 — 잔여 ① 이 요구하는 "실 수집" 을 정의상 만족하지 못하며 **판정면 이동 0**. (iii)
    ADR-0057 `D1` 과는 오히려 같은 방향(격리 유지)이라 충돌하는 기박제 결정은 **없다**.
  - **㉢ 해소 불요** — 잔여 ① 을 **판정면 밖의 관찰 축**으로 문서상 재정의한다. (i) 비용 **0**
    (새 배선 · 새 측정 없음). (ii) `§3` 표에서 REQ-047 pass/fail 을 지는 S1 판정 칸은 1h 예산
    외삽(`FULL_RUN_BUDGET_MS` · `BATCH_P95_MS`)이고 그 외삽은 ADR-0057 `D4` 대로 **stub 조건 축소
    표본을 전제로** 서 있으므로, 실 수집 왕복은 그 판정면 **밖**이다 — **판정면 이동 0**. (iii)
    충돌하는 기박제 결정 **없다**(오히려 `§3` 표 각주의 "stub 조건 baseline" 성격 구분과 같은
    층위다).
- **③ 조건 2 개 (T-1686 · T-1704 서식 승계)**
  - **ⓐ 판정면 이동 여부** — 그 후보가 `§3` 표의 **판정용 임계**(REQ-047 1h 예산 ·
    `FULL_RUN_BUDGET_MS` · `BATCH_P95_MS` 외삽)의 숫자를 문자 단위로 바꾸거나, 숫자를 그대로 두더라도
    그 임계가 재는 **표본 구성**(batch iteration 이 실제로 무엇을 도는가)을 바꾸면 **true**, 아니면
    **false** 다. T-1703 조건 ⓐ 의 "숫자를 그대로 두더라도 표본 구성을 바꾸면 true" 를 그대로 쓴다.
  - **ⓑ 자율 집행 가능 여부** — 그 후보를 [CLAUDE.md](../../CLAUDE.md) `§5` 의 BLOCKED 트리거
    (**새 외부 dependency** · **외부 자격증명 필요** · **Security/auth 관련 변경**)를 하나도 건드리지
    않고 agent 가 끝낼 수 있으면 **true**, 하나라도 건드리면 **false** 다.
  - **분기 결론 3 값** — **ⓐ false → `해소 불요`**(ⓑ 를 볼 필요도 없다 — 판정이 요구하지 않는
    축은 공백이 아니라 관찰 취향이며, T-1703 조건 ⓑ 의 "대응 칸이 없으면 그것은 공백이 아니다" 와
    같은 층위다) · **ⓐ true + ⓑ true → `자율 집행 채택`** · **ⓐ true + ⓑ false →
    `사람 승인 대기`**(판정면을 실제로 움직이지만 agent 권한 밖이라 `§5` 경로로 사람에게 올린다).
- **④ 결론 — 후보마다 1 값** (③ 을 ② 의 3 후보에 기계 대입한 결과이며 근거는 ① 인용뿐이다)
  - **㉠ 실 자격증명 주입 → `사람 승인 대기`.** ⓐ **true** — ②-㉠-(ii) 대로 batch 표본이 stub
    조건을 벗어나 판정면을 실제로 움직인다(그게 잔여 ① 이 존재하는 이유다). ⓑ **false** —
    ①-(ㄹ) 의 "자격증명 0" 을 뒤집는 일이라 `§5` 의 외부 자격증명 · security/auth 트리거에 정면으로
    걸린다.
  - **㉡ 왕복 재현 대체안 → `해소 불요`.** ⓐ **false** — ②-㉡-(ii) 대로 shape 만 재현해서는 실
    수집 조건의 판정 입력이 되지 못한다. ⓑ 는 자격증명 0 · 새 dependency 0 이면 **true** 지만
    규칙상 ⓐ false 에서 결론이 확정된다(왕복 *관찰* 수요가 따로 서면 그것은 잔여 ① 의 해소가 아니라
    별도 축의 신설이다).
  - **㉢ 해소 불요 → `해소 불요`.** 정의상 같은 값이며 근거는 ②-㉢-(ii) 다.
  - 따라서 `자율 집행 채택` 은 **0 건**이고, 결론 분포는 **`해소 불요` 2 건 + `사람 승인 대기`
    1 건**이다. **`해소 불요` 는 잔여 ① 항목을 지운다는 뜻이 아니다** — `§5` item 5 의 잔여 ①
    원문은 [CLAUDE.md](../../CLAUDE.md) `§12` 소급 치환 금지대로 **문자 단위 무변경**이고 잔여
    개수도 **1 개** 그대로이며, 본 소절이 add-only 로 그 성격(판정면 밖의 관찰 축)을 규정할 뿐이다.
- **⑤ 집행 경로** — `자율 집행 채택` 인 후보가 있으면 그 집행은 T-1668 규칙 ④ 를 승계해 **코드
  `pr` + 문서 `direct` 2 task split** 으로 **별도 slice 소관**이며, 판단과 집행을 한 commit 에
  합치지 않는다. 본 결론에는 그 값이 **0 건**이므로 split 은 **발화하지 않고 본 slice 는 아무것도
  배선하지 않는다**. `사람 승인 대기` 가 나온 ㉠ 은 **본 slice 에서 `humanQuestion` 을 만들지
  않는다**(본 slice 는 판단 박제만 한다) — 아래 재개 트리거 **T1** 이 발화할 때 후속 slice 가
  [CLAUDE.md](../../CLAUDE.md) `§5` 경로로 사람에게 올린다.
- **⑥ 재개 트리거 T1~T3 · 판정면 불변 · 코드 0 LOC** — 아래 관측이 나오면 본 판단을 다시 연다.
  **T1** 오너가 부하 job 용 GitHub/Confluence 자격증명을 제공하기로 결정하는 경우(㉠ 의 ⓑ 가 true 로
  뒤집혀 결론이 `자율 집행 채택` 이 되고, 집행은 `§5` 승인 뒤 규칙 ④ split 소관이다) ·
  **T2** 수집 어댑터에 credential-free 실 왕복 경로가 생겨 ㉡ 이 shape 이 아니라 실 latency 를
  재현하게 되는 경우(㉡ 의 ⓐ 가 true 로 뒤집힌다) · **T3** REQ-047 판정면이 실 수집 왕복을 포함하도록
  재정의되는 경우(`§3` 표에 실 수집 축 칸이 신설되거나 1h 예산 외삽의 stub 조건 전제가 바뀌는 별도
  판정 — ㉢ 의 ⓐ 가 true 로 뒤집힌다). 본 소절은 판단만 적으므로 위 `§3` 임계 표 **8 행** ·
  `각주 — 임계 fix 시점`(T-1644) · `S1 관찰용 p95 게이트 재확정 규칙`(T-1668) ·
  `#### S2 · S3 baseline 후 fix 표기 해제 판단 (사전 박제, T-1704)` 소절 · 기박제 회차 본문은
  **문자 단위 무변경**이고, `test/` · `src/` · `web/` · `.github/workflows/` · `package.json` 의
  diff 는 **0 파일 · 0 LOC** 이다.
...
5. **baseline 확정 + 임계 fix** — **S1 baseline 실측 6 회 완료**(T-1637 run **32459501970** ·
   ...
   **문자 단위 무변경**이며, 2 표본에 규칙을 대입하는 기계 판정은 여전히 **별도 slice 소관**이다.
    ...
    **그 표에 남은 `baseline 후 fix` 태그 2 칸을 언제 뗄지도 T-1704 가 집행 전에 굳혔다** —
    `§3` 에 `#### S2 · S3 baseline 후 fix 표기 해제 판단 (사전 박제, T-1704)` 소절을 add-only 로
    신설해 S1 선례에서만 끌어온 해제 요건 3 항((ㄱ) 표본 수 — 관찰용 수치 칸 **3 개** · error rate
    칸 **5 개** · (ㄴ) 환경 메타 7 항목 · 프로파일 · dataset 규모 동일 · (ㄷ) 산정식은 평균+3σ
    100ms 올림 / 전 표본 임계 여유 만족이며 새 식 발명 금지 + T-1668 규칙 ③ 승계)과 조건 2 개
    (ⓐ 요건 충족 · ⓑ 판정용/관찰용) · 분기 결론 3 값(`해제 채택` · `해제 유예` · `해제 불요`)을
    박았다. 기계 대입 결과는 **S2 `p50 latency / throughput` → `해제 불요`**(ⓐ true 이지만 ⓑ
    관찰용이라 떼도 REQ 판정 임계 이동 **0**) · **S3 `error rate` → `해제 채택`**(표본 5 회 전량
    `http_req_failed` **0.00%**)이고, 후자의 집행은 규칙 ④ split 대로 **코드 0 LOC · 문서
    `direct` 별도 slice** 소관이라 본 slice 는 표기를 실제로 떼지 않는다. 함께 `§3` 의
    "S2 · S3 … 실측이 아직 **0 회**" bullet 에 무효화 표기를 add-only 로 덧붙여(원문은 `§12`
    ...
    **그 세 항목이 닫히면서 남은 잔여 ① 의 해소 경로 자체를 T-1706 이 배선 전에 판단했다** —
    `§3` 에 `#### 실 수집 왕복(§5 잔여 ①) 해소 경로 판단` 소절(사전 박제, T-1706)을 add-only 로
    신설해, 기박제 사실 4 항((ㄱ) seed 성공 run **32665014391** 의 `person 133 건 /
    serviceIdentity 133 건` · (ㄴ) 표본 로그 run **32677333740** 의 `표본 취득 133명 / 요청 133명` ·
    (ㄷ) 두 회차 `http_reqs` **7** · iteration **825.88ms** · (ㄹ) LLM **stub**(ADR-0057 `D1`) 과
    자격증명 **0**) 위에 후보 **3 종**(㉠ 실 자격증명 주입 · ㉡ 왕복 재현 대체안 · ㉢ 해소 불요)과
    조건 2 개(ⓐ 판정면 이동 여부 · ⓑ 자율 집행 가능 여부) · 분기 결론 3 값(`자율 집행 채택` ·
    `사람 승인 대기` · `해소 불요`)을 굳혔다. 기계 대입 결과는 **㉠ → `사람 승인 대기`**(ⓐ true ·
    ⓑ false — [CLAUDE.md](../../CLAUDE.md) `§5` 의 외부 자격증명 · security/auth 트리거) ·
    **㉡ → `해소 불요`**(ⓐ false — shape 만 재현해서는 실 수집 조건의 판정 입력이 못 된다) ·
    **㉢ → `해소 불요`** 라 `자율 집행 채택` 이 **0 건**이고, 따라서 T-1668 규칙 ④ split 은
    발화하지 않아 **본 slice 는 아무것도 배선하지 않으며** ㉠ 에 대한 `humanQuestion` 도 만들지
    않는다(재개 트리거 **T1** 발화 시 후속 slice 가 `§5` 경로로 올린다). `해소 불요` 는 항목을
    지운다는 뜻이 아니므로 위 잔여 ① 원문은 `§12` 소급 치환 금지대로 **문자 단위 무변경**이고
    **잔여 개수 1 개 유지** · ② · ③ 표기도 그대로다. 본 slice 역시 새 dispatch · rerun · 실측
    **0** · 코드 **0 LOC** 이라 회분 표기(S1 **16 회** · S2 **6 회** · S3 **5 회**) 와 `140 행`
    checkbox `[ ]` 는 무변경이다.
