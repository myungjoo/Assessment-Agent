...
#### S2 단계 분해 확대 판단 (사전 박제, T-1703)

위 두 소절(T-1687 설계 · T-1698 판정 규칙)이 굳힌 단계 분해 축을 **S2 로 확대할지**를 배선
**전에** 판단한다 — `§3.1` `#### S2 5 회차` · `#### S2 6 회차` 가 "단계 분해는 S2 축에서 여전히
미확보" 를 이월했으므로, 배선 slice 가 이 공백을 즉석에서 메우면 축 정의가 구현에 맞춰 사후
정당화된다. 아래 `#### K6_SEED_PERSONS 상한 상향 판단 (T-1686)` 이 밟은 **"배선 전 판단 소절"**
형태(조건 열거 → 증거 → 분기 결론 → 재판단 트리거)를 그대로 승계하며, 본 소절은 **새 측정 0 ·
새 dispatch 0 · 코드 변경 0** 이다.

- **① 사실 확정 — S2 에는 이식할 단계 축 자체가 없다(기박제 원문 인용)** —
  [`s2-read.js`](../../test/load/s2-read.js) `57~78 행` 의 `options` 는 `vus: 5` ·
  `duration: "20s"` 의 **상수 VU** 이고 `options.stages` 정의는 **0** 이다. 같은 블록의 머리
  주석도 **"반복 조회 프로파일 — smoke(1 VU × 1 iteration) 보다 큰 지속 부하. ramping stages 는
  S3(동시성 내성) 소관이라 쓰지 않는다."** 로 그 부재를 **의도**로 이미 못 박아 두었다. 따라서
  T-1687 조항 ③ 의 "단계 값 = `options.stages` 경계와 1:1 대응하는 분기 없는 산술 1 식" 은 S2 에
  **그대로 이식할 대상이 없다**. 인용 사실이며 새 측정 · 추정은 **0** 이다.
- **② 축 후보 3 종** — S2 에 단계 축을 만들 수단을 아래 **3 종**으로 굳히고 각각 (i) 무엇을
  cover 하는지 · (ii) 판정면을 움직이는지 · (iii) 새 외부 dependency 를 늘리는지를 적는다.
  - **㉠ `duration` 시간 등분 — 경과시간 → 구간 index 산술 1 식으로 tag** — (i) 20s 지속 구간을
    등분해 *시간 경과* 축의 지연 추이를 cover 한다(부하 증량 축은 cover 하지 못한다 — VU 가 내내
    **5** 로 상수라 구간 간 부하 차이가 **0** 이다). (ii) 판정 축(`route`)과 직교한 새 tag key
    1 개 + custom `Trend` 로만 값을 만들면 `thresholds` **6 항목** 과 전역 · route 4 종
    sub-metric 의 표본은 문자 단위 그대로다(T-1687 조항 ③ · ⑥-(다) 승계) — **판정면 이동 0**.
    (iii) `k6/metrics` 는 k6 런타임 내장이라 **새 dependency 0**.
  - **㉡ `s2-read.js` 에 ramping `stages` 신규 도입** — (i) VU 증량 축의 저하 곡선을 cover 한다
    (S3 와 동형). (ii) `vus: 5` · `duration: "20s"` 상수 프로파일 자체를 갈아치우므로 임계 숫자를
    한 자도 건드리지 않아도 전역 `http_req_duration` 과 `{route:persons|groups|parts|me}` 4 종
    sub-metric 의 **표본 구성이 통째로 바뀐다** — **판정면 이동 있음**. ① 이 인용한 "ramping
    stages 는 S3 소관" 머리 주석 규약과도 정면 충돌한다. (iii) **새 dependency 0**.
  - **㉢ 확대하지 않는다** — (i) 새로 cover 하는 것은 **없다**. S2 는 이미 `route` 축 **4 종**
    (`persons` · `groups` · `parts` · `me`) 분해를 갖고 있고, 위 `§3` 표에서 `latency cliff 부재`
    판정을 지는 행은 **S3 행 하나뿐**이라 S2 축은 단계 분해의 판정 수요 자체가 없다. (ii) 판정면
    이동 **0**. (iii) **새 dependency 0**.
- **③ 판정 규칙 (T-1686 서식 승계 — 조건 2 개 → 분기 결론 3 값)**
  - **ⓐ 그 후보가 S2 의 판정면을 움직이는가** — `thresholds` **6 항목**(전역
    `http_req_duration` · `http_req_failed` · `{route:persons}` · `{route:groups}` ·
    `{route:parts}` · `{route:me}`)의 **임계 숫자를 문자 단위로** 바꾸거나, 숫자를 그대로 두더라도
    **표본 구성**(VU 수 · 지속 프로파일 · sub-metric 분할)을 바꾸면 **true** 다. 근거는 T-1687
    조항 ② — "export 는 관찰 전용이고 판정 임계는 불변" 이다.
  - **ⓑ 그 후보가 닫는 공백이 `§3` 표의 어느 칸에 대응하는가** — 대응 칸이 있으면 **true**,
    없으면 **false** 다. **대응 칸이 없으면 그것은 공백이 아니다** — 판정이 요구하지 않는 축의
    관찰 취향일 뿐이며, 결론을 표 칸에 1:1 로 귀속시킨 T-1698 조항 ④ 와 같은 층위다.
  - **분기 결론 3 값** — **ⓑ false → `확대 불요`**(닫을 칸이 없으므로 ⓐ 를 볼 필요도 없다) ·
    **ⓑ true + ⓐ false → `확대 채택`** · **ⓑ true + ⓐ true → `확대 유예`**(판정면을 움직이는
    변경은 본 소절이 아니라 임계 재산정의 **별도 판정** 소관이라 그 판정 뒤로 미룬다).
- **④ 결론 — `확대 불요`** (③ 을 ② 의 3 후보에 기계 대입한 결과이며 세 후보가 같은 값으로 모인다).
  - **㉠** — ⓑ **false** 다: 위 `§3` 표 S2 행은 `p95 latency` · `p50 latency / throughput` ·
    `error rate` **3 칸**뿐이고 어느 칸도 단계(시간 구간) 축을 요구하지 않는다. ⓐ 는 false 지만
    규칙상 ⓑ false 에서 결론이 확정돼 **`확대 불요`**.
  - **㉡** — ⓑ **false**(같은 이유) · ⓐ **true** → **`확대 불요`**. ⓑ 가 true 였더라도 ⓐ true 라
    `확대 유예` 였을 뿐 채택은 아니다.
  - **㉢** — 정의상 **`확대 불요`** 와 같다.
  - 따라서 결론은 **`확대 불요` 1 값**이고, T-1687 조항 ⑤ 의 split(코드 `pr` → 문서 `direct`)은
    **발화하지 않는다** — S2 확대 배선 `pr` slice 는 **만들지 않는다**.
  - **재개 트리거 (아래 관측이 나오면 본 판단을 다시 연다)** — **T1** `§3` 표 S2 행에 단계(부하
    단계별 또는 시간 구간별) 축 칸을 신설하는 별도 판정이 서는 경우 · **T2** S2 프로파일을 상수
    VU 에서 ramping 으로 바꾸는 별도 판정이 확정되는 경우(그때는 ⓐ 가 그 판정에서 이미 처리된
    뒤다) · **T3** S2 회차의 전역 p95 가 임계 `3000ms` 의 절반(**1500ms**)을 넘어 지연의 시간 축
    분해가 진단에 실제로 필요해지는 경우.
  - **T-1701 이월 문장과의 관계 — 대체** — `§3.1` `#### S2 5 회차` · `#### S2 6 회차` 의 "단계
    분해는 S2 축에서 여전히 미확보 … `s2-read.js` 로의 확대 배선은 별도 `pr` slice 소관" 문장은
    **그 시점 사실로 문자 단위 그대로 둔다**([CLAUDE.md](../../CLAUDE.md) `§12` 소급 치환 금지).
    다만 그 문장의 **집행 예고**(`pr` slice 로의 확대 배선) 부분은 본 소절의 `확대 불요` 결론이
    **대체**하며, 위 재개 트리거가 발화하기 전에는 그 slice 를 만들지 않는다.
- **⑤ 판정면 불변 · 코드 0 LOC** — 본 소절은 판단만 적는다.
  [`s2-read.js`](../../test/load/s2-read.js) 의 `thresholds` · `vus` · `duration` ·
  `summaryTrendStats` 는 물론 `test/` · `src/` · `.github/workflows/` · `package.json` 의 diff 가
  **0 파일**이고, 위 `§3` 임계 표 · `#### 단계별 percentile export 설계` ·
  `#### S3 latency cliff 판정 규칙` 소절도 **문자 단위 무변경**이다.
...
5. **baseline 확정 + 임계 fix** — **S1 baseline 실측 6 회 완료**(T-1637 run **32459501970** ·
   ...
   **문자 단위 무변경**이며, 2 표본에 규칙을 대입하는 기계 판정은 여전히 **별도 slice 소관**이다.
    ...
    **그 단계 축을 S2 로 확대할지의 판단은 T-1703 이 배선 전에 닫았다** — `§3` 의
    `#### S2 단계 분해 확대 판단 (사전 박제, T-1703)` 소절을 add-only 로 신설해, S2 프로파일이
    `vus: 5` · `duration: "20s"` **상수 VU** 이고 `options.stages` 정의가 **0** 이라는 기박제
    사실 위에 축 후보 **3 종**(㉠ `duration` 시간 등분 tag · ㉡ ramping `stages` 신규 도입 ·
    ㉢ 확대하지 않음)과 T-1686 서식을 승계한 조건 2 개(ⓐ 판정면 이동 여부 · ⓑ `§3` 표 대응 칸
    존재 여부) · 분기 결론 3 값(`확대 채택` · `확대 유예` · `확대 불요`)을 굳혔다. 기계 대입
    결과는 3 후보 모두 **ⓑ false**(표 S2 행 3 칸 중 단계 축을 요구하는 칸이 없다) 라 결론은
    **`확대 불요` 1 값**이고, 따라서 T-1687 조항 ⑤ 의 split 은 발화하지 않아 **S2 확대 배선
    `pr` slice 는 만들지 않는다**(재개 트리거 T1~T3 만 남긴다). T-1701 이 이월한 "별도 `pr`
    slice 소관" 문장은 그 시점 사실로 **문자 단위 그대로 두고** 집행 예고 부분만 본 결론이
    대체한다. 본 slice 역시 새 dispatch · rerun · 실측 **0** · 코드 **0 LOC** 이라 회분 표기
