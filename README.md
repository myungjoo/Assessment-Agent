...
# 계정·평가 대상 관리 UX (오너 요구 2026-08-26)

## 계정 생성 UX

- 계정 생성 화면(첫 로긴 SuperAdmin 지정, Admin 의 사용자 추가 모두 포함)은 아이디와 암호가 만족해야 하는 조건(허용 문자·형식·최소/최대 길이 등)을 입력 전에 화면에 안내하여야 한다.
- 조건 미충족으로 계정 생성이 실패하면 어떤 입력이 어떤 조건을 위반했는지 구체적인 사유를 사용자에게 표시하여야 한다. "이미 등록된 사용자이거나 입력이 올바르지 않습니다" 같은 포괄 오류 문구 하나로 뭉뚱그리는 것은 금지한다 — 사용자가 무엇을 고쳐야 할지 알 수 없기 때문이다.
- 아이디 중복 오류와 형식/길이 위반 오류는 구분하여 표시한다.

## 평가 대상 관리 UI

- 현재 로그인 직후 화면에는 "평가 대상을 선택하면 결과가 표시됩니다" 문구만 표시되어, 평가 대상이 등록되지 않은 초기 상태에서는 사용자가 다음 행동을 진행할 수 없다.
- 로그인 후 화면에서 평가 대상을 추가·편집할 수 있는 인터페이스를 제공하여야 한다. 평가 대상은 두 종류다:
    - 평가 대상 인원 (대상자) — "평가 대상 인원" 섹션의 추가/삭제/변경/Deactivate/Activate 를 Web UI 상에서 수행할 수 있어야 한다.
    - 평가 대상 시스템 — GitHub (github.com 등) 의 organization/repository 지정, Confluence 주소(base URL)와 SPACE 지정 등 수집 대상 시스템의 등록·편집.
- 편집은 Admin 등급만 수행할 수 있고 User 등급은 조회만 가능하다 (보안 특성 섹션과 일관).


# UI 실동작·기본기 개선 (오너 요구 2026-08-26)

## 대시보드 실동작

- 대시보드 화면 안에서 평가 대상 인원을 직접 선택할 수 있어야 한다. 인원 미선택 안내문만 보이고 선택 수단이 없는 상태는 허용하지 않는다.
- 평가 결과 표시(테이블·상세 패널·점수 분포·시계열)는 backend 가 실제로 반환하는 필드(양 volume, 난이도 difficulty, 점수 contributionScore, 평가문 narrative, 기간 period/periodStart)와 계약이 일치하여야 하며, 실데이터가 화면에 렌더되는지 검증까지 포함한다. 프런트가 기대하는 필드명이 backend 응답과 달라 값이 0/빈칸으로 표시되는 상태는 결함으로 간주한다.
- 점수 분포 등 시각화의 축·구간은 실제 지표 스케일에 맞춘다 (0–100 임의 가정 금지 — 실제 metricScore 스케일 기준).
- 조회 기간(일/주/월 + 시작 시점)을 지정하는 UI 를 제공하고, 사용자 지정 기간 평가 (POST /api/assessment-evaluation/period) 도 UI 에서 호출할 수 있어야 한다.

## 평가 대상 인원의 서비스 ID 매핑 관리

- 인원별 서비스 ID 매핑("평가 대상 인원" 섹션 — 서비스별 ID, primary ID 지정 포함)을 조회·추가·수정·삭제하는 API 와 Admin UI 를 제공하여야 한다.
- 인원 추가/편집 동선에서 서비스 ID 매핑까지 이어서 입력할 수 있어야 한다 — 이름/email 만 입력 가능한 상태로는 인원이 수집 대상으로 실제 연결되지 않는다.

## UI 기본기

- 전역 스타일(CSS)을 도입하여 화면이 브라우저 기본 스타일의 세로 나열이 아닌, 구획·간격·표 스타일이 있는 실사용 가능한 형태여야 한다. 관리 화면의 다수 섹션은 탭 또는 구획 내비게이션으로 나눈다.
- 로그아웃 기능을 제공한다 (backend 세션/쿠키 무효화 포함).
- 새로고침 시 유효한 세션이면 로그인 화면으로 되돌리지 않고 인증 상태를 복원한다.
- 평가 진행 중 경고 배너는 실행 상태를 자동 갱신(polling)으로 반영한다 — 실행 상태 조회 endpoint 신설 포함.
- 폼 오류 등 여러 줄 안내는 줄 단위로 구분되어 표시한다 (한 줄 합침 금지).
