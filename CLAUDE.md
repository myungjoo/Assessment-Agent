...
1. **활성 driver 는 항상 1개** — 진입점(cron + 여러 기기 `/loop`)은 여럿 무장해도 되나, ref-CAS 가 직렬화해 한 순간 한 driver 만 작업한다. 나머지는 lock 점유를 보고 즉시 종료(no-op). **각 진입점은 lock 점검·STATE read 전 반드시 `git fetch`** 하고, driver loop 는 origin/main 추적 체크아웃에서만 실행한다(feature-worktree 금지 — LOOP.md §1[1]·§4). **cloud `/loop` 포함 모든 진입점은 권위 `claude/lock-driver` CAS 획득을 무조건 선행 수행한다 ([ADR-0034](docs/decisions/ADR-0034-cloud-entrypoint-mandatory-lock-cas.md))** — `gh 부재`·`main 직접 push 불가` 는 lock-skip 근거가 아니며(lock 은 raw `git push` 로 허용 prefix `claude/*` 에 CAS), 미획득 시 mirror-only 격리 작업 없이 즉시 stand-down(no-op) 한다. 위반 시 2026-06-09 #246/#247 처럼 중복-PR 사고가 난다(두 cloud `/loop` 세션이 권위 lock 미변경 채 같은 task 동시 수행).
