...
6. `docs/requirements.md` 의 **REQ status 재판정 task 는 그 REQ 를 구현하는 slice 가 머지된 뒤 1 회만** 생성한다 (판정 주체는 planner). 같은 구현 arc 안에서 구현 **전** 재판정 task 를 별도로 만드는 것은 금지한다 — 구현 전에 필요한 "이 REQ 가 아직 유효한가" 판단은 planner 의 **issue-still-relevant pre-check** ([.claude/agents/planner.md](.claude/agents/planner.md) `§ Pre-check: issue-still-relevant` — "task 의 의도 (target endpoint / symbol / 변경 의도) 를 task title + Why 절에서 추출", "doc-only task: 대상 파일의 § / row 가 이미 박제됐는지 확인") 로 수행하고, 그 결과는 **구현 task 의 `Why` / `plannerNote` 에 박제**한다. 별도 task · 별도 commit 으로 승격하지 않는다.
   - **예외 (a) — 구현 arc 와 무관한 drift 정정**: 진행 중인 구현 arc 와 무관하게 발견된 status drift (오래 전 shipped 인데 `PLANNED` 로 방치된 REQ 등) 의 정정은 언제든 허용한다. 본 룰이 막는 것은 **한 REQ 를 한 구현 arc 안에서 두 번** 판정하는 왕복이지 재판정 자체가 아니다 — `requirements.md` 의 status 가 실제 shipped 상태와 어긋나는 것은 진짜 결함이다.
   - **예외 (b) — 기준은 arc 당 1 회가 아니라 REQ 당 1 회**: 구현 arc 가 여러 REQ 에 걸쳐 있고 중간 REQ 가 먼저 완결됐다면, 그 REQ 의 재판정은 arc 종료를 기다리지 않아도 된다.
   - **근거 (실측)**: 2026-08-29 ~ 08-31 3 일간 commit 183 건 중 48 건이 재판정 · 승격 · 마커 · doc-sync · 정정 성격이었고, 그 안에 REQ-075 ([T-1792](docs/tasks/T-1792-requirements-req075-display-contract-rejudge.md) 구현 전 → [T-1795](docs/tasks/T-1795-requirements-req075-narrative-rejudge.md) 구현 후) · REQ-077 ([T-1798](docs/tasks/T-1798-requirements-req077-period-ui-rejudge.md) 구현 전 → [T-1800](docs/tasks/T-1800-requirements-req077-query-axis-rejudge.md) 구현 후) 처럼 같은 REQ 를 구현 앞뒤로 두 번 판정한 왕복 쌍이 있다. 두 task 의 `Why` 를 대조하면 **구현 전 재판정이 만든 유일한 정보는 T-1798 의 "조회(GET) 축 잔여" 인데, 그것은 이미 planner pre-check 가 T-1798 의 `Why` 에 적어둔 사실을 row 로 옮긴 것이었고 구현 후 T-1800 이 같은 REQ 를 다시 실측해 덮어썼다** — 즉 구현 전 재판정은 새 정보를 만들지 않고 task 문서 · `direct` commit · fire 만 소비했다.
...
본 시스템의 lock은 [ADR-0028](docs/decisions/ADR-0028-cloud-proxy-branch-lock.md)(ADR-0009 invariant 보존) 에 따라 **`refs/heads/claude/lock-driver` 브랜치(commit ref) tip 의 `git push --force-with-lease` CAS** 로 동작하는 **강한 mutex**다. ref push 는 서버 측 원자적 연산이라 N 개 기기·진입점이 동시에 push 해도 1개만 lock 을 획득한다 — **여러 기기의 `/loop` + cron 을 동시에 무장(armed)해도 안전**하다(어느 순간에도 활성 driver 는 정확히 1개). `claude/*` 는 cloud proxy 가 기본 허용하는 prefix 라 cron@cloud 도 credential 0 으로 자율 lock CAS 가 가능하다(옛 전용 blob ref `refs/locks/driver` 는 proxy 가 403 으로 거부했었다 — ADR-0028 이 저장 위치만 `claude/lock-driver` 브랜치로 이전해 해소; CAS 원자성·강한 mutex·multi-machine 안전·60분 stale·read 전 fetch invariant 는 ADR-0009 그대로 불변). 단 아래 read-전-fetch 규율이 전제다. (과거에는 STATE.json 인메모리 lock 의 **약한 mutex** 였고 동시 실행을 정책으로 회피했다 — ADR-0009 가 이를 대체.)

1. **활성 driver 는 항상 1개** — 진입점(cron + 여러 기기 `/loop`)은 여럿 무장해도 되나, ref-CAS 가 직렬화해 한 순간 한 driver 만 작업한다. 나머지는 lock 점유를 보고 즉시 종료(no-op). **각 진입점은 lock 점검·STATE read 전 반드시 `git fetch`** 하고, driver loop 는 origin/main 추적 체크아웃에서만 실행한다(feature-worktree 금지 — LOOP.md §1[1]·§4). **모든 진입점(cron · 로컬/cloud `/loop` · headless)은 작업 시작 전 권위 `claude/lock-driver` CAS 획득을 무조건 선행 시도해야 한다 — `gh 부재`·`main 직접 push 불가`는 lock-skip 근거가 아니며(lock 은 raw git push·`claude/*` prefix·main 아님), 미획득 시 stand-down(no-op 종료)이고 feature branch + draft PR 격리 병렬 작업은 금지**(ADR-0034 흡수, #246/#247 중복-PR 사고 차단 — LOOP.md §1[1]·§4).

   - **토글-gated N-driver 경로 (ADR-0036 fine-grained concurrency, `flags.fineGrainedConcurrency` 기준 분기)**: 위 "활성 driver 는 항상 1개"(coarse mutex — 한 task 전체 cycle 동안 단일 lock 점유)는 **토글 OFF(`flags.fineGrainedConcurrency=false`) 시의 동작**이다 — 단 **현 상태는 stage 5b ON(T-0350: `=true` + `maxConcurrentClaims=2`)** 으로 `commitMode: direct` task 끼리만 동시 claim 2개 허용(pr-mode 는 활성 claim 0 일 때만 단독 — LOOP.md §1[2] (a2) direct-only 조건)이며, §D8 (d) 회로 차단기 강등 시 OFF 로 복귀한다. [ADR-0036](docs/decisions/ADR-0036-fine-grained-concurrency.md)(ACCEPTED, ADR-0009 "활성 driver 1개" invariant 부분 supersede)는 토글 ON 시 동작을 다음으로 좁힌다: **lock 을 critical section(claim 박제 + STATE/journal/counters write + lock ref 자체) 점유 시에만 짧게** 잡고, implement/test/review/CI-wait 는 lock-free 로 둔다. task 소유는 lock 이 아니라 **claim registry**(lock 과 동일한 `claude/lock-driver` 브랜치 tip tree 의 `claims.json` 배열)로 표현해, **lock 보유 driver 는 한 시점 1개지만 claim 보유 = task 진행 중 driver 는 N 개**가 될 수 있다(파일-disjoint·의존성 없는 독립 task 끼리만 동시 claim — ADR-0036 §Decision 0). select+claim 은 lock 하에서 atomic 하므로 이중 claim 0(ADR-0036 §Decision 1), orphan claim 은 lock stale 과 동형 60분(server-time 기준, ADR-0036 §Decision 5)으로 회수하며 `prNumber` 보유 claim 은 새 PR 생성 대신 resume 한다(#246/#247 중복-PR 차단). 보존 불변: CAS 원자성·branch-ref 저장(ADR-0028)·read 전 fetch·60분 stale·STATE single-writer(§9)·counters origin+1 — 전부 그대로(ADR-0036 §Decision 1 "보존" 목록).
   - **이 N-driver 서술은 토글 OFF 동안 forward-looking spec — driver 동작 불변**: `flags.fineGrainedConcurrency=false`(회로 차단기 강등 시 복귀값)면 driver loop 는 위 "활성 driver 는 항상 1개" coarse mutex 그대로 동작하며, 본 doc-sync 는 driver 행동을 **바꾸지 않는다**. loop-level 토글-gated 분기(critical-section reclaim + select+claim pickup, server-time `RECLAIM_NOW` 주입 계약, claimable 부재 시 no-op)는 [LOOP.md §1[2]](docs/LOOP.md)(ADR-0036 stage3 slice1)에 박제돼 있고 토글 ON(rollout stage5) 일 때만 진입한다. 운영 면(per-PR CI concurrency group·관측성)은 [docs/architecture/concurrency.md](docs/architecture/concurrency.md) 참조. 토글은 stage 5a ON(T-0348) → 5a 게이트 충족 실측(T-0349 무사고 실사용 fire — concurrency.md §7) 후 **stage 5b**(T-0350 direct, `maxConcurrentClaims=2`, direct-only 병렬)로 진입; 5c 진입은 5b 정확성 게이트(이중 claim 0 + bookkeeping 충돌 0) 통과 후 별도 task.
   - **stage 5 기본-ON 안전장치 5종 (ADR-0036 §Decision 8, 2026-06-10 T-0341 amend)**: 토글 ON 의 안전성은 "최악의 경우의 동작이 coarse 단일-driver(ADR-0009) 와 정확히 같다"는 원칙에서 나오며 — (a) **fail-safe 강등**(claim-pickup 판정 불확실 시 단일-task fallback) · (b) **claim 시점 런타임 재검증**(driver 가 후보의 `touchesFiles` 교집합 + `dependsOn` 머지 상태 실검사) · (c) **integrator merge 직전 rebase + CI green 재확인**(semantic conflict 의 마지막 그물) · (d) **`concurrencyIncidents` 회로 차단기**(`double-claim`/`merge-conflict-code`/`reclaim-misfire`/`ci-cost-overrun` 슬러그 같은 유형 2회 누적 시 lock-하 자동 OFF 강등 + notifier) · (e) **5a→5b→5c 3단계 이행**(5a `maxConcurrentClaims=1` 메커니즘 검증 → 5b direct-only 병렬 → 5c 전면 병렬 + 30일 dogfood) 5종이 stage 5 진입 전 박제·구현돼야 한다. 본 5종 박제·구현은 별도 task chain — 본 CLAUDE.md 단락은 **인지 박제만** 이며 토글 OFF 동안 driver 동작 변화 0(여전히 `flags.fineGrainedConcurrency=false`).
2. **`/schedule` cron 간격 ≥ 평균 task 소요시간 × 2**. 예: task 평균 15분이면 cron 간격 ≥ 30분. 처음엔 2시간 간격으로 시작해 안정화되면 조정. **multiTaskFire 활성 시 재조정** ([§2.5](#25-multi-task-fire-실험적-기본-off), [ADR-0020](docs/decisions/ADR-0020-multi-task-fire-cron-n2-activation.md)): `flags.multiTaskFire` 가 `true` 이면 한 cron fire 가 N=2 task 를 연속 처리할 수 있으므로 cron 간격 ≥ `(N × 평균 task 소요시간) × 2 = (2 × 평균) × 2` 로 scale 한다. 예: task 평균 15분이면 활성 시 cron 간격 ≥ `(2×15)×2 = 60분`. 근거: §2.5 (d) 의 lock 45분 임계가 §2 step 2 의 60분 stale 임계를 보호 — 2-task fire 가 lock 점유를 60분 너머로 끌고 가지 않도록 간격을 키운다. 본 재조정은 `holder=cron` + flag 활성 시에만 적용되며, `/loop`(holder=loop) 단일-task 가이드(평균 15분 → 30분)는 위 그대로 유지한다.
...
규칙 2·3 (cron 간격·시간대 분리)은 **강한 mutex 도입 후에도 권장**이다 — 충돌이 안전하게 흡수되더라도, 무의미한 wake·즉시종료의 비용(LLM/CI)을 줄여준다. 규칙 4·5 의 충돌 흡수 동작은 branch-ref CAS 가 lock 취득 단계에서 1차 직렬화하고, 그래도 겹치는 콘텐츠 push 는 LOOP.md §4 graceful 종료가 2차로 흡수한다. ADR-0009 가 과거 "multi-operator 환경 필요 시 강한 mutex(별도 ADR)로 전환" 예고를 실제로 구현했고, ADR-0028 이 그 lock 저장소를 cloud proxy 호환 `claude/lock-driver` 브랜치로 이전했다 — multi-machine `/loop` + cron@cloud 동시 무장이 본 branch-ref CAS 위에서 credential 0 으로 지원된다.
