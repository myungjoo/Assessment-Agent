// confluence-token-decrypt spec — T-0185 acceptance (R-112: happy / error / branch /
// negative 4 카테고리 + never-read-back 비노출 검증 + coverage line/function ≥ 80%).
// milestone-3 GitHub 측 spec (src/github/github-token-decrypt.spec.ts, T-0179) 의
// mirror — happy / error / 방어 분기 / never-read-back 케이스 구성을 그대로 따른다.
//
// 본 spec 은 실 Confluence token / 실 암호문을 박제하지 않는다 (CLAUDE.md §9). test
...
import {
  decryptConfluenceInstanceConfigToken,
  decryptConfluenceInstanceToken,
} from "./confluence-token-decrypt";
...
// 명백히 가짜인 test 전용 token 평문 — 실 Confluence token 아님 (CLAUDE.md §9).
const FAKE_TOKEN = "atl_FAKE_test_token_not_real_000000000000";

// ConfluenceInstanceConfig fixture 생성 — tokenEnc 만 가변, 나머지 필드는 고정 더미
// (helper 는 tokenEnc 만 보므로 다른 필드 실값 불요 — CLAUDE.md §9).
function makeInstance(tokenEnc: string): ConfluenceInstanceConfig {
  return {
    key: "cloud",
    baseUrl: "https://acme.atlassian.net/wiki/rest/api",
    authUser: "ci@acme.example",
    tokenEnc,
    spaceAllowlist: ["ENG", "DOC"],
  };
}

describe("decryptConfluenceInstanceToken", () => {
  ...
  describe("happy path (유효 envelope → 평문 token 복원)", () => {
    ...
    it("유효 envelope 를 helper 로 복호화하면 원본 평문 token 을 반환한다 (happy)", () => {
      ...
      const { envelope, decrypted } = withEnvKey(key, () => {
        ...
        return {
          envelope: env,
          decrypted: decryptConfluenceInstanceToken(cipher, env),
        };
      });
    ...
    // ConfluenceInstanceConfig overload 도 동일하게 동작 (config.tokenEnc 위임).
    it("ConfluenceInstanceConfig overload 도 tokenEnc 를 복호화해 평문 token 을 반환한다 (happy — config)", () => {
      const key = randomKeyBase64();

      const decrypted = withEnvKey(key, () => {
        const cipher = new LlmApiKeyCipher();
        const instance = makeInstance(cipher.encrypt(FAKE_TOKEN));
        return decryptConfluenceInstanceConfigToken(cipher, instance);
      });
    ...
    it("helper 호출 시점에만 cipher.decrypt 가 1회 호출된다 (JIT — eager 복호화 아님)", () => {
      ...
      const result = decryptConfluenceInstanceToken(
        cipher,
        "fake-envelope-base64",
      );
  ...
  describe("빈/공백-only/undefined tokenEnc 방어 분기 (branch — fail-fast)", () => {
    ...
    it("빈 문자열 tokenEnc 는 cipher 호출 전에 throw 한다 (branch — empty)", () => {
      ...
      expect(() => decryptConfluenceInstanceToken(cipher, "")).toThrow();
    ...
    it("undefined tokenEnc 는 cipher 호출 전에 throw 한다 (branch — undefined)", () => {
      ...
      expect(() => decryptConfluenceInstanceToken(cipher, undefined)).toThrow();
      expect(decryptMock).not.toHaveBeenCalled();
    });

    // overload 경로에서도 instance.tokenEnc 가 비정상값이면 동일 fail-fast.
    it("overload 의 instance.tokenEnc 가 빈 문자열이면 cipher 호출 전에 throw 한다 (branch — config empty)", () => {
      const decryptMock = jest.fn();
      const cipher = { decrypt: decryptMock } as unknown as LlmApiKeyCipher;

      expect(() =>
        decryptConfluenceInstanceConfigToken(cipher, makeInstance("")),
      ).toThrow();
      expect(decryptMock).not.toHaveBeenCalled();
    });

    it("overload 의 instance.tokenEnc 가 공백-only 면 cipher 호출 전에 throw 한다 (branch — config whitespace)", () => {
      const decryptMock = jest.fn();
      const cipher = { decrypt: decryptMock } as unknown as LlmApiKeyCipher;
  ...
  describe("error path 전파 (negative — cipher.decrypt throw swallow 금지)", () => {
    ...
    it("wrong-key envelope 는 cipher 의 throw 를 그대로 전파한다 (negative — wrong-key)", () => {
      ...
      withEnvKey(keyB, () => {
        const cipher = new LlmApiKeyCipher();
        expect(() =>
          decryptConfluenceInstanceToken(cipher, envelope),
        ).toThrow();
      });
    });
    ...
    it("변조된 envelope (auth-tag tamper) 는 throw 를 전파한다 (negative — tamper)", () => {
      ...
      withEnvKey(key, () => {
        ...
        expect(() =>
          decryptConfluenceInstanceToken(cipher, tampered),
        ).toThrow();
      });
    });
    ...
    it("깨진/잘린 base64 envelope 는 throw 를 전파한다 (negative — 잘린 입력)", () => {
      ...
      withEnvKey(key, () => {
        ...
        expect(() =>
          decryptConfluenceInstanceToken(cipher, tooShort),
        ).toThrow();
      });
    });
    ...
    it("master key env 부재 시 cipher 의 throw 를 전파한다 (negative — missing-env)", () => {
      withEnvKey(undefined, () => {
        const cipher = new LlmApiKeyCipher();
        expect(() =>
          decryptConfluenceInstanceToken(cipher, "any-nonempty-envelope"),
        ).toThrow(ENC_KEY_ENV);
      });
    });
  ...
  describe("never-read-back / secret 비노출 검증", () => {
    ...
    it("빈 입력 방어 분기의 error message 에 token 평문이 없다 (애초에 평문 부재)", () => {
      ...
      let caught: unknown;
      try {
        decryptConfluenceInstanceToken(cipher, "");
      } catch (err) {
        caught = err;
      }
