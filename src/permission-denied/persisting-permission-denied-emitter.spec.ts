// PersistingPermissionDeniedEmitter spec — T-0211 acceptance (R-112: happy / error /
// branch / negative 4 카테고리 + coverage line/function ≥ 80%). ADR-0022 §6 emitter
// 패턴 / §1 정규화 / §6.3 fire-and-forget 흡수 위상을 검증한다.
//
// 본 spec 은 PermissionDeniedRecordService 를 Jest mock(`jest.fn()`)으로 대체해
// PostgreSQL container 없이 isolated 하게 실행된다. 검증 포인트:
//   - happy: emit({host,path,status}) 가 record(...)를 1 회, provider:"github" +
//     instanceRef===host + resourceRef===path + httpStatus===status 로 호출.
//   - error: record reject(DB 장애 모사) 시 emit 이 그 reject 를 흡수하고 throw 하지
//     않음(fire-and-forget + swallow, ADR-0022 §6.3).
//   - negative: token 평문이 record 입력 어디에도 새지 않음(host/path/status 만 매핑) /
//     reject 흡수 후 logger.warn 로 흔적만 남김(조용한 유실 방지).
...
import { PersistingPermissionDeniedEmitter } from "./persisting-permission-denied-emitter";
...
function buildEmitter(): {
  emitter: PersistingPermissionDeniedEmitter;
  record: jest.Mock;
} {
  const record = jest.fn();
  const emitter = new PersistingPermissionDeniedEmitter({
    record,
  } as never);
  return { emitter, record };
}

// 대표 GitHub 권한 거부 이벤트 fixture — host / path / status 만(token 평문 부재).
function buildEvent(
...
describe("PersistingPermissionDeniedEmitter", () => {
  ...
  // ------------------------------------------------------------------
  // Happy path — emit 이 record 를 1 회, github 정규화 매핑으로 호출
  // ------------------------------------------------------------------
  it("emit 이 record 를 1 회 provider:'github' + instanceRef:host + resourceRef:path + httpStatus:status 로 호출한다 (happy — 정규화 매핑)", async () => {
    const { emitter, record } = buildEmitter();
    record.mockResolvedValueOnce({ id: "pdr-1" });
  ...
  it("401 이벤트도 동일 정규화로 record 를 호출한다 (branch — 401 status 분기)", async () => {
    const { emitter, record } = buildEmitter();
    record.mockResolvedValueOnce({ id: "pdr-2" });
    ...
    expect(record).toHaveBeenCalledTimes(1);
    expect(record).toHaveBeenCalledWith(
      expect.objectContaining({ provider: "github", httpStatus: 401 }),
    );
  });
  ...
  // emit 경계를 깨지 않아야 한다 — 의존성 실패(service 오동작) 경로 방어.
  it("record 가 동기 throw 하는 비정상 의존성에서도 흡수 시도한다 (error — 의존성 오동작)", async () => {
    const { emitter, record } = buildEmitter();
    // record 가 rejected Promise 를 반환(가장 흔한 async throw 형태).
  ...
  // Negative cases (각 1+)
  // ------------------------------------------------------------------
  // (token 비노출) 이벤트에 token 평문이 부재하므로 record 입력에도 host/path/status
  ...
  it("record 입력에 host/path/status 외 secret/principal/reason 이 섞이지 않는다 (negative — token 비노출 §1 invariant)", async () => {
    const { emitter, record } = buildEmitter();
    record.mockResolvedValueOnce({ id: "pdr-3" });
    ...
    expect(JSON.stringify(callArg)).not.toMatch(/token|secret|ghp_|bearer/i);
  });
  ...
  it("reject 1 회 흡수 후에도 후속 emit 이 정상 record 호출한다 (negative — 흡수 후 인스턴스 무손상)", async () => {
    ...
    record.mockResolvedValueOnce({ id: "pdr-4" });
