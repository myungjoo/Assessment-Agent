...
//                  GET 목록 · POST 생성 · PATCH 수정 · DELETE 삭제 4 endpoint 노출
//                  (T-1748 + T-1749 + T-1750 + T-1751, ADR-0058 §Follow-ups (b) 절단
//                  chain — 잔여 1 route(primary 지정)는 후속 slice).
...
@Module({
  ...
  providers: [
    ...
    // 차이점). immutable 이라 update/deactivate/reactivate 미박제.
    ContributionService,
  ...
  exports: [
    ...
    // 하도록 노출.
    ContributionRepository,
    // ContributionService export (T-0115) — 후속 ContributionController / endpoint
    // 가 다른 module 에서 본 service 를 inject 가능하도록 노출.
    ContributionService,
