...
// 1:1 mirror. 단 Contribution 은 immutable (ADR-0006 Decision §2 — `updatedAt` 미정의,
// 개별 commit/PR/문서 단위의 1 회성 영속, 재수집 시 재생성):
//   - update / deactivate / reactivate 미박제.
//   - lifecycle 은 create → read → hard delete (remove) 의 3 phase 만.
//
// AssessmentService 와의 차이점:
//   - Contribution 은 `@@unique` 부재 → P2002 (unique constraint) 가 발생하지 않는다.
//     따라서 P2002 → ConflictException 변환 분기를 박제하지 않는다 (stray P2002 가
//     surface 되면 그대로 re-throw — 변환 안 함).
//   - 대신 Contribution 은 `assessmentId` N:1 FK 를 보유 → Assessment row 부재 시
//     Prisma 의 P2003 (FK constraint 위반) 이 propagate. 이를 BadRequestException 으로
//     변환 (잘못된 참조 input → 400, ADR-0006 §2 + contribution.repository.ts 박제).
//
...
//     AssessmentService 와 중복되나 기존 service 를 건드리면 diff 확장 + 회귀 위험 —
//     별도 refactor follow-up).
//   - update / softDelete / restore 없음 (Contribution 은 immutable, ADR-0006 §2).
import {
  BadRequestException,
  Injectable,
  NotFoundException,
...
// difficulty 는 AssessmentService 의 VALID_DIFFICULTIES 와 동일 값이나 본 task 는
// AssessmentService 를 import 해 결합하지 않고 자체 상수로 박제 (공용화는 별도
// refactor follow-up — 본 task 에서 다른 service 수정 금지).
...
// 을 늘리지 않기 위해 duck typing (`error.code`) 으로 통일 (AssessmentService /
// PersonService 의 동일 helper mirror — 공용화는 별도 refactor follow-up).
...
export class ContributionService {
  ...
  // create — REQ-033 commit·문서별 기여도/난이도/양 데이터 영속. (1) sourceType /
  // difficulty 의 enum-as-String literal 값을 ADR-0006 §2 의 허용 집합으로 검증
  // (잘못된 literal 이면 BadRequestException — service-layer 책임). (2) 검증 통과 후
  // repository.create 호출. (3) `assessmentId` FK 위반 (Assessment row 부재) 시
  // propagate 된 P2003 를 BadRequestException 으로 변환 (잘못된 참조 input → 400).
  // Contribution 은 `@@unique` 부재 → P2002 변환 분기 없음 (AssessmentService 와의
  // 차이점 — stray P2002 는 그대로 re-throw).
    ...
    try {
      ...
      if (getPrismaErrorCode(error) === "P2003") {
        throw new BadRequestException(
          `invalid assessmentId reference: ${input.assessmentId}`,
        );
      }
      throw error;
    }
  ...
  // Contribution 전체 조회). literal 검증 대상 없음 — repository 에 그대로 forward.
  // 매칭 row 0 시 빈 배열 [] 그대로 반환 (NotFoundException 던지지 않음 — 컬렉션
  // 조회의 정상 결과).
  ...
  // (AssessmentService.remove mirror).
