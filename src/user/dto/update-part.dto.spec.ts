// UpdatePartDto spec — CI scripts/check-spec-presence.sh 동반 spec 의무 강제.
...
// validate 만으로 충분. UpdateGroupDto.spec (T-0066) 의 동일 패턴 mirror.
//
// Group precedent 와의 차이 (T-0069 의 핵심 분기 박제):
//   - 본 DTO 자체는 Group 과 동일한 payload validation 만 책임 — P2002 분기는
//     repository / service layer 에서 박제 (DTO 단계에선 동일).
//   - 단 JSDoc + name 필드 주석에 schema 의 `@unique` 정의 박제 의무 (Group
//     precedent 의 "P2002 분기 부재" 와 명확히 대비되도록).
...
async function validatePlain(payload: unknown): Promise<string[]> {
  const dto = plainToInstance(UpdatePartDto, payload);
  const errors = await validate(dto);
  return errors.flatMap((e) => Object.keys(e.constraints ?? {}));
}

describe("UpdatePartDto", () => {
  // ----------------------------------------------------------------------
  ...
  // happy #1 과 동일 의미이나 UpdateGroupDto.spec 의 명시적 branch 패턴 mirror.
  // ----------------------------------------------------------------------
