...
// HTTP-facing layer. T-1748(골격 + GET) → T-1749(POST 생성) → T-1750(PATCH 수정) slice.
//
...
// **GET 목록 + POST 생성 + PATCH 수정 3 route** 가 배선돼 있고 나머지 2 route(DELETE
// 삭제 · primary 지정)는 후속 slice.
//
// route (ADR-0058 §Decision 1 의 GET · POST · PATCH 3 행):
...
//
// RBAC (§Decision 4 — 조회 User+ / 편집 Admin+): 세 route 모두 `@UseGuards(JwtAuthGuard,
// RolesGuard)` 를 같은 순서로 달되 tier 는 다르다 — GET 은 `@Roles("User")`, POST · PATCH
// 는 편집이라 `@Roles("Admin")`. RolesGuard 의 기존 escalation(ROLE_HIERARCHY)으로 상위
...
// PATCH 의 3 단 404(Person 부재 · 소유 아님 · `P2025`)도 service 가 이미 NotFoundException
// 으로 만들어 준다(§Decision 5 b · e). 그 외 오류도 흡수하지 않는다. 본 controller 에
// try/catch 없음.
//
// 책임 경계 (Out of Scope): DELETE · primary 지정 2 route 미노출(각각 후속
