// GroupController — `/api/groups` 8 REST endpoint. T-0055 acceptance §A 박제
// (CRUD 4 endpoint) + T-0057 acceptance §A 확장 (N:M membership 3 endpoint) +
// T-0068 acceptance §A 확장 (@Patch(":id") — Group.name partial update).
//
...
//   - PATCH  /api/groups/:id                        → update        (200, T-0068 추가)
...
//
// 책임 경계 (Out of Scope — T-0068 시점):
//   - AuthGuard (Admin+ / User+) 적용 안 함 — 후속 task 책임. PartController 동일 정책.
//   - PATCH endpoint 추가 (T-0068 박제) — Group.name 부분 수정 + ValidationPipe wire.
//     PartController PATCH 는 부재 유지 — Part 도메인 PATCH 4-layer 신설은 후속 task.
//     N:M membership PATCH (예: `PATCH /api/groups/:id/members/:membershipId`) 는
//     별도 후속 — PersonGroupMembership 자체에 mutable field 부재 (schema.prisma).
...
export class GroupController {
  ...
  // PATCH /api/groups/:id — Group 부분 수정. T-0068 추가. 200 OK + 수정된 Group row.
  // PersonController.update (T-0036/T-0037) 1:1 mirror — RFC-7396 JSON Merge Patch
  // semantic, ValidationPipe wire + service.update 단일 forward. UpdateGroupDto 의
  // `name?: string` 단일 필드만 cover (Group 의 다른 컬럼 createdAt/updatedAt 은
  ...
  //     GroupRepository.update 호출 → Prisma 가 partial update.
  //   - `patch.name === undefined` (빈 `{}` payload 등가) → service.update 가 빈 객체
  //     `{}` forward → Prisma `@updatedAt` directive 만 갱신 (PATCH no-op semantic).
  //     PersonController.update L233-242 의 동일 no-op 처리 mirror.
  //
  // error propagation (자동 — controller 는 raw forward):
  //   - service 가 throw 한 NotFoundException (P2025 변환, T-0067 박제) → 404 자동.
  //   - ValidationPipe 가 throw 한 BadRequestException (UpdateGroupDto 의 IsString /
  //     IsNotEmpty / MaxLength(255) / IsOptional 위반) → 400 자동. controller-scope
  //     `@UsePipes` (L74-80) 가 본 endpoint 도 cover — 별도 wire 0.
  //   - 그 외 (P2002 변환 분기 부재 — Group.name `@unique` 미정의 / 그 외 raw error)
  //     → service 그대로 propagate.
  //
  // 책임 경계 (Out of Scope):
  //   - AuthGuard 적용 안 함 — ADR-0008 auth credential 미박제 상태 유지.
  //   - N:M membership PATCH (예: `:id/members/:membershipId`) 미노출 — membership
  //     자체에 mutable field 부재 (schema.prisma L123-133 박제).
  //   - active toggle / 다른 도메인 필드 PATCH 미지원 — UpdateGroupDto 가 `name`
  //     단일 필드만 정의.
  @Patch(":id")
  async update(
    @Param("id") id: string,
    @Body() patch: UpdateGroupDto,
  ): Promise<Group> {
