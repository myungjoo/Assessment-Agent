...
import { buildProvidersPath } from './AdminView';
// 공용 invariant 추출기(T-1201 신설) import — inline 복사본 삭제·동작 무변경(T-1207 이관 slice).
// 이 spec 이 참조하는 export 중 **공용과 글자-동일한 5종만** import. richer extractHandlerMethods(5-field
// hasBody/hasParam/hasQuery)·HandlerDecorator(5-field)·extractProvidersFireMethod·pathParams·per-spec 타입/diffContract 는 inline 유지.
import {
  composeRoute,
...
// R-112 — LLM provider 목록 조회(GET /api/llm/providers) web↔backend **계약 drift guard**.
// 직전 GET slice difficulty-mapping-list(T-1188) mirror — 발사 대상을 difficulty-mappings → providers 로
// 바꾸고 GET 패턴(GET method · bare @Get() 세그먼트 0 · 핸들러 인자 0 · `?_r=nonce` cache-buster 무해)을
// 재적용. **신규 대조 축**: provider controller 는 같은 소스에 두 GET 핸들러 — @Get() findAll(목록) 과
// @Get(":id") findById(단건) — 를 가진다(difficulty-mapping 은 @Get() 하나뿐). 추출기가 web 목록 발사(bare
// base)에 대응하는 @Get() findAll 을 @Get(":id") findById 대조군에서 정확히 판별해야 한다(세그먼트 0 GET vs
// 세그먼트 1 GET). 정규식 추출기만 — 새 devDependency 0, 공용 helper 추출은 Out of Scope refactor slice.
...
// 괄호 균형 0 까지 이어붙여 인자 decorator 를 탐지. GET findAll 은 인자 0, findById 는 @Param 1 이 정답.
...
// AdminView 조회 call site 의 발사 method 추론. `useApiResource<LlmProviderRow[]>(providersPath)` 를
// 옵션 인자 없이(단일 인자) 호출 → request→fetch default GET. 인자 2+ (options 전달) 면 non-GET 가능.
function extractProvidersFireMethod(source: string): string | null {
  const matched = /useApiResource<LlmProviderRow\[\]>\(\s*([^)]*)\)/.exec(stripComments(source));
  if (!matched) {
    return null;
  }
...
const WEB_FIRE_METHOD = extractProvidersFireMethod(ADMIN_VIEW_SOURCE);
...
const providersFire = (nonce: number): WebFire => ({ path: buildProvidersPath(nonce), method: WEB_FIRE_METHOD ?? 'GET' });

describe('AdminView — LLM provider 목록 조회 web↔backend 계약 drift guard (T-1189)', () => {
  ...
  it('backend @Controller("api/llm/providers") 3-세그먼트 base 를 /api/llm/providers 로 정규화한다 (분기 — base 파싱)', () => {
    expect(ROUTE).toBe('api/llm/providers');
    expect(String(ROUTE).split('/')).toHaveLength(3); // api·llm·providers
  ...
  it('같은 소스의 @Get(":id") findById 는 세그먼트 1 + @Param 을 가져 @Get() findAll 과 정확히 판별된다 (분기 — GET-vs-GET 판별, 신규 축)', () => {
    expect(FIND_BY_ID).not.toBeNull();
    expect(FIND_BY_ID?.method).toBe('GET'); // findById 도 GET — method 만으로는 구분 불가
    expect(FIND_BY_ID?.subPath).toBe(':id'); // 세그먼트 1 — findAll(세그먼트 0)과 구분되는 핵심
    ...
    expect(composeRoute(String(ROUTE), FIND_ALL?.subPath ?? '')).not.toBe(
      composeRoute(String(ROUTE), FIND_BY_ID?.subPath ?? ''),
    );
  });
  it('buildProvidersPath(0) 발사(GET /api/llm/providers)가 backend findAll 계약과 완전 일치한다 (happy-path — 경로 정합)', () => {
    const fired = providersFire(0);
    expect(buildProvidersPath(0)).toBe(BASE);
  ...
  it('web 조회 call site 가 옵션 인자 없이 useApiResource 를 호출해 default GET 이고 backend 도 @Get 이다 (happy-path — method 정합)', () => {
    ...
    expect(providersFire(0).method).toBe(LIST_CONTRACT.method); // 양측 method == GET
  });
  it('buildProvidersPath(5) 은 `?_r=5` cache-buster 를 붙이되 base 는 backend route 와 여전히 일치한다 (happy-path — query 무해)', () => {
    const fired = providersFire(5);
  ...
  it.each<[string, () => BackendContract]>([
    ['(a) backend base 를 api/llm/provider(단수 오타)로', () => ({ ...LIST_CONTRACT, route: 'api/llm/provider' })],
    ['(a) backend base 를 api/llm/providers-x(접미 drift)로', () => ({ ...LIST_CONTRACT, route: 'api/llm/providers-x' })],
    ...
    expect(diffContract(providersFire(0), build())).toEqual([expect.stringContaining('path 불일치')]);
  });
  it.each<[string, () => BackendContract]>([
    ...
    expect(diffContract(providersFire(0), build())).toEqual([expect.stringContaining('method 불일치')]);
  });
  it('추출기가 `?_r=n` query 를 path 세그먼트로 착각하지 않되 진짜 추가 세그먼트는 여전히 잡는다 (negative (d) — query vs 세그먼트 구분)', () => {
    ...
    expect(diffContract(providersFire(5), LIST_CONTRACT)).toEqual([]);
    ...
    expect(diffContract(providersFire(5), withSegment)).toEqual([expect.stringContaining('path 불일치')]);
  });
  ...
  it('@Get(":id") findById 를 세그먼트 0 로 오축소하거나 @Get() findAll 로 병합하지 않는다 (negative (g) — findById 판별 실패 방지)', () => {
    // 두 GET 핸들러가 서로 다른 route 로 정확히 추출됨 — findAll 은 bare base, findById 는 :id 세그먼트.
    expect(FIND_ALL?.subPath).toBe(''); // findAll 이 findById 의 :id 를 흡수하지 않음
    ...
    // web 목록 발사(bare base)를 findById 계약에 대면 path 불일치로 잡힘(잘못 매칭 시의 회귀 검출).
    const findByIdContract: BackendContract = { ...LIST_CONTRACT, subPath: FIND_BY_ID?.subPath ?? '' };
    expect(diffContract(providersFire(0), findByIdContract)).toEqual([expect.stringContaining('path 불일치')]);
  });
  it('주석 줄의 "@Get()"/"@Controller(...)" 를 실 decorator 로 오인하지 않는다 (negative (f) — 주석 false-positive)', () => {
    ...
    expect(diffContract(providersFire(0), drifted)).toEqual(['backend 계약 추출 실패']);
  });
  it('web call site 가 옵션 인자를 전달하면(단일 인자 아님) GET 추론이 깨져 method 불일치로 잡힌다 (negative — 발사 override drift)', () => {
    const overridden = extractProvidersFireMethod(
      ["const x = useApiResource<LlmProviderRow[]>(providersPath, { method: 'POST' });"].join('\n'),
    );
  ...
  it('빈 소스 입력이면 추출기가 null·빈 객체를 반환하고 대조가 통과하지 않는다 (negative — 소스 유실)', () => {
    ...
    expect(extractProvidersFireMethod('')).toBeNull();
    ...
    expect(diffContract(providersFire(0), empty)).toEqual(['backend 계약 추출 실패']);
  });
});
