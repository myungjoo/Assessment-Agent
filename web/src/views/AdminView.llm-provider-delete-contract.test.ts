...
// method + body-less 대조) · llm-provider-create(T-1184, `api/llm/providers` 3-세그먼트 base 파싱 축)에서
// 차용(상세 주석은 그 파일들; 정규식만 — 새 devDependency 0, 공용 helper 추출은 Out of Scope refactor slice).
// 본 task 로 LLM provider GET/POST/PATCH/DELETE 4-CRUD 계약 guard 가 완결된다. 특화 축: (1) `api/llm/providers`
// 3-세그먼트 base 에 `@Delete(":id")`(DELETE method + param 1) 첫 결합(T-1184 create 는 bare `@Post()` param 0).
...
function extractHandlerMethods(source: string): Record<string, HandlerDecorator> {
  ...
  for (const line of stripComments(source).split('\n')) {
    ...
    if (/^[ \t]*@/.test(line)) {
      continue; // 그 외 decorator(@HttpCode/@UseGuards/@Roles/@Param)는 handler 로 오인하지 않는다.
    }
...
async function fireDeleteProvider(id: string): Promise<WebFire> {
  ...
  if (!fired) {
    throw new Error('provider 삭제 러너가 발사하지 않았다');
  }
  return fired;
}

describe('AdminView — LLM provider 삭제 web↔backend 계약 drift guard (T-1186)', () => {
  ...
  it('backend @Controller("api/llm/providers") 3-세그먼트 base 를 /api/llm/providers 로 정규화한다 (분기 — base 파싱)', () => {
    ...
    expect(String(ROUTE).split('/')).toHaveLength(3); // 3-세그먼트 base
    expect(normalizeRoute(String(ROUTE))).toBe('/api/llm/providers');
  });
  it('backend @Delete(":id") 세그먼트를 api/llm/providers base 와 합성해 path param 정확히 1개(:id) template 을 만든다 (분기 — :id 합성)', () => {
    ...
    expect(expectedPath(String(ROUTE), DELETE_CONTRACT.subPath, PROVIDER_ID)).toBe('/api/llm/providers/lp-1');
  });
  ...
  it('provider 삭제 발사(DELETE /api/llm/providers/:id)가 backend delete 계약과 완전 일치한다 (happy-path)', async () => {
    const fired = await fireDeleteProvider(PROVIDER_ID);
    expect(diffContract(fired, DELETE_CONTRACT, PROVIDER_ID)).toEqual([]);
    expect(fired.path).toBe('/api/llm/providers/lp-1');
    expect(fired.method).toBe('DELETE');
  });
  it('web 러너가 id 를 encodeURIComponent 로 안전 인코딩해 path 세그먼트에 넣는다 (분기 — id 인코딩, 비정상 문자)', async () => {
    const weirdId = 'lp 1/x?a';
    const fired = await fireDeleteProvider(weirdId);
  ...
  it('options.body 부재면 JSON.parse SyntaxError 없이 빈 키 집합 + hasContentType=false 로 매핑된다 (분기 — options → method/body/헤더 부재 매핑)', () => {
    const fired = toFire('/api/llm/providers/lp-1', { method: 'DELETE' } as RequestOptions);
    expect(fired.method).toBe('DELETE');
    expect(fired.bodyKeys.size).toBe(0);
    expect(fired.hasContentType).toBe(false);
  });
  ...
  it.each<[string, () => Promise<{ fire: WebFire; backend: BackendContract; id: string }>]>([
    ['(a) backend base 를 api/llm/provider(오타)로', async () => ({ fire: await fireDeleteProvider(PROVIDER_ID), backend: { ...DELETE_CONTRACT, route: 'api/llm/provider' }, id: PROVIDER_ID })],
    ["(a') backend base 를 api/llm/providers-x 로", async () => ({ fire: await fireDeleteProvider(PROVIDER_ID), backend: { ...DELETE_CONTRACT, route: 'api/llm/providers-x' }, id: PROVIDER_ID })],
    ['(b) @Delete(":id") 를 @Delete(":id/archive")(세그먼트 추가, param 2)로', async () => ({ fire: await fireDeleteProvider(PROVIDER_ID), backend: { ...DELETE_CONTRACT, subPath: extractHandlerMethods(['  @Delete(":id/archive")', '  async delete() {}'].join('\n')).delete.subPath }, id: PROVIDER_ID })],
    ['(c) @Delete(":id") 를 bare @Delete()(세그먼트 0)로', async () => ({ fire: await fireDeleteProvider(PROVIDER_ID), backend: { ...DELETE_CONTRACT, subPath: extractHandlerMethods(['  @Delete()', '  async delete() {}'].join('\n')).delete.subPath }, id: PROVIDER_ID })],
    ['(g) web 이 id 를 encodeURIComponent 없이 raw 삽입', async () => { const f = await fireDeleteProvider('lp 1/x'); return { fire: { ...f, path: '/api/llm/providers/lp 1/x' }, backend: DELETE_CONTRACT, id: 'lp 1/x' }; }],
  ...
  it('backend 가 method 를 @Patch/@Post 로 바꿨는데 web 은 DELETE 발사 → method 불일치로 잡힌다 (negative (d) — method drift)', async () => {
    ...
    for (const src of ['  @Patch(":id")', '  @Post(":id")']) {
      ...
      expect(diffContract(fired, drifted, PROVIDER_ID)).toEqual([expect.stringContaining('method 불일치')]);
    }
  });
  it('backend 가 @Body() dto 를 추가했는데 web 은 body 미발사 → body-less 정합 위반으로 잡힌다 (negative (e) — @Body 추가 drift)', async () => {
    const fired = await fireDeleteProvider(PROVIDER_ID);
    const bodyfulSrc = ['  async delete(', '    @Param("id") id: string,', '    @Body() dto: DeleteProviderDto,', '  ): Promise<void> {}'].join('\n');
    expect(handlerHasBody(bodyfulSrc, 'delete')).toBe(true);
    const drifted: BackendContract = { ...DELETE_CONTRACT, hasBody: true };
    expect(diffContract(fired, drifted, PROVIDER_ID)).toEqual([expect.stringContaining('backend @Body 요구 vs web body 부재')]);
  });
  it('web 이 불필요한 body/Content-Type 를 붙이면 body-less 대조가 fail 한다 (negative (f) — 초과 body/헤더 400/415 예방)', async () => {
    ...
    const issues = diffContract(withBody, DELETE_CONTRACT, PROVIDER_ID);
  ...
  it('주석 줄의 "@Delete(":id")"/"@Controller(...)" 를 실 decorator 로 오인하지 않는다 (negative (h) — 주석 false-positive)', async () => {
    ...
    expect(diffContract(await fireDeleteProvider(PROVIDER_ID), drifted, PROVIDER_ID)).toEqual(['backend 계약 추출 실패']);
  });
  it('빈 소스 입력이면 추출기가 null 을 반환하고 대조가 통과하지 않는다 (negative — 소스 유실)', async () => {
    ...
    expect(diffContract(await fireDeleteProvider(PROVIDER_ID), empty, PROVIDER_ID)).toEqual(['backend 계약 추출 실패']);
  });
});
