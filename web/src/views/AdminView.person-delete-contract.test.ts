...
import type { DeletePersonDeps } from './AdminView';
import { runDeletePerson } from './AdminView';
// 공용 invariant 추출기(T-1201 신설) import — inline 복사본 삭제·동작 무변경(T-1217 이관 slice, mutation stream person-delete).
// 이 spec 이 참조하는 export 중 **공용과 글자-동일한 4종만** import(stripQuery/extractHandlerParams/pathSegments 제외 — 부재/미사용). richer
// extractHandlerMethods(hasBody·주석 상이)·HandlerDecorator·handlerHasBody·BackendContract/WebFire·diffContract·toFire 는 inline 유지.
import {
  composeRoute,
...
// R-112 — 인원 삭제(DELETE /api/persons/:id) web↔backend **계약 drift guard**. 추출기/대조기 패턴은 선례
// group-delete(T-1177, DELETE+body 부재+`@Delete(":id")` 단일 path param) · person-update(T-1179,
// `api/persons` base + `:id` 단일 세그먼트 + encodeURIComponent) 차용(상세 주석은 그 파일들; 정규식만 —
// 새 devDependency 0, 공용 helper 추출은 Out of Scope refactor slice). 신규 축: (1) `api/persons` base 에
// `@Delete(":id")`(DELETE method + param 1) 첫 결합(T-1178 person-create 는 bare `@Post()` param 0,
// T-1179 person-update 는 `@Patch(":id")` PATCH). (2) DELETE + body/Content-Type 부재 AND backend `@Body`
// decorator **부재**(T-1179 PATCH body 존재 축과 대비). (3) `@Delete` method decorator 대조.
...
// handler 이름 → {method, subPath}(`remove`→{DELETE, ':id'}). decorator 없는 handler 미수록.
...
// 본다 — 반환 타입(`Promise<void>`)에는 @Body 가 없으므로 안전. `remove`→false, `update`→true.
...
const REMOVE_HANDLER = extractHandlerMethods(CONTROLLER_SOURCE).remove ?? null;
const DELETE_CONTRACT: BackendContract = {
  route: ROUTE,
  method: REMOVE_HANDLER?.method ?? null,
  subPath: REMOVE_HANDLER?.subPath ?? '',
  hasBody: handlerHasBody(CONTROLLER_SOURCE, 'remove'),
};
const PERSON_ID = 'p-1';
...
async function fireDeletePerson(id: string): Promise<WebFire> {
  ...
  await runDeletePerson(id, deps);
  if (!fired) {
    throw new Error('인원 삭제 러너가 발사하지 않았다');
  }
  return fired;
}

describe('AdminView — 인원 삭제 web↔backend 계약 drift guard (T-1180)', () => {
  ...
  it('backend @Delete(":id") 세그먼트를 api/persons base 와 합성해 path param 정확히 1개(:id) template 을 만든다 (분기 — :id 첫 DELETE 결합)', () => {
    ...
    expect(expectedPath(String(ROUTE), DELETE_CONTRACT.subPath, PERSON_ID)).toBe('/api/persons/p-1');
  });
  it('backend remove 핸들러가 @Body decorator 없는 body-less 핸들러다 (분기 — @Body 부재 판정)', () => {
  ...
  it('인원 삭제 발사(DELETE /api/persons/:id)가 backend remove 계약과 완전 일치한다 (happy-path)', async () => {
    const fired = await fireDeletePerson(PERSON_ID);
    expect(diffContract(fired, DELETE_CONTRACT, PERSON_ID)).toEqual([]);
    expect(fired.path).toBe('/api/persons/p-1');
  ...
  it('web 러너가 id 를 encodeURIComponent 로 안전 인코딩해 path 세그먼트에 넣는다 (분기 — path 인코딩, 비정상 문자)', async () => {
    ...
    const fired = await fireDeletePerson(weirdId);
  ...
  it('options.body 부재면 JSON.parse SyntaxError 없이 빈 키 집합 + hasContentType=false 로 매핑된다 (분기 — options → method/body/헤더 부재 매핑)', () => {
    const fired = toFire('/api/persons/p-1', { method: 'DELETE' } as RequestOptions);
    expect(fired.method).toBe('DELETE');
    expect(fired.bodyKeys.size).toBe(0);
    expect(fired.hasContentType).toBe(false);
  });
  ...
  it.each<[string, () => Promise<{ fire: WebFire; backend: BackendContract; id: string }>]>([
    ['(a) backend base 를 api/person(오타)로', async () => ({ fire: await fireDeletePerson(PERSON_ID), backend: { ...DELETE_CONTRACT, route: 'api/person' }, id: PERSON_ID })],
    ["(a') backend base 를 api/people 로", async () => ({ fire: await fireDeletePerson(PERSON_ID), backend: { ...DELETE_CONTRACT, route: 'api/people' }, id: PERSON_ID })],
    ['(b) @Delete(":id") 를 @Delete(":id/deactivate")(세그먼트 추가, param 2)로', async () => ({ fire: await fireDeletePerson(PERSON_ID), backend: { ...DELETE_CONTRACT, subPath: extractHandlerMethods(['  @Delete(":id/deactivate")', '  async remove() {}'].join('\n')).remove.subPath }, id: PERSON_ID })],
    ['(c) @Delete(":id") 를 bare @Delete()(세그먼트 0)로', async () => ({ fire: await fireDeletePerson(PERSON_ID), backend: { ...DELETE_CONTRACT, subPath: extractHandlerMethods(['  @Delete()', '  async remove() {}'].join('\n')).remove.subPath }, id: PERSON_ID })],
    ['(g) web 이 id 를 encodeURIComponent 없이 raw 삽입', async () => { const f = await fireDeletePerson('p 1/x'); return { fire: { ...f, path: '/api/persons/p 1/x' }, backend: DELETE_CONTRACT, id: 'p 1/x' }; }],
  ...
  it('backend 가 method 를 @Patch/@Post 로 바꿨는데 web 은 DELETE 발사 → method 불일치로 잡힌다 (negative (d) — method drift)', async () => {
    ...
    for (const src of ['  @Patch(":id")', '  @Post(":id")']) {
      const handler = extractHandlerMethods([src, '  async remove() {}'].join('\n')).remove;
  ...
  it('backend 가 @Body() dto 를 추가했는데 web 은 body 미발사 → body-less 정합 위반으로 잡힌다 (negative (e) — @Body 추가 drift)', async () => {
    const fired = await fireDeletePerson(PERSON_ID);
    const bodyfulSrc = ['  async remove(', '    @Param("id") id: string,', '    @Body() dto: DeletePersonDto,', '  ): Promise<void> {}'].join('\n');
    expect(handlerHasBody(bodyfulSrc, 'remove')).toBe(true);
    const drifted: BackendContract = { ...DELETE_CONTRACT, hasBody: true };
    expect(diffContract(fired, drifted, PERSON_ID)).toEqual([expect.stringContaining('backend @Body 요구 vs web body 부재')]);
  });
  it('web 이 불필요한 body/Content-Type 를 붙이면 body-less 대조가 fail 한다 (negative (f) — 초과 body/헤더 400/415 예방)', async () => {
    ...
    const issues = diffContract(withBody, DELETE_CONTRACT, PERSON_ID);
  ...
  it('주석 줄의 "@Delete(":id")"/"@Controller(...)" 를 실 decorator 로 오인하지 않는다 (negative (h) — 주석 false-positive)', async () => {
    ...
    const fakeHandler = ['  // @Delete(":id") — 주석뿐, decorator 없음', '  async remove() {}'].join('\n');
    expect(extractControllerRoute(fakeController)).toBeNull();
    expect(extractHandlerMethods(fakeHandler).remove).toBeUndefined();
    const drifted: BackendContract = {
      ...DELETE_CONTRACT,
      route: extractControllerRoute(fakeController),
      method: extractHandlerMethods(fakeHandler).remove?.method ?? null,
    };
    expect(diffContract(await fireDeletePerson(PERSON_ID), drifted, PERSON_ID)).toEqual(['backend 계약 추출 실패']);
  });
  it('빈 소스 입력이면 추출기가 null 을 반환하고 대조가 통과하지 않는다 (negative — 소스 유실)', async () => {
    ...
    expect(handlerHasBody('', 'remove')).toBe(false);
    const empty: BackendContract = {
      route: extractControllerRoute(''),
      method: extractHandlerMethods('').remove?.method ?? null,
      subPath: extractHandlerMethods('').remove?.subPath ?? '',
      hasBody: handlerHasBody('', 'remove'),
    };
    expect(diffContract(await fireDeletePerson(PERSON_ID), empty, PERSON_ID)).toEqual(['backend 계약 추출 실패']);
  });
});
