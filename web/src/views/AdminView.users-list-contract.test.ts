...
// 공용 invariant 추출기(T-1201 신설) import — inline 복사본 삭제·동작 무변경(T-1204 이관 slice 3).
...
// AdminView 조회 call site(T-1891 이후 useAdminUsers hook) 의 발사 method 추론.
// `useApiResource<UserRow[]>(usersPath)` 를
...
// T-1891 순수 추출로 조회 call site 가 useAdminUsers hook 으로 옮겨갔다 — 읽는 파일(pointer)만
// 새 모듈로 바꾸고 추출기 · 계약 단언 내용은 그대로 둔다(계약 무변경).
const USERS_HOOK_SOURCE = readFileSync(new URL('./useAdminUsers.ts', import.meta.url), 'utf8');
...
const WEB_FIRE_METHOD = extractUsersFireMethod(USERS_HOOK_SOURCE);
