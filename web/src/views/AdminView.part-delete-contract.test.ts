...
import type { DeletePartDeps } from './AdminView';
import { runDeletePart } from './AdminView';
// 공용 invariant 추출기(T-1201 신설) import — inline 복사본 삭제·동작 무변경(T-1220 이관 slice, mutation stream part-delete).
// 이 spec 이 참조하는 export 중 **공용과 글자-동일한 4종만** import(stripQuery/extractHandlerParams/pathSegments 제외 — 부재/미사용). richer
// extractHandlerMethods(hasBody·주석 상이)·HandlerDecorator·handlerHasBody·BackendContract/WebFire·pathParams·diffContract·toFire 는 inline 유지.
import {
  composeRoute,
...
// R-112 — 파트 삭제(DELETE /api/parts/:id) web↔backend **계약 drift guard**. 추출기/대조기 패턴은 선례
// person-delete(T-1180, DELETE+body 부재+`@Delete(":id")` 단일 path param + `@Body` 부재) · part-update
// (T-1182, `api/parts` base + `:id` 단일 세그먼트 + encodeURIComponent) · part-create(T-1181, `api/parts`
// base) 차용(상세 주석은 그 파일들; 정규식만 — 새 devDependency 0, 공용 helper 추출은 Out of Scope refactor
// slice). 신규 축: (1) `api/parts` base 에 `@Delete(":id")`(DELETE method + param 1) 첫 결합(T-1181 part-create
// 는 bare `@Post()` param 0, T-1182 part-update 는 `@Patch(":id")` PATCH). (2) DELETE + body/Content-Type
// 부재 AND backend `@Body` decorator **부재**(T-1182 PATCH body 존재 축과 대비, T-1180 person-delete body
// 부재 축과 정합). (3) `@Delete` method decorator 대조. 핸들러 이름은 `delete`(person 은 `remove`).
...
async function fireDeletePart(id: string): Promise<WebFire> {
  ...
  await runDeletePart(id, deps);
  if (!fired) {
    throw new Error('파트 삭제 러너가 발사하지 않았다');
  }
  return fired;
}

describe('AdminView — 파트 삭제 web↔backend 계약 drift guard (T-1183)', () => {
  ...
  it('backend @Delete(":id") 세그먼트를 api/parts base 와 합성해 path param 정확히 1개(:id) template 을 만든다 (분기 — :id 첫 DELETE 결합)', () => {
    ...
    expect(expectedPath(String(ROUTE), DELETE_CONTRACT.subPath, PART_ID)).toBe('/api/parts/pt-1');
  });
  ...
  it('파트 삭제 발사(DELETE /api/parts/:id)가 backend delete 계약과 완전 일치한다 (happy-path)', async () => {
    const fired = await fireDeletePart(PART_ID);
    expect(diffContract(fired, DELETE_CONTRACT, PART_ID)).toEqual([]);
    expect(fired.path).toBe('/api/parts/pt-1');
  ...
  it('web 러너가 id 를 encodeURIComponent 로 안전 인코딩해 path 세그먼트에 넣는다 (분기 — path 인코딩, 비정상 문자)', async () => {
    ...
    const fired = await fireDeletePart(weirdId);
  ...
  it('options.body 부재면 JSON.parse SyntaxError 없이 빈 키 집합 + hasContentType=false 로 매핑된다 (분기 — options → method/body/헤더 부재 매핑)', () => {
    const fired = toFire('/api/parts/pt-1', { method: 'DELETE' } as RequestOptions);
    expect(fired.method).toBe('DELETE');
    expect(fired.bodyKeys.size).toBe(0);
    expect(fired.hasContentType).toBe(false);
  });
  ...
  it.each<[string, () => Promise<{ fire: WebFire; backend: BackendContract; id: string }>]>([
    ['(a) backend base 를 api/part(오타)로', async () => ({ fire: await fireDeletePart(PART_ID), backend: { ...DELETE_CONTRACT, route: 'api/part' }, id: PART_ID })],
    ["(a') backend base 를 api/parties 로", async () => ({ fire: await fireDeletePart(PART_ID), backend: { ...DELETE_CONTRACT, route: 'api/parties' }, id: PART_ID })],
    ['(b) @Delete(":id") 를 @Delete(":id/archive")(세그먼트 추가, param 2)로', async () => ({ fire: await fireDeletePart(PART_ID), backend: { ...DELETE_CONTRACT, subPath: extractHandlerMethods(['  @Delete(":id/archive")', '  async delete() {}'].join('\n')).delete.subPath }, id: PART_ID })],
    ['(c) @Delete(":id") 를 bare @Delete()(세그먼트 0)로', async () => ({ fire: await fireDeletePart(PART_ID), backend: { ...DELETE_CONTRACT, subPath: extractHandlerMethods(['  @Delete()', '  async delete() {}'].join('\n')).delete.subPath }, id: PART_ID })],
    ['(g) web 이 id 를 encodeURIComponent 없이 raw 삽입', async () => { const f = await fireDeletePart('pt 1/x'); return { fire: { ...f, path: '/api/parts/pt 1/x' }, backend: DELETE_CONTRACT, id: 'pt 1/x' }; }],
  ...
  it('backend 가 @Body() dto 를 추가했는데 web 은 body 미발사 → body-less 정합 위반으로 잡힌다 (negative (e) — @Body 추가 drift)', async () => {
    const fired = await fireDeletePart(PART_ID);
    const bodyfulSrc = ['  async delete(', '    @Param("id") id: string,', '    @Body() dto: DeletePartDto,', '  ): Promise<void> {}'].join('\n');
    expect(handlerHasBody(bodyfulSrc, 'delete')).toBe(true);
    const drifted: BackendContract = { ...DELETE_CONTRACT, hasBody: true };
    expect(diffContract(fired, drifted, PART_ID)).toEqual([expect.stringContaining('backend @Body 요구 vs web body 부재')]);
  });
  it('web 이 불필요한 body/Content-Type 를 붙이면 body-less 대조가 fail 한다 (negative (f) — 초과 body/헤더 400/415 예방)', async () => {
    ...
    const issues = diffContract(withBody, DELETE_CONTRACT, PART_ID);
  ...
  it('주석 줄의 "@Delete(":id")"/"@Controller(...)" 를 실 decorator 로 오인하지 않는다 (negative (h) — 주석 false-positive)', async () => {
    ...
    expect(diffContract(await fireDeletePart(PART_ID), drifted, PART_ID)).toEqual(['backend 계약 추출 실패']);
  });
  it('빈 소스 입력이면 추출기가 null 을 반환하고 대조가 통과하지 않는다 (negative — 소스 유실)', async () => {
    ...
    expect(diffContract(await fireDeletePart(PART_ID), empty, PART_ID)).toEqual(['backend 계약 추출 실패']);
  });
});
