// realdata-e2e-result-issue-command-args-consistency.spec.ts —
// T-1031 colocated unit spec for
// `assertRealDataResultIssueCommandArgsConsistent`.
//
// R-112 cover 구조:
//   - happy-path: 실 producer `buildRealDataResultIssueCommandArgs` 산출을 가드에 넣으면
//     throw 0(void) — 서로 다른 descriptor fixture(title/marker/body 조합). producer 를
//     실제 호출해 얻은 command-args round-trip(oracle↔producer 규칙 일치 증명).
...
//
// mirror source: daily 축 T-0990 spec
// (realdata-e2e-daily-step-dual-leg-run-report-issue-command-args-consistency.spec.ts).
// 요약축 producer / 타입 / labels 상수(["realdata-e2e", "result"]) 로 옮긴다.
import {
...
const FIXED_LABELS = ["realdata-e2e", "result"];

// makeDescriptor — 결정론적 result-issue descriptor fixture. run 식별자 조합을 명시적으로
// 받아 producer(T-0582) 조립 규칙과 동형인 title/marker/body(marker-first) 를 합성.
// spec 안 인라인 구성(공용 mock helper 추출 불요).
function makeDescriptor(
  opts: { dateToken?: string; gitSha?: string; extraBody?: string[] } = {},
): RealDataResultIssueDescriptor {
  const dateToken = opts.dateToken ?? "2026-07-16";
  const gitSha = opts.gitSha ?? "abc1234";
  const token = `${dateToken}@${gitSha}`;
  const marker = `<!-- realdata-e2e-result-issue: ${token} -->`;
  return {
    title: `실 평가 e2e 결과 ${token}`,
    marker,
    body: [
      marker,
      "",
      ...(opts.extraBody ?? [
        "결과 요약: 전체 3건 / pass 3 / fail 0",
        "",
        "## 실 평가 e2e 결과",
        "- pass: 3",
        "- fail: 0",
...
function makeArgs(
  ...
  return buildRealDataResultIssueCommandArgs(descriptor);
}

describe("assertRealDataResultIssueCommandArgsConsistent", () => {
  describe("happy-path (정합 descriptor↔command-args → void)", () => {
    ...
    it("marker-only body descriptor 도 정합(void)", () => {
      const marker = "<!-- realdata-e2e-result-issue: 2026-07-13@cafe123 -->";
      const descriptor: RealDataResultIssueDescriptor = {
        title: "실 평가 e2e 결과 2026-07-13@cafe123",
        marker,
        body: marker,
      };
      expect(() =>
  ...
  describe("error path — commandArgs 구조 결손 → TypeError", () => {
    ...
    it("commandArgs 비-객체(문자열) → TypeError", () => {
      expect(() =>
        assertRealDataResultIssueCommandArgsConsistent(
          "not-an-object" as unknown as RealDataResultIssueCommandArgs,
          makeDescriptor(),
        ),
    ...
    it("createArgs 부재(null) → TypeError", () => {
      ...
      const args = {
        ...makeArgs(descriptor),
        createArgs:
          null as unknown as RealDataResultIssueCommandArgs["createArgs"],
      };
      expect(() =>
        assertRealDataResultIssueCommandArgsConsistent(args, descriptor),
    ...
    it("createArgs 비-객체(문자열) → TypeError", () => {
      ...
      const args = {
        ...makeArgs(descriptor),
        createArgs:
          "x" as unknown as RealDataResultIssueCommandArgs["createArgs"],
      };
      expect(() =>
        assertRealDataResultIssueCommandArgsConsistent(args, descriptor),
    ...
    it("updateArgs 부재(undefined) → TypeError", () => {
      ...
      const args = {
        ...makeArgs(descriptor),
        updateArgs:
          undefined as unknown as RealDataResultIssueCommandArgs["updateArgs"],
      };
      expect(() =>
        assertRealDataResultIssueCommandArgsConsistent(args, descriptor),
    ...
    it("updateArgs 비-객체(문자열) → TypeError", () => {
      ...
      const args = {
        ...base,
        updateArgs:
          "updateArgs" as unknown as RealDataResultIssueCommandArgs["updateArgs"],
      };
      expect(() =>
        assertRealDataResultIssueCommandArgsConsistent(args, descriptor),
  ...
  describe("negative 충분 cover — command-args 필드별 drift → RangeError", () => {
    ...
    it("(d-mut) labels 문자열 변경 → RangeError(labels[i])", () => {
      ...
      const args = {
        ...
        createArgs: {
          ...base.createArgs,
          labels: ["realdata-e2e", "result-CHANGED"],
        },
      };
      expect(() =>
        assertRealDataResultIssueCommandArgsConsistent(args, descriptor),
    ...
    it("descriptor 측 슬롯이 command-args 와 어긋나도 RangeError(양방향 어느 쪽이든 노출)", () => {
      ...
      const mismatched = {
        ...makeDescriptor(),
        marker: "<!-- realdata-e2e-result-issue: 2026-07-16@0000000 -->",
      };
      expect(() =>
        assertRealDataResultIssueCommandArgsConsistent(args, mismatched),
      ).toThrow(RangeError);
    });
  });
