// realdata-e2e-daily-step-dual-leg-run-report-issue-gh-argv-consistency.ts —
// 실 평가 e2e daily-step dual-leg run report 이슈 gh 인자-벡터(argv)가 입력 action +
// 명령-args 의 title/body/labels 를 argv 위치로 정합 round-trip 했는지 검증하는 순수
// 가드(T-0992 박제, 요약축 선례 T-0653 mirror).
//
// 책임:
//   - `buildRealDataDailyStepDualLegRunReportIssueGhArgv`(T-0899,
//     `realdata-e2e-daily-step-dual-leg-run-report-issue-gh-argv.ts`)는
//     `RealDataDailyStepDualLegRunReportIssueAction`(create/update) +
//     `RealDataDailyStepDualLegRunReportIssueCommandArgs` 를 실 `gh` 명령에 그대로 넘길
//     인자-벡터(argv, `string[]`)로 합성한다. create 분기는 `["issue", "create",
//     "--title", title, "--body", body, ...("--label", <label>) 전개]`, update 분기는
//     `["issue", "edit", String(issueNumber), "--title", title, "--body", body]` 를
//     산출한다. 이 빌더는 `assertNonBlank`·`assertPositiveIssueNumber` 같은 inline 식별자
//     guard 만 보유하고, **산출 argv 가 명령-args 의 title/body/labels 를 argv 의 올바른
//     위치로 정합 전파했는지 검증하는 독립 불변식 가드는 부재** 하다. 즉 빌더가 회귀(예:
//     `--title` 뒤 값이 body 와 뒤바뀜, label flag-pair 의 순서·개수 어긋남, create
//     분기인데 `issue edit` argv 가 나옴)해도 argv 구조 불변식을 런타임에서 강제하는 가드가
//     없어, 손상 argv 가 `execFile('gh', argv)` live wiring 으로 새면 잘못된 gh 명령이
//     실행된다. 본 가드가 그 빈칸을 채운다.
//
// 🔥 command-args single-source(argv-side round-trip):
//   - 본 가드는 argv 빌더(T-0899)를 **import 하지 않는다**(oracle 독립성 — 빌더를 재호출해
//     비교하면 양방향 drift 가 상쇄돼 결함을 놓친다). action(create/update) + commandArgs 를
//     single-source 로 삼아 argv 의 동사·title/body/labels 위치 불변식을 **가드 안에 독립
//     재현**해 실제 argv 와 byte-identical 대조한다.
//
...
// DB write·migration 0, live LLM 호출 0. 타입만 `import type` 로 참조하므로(value import
// 0) 이 가드가 나중에 producer 에 self-wire 배선돼도 런타임 순환 의존이 생기지 않는다
// (consistency → gh-argv value 엣지 0).
//
// 책임 경계(task Out of Scope):
//   - `buildRealDataDailyStepDualLegRunReportIssueGhArgv` 본문·출력 타입 변경 0(타입만
...
//   - 산출 경로 자동 배선(`buildRealDataDailyStepDualLegRunReportIssueGhArgv` 산출 직전
//     self-wire) 0 — 순수 가드 helper 까지. self-wire 는 별도 follow-up slice(T-0991
//     mirror).
//   - `--repo owner/repo` 인자 / repo slug 정합 검증 0 — 빌더가 issue create/edit 핵심
//     인자만 산출하고 repo 컨텍스트는 caller 책임(본 가드는 빌더가 실제 산출하는 argv
//     범위만).
...
function assertActionStructure(
  ...
  if (action === null || action === undefined) {
    throw new TypeError(
      "action 이 null/undefined 일 수 없다 — RealDataDailyStepDualLegRunReportIssueAction 객체가 필요하다.",
    );
  }
...
/**
 * 실 평가 e2e daily-step dual-leg run report 이슈 gh 인자-벡터(argv)가 입력 action +
 * 명령-args 의 title/body/labels 를 argv 위치로 정합 round-trip 했는지 런타임에서 검증하는
 * 순수 가드(PLAN.md P5 109행 step ④ 결과 박제 chain 의 argv-layer 무결성 조각). 요약축
 * 선례 `assertRealDataResultIssueGhArgvPreservesCommandArgs`(T-0653)를 daily-step 축으로
 * 옮긴 판이다 — argv 가 single-source(action + commandArgs)를 보존하는지 검증한다.
 *
 * 검증하는 불변식(single source — action + commandArgs, 빌더 T-0899 의 argv 합성 규칙 강제):
 ...
 *
 * 검사 순서: 구조(action / argv / commandArgs) → 동사 분기 판정 → 각 위치 정합. 가장 먼저
 ...
 */
export function assertRealDataDailyStepDualLegRunReportIssueGhArgvPreservesCommandArgs(
  argv: string[],
  action: RealDataDailyStepDualLegRunReportIssueAction,
