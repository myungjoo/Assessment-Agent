// realdata-e2e-daily-step-collect-command-plan-consistency.ts — 실 평가 e2e **step④
// daily-test step_collect 진입측 leaf 컴포저** 산출 ↔ gating single-source 재유도 정합
// 순수 가드 (T-0890 박제, eval-leg T-0693 의 collect-leg mirror).
//
// 책임:
//   - `buildRealDataDailyStepCollectCommandPlan(env)`(T-0887) 는 입력 env 의 gating 판정
//     (`resolveRealDataE2eLiveGating(env)`)을 받아 `{ action, argv?, reason }`
//     (`RealDataDailyStepCollectCommandPlan`) 으로 합성하는 **step④ 진입측 leaf 컴포저**다.
//     PLAN.md 109행 ④ 단계의 `deploy/daily-test.sh` step_collect bash 배선이 이 plan 만
...
//     config/spec-path drift(특히 eval-leg spec 로의 교차오염), argv 길이/순서 어긋남,
//     action="skip" 인데 argv 존재(잘못 spawn 유발), reason 재포장 — 을 build-time 에
//     잡을 장치가 없었다. 본 가드는 합성 회귀로 손상된 step_collect command plan 이 bash
//     배선으로 새기 전 build-time 에 fail-fast throw 로 차단한다.
//
...
// eval-leg leaf 가드)의 collect-leg mirror — 차이점:
//   (a) 재유도 source gating 은 eval-leg 과 동일한 `resolveRealDataE2eLiveGating(env)`
//       단일 helper 위임(두 leg 활성/skip 판정 single source 공유)이라 gating 키 규칙
//       재구현 0 — 위임만.
//   (b) canonical argv 의 spec 경로만 collection live smoke spec 을 가리킨다 — collect-leg
//       컴포저가 export 한 `REALDATA_E2E_GITHUB_COLLECTION_SMOKE_SPEC_PATH` 상수를 import
//       재사용(중복 정의 0). eval-leg spec 로의 교차오염도 이 상수 ===-equal 로 catch.
//   (c) action 이 enum("run"/"skip")이라 enum 위반(다른 값) 분기도 RangeError 로 박제.
//
// Out of Scope (task T-0890):
//   - 컴포저 본문 수정 / self-wire 배선 — 본 가드는 외부 독립 검증만. self-wire 는
//     별도 후속 task(T-0891, eval-leg T-0694 짝 패턴 mirror).
...
//   - 실 jest spawn / 실 daily-test.sh step_collect wiring / Ollama / live-LLM /
//     credential wiring 0 — build-time 순수 가드만.
//   - eval-leg / seed-side / result-issue-side 가드/컴포저 변경 0 — step④ 진입측
//     daily-step-collect-command-plan seam 만.
import {
...
// 것과 구분해 노출(디버깅 가독성). eval-leg T-0693 mirror 와 동형 helper.
...
function assertPlanStructure(
  plan: RealDataDailyStepCollectCommandPlan | null | undefined,
): asserts plan is RealDataDailyStepCollectCommandPlan {
  if (
    plan === null ||
    plan === undefined ||
    typeof plan !== "object" ||
...
/**
 * 실 평가 e2e **step④ 진입측 leaf 컴포저**(`buildRealDataDailyStepCollectCommandPlan`)
 * 산출 plan 이, 주입된 입력 `env` 의 `resolveRealDataE2eLiveGating(env)` 재유도 결과와
 * 정합함을 런타임에서 검증하는 순수 가드(PLAN.md P5 109행 step ④ build-time chain 의
 * collect-leg 진입측 leaf-seam 무결성 조각). eval-leg leaf 가드
 * `assertRealDataDailyStepEvalCommandPlanConsistentWithGating`(T-0693)의 collect-leg
 * mirror.
 *
 * 검증하는 불변식:
 *   - gating.enabled=true ⇒ plan.action==="run" ∧ plan.argv 가 canonical 4-요소 벡터
 ...
 *
 * @param plan leaf 컴포저 산출 RealDataDailyStepCollectCommandPlan. 변형하지 않는다.
 ...
 */
export function assertRealDataDailyStepCollectCommandPlanConsistentWithGating(
