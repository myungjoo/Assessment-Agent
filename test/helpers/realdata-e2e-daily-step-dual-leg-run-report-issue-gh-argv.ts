// realdata-e2e-daily-step-dual-leg-run-report-issue-gh-argv.ts — 실 평가 e2e
// daily-step dual-leg run report 이슈 action + 명령-args → gh 인자-벡터(argv) 순수
// 빌더 (T-0899 박제, summary 축 T-0585 mirror).
//
// 책임:
//   - T-0898 의 `resolveRealDataDailyStepDualLegRunReportIssueAction` 은 어느 분기
//     (create/update)를 어느 이슈 번호에 실행할지를 결정했고
//     (`RealDataDailyStepDualLegRunReportIssueAction`), T-0897 의
//     `buildRealDataDailyStepDualLegRunReportIssueCommandArgs` 는 create/update 양쪽
//     인자 묶음을 모두 산출했다(`RealDataDailyStepDualLegRunReportIssueCommandArgs`).
//     PLAN.md 109행 step ④ 는 그 결과를 "daily-test result/rolling 이슈에 박제"하라
//     지시한다 — 실 gh 호출 직전에 두 산출물을 결합해 **실제 `gh` 명령에 그대로 넘길
//     인자-벡터(argv)** 를 만드는 단계가 아직 비어있다. 본 helper 가 그 마지막
//     build-time layer(argv 합성, step ④ 박제 경계) 다.
//   - caller(live wiring)는 (1) T-0897 명령-args + (2) T-0898 action 을 본 빌더에
//     입력해 완성된 argv 를 받고, (3) 그 argv 를 `execFile('gh', argv)` 로 실 호출한다.
//     본 helper 는 (3) 직전의 argv 합성만 순수 함수로 박제 — 실 gh 실행은 deferred
//     (본 helper 는 argv 합성만; credential gate).
//
...
//     credential 0, gh 실행 0. 외부 CLI 라이브러리(execa 등) 0 — 내장 배열 연산만.
//     순수 함수.
//
// 🔥 무공유 보장 (입력 mutate 0):
...
//
// 🔥 type 재사용 (중복 정의 0):
//   - `RealDataDailyStepDualLegRunReportIssueAction` 은
...
//     `import type` 재사용한다. 신규 type 정의 없음 — 두 기존 타입을 입력받아
//     `string[]` 만 산출.
//
// Out of Scope (task T-0899):
...
//     별도 wiring slice).
//   - create vs update 분기 결정 자체(T-0898 resolver 위임만 — 본 빌더는 주어진 action 을
//     소비만; resolver 재구현 금지).
//   - 명령-args 합성 자체(T-0897 위임만 — searchQuery / createArgs / updateArgs 재합성
//     금지).
//   - search argv 합성 / `gh search issues --json ...` 옵션 합성 / stdout 파싱 — 별도
//     slice(summary 축 T-0586/T-0587 mirror 후속).
//   - `deploy/daily-test.sh` 의 dual-leg step wiring / `latest-result.json` 연동(step ④
...
// buildRealDataDailyStepDualLegRunReportIssueGhArgv — action(create/update) + 명령-args
// 를 입력받아 실 `gh` 명령에 그대로 넘길 인자-벡터(argv, `gh` 실행 파일명 제외)를
// 산출하는 **순수 함수**.
//
// 분기:
...
export function buildRealDataDailyStepDualLegRunReportIssueGhArgv(
