// realdata-e2e-daily-step-dual-leg-run-report-issue-action-consistency.ts — 실 평가
// e2e daily-step dual-leg run report 이슈 create-or-update action
// (`resolveRealDataDailyStepDualLegRunReportIssueAction`, T-0898) 의 분기 결정과 최소
// issueNumber 선택이, 동일 (searchHits, marker) 로부터 **독립 재유도**한 expected action 과
// deep-equal 정합한지 검증하는 순수 가드(T-0995 박제).
//
// 동기: leaf resolver `resolveRealDataDailyStepDualLegRunReportIssueAction`(T-0898,
// `realdata-e2e-daily-step-dual-leg-run-report-issue-action.ts`)는 (1) body 가 marker 를
// 포함하는 후보 0건이면 {action:'create'}, (2) 1+건이면 후보 중 **최소 number(가장 오래된
// 이슈)** 로 {action:'update', issueNumber} 를 산출한다(멱등 회귀 보호 — 이슈 중복 방지). 이
// leaf 는 PLAN.md 109행 step ④ 박제 경계의 분기 layer 인데, 그 후보 필터링/최소 선택/create-
// update 분기가 회귀로 drift(예: 최소 대신 최대 선택·후보 판정 기준 변경·create/update 경계
// 오류)하면 build-time 에서 이를 잡는 독립 불변식 가드가 부재했다(NO-GUARD leaf). 본 가드가 그
// 빈칸을 채운다. 요약축 선례 T-0703(`assertRealDataResultIssueActionConsistentWithInputs`,
// `realdata-e2e-result-issue-action-consistency.ts`)을 daily-step 축으로 옮긴 판이다.
//
...
//   `resolveRealDataDailyStepDualLegRunReportIssueAction` 재호출 금지 — 재호출은 동일 로직
//   drift 를 양방향 상쇄해 잡지 못한다(재구현이 핵심 — leaf resolver 라 로직 독립 재유도).
//
...
//
// oracle 독립성 / type 분리: 본 가드는 producer(T-0898)를 **import 하지 않고**(로직 재구현 —
// 재호출 0) hit/action 타입만 `import type` 로 참조한다. 나중에 producer 에 value import 로
// self-wire 배선돼도 런타임 순환 의존이 생기지 않는다(self-wire 는 후속 slice, 본 task 범위 밖).
//
// Out of Scope (T-0995): producer 본문 수정 / self-wire 배선(후속 별도 slice) · 상위 종단 가드
// (issue-gh-command-plan/gh-argv/command-args) 수정 · 명령-args 합성·gh argv 통합 · production
// src 변경 · 실 gh 호출 / live wiring · 자동 복구/재유도/정규화 · zod·ajv 등 외부 validation
// 도입 — 전부 0.
import type {
...
function assertPositiveNumber(value: number): void {
  if (!Number.isInteger(value) || value <= 0) {
    throw new Error(
      `RealDataDailyStepDualLegRunReportIssueSearchHit.number 가 양의 정수가 아닙니다(${value}) — gh 응답 파싱 사고 방지를 위해 0 이하/비정수 number 는 허용되지 않습니다.`,
    );
  }
}
...
function assertSearchHitsStructure(
  searchHits:
    ...
    | null
    | undefined,
): asserts searchHits is RealDataDailyStepDualLegRunReportIssueSearchHit[] {
...
function assertActionStructure(
  ...
  if (action === null || typeof action !== "object" || Array.isArray(action)) {
    throw new TypeError(
      `action 이 객체가 아니다(타입: ${describe(action)}) — RealDataDailyStepDualLegRunReportIssueAction 이 필요하다.`,
    );
  }
...
// 컴포저(`resolveRealDataDailyStepDualLegRunReportIssueAction`, T-0898)의 후보 추출
// (`body.includes(marker)`)·최소 number 선택(`Math.min`)·create/update 분기를 의도적으로
// 재구현한다(컴포저 재호출 0 — 재호출은 동일 로직 drift 를 양방향 상쇄해 잡지 못한다).
// marker/number guard 도 동형으로 재실행해 컴포저와 같은 위반을 같은 throw 로 전파한다.
function deriveExpectedAction(
  searchHits: RealDataDailyStepDualLegRunReportIssueSearchHit[],
  marker: string,
): RealDataDailyStepDualLegRunReportIssueAction {
...
/**
 * 실 평가 e2e daily-step dual-leg run report 이슈 create-or-update action
 * (`resolveRealDataDailyStepDualLegRunReportIssueAction`, T-0898) 의 분기 결정과 최소
 * issueNumber 선택이, 동일 (searchHits, marker) 로부터 가드 안에서 독립 재유도한 expected
 * action 과 deep-equal byte-identical 정합함을 런타임에서 검증하는 순수 가드(PLAN.md P5 step
 * ④ 결과 박제 chain 의 멱등 분기-layer 무결성 조각). 요약축 선례 T-0703
 * (`assertRealDataResultIssueActionConsistentWithInputs`)의 "컴포저 재호출이 아니라 로직 독립
 * 재구현" 정신을 daily-step 축의 action 분기 layer 로 mirror 한다.
 *
 * 검증하는 불변식(single source — 컴포저 재호출 0, 후보 추출·최소 선택·분기 독립 재구현):
 ...
 */
export function assertRealDataDailyStepDualLegRunReportIssueActionConsistentWithInputs(
