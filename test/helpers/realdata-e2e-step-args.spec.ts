...
describe("buildRealDataE2eStepArgs — run plan → 평가+publish step-args 단일 진입 aggregator", () => {
  ...
  // T-0672 self-wire 배선 검증 — aggregator 가 산출 컨테이너({evaluation, publish}) 반환
  // 직전 consistency 가드를 (산출 stepArgs, runPlan, activities, results) 인자로 정확히
  // 1회 self-assert 하는지, 정상 합성이면 throw 0·반환 컨테이너 byte-identical·무공유 불변,
  // 가드가 throw 하면 aggregator 가 삼키지 않고 전파하는지, 위임 throw 입력에서는 가드
  // 진입 전 위임 throw 가 전파(가드 미호출)되는지 검증.
  describe("consistency 가드 self-wire (T-0672) — 반환 직전 self-assert 배선", () => {
    afterEach(() => {
      jest.restoreAllMocks();
    });

    it("정상 합성 → 가드가 (산출 stepArgs, runPlan, activities, results) 인자로 정확히 1회 호출됨", () => {
      ...
      // 가드에 넘어간 첫 인자가 aggregator 가 반환한 바로 그 stepArgs 참조여야 한다(검증 대상 일치).
    ...
    it("빈 activities + 빈 results 분기에서도 가드가 (산출 stepArgs, runPlan, [], []) 인자로 정확히 1회 호출됨", () => {
      const spy = jest.spyOn(
        consistency,
        "assertRealDataE2eStepArgsConsistentWithSources",
      );
      ...
      expect(spy).toHaveBeenCalledTimes(1);
      expect(spy).toHaveBeenCalledWith(stepArgs, runPlan, activities, results);
    });

    it("정상 합성 → 가드 통과 후 반환 컨테이너가 가드 미배선 기대값(두 위임 산출)과 byte-identical(불변)", () => {
      const runPlan = makeRunPlan();
      const activities = mixedActivities();
      ...
      // self-wire 가 반환 컨테이너를 변형하지 않음 — 두 위임 산출과 byte-identical.
      expect(stepArgs.evaluation).toEqual(
        buildRealDataEvaluationStepArgs(runPlan, activities),
      );
      expect(stepArgs.publish).toEqual(
    ...
    it("평가 위임 throw 입력(modelId 공백-only)에서는 가드 진입 전 평가 위임 throw 가 전파(가드 미호출)", () => {
      const spy = jest.spyOn(
        consistency,
        "assertRealDataE2eStepArgsConsistentWithSources",
      );
      ...
      // 평가 위임 단계에서 throw → 가드 self-assert 까지 도달하지 못함.
      expect(spy).not.toHaveBeenCalled();
    });

    it("publish 위임 throw 입력(gitSha 공백-only)에서는 가드 진입 전 publish 위임 throw 가 전파(가드 미호출)", () => {
      const spy = jest.spyOn(
        consistency,
        "assertRealDataE2eStepArgsConsistentWithSources",
      );
      ...
      // publish 위임 단계에서 throw → 가드 self-assert 까지 도달하지 못함.
      expect(spy).not.toHaveBeenCalled();
    });

    it("publish 위임 throw 입력(dateToken 공백-only)에서도 가드 진입 전 위임 throw 가 전파(가드 미호출)", () => {
      const spy = jest.spyOn(
        consistency,
        "assertRealDataE2eStepArgsConsistentWithSources",
      );
    ...
    it("self-wire 배선 후에도 입력 비변형 + 동일 입력 두 번 deterministic + 반환 컨테이너 무공유", () => {
      const runPlan = makeRunPlan();
      const activities = mixedActivities();
      const results = [...MULTIPLE_RESULTS];
      const runPlanSnapshot = JSON.stringify(runPlan);
      ...
      expect(a).toEqual(b);
      // 무공유(반환 컨테이너의 evaluation/publish 트리 mutate 가 후속 호출 결과에 누출 0).
