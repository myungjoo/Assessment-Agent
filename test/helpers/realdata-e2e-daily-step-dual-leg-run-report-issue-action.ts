// realdata-e2e-daily-step-dual-leg-run-report-issue-action.ts — 실 평가 e2e
// daily-step dual-leg run report 이슈 search response → create-or-update action
// 순수 resolver (T-0898 박제).
//
// 책임:
...
//     ({searchQuery, createArgs, updateArgs})으로 산출했다. PLAN.md 109행 step ④ 는
//     그 결과를 "daily-test result/rolling 이슈에 박제"하라 지시한다 — 실 gh issue
//     호출(create/edit) 직전에 **기존 이슈가 있는지·어느 이슈를 갱신할지** 를 결정하는
//     분기가 필요하다. 본 helper 가 그 분기 layer 다.
...
//     descriptor 를 받고, (3) action 에 따라 T-0897 의 createArgs / updateArgs 중
//     하나로 `gh issue create` 또는 `gh issue edit <issueNumber>` 를 실행한다. 본
//     helper 는 (2) 만 순수 함수로 박제 — 실 gh search 는 여전히 deferred(분기 결정만).
//   - summary 축(T-0583 명령-args → **T-0584** 분기 resolver)의 선례와 정확히 동형.
//     dual-leg 축은 명령-args(T-0897)까지만 있고 그 분기 layer(T-0584 mirror)가 없었다.
//     본 task 가 그 gap 을 순수 함수로 메운다.
//
// 🔥 멱등 회귀 보호 (최소 번호 update):
//   - 후보(body 가 marker 를 부분 문자열로 포함하는 hit) 0건 → create(신규 생성).
//   - 후보 1건 → 그 이슈 update.
...
//   - 실 gh search issues 호출은 deferred(본 helper 는 분기 결정만 — 부수효과 0). 실
//     네트워크 호출 0, `process.env` 읽기 0, DB 접근 0, live-LLM 0, credential 0. 외부
//     템플릿/해시/CLI 라이브러리 0 — 내장 string/배열 연산만. 순수 함수.
//
// 🔥 무공유 보장 (입력 mutate 0):
//   - 본 resolver 는 입력 `searchHits` 배열·각 hit 객체를 변형하지 않는다(읽기만).
//     호출마다 새 action 객체를 반환 — 입력 / 다음 호출 결과와 무공유.
//
// 🔥 type 분리 (descriptor / 명령-args import 불요):
//   - 본 resolver 는 marker 를 문자열로만 보고 hit body 의 포함 여부만 판정하므로
//     T-0896 descriptor / T-0897 명령-args 타입을 import 하지 않는다(분리 책임). 신규
//     type 은 입력 `RealDataDailyStepDualLegRunReportIssueSearchHit` + 출력
//     `RealDataDailyStepDualLegRunReportIssueAction` 만 자급 정의.
//
// Out of Scope (task T-0898):
...
//     해서 `RealDataDailyStepDualLegRunReportIssueSearchHit[]` 로 전달하는 책임).
//   - `deploy/daily-test.sh` 의 dual-leg spawn wiring(step ④ live wiring, ADR-0045 gate).
//   - 명령-args 합성 자체(T-0897 위임만 — searchQuery / createArgs / updateArgs 재합성
...
export interface RealDataDailyStepDualLegRunReportIssueSearchHit {
  number: number;
  title: string;
  body: string;
}

// RealDataDailyStepDualLegRunReportIssueAction — create-or-update 분기 결정의
// discriminated union.
//   - {action: 'create'}: 후보 0건 → 신규 이슈 생성.
//   - {action: 'update', issueNumber}: 후보 1+ 건 → 해당(최소 번호) 이슈 갱신.
export type RealDataDailyStepDualLegRunReportIssueAction =
  | { action: "create" }
...
// issue number guard — gh 응답이 정상이면 number 는 항상 양수다. 0 이하/비정수면 파싱
// 사고로 간주하고 명시적 throw(비정상 number 가 update issueNumber 로 새는 것을 차단).
...
// resolveRealDataDailyStepDualLegRunReportIssueAction — gh search 응답(searchHits) +
// 멱등 marker 를 입력받아 create-or-update 분기를 결정하는 **순수 함수**.
//
// 분기:
//   - guard: marker 빈/공백 → throw. 각 hit 의 number 가 0 이하/비정수 → throw.
//   - 후보(body 가 marker 를 부분 문자열로 포함하는 hit) 추출:
//       0건 → {action: 'create'}.
...
export function resolveRealDataDailyStepDualLegRunReportIssueAction(
