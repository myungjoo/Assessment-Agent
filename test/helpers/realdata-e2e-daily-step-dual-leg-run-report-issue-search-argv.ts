// realdata-e2e-daily-step-dual-leg-run-report-issue-search-argv.ts — 실 평가 e2e
// daily-step dual-leg run report 이슈 searchQuery → gh search issues 인자-벡터(argv)
// 순수 빌더 (T-0900 박제).
//
// 책임:
//   - T-0897 의 `buildRealDataDailyStepDualLegRunReportIssueCommandArgs` 가 dual-leg
//     run report descriptor 를 멱등 search-or-update 명령-args 묶음
//     ({searchQuery, createArgs, updateArgs})으로 산출했다. T-0898 의
//     `resolveRealDataDailyStepDualLegRunReportIssueAction` 은 그 `searchHits` 를
...
//   - T-0899 가 create/edit 의 argv 를 박제했다면, 본 helper 는 search 의 argv 를
//     박제해 build-time chain 의 양 끝(search ↔ create/edit)을 모두 닫는다. summary
//     축(create/edit argv T-0585 → **T-0586** search argv)의 선례와 정확히 동형 —
//     dual-leg 축은 create/edit argv(T-0899)까지만 있고 그 search argv layer(T-0586
//     mirror)가 없었다. 본 task 가 그 gap 을 순수 함수로 메운다.
//   - caller(live wiring)는 (1) command-args 로부터 본 빌더로 search argv 를 얻고,
//     (2) `execFile('gh', searchArgv)` → JSON parse → searchHits[] 를 얻고, (3) T-0898
//     resolver 로 action 을 결정하고, (4) T-0899 빌더로 create/edit argv 를 얻고,
...
//
// 🔥 --json 필드 정합 (T-0898 cross-reference):
//   - `--json` 인자는 `"number,title,body"` 고정 — 이 세 필드가 T-0898
//     `RealDataDailyStepDualLegRunReportIssueSearchHit`({number, title, body})의 모든
//     멤버와 정확히 일치하도록 요청한다(분리 책임 — type import 는 cross-check 만, 실행
//     의존 아님; spec 주석 cross-reference).
//
// 🔥 type 재사용 (중복 정의 0):
//   - `RealDataDailyStepDualLegRunReportIssueCommandArgs` 는
//     `./realdata-e2e-daily-step-dual-leg-run-report-issue-command-args`(T-0897)에서
//     `import type` 재사용한다. 신규 type 정의 없음 — commandArgs 입력만 받아
//     `string[]` 만 산출.
//
// Out of Scope (task T-0900):
...
//     `RealDataDailyStepDualLegRunReportIssueSearchHit[]` 산출(caller 책임 — 본 빌더는
//     input argv 만; summary 축 T-0587 mirror 후속 slice).
...
//   - action resolver 분기 결정(T-0898 위임만 — 본 빌더는 search 의 argv 만; searchHits
//     해석은 그 단계).
...
// 대신 named constant 로 박제.
export const REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_JSON_FIELDS =
  "number,title,body";

// --limit 결정론 상수 — dual-leg run report 이슈는 동일 marker 당 1건만 박제될 예정이나,
// 우연적 다중 매칭(gh search 의 느슨한 매칭) 대비 충분한 상한을 둔다. T-0898 resolver 가
// 후보 2+ 건도 최소 번호로 멱등 수렴하므로 상한은 안전 여유분. 매직 넘버 대신 named
// constant 로 박제.
export const REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_LIMIT = "30";

// 빈/공백-only searchQuery guard — searchQuery 가 빈 문자열·공백-only 면 `--match body`
// 검색이 전체(또는 무의미) 매칭으로 번져 잘못된 분기를 유발하므로 명시적 throw 한다
// (조용한 통과 차단 — T-0898 marker guard 와 동형 정합).
...
// buildRealDataDailyStepDualLegRunReportIssueSearchGhArgv — 명령-args 의 searchQuery 를
// 입력받아 실 `gh search issues` 명령에 그대로 넘길 인자-벡터(argv, `gh` 실행 파일명
// 제외)를 산출하는 **순수 함수**.
//
// 산출 argv:
//   ["search", "issues", "--match", "body", searchQuery, "--json",
//    "number,title,body", "--limit", "30"]
//   - `--match body` 고정 — marker 는 issue body 안에 박혀있으므로 body 매칭(T-0897
//     command-args searchQuery = descriptor.marker 정합).
...
export function buildRealDataDailyStepDualLegRunReportIssueSearchGhArgv(
