// realdata-e2e-daily-step-dual-leg-run-report-issue-search-hit-shape.ts — 실 평가 e2e
// daily-step dual-leg run report 이슈 search 가 파싱 산출하는
// `RealDataDailyStepDualLegRunReportIssueSearchHit` 의 **자기 own enumerable 키 집합**이
// 선언된 정규 parse-shape 키 집합(`REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_
// SEARCH_PARSE_SHAPE_KEYS`)과 정확히 일치하는지 검증하는 순수 가드 (T-1014 박제).
//
// 책임:
//   - `parseRealDataDailyStepDualLegRunReportIssueSearchOutput`
//     (`realdata-e2e-daily-step-dual-leg-run-report-issue-search-parse.ts`, line 121)는
//     `gh search issues ... --json <fields>` stdout JSON 을 정규화해 산출 hit 를
//     `{number, title, body}` 만 추출한다(line 152~156, 추가 필드 drop).
//   - `REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_PARSE_SHAPE_KEYS =
//     ["number","title","body"]`(T-1012,
//     `realdata-e2e-daily-step-dual-leg-run-report-issue-search-json-fields.ts` line 75)는
//     그 추출 shape 의 정규 키 목록을 named constant 로 박제한다.
...
//     json-fields 가드(T-1012)는 상수를 `--json` 요청 필드와만 묶지, 파서가 실제
//     산출하는 hit 와 묶지 않는다. 본 가드가 그 constant↔produced-hit seam 을 집합
//     동치 비교로 닫는다 — T-1012 request-side 가드의 parse-output consumer-side sibling.
//
...
//
// 🔥 R-59 raw 미접촉:
//   - 키 이름 문자열만 비교하며 hit 의 값(title/body 본문)은 읽지 않는다 — raw 활동 본문
//     파일/전역 저장 0.
//
// 책임 경계(task Out of Scope — T-1014):
//   - 파서(`...issue-search-parse.ts`) / `RealDataDailyStepDualLegRunReportIssueSearchHit`
//     interface(`...issue-action.ts`) / 상수(`...issue-search-json-fields.ts`) 본문 변경
//     0 — import·read 만.
//   - 신설 가드의 builder/파서 self-wire(파서 산출 직전 본 가드 호출 배선) — 본 task 는
//     신설만, self-wire 는 Follow-up(요약축 T-0659 self-wire 동형).
//   - json-fields request-side 가드(T-1012) / 그 self-wire(T-1013) 변경 0 — 본 가드는 그
//     consumer-side sibling 신설이지 기존 가드 수정 아님.
...
//
// 패턴 mirror: 요약축 `realdata-e2e-result-issue-search-hit-shape.ts`(T-0659 동형) — 순수
// 가드 / 구조 결손=TypeError·값 정합 위반=RangeError 구분 / single-source 비교 /
// 한국어 JSDoc·책임 경계 주석 / 자동 복구 0 / 산출 경로 자동 배선 0. 본 가드는 그 에러
// 정책·관례·JSDoc 톤을 mirror 하되 어휘만 daily-step 축으로 치환하며, "파싱 산출 hit 의
...
// 본 모듈은 정규 키 목록 상수를 신규 정의하지 않고 T-1012 single-source 를 그대로
// re-export 만 한다(소비자가 한 곳에서 가드+상수를 함께 import 할 수 있게). 정의·진실의
// 원천은 여전히 `realdata-e2e-daily-step-dual-leg-run-report-issue-search-json-fields.ts`.
export { REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_PARSE_SHAPE_KEYS };
...
function assertHitStructure(
  hit: RealDataDailyStepDualLegRunReportIssueSearchHit | null | undefined,
): asserts hit is RealDataDailyStepDualLegRunReportIssueSearchHit {
  if (hit === null || hit === undefined) {
    throw new TypeError(
      "hit 가 null/undefined 일 수 없다 — 파서가 산출한 RealDataDailyStepDualLegRunReportIssueSearchHit 객체가 필요하다.",
    );
  }
...
/**
 * 실 평가 e2e daily-step dual-leg run report 이슈 search 의 파서
 * `parseRealDataDailyStepDualLegRunReportIssueSearchOutput` 가 산출한
 * `RealDataDailyStepDualLegRunReportIssueSearchHit` 의 **자기 own enumerable 키 집합**이
 * 선언된 정규 parse-shape 키 집합(`parseShapeKeys`)과 정확히 일치하는지 런타임에서 검증하는
 * 순수 가드(PLAN.md P5 109행 step ④ 결과 박제 chain 의 parse-shape ↔ produced-hit seam
 * 무결성 조각). T-1012 json-fields 가드(request-side)의 parse-output consumer-side sibling.
 *
 ...
 *
 * @param hit 파서가 산출한 `RealDataDailyStepDualLegRunReportIssueSearchHit`(예:
 *   `{number:42, title:"t", body:"b"}`). 변형하지 않는다(own 키만 읽어 비교). 본 가드는
 *   키 집합만 보며 값은 읽지 않는다.
 ...
 */
export function assertRealDataDailyStepDualLegRunReportIssueSearchHitShapeMatchesParseShapeKeys(
  hit: RealDataDailyStepDualLegRunReportIssueSearchHit,
  parseShapeKeys: readonly string[],
): void {
