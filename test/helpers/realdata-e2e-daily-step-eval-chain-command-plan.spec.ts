// realdata-e2e-daily-step-eval-chain-command-plan.spec.ts — T-1121 colocated unit spec.
//
// R-112 cover 구조:
//   - happy-path: gating env 7 종 모두 set → action="run" + argv 가 full-chain live smoke
...
import {
  buildRealDataDailyStepEvalChainCommandPlan,
  REALDATA_E2E_EVAL_CHAIN_LIVE_SMOKE_SPEC_PATH,
  REALDATA_E2E_SMOKE_JEST_CONFIG,
} from "./realdata-e2e-daily-step-eval-chain-command-plan";
import {
...
describe("buildRealDataDailyStepEvalChainCommandPlan", () => {
  describe("happy-path (enabled → run)", () => {
    it('gating env 7 종 모두 set 시 action="run" 을 산출한다', () => {
      const plan = buildRealDataDailyStepEvalChainCommandPlan(makeEnabledEnv());
      expect(plan.action).toBe("run");
    });

    it("run 산출의 argv 는 smoke config + full-chain live smoke spec 경로를 정확히 포함한다", () => {
      const plan = buildRealDataDailyStepEvalChainCommandPlan(makeEnabledEnv());
    ...
    it("argv 는 full-chain smoke spec 경로(eval-chain-live)를 가리킨다", () => {
      const plan = buildRealDataDailyStepEvalChainCommandPlan(makeEnabledEnv());
      expect(REALDATA_E2E_EVAL_CHAIN_LIVE_SMOKE_SPEC_PATH).toBe(
        "test/smoke/realdata-e2e-eval-chain-live.smoke-spec.ts",
      );
      expect(plan.argv).toContain(REALDATA_E2E_EVAL_CHAIN_LIVE_SMOKE_SPEC_PATH);
    });
    ...
    it("argv 가 단일-spec bound(--runTestsByPath + 단일 경로)로 묶인다", () => {
      ...
      expect(pathArgs).toEqual([REALDATA_E2E_EVAL_CHAIN_LIVE_SMOKE_SPEC_PATH]);
    });
  ...
  describe("error path (disabled → skip)", () => {
    it('gating env 전부 부재 시 action="skip" + throw 0', () => {
      ...
      const plan = buildRealDataDailyStepEvalChainCommandPlan({});
  ...
  describe("flow / branch 분기 cover", () => {
    it("enabled true → run, enabled false → skip 의 reason 이 분기별로 구분된다", () => {
      const runPlan =
        buildRealDataDailyStepEvalChainCommandPlan(makeEnabledEnv());
      const skipPlan = buildRealDataDailyStepEvalChainCommandPlan({});
  ...
  describe("negative cases 충분 cover", () => {
    ...
    it("(1) PAT 만 부재한 부분 set → skip", () => {
      ...
      const plan = buildRealDataDailyStepEvalChainCommandPlan(env);
      expect(plan.action).toBe("skip");
      expect(plan.argv).toBeUndefined();
    ...
    it("(1c) LLM model 만 부재한 부분 set → skip", () => {
      const env = makeEnabledEnv({ [REALDATA_E2E_LLM_MODEL_ENV]: undefined });
      const plan = buildRealDataDailyStepEvalChainCommandPlan(env);
      expect(plan.action).toBe("skip");
      expect(plan.argv).toBeUndefined();
    });
    ...
    it("(2b) 빈 문자열 API version 값 → skip", () => {
      const env = makeEnabledEnv({ [REALDATA_E2E_LLM_API_VERSION_ENV]: "" });
      const plan = buildRealDataDailyStepEvalChainCommandPlan(env);
      expect(plan.action).toBe("skip");
    });
    ...
    it("(5-run) run 경로에서 입력 env 객체를 mutate 하지 않는다", () => {
      ...
      buildRealDataDailyStepEvalChainCommandPlan(env);
  ...
  describe("결정론·무공유", () => {
    it("동일 env 두 번 호출 → deep-equal 산출", () => {
      const a = buildRealDataDailyStepEvalChainCommandPlan(makeEnabledEnv());
      const b = buildRealDataDailyStepEvalChainCommandPlan(makeEnabledEnv());
      expect(a).toEqual(b);
    });
    ...
    it("skip 경로도 동일 env 두 번 호출 시 deep-equal(결정론)", () => {
      const a = buildRealDataDailyStepEvalChainCommandPlan({});
      const b = buildRealDataDailyStepEvalChainCommandPlan({});
      expect(a).toEqual(b);
    });
  });
});
