// realdata-e2e-daily-step-dual-leg-run-report-issue-outcome-parse-shape.ts — 실 평가
// e2e daily-step dual-leg run report 이슈 outcome 파서가 산출하는
// `RealDataDailyStepDualLegRunReportIssueOutcome` 의 **자기 own enumerable 키 집합**이
// 선언된 정규 parse-shape 키 집합(`REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_OUTCOME_PARSE_SHAPE_KEYS`)
// 과 정확히 일치하는지 검증하는 순수 가드 (T-0904 박제).
//
// 책임:
//   - `parseRealDataDailyStepDualLegRunReportIssueCreateEditOutput`(T-0903,
//     `realdata-e2e-daily-step-dual-leg-run-report-issue-output-parse.ts` line 108~)는
//     `gh issue create` / `gh issue edit <n>` 의 stdout 을 정규화해 outcome 을
//     `{issueNumber, url}` 만 산출한다(line 123~126, 추가 필드 drop).
//   - `RealDataDailyStepDualLegRunReportIssueOutcome` interface(같은 파일 line 66~69,
//     `{issueNumber:number; url:string}`)는 그 산출 outcome 의 선언된 shape.
...
//     dual-leg 축 producer↔declared-shape seam(post-execution parse-shape)을 집합 동치
//     비교로 닫는다 — summary 축 T-0661(`assertRealDataResultIssueOutcomeMatchesParseShape`)
//     의 정확한 dual-leg mirror.
//   - PLAN.md P5 109행 step ④ 결과 박제 chain: (1) 컴포저(T-0894) → … → (9) gh
//     command-plan 종단 컴포저(T-0902) → (10) create/edit stdout → outcome 파서(T-0903)
//     → **(11) 본 outcome producer↔declared-shape parse-shape 가드(T-0904)**.
//   - search 측이 `--json`/json-fields 같은 선행 상수를 re-export 한 것과 달리,
//     post-execution 측에는 진실의 원천이 될 선행 상수가 없으므로, 본 가드가 **자체
//     single-source 정규 키 목록 상수**
//     `REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_OUTCOME_PARSE_SHAPE_KEYS
//     = ["issueNumber", "url"]` 를 신규 정의한다(T-0661 동형 근거).
//
// 🔥 build-time 완결 — dependency-free (cloud cron 자율 실행 가능):
//   - 실 네트워크 호출 0, `process.env` 읽기 0, DB 접근 0, live-LLM 0, credential 0,
//     gh 실행 0. 새 외부 dependency 0(zod·ajv 등 도입 0). 내장 객체/배열/Set 연산만.
//     실 `execFile('gh', ...)` deferred — 본 가드는 이미 산출된 outcome 의 키 집합만 비교.
//
// 🔥 순수성·결정론·무공유:
...
//   - 본 가드는 outcome 의 **키 이름 문자열만** 비교하고 이슈 본문/narrative/credential 은
//     보유·노출하지 않는다. outcome 의 값(issueNumber/url 본문)은 읽지 않는다.
//
// 책임 경계(task Out of Scope — T-0904):
//   - 파서(`realdata-e2e-daily-step-dual-leg-run-report-issue-output-parse.ts`) /
//     `RealDataDailyStepDualLegRunReportIssueOutcome` interface 본문 변경 0 —
//     import·read 만(신규 type 정의 0).
//   - 신설 가드의 producer self-wire(파서 산출 직전 본 가드 호출 배선) — 본 task 는
//     신설만, self-wire 는 후속 slice(summary 축 T-0662 mirror).
//   - outcome 값-정합 재유도 가드(summary 축 T-0723/T-0724 mirror) — 본 task 는 키 집합
//     set-equality(shape)만.
//   - search-side parse-shape 가드 / summary 축(T-0580~T-0661) helper 변경·재호출 0 —
//     본 가드는 dual-leg 축 post-execution seam 신설이지 기존 가드 수정 아님.
...
//
// 패턴 mirror: `realdata-e2e-result-issue-outcome-parse-shape.ts`(T-0661 동형) — 순수
// 가드 / 구조 결손=TypeError·집합 정합 위반=RangeError 구분 / single-source 비교 /
// 한국어 JSDoc·책임 경계 주석 / 자동 복구 0 / 산출 경로 자동 배선 0. 본 가드는 그
// 에러 정책·관례·JSDoc 톤을 mirror 하되, 검증 대상이 summary outcome `{issueNumber,url}`
// 이 아니라 dual-leg outcome `{issueNumber,url}` 이고, 정규 키 목록은 re-export 가 아니라
// **본 모듈에서 신규 정의**한다.
...
// 정규 outcome parse-shape 키 목록 —
// `parseRealDataDailyStepDualLegRunReportIssueCreateEditOutput`(T-0903) 가 정규화
// 반환하는 `{issueNumber, url}` 의 키와 `RealDataDailyStepDualLegRunReportIssueOutcome`
...
// 없으므로 신규 정의).
//   - cross-reference: realdata-e2e-daily-step-dual-leg-run-report-issue-output-parse.ts
//     line 123~126 (`{issueNumber, url: match[0].trim()}`)
//   - cross-reference: realdata-e2e-daily-step-dual-leg-run-report-issue-output-parse.ts
//     line 66~69 (`interface RealDataDailyStepDualLegRunReportIssueOutcome
//     { issueNumber: number; url: string; }`)
// 본 상수의 순서·중복·집합 내용이 곧 outcome parse-shape 의 진실의 원천.
export const REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_OUTCOME_PARSE_SHAPE_KEYS =
  ["issueNumber", "url"] as const;
...
// RangeError 가 아니라 TypeError 로 구분한다(값·집합 정합 위반과 분리). 배열도 거부 —
// outcome 은 키-값 객체여야 한다.
function assertOutcomeStructure(
  outcome: RealDataDailyStepDualLegRunReportIssueOutcome | null | undefined,
): asserts outcome is RealDataDailyStepDualLegRunReportIssueOutcome {
  if (outcome === null || outcome === undefined) {
    throw new TypeError(
      "outcome 이 null/undefined 일 수 없다 — 파서가 산출한 RealDataDailyStepDualLegRunReportIssueOutcome 객체가 필요하다.",
    );
  }
...
// 로 별도 거부한다(T-0661 convention 동형).
...
/**
 * 실 평가 e2e daily-step dual-leg run report 이슈 outcome 파서
 * `parseRealDataDailyStepDualLegRunReportIssueCreateEditOutput`(T-0903) 가 산출한
 * `RealDataDailyStepDualLegRunReportIssueOutcome` 의 **자기 own enumerable 키 집합**이
 * 선언된 정규 parse-shape 키 집합(`parseShapeKeys`)과 정확히 일치하는지 런타임에서
 * 검증하는 순수 가드(PLAN.md P5 109행 step ④ 결과 박제 chain 의 outcome producer ↔
 * declared-shape seam 무결성 조각). summary 축 T-0661 outcome ↔ parse-shape 가드의
 * dual-leg mirror.
 *
 ...
 *
 * @param outcome 파서가 산출한 `RealDataDailyStepDualLegRunReportIssueOutcome`(예:
 *   `{issueNumber:42, url:"https://github.com/o/r/issues/42"}`). 변형하지 않는다(own
 *   키만 읽어 비교). 본 가드는 키 집합만 보며 값은 읽지 않는다.
 ...
 */
export function assertRealDataDailyStepDualLegRunReportIssueOutcomeMatchesParseShape(
  outcome: RealDataDailyStepDualLegRunReportIssueOutcome,
  parseShapeKeys: readonly string[],
): void {
