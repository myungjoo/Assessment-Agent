...
//     차단한다.
//
// 🔥 topology 차이 (요약축 T-0665 2 source·2단 → daily-step single source·3단):
...
//     report+commandArgs) + search-argv 2단 위임이지만, daily-step 은 **single source
//     `report`** 이고 재유도가 descriptor(report → descriptor) → command-args(descriptor →
//     commandArgs) → search-argv(commandArgs → searchArgv) **3단 순차 위임**이다. 따라서
//     시그니처는 `(plan, report)` 단일 source 이고, plan 의 첫 필드는 요약축의 `report`
//     (rendered) 자리에 대응하는 `descriptor` 다.
//
// 검증하는 불변식(single source — 세 위임 함수 직접 순차 호출 재유도):
...
//     않는다 — 위임 호출만(drift 0 보장의 핵심, SSOT 보존).
//
...
//     `assertNonBlank` throw)하면 가드가 삼키지 않고 그대로 전파(자체 try/catch 0). 이
//     throw 는 descriptor 단계에서 발생하므로 command-args·search-argv 재유도는 미도달한다.
...
// TypeError / 값 정합 위반=RangeError 구분 fail-fast). 본 가드는 그 daily-step mirror —
// describe/throw 계약·메시지 포맷을 동형으로 따르되 source 를 report 하나로, 재유도를 3단
// 위임으로 조정한다.
//
 ...
 * single source `report` 를 세 위임 함수로 직접 순차 재유도한 결과와 byte-identical
 * 정합함을 런타임에서 검증하는 순수 가드(PLAN.md P5 109행 step ④ 결과 박제 chain 의
 * post-composition 무결성 조각). 요약축 종단 가드
 * `assertRealDataResultIssuePublishPlanConsistentWithSources`(T-0665) 의 daily-step
 * mirror — source 를 report 하나로, 재유도를 3단 순차 위임으로 조정한다.
 *
 * 검증하는 불변식(single source — 세 위임 함수 직접 순차 호출 재유도):
 *   expectedDescriptor  = buildRealDataDailyStepDualLegRunReportIssueDescriptor(report)
 *   expectedCommandArgs = buildRealDataDailyStepDualLegRunReportIssueCommandArgs(
 ...
 *   - 재유도 expected 와 `plan` 의 어느 구성요소라도 drift → 한국어 RangeError. 메시지에
 *     어느 구성요소가 어긋났는지 포함.
 *   - 재유도 chain 이 throw(report.gitSha/dateToken 빈/공백 → descriptor 위임 throw)하면
 *     가드가 삼키지 않고 그대로 전파(가드 본문 재유도 단계의 위임 throw — 자체 try/catch 0).
 *     이 throw 는 descriptor 단계에서 발생하므로 command-args·search-argv 재유도는 미도달.
 *   - silent 통과(위반인데 정상 void) 0.
 *
 * 검사 순서: 구조(plan 존재 · descriptor/commandArgs object · searchArgv string[]) →
 * 재유도(descriptor → command-args → search-argv 순차) → 구성요소별 순회 비교(descriptor →
 ...
 *   한다.
 * @param report 재유도 chain 의 single source. 변형하지 않는다(읽기만). 세 위임 함수에
 *   순차로 넘겨 expected descriptor/commandArgs/searchArgv 를 재유도한다. gitSha/dateToken
 *   빈/공백이면 descriptor 위임 guard throw 가 전파된다.
...
export function assertRealDataDailyStepDualLegRunReportIssuePublishPlanConsistentWithSource(
  ...
  // 기대값 재유도 — 컴포저가 내부에서 엮는 세 위임 함수를 본 가드가 직접 같은 순서로
  ...
  // command-args·search-argv 재유도는 미도달한다(순차 short-circuit).
