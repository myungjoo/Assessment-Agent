// realdata-e2e-daily-step-dual-leg-run-report-issue-search-parse-consistency.ts —
// 실 평가 e2e daily-step dual-leg run report 이슈 검색 파서
// `parseRealDataDailyStepDualLegRunReportIssueSearchOutput`(T-0901) 의 **산출**
// (`RealDataDailyStepDualLegRunReportIssueSearchHit[]`) 이 raw `gh search issues
// --json number,title,body` stdout 으로부터 **독립 재유도**한 expected 배열과
// deep-equal 정합한지 검증하는 순수 가드(T-0908 박제).
//
// 동기: dual-leg chain 의 입력측 search leg 컴포저
// `parseRealDataDailyStepDualLegRunReportIssueSearchOutput`(T-0901,
// realdata-e2e-daily-step-dual-leg-run-report-issue-search-parse.ts)은 `gh search
// issues --json number,title,body` 의 stdout(JSON 문자열)을
// `RealDataDailyStepDualLegRunReportIssueSearchHit[]` 로 파싱·검증하지만, 그 산출
// 배열 **전체**가 raw stdout 으로부터 올바른 개수·순서·필드값(number/title/body)·추가
// 필드 drop 으로 단조 재유도됐는지 build-time 에 확인하는 **값-정합 가드는 부재**였다 —
// 파서가 silent 하게 개수·순서·값·추가필드를 산출해도 이를 잡을 수단이 없었다. 본 가드는
...
// action resolver(T-0898)로 새기 전 차단). summary 축 선례
// T-0721(assertRealDataResultIssueSearchOutputConsistentWithStdout,
// realdata-e2e-result-issue-search-parse-consistency.ts)의 정확한 dual-leg 축 mirror.
//
// 재유도 규칙(single-source 동형): 컴포저(T-0901)와 동일한 검증 규약을 **독립 재구현**한다
...
// (`parseRealDataDailyStepDualLegRunReportIssueSearchOutput`)는 **호출하지 않는다** —
// 재호출 deep-equal 은 양방향 drift 상쇄라 무의미하다(독립 재유도가 핵심).
//
...
//
// Out of Scope (T-0908): 컴포저 본문 수정 / self-wire 배선(후속 slice — summary 축
// T-0721→T-0722·dual-leg output 축 T-0906→T-0907 분리 패턴 동형) · output-parse 값-정합
// 가드(T-0906/T-0907)·outcome-parse-shape set-equality 가드(T-0904/T-0905) 수정 ·
// 자동 복구/재합성/정규화 · zod·ajv 등 외부 validation 도입 · production `src/` 변경 —
// 전부 0.
...
// reDeriveExpectedHits — raw stdout 만으로 expected
// `RealDataDailyStepDualLegRunReportIssueSearchHit[]` 를 **독립 재유도**한다.
// 컴포저(T-0901)의 검증 규약(배열 → non-null 객체 → number 양정수 → title/body 문자열 →
// `{number,title,body}` 정규화)을 의도적으로 재구현
// (`parseRealDataDailyStepDualLegRunReportIssueSearchOutput` 재호출 0 — 재호출은 양방향
// drift 상쇄로 의미가 없다). 구조 결손은 TypeError 로 분기한다(값 정합 위반 RangeError 와
// 구분). 입력 stdout 은 불변 문자열이라 mutate 불가.
function reDeriveExpectedHits(
  stdout: string,
...
): asserts hits is RealDataDailyStepDualLegRunReportIssueSearchHit[] {
  if (!Array.isArray(hits)) {
    throw new TypeError(
      `hits 가 배열이 아니다(${typeof hits}, 값: ${String(
        hits,
      )}) — 파서 산출은 RealDataDailyStepDualLegRunReportIssueSearchHit[] 여야 한다.`,
    );
  }
...
// areHitsDeepEqual — 두 `RealDataDailyStepDualLegRunReportIssueSearchHit[]` 가
// 개수·순서·필드값(number ===, title ===, body ===) 면에서 deep-equal 인지 비교한다.
// 추가 필드 drop 정합은 산출 hit 의 키 집합이 정확히 `{number,title,body}` 인지 함께
// 검사해 누설을 잡는다(재유도 expected 는 항상 3 키만 가지므로 산출이 추가 키를 누설하면
// 불일치). 순수 비교(쓰기 0).
function areHitsDeepEqual(
  actual: RealDataDailyStepDualLegRunReportIssueSearchHit[],
  expected: RealDataDailyStepDualLegRunReportIssueSearchHit[],
): boolean {
...
/**
 * 실 평가 e2e daily-step dual-leg run report 이슈 검색 파서 산출 `hits` 의 **값**이 raw
 * `stdout` 으로부터 독립 재유도한 expected 배열과 deep-equal 정합함을 런타임에서 검증하는
 * 순수 가드(PLAN.md P5 109행 step ④ 표현 surface 무결성 조각 / REQ-059·REQ-032).
 * `parseRealDataDailyStepDualLegRunReportIssueSearchOutput`(T-0901)의 값-정합 가드 —
 * 컴포저가 number/title/body 값 drift·hit 누락/중복/재정렬·추가필드 누설을 silent 하게
 * 산출하면, 본 가드는 stdout 을 독립 재유도해 그 값 회귀를 fail-fast 로 잡는다. summary 축
 * T-0721 의 dual-leg 축 mirror.
 *
 ...
 *
 * @param hits 검증 대상 파서 산출
 *   (`parseRealDataDailyStepDualLegRunReportIssueSearchOutput` 결과). 변형하지 않는다
 *   (읽기·비교만).
 ...
 *   위반, 기대 vs 실측 포함).
 */
export function assertRealDataDailyStepDualLegRunReportIssueSearchOutputConsistentWithStdout(
  hits: RealDataDailyStepDualLegRunReportIssueSearchHit[],
  stdout: string,
): void {
