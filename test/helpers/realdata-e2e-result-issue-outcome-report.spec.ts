...
describe("buildRealDataResultIssueOutcomeReport — e2e 실행 리포트 컴포저", () => {
  ...
  describe("T-0726 — 값-정합 가드 producer self-wire(산출↔(outcome, run) deep-equal)", () => {
    ...
    // 순서-lock (outcome-report daily 정본 T-1036 ⑪ mirror — 요약축 result-issue leg) —
    // self-wire 두 가드의 **상대 호출 순서(SummaryLine→OutputConsistent)**를
    // invocationCallOrder 2 자 부등식으로 못박는다. 기존 ⑨ 는 두 가드가 각 1 회 호출됨만
    // 검증했고 상대 순서는 lock 하지 않았다 — SummaryLine 가드와 OutputConsistent 가드의
    // self-wire 순서가 실수로 뒤바뀌어도 통과했다. 본 test 가 그 gap 을 닫는다.
    it("⑩ self-wire 두 가드 호출 순서 보존(SummaryLine→OutputConsistent) — summaryLineSpy 가 outputConsistentSpy 보다 먼저 호출됨(역전 시 fail)", () => {
      ...
      // 두 가드 각 정확히 1 회 호출(daily 정본 T-1036 ⑪ mirror).
      ...
      // 실 구현 spy(호출 pass-through)이므로 산출 report byte-identical 회귀 0 재확인.
      expect(report).toEqual({
        issueNumber: 42,
        url: "https://github.com/acme/repo/issues/42",
        gitSha: "abc1234",
    ...
    // SummaryLine-first fail-fast (outcome-report daily 정본 T-1036 ⑫ mirror) — summary-line
    // 가드가 throw 하면 producer 가 전파하고 output-consistency 가드까지 도달하지 않음을
    // 못박아 SummaryLine→OutputConsistent 순서의 fail-fast 의미를 결정적이게 한다.
