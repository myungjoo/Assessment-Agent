// realdata-e2e-daily-step-dual-leg-run-report-issue-command-args-body-marker.ts —
// 실 평가 e2e daily-step dual-leg run report 이슈 멱등 명령-args 의 body marker-first
// 구조 불변식 검증 순수 가드(T-1008 박제, 요약축 T-0649 mirror).
//
// 책임:
...
//     descriptor.body 를 `createArgs.body` 와 `updateArgs.body` **양쪽 모두에 그대로
//     전달**하고, descriptor.marker 를 `searchQuery` 로 전달한다 — create 든 update 든
//     marker 라인이 양 경로에 보존돼야 search-or-update 멱등성이 성립한다. 그러나 이 멱등
//     정합 불변식 — ① createArgs.body 가 descriptor.body 와 byte-identical / ②
//     updateArgs.body 가 descriptor.body 와 byte-identical / ③ 두 body 의 첫 라인이
//     descriptor.marker(marker-first) / ④ searchQuery 가 descriptor.marker 와
//     byte-identical — 은 빌더 본문 주석과 T-0897 spec happy-path 단언으로만 박제돼 있고
//     **런타임에서 강제되는 독립 불변식 가드가 부재** 하다. 본 가드가 그 빈칸을 채운다.
//     부정합 명령-args 가 gh issue 실배선·rolling 이슈 surface 로 새기 전 fail-fast throw
//     로 차단한다.
//
// 🔥 descriptor-only(summary 재유도 0):
//   - `assertRealDataDailyStepDualLegRunReportIssueCommandArgsConsistent`(T-0990) 는
//     descriptor 필드로부터 expected 명령-args 를 독립 재유도해 필드별 byte-identical 대조
//     한다(full command-args-consistency). 본 가드는 그와 축이 다르다 — 명령-args body 가
//     descriptor.body/marker 를 marker-first 로 보존하는지의 body 축 불변식만 좁게 검증한다.
...
//     는 별도 follow-up slice(요약축 producer self-wire mirror).
//   - JSON schema / 외부 validation 라이브러리(zod·ajv) 도입 0 — 순수 string 비교만.
//   - `createArgs.labels` / `title` 자체 구조 검증 0 — 본 가드는 body marker-first 전파·
//     searchQuery 정합에 한정.
//
// 패턴 mirror: `realdata-e2e-result-issue-command-args-body-marker.ts`(T-0649, 요약축 —
// main 박제) 의 `assertRealDataResultIssueCommandArgsBodyPreservesDescriptor`. 불변식·에러
// 정책(구조 결손=TypeError·값 정합 위반=RangeError 구분)·메시지 포맷·순수성·descriptor-only
// (summary 미import) 를 daily-step 축으로 동형 이식. 인자·타입만 daily-step 축으로 치환한다.
...
/**
 * 실 평가 e2e daily-step dual-leg run report 이슈 멱등 명령-args 의 body marker-first
 * 구조 불변식을 런타임에서 검증하는 순수 가드(PLAN.md P5 109행 step ④ 결과 박제 chain 의
 * consumer-side 무결성 조각 / REQ-059·REQ-032). 요약축
 * `assertRealDataResultIssueCommandArgsBodyPreservesDescriptor`(T-0649)의 daily-step
 * mirror — command-args 는 summary 를 미보유하므로 descriptor.body/marker single-source
 * 만으로 검증한다(summary 재유도 0).
 *
 * 검증하는 불변식(single source — descriptor.body / descriptor.marker, 빌더 L150~163 의
 * body 전파 규칙 강제):
 ...
 *   불변식·기대값 vs 실측값을 포함.
 */
export function assertRealDataDailyStepDualLegRunReportIssueCommandArgsBodyPreservesDescriptor(
