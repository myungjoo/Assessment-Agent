// realdata-e2e-daily-step-dual-leg-run-report-issue-search-json-fields.ts —
// 실 평가 e2e daily-step dual-leg run report 이슈 search 의
// `gh search issues --json <fields>` 요청 필드 집합이 search-parse 가 추출하는
// `RealDataDailyStepDualLegRunReportIssueSearchHit` shape 키 집합과 정확히 일치하는지
// 검증하는 순수 가드(T-1012 박제, 요약축 T-0657 mirror).
//
// 책임:
//   - `buildRealDataDailyStepDualLegRunReportIssueSearchGhArgv`(T-0898 계열,
...
//     = "number,title,body"` 콤마 문자열을 `--json` 인자로 합성해 gh 에 요청할 필드
//     집합을 박제한다(search-argv.ts line 99~100).
//   - `parseRealDataDailyStepDualLegRunReportIssueSearchOutput`(T-0898 계열,
//     `realdata-e2e-daily-step-dual-leg-run-report-issue-search-parse.ts`)는 gh stdout
//     JSON 을 정규화해 `RealDataDailyStepDualLegRunReportIssueSearchHit = {number,
//     title, body}` 만 추출한다(search-parse.ts line 151~156, 추가 필드 drop).
...
//     ["number", "title", "body"] as const` 를 정규 키 목록의 단일 출처로 박제한다.
//     parser 가 추출하는 `{number, title, body}` 정규화 shape
//     (`realdata-e2e-daily-step-dual-leg-run-report-issue-search-parse.ts` line 151~156)
//     및 `RealDataDailyStepDualLegRunReportIssueSearchHit` interface 멤버
//     (`number: number; title: string; body: string`, T-0898) 와 동일 — type-level
//     정합은 cross-reference 주석으로 박제(런타임 의존 아님; 추출 shape 의 진실은 parser,
//     본 가드는 그 키 목록을 named constant 로 외화; parser 를 value import 하지 않음).
//
// 책임 경계(task Out of Scope — T-1012):
//   - `buildRealDataDailyStepDualLegRunReportIssueSearchGhArgv` self-wire(반환 직전 본
//     가드 호출 배선) — 본 task 는 신설·정합 봉인만, self-wire 는 Follow-up ① 의 별도
//     slice(요약축 T-0658 mirror, cap·단일책임).
//   - `parseRealDataDailyStepDualLegRunReportIssueSearchOutput` /
//     `buildRealDataDailyStepDualLegRunReportIssueSearchGhArgv` 본문 변경 0 — 상수·
//     shape 키만 재선언·읽기, 로직 수정 0.
//   - `RealDataDailyStepDualLegRunReportIssueSearchHit` interface 변경 / 신규 type 정의
//     0 — type-level cross-check 만(주석 수준).
...
//     search-hit-shape) 정합 가드 — 본 가드는 search-argv-요청 필드 ↔ search-parse-추출
//     shape 키 seam 한 곳만.
//   - 새 외부 dependency / Prisma migration / STATE schema 변경 — 0.
//   - production `src/` 코드 변경 — 0(test helper 단독). raw 활동 본문 저장(R-59) 0.
//
// 패턴 mirror: `realdata-e2e-result-issue-search-json-fields.ts`(T-0657 요약축 동형) —
// 순수 가드 / 구조 결손=TypeError·값 정합 위반=RangeError 구분 / single-source 비교 /
// 한국어 JSDoc·책임 경계 주석 / 자동 복구 0 / 산출 경로 자동 배선 0. 본 가드는 그 에러
// 정책·관례·JSDoc 톤을 mirror 하되 어휘만 daily-step 축으로 치환하며, "요청 필드 집합 ↔
// 추출 shape 키 집합" 의 set-equality 정합을 검증한다.

// 정규 search-parse 추출 shape 키 목록 — `parseRealDataDailyStepDualLegRunReport
// IssueSearchOutput`(T-0898) 가 정규화하는 `{number, title, body}` 의 키와
// `RealDataDailyStepDualLegRunReportIssueSearchHit`(T-0898) interface 멤버
// (`number: number; title: string; body: string`) 와 동일. 매직 배열 대신 named
// constant 로 박제 — 본 가드의 추출-측 single-source.
//   - cross-reference: realdata-e2e-daily-step-dual-leg-run-report-issue-search-parse.ts
//     line 151~156 (return `{number: record.number, title: record.title,
//     body: record.body}`)
// 본 상수의 순서·중복·집합 내용이 곧 추출 shape 의 진실의 원천.
export const REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_PARSE_SHAPE_KEYS =
  ["number", "title", "body"] as const;
...
/**
 * 실 평가 e2e daily-step dual-leg run report 이슈 search 의 `gh search issues --json
 * <fields>` 요청 필드 집합이 `parseRealDataDailyStepDualLegRunReportIssueSearchOutput`
 * 가 추출하는 `RealDataDailyStepDualLegRunReportIssueSearchHit` shape 키 집합과
 * 정확히 일치하는지 런타임에서 검증하는 순수 가드(PLAN.md P5 109행 step ④ 결과 박제
 * chain 의 search-json-fields ↔ search-parse-shape seam 무결성 조각, 요약축 T-0657
 * mirror).
 *
 ...
 */
export function assertRealDataDailyStepDualLegRunReportIssueSearchJsonFieldsMatchParseShape(
