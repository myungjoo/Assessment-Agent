...
// T-1077 구조-선행성 spy 용 namespace import — 가드의 유일 재유도 delegate
...
describe("assertRealDataDailyStepCollectCommandPlanConsistentWithGating", () => {
  ...
  // ── T-1077 구조-검사 선행성 order-lock (구조 결손 → gating 재유도 delegate 0-call) ──
  // Why: 가드 본문은 2 구조 assert(assertPlanStructure L162 / assertEnvStructure L163)를
  // gating 재유도 위임(resolveRealDataE2eLiveGating L174)보다 **먼저** 수행한다. 기존 구조
  // error-path 블록(L110~180)은 plan/env 구조 결손에 .toThrow(TypeError) 만 assert 하고
  // 재유도 delegate 호출 횟수를 못박지 않았다 — 구조 결손 입력 시 delegate 가
  ...
  // 재유도를 구조 검사 위로 끌어올림)로 깨지면 반드시 fail 하도록 못박는다(T-1066~T-1076
  // defense-in-depth 의 daily-step-collect command-plan mirror, 단일 delegate).
  //
  ...
  //     선행성 관점에서 명확화.
  describe("T-1077 — 구조-검사 선행성 order-lock(구조 결손 → 재유도 delegate 0-call)", () => {
    afterEach(() => {
    ...
    const STRUCTURE_DEFICIENT_CASES: Array<{
      label: string;
      makePlan: () => RealDataDailyStepCollectCommandPlan;
      makeEnv: () => NodeJS.ProcessEnv;
      messagePattern: RegExp;
    }> = [
      {
        label: "plan null",
        makePlan: () => null as unknown as RealDataDailyStepCollectCommandPlan,
      ...
      {
        label: "plan array",
        makePlan: () => [] as unknown as RealDataDailyStepCollectCommandPlan,
        makeEnv: () => makeEnabledEnv(),
    ...
    it("재유도-전 RangeError 대조(action enum 위반) — 구조 통과했으나 재유도 前 enum 검사에서 throw → delegate 0-call", () => {
      // action enum RangeError(L166)는 재유도 위임 **전** 이라 delegate 0-call. 값 경계
