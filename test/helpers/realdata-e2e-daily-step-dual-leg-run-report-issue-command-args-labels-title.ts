// realdata-e2e-daily-step-dual-leg-run-report-issue-command-args-labels-title.ts —
// 실 평가 e2e daily-step dual-leg run report 이슈 멱등 명령-args 의 labels·title 정합
// 불변식 검증 순수 가드(T-1010 박제, 요약축 T-0651 mirror).
//
// 책임:
//   - `buildRealDataDailyStepDualLegRunReportIssueCommandArgs`(T-0897,
//     `realdata-e2e-daily-step-dual-leg-run-report-issue-command-args.ts`)는
//     `RealDataDailyStepDualLegRunReportIssueDescriptor` 를 gh issue 멱등 search-or-update
//     명령-args(`{ searchQuery, createArgs, updateArgs }`)로 변환한다. 이 빌더는
//     descriptor.title 을 `createArgs.title` 과 `updateArgs.title` **양쪽 모두에 그대로
//     전달**하고, 고정 결정론 상수 집합(`DUAL_LEG_RUN_REPORT_ISSUE_LABELS =
//     ["realdata-e2e", "daily-step-dual-leg-run-report"]`)의 **새 배열 복제**를
//     `createArgs.labels` 로 전달한다. 그러나 이 두 정합 불변식 — ① createArgs.title /
//     updateArgs.title 이 둘 다 descriptor.title 과 byte-identical(create/update 어느
//     경로로 박제하든 동일 제목 → 멱등 식별) / ② createArgs.labels 가 고정 상수와
//     순서·원소·개수까지 정확히 일치하고 상수 자체 참조와 무공유(빌더가 복제) — 은 빌더
//     본문 주석과 T-0897 spec happy-path 단언으로만 박제돼 있고 **런타임에서 강제되는
//     독립 불변식 가드가 부재** 하다. 본 가드가 그 빈칸을 채운다. 제목이 갈라지거나 label
//     이 누락·추가·순서변경·무공유 위반된 명령-args 가 gh issue 실배선·rolling 이슈 surface
//     로 새기 전 fail-fast throw 로 차단한다.
//
// 🔥 body marker 가드(T-1008)와의 분담 — 동형 mirror, 비중복:
//   - `assertRealDataDailyStepDualLegRunReportIssueCommandArgsBodyPreservesDescriptor`
//     (T-1008) 는 body marker-first 구조와 searchQuery 정합만 검증한다(create/update body
//     byte-identical 전파 + marker-first + searchQuery). 본 가드는 그 가드가 닿지 않는
//     **labels·title-side mirror** — title 3자 정합·labels 고정-상수 정합·labels 무공유 만
//     검증한다. 두 가드는 command-args 구조 무결성의 두 축(body 축 / labels·title 축)으로
//     비중복 분담한다. full command-args 재유도 대조는 T-0990 consistency 가드가 cover —
//     본 가드는 그와 축이 다르다(labels·title 좁은 축, single-source 비교).
//
...
// 명령-args 면 항상 void 반환, 부정합 명령-args 면 항상 동일 위치 throw). 새 외부 dependency
// 0, DB write·migration 0, live LLM 호출 0. 같은 디렉토리 타입 import 라 runtime cycle 0.
//
// 책임 경계(task T-1010 Out of Scope):
//   - `buildRealDataDailyStepDualLegRunReportIssueCommandArgs` /
//     `buildRealDataDailyStepDualLegRunReportIssueDescriptor` 본문·출력 타입 변경 0(타입만
//     `import type` 소비, 재정의 0). 본 가드는 import·비교·throw 만.
//   - 자동 복구 / args 재합성 / 정규화 / 기본값 채움 / label 자동 보정 / title 자동 교정 0 —
//     부정합 명령-args 를 고치거나 silent 수선하지 않는다(fail-fast). 복구는 호출처 책임.
//   - 산출 경로 자동 배선(빌더 산출 직전 self-wire) 0 — 순수 가드 helper 까지. self-wire
//     는 별도 follow-up slice(요약축 T-0652 mirror — 기존 body marker 가드 self-wire 옆에).
//   - JSON schema / 외부 validation 라이브러리(zod·ajv) 도입 0 — 순수 string 비교만.
//   - body marker-first / searchQuery 정합 검증 0 — 그 축은 body marker 가드(T-1008)가 cover.
//
// 패턴 mirror: `realdata-e2e-result-issue-command-args-labels-title.ts`(T-0651, 요약축 —
// main 박제) 의 `assertRealDataResultIssueCommandArgsLabelsTitleConsistent`. 불변식 (1)~(4)·
// 에러 정책(구조 결손=TypeError·값 정합 위반=RangeError 구분)·메시지 포맷·JSDoc 톤·자동 복구
// 0·산출 경로 자동 배선 0 을 daily-step 축으로 동형 이식한다. 인자·타입만 daily-step 축으로
// 치환하고 불변식·에러 정책·single-source 비교 규약은 그대로 옮긴다.
...
function assertCommandArgsStructure(
  args: RealDataDailyStepDualLegRunReportIssueCommandArgs | null | undefined,
): asserts args is RealDataDailyStepDualLegRunReportIssueCommandArgs {
  if (args === null || args === undefined) {
    throw new TypeError(
      "args 가 null/undefined 일 수 없다 — RealDataDailyStepDualLegRunReportIssueCommandArgs 객체가 필요하다.",
    );
  }
...
function assertDescriptorStructure(
  ...
  if (descriptor === null || descriptor === undefined) {
    throw new TypeError(
      "descriptor 가 null/undefined 일 수 없다 — RealDataDailyStepDualLegRunReportIssueDescriptor 객체가 필요하다.",
    );
  }
...
/**
 * 실 평가 e2e daily-step dual-leg run report 이슈 멱등 명령-args 의 labels·title 정합
 * 불변식을 런타임에서 검증하는 순수 가드(PLAN.md P5 109행 step ④ 결과 박제 chain 의
 * consumer-side 무결성 조각 / REQ-059·REQ-032). body marker 가드
 * `assertRealDataDailyStepDualLegRunReportIssueCommandArgsBodyPreservesDescriptor`(T-1008)
 * 의 labels·title-side mirror — 그 가드가 닿지 않는 title 3자 정합·labels 고정-상수 정합·
 * labels 무공유 만 검증한다(body marker-first / searchQuery 축은 그 가드가 이미 cover,
 * 본 가드는 비중복). 요약축 `assertRealDataResultIssueCommandArgsLabelsTitleConsistent`
 * (T-0651)의 daily-step mirror.
 *
 * 검증하는 불변식(single source — descriptor.title / expectedLabels, 빌더 L155~161 의
 ...
 *
 * @param args 검증 대상 dual-leg run report 이슈 멱등 명령-args. 변형하지 않는다(읽기·비교만).
 ...
 *   `DUAL_LEG_RUN_REPORT_ISSUE_LABELS`). 변형하지 않는다(읽기·비교만). 무공유 검증의 비교
 *   대상이기도 하므로 createArgs.labels 와 동일 참조면 (4) 위반.
 ...
 *   불변식·기대값 vs 실측값을 포함.
 */
export function assertRealDataDailyStepDualLegRunReportIssueCommandArgsLabelsTitleConsistent(
  args: RealDataDailyStepDualLegRunReportIssueCommandArgs,
  ...
  if (actualLabels.length !== expectedLabels.length) {
    throw new RangeError(
      `불변식(3) 위반: createArgs.labels 의 개수가 expectedLabels 와 다르다 — 기대 ${expectedLabels.length}개([${expectedLabels.join(", ")}]), 실측 ${actualLabels.length}개([${actualLabels.join(", ")}]). label 이 누락·추가됐다(rolling 이슈 분류·검색 필터 손상 위험).`,
    );
  }
  for (let i = 0; i < expectedLabels.length; i += 1) {
