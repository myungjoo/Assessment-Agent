// realdata-e2e-daily-step-dual-leg-run-report-issue-gh-command-plan-consistency.ts —
// 실 평가 e2e **step④ 결과 박제 daily-step dual-leg run report 종단 컴포저** 산출 ↔
// 입력 (stdout, commandArgs) single-source 재유도 정합 순수 가드 (T-0994 박제, 요약축
// 선례 T-0695 mirror).
//
// 책임:
//   - `resolveRealDataDailyStepDualLegRunReportIssueGhCommandPlan(stdout, commandArgs)`
//     (T-0902, `realdata-e2e-daily-step-dual-leg-run-report-issue-gh-command-plan.ts`)는
//     (1) `parseRealDataDailyStepDualLegRunReportIssueSearchOutput(stdout)` → (2)
//     `resolveRealDataDailyStepDualLegRunReportIssueAction(hits, searchQuery)` → (3)
//     `buildRealDataDailyStepDualLegRunReportIssueGhArgv(action, commandArgs)` 3-단계를
//     합성해 `{action, argv}`(`RealDataDailyStepDualLegRunReportIssueGhCommandPlan`) plan
//     을 산출하는 **step④ 결과 박제 daily-step dual-leg 종단 컴포저**다. issue-gh-argv
//     sub-helper 는 producer(T-0899)→consistency(T-0992)→self-wire(T-0993) 삼단이 완결됐고
//     command-args sub-helper 도 T-0897/T-0990/T-0991 짝이 완결됐으나, 이 **종단 컴포저**
//     에는 sibling 관례인 `-consistency` drift-guard 짝이 부재했다. 합성 회귀 — action 분기
//     오매핑(create/update 뒤바뀜)·argv 동사 drift(`issue create`↔`issue edit`)·title/body
//     위치 drift·label flag-pair 길이/순서 어긋남·update issueNumber drift·argv 잉여/누락
//     원소·hits 재해석 drift·marker(=searchQuery) 재합성 — 을 build-time 에 잡을 장치가
...
//   - 입력 `(stdout, commandArgs)` 로 동일 3 위임 helper(`parseRealDataDailyStepDualLeg-
//     RunReportIssueSearchOutput` → `resolveRealDataDailyStepDualLegRunReportIssueAction`
//     → `buildRealDataDailyStepDualLegRunReportIssueGhArgv`)를 재호출해 expected
//     `{action, argv}` 를 single-source 재유도한다(합성 규칙 재구현 0 — 위임만).
...
//
// 에러 정책(구조 결손 = TypeError / 값 정합 위반 = RangeError — 요약축 T-0695 mirror):
...
//     는 별도 후속 slice(T-0993 mirror).
//   - 위임 helper(parse T-0901 / resolveAction T-0898 / buildGhArgv T-0899) 수정 — 본
//     가드는 호출(재유도)만. 각 helper 의 합성 규칙·시그니처·throw 정책 불변.
//   - argv-leaf 가드(`-issue-gh-argv-consistency.ts`, T-0992)가 이미 cover 하는
//     argv↔commandArgs round-trip 의 내부 위치 정합 세부(C0~C3/U0~U4 단위 검증) — 본 가드는
...
//     컨텍스트는 caller 책임).
//
// oracle 독립성: 본 가드는 컴포저 producer(T-0902)를 **import 하지 않는다**(재유도 금지 —
// 양방향 drift 상쇄 방지). plan/action/command-args 타입만 `import type` 로 참조하므로
// 이 가드가 나중에 producer 에 value import 로 배선돼도(self-wire, T-0993 mirror) 런타임
// 순환 의존이 생기지 않는다(consistency → gh-command-plan value 엣지 0).
...
import type { RealDataDailyStepDualLegRunReportIssueGhCommandPlan } from "./realdata-e2e-daily-step-dual-leg-run-report-issue-gh-command-plan";
...
// 것을 분리 노출(디버깅 가독성). 요약축 T-0695 와 동형 helper.
...
function assertPlanStructure(
  plan: RealDataDailyStepDualLegRunReportIssueGhCommandPlan | null | undefined,
): asserts plan is RealDataDailyStepDualLegRunReportIssueGhCommandPlan {
  if (
    plan === null ||
    plan === undefined ||
    typeof plan !== "object" ||
...
// 구조 결손 분류(컴포저 type RealDataDailyStepDualLegRunReportIssueAction 의
// discriminated union 위반).
function assertPlanActionEnum(
  action: RealDataDailyStepDualLegRunReportIssueAction | null | undefined,
): asserts action is RealDataDailyStepDualLegRunReportIssueAction {
  if (action === null || action === undefined || typeof action !== "object") {
    throw new TypeError(
      `plan.action 이 객체가 아니다(타입: ${describe(action)}) — RealDataDailyStepDualLegRunReportIssueAction discriminated union 이 필요하다.`,
    );
  }
...
/**
 * 실 평가 e2e **step④ 결과 박제 daily-step dual-leg run report 종단 컴포저**
 * (`resolveRealDataDailyStepDualLegRunReportIssueGhCommandPlan`) 산출 plan 이, 주입된
 * 입력 `(stdout, commandArgs)` 로 동일 3 위임 helper(parse → resolveAction → buildGhArgv)
 * 를 재호출해 single-source 재유도한 expected plan 과 정합함을 런타임에서 검증하는 순수
 * 가드(PLAN.md P5 109행 step ④ build-time chain 의 결과 박제 종단 seam 무결성 조각).
 * 요약축 선례 `assertRealDataResultIssueGhCommandPlanConsistentWithInputs`(T-0695)의
 * daily-step-side mirror.
 *
 ...
 * 먼저 어긋난 지점에서 throw.
 *
 * @param plan 종단 컴포저 산출 `RealDataDailyStepDualLegRunReportIssueGhCommandPlan`.
 *   변형하지 않는다.
 * @param stdout 재유도 source — `parseRealDataDailyStepDualLegRunReportIssueSearchOutput
 *   (stdout)` 재호출에 사용. 변형 불가(문자열).
 * @param commandArgs 재유도 source — `resolveRealDataDailyStepDualLegRunReportIssueAction`
 *   / `buildRealDataDailyStepDualLegRunReportIssueGhArgv` 재호출에 사용. 변형하지 않는다
 *   (읽기·전달만).
 * @returns 정합이면 void.
 ...
 */
export function assertRealDataDailyStepDualLegRunReportIssueGhCommandPlanConsistentWithInputs(
  ...
  const expectedAction = resolveRealDataDailyStepDualLegRunReportIssueAction(
    expectedHits,
    commandArgs.searchQuery,
  );
  const expectedArgv = buildRealDataDailyStepDualLegRunReportIssueGhArgv(
    expectedAction,
    commandArgs,
  );
  ...
  if (
    ...
    throw new RangeError(
      `정합 위반: plan.action.issueNumber 가 재유도 최소 number 와 다르다 — 기대=${expectedAction.issueNumber}, 실측=${plan.action.issueNumber}. resolveRealDataDailyStepDualLegRunReportIssueAction 의 최소 number 멱등 회귀 보호와 어긋났다.`,
    );
  }
