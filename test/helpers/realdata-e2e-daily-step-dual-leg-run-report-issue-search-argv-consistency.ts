// realdata-e2e-daily-step-dual-leg-run-report-issue-search-argv-consistency.ts —
// 실 평가 e2e daily-step dual-leg run report 이슈 search 인자-벡터(argv)가 입력
// 명령-args 의 searchQuery 를 argv 위치로 정합 round-trip 했고 고정 인자(`--match body` /
// `--json` 필드 / `--limit` 값) shape 를 유지했는지 검증하는 순수 가드(T-0998 박제).
//
// 책임:
//   - `buildRealDataDailyStepDualLegRunReportIssueSearchGhArgv`(T-0900,
//     `realdata-e2e-daily-step-dual-leg-run-report-issue-search-argv.ts`)는
//     `RealDataDailyStepDualLegRunReportIssueCommandArgs` 의 searchQuery 를 실 `gh search
//     issues --json number,title,body` 호출에 그대로 넘길 인자-벡터(argv, `string[]`)로
//     합성한다. 산출 argv 는
//     `["search", "issues", "--match", "body", searchQuery, "--json",
//      REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_JSON_FIELDS, "--limit",
//      REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_LIMIT]`(길이 9) 이다. 이
//     빌더는 `assertSearchQueryNonBlank` inline guard 만 보유하고, **산출 argv 가
//     commandArgs.searchQuery 를 argv 의 올바른 위치(index 4)로 정합 전파했는지, 고정
//     인자(`--match body` 위치·`--json` 필드 문자열·`--limit` 값·동사 prefix)가 drift 하지
//     않았는지 검증하는 독립 불변식 가드는 부재** 하다. 즉 빌더가 회귀(예: searchQuery 값이
//     다른 위치로 새거나, `--match body` 위치가 어긋나거나, `--json` 필드가
//     `RealDataDailyStepDualLegRunReportIssueSearchHit` 멤버와 어긋나거나, `--limit` 상수가
//     drift 하거나, `["search","issues"]` 동사 prefix 가 빠지면) 손상 argv 가
//     `execFile('gh', searchArgv)` live wiring 으로 새어 잘못된 gh 검색이 실행되고 분기 결정
//     (T-0898 resolver)이 오염된다. 본 가드가 그 빈칸을 채운다.
//
// 🔥 command-args single-source(search-argv-side mirror):
//   - 본 가드는 요약축 선례 `assertRealDataResultIssueSearchGhArgvPreservesCommandArgs`
//     (T-0655, result 축 search argv 가드)의 daily-step 축 mirror 다. 그 가드는 build-time
//     chain 의 search argv 끝이 명령-args 의 searchQuery 를 보존하는지 검증했고, 본 가드는
//     daily-step dual-leg 축의 search argv 끝이 동일 불변식을 만족하는지 검증한다.
//     commandArgs 를 single-source 로 삼아 argv 의 searchQuery 위치 정합 + 고정 인자 shape 만
//     비교한다(descriptor·resolver 재유도 0 — upstream 가드가 cover).
//
...
//   - `buildRealDataDailyStepDualLegRunReportIssueSearchGhArgv` 본문·출력 타입 변경 0
//     (타입·상수만 import 소비, 재정의 0). 본 가드는 import·비교·throw 만.
//   - 자동 복구 / argv 재합성 / 정규화 / 기본값 채움 0 — 부정합 argv 를 고치거나 silent
//     수선하지 않는다(fail-fast). 복구는 호출처 책임.
//   - 산출 경로 자동 배선(`buildRealDataDailyStepDualLegRunReportIssueSearchGhArgv` 산출
//     직전 self-wire) 0 — 순수 가드 helper 까지. self-wire 는 별도 follow-up slice(T-0654 가
//     result 축 create/edit 가드를 self-wire 한 것의 daily-step search-side mirror).
...
//   - **빌더 producer(T-0900) 함수 재호출 0** — commandArgs.searchQuery + 두 named
//     constant 만 single-source 로 비교(argv 합성 규칙을 재구현하면 양방향 drift 가 상쇄돼
//     가드가 무력화되므로 재구현하지 않는다).
//
// 패턴 mirror: `realdata-e2e-result-issue-search-argv-consistency.ts`(T-0655, 순수 함수 /
// 구조 결손=TypeError·값 정합 위반=RangeError 구분 / single-source 비교 / 한국어 JSDoc·
// 책임 경계 주석 / 자동 복구 0 / 산출 경로 자동 배선 0). 본 가드는 그 에러 정책·가드 관례·
// JSDoc 톤을 mirror 하되, result 축이 아닌 daily-step dual-leg 축 search argv round-trip
// 정합을 검증한다.
...
function assertCommandArgsStructure(
  ...
  if (commandArgs === null || commandArgs === undefined) {
    throw new TypeError(
      "commandArgs 가 null/undefined 일 수 없다 — RealDataDailyStepDualLegRunReportIssueCommandArgs 객체가 필요하다.",
    );
  }
...
// assertSearchQueryNonBlank — 입력 searchQuery 가 빈/공백-only 면 거부(T-0900 빌더의
...
function assertSearchQueryNonBlank(searchQuery: string): void {
  if (searchQuery.trim().length === 0) {
    throw new RangeError(
      "commandArgs.searchQuery 가 비어있다 — 빈/공백-only searchQuery 는 전체 매칭 사고를 유발하므로 허용되지 않는다(T-0900 동형 거부).",
    );
  }
}

/**
 * 실 평가 e2e daily-step dual-leg run report 이슈 search 인자-벡터(argv)가 입력 명령-args 의
 * searchQuery 를 argv 위치로 정합 round-trip 했고 고정 인자 shape 를 유지했는지 런타임에서
 * 검증하는 순수 가드(PLAN.md P5 109행 step ④ 결과 박제 chain 의 search-argv-layer 무결성 조각).
 * 요약축 선례 `assertRealDataResultIssueSearchGhArgvPreservesCommandArgs`(T-0655)의 daily-step
 * dual-leg 축 mirror — build-time chain 의 search argv 끝이 commandArgs.searchQuery 를 보존하는지
 * 검증한다.
 *
 * 검증하는 불변식(single source — commandArgs + T-0900 named constant, 빌더의 search argv
 * 합성 규칙 강제):
 ...
 *   (S3) argv[5]==='--json' 이고
 *        argv[6]===REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_JSON_FIELDS
 *        (단일-source 상수 정합; `RealDataDailyStepDualLegRunReportIssueSearchHit` 멤버와의
 *        cross-check 는 빌더가 보유, 가드는 그 상수와의 일치만 비교).
 *   (S4) argv[7]==='--limit' 이고
 *        argv[8]===REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_LIMIT
 *        (단일-source 상수 정합).
 ...
 */
export function assertRealDataDailyStepDualLegRunReportIssueSearchGhArgvPreservesCommandArgs(
  argv: string[],
  ...
  // 입력 의미 검증 — 빈/공백 searchQuery 거부(T-0900 동형). 구조는 온전하나 의미 위반이라
  // RangeError 로 구분한다.
  ...
  if (
    ...
    throw new RangeError(
      `불변식(S3) 위반: argv[6](--json 필드)가 REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_JSON_FIELDS 와 byte-identical 하지 않다 — 기대='${REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_JSON_FIELDS}', 실측='${String(
        argv[6],
  ...
  if (argv[8] !== REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_LIMIT) {
    throw new RangeError(
      `불변식(S4) 위반: argv[8](--limit 값)이 REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_LIMIT 와 byte-identical 하지 않다 — 기대='${REAL_DATA_DAILY_STEP_DUAL_LEG_RUN_REPORT_ISSUE_SEARCH_LIMIT}', 실측='${String(
        argv[8],
      )}'. --limit 상수가 drift 됐다.`,
    );
  }
