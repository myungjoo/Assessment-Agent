// realdata-e2e-daily-step-dual-leg-run-report-issue-output-parse.ts — 실 평가 e2e
// daily-step dual-leg run report 이슈 `gh issue create` / `gh issue edit <n>` 의
// stdout(이슈 URL 을 포함한 줄) → `RealDataDailyStepDualLegRunReportIssueOutcome`
// 순수 파서·검증기 (T-0903 박제).
//
// 책임:
//   - build-time chain 의 **실행-후 해석(post-execution interpretation)** 측 누락 link
//     를 채운다. dual-leg run report 축의 chain 은 이미 (1) 컴포저(T-0894) → …
//     → (6) gh create/edit argv 빌더(T-0899) → (7) gh search argv 빌더(T-0900) →
//     (8) search stdout 파서(T-0901) → (9) gh command plan 종단 컴포저(T-0902)까지
//     순수 함수로 닫혔다. 그러나 caller(live wiring)가 `execFile('gh', argv)` 로
//     이슈를 실 박제한 뒤 **그 stdout(생성/수정된 이슈 URL) 을 `{issueNumber, url}`
//     로 파싱·검증하는 단계** 가 빠져있었다. 본 helper 가 그 (실행 후) 단계를 순수
//     함수로 박제한다.
//   - 본 파서는 T-0901 `parseRealDataDailyStepDualLegRunReportIssueSearchOutput`
//     (검색 응답 파싱, **실행 전** 측)의 정확한 대칭 — **실행 후** 측 stdout 파싱이다.
//     이 slice 가 박제되면 dual-leg build-time chain 이 입력(search)부터 출력(create/
//     edit 결과 확인)까지 round-trip 으로 닫힌다. summary 축의 선례 T-0589
//     (`parseRealDataResultIssueCreateEditOutput`)의 dual-leg 축 mirror 다.
//
// 🔥 엄격 검증 (조용한 통과 금지):
...
//   - issueNumber 규약은 T-0898 `assertPositiveNumber`(양의 정수)와 동형 — 해당
//     함수는 비-export private 라 본 helper 는 자체 inline 검증(정규 십진 양의 정수
//     매칭)으로 동형 규약을 재현한다(복제 최소·중복 정의 금지 원칙).
//
// 🔥 다중 줄 결정론 (첫 매칭 URL):
...
//     출력 `RealDataDailyStepDualLegRunReportIssueOutcome` 은 `{issueNumber, url}` 만
//     담는다. 실 `execFile('gh', ...)` 은 deferred — 본 파서는 이미 받은 stdout →
//     outcome 산출까지만.
//
// 🔥 결정론·무공유:
...
//     gh 실행 0. 외부 라이브러리(zod/execa 등) 0 — 내장 정규표현식 + 수동 검증만.
//     순수 함수.
//
// Out of Scope (task T-0903):
...
//   - argv 합성(T-0899 위임) · search argv 합성(T-0900 위임) · search 응답 파싱(T-0901
//     위임) · 종단 command-plan 합성(T-0902 위임) — 본 helper 는 create/edit stdout
//     파싱 단일 책임.
//   - parse-shape / outcome-consistency 값-정합 가드(summary 축 T-0661/T-0662/T-0723/
//     T-0724 후속 slice) — 본 task 는 base 파서만.
...
// self-wire(T-0907) — 파서가 정규화 outcome 을 반환하기 직전에, 산출 outcome 전체가 raw
// stdout 으로부터 올바른 값(issueNumber 값·url trim 정규화·첫 매칭 URL 결정론)으로 재유도
// 됐는지 self-assert 하기 위해 T-0906 신설 값-정합 가드를 값 import 한다. 가드 모듈은
// `RealDataDailyStepDualLegRunReportIssueOutcome` 를 type-only 로만 import 하므로(value
// import 0) 본 파서가 가드를 top-level value import 해도 순환 의존이 생기지 않는다(tsc green).
// summary 축 T-0724 self-wire 의 dual-leg mirror.
...
export interface RealDataDailyStepDualLegRunReportIssueOutcome {
  issueNumber: number;
  url: string;
}
...
// T-0898 `assertPositiveNumber` 규약과 동형(양의 정수). 정규 십진 양의 정수(`[1-9]\d*`)
// 만 허용하므로 통과 시 `Number(raw)` 는 항상 양의 정수 — 별도 재검증 불요.
...
// parseRealDataDailyStepDualLegRunReportIssueCreateEditOutput — `gh issue create` /
// `gh issue edit <n>` 의 stdout(이슈 URL 을 포함한 문자열)을 입력받아
// `RealDataDailyStepDualLegRunReportIssueOutcome` 로 파싱·검증하는 **순수 함수**.
//
// 동작:
...
//     URL 을 사용해 결정론적.
export function parseRealDataDailyStepDualLegRunReportIssueCreateEditOutput(
  stdout: string,
  ...
  // self-wire(T-0907) — 산출 outcome 전체가 raw stdout 으로부터 올바른 값(issueNumber·url·
