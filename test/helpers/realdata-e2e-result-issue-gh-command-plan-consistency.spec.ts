...
// T-1063 순서-lock leg 10 — guard 재유도가 호출하는 3 delegate 를 namespace 로 잡아
// jest.spyOn 배선(정합-경로 상대 호출 순서 invocationCallOrder 부등식 2개 + 데이터-의존
// reference 페어링 2개 계측). 세 프로퍼티에 pass-through spy 를 건다.
...
describe("assertRealDataResultIssueGhCommandPlanConsistentWithInputs", () => {
  // T-1063 spy 격리 — 순서-lock describe 의 jest.spyOn 이 후속 test 로 새지 않도록
  ...
  // T-1063 — consistency-guard 재유도 위임 순서-lock leg 10(result-issue gh-command-plan
  // 3-stage, T-1060 daily-leg gh-command-plan 3-stage 의 result-issue mirror). guard
  // (`assert…GhCommandPlanConsistentWithInputs`)는 3 distinct builder 를 데이터-의존 chain
  // (2 order-edge)으로 재유도한다: parse(builder ①, stdout → hits) → resolveAction(builder ②,
  // hits + searchQuery → action) → buildGhArgv(builder ③, action + commandArgs → argv).
  // builder ②가 builder ① 산출 hits 를, builder ③이 builder ② 산출 action 을 각 첫 인자로
  // 소비하므로 반드시 parse → resolveAction → buildGhArgv 순으로 완료돼야 한다. 현행 spec 은
  // 세 재유도를 실 입력 throw 로만 검증(spec invocationCallOrder=0)해, guard 본문에서 세 재유도를
  // 재정렬하거나 어느 builder 인자를 앞 산출이 아닌 값으로 바꿔도(deep-equal 은 순서-무관) 검출
  // 못 한다. T-1060 의 3-builder 순서-lock 선례를 result-issue 식별자로(edge 2개·reference-페어링
  // 2개) 옮겨 그 gap 을 봉한다.
  //
  ...
  //   - error/negative(a fail-fast edge): parse 위임 강제 throw → resolveAction·buildGhArgv
  //     미도달(각 0회) — 첫 stage throw 가 뒤 두 stage 도달 전에 선전파.
  //   - error/negative(b 종단 throw 순서 재확인): buildGhArgv 위임 강제 throw → parse·
  //     resolveAction 은 이미 각 1회(순서 상 두 앞 stage 가 buildGhArgv 보다 먼저 평가됨을
  //     negative 경로에서도 재확인).
  describe("T-1063 — 재유도 위임 순서-lock(parse → resolveAction → buildGhArgv)", () => {
    ...
    it("(branch/무공유 재확인) pass-through spy 하에서도 guard 가 void 반환 + plan/stdout/commandArgs mutate 0", () => {
      ...
      jest.spyOn(searchParseModule, "parseRealDataResultIssueSearchOutput");
      jest.spyOn(actionModule, "resolveRealDataResultIssueAction");
      jest.spyOn(ghArgvModule, "buildRealDataResultIssueGhArgv");
    ...
    it("(a fail-fast edge) parse 위임이 throw 하면 resolveAction·buildGhArgv 에 도달하지 못한다(각 0회)", () => {
      const stdout = "[]";
      const commandArgs = makeCommandArgs();
      const plan = buildConsistent(stdout, commandArgs);
      jest
        .spyOn(searchParseModule, "parseRealDataResultIssueSearchOutput")
        .mockImplementation(() => {
    ...
    it("(b 종단 throw 순서 재확인) buildGhArgv 위임이 throw 하면 parse·resolveAction 은 이미 각 1회 호출됨", () => {
      const stdout = "[]";
      const commandArgs = makeCommandArgs();
      const plan = buildConsistent(stdout, commandArgs);
      const parseSpy = jest.spyOn(
      ...
      const ghArgvSpy = jest
        .spyOn(ghArgvModule, "buildRealDataResultIssueGhArgv")
        .mockImplementation(() => {
