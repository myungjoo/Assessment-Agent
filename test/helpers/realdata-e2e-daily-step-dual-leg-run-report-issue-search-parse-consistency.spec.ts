// realdata-e2e-daily-step-dual-leg-run-report-issue-search-parse-consistency.spec.ts —
// T-0908 colocated unit spec for
...
// mutate 0). 컴포저 `parseRealDataDailyStepDualLegRunReportIssueSearchOutput` 로 정상
// 산출을 만들되, 손상 fixture 는 산출 또는 stdout 한쪽만 변조해 만든다. summary 축
// T-0721 spec 의 dual-leg 축 mirror.
...
describe("assertRealDataDailyStepDualLegRunReportIssueSearchOutputConsistentWithStdout", () => {
  describe("happy-path (정합 산출↔stdout → void)", () => {
    it("1 hit — 컴포저 산출을 그대로 넘기면 throw 0(void)", () => {
      const hits =
        parseRealDataDailyStepDualLegRunReportIssueSearchOutput(ONE_HIT_STDOUT);
      expect(() =>
        assertRealDataDailyStepDualLegRunReportIssueSearchOutputConsistentWithStdout(
          hits,
          ONE_HIT_STDOUT,
    ...
    it("정합 쌍이면 void(undefined) 를 반환한다", () => {
      const hits =
        parseRealDataDailyStepDualLegRunReportIssueSearchOutput(TWO_HIT_STDOUT);
      expect(
        assertRealDataDailyStepDualLegRunReportIssueSearchOutputConsistentWithStdout(
          hits,
          TWO_HIT_STDOUT,
    ...
    it("gh 미래 추가 필드가 섞인 원소도 {number,title,body} drop 정합(void)", () => {
      // stdout 원소에 url/updatedAt 추가 필드가 있어도 컴포저·재유도 둘 다 3 키로 drop
      // 하므로 정합. 추가필드 격리(R-59) 정합 확인.
  ...
  describe("구조 결손 — hits 측 → TypeError (negative: hits 비배열/원소 비객체)", () => {
    ...
    it("hits 원소 숫자 → TypeError", () => {
      expect(() =>
        assertRealDataDailyStepDualLegRunReportIssueSearchOutputConsistentWithStdout(
          [7] as unknown as RealDataDailyStepDualLegRunReportIssueSearchHit[],
          ONE_HIT_STDOUT,
        ),
      ).toThrow(TypeError);
    });
  ...
  describe("구조 결손 — stdout 측 → TypeError (negative: stdout 비-string/비-JSON/비배열/원소 결손)", () => {
    ...
    it("stdout JSON 원소 number 비숫자(문자열) → TypeError", () => {
      const hits =
        parseRealDataDailyStepDualLegRunReportIssueSearchOutput(ONE_HIT_STDOUT);
      const stdout = JSON.stringify([{ number: "12", title: "t", body: "b" }]);
      expect(() =>
        assertRealDataDailyStepDualLegRunReportIssueSearchOutputConsistentWithStdout(
          hits,
          stdout,
    ...
    it("stdout JSON 원소 number 비정수(소수) → TypeError", () => {
      const hits =
        parseRealDataDailyStepDualLegRunReportIssueSearchOutput(ONE_HIT_STDOUT);
  ...
  describe("§9 / REQ-059 정합 — raw 활동 본문·credential 미노출", () => {
