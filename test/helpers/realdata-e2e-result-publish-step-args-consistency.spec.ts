...
//
// R-112 cover 구조:
//   - happy-path: 정상 (runPlan, results) 으로 컴포저(`buildRealDataResultPublishStepArgs`)
...
//     runPlan.run 비-object / report·commandArgs 비-object / searchArgv 비-배열·원소
//     비-string → 각 분기 별 TypeError(필드별·결손별 분기마다).
...
// pipeline fixture — 본 가드는 runPlan.run 만 본다(위임도 마찬가지). pipeline 필드
// 는 type 충족 위해 유효 슬롯 1 건만 채운다.
...
// runPlan fixture 생성기 — 주어진 run 을 담은 유효 RealDataE2eRunPlan.
...
describe("assertRealDataResultPublishStepArgsConsistentWithSources", () => {
  describe("happy-path (정합 plan → void)", () => {
    ...
    it("다른 유효 run 식별자 조합도 round-trip 정합(void)", () => {
      ...
      const runPlan = makeRunPlan(run);
      const plan = makePlan(runPlan, HAPPY_RESULTS);
      expect(() =>
        assertRealDataResultPublishStepArgsConsistentWithSources(
          plan,
          runPlan,
  ...
  describe("값 정합 위반 — 구성요소 drift → RangeError (negative (a)~(c))", () => {
    ...
    it("commandArgs drift(searchQuery 변형) → RangeError(commandArgs 노출)", () => {
      const runPlan = makeRunPlan();
      const plan = makePlan(runPlan);
      const corrupted: RealDataResultIssuePublishPlan = {
        ...plan,
    ...
    it("searchArgv drift(위치 swap) → RangeError(searchArgv 노출)", () => {
      const runPlan = makeRunPlan();
      const plan = makePlan(runPlan);
  ...
  describe("구조 결손 — null/undefined → TypeError (negative (b))", () => {
    ...
    it("runPlan.run 비-object(배열) → TypeError", () => {
      const corruptedRunPlan = {
        pipeline: makePipeline(),
        run: [],
      } as unknown as RealDataE2eRunPlan;
  ...
  describe("재유도 chain throw 전파 — 가드가 삼키지 않음 (branch cover)", () => {
    it("runPlan.run.gitSha 공백-only → 재유도 하위 guard throw 가 전파", () => {
      // 정상 plan 을 만든 후, 가드 인자로 공백 gitSha 를 가진 runPlan 을 넣어 재유도
      // 단계에서 위임 publish-plan 의 하위 report-plan guard 가 throw 하게 한다.
      const happyPlan = makePlan();
    ...
    it("runPlan.run.dateToken 빈 문자열 → 재유도 하위 guard throw 가 전파", () => {
      const happyPlan = makePlan();
      const blankRunPlan: RealDataE2eRunPlan = {
        pipeline: makePipeline(),
