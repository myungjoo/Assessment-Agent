// realdata-e2e-daily-test-step-eval-shell-argv-helper-plan-full-vector-parity-drift.smoke-spec.ts
// — 실 평가 e2e step④ `deploy/daily-test.sh` `step_eval` 의 하드코딩 jest argv ↔ 그 정본
// (canonical source) TS helper `buildRealDataDailyStepEvalCommandPlan(env).argv`(run 분기)
// full-ordered-vector cross-artifact(bash↔TS) drift-detection non-gated build-time smoke
// (T-0790 박제, PLAN.md 109행 🟢 실 평가 e2e step④).
//
// 본 spec 의 존재 이유 — cross-artifact parity gap 해소:
//   - PLAN 109행 step④ 는 "`deploy/daily-test.sh` 에 `step_eval` 추가 ... jest argv 는
//     `buildRealDataDailyStepEvalCommandPlan` 의 run 분기 산출을 mirror(정본은 그 helper)"
//     를 명시한다. 즉 nightly 자율 실 평가 e2e 가 어떤 jest 명령으로 live smoke 를 spawn
//     하는가는 TS helper `buildRealDataDailyStepEvalCommandPlan(env).argv`(run 분기) 가
//     **정본(canonical source)** 이고, bash `step_eval()` 함수는 그것을 손으로 mirror 한
//     사본이다(`deploy/daily-test.sh` 187~189행).
//   - 이 정본-사본 parity 는 origin/main 시점에 **약하게만** 검증됐다:
//      (1) bash test `deploy/daily-test-step-eval.test.sh`(T-0612) 는 argv 를
//          `grep -q` substring-presence 로만 검사 — full-ordered 벡터·순서·길이·flag↔value
//          페어링 parity 0(순서가 뒤집히거나 flag 가 추가/누락돼도 못 잡음).
//      (2) TS smoke `realdata-e2e-daily-step-eval-command-plan-assembly`(T-0736) 는 helper
//          단독 in-memory 검증이고 실 `deploy/daily-test.sh` bash 배선을 명시 Out-of-Scope.
...
//     `step_eval()` 함수 본문에서 jest argv 토큰을 추출하고, gating-enabled env 로
...
//   - `deploy/daily-test.sh` 변경 0 — readFileSync 로 읽기만. step_eval/argv 미수정
...
//   - 실 `deploy/daily-test.sh` 실행 / source / 실 jest spawn / 실 redeploy·health·liveness·
//     auth step 실행 0.
//   - helper 조립 로직·gating 판정 분기·self-assert guard 재단언 0(T-0736/gating spec cover).
//     본 task 는 helper 산출 argv 를 expected 로 사용만(로직 재현 0).
...
import type { RealDataDailyStepEvalCommandPlan } from "../helpers/realdata-e2e-daily-step-eval-command-plan";
import {
...
// shell 소스의 `step_eval()` 함수 본문에서 jest argv 토큰을 추출하는 보조 함수.
//
// `deploy/daily-test.sh` 187~189행은 multi-line line-continuation(`\`) 으로 jest argv 를
// 박는다:
//
...
//   1. `step_eval()` 함수 본문(여는 `step_eval() {` ~ 매칭 닫는 `}`)을 슬라이스한다.
...
function extractStepEvalJestArgv(shellSource: string): string[] {
  // (1) step_eval 함수 본문 슬라이스 — `step_eval() {` 부터 같은 들여쓰기 닫는 `}` 까지.
  const fnHeaderMatch = shellSource.match(/step_eval\s*\(\)\s*\{/);
...
function expectRunPlanArgv(plan: RealDataDailyStepEvalCommandPlan): string[] {
...
describe("realdata-e2e step④ daily-test.sh step_eval shell argv ↔ helper run-branch argv full-vector parity drift smoke (T-0790)", () => {
  describe("Happy-path: shell argv 추출 + helper run-branch argv 산출", () => {
    it("실 deploy/daily-test.sh 를 읽어 step_eval argv 추출 + gating-enabled env helper 호출이 run plan 을 낸다", () => {
      const shellSource = readFileSync(DAILY_TEST_SH_PATH, "utf8");
      const shellArgv = extractStepEvalJestArgv(shellSource);
    ...
    it("repo-root 경로가 __dirname 기준으로 robust 하게 해석되어 daily-test.sh 가 실재한다", () => {
      ...
      const shellSource = readFileSync(DAILY_TEST_SH_PATH, "utf8");
      expect(shellSource).toContain("step_eval()");
      expect(shellSource).toContain("pnpm exec jest");
    });
  });

  describe("full-ordered-vector byte-identical parity 수렴(핵심 불변식)", () => {
    it("shell step_eval 추출 argv 가 helper run-branch argv 와 byte-identical(toEqual — 순서·길이·요소 1:1)", () => {
      const shellSource = readFileSync(DAILY_TEST_SH_PATH, "utf8");
      const shellArgv = extractStepEvalJestArgv(shellSource);
      const plan = buildRealDataDailyStepEvalCommandPlan(
        buildGatingEnabledEnv(),
  ...
  describe("요소별 위치-정합 1:1 수렴(핵심 불변식 2)", () => {
    it("추출 argv 의 각 위치가 canonical 토큰·상수와 일치하고 length === 4", () => {
      ...
      expect(shellArgv[3]).toBe(REALDATA_E2E_LIVE_SMOKE_SPEC_PATH);
    });
  ...
  describe("helper 상수 ↔ shell 사본 토큰 1:1 수렴", () => {
    it("helper export 상수가 shell 소스에 literal 로 등장하고 추출 argv 대응 요소와 byte-identical", () => {
      ...
      expect(shellSource).toContain(REALDATA_E2E_LIVE_SMOKE_SPEC_PATH);
      expect(shellArgv[1]).toBe(REALDATA_E2E_SMOKE_JEST_CONFIG);
      expect(shellArgv[3]).toBe(REALDATA_E2E_LIVE_SMOKE_SPEC_PATH);
    });
  });
  ...
  describe("skip-branch argv 부재 boundary 수렴(REQ-059 / §9 경계)", () => {
    it("gating-disabled env → plan.action skip · plan.argv undefined, shell 측도 mark eval SKIP 분기 존재", () => {
      ...
      // shell 측 정합 — gating 부재 시 jest spawn 0(mark eval SKIP) 분기가 소스에 존재.
      const shellSource = readFileSync(DAILY_TEST_SH_PATH, "utf8");
      expect(shellSource).toContain("mark eval SKIP");
  ...
  describe("Error path / negative cases 충분 cover", () => {
    ...
    it("step_eval 함수 부재 합성 shell → 추출 결과 빈 배열(argv 못 찾음이 silent PASS 로 미누출)", () => {
      const noStepEval = "#!/bin/bash\necho noop\n";
      const argv = extractStepEvalJestArgv(noStepEval);
      const plan = buildRealDataDailyStepEvalCommandPlan(
        buildGatingEnabledEnv(),
      );
      const helperArgv = expectRunPlanArgv(plan);
    ...
    it("flag 누락 합성 shell(--config 만, --runTestsByPath 부재) → 추출 argv 가 helper 벡터와 not.toEqual", () => {
      ...
      const argv = extractStepEvalJestArgv(missingFlag);
      const plan = buildRealDataDailyStepEvalCommandPlan(
        buildGatingEnabledEnv(),
      );
      const helperArgv = expectRunPlanArgv(plan);
    ...
    it("순서 뒤집힘 합성 shell(runTestsByPath → config) → 추출 argv 가 helper 벡터와 not.toEqual", () => {
      ...
      const swappedOrder = [
        ...
        "step_eval() {",
        "  if ( cd x && pnpm exec jest \\",
        "        --runTestsByPath test/smoke/realdata-e2e-live.smoke-spec.ts \\",
        "        --config ./test/jest-smoke.json ); then",
        "    return 0",
        "  fi",
        "}",
      ].join("\n");
      const argv = extractStepEvalJestArgv(swappedOrder);
      const plan = buildRealDataDailyStepEvalCommandPlan(
        buildGatingEnabledEnv(),
      );
      const helperArgv = expectRunPlanArgv(plan);
  ...
  describe("결정론·no-mutation", () => {
    it("동일 shell 소스·동일 env 로 두 번 추출+helper 호출 → 두 번 byte-identical deep-equal", () => {
      const shellSource = readFileSync(DAILY_TEST_SH_PATH, "utf8");
      const argv1 = extractStepEvalJestArgv(shellSource);
      const argv2 = extractStepEvalJestArgv(shellSource);
      const plan1 = buildRealDataDailyStepEvalCommandPlan(
        buildGatingEnabledEnv(),
      );
      const plan2 = buildRealDataDailyStepEvalCommandPlan(
        buildGatingEnabledEnv(),
      );
    ...
    it("helper 호출이 입력 env 를 mutate 0 · 추출 함수가 입력 shell 문자열을 mutate 0", () => {
      ...
      extractStepEvalJestArgv(shellSource);
