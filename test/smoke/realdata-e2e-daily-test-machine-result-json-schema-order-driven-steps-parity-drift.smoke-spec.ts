...
// ↔ `steps` 객체 키 집합 == `ORDER` 배열(redeploy·health·liveness·auth·eval·collect·rediscovery·eval_chain) ↔ `failedStep`
...
// step 순회 정본 ORDER(`deploy/daily-test.sh` 287행) — steps 객체 키 집합 == 이 벡터.
// T-0888 에서 `collect` step 이 6번째 원소로 append 됨(step_collect bash 배선).
// T-0943 에서 `rediscovery` step 이 7번째 원소로 append 됨(step_rediscovery read-only bash 배선).
// T-1122 에서 `eval_chain` step 이 8번째 원소로 append 됨(step_eval_chain full-chain bash 배선).
...
describe("realdata-e2e step④ daily-test.sh 머신 요약 JSON 6-키 스키마 ↔ ORDER-driven steps ↔ failedStep null/quoted 분기 parity drift smoke (T-0791)", () => {
  ...
  describe("steps 키 집합 == ORDER byte-identical 수렴(핵심 불변식 2 — 본 task 의 새 표면)", () => {
    it("추출한 ORDER 가 [redeploy·health·liveness·auth·eval·collect·rediscovery·eval_chain]와 byte-identical(8-원소 ordered)", () => {
      const shellSource = readFileSync(DAILY_TEST_SH_PATH, "utf8");
      const order = extractOrderArray(shellSource);
  ...
  describe("Error path / negative cases 충분 cover", () => {
    ...
    it("ORDER 에 원소가 추가된 합성(또는 순서 swap) → expected 6-원소 벡터와 not.toEqual(ORDER drift 검출)", () => {
      // 원소 추가 합성(8-원소 정본에 deploy 를 더한 9-원소) — steps 키 contract 를 깨뜨릴 risk 를 본 smoke 가 잡음.
      const orderAdded =
        "ORDER=(redeploy health liveness auth eval collect rediscovery eval_chain deploy)";
      const extracted = extractOrderArray(orderAdded);
      expect(extracted).not.toEqual([...EXPECTED_ORDER]);
      expect(extracted).toContain("deploy");

      // 순서 swap 합성(8-원소 정본의 [0]↔[1] swap).
      const orderSwapped =
        "ORDER=(health redeploy liveness auth eval collect rediscovery eval_chain)";
