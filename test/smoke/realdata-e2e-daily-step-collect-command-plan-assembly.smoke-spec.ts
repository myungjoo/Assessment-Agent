// realdata-e2e-daily-step-collect-command-plan-assembly.smoke-spec.ts — 실 평가 e2e
// daily-test `step_collect` command-plan 조립 체인 non-gated build-time smoke (T-0892 박제,
// PLAN.md 109행 🟢 실 평가 e2e). eval-leg 정본 smoke(T-0736)의 collect-leg mirror.
//
// 본 spec 의 존재 이유 — public CI gap 해소:
//   - PLAN 109행 step ④(`deploy/daily-test.sh` 의 `step_collect`) 진입 경계는 순수 컴포저
//     `buildRealDataDailyStepCollectCommandPlan(env)`(T-0887) 가 닫는다 — gating 판정을
//     `resolveRealDataE2eLiveGating(env)`(T-0610) 에 위임하고 그 `enabled` 분기를
//     `{ action:"run", argv:[...] }` / `{ action:"skip" }` plan 으로 매핑한다. run 분기는
//     단일-spec bound jest argv(--config + --runTestsByPath)를 합성하되, spec 경로만
//     collection live smoke(`realdata-e2e-github-collection-live.smoke-spec.ts`, T-0806)를
//     가리킨다(eval-leg 과 동형이되 대상 leg 만 교체).
//   - 이 컴포저는 컴포저 단위 unit/consistency spec
//     (`realdata-e2e-daily-step-collect-command-plan.spec.ts`) + 독립 정합 가드(T-0890)
//     + self-wire(T-0891) 로는 닫혀 있으나, **gating → action → jest-argv 합성을 묶은
//     조립 체인 단위의 non-gated build-time smoke** 는 부재였다 — 즉 action↔gating 오매핑,
//     skip 인데 argv 존재, run 인데 argv config/spec-path drift, argv 길이/순서 어긋남,
//     reason 재포장 같은 조립 surface 회귀는 컴포저 unit spec 밖의 조립 레벨에서는 CI
//     그물이 없었다(eval-leg 은 T-0736 이 동형 그물을 이미 갖고 있으나 collect-leg 은 부재).
...
//     spawn / 실 github 수집) 는 복제하지 않고, gating→action→argv 조립만 검증한다. 따라서
//     본 spec 은:
//
//      🔥 실 LLM 호출 0 — orchestrator / scoring service / gateway 미사용. gating→action→
//         argv 조립만 — 실 수집·평가 실행은 본 컴포저 범위 밖.
...
//         guard 신설 금지 — 컴포저 self-wire 로 이미 닫힘, T-0890/T-0891).
//
// eval-leg 정본(T-0736)의 collect-leg 대칭 sibling 이며, 컴포저 unit spec 가 닫지 못하는
// 조립 레벨 회귀를 non-gated 로 cover 한다. 본 머지로 collect-leg command-plan 사슬이
// 컴포저(T-0887)→bash(T-0888)→parity smoke(T-0889)→가드(T-0890)→self-wire(T-0891)→조립
// smoke(T-0892)까지 eval-leg 와 동형으로 완전 대칭이 된다.
//
// Out of Scope (T-0892):
//   - eval-leg 조립 smoke(`realdata-e2e-daily-step-eval-command-plan-assembly.smoke-spec.ts`,
//     T-0736) — 절대 건드리지 않음(file-disjoint 병렬).
//   - T-0889 shell↔TS parity-drift smoke 재구현 — 본 spec 은 컴포저 조립 surface 검증만.
//   - 실 `deploy/daily-test.sh` bash 배선 / 실 jest 프로세스 spawn / 실 live smoke 실행 /
//     실 github 수집.
//   - 컴포저 소스(`realdata-e2e-daily-step-collect-command-plan.ts`) / gating helper /
...
// run 분기 argv/reason 어디에도 이 값이 나타나면 안 된다(§9).
...
// spec bound. 컴포저 상수로부터 유도(하드코딩 drift 방지). eval-leg 과 동형이되 spec 경로만
// collection live smoke 를 가리킨다.
...
describe("Smoke(non-gated): 실 평가 e2e daily-test step_collect command-plan 조립 체인(env→{action,argv,reason}) live 0 검증", () => {
  describe("happy path — gating 완전 set → action=run + canonical argv", () => {
    it("gating env 7 종 완전 set → action='run' + argv 가 canonical 4-요소 벡터 + reason 전파", () => {
      const plan = buildRealDataDailyStepCollectCommandPlan(fullGatingEnv());
      ...
      // canonical argv 의 spec 경로는 collection live smoke 를 가리킨다(eval-leg 과 대상 leg 구분).
      expect(plan.argv?.[3]).toBe(
        "test/smoke/realdata-e2e-github-collection-live.smoke-spec.ts",
      );
      // config flag 는 기존 smoke jest config(jest-smoke.json)를 재사용.
      expect(plan.argv?.[1]).toBe("./test/jest-smoke.json");
  ...
  describe("negative cases — 불완전 gating 은 run 으로 새지 않음 + credential echo 0", () => {
    ...
    it("(b) enable flag 만 set, 나머지 누락 → skip(부분-set 은 run 으로 새지 않음)", () => {
      ...
      const plan = buildRealDataDailyStepCollectCommandPlan(partial);
      expect(plan.action).toBe("skip");
      expect(plan.argv).toBeUndefined();
    });
    ...
    it("(c) run 분기 argv 가 실 credential 값을 echo 하지 않음 — argv 는 spec 경로·config flag 만(§9)", () => {
      const plan = buildRealDataDailyStepCollectCommandPlan(fullGatingEnv());
      expect(plan.action).toBe("run");
      ...
      // 정규식 단언 — token-like 접미사(TOKENLIKE / ghp_ prefix)가 argv 어디에도 없음.
      expect(joinedArgv).not.toMatch(/TOKENLIKE|ghp_/);
    });
    ...
    it("run 분기 reason 도 실 credential 값을 echo 하지 않음(§9 — reason 은 활성 사실만)", () => {
      ...
      expect(plan.reason).not.toMatch(/TOKENLIKE|ghp_/);
    });
  });

  describe("결정론 · 무공유 — 동일 입력 두 호출의 deep-equal + 참조 비공유 + 입력 불변", () => {
    it("(d) 같은 완전 env 두 호출 → plan deep-equal 이면서 plan · plan.argv 참조 비공유(새 배열)", () => {
    ...
    it("(d') 같은 빈 env 두 호출 → skip plan deep-equal(argv 둘 다 undefined)", () => {
      const a = buildRealDataDailyStepCollectCommandPlan({});
      const b = buildRealDataDailyStepCollectCommandPlan({});
      expect(a).toEqual(b);
      expect(a).not.toBe(b);
    });

    it("(e) 입력 env 객체가 호출 전후로 mutate 되지 않음(run 분기)", () => {
      const env = fullGatingEnv();
    ...
    it("(e') 입력 env 객체가 호출 전후로 mutate 되지 않음(skip 분기)", () => {
