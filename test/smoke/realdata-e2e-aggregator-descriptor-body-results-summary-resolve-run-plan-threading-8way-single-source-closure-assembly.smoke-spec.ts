// realdata-e2e-aggregator-descriptor-body-results-summary-resolve-run-plan-threading-8way-single-source-closure-assembly.smoke-spec.ts
// — 실 평가 e2e step④ aggregator descriptor-body 합류 8-way single-source closure:
// pre-실행 aggregator `buildRealDataE2eStepArgs(runPlan, activities, results)` 의 publish leg
// **사람-대면 이슈 본문 렌더 surface `report.descriptor.body`** 를 단일 source `(results,
// runPlan.run)` 재유도 byte-identical 로 closure 에 묶어, 검증 source `(runPlan, activities,
// results)` 세 source 가 다음 8 축의 동시 source 임을 한 chain 으로 박제한다:
//   (축 8, 본 task 의 새 표면) **publish.report.descriptor.body results+run-재유도
//      byte-identical** — `stepArgs.publish.report.descriptor.body` 가 동일 `results`·동일
//      `runPlan.run` 으로 직접 호출한 `buildRealDataResultIssueCommandPlan(results,
//      runPlan.run).report.descriptor.body` 와 byte-identical string equal(`toBe`). 사람-대면
//      본문(marker 라인 + summary 한 줄 + markdown 본문 3 블록)이 단일 `(results, run)`
//      source 로부터 동형 렌더됨. **재유도는 command-plan 경유**(aggregator 가 실제 거치는
//      동일 위임 경로 — `buildRealDataResultIssueDescriptor`·`renderRealDataResultSummaryMarkdown`
//      직접 호출 0, SSOT). summary(정량 집계 객체)와 distinct 한 렌더 surface.
//   (축 7, T-0776 cover 영역의 8-way 묶음 항) **publish.report.summary results-재유도
//      byte-identical** — `stepArgs.publish.report.summary` deep-equal command-plan 재유도.
//   (축 6, T-0775 cover 영역의 묶음 항) **evaluation.inputs activities-재유도 byte-identical
//      + callArgs[i].input === inputs[i] reference 페어링**.
...
//
// 이 8-way 가 **세 검증 source `(runPlan, activities, results)`** single-source 에서 한 chain
// 동시-호출로 byte-identical 수렴함을 박제하는 첫 aggregator-descriptor-body 합류 8-way
// single-source closure non-gated build-time smoke (T-0777 박제, PLAN.md 109행 🟢 실 평가
// e2e step④). publish leg 의 두 출력 surface(정량 집계 summary + 사람-대면 렌더 body)가
// 모두 단일-source 재유도로 묶이는 slice.
//
// 본 spec 의 존재 이유 — public CI gap 해소(publish leg 의 descriptor.body 렌더 재유도 축을
// 8번째 축으로 합류):
...
//     산출). T-0776 이 세 번째 source `results` 의 정량 집계 객체 `report.summary` 를
//     closure 에 묶어 7-way 를 완성했다.
//   - 직전 sibling 들은 publish leg 의 distinct 한 출력 surface `report.descriptor.body`(사람-
//     대면 이슈 본문 렌더)를 byte-identical 재유도로 closure 에 합류시키지 않았다:
//       T-0776 (aggregator 7-way) — publish leg `report.summary`(=results 재유도 객체)+
//         evaluation inputs/modelId+marker/searchArgv/run-identity 를 단일 (runPlan,
//         activities, results) source 로 수렴 박제. 그러나 `report.descriptor.body`(=summary 의
//         렌더 산물)는 credential surface·variance(not.toBe)로만 사용 — body 의 단일-source
//         `(results, run)` 재유도 byte-identical 정합은 closure 에 미합류.
//       T-0772/0773/0774 (aggregator 3·4·5-way) — descriptor.body 를 `not.toBe`(results 분포
//         다르면 body 도 달라야 함) variance 단언으로만 사용 — byte-identical 재유도 미합류.
//       descriptor / descriptor-body-consistency helper spec — body 3블록 구조·렌더 자체는
//         박제했으나 aggregator·resolve·post 미합류(단독 helper 호출만).
//   - 본 spec 은 그 빈 자리를 채워 **publish leg 의 descriptor.body 렌더 산물을 단일-source
//     `(results, runPlan.run)` 재유도 byte-identical 로 closure 에 묶어**, 검증 source
//     `(runPlan, activities, results)` 세 source 가 aggregator publish leg 의 두 출력
//     surface(정량 집계 summary + 사람-대면 렌더 body)와 evaluation leg(inputs+modelId), 그리고
//     marker/searchArgv/run-identity 의 source 임을 resolve+post 까지 묶은 한 chain 으로 박제한다.
//   - 이 8-way 가 세 검증 source single-source 로 수렴함이 search-or-update 멱등성(REQ-009)·
//     raw 미보유 평가 입력/결과 집계 정합(REQ-032)·결과 리포트 재실행 정합(REQ-037)의
//     aggregator-level "정량 집계(summary)+사람-대면 본문(body)+평가 입력(inputs)+평가 정책
...
//      🔥 실 LLM scoreUnit 호출 0 / 실 DB 접근 0 / 실 jest spawn 0 — 8-way 수렴 조립만.
...
//         재사용만(가드/helper 신설 0). `extractSearchMarker` 는 spec 로컬 함수(T-0776/T-0729
//         패턴 차용).
...
//     search-stdout / exec-stdout 만. body 는 **사람-대면 본문 식별자 운반만** 검증(실 박제 0).
//   - aggregator 7-way(results-summary 재유도 + evaluation.inputs 재유도 + callArgs.input
//     페어링 + modelId + publish marker/searchArgv/run-identity) 자체 재단언(T-0776 cover).
//     본 task 는 publish leg `report.descriptor.body`(=results+run source 렌더 재유도
//     byte-identical)를 8번째 축으로 합류시킨 부분만.
//   - descriptor.body 의 3블록(marker 라인 / summary 한 줄 / markdown 본문) 합성 로직·
//     self-guard 자체 재단언(descriptor / descriptor-body-consistency helper spec,
//     T-0582/T-0644 cover). 본 task 는 aggregator 산출 descriptor.body 가 단일 source
//     `(results, run)` 재유도(command-plan 경유)와 byte-identical 함만.
//   - renderRealDataResultSummaryMarkdown(T-0581) / formatRealDataResultSummaryLine
//     (T-0642/T-0644) 렌더 형식 자체 재단언(각 helper spec cover). 본 task 는 aggregator body
//     가 동일 위임 경로 재유도와 byte-identical 함만.
...
const MODEL_ID =
  "cfg-realdata-e2e-aggregator-descriptor-body-results-summary-resolve-threading-smoke";
...
// T-0776/T-0729 sibling 헬퍼 패턴 mirror.
...
function syntheticActivity(
  author: string,
  externalId = "realdata-e2e-aggregator-descriptor-body-c1",
...
function defaultActivities(): GithubActivity[] {
  ...
  return [
    syntheticActivity(firstUsername, "aggregator-descriptor-body-c1", "commit"),
    syntheticActivity(firstUsername, "aggregator-descriptor-body-p1", "pr"),
    syntheticActivity(firstUsername, "aggregator-descriptor-body-i1", "issue"),
  ];
}
...
function syntheticResult(
  ...
  return {
    unitId,
    narrative:
      "synthetic evaluation narrative — aggregator descriptor-body+results-summary+evaluation-inputs+modelId+searchArgv ↔ resolve ↔ outcome-step-args run-plan-threading 8-way closure smoke fixture",
    difficulty,
    contribution,
    volume,
  };
...
// 다른 분포(Y) results fixture — summary·body 변별성 단언용(다른 count·difficulty/
// contribution 분포·totalVolume). defaultResults 와 다른 정량 집계·본문 렌더를 산출한다.
...
// thread 하고, results 가 publish leg 의 report.descriptor.body(축 8)·report.summary(축 7)
// 재유도 source 가 된다.
function runChain(
  hitsNumbers: number[],
  ...
  // 재유도 source(축 6), 동일 results 가 publish.report.descriptor.body(축 8)·
  // report.summary(축 7) 재유도 source.
...
describe("Smoke(non-gated): 실 평가 e2e step④ aggregator descriptor-body 합류 8-way single-source closure buildRealDataE2eStepArgs(.publish.report.descriptor.body(=buildRealDataResultIssueCommandPlan(results, runPlan.run).report.descriptor.body byte-identical) + .publish.report.summary(=동 command-plan.report.summary) + .evaluation {inputs(=buildRealDataEvaluationInputs(activities), callArgs[i].input===inputs[i]), callArgs[].options.modelId} + .publish {descriptor.marker, commandArgs.searchQuery, searchArgv --match})↔resolve↔buildRealDataResultOutcomeStepArgs 동일 (runPlan, activities, results) 한 chain 동시-호출(body 재유도 + summary 재유도 + inputs 재유도 + modelId + marker 3-축 + issueNumber + run-identity 8축 동시 수렴, 재전달 0) live-gh/live-LLM 0 검증", () => {
  describe("happy path — aggregator 8-way chain 합성(다섯 산출물 모두 정상)", () => {
    it("(a) 유효 runPlan + activities + results + searchStdout + execStdout → stepArgs.evaluation({inputs,callArgs} 비어있지 않음) / stepArgs.publish({report,commandArgs,searchArgv} 정상·report.descriptor.body 비어있지 않은 문자열·report.summary 존재·report.descriptor.marker 존재) / resolvePlan(update {action, issueNumber} + argv) / outcomeReport(5필드) 다섯 산출물 모두 정상", () => {
  ...
  describe("publish-leg descriptor-body 렌더 재유도 byte-identical 수렴(branch — 핵심 불변식 1, 본 task 의 새 표면 — 축 8)", () => {
    it("(b) stepArgs.publish.report.descriptor.body === buildRealDataResultIssueCommandPlan(results, runPlan.run).report.descriptor.body byte-identical string equal — aggregator 의 publish leg 사람-대면 본문(marker 라인 + summary 한 줄 + markdown 본문)이 단일 (results, run) source 로부터 동형 렌더됨. 재유도는 command-plan 경유(buildRealDataResultIssueDescriptor·renderRealDataResultSummaryMarkdown 직접 호출 0 — aggregator 와 동일 위임 경로 SSOT). 보조: body 첫 줄 === descriptor.marker, body 가 run gitSha 포함(3블록 구조 확인)", () => {
      ...
      // 동일 results·동일 runPlan.run 으로 직접 호출한 command-plan 의 descriptor.body 와
      // byte-identical string equal — aggregator 가 body 를 같은 source·같은 위임 경로로 재유도.
      const directBody = buildRealDataResultIssueCommandPlan(
        results,
        runPlan.run,
      ).report.descriptor.body;
      expect(stepArgs.publish.report.descriptor.body).toBe(directBody);

      // 보조 — body 3블록 구조 확인(marker 라인 + summary 한 줄 + markdown 본문).
      const body = stepArgs.publish.report.descriptor.body;
      // body 첫 줄이 descriptor.marker(= body 가 marker 라인을 첫 블록으로 포함).
      expect(body.split("\n")[0]).toBe(
        stepArgs.publish.report.descriptor.marker,
      );
      // body 가 run gitSha·dateToken 을 포함(marker 라인의 run token 경유).
      expect(body.includes(runPlan.run.gitSha)).toBe(true);
      expect(body.includes(runPlan.run.dateToken)).toBe(true);
    });
  });

  describe("publish-leg results-summary 재유도 byte-identical 수렴(branch — 핵심 불변식 2, 축 7)", () => {
    it("(c) stepArgs.publish.report.summary deep-equal buildRealDataResultIssueCommandPlan(results, runPlan.run).report.summary — body(렌더 산물)와 summary(정량 집계 객체)가 같은 단일 source (results, run)의 두 distinct surface 임을 명시", () => {
      const runPlan = buildRunPlan();
      const results = defaultResults();
      const { stepArgs } = runChain(
        [7, 13],
      ...
      // summary deep-equal(정량 집계 객체) AND body string equal(렌더 산물) — 같은 source.
  ...
  describe("evaluation-leg inputs 재유도 + callArgs.input 페어링(branch — 핵심 불변식 3, 축 6)", () => {
    it("(d) stepArgs.evaluation.inputs deep-equal buildRealDataEvaluationInputs(activities) AND 모든 callArgs[i].input === inputs[i] referential 페어링 + 길이 일치", () => {
  ...
  describe("evaluation-leg modelId thread 수렴(branch — 핵심 불변식 4, 축 5)", () => {
    it("(e) 모든 stepArgs.evaluation.callArgs[i].options.modelId === runPlan.pipeline.modelId — 단일 검증 runPlan 의 평가 정책 modelId 가 평가 호출-args 전체에 동형 thread", () => {
      const runPlan = buildRunPlan();
  ...
  describe("publish-leg 내부 marker 3-축 일치(branch — 핵심 불변식 5, 축 2~4)", () => {
    it("(f) extractSearchMarker(searchArgv) === report.descriptor.marker === commandArgs.searchQuery — 세 내부 marker 축이 byte-identical, marker 가 searchArgv 안 정확히 1회 등장, AND body 첫 줄 === marker(body 가 marker 를 포함)", () => {
      ...
      // body 첫 줄(=marker 라인)이 그 marker 와 동일 — body 가 marker 를 포함.
      expect(stepArgs.publish.report.descriptor.body.split("\n")[0]).toBe(
        descriptorMarker,
      );
    });
  });

  describe("marker → resolve issueNumber + post run-identity 수렴(branch — 핵심 불변식 6, 축 1)", () => {
    it("(g) search hit N → resolve.action.update.issueNumber → outcome step-args.issueNumber 세 지점 모두 동일 N + url 에 /issues/N + 동일 runPlan.run 전파(marker run token == outcomeReport.{gitSha,dateToken,summaryLine})", () => {
      const N = 7;
      const runPlan = buildRunPlan();
  ...
  describe("8-way 단일-source 묶음 단언(branch — closure 종단, 본 task 의 종단 박제)", () => {
    it("(h) publish.report.descriptor.body == buildRealDataResultIssueCommandPlan(results, runPlan.run).report.descriptor.body, publish.report.summary == 동 command-plan.report.summary, evaluation.inputs == buildRealDataEvaluationInputs(activities), callArgs[].input === inputs[i], callArgs[].options.modelId == runPlan.pipeline.modelId, searchArgv --match 토큰 == descriptor.marker == commandArgs.searchQuery, 그 marker 로 resolve 가 찾은 N == post 가 해석한 N, 그 marker 의 run token == post 가 전파한 {gitSha,dateToken} 가 세 검증 source (runPlan, activities, results) single-source 에서 8-way 동시 성립", () => {
      const N = 7;
      const runPlan = buildRunPlan();
      const activities = defaultActivities();
      const results = defaultResults();
      ...
      // axis 8(새 표면) — publish.report.descriptor.body byte-identical command-plan 재유도.
      expect(stepArgs.publish.report.descriptor.body).toBe(
        cmdPlan.report.descriptor.body,
      );
      ...
      expect(outcomeReport.gitSha).toBe(runPlan.run.gitSha);
      expect(outcomeReport.dateToken).toBe(runPlan.run.dateToken);
      // body 가 marker 라인(첫 블록)으로 그 run token 을 운반 — body↔marker↔post 한 source.
      expect(stepArgs.publish.report.descriptor.body.split("\n")[0]).toBe(
        marker,
      );
      // 8-way 동시 closure — 세 검증 source single-source 에서 여덟 축이 한 chain 으로 묶임.
      expect(`${runPlan.run.dateToken}@${runPlan.run.gitSha}`).toBe(token);
    });
  });

  describe("results 축 변별성(branch — results 가 body·summary 를 결정, activities·modelId·run 은 독립 축)", () => {
    it("(i) results_A(분포 X) vs results_B(분포 Y) 로 같은 runPlan·activities·N 으로 chain → A.descriptor.body == cmdPlan(results_A,run).descriptor.body, B.descriptor.body == (results_B,run) 각각 byte-identical(results 가 body 를 결정) → 두 chain 의 body·summary 는 서로 다름(not.toBe/not.toEqual) → 그러나 evaluation.inputs / modelId / descriptor.marker / searchArgv --match / resolve issueNumber / outcomeReport.{gitSha,dateToken} 는 두 chain 동일(results 변경이 inputs·modelId·marker·run-identity·issueNumber 어느 축에도 누설 0)", () => {
      const runPlan = buildRunPlan();
      const activities = defaultActivities();
      ...
      // 축 8 — body 가 results 로부터 각각 재유도(byte-identical command-plan).
      expect(chainA.stepArgs.publish.report.descriptor.body).toBe(
        buildRealDataResultIssueCommandPlan(resultsA, runPlan.run).report
          .descriptor.body,
      );
      expect(chainB.stepArgs.publish.report.descriptor.body).toBe(
        buildRealDataResultIssueCommandPlan(resultsB, runPlan.run).report
          .descriptor.body,
      );
      // body·summary 변별(results 분포 다름이 본문·집계에 반영).
  ...
  describe("activities·modelId·run 변별성(branch — 같은 results, 다른 activities/runPlan → body 의 집계 부분 불변·run 부분 변별)", () => {
    it("(j) 동일 results·동일 N 고정 + (a) activities_A vs activities_B → 두 chain 의 descriptor.body 동일(body 는 results+run 종속, activities 무관)·inputs 변별, (b) runPlan_A{model-x,run_A} vs runPlan_B{model-y,run_B} → descriptor.body 의 marker 라인·run token 부분은 run 종속 변별(body.includes(run.gitSha) 각 run 으로 다름) 이나 두 chain 모두 각 (results, run) 재유도와 byte-identical — modelId·marker·outcomeReport.{gitSha,dateToken} 도 변별", () => {
      const results = defaultResults();
      const firstUsername =
        buildRealDataE2eSeed()[0].serviceIdentities[0].externalId;
      const activitiesA: GithubActivity[] = [
        syntheticActivity(firstUsername, "descriptor-body-Ac1", "commit"),
        syntheticActivity(firstUsername, "descriptor-body-Ap1", "pr"),
      ];
      const activitiesB: GithubActivity[] = [
        syntheticActivity(firstUsername, "descriptor-body-Bc2", "commit"),
        syntheticActivity(firstUsername, "descriptor-body-Bp2", "pr"),
        syntheticActivity(firstUsername, "descriptor-body-Bi2", "issue"),
      ];

      // (a) 다른 activities, 같은 runPlan·results → body 동일(results+run 종속), inputs 변별.
      const runPlan = buildRunPlan();
      const chainAa = runChain([7, 13], runPlan, activitiesA, results);
      const chainAb = runChain([7, 13], runPlan, activitiesB, results);
      // body 는 results+run 종속 — activities 변경에 불변.
      expect(chainAa.stepArgs.publish.report.descriptor.body).toBe(
        chainAb.stepArgs.publish.report.descriptor.body,
      );
      ...
      // (b) 다른 runPlan(modelId+run), 같은 activities·results → body 의 run 부분 변별,
      ...
      // 각 chain 의 body 는 각 (results, run) 재유도와 byte-identical.
      expect(chainBx.stepArgs.publish.report.descriptor.body).toBe(
        buildRealDataResultIssueCommandPlan(results, runPlanX.run).report
          .descriptor.body,
      );
      expect(chainBy.stepArgs.publish.report.descriptor.body).toBe(
        buildRealDataResultIssueCommandPlan(results, runPlanY.run).report
          .descriptor.body,
      );
      // body 의 run-종속 부분(marker 라인 run token) 변별.
      expect(chainBx.stepArgs.publish.report.descriptor.body).not.toBe(
        chainBy.stepArgs.publish.report.descriptor.body,
      );
      expect(
        chainBx.stepArgs.publish.report.descriptor.body.includes(
          runPlanX.run.gitSha,
        ),
      ).toBe(true);
      expect(
        chainBy.stepArgs.publish.report.descriptor.body.includes(
          runPlanY.run.gitSha,
        ),
      ).toBe(true);
  ...
  describe("create 분기 격리(branch — 검색 미스 → create, body·summary·inputs·modelId·searchArgv·post 무관)", () => {
    it("(k) 빈 hit search stdout('[]') → resolve.action 이 create 분기(action.update 부재) — stepArgs.publish.report.descriptor.body(= results+run 재유도) / report.summary / evaluation.inputs / callArgs[].options.modelId / searchArgv --match marker 는 create/update 두 분기 모두 동일(검색 결과가 body·summary·inputs·modelId·검색 인자 벡터를 바꾸지 0), post 는 여전히 execStdout 의 N 으로 issueNumber 산출(누설 0)", () => {
      const runPlan = buildRunPlan();
      const activities = defaultActivities();
      const results = defaultResults();
      const stepArgs = buildRealDataE2eStepArgs(runPlan, activities, results);
      ...
      // body(= results+run 재유도) / summary / inputs / modelId / searchArgv --match marker 는
  ...
  describe("error path / negative cases — 여덟 boundary 거부 대칭 박제(R-112 negative 충분 cover)", () => {
    it("(l) runPlan.pipeline.modelId 빈('') → run-plan 합성 단계(buildRealDataE2eRunPlan) 측 pipeline modelId guard throw(평가 leg 비식별 — modelId 정책 미결정이면 run plan 산출 차단)", () => {
      const seeds = buildRealDataE2eSeed();
      expect(() =>
    ...
    it("(l') runPlan.pipeline.modelId 공백-only('   ') 우회 합성 → aggregator(buildRealDataE2eStepArgs) 측 evaluation 위임 하위 modelId guard throw(aggregator 단계 거부 — inputs 재유도 도달 전 차단)", () => {
    ...
    it("(m) runPlan.run.gitSha 빈('') → aggregator측 publish 위임 descriptor assertNonBlank('gitSha') throw(descriptor.body 합성 도달 전 차단). modelId·activities·results 정상이어도 차단", () => {
      const base = buildRunPlan();
    ...
    it("(m') runPlan.run.dateToken 빈('') → aggregator측 publish 위임 descriptor assertNonBlank('dateToken') throw 대칭", () => {
      const base = buildRunPlan();
      const blankRunPlan: RealDataE2eRunPlan = {
        pipeline: base.pipeline,
    ...
    it("(n) runPlan.run.gitSha 빈('') → post(buildRealDataResultOutcomeStepArgs) 측 위임 assertNonBlank throw(post boundary 종단 비식별 — execStdout URL 정상이어도 차단)", () => {
      const base = buildRunPlan();
      const blankRunPlan: RealDataE2eRunPlan = {
        pipeline: base.pipeline,
        run: { gitSha: "", dateToken: "2026-06-29" },
    ...
    it("(o) searchStdout 비JSON('not json') → resolve parse 위임 throw(stepArgs.publish.commandArgs 정상이어도 hits 추출 실패로 차단)", () => {
    ...
    it("(p) execStdout URL 미발견(빈 문자열) → post parse 위임 throw(runPlan.run 정상이어도 outcome 추출 실패)", () => {
      expect(() =>
        buildRealDataResultOutcomeStepArgs(buildRunPlan(), ""),
      ).toThrow();
    });

    it("(p') execStdout /issues/0 → post assertPositiveIssueNumber throw(post 비식별)", () => {
      expect(() =>
    ...
    // body 축 negative — production 집계(`buildRealDataResultSummary`)는 모든 difficulty/
    // contribution 을 production 타입(Difficulty / ContributionLevel)으로 좁혀진 typed union
    // 으로 받으며 throw 분기 0 으로 관대하게 집계하고, descriptor body 렌더(markdown/summary
    // line)도 정상 summary 에 대해 throw 0 으로 렌더한다(result-summary.ts / summary-markdown.ts
    // / descriptor 시그니처 확인 — 슬롯 누적·렌더만, 미지원 enum throw 분기 0). 따라서 본 task 의
    // body 축 negative 는 task line 95 의 명시 대체 — **빈 results → body 는 marker 라인 +
    // count 0 summary 한 줄 + 빈/0 markdown 본문 정상(throw 0) 경계 단언**으로 대체한다.
    // aggregator 가 빈 results 를 그대로 전파해 publish 위임이 빈-집계 body 를 렌더하고 그것이
    // command-plan 재유도와 byte-identical 함을 확인.
    it("(q) body 축 negative 대체 — 빈 results → stepArgs.publish.report.descriptor.body 는 marker 라인 + count 0 summary 한 줄 + markdown 본문 정상(throw 0), command-plan 재유도와 byte-identical, body 첫 줄 === marker, evaluation leg / resolve / post 는 정상 진행(body 축 빈 경계가 8-way closure 의 다른 축을 차단하지 0)", () => {
      const runPlan = buildRunPlan();
      const activities = defaultActivities();
      const stepArgs = buildRealDataE2eStepArgs(runPlan, activities, []);

      // 빈 results → body 정상 렌더(throw 0), 비어있지 않은 문자열.
      const body = stepArgs.publish.report.descriptor.body;
      expect(typeof body).toBe("string");
      expect(body.length).toBeGreaterThan(0);
      // command-plan 재유도와 byte-identical(빈 경계에도 재유도 일관).
      expect(body).toBe(
        buildRealDataResultIssueCommandPlan([], runPlan.run).report.descriptor
          .body,
      );
      ...
      // evaluation leg / resolve / post 는 정상 진행(body 빈 경계가 다른 축 차단 0).
      expect(stepArgs.evaluation.inputs).toEqual(
        buildRealDataEvaluationInputs(activities),
      );
      const marker = stepArgs.publish.report.descriptor.marker;
  ...
  describe("결정론·무공유·no-mutation — chain 두 번 호출 + 입력 불변 + 무공유", () => {
    it("(r) 동일 (runPlan, activities, results, searchStdout, execStdout) chain 두 번 → stepArgs(evaluation+publish)/resolvePlan/outcomeReport 모두 deep-equal(결정론)", () => {
      const runPlan = buildRunPlan();
      const activities = defaultActivities();
      const results = defaultResults();
    ...
    it("(s) no-mutation — 입력 runPlan(특히 aggregator publish leg 와 직접 호출 command-plan 재유도가 같은 results·runPlan 공유 읽기)·activities·results chain 호출 후 deep-equal(원본 불변)", () => {
      const runPlan = buildRunPlan();
      const activities = defaultActivities();
      const results = defaultResults();
    ...
    it("(t) 무공유 — 직접 호출 buildRealDataResultIssueCommandPlan(results, runPlan.run).report.descriptor 와 stepArgs.publish.report.descriptor 는 deep-equal 이되 referential 분리(not.toBe — 무공유). body 는 string 이라 === 동치이나 descriptor 객체는 referential 분리로 무공유 확인", () => {
      const runPlan = buildRunPlan();
      const results = defaultResults();
      ...
      // descriptor 재유도와 deep-equal 이지만 referential 분리(매 호출 새 객체).
      const directDescriptor = buildRealDataResultIssueCommandPlan(
        results,
        runPlan.run,
      ).report.descriptor;
      expect(stepArgs.publish.report.descriptor).toEqual(directDescriptor);
      expect(stepArgs.publish.report.descriptor).not.toBe(directDescriptor);
      // body 는 string 이라 === 동치(불변 primitive — 무공유 무관).
      expect(stepArgs.publish.report.descriptor.body).toBe(
        directDescriptor.body,
      );
  ...
  describe("credential 누출 0(branch — §9 / R-59 / REQ-059 정합)", () => {
    it("(u) stepArgs.publish.report.descriptor.{title,marker,body} · report.summary(직렬화) · evaluation.inputs(직렬화) · callArgs(직렬화) · searchArgv(각 원소) · commandArgs.searchQuery · resolvePlan.argv · outcomeReport.{url,summaryLine} 어디에도 token/secret/PAT/--auth 어휘 미등장(특히 descriptor.body 사람-대면 markdown 본문 명시)", () => {
      const { stepArgs, resolvePlan, outcomeReport } = runChain([7, 13]);
      ...
      // descriptor.body(사람-대면 markdown 본문)에 credential 어휘 미등장 명시.
      expect(stepArgs.publish.report.descriptor.body).not.toMatch(
        credentialPattern,
      );
