// realdata-e2e-daily-step-dual-leg-run-report-create-branch-searchargv-marker-
// createbody-single-source-convergence-assembly.smoke-spec.ts — 실 평가 e2e
// daily-step dual-leg run report 의 **create 분기(빈 search)** 에서 두 개의 서로
// 다른 gh 호출이 **동일 descriptor.marker 단일 source 로부터 byte-identical 하게
// 관통** 함을 박제하는 create-branch cross-call marker single-source convergence
// non-gated build-time smoke (T-0928 박제, PLAN.md 109행 🟢 실 평가 e2e step ④).
//
// 두 gh 호출(create 분기 절단면):
...
//   - call 2 (create) — 검색이 빈 결과("[]")면 action 이 create 로 해소되어 종단
//     컴포저 resolveRealDataDailyStepDualLegRunReportIssueGhCommandPlan(stdout,
//     commandArgs)(T-0902) 가 argv `["issue","create","--title",title,"--body",body,
//     ...labels]` 를 산출한다. `--body` 다음 원소 body 의 **선두 라인**(body.split(
//     "\n")[0]) = descriptor.marker(descriptor helper: body = [marker,"",...markdown]).
//
// 본 spec 의 존재 이유 — create-branch cross-call marker single-source seam 봉합:
//   검색하는 marker(call 1)와 새로 박제하는 이슈 body 에 심는 marker(call 2)가
//   **동일 descriptor.marker 단일 source 로부터 byte-identical 하게 관통** 해야 step ④
//   의 create-side 멱등(REQ-009 — "동일 run 의 이슈를 찾아 갱신")이 성립한다: 오늘 밤
//   run 이 marker M 으로 검색해 없으면 body 선두에 marker M 을 심어 이슈를 만들고,
//   내일 밤 run 이 다시 marker M 으로 검색하면 그 이슈를 찾아 **중복 생성 대신 갱신**
//   한다. 만약 검색 marker 와 create body marker 가 어긋나면(silent drift) — 매
//   nightly 가 자기가 만든 이슈를 못 찾아 **중복 이슈를 무한 양산** 한다.
//
//   형제 smoke 들이 각기 단일 gh 호출 축만 닫았다:
//     * T-0919(search-side marker-medium 4-boundary closure) 는 call 1 의 searchArgv
//       원소 marker 만 자산화하고 create body argv 를 다루지 않는다.
//     * T-0927(gh-command-plan create/update argv single-source) 은 call 2 의
//       create-branch plan.argv 가 single-source 빌더 산출과 byte-identical 함만
//       자산화하고 그 body 선두 marker 가 call 1 searchArgv 검색 marker 와 동일 source
//       임은 미단언(searchArgv 를 다루지 않는다).
//     * T-0921(update-branch marker⊥issueNumber orthogonal) 은 **update/edit 분기** 의
//       marker(=search 매체) ⊥ issueNumber(=edit 매체) 직교성만 자산화하고 **create
//       분기** 는 명시적으로 격리 대상(재단언 금지). create 분기에는 issueNumber 매체가
//       없고 대신 marker 가 **검색·생성 두 지점에 동시 등장** 하므로 T-0921 의 직교 축과
//       다른 축(marker single-source cross-call 수렴)이다.
//
//   본 spec 은 T-0921 의 **create-side 대칭 sibling** 으로, `report → descriptor →
//   commandArgs → (1) searchArgv, (2) 빈 search stdout("[]") 에 종단 컴포저 → create
...
//    🔥 실 gh 실행 0 — search / issue create 실 실행 0. execFile('gh', …) 0.
...
//     축 재단언 — 본 spec 은 create 분기 검색 marker(call 1) ↔ create body 선두 marker
//     (call 2) single-source cross-call 수렴만(update 분기는 격리 확인만).
//   - 형제 T-0919 의 search-argv marker 4-boundary single-source closure 축·T-0927 의
//     gh-command-plan create/update argv single-source byte-identical 축 자체 재단언 —
//     본 spec 은 두 gh 호출을 결합한 cross-call marker 수렴 단일 축.
//   - descriptor body 의 markdown 본문 렌더(marker 이후 블록)·title 형식·labels 형식
//     재단언 — 각 helper/smoke 가 이미 cover. 본 spec 은 marker 라인(검색·생성 두 지점)
//     single-source 수렴만.
...
// create-branch plan.argv 의 body 원소 추출 — `--body` 다음 원소. body 선두 라인이 marker.
function createBodyOf(argv: string[]): string {
  const bodyIdx = argv.indexOf("--body");
  return argv[bodyIdx + 1];
}

// create body 선두 라인 = marker(descriptor helper: body = [marker,"",...markdown]).
function createBodyHeadLineOf(argv: string[]): string {
  return createBodyOf(argv).split("\n")[0];
}

// create-branch cross-call 조립 진입점 — 단일 source(run + 두 leg outcome) 로부터 report
// → descriptor → commandArgs 를 관통시켜 유효 commandArgs 를 얻고, (1) searchArgv 를,
// (2) 빈 search stdout("[]") 에 종단 컴포저를 적용해 create plan 을 산출한다. 각 stage 의
// guard throw 는 자체 try/catch 없이 그대로 전파된다. opts.searchStdout 으로 update 분기
// 격리 test 도 같은 진입점을 재사용한다.
function assembleCreateBranch(
  evalLeg: RealDataDailyStepLegRunOutcome,
  collectLeg: RealDataDailyStepLegRunOutcome,
  run: RealDataResultIssueRunRef,
  opts?: { searchStdout?: string },
) {
  ...
  // (2) command-args — searchQuery = descriptor.marker, createArgs.body = descriptor.body.
  const commandArgs =
  ...
  // (4) call 2 — 빈 search stdout("[]") → create 분기 plan.argv(create body 선두 = marker).
...
describe("Smoke(non-gated): dual-leg run report create-branch(빈 search) cross-call marker(searchArgv 검색 marker + create body 선두 marker) single-source convergence live-gh 0 검증", () => {
  describe("happy path — create-branch marker cross-call 수렴", () => {
    it("단일 source(run + 두 leg outcome) → 빈 search('[]') → plan.action.action === 'create' 이고 searchArgv 검색 marker 와 create body 선두 라인이 각각 descriptor.marker 와 byte-identical(===) AND 서로도 byte-identical", () => {
      const { descriptor, searchArgv, plan } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
      );
      ...
      const createBodyHead = createBodyHeadLineOf(plan.argv);

      // (1) 두 지점이 각각 descriptor.marker 와 byte-identical.
      expect(searchMarker).toBe(descriptor.marker);
      expect(createBodyHead).toBe(descriptor.marker);

      // (2) 두 지점이 서로도 byte-identical(단일 source 관통).
      expect(searchMarker).toBe(createBodyHead);
    });

    it("create plan.argv 는 issue create argv 형(argv[0]==='issue', argv[1]==='create')이고 `--body` 원소를 보유 — create body 선두 marker 추출 전제 확인", () => {
      const { plan } = assembleCreateBranch(evalOutcome(), collectOutcome(), {
        ...RUN_A,
      });
  ...
  describe("핵심 1 — marker 등장 지점 유일성", () => {
    it("searchArgv 에서 marker 는 정확히 검색질의 1원소로만 등장 — 다른 원소(search/issues/--match/body/--json/필드/--limit)에는 marker 온전체 부재", () => {
      const { descriptor, searchArgv } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
      );
    ...
    it("create plan.argv 에서 marker(온전체)는 create body 원소 안에서만 등장 — title 원소·labels 원소에는 marker 온전체 부재(title 은 body 선두 marker 라인과 !==)", () => {
      const { descriptor, plan } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
      );
      const marker = descriptor.marker;
      const body = createBodyOf(plan.argv);
      ...
      expect(title).not.toBe(createBodyHeadLineOf(plan.argv));

      // body 를 제외한 나머지 argv 원소(issue/create/--title/title/--body/--label/labels)
      // 어디에도 marker 온전체 부재.
      const nonBodyElems = plan.argv.filter((el) => el !== body);
      expect(nonBodyElems.every((el) => !el.includes(marker))).toBe(true);
    });

    it("create body 안에서 marker 라인은 정확히 1회만 등장(body.split(marker).length === 2, 중복 0)", () => {
      const { descriptor, plan } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
  ...
  describe("핵심 2 — run 분포 변별(두 지점 동반 변화 + 여전히 상호 byte-identical)", () => {
    it("(a) 서로 다른 run(A/B) 각각 create-branch chain → 각 run 안에서 (searchMarker, createBodyHead) 는 서로 byte-identical", () => {
      const chainA = assembleCreateBranch(evalOutcome(), collectOutcome(), {
        ...RUN_A,
      });
      const chainB = assembleCreateBranch(evalOutcome(), collectOutcome(), {
        ...RUN_B,
      });

      const searchMarkerA = searchMarkerOf(chainA.searchArgv);
      const createHeadA = createBodyHeadLineOf(chainA.plan.argv);
      const searchMarkerB = searchMarkerOf(chainB.searchArgv);
      const createHeadB = createBodyHeadLineOf(chainB.plan.argv);

      expect(searchMarkerA).toBe(createHeadA);
      expect(searchMarkerB).toBe(createHeadB);
    ...
    it("(b) run A ≠ run B → searchMarker 두 값 서로 다름 AND createBodyHead 두 값 서로 다름(두 지점이 run 에 따라 함께 변함 — run-token 단일 source 동반 관통)", () => {
      const chainA = assembleCreateBranch(evalOutcome(), collectOutcome(), {
        ...RUN_A,
      });
      const chainB = assembleCreateBranch(evalOutcome(), collectOutcome(), {
      ...
      expect(createBodyHeadLineOf(chainA.plan.argv)).not.toBe(
        createBodyHeadLineOf(chainB.plan.argv),
      );
    });
  });

  describe("핵심 3 — leg outcome 무관 marker 안정(멱등 근거)", () => {
    it("동일 run + leg outcome 조합만 다르게(pass/pass vs fail/skip) → searchMarker 와 createBodyHead 가 서로 byte-identical 하고, 두 값 모두 leg outcome 변경 전과 동일(marker 는 run-token 만의 함수)", () => {
      const passChain = assembleCreateBranch(
        { leg: "eval", action: "run", passed: true },
        { leg: "collect", action: "run", passed: true },
        { ...RUN_A },
      );
      const mixedChain = assembleCreateBranch(
      ...
      const passCreate = createBodyHeadLineOf(passChain.plan.argv);
      const mixedSearch = searchMarkerOf(mixedChain.searchArgv);
      const mixedCreate = createBodyHeadLineOf(mixedChain.plan.argv);

      // 각 chain 안에서 두 지점 상호 identical.
      expect(passSearch).toBe(passCreate);
      expect(mixedSearch).toBe(mixedCreate);
  ...
  describe("branch — update 분기 격리(create body argv 미산출)", () => {
    it("marker 를 body 에 담은 1+건 search stdout → plan.action.action === 'update' AND plan.argv 가 issue edit argv(argv[0]==='issue', argv[1]==='edit')로 create body argv(argv[1]==='create') 미산출", () => {
      // 먼저 marker 를 얻어 search hit stdout 을 합성.
      const base = assembleCreateBranch(evalOutcome(), collectOutcome(), {
        ...RUN_A,
      });
      const marker = base.descriptor.marker;
      const hitStdout = JSON.stringify([
        {
          number: 77,
          title: "기존 dual-leg run report 이슈 #77",
          body: `${marker}\n\n본문 일부`,
        },
      ]);

      const { plan } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
        { searchStdout: hitStdout },
      );

      expect(plan.action.action).toBe("update");
      expect(plan.argv[0]).toBe("issue");
      expect(plan.argv[1]).toBe("edit");
      // create-branch 의 issue create argv 는 update 분기에서 생성되지 않는다.
      expect(plan.argv[1]).not.toBe("create");
    });
  });

  describe("핵심 4 — single-source 독립 재유도 + 결정론", () => {
    it("컴포저·searchArgv 빌더 재호출 없이 descriptor.marker 를 진실의 원천으로 삼아 searchMarker·createBodyHead 가 그 descriptor.marker 와 각각 deep-equal — 두 지점 marker 가 하드코딩이 아니라 descriptor.marker 파생", () => {
      const { descriptor, searchArgv, plan } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
      ...
      expect(createBodyHeadLineOf(plan.argv)).toEqual(descriptor.marker);
    });

    it("결정론 — 동일 입력 두 번 chain → 두 (searchMarker, createBodyHead) 쌍 deep-equal(byte-identical)", () => {
      const first = assembleCreateBranch(evalOutcome(), collectOutcome(), {
        ...RUN_A,
      });
      const second = assembleCreateBranch(evalOutcome(), collectOutcome(), {
        ...RUN_A,
      });
      expect(searchMarkerOf(first.searchArgv)).toEqual(
        searchMarkerOf(second.searchArgv),
      );
      expect(createBodyHeadLineOf(first.plan.argv)).toEqual(
        createBodyHeadLineOf(second.plan.argv),
      );
    });
  });

  describe("error path / negative cases — 예외 분기마다 각 1+", () => {
    it("(a) searchArgv 검색 marker drift 검출 — chain 산출 searchArgv 를 복제해 검색 marker 원소를 변형한 synthetic argv 의 검색 marker 가 create body 선두 marker 와 !==(cross-call byte-identical 위반 검출)", () => {
      const { searchArgv, plan } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
      );
      const createHead = createBodyHeadLineOf(plan.argv);
      // 검색 marker 원소 위치를 변형한 synthetic 복제.
      const matchIdx = searchArgv.indexOf("--match");
      const drifted = [...searchArgv];
      drifted[matchIdx + 2] = `${drifted[matchIdx + 2]}-DRIFT`;
      expect(searchMarkerOf(drifted)).not.toBe(createHead);
    });

    it("(b) create body 선두 marker drift 검출 — create plan.argv 의 body 원소를 복제해 선두 marker 라인을 변형한 synthetic body 의 선두 라인이 searchArgv 검색 marker 와 !==", () => {
      const { searchArgv, plan } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
    ...
    it("(c) descriptor guard 상류 차단 — run.gitSha 빈/공백이면 descriptor guard throw 로 marker·searchQuery·body 자체 미산출(chain 조립 차단)", () => {
      expect(() =>
        assembleCreateBranch(evalOutcome(), collectOutcome(), {
          gitSha: "   ",
    ...
    it("(d) searchArgv 빌더 guard 상류 차단 — commandArgs.searchQuery(= marker) 빈/공백인 commandArgs 를 searchArgv 빌더에 직접 넣으면 throw(빈 검색질의 전체 매칭 사고 차단)", () => {
      ...
      const { commandArgs } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
      );
    ...
    it("(e) 컴포저 파서 guard 상류 차단(create 분기 argv 미산출) — 비-JSON('{')/비배열 JSON('{}') stdout → 파서 throw 전파로 plan(및 create body argv) 미산출", () => {
      expect(() =>
        assembleCreateBranch(
          evalOutcome(),
          collectOutcome(),
          { ...RUN_A },
          {
  ...
  describe("결정론 · 무공유 · no-mutation", () => {
    it("동일 (run, leg outcomes) 입력 chain 두 번 → searchArgv·plan.argv 가 두 번 deep-equal(byte-identical)", () => {
      const first = assembleCreateBranch(evalOutcome(), collectOutcome(), {
        ...RUN_A,
      });
      const second = assembleCreateBranch(evalOutcome(), collectOutcome(), {
    ...
    it("no-mutation — chain 호출이 commandArgs(중첩 createArgs/labels 포함) 및 run 을 mutate 0(호출 전후 JSON snapshot deep-equal)", () => {
      ...
      const { commandArgs } = assembleCreateBranch(evalLeg, collectLeg, run);
  ...
  describe("raw / credential 누출 0(R-59 / REQ-059)", () => {
    it("searchArgv·create-branch plan.argv 의 어느 원소도 GH_TOKEN/PAT/ghp_/--token/GITHUB_TOKEN 어휘 미포함", () => {
      const { searchArgv, plan } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
    ...
    it("marker(= 검색질의·create body 선두 라인)가 안정 run-token(`${dateToken}@${gitSha}` + 고정 prefix)만 담고 raw 활동 narrative·credential 미포함", () => {
      const { descriptor, searchArgv, plan } = assembleCreateBranch(
        evalOutcome(),
        collectOutcome(),
        { ...RUN_A },
      ...
      expect(createHead).toContain(runTokenOf(RUN_A));
      // narrative·credential 어휘 미포함.
      expect(searchMarker).not.toContain("narrative");
      expect(createHead).not.toContain("narrative");
      expect(descriptor.marker).not.toContain("ghp_");
    });

    it("leg outcome.specPath 에 sentinel 을 넣어도 searchArgv 검색 marker·create body 선두 marker 에 sentinel 미누출(marker 는 run-token 파생만)", () => {
      ...
      const { searchArgv, plan } = assembleCreateBranch(evalLeg, collectLeg, {
        ...RUN_A,
      });
      expect(searchMarkerOf(searchArgv)).not.toContain(sentinel);
      expect(createBodyHeadLineOf(plan.argv)).not.toContain(sentinel);
    });
