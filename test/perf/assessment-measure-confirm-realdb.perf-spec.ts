...
describe("S2 measure→confirm-or-compare perf-spec — 실 DB round-trip baseline 확정·비교 (GET /api/assessments, period optional 분기, REQ-048)", () => {
  ...
  describe("체크인 baseline 실측 clock 관찰(ci-realdb-assessment-read)", () => {
    ...
    // step 요약 배선 **실호출** — ADR-0056 §Decision 3 (b) 의 "상대 회귀를 로그와 step 요약으로
    // 가시화" 계약을 person route(T-1616)가 연 정본에 이어 **두 번째 route** 로 확산한다.
    // 구조만 승계하고 국면은 본 route 고유 축(**`period` optional 분기** · **실 JWT guard 401
    // 전량 표본**)으로 갈아끼운다 — 주입값 형태 위반 · `null` deps 전파 · 기본값 결선 같은 일반
    // 계약 국면은 helper colocated spec(`checkin-baseline-spec-wiring.spec.ts`)과 person route 가
    // 이미 cover 하므로 복제하지 않는다(T-1575 중복 국면 삭제 선례). 요약 본문 문자열 ·
    // 환경변수명 · 판별 슬러그는 **재작성 0**(전량 위임 계약 + import 상수). **exit code 불변** —
    // 요약 경로의 실패(포매터 · append 예외)는 위임이 삼켜 판별 union 으로만 보고되므로
    // (T-1614 계약) 본 배선이 perf 스위트를 red 로 만드는 새 경로를 만들지 않는다.
    describe("step 요약 배선 실호출(emitCheckinStepSummaryForSpec)", () => {
      /** 요약 heading 문구 — 국면마다 리터럴을 반복하지 않도록 이 상수 하나만 쓴다. */
      const STEP_SUMMARY_TITLE = "체크인 baseline — 실 DB assessment read";
      /**
       * 요약 축 전용 fixture label — 실측 label(`ci-realdb-assessment-read`) · 배선 label
       * (`realdb-assess-mc`) 과 분리해 체크인될 baseline 파일 경로와 겹치지 않는다.
       */
      const summaryEnv: BaselineEnvMeta = {
        label: "realdb-assess-step-summary",
        concurrency: 1,
      };
      /**
      ...
      /** 기준 baseline · 기본 candidate 공용 probe — 매칭 0 건이라 200 + 빈 배열(seed 무의존). */
      const probe = (): RequestFn => read("?personId=checkin-summary-probe");
      ...
      const comparedFor = async (
        ...
        const baseline = await measureBaselineCandidate(probe(), summaryEnv, {
          iterations: SUMMARY_ITER,
          now: createStepClock(10),
        });
      ...
      // happy 겸 분기 (1) — 결정적 append 경로. 임시 트리 안에서만 compared 를 만들고 주입 env 로
      // 태워 append 가 정확히 1 회 불린다.
      it("happy · 분기 (1): compared outcome + 주입 env → appended · 경로 일치 · append 1 회", async () => {
      ...
      it("error (a): append 가 던져도 전파 0 + failed/append-threw 로만 보고", async () => {
        const outcome = await comparedFor(await summaryCandidate());
        const append = jest.fn(() => {
          throw new Error("realdb-assess-step-summary-append");
        });
      ...
      // error (b) — **본 route 고유 guard 축**. Cookie 미부착이라 전량 401 이고 성공 표본이 0 인
      // 오류 표본이어도 요약 경로는 관찰-only 로 끝난다.
      it("error (b): 401 전량 표본(errorRate=1 · count=0) outcome 도 throw 0 + 동일 판별 union", async () => {
        const id = await seed();
        const candidate = await summaryCandidate(
          read(`?personId=${id}`, false),
        );
      ...
      // 분기 (2) + (3) — append 를 0 회로 두는 단락 세 갈래를 한 국면에 모은다(person route 의
      // 9 개 국면을 그대로 복제하지 않는다 — 같은 단락 축이라 국면을 쪼갤 이득이 없다).
      it("분기 (2)(3): 토글 off → not-compared, compared 여도 env 부재 → env-absent · 공백-only → env-blank(각 append 0 회)", async () => {
        const off = checkForSummary(await summaryCandidate(), {});
        expect(off).toMatchObject({ status: "skipped", reason: "disabled" });
        const offAppend = jest.fn();
        expect(
          emitCheckinStepSummaryForSpec(
            off,
            STEP_SUMMARY_TITLE,
            depsWith(summaryPathOf("step-summary-off.md"), offAppend),
          ),
        ).toEqual({ status: "skipped", reason: "not-compared" });

        const compared = await comparedFor(await summaryCandidate());
        ...
        for (const spy of [offAppend, absentAppend, blankAppend]) {
      ...
      // negative (b) — **본 route 고유 `period` 분기 축**. 지정(week 3 행) · 미지정(전체 5 행)
      // 두 표본의 outcome 을 연속 emit 해도 두 호출이 서로 독립이다(부작용 누적 0 · 인자 변형 0).
      it("negative (b): period 지정 · 미지정 두 표본 연속 emit → append 각 1 회(합 2) · 인자 변형 0 · 상태 비이월", async () => {
        const id = await seed();
        const week = await summaryCandidate(
          read(`?personId=${id}&period=week`),
        );
        expect(rows()).toHaveLength(WEEK_ROWS);
        const all = await summaryCandidate(read(`?personId=${id}`));
        expect(rows()).toHaveLength(TOTAL_ROWS);

        const weekOutcome = await comparedFor(week);
        const allOutcome = await comparedFor(all);
        const snapshots = [
          JSON.stringify(weekOutcome),
          JSON.stringify(allOutcome),
        ];
        const target = summaryPathOf("step-summary-neg-b.md");
        const append = jest.fn();
        const deps = depsWith(target, append);
        ...
        // 두 번째 호출이 첫 번째의 상태를 이어받지 않는다 — 판정도 append 횟수도 각각 독립이다.
        expect(first).toEqual({ status: "appended", path: target });
        expect(second).toEqual(first);
        expect(append).toHaveBeenCalledTimes(2);
        expect(append.mock.calls[1][0]).toBe(target);
        // 인자 변형 0 — 두 outcome 객체가 emit 전후로 동일하다.
        expect([
          JSON.stringify(weekOutcome),
          JSON.stringify(allOutcome),
        ]).toEqual(snapshots);
