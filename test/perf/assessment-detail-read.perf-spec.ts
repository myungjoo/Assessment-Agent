// assessment-detail-read.perf-spec.ts — S2 조회 latency harness 의 *열일곱 번째 실
// perf-spec* 이자 **세 번째 path-param detail(:id) read** 배선.
// (T-0846, load-resilience-test-plan §5 follow-up #17 / REQ-048, 조회 p95 < 3s)
//
// 목적: T-0828(percentile/summarizeLatency/errorRate 순수 primitive) + T-0829
// (collectLatencySamples/assertS2Threshold 순수 orchestration)가 신설하고 T-0830~T-0845
// 가 16 개 조회 endpoint(list/query/self-read 14 + summary :id detail + group :id detail)
// 에 배선한 collector 를, `AssessmentController` 의 **detail** endpoint 인
// `GET /api/assessments/:id` (`findOne` → `service.findById(id)` — 단일 Assessment 상세,
// row 부재 시 service 가 `NotFoundException` throw → 404 자동 mapping)에 배선한다.
// T-0844(summary :id)·T-0845(group :id) 에 이어 **세 번째 path-param `:id` detail read**
// 이자 평가 자료(Assessment) 상세 조회 경로에서 harness 재사용을 실증한다. jest-perf.json
// (`testRegex: test/perf/.*\.perf-spec\.ts$`)에 매칭돼 `pnpm test:perf` 로만 실행되며
// (기본 `pnpm test` 는 `.spec.ts$` 만 매칭 → picking 0), 앞선 16 perf-spec 과 함께
// 열일곱 다 실행된다.
//
// 앞선 detail(:id) spec(summary/group)과의 차이(본 spec 고유 특성): 배선 대상 controller 가
// `AssessmentController` 로, 배선 대상 메서드가 `service.findById` 로 바뀐다.
// `AssessmentController` 의 detail 핸들러(`@Get(":id") findOne`)는 `@Param("id")` 로 받은
// id 를 `service.findById(id)` 로 raw forward 하고(controller 자체 분기 없음), row 존재 시
// 200(단일 Assessment), row 부재 시 service 가 `NotFoundException`(404)을 던진다. detail
// 핸들러는 `@UseGuards(JwtAuthGuard, RolesGuard)` + `@Roles("User")` 가드 스택을 적용하므로
// summary-detail-read(T-0844)/group-detail-read(T-0845) 의 `overrideGuard` 부트스트랩
// 패턴을 mirror 한다(self-read 가 아니므로 req.user 박제 불요 — canActivate true 만으로
// 충분). non-2xx 분류 실증은 mocked `findById` 가 `NotFoundException`(404)/일반 예외(500)
// 를 던져 endpoint 가 404/500 을 반환하는 error path 로 커버한다.
//
...
//   - `GET /api/assessments?personId=` list/query read 배선 — 별도 slice.
...
// mock AssessmentService — controller 가 주입받는 4 primitive 중 detail 경로가 실제로
...
describe("S2 조회 latency perf-spec — AssessmentController detail(:id) 배선 (REQ-048)", () => {
  ...
  // 상세 조회 endpoint(`GET /api/assessments/:id`)를 1회 호출하고 collector 가 소비할
  ...
  // 단일 Assessment 정상 응답 shape (mock findById resolve 값).
  const okAssessment = {
    id: "a-1",
    personId: "p-1",
    period: "week",
  ...
  describe("happy path — mock service 정상 응답(200)", () => {
    it("정상 200 응답 N회 → total===N, failures===0, samplesMs.length===N, assertS2Threshold pass", async () => {
      // mock 이 단일 Assessment 반환 → controller 가 200 + JSON object.
      service.findById.mockResolvedValue(okAssessment);
      const N = 5;
  ...
  describe("error path — mock service 예외 → endpoint non-2xx", () => {
    ...
    it("findById 가 NotFoundException → 404 응답 N회 → 전부 failures, assertS2Threshold pass===false + errorRate 사유", async () => {
      ...
      const result = await collectLatencySamples(readRequest("a-missing"), N);
  ...
  describe("negative cases 충분 cover", () => {
    ...
    it("(c) mixed — 4회 중 1회만 404(NotFound) → failures===1 부분 집계 정확, samplesMs===3", async () => {
      let call = 0;
      // 2번째 호출만 NotFoundException(404), 나머지는 정상 Assessment(200).
