...
- **잔여** — 실측 범위는 endpoint 15 개(조회 route 31)뿐이다. perf-spec 63 개 중 read 계열 glob 은 51 개
  ...
  5 건 한정으로 착수** 됐다 — T-1592 가 `test/perf/baselines/baseline-ci-realdb-person-read.json`
  ...
  `GET /api/assessments` 의 T-1600 실측 20 표본 전사) 을, T-1603 이
  `baseline-ci-realdb-contribution-read.json`(`env.label = ci-realdb-contribution-read` —
  slice 27 `GET /api/contributions` 의 T-1602 실측 20 표본 전사) 을, T-1605 가
  `baseline-ci-realdb-summary-read.json`(`env.label = ci-realdb-summary-read` —
  slice 25 `GET /api/summaries` 의 T-1604 실측 20 표본 전사) 을, T-1607 이
  `baseline-ci-realdb-app-root-read.json`(`env.label = ci-realdb-app-root-read` —
  slice 28 `GET /api` 의 T-1606 실측 20 표본 전사) 을 **다섯 건만** 저장소에
...
- **임계값 3000ms 는 불변** — `DEFAULT_P95_MAX_MS = 3000`(REQ-048)을 바꾸지 않는다. slice 1~24 는
  ...
  `ci-realdb-person-read` 와 T-1601 이 넣은 `ci-realdb-assessment-read`, T-1603 이 넣은
  `ci-realdb-contribution-read`, T-1605 가 넣은 `ci-realdb-summary-read`, T-1607 이 넣은
  `ci-realdb-app-root-read` **5 건뿐** 이며
