...
- **잔여** — 실측 범위는 endpoint 15 개(조회 route 31)뿐이다. perf-spec 63 개 중 read 계열 glob 은 51 개
  ...
  3 건 한정으로 착수** 됐다 — T-1592 가 `test/perf/baselines/baseline-ci-realdb-person-read.json`
  ...
  slice 27 `GET /api/contributions` 의 T-1602 실측 20 표본 전사) 을 **세 건만** 저장소에
...
- **임계값 3000ms 는 불변** — `DEFAULT_P95_MAX_MS = 3000`(REQ-048)을 바꾸지 않는다. slice 1~24 는
  ...
  `ci-realdb-contribution-read` **3 건뿐** 이며
