// part-detail-read.perf-spec.ts — S2 조회 latency harness 의 *열아홉 번째 실 perf-spec*
// 이자 **다섯 번째 path-param detail(:id) read** 배선(첫 Part entity 상세 조회 경로).
// (T-0848, load-resilience-test-plan §5 follow-up #19 / REQ-048, 조회 p95 < 3s)
//
// 목적: T-0828(percentile/summarizeLatency/errorRate 순수 primitive) + T-0829
// (collectLatencySamples/assertS2Threshold 순수 orchestration)가 신설하고 T-0830~T-0847
// 가 18 개 조회 endpoint(list/query/self-read 14 + summary·group·assessment·person :id
// detail 4)에 배선한 collector 를, `PartController` 의 **detail** endpoint 인
// `GET /api/parts/:id` (`findById` → `service.findById(id)` — 단일 Part 상세, row 부재
// 시 service 가 `NotFoundException` throw → 404 자동 mapping)에 배선한다. 직전
// T-0847(person :id) 이 **네 번째 path-param `:id` detail read** 였고, 본 spec 은
// **다섯 번째 detail(:id) read** 이자 **첫 Part entity 상세 조회** 경로라, harness 가
// 단일 상세 조회(404 분기 있는 detail) 경로에서 재사용됨을 한 slice 더 실증한다.
// jest-perf.json(`testRegex: test/perf/.*\.perf-spec\.ts$`)에 매칭돼 `pnpm test:perf`
// 로만 실행되며(기본 `pnpm test` 는 `.spec.ts$` 만 매칭 → picking 0), 앞선 18 perf-spec
// 과 함께 열아홉 다 실행된다.
//
// 앞선 spec 과의 차이(본 spec 고유 특성): (1) 배선 대상이 part-read(list, T-0835) 의
// `findAll`(`GET /api/parts`) → `findById`(`GET /api/parts/:id`) 로, list read 에서
// detail(:id) read 로 바뀐다. (2) PartController 는 `@UseGuards`/`@Roles` 를 부착하지
// 않는다(part.controller.ts L50~58 은 `@Controller` + `@UsePipes(ValidationPipe)` 만 —
// auth credential 미박제 정책). 따라서 본 spec 은 group-detail-read(T-0845) 와 동일하게
// `overrideGuard` 없이 controller 를 순수 부트스트랩한다(part-read(list) 와 동일 패턴 —
// 적용해도 no-op). ValidationPipe 는 GET :id 경로에 body 검증이 없어(단순 `@Param` raw
// forward) 배선에 영향 없다. `PartController.findById` 는 `@Param("id")` 로 받은 id 를
// `service.findById(id)` 로 raw forward 하고(controller 자체 분기 없음), row 존재 시
// 200(단일 Part), row 부재 시 service 가 `NotFoundException`(404)을 던진다. non-2xx
// 분류 실증은 mocked `findById` 가 `NotFoundException`(404)/일반 예외(500)를 던져
// endpoint 가 404/500 을 반환하는 error path 로 커버한다.
//
// 결정론 전략 (Acceptance — 실 DB·실 LLM·외부 I/O 무의존):
//   - `PartService` 는 mock(`useValue`) — DB round-trip 없이 controller ↔ collector
...
//   - `GET /api/parts/:id/persons`(findPersons) 등 sub-resource read 배선 — 별도 후속 slice.
...
// mock PartService — controller 가 주입받는 메서드 중 detail 경로가 실제로 호출하는
// 것은 `findById` 뿐. 각 test 가 mockResolvedValue / mockRejectedValue 로 응답을 제어해
// endpoint status(200 / 404 / 500)를 결정론적으로 만든다. (PartController 는 guard
// 미적용이라 인증/인가 분기 노이즈가 없다.)
type MockPartService = {
  findById: jest.Mock;
};

describe("S2 조회 latency perf-spec — PartController detail(:id) 배선 (REQ-048)", () => {
  ...
  // 단일 Part 상세 조회 endpoint(`GET /api/parts/:id`)를 1회 호출하고 collector 가
  ...
  describe("happy path — mock service 정상 응답(200)", () => {
    it("정상 200 응답 N회 → total===N, failures===0, samplesMs.length===N, assertS2Threshold pass", async () => {
      // mock 이 단일 Part 반환 → controller 가 200 + JSON object.
      service.findById.mockResolvedValue({ id: "p-1", name: "인프라파트" });
      const N = 5;
  ...
  describe("error path — mock service 예외 → endpoint non-2xx", () => {
    ...
    it("findById 가 NotFoundException → 404 응답 N회 → 전부 failures, assertS2Threshold pass===false + errorRate 사유", async () => {
      ...
      service.findById.mockRejectedValue(
        new NotFoundException("part not found: p-missing"),
      );
      const N = 4;
  ...
  describe("negative cases 충분 cover", () => {
    ...
    it("(a) 존재하지 않는 id 조회 → findById NotFoundException(404) → failures 로 분류, pass===false", async () => {
      service.findById.mockRejectedValue(
        new NotFoundException("part not found: p-does-not-exist"),
      );
      const N = 3;
    ...
    it("(c) mixed — 4회 중 1회만 404(NotFound) → failures===1 부분 집계 정확, samplesMs===3", async () => {
      let call = 0;
      // 2번째 호출만 NotFoundException(404), 나머지는 정상 Part(200).
      service.findById.mockImplementation(async () => {
        call += 1;
        if (call === 2) {
          throw new NotFoundException("part not found: p-1");
        }
        return { id: "p-1", name: "인프라파트" };
      });
      const N = 4;
