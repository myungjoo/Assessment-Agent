...
//     성격의 국면 3 개를 factory 로 승격했다. T-1575 가 그 중복분 3 개를 삭제해 지금 남은
//     기본 바인딩 고유분은 factory 에 없는 seam 2 개뿐이다(합 4 개 — 아래 고유 국면 블록 주석).
...
describe("S2 measure→confirm-or-compare perf-spec — SummaryController 조회(GET /api/summaries?personId) 실 supertest + guard override + fs baseline round-trip 배선 (REQ-048)", () => {
  ...
  describe("happy path — established(최초 확정 write) / compared(로드·비교) 양 분기 실 실행", () => {
    ...
    it("(b) compared — 같은 baseDir 재호출 → outcome=compared + comparison·report, 동일 합성 latency 라 regressed=false(주입 clock 결정론, 실 GET /api/summaries 200 반영)", async () => {
      const baseDir = baselineDir("baselines");
  ...
  // colocated spec(`checkin-baseline-spec-suite.spec.ts`)의 책임이라 여기서 중복 작성하지
  // 않는다 — 본 spec 은 factory 를 **유효 options 로만** 호출한다.
  registerCheckinBaselineWiringSuite({
    envMeta: env,
    // 측정은 collector 위임(주입 clock 으로 결정론화) — 교체 전 조립 그대로.
    measure: (stepMs) =>
  ...
  // 위 factory 호출과 별도로 **spec 고유 통합 국면**(주입 토글 2 + 기본 바인딩 2, 합 4 개)을
  // 남긴다. 앞 2 개는 토글을 전역 변수 대신 `processEnv` 주입으로 태워(기본값 바인딩이 명시 값을
  // 우선) 전역 오염 · 원복 책임을 아예 만들지 않는다 — factory 의 토글 hook 과 이중으로 겹치지
  // 않는다. 뒤 2 개(T-1572 가 넣은 5 개 중 T-1575 정리 후 잔존분)는 반대로 **기본 바인딩** 자체를
  // 검증 대상으로 삼아 전역 토글을 `withGlobalFlag` 의 `try/finally` 안에서만 세팅하고 진입 전
  // 값으로 정확히 원복한다.
  describe("체크인 baseline 확인 배선 — spec 고유 통합 국면 (ADR-0056 §Follow-ups (b))", () => {
    ...
    it("negative (c) 어떤 국면에서도 저장소 실경로 test/perf/baselines 아래 내용이 변하지 않음", async () => {
      ...
      const before = snap();
      const repoRoot = baselineDir("repo");
      ...
      checkCheckinBaselineForSpec({ envMeta: env, candidate, processEnv: {} });
      checkCheckinBaselineForSpec({
        envMeta: env,
        candidate,
        repoRoot,
        processEnv: enabledEnv,
      });
      seedCheckinBaselineFixture(env, repoRoot, candidate);
      expect(
        checkCheckinBaselineForSpec({
          envMeta: env,
          candidate,
    ...
    // ── T-1572 고유분 중 잔존 — **토글 on × 저장소 실경로 기본 바인딩** 국면 2 개 ──
    // 아래 국면들은 `repoRoot` · `processEnv` 를 **생략**해 어댑터 기본값(전역 `process.env` ×
    // `defaultCheckinRepoRoot()`)을 그대로 탄다. T-1573 이 같은 성격의 국면 3 개를 factory 로
    // 승격해 T-1572 고유분 5 개 중 3 개가 1:1 중복이 됐고, T-1575 가 그 3 개를 삭제했다 —
    // 삭제분 → factory 이관 대응: `happy (e)` → `happy (c) 토글 on × 기본 바인딩` ·
    // `분기 (g)` → `분기 (d) 기본 바인딩 존재-조회` · `negative (h)` → `negative (c) 기본 바인딩
    // on/off 연속`(단언 전량 포함 — 정본 대조 확인).
    // 남는 2 개는 factory 국면 10 개에 **없는 고유 seam** 이라 유지한다:
    //   - `error (f)` — 기본 바인딩 × 경로 해석이 fs 접근보다 **선행**함을 실경로 목록 불변으로
    //     단언한다(실경로 mkdir · write 0). factory 의 `error (1)` 은 임시 `repoRoot`,
    //     `error (2)` 는 seed 인자 검증이라 이 조합이 없다.
    //   - `negative (i)` — spec 지역 `withGlobalFlag` 의 `try/finally` 원복 계약(정상 · 예외 국면
    //     모두 진입 전 값 복귀)이 검증 대상이다. factory 는 자기 `beforeEach`/`afterEach` 로
    //     원복하므로 이 지역 wrapper 의 계약은 factory 국면이 대신 실행하지 못한다.
