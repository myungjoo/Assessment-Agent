// contribution-detail-read.perf-spec.ts — S2 조회 latency harness 의 *스무 번째 실
// perf-spec* 이자 **여섯 번째 path-param detail(:id) read** 배선(첫 Contribution entity
// 상세 조회 경로).
// (T-0849, load-resilience-test-plan §5 follow-up #20 / REQ-048, 조회 p95 < 3s)
//
// 목적: T-0828(percentile/summarizeLatency/errorRate 순수 primitive) + T-0829
// (collectLatencySamples/assertS2Threshold 순수 orchestration)가 신설하고 T-0830~T-0848
// 가 19 개 조회 endpoint(list/query/self-read 14 + summary·group·assessment·person·part
// :id detail 5)에 배선한 collector 를, `ContributionController` 의 **detail** endpoint 인
// `GET /api/contributions/:id` (`findOne` → `service.findById(id)` — 단일 Contribution
// 상세, row 부재 시 service 가 `NotFoundException` throw → 404 자동 mapping)에 배선한다.
// 직전 T-0848(part :id) 이 **다섯 번째 path-param `:id` detail read** 였고, 본 spec 은
// **여섯 번째 detail(:id) read** 이자 **첫 Contribution entity 상세 조회** 경로라,
// harness 가 단일 상세 조회(404 분기 있는 detail) 경로에서 재사용됨을 한 slice 더
// 실증한다. jest-perf.json(`testRegex: test/perf/.*\.perf-spec\.ts$`)에 매칭돼
// `pnpm test:perf` 로만 실행되며(기본 `pnpm test` 는 `.spec.ts$` 만 매칭 → picking 0),
// 앞선 19 perf-spec 과 함께 스무 다 실행된다.
//
// 앞선 detail(:id) spec 과의 차이(본 spec 고유 특성): 배선 대상 controller 가
// `ContributionController` 로, 배선 대상 메서드가 `service.findById` 로 바뀐다.
// `ContributionController` 의 detail 핸들러(`@Get(":id") findOne`)는 `@Param("id")` 로
// 받은 id 를 `service.findById(id)` 로 raw forward 하고(controller 자체 분기 없음), row
// 존재 시 200(단일 Contribution), row 부재 시 service 가 `NotFoundException`(404)을
// 던진다. detail 핸들러는 `@UseGuards(JwtAuthGuard, RolesGuard)` + `@Roles("User")` 가드
// 스택을 적용하므로 assessment-detail-read(T-0846) 의 `overrideGuard` 부트스트랩 패턴을
// mirror 한다(self-read 가 아니므로 req.user 박제 불요 — canActivate true 만으로 충분).
// non-2xx 분류 실증은 mocked `findById` 가 `NotFoundException`(404)/일반 예외(500)를
// 던져 endpoint 가 404/500 을 반환하는 error path 로 커버한다.
//
// 결정론 전략 (Acceptance — 실 DB·실 LLM·외부 I/O 무의존):
//   - `ContributionService` 는 mock(`useValue`) — DB round-trip 없이 controller ↔
...
//   - `GET /api/contributions?assessmentId=`(findByAssessment) 등 list/query read 배선 —
//     별도 slice.
...
// mock ContributionService — controller 가 주입받는 4 primitive 중 detail 경로가 실제로
...
describe("S2 조회 latency perf-spec — ContributionController detail(:id) 배선 (REQ-048)", () => {
  ...
  // 상세 조회 endpoint(`GET /api/contributions/:id`)를 1회 호출하고 collector 가 소비할
  ...
  // 단일 Contribution 정상 응답 shape (mock findById resolve 값).
  const okContribution = {
    id: "c-1",
    assessmentId: "a-1",
    sourceType: "commit",
    difficulty: "medium",
  ...
  describe("happy path — mock service 정상 응답(200)", () => {
    it("정상 200 응답 N회 → total===N, failures===0, samplesMs.length===N, assertS2Threshold pass", async () => {
      // mock 이 단일 Contribution 반환 → controller 가 200 + JSON object.
      service.findById.mockResolvedValue(okContribution);
      const N = 5;
  ...
  describe("error path — mock service 예외 → endpoint non-2xx", () => {
    ...
    it("findById 가 NotFoundException → 404 응답 N회 → 전부 failures, assertS2Threshold pass===false + errorRate 사유", async () => {
      ...
      service.findById.mockRejectedValue(
        new NotFoundException("contribution not found: c-missing"),
      );
      const N = 4;

      const result = await collectLatencySamples(readRequest("c-missing"), N);
  ...
  describe("negative cases 충분 cover", () => {
    ...
    it("(a) 존재하지 않는 id 조회 → findById NotFoundException(404) → failures 로 분류, pass===false", async () => {
      service.findById.mockRejectedValue(
        new NotFoundException("contribution not found: c-does-not-exist"),
      );
      const N = 3;

      const result = await collectLatencySamples(
        readRequest("c-does-not-exist"),
        N,
      );
    ...
    it("(c) mixed — 4회 중 1회만 404(NotFound) → failures===1 부분 집계 정확, samplesMs===3", async () => {
      let call = 0;
      // 2번째 호출만 NotFoundException(404), 나머지는 정상 Contribution(200).
      service.findById.mockImplementation(async () => {
        call += 1;
        if (call === 2) {
          throw new NotFoundException("contribution not found: c-1");
        }
        return okContribution;
      });
      const N = 4;
