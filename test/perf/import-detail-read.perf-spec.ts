// import-detail-read.perf-spec.ts — S2 조회 latency harness 의 *스물네 번째
// 실 perf-spec* 이자 **열 번째 path-param detail(:id) read** 배선.
// (T-0853, load-resilience-test-plan §5 follow-up #24 / REQ-048 조회 p95 < 3s, REQ-096
// Admin export/import 가시성)
//
// 목적: T-0828(percentile/summarizeLatency/errorRate 순수 primitive) + T-0829
// (collectLatencySamples/assertS2Threshold 순수 orchestration)가 신설하고 T-0830~T-0852
// 이 23 개 조회 endpoint(list/query/self-read 14 + summary·group·assessment·person·part·
// contribution·user·llm-provider-config·export :id detail 9)에 배선한 collector 를,
// `ImportController` 의 **detail** endpoint 인 `GET /api/admin/import/:id` (`findJob` →
// `service.findJob(id)` — 단건 status polling 조회, row 부재 시 service 의
// `findUniqueOrThrow` 가 P2025 → `NotFoundException`(404) 변환, 정상 시 단건 `ImportJob`
// (200)) 에 배선한다. 직전 export :id detail slice(T-0852)와 export↔import counterpart
// 로 대칭이며, harness 가 import 상세 조회 경로에서 재사용됨을 실증한다. jest-perf.json
// (`testRegex: test/perf/.*\.perf-spec\.ts$`)에 매칭돼 `pnpm test:perf` 로만 실행되며
// (기본 `pnpm test` 는 `.spec.ts$` 만 매칭 → picking 0), 앞선 23 perf-spec 과 함께 스물네
// 다 실행된다.
//
// 앞선 detail(:id) spec 과의 차이(본 spec 고유 특성):
//   - 배선 대상 controller 가 `ImportController`, 배선 대상 메서드가 `service.findJob`
//     으로 바뀐다. detail 핸들러(`@Get(":id") findJob`)는 `@Param("id")` 로 받은 id 를
//     `service.findJob(id)` 로 raw forward 하고(controller 자체 분기 없음), row 존재 시
//     200(단건 ImportJob), row 부재 시 service 가 P2025 → `NotFoundException`(404)로 변환.
//     export :id(T-0852)와 동형 — Export→Import controller/service 치환.
//   - detail 핸들러는 `@UseGuards(JwtAuthGuard, RolesGuard)` + `@Roles("Admin")` 로 **가드가
//     부착된 Admin :id detail** 이라 export-detail(T-0852)·llm-provider-config-detail
//     (T-0851)·assessment-detail(T-0846) 의 `overrideGuard(JwtAuthGuard)`·
//     `overrideGuard(RolesGuard)` 무력화 + `findJob` mock 패턴을 mirror 한다(user :id
//     (T-0850)는 controller 자체 403 분기였던 반면, 본 endpoint 는 RolesGuard 가 가드하는
//     것을 override 로 통과 — controller 자체 authorization 분기가 없어 req.user 박제
//     불요, canActivate true 만으로 충분).
//   - 같은 import 모듈 sibling spec(import-running-read.perf-spec.ts, T-0842)의 부트스트랩
//     (ImportController + `ImportJobService` mock 단일 주입, mock type 에 `findJob:
...
//   - `ImportJobService` 는 mock(`useValue`) — DB round-trip·실 Prisma 없이 controller ↔
...
//   - `GET /api/admin/import/running` list read 배선(별도 import-running-read.perf-spec.ts
//     이미 존재)·`GET /api/admin/import/modes` 배선 — 본 spec 은 `:id` detail 만.
//   - 다른 미배선 detail(:id) endpoint 배선 — 별도 slice(남은 controller findById/detail).
...
import { ImportRestoreService } from "../../src/import/import-restore.service";
...
// mock ImportJobService — controller 가 주입받는 메서드 중 detail 경로가 실제로 호출하는
// 것은 `findJob` 뿐(나머지 2 는 shape 정합용 jest.fn). 각 test 가 mockResolvedValue /
// mockRejectedValue 로 응답을 제어해 endpoint status(200 / 404 / 500)를 결정론적으로
// 만든다. findRunning/createJob 는 본 detail 경로가 호출하지 않지만, service shape 정합을
// 위해 jest.fn 으로 함께 둔다(호출 안 됨). sibling import-running spec 의 mock type 에
// 이미 `findJob: jest.Mock` 선언.
...
// 결정론적 ImportJob 한 건 — mock findJob 의 정상 반환값. findJob(id) 는 저장된 단건 job
// 을 raw forward 하므로 fixture 는 JSON 직렬화만 되면 충분(실 DB row 가 아니어도 latency
// 배선 측정에 충분). ImportJob 은 ExportJob 대비 mode(REPLACE/MERGE) + restoredRowCount
// 필드가 고유(prisma/schema.prisma model ImportJob).
function buildImportJob(overrides: Partial<ImportJob> = {}): ImportJob {
  return {
    id: "import-job-1",
    status: "SUCCEEDED",
    mode: "REPLACE",
    ...
    error: null,
    artifactRef: "artifact://import-job-1",
    restoredRowCount: 42,
    ...overrides,
  } as ImportJob;
}

describe("S2 조회 latency perf-spec — ImportController detail(:id) 배선 (REQ-048)", () => {
  ...
  beforeAll(async () => {
    ...
    const moduleRef: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        // T-1299 preview 배선으로 추가된 restore 의존 — 본 조회 경로는 호출하지 않으므로
        // DI 해석만 만족시키는 빈 mock 을 둔다 (계측 대상 불변).
        {
          provide: ImportRestoreService,
          useValue: { previewFromDump: jest.fn() },
        },
      ],
    })
  ...
  // 상세 조회 endpoint(`GET /api/admin/import/:id`)를 1회 호출하고 collector 가 소비할
  ...
  describe("happy path — mock service 정상 응답(200)", () => {
    it("정상 200 응답 N회 → total===N, failures===0, samplesMs.length===N, assertS2Threshold pass, findJob 가 요청 id 로 호출", async () => {
      // mock 이 단일 ImportJob 반환 → controller 가 200 + JSON object.
      service.findJob.mockResolvedValue(buildImportJob());
      const N = 5;
      ...
      expect(service.findJob).toHaveBeenCalledWith("import-job-1");
    });
  });
  ...
  describe("error path — mock service 예외 → endpoint non-2xx", () => {
    ...
    it("findJob 이 NotFoundException → 404 응답 N회 → 전부 failures, assertS2Threshold pass===false + errorRate 사유", async () => {
      ...
      service.findJob.mockRejectedValue(
        new NotFoundException("import job not found: import-missing"),
      );
      const N = 4;

      const result = await collectLatencySamples(
        readRequest("import-missing"),
        N,
      );
  ...
  describe("negative cases 충분 cover", () => {
    ...
    it("(a) 존재하지 않는 id 조회 → findJob NotFoundException(404) → failures 로 분류, pass===false", async () => {
      service.findJob.mockRejectedValue(
        new NotFoundException("import job not found: import-does-not-exist"),
      );
      const N = 3;

      const result = await collectLatencySamples(
        readRequest("import-does-not-exist"),
        N,
      );
    ...
    it("(c) mixed — 4회 중 1회만 404(NotFound) → failures===1 부분 집계 정확, samplesMs===3", async () => {
      let call = 0;
      // 2번째 호출만 NotFoundException(404), 나머지는 정상 ImportJob(200).
      service.findJob.mockImplementation(async () => {
        call += 1;
        if (call === 2) {
          throw new NotFoundException("import job not found: import-job-1");
        }
