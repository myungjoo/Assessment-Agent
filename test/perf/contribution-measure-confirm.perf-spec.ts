// contribution-measure-confirm.perf-spec.ts — S2 measure→confirm-or-compare 를 실 조회 endpoint
// (`ContributionController` `GET /api/contributions?assessmentId=<id>`)에 배선한
// *네 번째(이자 평가-결과 read 표면의 마지막 distinct) fs+HTTP 통합 perf-spec*.
// (T-0883, load-resilience-test-plan §5 follow-up #2 / REQ-048 조회 p95 < 3s)
//
...
// `GET /api/summaries?personId=<id>` 로 두 번째, T-0882(assessment-measure-confirm.perf-spec.ts)가
// `AssessmentController` `GET /api/assessments?personId=&period=` 로 세 번째 배선했다. 본 spec 은
// REQ-048 이 겨냥하는 **평가 결과 조회·시각화 경로**의 마지막 distinct read 표면인
// **`ContributionController` `GET /api/contributions?assessmentId=<id>`**(REQ-033 aggregate-level
// 기여 조회 = 시각화의 기여 내역)를 같은 loop 에 배선한다 — guard stack(JwtAuthGuard/RolesGuard)
// override + assessmentId 필수 query-param 분기(부재/빈 string→400)에 태워, harness 가 요약·평가·
// 기여 3 read 경로 전반에서 established/compared round-trip 을 실증하도록 한다.
//
// endpoint 선택 근거(S2 조회 경로의 대표 read — 네 번째이자 마지막 genuinely-distinct 경로):
//   - `ContributionController` `GET /api/contributions?assessmentId=` 는 (1) `JwtAuthGuard`/
//     `RolesGuard` 두 가드가 부착된 인증/인가 read 이고, (2) `@Query("assessmentId")` 가 필수라
//     부재/빈 string 시 controller 가 `BadRequestException`(400)을 강제하는 고유 query-param
//     예외 분기를 가진다. summary/assessment 와 다른 실 조회 route(다른 controller·다른 필수
//     query `assessmentId`·다른 400 분기)라 genuine 이다.
//   - `ContributionService` 는 mock(`useValue`) — DB round-trip 없이 controller ↔ collector 배선만
...
//     으로 도달한다.
//
// 직전 T-0882(assessment-measure-confirm.perf-spec.ts)와의 차이(본 spec 고유 특성):
//   - assessment 는 `?personId=` 필수 + `?period=` optional 다중-query 분기를 태웠지만, contribution 은
//     `?assessmentId=` 단일 필수 query 만 가진다(assessment 의 `period` optional 분기가 없어 그
//     부분만 덜어낸 형태). 400 강제 분기는 assessmentId 부재/빈 string 로 동일하게 태운다.
//
// 결정론 전략 (Acceptance — 실 DB·실 Prisma·실 LLM·외부 I/O 무의존):
//   - `ContributionService` 는 mock(`useValue`) — `findByAssessment` 은 mockResolvedValue 로
//     결정론적 배열을 반환한다. guard 는 `overrideGuard` 로 통과.
...
//   - 실패(non-2xx) 분기는 (1) 요청 wrapper 레벨 인위 503 주입, (2) **assessmentId 부재 실 400**
...
//   - 또 다른 endpoint(person/group/part/export/user 등 조직 구조 read) 동시 배선 / `@Get(":id")`
//     findOne(contribution-detail) 배선 / write route perf(POST/DELETE) / 실 Postgres·Prisma·LLM
...
//     ContributionController findByAssessment 조회 endpoint 1개만 top loop 에 태운 네 번째 통합
//     배선이며, 실 app 부트스트랩 + guard override + 임시 baseDir 격리 + loop 호출 + expect 만
//     책임진다.
...
import { ContributionController } from "../../src/user/contribution.controller";
import { ContributionService } from "../../src/user/contribution.service";
...
// mock ContributionService — 조회(findByAssessment) 경로가 실제 호출하는 것은 findByAssessment
// 뿐. 나머지 3 primitive 는 controller 부트스트랩에 필요해 형식적으로 채운다(조회 경로에서 미호출).
...
describe("S2 measure→confirm-or-compare perf-spec — ContributionController 조회(GET /api/contributions?assessmentId) 실 supertest + guard override + fs baseline round-trip 배선 (REQ-048)", () => {
  let app: INestApplication;
  let service: MockContributionService;

  /** 매 test 마다 새로 만드는 격리 임시 디렉토리 루트(afterEach 에서 재귀 삭제). */
  let tmpRoot: string;

  /** 결정론 env-meta fixture(label/concurrency 고정 → 파일명 slug 결정적). */
  const env: BaselineEnvMeta = {
    label: "ci-contribution-read",
    concurrency: 1,
  };

  /** 고정 assessmentId — 실 200 조회 경로가 결정론적으로 도달하도록 test 전반에 재사용. */
  const FIXED_ASSESSMENT_ID = "a-1";

  // 통과 guard — canActivate 가 항상 true. 인증/인가 layer 를 벗겨 harness 배선만 측정
  // (contributions e2e 는 실 guard stack 을 검증하는 별개 책임).
  ...
  beforeAll(async () => {
    ...
    // (contribution-read.perf-spec.ts 의 passGuard·overrideGuard 패턴 mirror).
    const moduleRef: TestingModule = await Test.createTestingModule({
      controllers: [ContributionController],
      providers: [{ provide: ContributionService, useValue: service }],
    })
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    // 기본 정상 반환 — 실 GET /api/contributions 요청이 200 + JSON 배열을 받도록.
    service.findByAssessment.mockResolvedValue([
      { id: "c-1", assessmentId: FIXED_ASSESSMENT_ID, sourceType: "commit" },
    ]);
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "s2-contribution-mc-"));
  });
  ...
  // 실 조회 endpoint(`GET /api/contributions?assessmentId=<id>`)를 1회 호출하고 collector 가
  // 소비할 { status } 를 반환하는 요청 함수. supertest 는 non-2xx 에도 reject 하지 않고 response
  // 를 resolve 하므로 status 로 성공 여부를 판정(collector 의 isSuccess 가 200~299 를 성공으로 분류).
  const readRequest =
    (assessmentId: string = FIXED_ASSESSMENT_ID): RequestFn =>
    async () => {
      const res = await request(app.getHttpServer()).get(
        `/api/contributions?assessmentId=${assessmentId}`,
      );
      return { status: res.status };
    };

  // assessmentId query 자체를 뺀 요청 함수 — ContributionController 가 controller 레벨에서
  // BadRequestException(400)을 강제하는 고유 실 예외경로(floor-case 에 없던 경로)를 태운다.
  const readRequestMissingAssessmentId: RequestFn = async () => {
    const res = await request(app.getHttpServer()).get("/api/contributions");
    return { status: res.status };
  };
  ...
  describe("happy path — established(최초 확정 write) / compared(로드·비교) 양 분기 실 실행", () => {
    it("(a) established — 빈 baseDir 첫 호출 → outcome=established + path, baseline JSON 실 write(round-trip 동치, 실 GET /api/contributions 200 반영)", async () => {
      ...
      // 실 GET /api/contributions 200 응답이 iterations 만큼 measure 에 반영됨(errorRate 0, count>0).
      expect(req).toHaveBeenCalledTimes(iterations);
      expect(service.findByAssessment).toHaveBeenCalledTimes(iterations);
  ...
  describe("flow / branch coverage — 부재→established vs 존재→compared, 옵션 위임", () => {
    ...
    it("opts.measure.iterations 지정 시 실 GET /api/contributions 요청 횟수가 그 값과 일치(요청 wrapper 호출 카운터)", async () => {
      const baseDir = baselineDir("baselines");
      const req = jest.fn(readRequest());
      await measureAndConfirmBaseline(req, env, baseDir, {
        measure: { iterations: 5 },
      });
      // iterations=5 → GET /api/contributions 5회 도달(measure 위임 검증). findByAssessment 도 5회 실호출.
      expect(req).toHaveBeenCalledTimes(5);
      expect(service.findByAssessment).toHaveBeenCalledTimes(5);
    });
  ...
  describe("negative cases 충분 cover", () => {
    it("(a) opts 미지정(undefined) → 기본 iterations 30·기본 compare tolerance 로 established 정상(기본치 경로 도달)", async () => {
      const baseDir = baselineDir("baselines");
      const req = jest.fn(readRequest());
      // 기본 iterations 30 이 measure 로 위임됨을 호출 횟수로 확인(실 GET /api/contributions 30회).
      const result = await measureAndConfirmBaseline(req, env, baseDir);
    ...
    it("(c) assessmentId 부재 요청(실 400) → errorRate 위반 candidate 가 throw 없이 established write(ContributionController 고유 query-param 예외경로, floor-case 에 없던 실 400)", async () => {
      const baseDir = baselineDir("baselines");
      // assessmentId query 를 뺀 실 요청 → controller 가 BadRequestException(400) 강제. collector 는
      // non-2xx(400)를 failure 로 분류 → errorRate 100% → pass=false. established 라 write 는 수행.
      const result = await measureAndConfirmBaseline(
        readRequestMissingAssessmentId,
        env,
        baseDir,
        { measure: { iterations: 4 } },
      );
      // 실 400 이 controller 레벨에서 강제됨(service 는 호출조차 안 됨 — 사전 분기).
      expect(service.findByAssessment).not.toHaveBeenCalled();
      expect(result.outcome).toBe("established");
      if (result.outcome === "established") {
        expect(fs.existsSync(result.path)).toBe(true);
        const persisted = parseBaselineReport(
    ...
    it("(d) 정상 assessmentId 요청은 established → compared 양쪽 도달, controller 가 service.findByAssessment(assessmentId) 로 위임", async () => {
      const baseDir = baselineDir("baselines-happy");
      const firstEstablished = await measureAndConfirmBaseline(
        readRequest(FIXED_ASSESSMENT_ID),
        env,
        baseDir,
        { measure: { iterations: 3 } },
      );
      expect(firstEstablished.outcome).toBe("established");
      // controller 가 assessmentId 를 그대로 findByAssessment 로 위임(단일 필수 query 분기).
      expect(service.findByAssessment).toHaveBeenLastCalledWith(
        FIXED_ASSESSMENT_ID,
      );
      const secondCompared = await measureAndConfirmBaseline(
        readRequest(FIXED_ASSESSMENT_ID),
        env,
        baseDir,
        { measure: { iterations: 3 } },
      );
      expect(secondCompared.outcome).toBe("compared");
    });
  ...
  registerCheckinBaselineWiringSuite({
    envMeta: env,
    // 측정은 collector 위임(주입 clock 으로 결정론화) — 실 GET /api/contributions 를 태운다.
    measure: (stepMs) =>
