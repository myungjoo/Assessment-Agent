// group-read.perf-spec.ts — S2 조회 latency harness 의 *다섯 번째 실 perf-spec*.
// (T-0834, load-resilience-test-plan §5 follow-up #5 / REQ-048, 조회 p95 < 3s)
//
...
// ContributionController 에, T-0833 이 PersonController 에 배선한 collector 를,
// **다섯 번째 조회 endpoint** 인 `GroupController` 의 `GET /api/groups`(REQ-048 Group
// 목록 조회)에 배선한다. harness 가 요약·평가·기여·인원·그룹 5 read 경로 전반에
// 재사용됨을 실증한다.
...
// assessment-read / contribution-read / person-read 와 함께 다섯 다 실행된다.
//
// 앞선 spec 과의 차이(본 spec 고유 특성): GroupController 는 `@UseGuards`/`@Roles`
// 를 부착하지 않는다(group.controller.ts L51/L159 — auth credential 미박제 정책).
// 따라서 본 spec 은 `overrideGuard` 없이 controller 를 순수 부트스트랩한다(적용해도
// no-op — person-read 와 동일). 또한 `GET /api/groups` 는 query-param 필수 분기(400)가
// 없는 단순 list read 라, non-2xx 분류 실증은 400 query 대신 **`GET /api/groups/:id`
// 의 404 분기**(mocked `findById` 이 `NotFoundException` throw)로 이어간다.
//
...
//   - `GET /api/groups/:id/persons`(findPersons) 배선 — findAll 을 주 measure 로,
...
// mock GroupService — controller 가 주입받는 메서드 중 조회(findAll/findById)만
// harness 에 필요. 각 test 가 mockResolvedValue / mockRejectedValue 로 응답을 제어해
// endpoint status(200 / 500 / 404)를 결정론적으로 만든다. (GroupController 는 guard
...
describe("S2 조회 latency perf-spec — GroupController 배선 (REQ-048)", () => {
  ...
  // Group 목록 조회를 1회 호출하고 collector 가 소비할 { status } 를 반환하는 요청
  ...
  // 단일 Group 상세 조회 요청 — mocked findById 이 NotFoundException 을 던지면 404 분기 도달.
  ...
  describe("happy path — mock service 정상 응답(200)", () => {
    it("정상 200 응답 N회 → total===N, failures===0, samplesMs.length===N, assertS2Threshold pass", async () => {
      // mock 이 Group 배열 반환 → controller 가 200 + JSON array.
      service.findAll.mockResolvedValue([{ id: "g-1", name: "백엔드팀" }]);
      const N = 5;
  ...
  describe("negative cases 충분 cover", () => {
    ...
    //     Group 0 은 목록 조회의 정상 결과(404 아님).
    it("(a) service 가 빈 배열([]) 반환(Group 0) → 200 성공 수집, pass===true", async () => {
    ...
    // (b) 404 분기 배선 — GET /api/groups/:id 에서 mocked findById 이 NotFoundException
    ...
    it("(b) GET /:id 에서 findById 이 404(NotFoundException) → collector 가 failures 로 분류, pass===false", async () => {
      service.findById.mockRejectedValue(
        new NotFoundException("group not found: g-1"),
      );
      const N = 3;
    ...
    it("(d) iterations===1 경계 → 단일 호출로도 harness 정상 동작", async () => {
      service.findAll.mockResolvedValue([{ id: "g-1" }]);
