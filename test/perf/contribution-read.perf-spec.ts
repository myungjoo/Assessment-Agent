// contribution-read.perf-spec.ts — S2 조회 latency harness 의 *세 번째 실 perf-spec*.
// (T-0832, load-resilience-test-plan §5 follow-up #3 / REQ-048, 조회 p95 < 3s)
//
...
// SummaryController 에, T-0831 이 AssessmentController 에 배선한 collector 를,
// **세 번째 조회 endpoint** 인 `ContributionController` 의
// `GET /api/contributions?assessmentId=`(REQ-033 aggregate-level 기여 조회)에 배선한다.
// harness 가 요약·평가·기여 3 read 경로 전반에 재사용됨을 실증한다.
...
// assessment-read 와 함께 셋 다 실행된다.
//
// AssessmentController 와의 공통점(본 spec 고유 검증): ContributionController.findByAssessment
// 는 `assessmentId` query 누락/빈 string 시 controller 자체가 `BadRequestException`(400) 을
// 강제하는 분기를 갖는다(assessment.controller.ts 의 personId 누락 패턴 mirror). harness 가
// 이 non-2xx(400) 응답도 `failures` 로 정확히 분류하는지 이어서 검증한다.
//
// 결정론 전략 (Acceptance — 실 DB·실 LLM·외부 I/O 무의존):
//   - `ContributionService` 는 mock(`useValue`) — DB round-trip 없이 controller ↔ collector
...
//     fail 분기는 mock 예외(500) 또는 assessmentId 누락(400) 으로 도달(errorRate 위반) —
...
//   - `@Get(":id")` findOne·POST/DELETE 등 다른 route perf 배선 — 본 spec 은 조회
//     findByAssessment 하나에 집중(추가 route 는 후속 slice).
...
// mock ContributionService — controller 가 주입받는 4 primitive 중 조회(findByAssessment)만
// harness 에 필요. 각 test 가 mockResolvedValue / mockRejectedValue 로 응답을 제어해
// endpoint status(200 / 500)를 결정론적으로 만든다. (assessmentId 누락 400 은 controller
...
describe("S2 조회 latency perf-spec — ContributionController 배선 (REQ-048)", () => {
  ...
  // assessmentId query 를 아예 누락한 요청 — controller 의 400 BadRequestException 분기 도달.
  const missingAssessmentIdRequest: RequestFn = async () => {
  ...
  describe("happy path — mock service 정상 응답(200)", () => {
    it("정상 200 응답 N회 → total===N, failures===0, samplesMs.length===N, assertS2Threshold pass", async () => {
      // mock 이 기여 배열 반환 → controller 가 200 + JSON array.
      service.findByAssessment.mockResolvedValue([
        { id: "c-1", assessmentId: "a-1", sourceType: "commit" },
      ]);
      const N = 5;
      ...
      expect(service.findByAssessment).toHaveBeenCalledTimes(N);
    });
  });
  ...
  describe("error path — mock service 예외 → endpoint non-2xx(500)", () => {
    it("service 예외로 500 응답 N회 → 전부 failures, assertS2Threshold pass===false + errorRate 사유", async () => {
      ...
      service.findByAssessment.mockRejectedValue(
  ...
  describe("negative cases 충분 cover", () => {
    // (a) assessmentId query 누락 → controller 의 BadRequestException(400) 분기 도달.
    //     harness 가 이 non-2xx(400) 를 failures 로 정확히 분류하는지 검증(본 spec 의
    //     핵심 추가 검증 — assessment.controller.ts personId 누락 mirror).
    it("(a) assessmentId 누락 요청 → controller 400 을 collector 가 failures 로 분류, pass===false", async () => {
      const N = 3;

      const result = await collectLatencySamples(missingAssessmentIdRequest, N);

      // 400 은 non-2xx → 전부 failure, 성공 표본 0.
      expect(result.total).toBe(N);
      expect(result.failures).toBe(N);
      expect(result.samplesMs).toHaveLength(0);
      // controller-layer 에서 막히므로 service.findByAssessment 는 호출조차 안 됨.
    ...
    //     매칭 Contribution 0 은 컬렉션 조회의 정상 결과(404 아님).
    ...
    it("(c) mixed — 4회 중 1회만 500 → failures===1 부분 집계 정확, samplesMs===3", async () => {
      ...
      service.findByAssessment.mockImplementation(async () => {
        call += 1;
        if (call === 2) {
    ...
    it("(d) iterations===1 경계 → 단일 호출로도 harness 정상 동작", async () => {
      service.findByAssessment.mockResolvedValue([{ id: "c-1" }]);
      ...
      expect(assertS2Threshold(result).pass).toBe(true);
      expect(service.findByAssessment).toHaveBeenCalledTimes(1);
    });
  });
});
