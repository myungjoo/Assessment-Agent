...
describe("S2 measure→confirm-or-compare perf-spec — 실 DB baseline 확정·비교 (GET /api/persons, guard 미부착 + soft-delete 필터, REQ-048)", () => {
  ...
  // 처리를 두지 않는다(이중 원복 0). 토글 off 기본 상태에서는 `fs` 조회 0 · write 0 이라 기존
  // `perf test` step 동작이 그대로고, 회귀는 관찰만 하며 exit code 를 바꾸지 않는다. 잘못된
  // options(non-object · non-function)로 인한 **등록 시점 TypeError** 국면은 factory colocated
