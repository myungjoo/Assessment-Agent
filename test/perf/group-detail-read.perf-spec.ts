// group-detail-read.perf-spec.ts — S2 조회 latency harness 의 *열여섯 번째 실 perf-spec*
// 이자 **두 번째 path-param detail(:id) read** 배선(첫 entity(Group) 상세 조회 경로).
// (T-0845, load-resilience-test-plan §5 follow-up #16 / REQ-048, 조회 p95 < 3s)
//
// 목적: T-0828(percentile/summarizeLatency/errorRate 순수 primitive) + T-0829
// (collectLatencySamples/assertS2Threshold 순수 orchestration)가 신설하고 T-0830~T-0844
// 가 15 개 조회 endpoint(list/query/self-read 14 + summary :id detail 1)에 배선한
// collector 를, `GroupController` 의 **detail** endpoint 인 `GET /api/groups/:id`
// (`findById` → `service.findById(id)` — 단일 Group 상세, row 부재 시 service 가
// `NotFoundException` throw → 404 자동 mapping)에 배선한다. 직전 T-0844(summary :id)
// 가 **첫 path-param `:id` detail read** 였고, 본 spec 은 **두 번째 detail(:id) read**
// 이자 **첫 entity(Group) 상세 조회** 경로라, harness 가 단일 상세 조회(404 분기 있는
// detail) 경로에서 재사용됨을 한 slice 더 실증한다.
// jest-perf.json(`testRegex: test/perf/.*\.perf-spec\.ts$`)에 매칭돼 `pnpm test:perf`
// 로만 실행되며(기본 `pnpm test` 는 `.spec.ts$` 만 매칭 → picking 0), 앞선 15 perf-spec
// 과 함께 열여섯 다 실행된다.
//
// 앞선 spec 과의 차이(본 spec 고유 특성): (1) 배선 대상이 group-read(list, T-0834) 의
// `findAll`(`GET /api/groups`) → `findById`(`GET /api/groups/:id`) 로, list read 에서
// detail(:id) read 로 바뀐다. (2) summary-detail-read(T-0844) 와 배선 형태(findById(:id))
// 는 같으나, `GroupController` 는 `@UseGuards`/`@Roles` 를 부착하지 않는다
// (group.controller.ts L51 — auth credential 미박제 정책). 따라서 본 spec 은
// summary-detail-read 와 달리 `overrideGuard` 없이 controller 를 순수 부트스트랩한다
// (group-read(list) 와 동일 패턴 — 적용해도 no-op). `GroupController.findById` 는
// `@Param("id")` 로 받은 id 를 `service.findById(id)` 로 raw forward 하고(controller
// 자체 분기 없음), row 존재 시 200(단일 Group), row 부재 시 service 가
// `NotFoundException`(404)을 던진다. non-2xx 분류 실증은 mocked `findById` 가
// `NotFoundException`(404)/일반 예외(500)를 던져 endpoint 가 404/500 을 반환하는 error
// path 로 커버한다.
//
// 결정론 전략 (Acceptance — 실 DB·실 LLM·외부 I/O 무의존):
//   - `GroupService` 는 mock(`useValue`) — DB round-trip 없이 controller ↔ collector
...
//   - `GET /api/groups/:id/persons`(findPersons) 등 sub-resource read 배선 — 별도 후속 slice.
...
// mock GroupService — controller 가 주입받는 메서드 중 detail 경로가 실제로 호출하는
// 것은 `findById` 뿐. 각 test 가 mockResolvedValue / mockRejectedValue 로 응답을 제어해
// endpoint status(200 / 404 / 500)를 결정론적으로 만든다. (GroupController 는 guard
// 미적용이라 인증/인가 분기 노이즈가 없다.)
type MockGroupService = {
  findById: jest.Mock;
};

describe("S2 조회 latency perf-spec — GroupController detail(:id) 배선 (REQ-048)", () => {
  ...
  // 단일 Group 상세 조회 endpoint(`GET /api/groups/:id`)를 1회 호출하고 collector 가
  ...
  describe("happy path — mock service 정상 응답(200)", () => {
    it("정상 200 응답 N회 → total===N, failures===0, samplesMs.length===N, assertS2Threshold pass", async () => {
      // mock 이 단일 Group 반환 → controller 가 200 + JSON object.
      service.findById.mockResolvedValue({ id: "g-1", name: "백엔드팀" });
      const N = 5;
