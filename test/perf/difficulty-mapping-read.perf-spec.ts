// difficulty-mapping-read.perf-spec.ts — S2 조회 latency harness 의 *열 번째 실 perf-spec*.
// (T-0839, load-resilience-test-plan §5 follow-up #10 / REQ-048, 조회 p95 < 3s)
//
...
// SummaryController 에, T-0831~T-0838 이 순서대로 Assessment·Contribution·Person·
// Group·Part·User·PermissionDeniedRecord·LlmProviderConfigController 에 배선한
// collector 를, **열 번째 조회 endpoint** 인 `DifficultyMappingController` 의
// `GET /api/llm/difficulty-mappings`(findAll → `findAllMappings` — 3 고정 슬롯
// (easy/medium/hard) 난이도↔model 매핑 목록 조회, REQ-096 Admin 가시성 / REQ-048 조회
// back)에 배선한다. harness 가 요약·평가·기여·인원·그룹·파트·사용자·권한거부·LLM설정·
// 난이도매핑 10 read 경로 전반에 재사용됨을 실증한다. jest-perf.json(`testRegex:
// test/perf/.*\.perf-spec\.ts$`)에 매칭돼 `pnpm test:perf` 로만 실행되며(기본
// `pnpm test` 는 `.spec.ts$` 만 매칭 → picking 0), 앞선 아홉 perf-spec 과 함께 열 다
// 실행된다.
//
// 앞선 slice 와의 차이(본 spec 고유 특성): 본 endpoint 는
// `@UseGuards(JwtAuthGuard, RolesGuard)` + `@Roles("Admin")` 로 **가드가 부착된 Admin
// list read** 다. 따라서 본 perf-spec 은 `llm-provider-config-read.perf-spec.ts`(가드 부착 +
// `overrideGuard` 사용, T-0838) 패턴을 mirror 해 `overrideGuard(JwtAuthGuard)`·
// `overrideGuard(RolesGuard)` 로 가드를 무력화하고 controller 를 결정론적으로
// 부트스트랩한다. findAll 핸들러는 query param 도 `@CurrentUser()` actor 도 읽지 않고
// mocked `service.findAllMappings()` 를 raw forward(repository.findMany 순서 그대로
// 반환, controller 자체 분기 없음)하므로 req.user 박제가 불요하다(canActivate 가 true 만
// 반환하면 충분). 난이도↔model 매핑 정책·isDifficulty 검증·RBAC escalation 정책 검증은
// out of scope — controller/service spec 책임, 본 spec 은 latency 배선만.
//
// non-2xx 분류 실증은 mocked `service.findAllMappings` 이 예외를 던져 endpoint 가 500
// (InternalServerError) 을 반환하는 error path 로 커버한다.
//
// 결정론 전략 (Acceptance — 실 DB·실 LLM·외부 I/O 무의존):
//   - `DifficultyMappingService` 는 mock(`useValue`) — DB round-trip·실 LLM 없이
...
//   - 난이도↔model 매핑 정책·isDifficulty 검증·RBAC escalation(Admin/non-Admin) 정책
//     검증 — 본 spec 은 latency 배선만(정책은 기존 controller/service spec).
//   - PATCH /api/llm/difficulty-mappings/:difficulty(assign — write) endpoint 배선 —
//     본 spec 은 list read(findAllMappings) 만 배선. write 경로는 필요 시 별도 slice.
...
import { DifficultyMappingController } from "../../src/llm/difficulty-mapping.controller";
...
// mock DifficultyMappingService — controller 가 주입받는 메서드 중 목록 조회
// (findAllMappings) 만 harness 에 필요. 각 test 가 mockResolvedValue / mockRejectedValue
// 로 응답을 제어해 endpoint status(200 / 500)를 결정론적으로 만든다. assignProviderConfig
// / resolveModel 은 본 조회 경로가 호출하지 않지만, service shape 정합을 위해 jest.fn 으로
// 함께 둔다(호출 안 됨).
type MockDifficultyMappingService = {
  findAllMappings: jest.Mock;
  assignProviderConfig: jest.Mock;
  resolveModel: jest.Mock;
};

// 결정론적 DifficultyMapping row 한 건 — mock findAllMappings 의 정상 반환값. 슬롯
// (difficulty) + FK(llmProviderConfigId) shape 만 흉내(실제 Prisma row 가 아니어도 JSON
// 직렬화만 되면 latency 배선 측정에 충분).
const SAMPLE_MAPPING = {
  id: "dm-easy",
  difficulty: "easy",
  llmProviderConfigId: "llm-1",
  createdAt: new Date(),
  updatedAt: new Date(),
};

describe("S2 조회 latency perf-spec — DifficultyMappingController 배선 (REQ-048)", () => {
  let app: INestApplication;
  let service: MockDifficultyMappingService;
  ...
  beforeAll(async () => {
    ...
    const moduleRef: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: DifficultyMappingService, useValue: service }],
    })
  ...
  // 난이도↔model 매핑 목록 조회를 1회 호출하고 collector 가 소비할 { status } 를 반환하는
  ...
  describe("happy path — mock service 정상 응답(200)", () => {
    it("정상 200 응답 N회 → total===N, failures===0, samplesMs.length===N, assertS2Threshold pass", async () => {
      // mock 이 mapping 배열 반환 → controller 가 200 + JSON array.
      service.findAllMappings.mockResolvedValue([SAMPLE_MAPPING]);
      const N = 5;
      ...
      expect(service.findAllMappings).toHaveBeenCalledTimes(N);
    });
  });
  ...
  describe("error path — mock service 예외 → endpoint non-2xx(500)", () => {
    it("service 예외로 500 응답 N회 → 전부 failures, assertS2Threshold pass===false + errorRate 사유", async () => {
      ...
      service.findAllMappings.mockRejectedValue(
  ...
  describe("negative cases 충분 cover", () => {
    ...
    //     슬롯 seed 전 빈 배열은 목록 조회의 정상 결과(404 아님 — raw forward).
    it("(a) service 가 빈 배열([]) 반환(슬롯 seed 전) → 200 성공 수집, pass===true", async () => {
      service.findAllMappings.mockResolvedValue([]);
      const N = 3;
    ...
    it("(b) mixed — 4회 중 1회만 500 → failures===1 부분 집계 정확, samplesMs===3", async () => {
      ...
      service.findAllMappings.mockImplementation(async () => {
        call += 1;
        if (call === 2) {
    ...
    it("(c) iterations===1 경계 → 단일 호출로도 harness 정상 동작", async () => {
      ...
      expect(assertS2Threshold(result).pass).toBe(true);
      expect(service.findAllMappings).toHaveBeenCalledTimes(1);
    });

    // (d) 다건 결과 — findAllMappings 이 3 슬롯(easy/medium/hard) mapping 배열을
    //     반환해도 latency 수집이 정상 동작(200, failures===0)함을 실증(단건·빈 배열 외
    //     다건 경로 커버).
    it("(d) service 가 3 슬롯 배열 반환 → 200 성공 수집, failures===0, pass===true", async () => {
      service.findAllMappings.mockResolvedValue([
        SAMPLE_MAPPING,
        { ...SAMPLE_MAPPING, id: "dm-medium", difficulty: "medium" },
        { ...SAMPLE_MAPPING, id: "dm-hard", difficulty: "hard" },
      ]);
      const N = 4;
