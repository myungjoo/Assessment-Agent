...
| 게이트 (c) 자체 점검 fail | PR comment 로 self-finding post (header: `> Committer self-check — integrator agent of Assessment-Agent`) — `gh pr comment` ↔ `mcp__github__add_issue_comment(issue_number, body)`. STATUS=ANOTHER_ROUND. |
